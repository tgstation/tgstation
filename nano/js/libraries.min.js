...
(function(y,f,k){if(!f){throw"requires jQuery or JsRender"}if(f.observable){return}var c="v1.0.0-alpha",w,n,H,t,A=f.event.special,s=f.views?f.views.sub:{},l=1,G=[].splice,j=[].concat,p=f.isArray,C=f.expando,q="object",E=s.propChng=s.propChng||"propertyChange",e=s.arrChng=s.arrChng||"arrayChange",x=s._cbBnds=s._cbBnds||{},g=E+".observe",u=f.isFunction,i=1,r=1,h=f.hasData;function v(I){return p(I)?new a(I):new b(I)}function b(I){this._data=I;return this}function a(I){this._data=I;return this}function B(I){return p(I)?[I]:I}function m(I){if(typeof I!=="number"){throw"Invalid index."}}function d(P,J){P=p(P)?P:[P];var N,O,L=J,M=L,I=P.length,K=[];for(N=0;N<I;N++){O=P[N];if(u(O)){G.apply(K,[K.length,1].concat(d(O.call(J,J),J)));continue}else{if(""+O!==O){J=M=O;if(M!==L){K.push(L=M)}continue}}if(M!==L){K.push(L=M)}K.push(O)}return K}function F(J,L){var I,K;for(I in J){K=true;break}if(!K){delete x[L]}}function z(K,J){if(!(K.data&&K.data.off)){var L=J.oldValue,I=K.data;if(K.type===e){I.cb.array(K,J)}else{if(I.prop==="*"||I.prop===J.path){if(typeof L===q){D(B(L),I.path,I.cb)}if(typeof(L=J.value)===q){o(B(L),I.path,I.cb)}I.cb(K,J)}}}}function o(){function S(ak,aj,al,ao){var am=h(ah),an=B(ah);w=0;if(I||ao){if(am){f(an).off(ak,z);if(w){delete w[f.data(ah,"obId")]}}}else{if(J=am&&f._data(ah)){J=J&&J.events;J=J&&J[al?e:E];K=J&&J.length;while(K--){if((ag=J[K].data)&&ag.cb===M){if(al){return}else{if(aj==="*"&&ag.prop!==aj){f(ah).off(ak+"."+ag.prop,z);if(w){delete w[f.data(ah,"obId")]}}}}}}f(an).on(ak,null,al?{cb:M}:{path:aj,prop:L,cb:M},z);if(ae){ae[f.data(ah,"obId")||f.data(ah,"obId",i++)]=ah}}}function ac(am,al,ak){am._ob=N(am,P);var aj=P;return function(){var ao=am._ob,an=al.length;if(typeof ao===q){aa(ao,true);if(an){D(B(ao),al,M,N)}}ao=am._ob=N(am,aj);if(typeof ao===q){aa(ao);if(an){o(B(ao),al,M,N,aj)}}}}function aa(aj,ak){if(M&&M.array&&p(aj)){var al=ah;ah=aj;S(e+".observe.obs"+M._bnd,k,true,ak);ah=al}}var Z,T,L,R,O,ah,I,M,V,K,ag,J,N,Q,ae,af,ad,ab=1,ai=g,U=j.apply([],arguments),Y=U.pop(),P=U[0],X=""+P!==P?U.shift():k,W=U.length;P=X;if(u(Y)){M=Y}else{if(Y===true){I=Y}else{if(Y){P=Y;ab=0}}Y=U[W-1];if(W&&Y===k||u(Y)){M=U.pop();W--}}if(u(U[W-1])){N=M;M=U.pop();W--}ai+=I?(M?".obs"+M._bnd:""):".obs"+(V=M._bnd=M._bnd||r++);if(I&&W===0&&X){f(X).off(g,z)}if(!I){ae=x[V]=x[V]||{}}af=0;for(Z=0;Z<W;Z++){R=U[Z];aa(ah,I);ah=X;if(""+R===R){T=R.split("^");if(T[1]){af=T[0].split(".").length;R=T.join(".");af=R.split(".").length-af}if(N&&(Q=N(R,X))){W+=Q.length-1;G.apply(U,[Z--,1].concat(Q));continue}T=R.split(".")}else{if(ab&&!u(R)){if(R._jsvOb){if(!I){R._cb=ad=ac(R,U.slice(Z+1));R._rt=P;ad._bnd=M._bnd}o(R._rt,U.slice(0,Z),R._cb,N,I);R=R._ob}ah=R}X=R;T=[X]}while(ah&&typeof ah==="object"&&(L=T.shift())!==k){if(""+L===L){if(L===""){continue}if((T.length<af+1)&&!ah.nodeType){if(!I&&(J=h(ah)&&f._data(ah))){J=J.events;J=J&&J.propertyChange;K=J&&J.length;while(K--){ag=J[K].data;if(ag&&ag.cb===M&&((ag.prop===L&&ag.path===T.join("."))||ag.prop==="*")){break}}if(K>-1){ah=ah[L];continue}}if(L==="*"){if(u(ah)){if(O=ah.depends){o(O,M,I||P)}}else{S(ai,L)}break}else{if(L&&!(u(O=ah[L])&&O.depends)){S(ai+"."+L,T.join("."))}}}L=L?ah[L]:ah}if(u(L)){if(O=L.depends){o(ah,d(O,ah),M,N,I||B(P))}break}ah=L}}aa(ah,I);if(V){F(ae,V)}return{cbId:V,bnd:ae,leaf:ah}}function D(){[].push.call(arguments,true);return o.apply(this,arguments)}f.observable=v;v.Object=b;v.Array=a;v.observe=o;v.unobserve=D;b.prototype={_data:null,data:function(){return this._data},observe:function(I,J){return o(this._data,I,J)},unobserve:function(I,J){return D(this._data,I,J)},setProperty:function(Q,N,L){var M,O,J,K,P=this,I=P._data;Q=Q||"";if(I){if(p(Q)){O=Q.length;while(O--){J=Q[O];P.setProperty(J.name,J.value,L===k||L)}}else{if(""+Q!==Q){for(O in Q){P.setProperty(O,Q[O],N)}}else{if(Q.indexOf(C)<0){K=Q.split(".");while(I&&K.length>1){I=I[K.shift()]}P._setProperty(I,K.join("."),N,L)}}}}return P},_setProperty:function(J,N,M,L){var O,I,K=N?J[N]:J;if(u(K)){if(K.set){I=K;O=K.set===true?K:K.set;K=K.call(J)}}if(K!==M||L&&K!=M){if(!(K instanceof Date)||K>M||K<M){if(O){O.call(J,M);M=I.call(J)}else{J[N]=M}this._trigger(J,{path:N,value:M,oldValue:K})}}},_trigger:function(J,I){f(J).triggerHandler(E,I)}};a.prototype={_data:null,data:function(){return this._data},insert:function(I,J){m(I);if(arguments.length>1){J=p(J)?J:[J];if(J.length){this._insert(I,J)}}return this},_insert:function(I,J){t=this._data;H=t.length;G.apply(t,[I,0].concat(J));this._trigger({change:"insert",index:I,items:J})},remove:function(J,K){m(J);K=(K===k||K===null)?1:K;if(K&&J>-1){var I=this._data.slice(J,J+K);K=I.length;if(K){this._remove(J,K,I)}}return this},_remove:function(J,K,I){t=this._data;H=t.length;t.splice(J,K);this._trigger({change:"remove",index:J,items:I})},move:function(L,K,I){m(L);m(K);I=(I===k||I===null)?1:I;if(I){var J=this._data.slice(L,L+I);this._move(L,K,I,J)}return this},_move:function(L,K,I,J){t=this._data;H=t.length;t.splice(L,I);t.splice.apply(t,[K,0].concat(J));this._trigger({change:"move",oldIndex:L,index:K,items:J})},refresh:function(I){var J=this._data.slice(0);this._refresh(J,I);return this},_refresh:function(J,I){t=this._data;H=t.length;G.apply(t,[0,t.length].concat(I));this._trigger({change:"refresh",oldItems:J})},_trigger:function(I){var K=t.length,J=f([t]);J.triggerHandler(e,I);if(K!==H){J.triggerHandler(E,{path:"length",value:K,oldValue:H})}}};A[E]=A[e]={remove:function(I){if((I=I.data)&&(I.off=1,I=I.cb)){w=x[n=I._bnd]}},teardown:function(I){if(w){delete w[f.data(this,"obId")];F(w,n)}}}})(this,this.jQuery||this.jsviews);
/*! JsViews v1.0.0-alpha: http://github.com/BorisMoore/jsviews and http://jsviews.com/jsviews */
(function(r,az,u){if(!az){throw"requires jQuery"}if(!az.views){throw"requires JsRender"}if(!az.observable){throw"requires jquery.observable"}if(az.link){return}var g="v1.0.0-alpha",V,aG,aP,e,au,at,M,L,ax,b,Q=r.document,h=az.views,K=h.sub,m=h.settings,n=K.extend,O=K.View(u,"top"),d=az.isFunction,l=h.templates,S=az.observable,aO=S.observe,af="data-jsv",ab=m.linkAttr||"data-link",q=m.propChng=m.propChng||"propertyChange",Z=K.arrChng=K.arrChng||"arrayChange",aH=K._cbBnds=K._cbBnds||{},P="change.jsv",y="onBeforeChange",x="onAfterChange",aB="onAfterCreate",c='"><\/script>',aE='<script type="jsv',I="script,["+af+"]",am=I+",["+ab+"]",aK={value:"val",input:"val",html:"html",text:"text"},J={from:{fromAttr:"value"},to:{toAttr:"value"}},ag=az.cleanData,ac=m.delimiters,ap=K.error,F=K.syntaxError,A=/<(?!script)(\w+)(?:[^>]*(on\w+)\s*=)?[^>]*>/,H=Q.createDocumentFragment(),z=Q.querySelector,ay={ol:1,ul:1,table:1,tbody:1,thead:1,tfoot:1,tr:1,colgroup:1,dl:1,select:1,optgroup:1},aI={tr:"table"},R=m.wrapMap={option:[1,"<select multiple='multiple'>","</select>"],legend:[1,"<fieldset>","</fieldset>"],thead:[1,"<table>","</table>"],tr:[2,"<table><tbody>","</tbody></table>"],td:[3,"<table><tbody><tr>","</tr></tbody></table>"],col:[2,"<table><tbody></tbody><colgroup>","</colgroup></table>"],area:[1,"<map>","</map>"],svg:[1,"<svg>","</svg>"],div:[1,"x<div>","</div>"]},E={br:1,img:1,input:1,hr:1,area:1,base:1,col:1,link:1,meta:1,command:1,embed:1,keygen:1,param:1,source:1,track:1,wbr:1},v={},ak={0:O},B={},W=1,aD=/^#(view\.?)?/,X=/(^|(\/>)|(<\/\w+>)|>|)(\s*)([#\/]\d+[_^])`(\s*)(<\w+(?=[\s\/>]))?|\s*(?:(<\w+(?=[\s\/>]))|(<\/\w+>)(\s*)|(\/>)\s*)/g,U=/(#)()(\d+)(_)/g,aC=/(#)()(\d+)([_^])/g,aQ=/(?:(#)|(\/))(\d+)(_)/g,Y=/(#)()(\d+)(\^)/g,T=/(?:(#)|(\/))(\d+)([_^])([-+@\d]+)?/g;R.optgroup=R.option;R.tbody=R.tfoot=R.colgroup=R.caption=R.thead;R.th=R.td;function av(a7){var a6,aU,aX,aW,a0,aR,aT,a9,a5,aZ,a3,a8,a4,a1,aV,aY,ba,a2=a7.target,aS=a2._jsvBnd;if(aS){a8=aS.slice(1).split("&");a4=a8.length;while(a4--){if(a3=B[a8[a4]]){if(aS=a3.to){aW=a3.linkCtx;aZ=aW.view;ba=aW.tag;a5=az(a2);aV=aZ.hlp(y,aW.ctx);aY=aZ.hlp(x,aW.ctx);aX=an(a2);a6=aK[aX];a0=d(aX)?aX(a2):a6?a5[a6]():a5.attr(aX);aT=aS[1];aS=aS[0];if(aT){if(d(aT)){aR=aT}else{aR=aZ.tmpl.converters;aR=aR&&aR[aT]||h.converters[aT]}}if(aR){a0=aR.call(ba,a0)}a1=aZ.linkCtx;aZ.linkCtx=aW;if((!aV||!(aU=aV.call(aZ,a7,a0)===false))&&(!ba||!ba.onBeforeChange||!(aU=ba.onBeforeChange(a7,a0)===false))&&a0!==u){if(ba&&ba.onChange){a0=ba.onChange(a0)}a9=aS[0];if(a0!==u&&a9){a9=a9._jsvOb?a9._ob:a9;if(ba){ba._.chging=true}S(a9).setProperty(aS[2]||aS[1],a0);if(ba){delete ba._.chging}if(aY){aY.call(aW,a7)}}}aZ.linkCtx=a1;if(aU){a7.stopImmediatePropagation()}}}}}}function ao(bc,a0,aR){var ba,bb,aW,a5,a1,be,a8,aV,a6,aU=this,a9=aU.data,bd=aU.elem,a2=aU.convert,aY="attr",a4=bd.parentNode,aZ=a4,aT=az(bd),a3=aU.view,aS=a3.ctx,a7=a3.linkCtx,aX=a3.hlp(y);a3.linkCtx=aU;a3.ctx=aU.ctx;if(a4&&(!aX||!(a0&&aX.call(aU,bc,a0)===false))&&!(a0&&bc.data.prop!=="*"&&bc.data.prop!==a0.path)){if(a0){aU.eventArgs=a0}if(!bc||a0||aU._initVal){delete aU._initVal;a5=aR.call(a3.tmpl,a9,a3,h);ba=aU.attr||an(bd,true,a2!==u);if(be=aU.tag){if(be._.chging||a0&&be.onUpdate&&be.onUpdate(bc,a0,a5)===false||ba==="none"){aq(aU,a9,bd);a3.linkCtx=a7;return}a5=be.tagName.slice(-1)===":"?h._cnvt(be.tagName.slice(0,-1),a3,a5):h._tag(be,a3,a3.tmpl,a5)}else{if(aR._ctxs){a2=a2===""?"true":a2;a5=a2?h._cnvt(a2,a3,a5):h._tag(aR._ctxs,a3,a3.tmpl,a5);be=a3._.tag;ba=aU.attr||ba}}if(be){be.parentElem=(aU.expr||be._elCnt)?bd:bd.parentNode;a8=be._prv;aV=be._nxt;ba=be.attr||ba;be.refresh=aw}if(d(a5)){ap(aU.expr+": missing parens")}if(ba==="visible"){ba="css-display";a5=a5?s(bd):"none"}if(a1=ba.lastIndexOf("css-",0)===0&&ba.substr(4)){if(aW=az.style(bd,a1)!==a5){az.style(bd,a1,a5)}}else{if(ba!=="link"){if(ba==="value"){if(bd.type==="checkbox"){a5=a5&&a5!=="false";aY="prop";ba="checked"}}else{if(ba==="radio"){if(bd.value===(""+a5)){a5=true;aY="prop";ba="checked"}else{aq(aU,a9,bd);a3.linkCtx=a7;return}}else{if(ba==="selected"||ba==="disabled"||ba==="multiple"||ba==="readlonly"){a5=(a5&&a5!=="false")?ba:null}}}bb=aK[ba];if(bb){if(aW=be||aT[bb]()!==a5){if(ba==="html"){if(be){a6=be._.inline;be.refresh(a5);if(!a6&&be._.inline){a3.linkCtx=a7;return}}else{aT.empty();aZ=bd;a3.link(a9,aZ,a8,aV,a5,be&&{tag:be._tgId})}}else{if(ba==="text"&&!bd.children[0]){if(bd.textContent!==u){bd.textContent=a5}else{bd.innerText=a5===null?"":a5}}else{aT[bb](a5)}}}}else{if(aW=aT[aY](ba)!=a5){aT[aY](ba,a5===u&&aY==="attr"?null:a5)}}}}if(a0&&aW&&(aX=a3.hlp(x))){aX.call(aU,bc,a0)}}aq(aU,a9,bd);a3.linkCtx=a7;a3.ctx=aS}}function N(aW,aU){var aT=this,aS=aT.hlp(y),aY=aT.hlp(x);if(!aS||aS.call(aW,aU)!==false){if(aU){var aX=aU.change,aV=aU.index,aR=aU.items;switch(aX){case"insert":aT.addViews(aV,aR);break;case"remove":aT.removeViews(aV,aR.length);break;case"move":aT.refresh();break;case"refresh":aT.refresh();break}}if(aY){aY.call(this,aW,aU)}}}function s(aT){var aS,aV,aU=r.getComputedStyle,aR=(aT.currentStyle||aU.call(r,aT,"")).display;if(aR==="none"&&!(aR=v[aV=aT.nodeName])){aS=Q.createElement(aV);Q.body.appendChild(aS);aR=(aU?aU.call(r,aS,""):aS.currentStyle).display;v[aV]=aR;Q.body.removeChild(aS)}return aR}function ar(aR){var aT,aV,aU=aR.data,aS=aR._.bnd;if(!aR._.useKey&&aS){if(aV=aR._.bndArr){az([aV[1]]).off(Z,aV[0]);aR._.bndArr=u}if(aS!==!!aS&&aS._.inline){if(aU){aS._.arrVws[aR._.id]=aR}else{delete aS._.arrVws[aR._.id]}}else{if(aU){aT=function(aW){if(!(aW.data&&aW.data.off)){N.apply(aR,arguments)}};az([aU]).on(Z,aT);aR._.bndArr=[aT,aU]}}}}function an(aS,aV,aT){var aU=aS.nodeName.toLowerCase(),aR=m.merge[aU];return aR?(aV?((aU==="input"&&aS.type==="radio")?"radio":aR.to.toAttr):aR.from.fromAttr):aV?aT?"text":"html":""}function k(a2,a0,a3,a6,aW,aS,a1){var aY,a4,aZ,a5,aT,aX=a2.parentElem,aR=a2._prv,aV=a2._nxt,aU=a2._elCnt;if(aR&&aR.parentNode!==aX){ap("Missing parentNode")}if(a1){a5=a2.nodes();if(aU&&aR&&aR!==aV){aM(aR,aV,aX,a2._.id,"_",true)}a2.removeViews(u,u,true);a4=aV;if(aU){aR=aR?aR.previousSibling:aV?aV.previousSibling:aX.lastChild}az(a5).remove();for(aT in a2._.bnds){aa(aT)}}else{if(a0){aZ=a6[a0-1];if(!aZ){return false}aR=aZ._nxt}if(aU){a4=aR;aR=a4?a4.previousSibling:aX.lastChild}else{a4=aR.nextSibling}}aY=a3.render(aW,aS,a2,a1||a0,a2._.useKey&&a1,true);a2.link(aW,aX,aR,a4,aY,aZ)}function G(aV,aU,aT){var aW,aR,aS;if(aT){aS="^`";aR=aU._.tag||{_:{inline:true,bnd:aT},tagCtx:{view:aU},flow:true};aW=aR._tgId;aR.refresh=aw;if(!aW){B[aW=W++]=aR;aR._tgId=""+aW}}else{aS="_`";ak[aW=aU._.id]=aU}return"#"+aW+aS+(aV===u?"":aV)+"/"+aW+aS}function aq(aW,aR,aX){var aY,aV,a0=aW.linkedElem,aT=aW.convertBack,a1=aW.tag,aS=[],aU=aW._bndId||""+W++,aZ=aW._hdlr;delete aW._bndId;if(a1){aS=a1.depends||aS;aS=d(aS)?a1.depends(a1):aS}if(!aW._depends||(""+aW._depends!==""+aS)){if(aW._depends){aO(aR,aW._depends,aZ,true)}aY=aO(az.isArray(aR)?[aR]:aR,aW.fn.paths||aW.fn,aS,aZ,aW._ctxCb);aY.elem=aX;aY.linkCtx=aW;aY._tgId=aU;aX._jsvBnd=aX._jsvBnd||"";aX._jsvBnd+="&"+aU;if(a0){aY.to=[[],aT];aV=a0.length;while(aV--){a0[aV]._jsvBnd=aX._jsvBnd}}aW._depends=aS;aW.view._.bnds[aU]=aU;B[aU]=aY;if(a0||aT!==u){a(aY,aT)}}}function i(aW,aV,aT,aU,aS,aR){return D(this,aW,aV,aT,aU,aS,aR)}function D(a8,aR,a4,aS,aV,a1,aU){if(a8&&aR){aR=aR.jquery?aR:az(aR);if(!aG){aG=Q.body;az(aG).on(P,av)}var a6,a5,aY,a3,aZ,a7,a0,aT,a9,aW=G,aX=aS&&aS.target==="replace",a2=aR.length;while(a2--){a0=aR[a2];if(""+a8===a8){aZ=aP(a0);aT=aZ.ctx;aZ.ctx=aS;aj(a8,a0,aP(a0),a4);aZ.ctx=aT}else{aV=aV||aP(a0);if(a8.markup!==u){if(aV.link===false){aS=aS||{};aS.link=aW=false}if(aX){a7=a0.parentNode}aY=a8.render(a4,aS,aV,u,u,aW);if(a7){a1=a0.previousSibling;aU=a0.nextSibling;az.cleanData([a0],true);a7.removeChild(a0);a0=a7}else{a1=aU=u;az(a0).empty()}}else{if(a8!==true){break}}if(a0._dfr&&!aU){a3=j(a0._dfr,true,U);for(a6=0,a5=a3.length;a6<a5;a6++){aZ=a3[a6];if((aZ=ak[aZ.id])&&aZ.data!==u){aZ.parent.removeViews(aZ._.key,u,true)}}a0._dfr=""}a9=aV.data;aT=aV.ctx;aV.data=a4;aV.ctx=aS;aV.link(a4,a0,a1,aU,aY);aV.data=a9;aV.ctx=aT}}}return aR}function aL(bh,a4,ba,bj,bk,bM){function a6(bY,bS,bQ,bU,bV,bO,b0,bZ,bX,bP,bT,bW){var bR="";bB=bZ||bX||"";bU=bU||bQ||bP||bW;if(bU){if(b&&(bQ||bW)&&!E[bv]){F("'<"+bv+"... />' in:\n"+bk)}bb=bz;bv=bt.shift();bz=ay[bv];if(bb){aW+=bN;bN="";if(!bz){bR=(bP||"")+aE+"@"+aW+c+(bT||"");aW=aT.shift()}else{aW+="-"}}}if(bz){if(bO){bN+=bO}else{bS=(bP||bW||"")}if(bB){bS+=bB;if(bN){bS+=" "+af+'="'+bN+'"';bN=""}}}else{bS=bO?(bS+bR+bV+aE+bO+c+b0+bB):bR||bY}if(bB){bt.unshift(bv);bv=bB.slice(1);if(bt[0]===aI[bv]){ap('"'+bv+'" has incorrect parent tag')}if((bz=ay[bv])&&!bb){aT.unshift(aW);aW=""}bb=bz;if(aW&&bz){aW+="+"}}return bS}function bs(bO,bR){var bW,bX,bV,bS,bP,bT,bU,bQ=[];if(bO){bc=bO.length;if(bO.tokens.charAt(0)==="@"){bR=bC.previousSibling;bC.parentNode.removeChild(bC);bC=null}bc=bO.length;while(bc--){aU=bO[bc];bV=aU.ch;if(bW=aU.path){bE=bW.length-1;while(bX=bW.charAt(bE--)){if(bX==="+"){if(bW.charAt(bE)==="-"){bE--;bR=bR.previousSibling}else{bR=bR.parentNode}}else{bR=bR.lastChild}}}if(bV==="^"){if(bB=B[bP=aU.id]){bU=bR&&(!bC||bC.parentNode!==bR);if(!bC||bU){bB.parentElem=bR}if(aU.elCnt){if(aU.open){if(bR){bR._dfr="#"+bP+bV+(bR._dfr||"")}}else{if(bU){bR._dfr="/"+bP+bV+(bR._dfr||"")}}}bQ.push([bU?null:bC,aU])}}else{if(aR=ak[bP=aU.id]){if(!aR.link){aR.parentElem=bR||bC&&bC.parentNode||a4;n(aR,V);aR._.onRender=G;aR._.onArrayChange=N;ar(aR)}bS=aR.parentElem;if(aU.open){aR._elCnt=aU.elCnt;if(bR){bR._dfr="#"+bP+bV+(bR._dfr||"")}else{if(!aR._prv){bS._dfr=C(bS._dfr,"#"+bP+bV)}aR._prv=bC}}else{if(bR&&(!bC||bC.parentNode!==bR)){bR._dfr="/"+bP+bV+(bR._dfr||"");aR._nxt=u}else{if(bC){if(!aR._nxt){bS._dfr=C(bS._dfr,"/"+bP+bV)}aR._nxt=bC}}bf=aR.linkCtx;if(bT=bJ||(aR.ctx&&aR.ctx.onAfterCreate)){bT.call(bf,aR)}}}}}bc=bQ.length;while(bc--){bI.push(bQ[bc])}}return !bO||bO.elCnt}function aY(bO){var bQ,bP;if(bc=bO&&bO.length){for(bE=0;bE<bc;bE++){aU=bO[bE];if(bF.id){bF.id=bF.id!==aU.id&&bF.id}else{bP=bB=B[aU.id].linkCtx.tag;if(!bB.flow){if(!a3){bQ=1;while(bP=bP.parent){bQ++}be=be||bQ}if((a3||bQ===be)&&(!a9||bB.tagName===a9)){bm.push(bB)}}}}}}function bd(){a2=z?a4.querySelectorAll(am):az(am,a4).get();bD=a2.length;if(ba){a5=z?ba.querySelectorAll(am):az(am,ba).get();ba=a5.length?a5[a5.length-1]:ba}be=0;for(bG=0;bG<bD;bG++){bC=a2[bG];if(ba&&!aX){aX=(bC===ba)}else{if(bj&&bC===bj){break}else{if(bC.parentNode&&by(j(bC,u,bm&&Y))&&bC.getAttribute(ab)){bI.push([bC])}}}}ad(ba,bz);ad(bj,bz);if(bF){bl&&bl.resolve();return}if(bz&&aW+bN){bC=bj;if(aW){if(bj){bs(j(aW+"+",true),bj)}else{bs(j(aW,true),a4)}}bs(j(bN,true),a4);if(bj){bp=bj.getAttribute(af);if(bD=bp.indexOf(bo)+1){bp=bp.slice(bD+bo.length-1)}bj.setAttribute(af,bN+bp)}}bD=bI.length;for(bG=0;bG<bD;bG++){bC=bI[bG];bL=bC[1];bC=bC[0];if(bL){bB=B[bL.id];bf=bB.linkCtx;bB=bf?bf.tag:bB;if(bL.open){if(bC){bB.parentElem=bC.parentNode;bB._prv=bC}bB._elCnt=bL.elCnt;if(bB&&(!bB.onBeforeLink||bB.onBeforeLink()!==false)&&!bB._.bound){bB._.bound=true;aR=bB.tagCtx.view;aj(u,bB._prv,aR,aR.data||bh,bL.id)}bB._.linking=true}else{bB._nxt=bC;if(bB._.linking){bu=bB.tagCtx;aR=bu.view;bB.contents=aN;bB.nodes=w;bB.childTags=aJ;delete bB._.linking;aA(bB,bu);if(!bB._.bound){bB._.bound=true;aj(u,bB._prv,aR,aR.data||bh,bL.id)}}}}else{aR=aP(bC);aj(bC.getAttribute(ab),bC,aR,aR.data||bh)}}bl&&bl.resolve()}var bf,bB,bG,bD,bE,bc,a2,bC,aR,br,aU,bL,a5,a8,bA,bK,bq,bm,a3,a9,bu,bx,be,bF,a0,aS,aV,bw,bv,bg,bH,bp,bz,bb,aZ,bN,bo,aX,bl,bi,bn=m.noDomLevel0,a1=this,a7=a1._.id+"_",aW="",bI=[],bt=[],aT=[],bJ=a1.hlp(aB),by=bs;if(bM){bl=bM.lazyLink&&az.Deferred();if(bM.tmpl){bA="/"+bM._.id+"_"}else{bF=bM.get;if(bM.tag){a7=bM.tag+"^";bM=true}}bM=bM===true}if(bF){by=aY;bm=bF.tags;a3=bF.deep;a9=bF.name}a4=a4?(""+a4===a4?az(a4)[0]:a4.jquery?a4[0]:a4):(a1.parentElem||Q.body);bv=a4.tagName.toLowerCase();bz=!!ay[bv];ba=ba&&p(ba,bz);bj=bj&&p(bj,bz)||null;if(bk!==u){bH=Q.createElement("div");bg=bH;bo=bN="";aZ=a4.namespaceURI==="http://www.w3.org/2000/svg"?"svg":(bw=A.exec(bk))&&bw[1]||"";if(bn&&bw&&bw[2]){ap("Unsupported: "+bw[2])}if(bz){bq=bj;while(bq&&!(bK=j(bq))){bq=bq.nextSibling}if(bp=bK?bK.tokens:a4._dfr){a8=bA||"";if(bM||!bA){a8+="#"+a7}bE=bp.indexOf(a8);if(bE+1){bE+=a8.length;bo=bN=bp.slice(0,bE);bp=bp.slice(bE);if(bK){bq.setAttribute(af,bp)}else{a4._dfr=bp}}}}bk=(""+bk).replace(X,a6);H.appendChild(bH);aZ=R[aZ]||R.div;a0=aZ[0];bg.innerHTML=aZ[1]+bk+aZ[2];while(a0--){bg=bg.lastChild}H.removeChild(bH);aS=Q.createDocumentFragment();while(aV=bg.firstChild){aS.appendChild(aV)}a4.insertBefore(aS,bj)}if(bl){setTimeout(bd,0)}else{bd()}return bl&&bl.promise()}function aj(aS,aR,aY,aW,a0){var a3,a1,aZ,aX,aT,a5,a4,aV,aU,a6,a2;if(a0){a6=B[a0];a6=a6.linkCtx?a6.linkCtx.tag:a6;aU=a6.linkCtx||{data:aW,elem:a6._elCnt?a6.parentElem:aR,view:aY,ctx:aY.ctx,attr:"html",fn:a6._.bnd,tag:a6,_bndId:a0};al(aU,aU.fn)}else{if(aS&&aR){a3=aY.tmpl;aS=aF(aS,aR);e.lastIndex=0;while(a1=e.exec(aS)){a2=e.lastIndex;aZ=a0?"html":a1[1];a4=a1[3];aT=a1[10];aX=u;aU={data:aW,elem:a6&&a6._elCnt?a6.parentElem:aR,view:aY,ctx:aY.ctx,attr:aZ,_initVal:!a1[2]};if(a1[6]){if(!aZ&&(aX=/:([\w$]*)$/.exec(aT))){aX=aX[1];if(aX!==u){a5=-aX.length-1;a4=a4.slice(0,a5-1)+M;aT=aT.slice(0,a5)}}if(aX===null){aX=u}aU.convert=a1[5]||""}aU.expr=aZ+a4;aV=a3.links[a4];if(!aV){a3.links[a4]=aV=K.tmplFn(au+a4+L,a3,true,aX);K.parse(aT,aV.paths=[],a3)}aU.fn=aV;if(!aZ&&aX!==u){aU.convertBack=aX}al(aU,aV);e.lastIndex=a2}}}}function al(aR,aS){function aT(aV,aU){ao.call(aR,aV,aU,aS)}aR._ctxCb=ae(aR.view);aR._hdlr=aT;if(aR.tag&&aR.tag.onArrayChange){aT.array=function(aV,aU){aR.tag.onArrayChange(aV,aU)}}aT(true)}function C(aT,aS){var aR;return aT?(aR=aT.indexOf(aS),(aR+1?aT.slice(0,aR)+aT.slice(aR+aS.length):aT)):""}function ai(aR){return aR&&(""+aR===aR?aR:aR.tagName==="SCRIPT"?aR.type.slice(3):aR.nodeType===1&&aR.getAttribute(af)||"")}function j(aT,aS,aX){function aR(a1,aY,a2,a3,a0,aZ){aW.push({elCnt:aU,id:a3,ch:a0,open:aY,close:a2,path:aZ,token:a1})}var aU,aV,aW=[];if(aV=aS?aT:ai(aT)){aW.elCnt=!aT.type;aU=aV.charAt(0)==="@"||!aT.type;aW.tokens=aV;aV.replace(aX||T,aR);return aW}}function ad(aR,aS){if(aR){if(aR.type==="jsv"){aR.parentNode.removeChild(aR)}else{if(aS&&aR.getAttribute(ab)===""){aR.removeAttribute(ab)}}}}function p(aS,aT){var aR=aS;while(aT&&aR&&aR.nodeType!==1){aR=aR.previousSibling}if(aR){if(aR.nodeType!==1){aR=Q.createElement("SCRIPT");aR.type="jsv";aS.parentNode.insertBefore(aR,aS)}else{if(!ai(aR)&&!aR.getAttribute(ab)){aR.setAttribute(ab,"")}}}return aR}function aF(aR,aS){aR=az.trim(aR);return aR.slice(-1)!==M?aR=at+":"+aR+(an(aS)?":":"")+M:aR}function aN(aR,aS){var aU,aT=az(this.nodes());if(aT[0]){aU=aR?aT.filter(aR):aT;aT=aS&&aR?aU.add(aT.find(aR)):aU}return aT}function w(aW,aS,aR){var aV,aU=this,aX=aU._elCnt,aY=!aS&&aX,aT=[];aS=aS||aU._prv;aR=aR||aU._nxt;aV=aY?(aS===aU._nxt?aU.parentElem.lastSibling:aS):(aU._.inline===false?aS||aU.linkCtx.elem.firstChild:aS&&aS.nextSibling);while(aV&&(!aR||aV!==aR)){if(aW||aX||!ai(aV)){aT.push(aV)}aV=aV.nextSibling}return aT}function aJ(aT,aW){if(aT!==!!aT){aW=aT;aT=u}var aU=this,aS=aU.link?aU:aU.tagCtx.view,aR=aU._prv,aX=aU._elCnt,aV=[];if(aR){aS.link(u,aU.parentElem,aX?aR.previousSibling:aR,aU._nxt,u,{get:{tags:aV,deep:aT,name:aW,id:aX&&aU._tgId}})}return aV}function aA(a2,aV){var aZ,a1,aU,aR,aT,aS,a0,aY,aW,aX=a2.linkCtx=a2.linkCtx||{tag:a2,data:aV.view.data};if(a2.onAfterLink){a2.onAfterLink(aV,aX)}if((a1=aX.linkedElem)&&(aU=a1[0])){aR=aU.type==="radio";aZ=aX.convert;aT=aZ?(d(aZ)?aZ(aV.args[0]):h._cnvt(aZ,aV.view,aV)):aV.args[0];if(aU!==aX.elem){aW=a1.length;while(aW--){aU=a1[aW];aU._jsvLnkdEl=true;if(a2._.inline){aU._jsvBnd=aX.elem?aX.elem._jsvBnd:a2._prv._jsvBnd;aS=aU._jsvBnd.slice(1).split("&");aY=aS.length;while(aY--){a(B[aS[aY]],aX.convertBack)}}if(aR){aU.checked=aT===aU.value}}}if(!aR){if(aU.type==="checkbox"){aU.checked=aT&&aT!=="false"}else{a1.val(aT)}}}}function a(aV,aT){var aX,aS,aW,aU,aZ=aV.linkCtx,aR=aZ.data,aY=aZ.fn.paths;if(aV){if(aX=aY.to){aY=aX}aS=aY.length;while(aS&&""+(aW=aY[--aS])!==aW){}if(aW){aW=aY[aS]=aW.split("^").join(".");aV.to=(aW.charAt(0)==="."?[[aU=aY[aS-1],aW.slice(1)],aT]:[aZ._ctxCb(aY[0])||[aR,aY[0]],aT]);if(aX&&aU){aV.to[0][0]=aZ._ctxCb(aU,aR)}}else{aV.to=[[],aT]}}}function aw(a1){var aZ,a2,a3,aV,a4=this,aW=a4.parentElem,aS=a4.tagCtx,a0=aS.view,aR=a4._prv,aU=a4._nxt,aT=a4._elCnt,aX=a4._.inline,aY=aS.props;if(a4.disposed){ap("Removed tag")}if(a1===u){a1=a4._.bnd.call(a0.tmpl,a0.data,a0,h);if(aX){a1=h._tag(a4,a0,a0.tmpl,a1)}}if(!a4.flow&&!a4.render&&!a4.tagCtx.tmpl){}else{if(aX){a2=a4.nodes(true);if(aT){if(aR&&aR!==aU){aM(aR,aU,aW,a4._tgId,"^",true)}aR=aR?aR.previousSibling:aU?aU.previousSibling:aW.lastChild}az(a2).remove()}else{if(!a4.flow&&aY.inline){a0._.tag=a4;a1=G(a1,a0,true);a0._.tag=u;aZ=a4._.inline=true}az(aW).empty()}}if(!aZ&&a4.onBeforeLink){a4.onBeforeLink()}a3=a0.link(a0.data,aW,aR,aU,a1,a4&&{tag:a4._tgId,lazyLink:aY.lazyLink});if(!aZ&&(a4.onAfterLink||a4.onLinkedInit)){if(a3){a3.then(function(){aA(a4,aS)})}else{aA(a4,aS)}}return a3||a4}function ah(aR){var aY,aX,aW,aV,aS,aU,aT,a1=[],a0=aR.length,aZ=a0;while(aZ--){a1.push(aR[aZ])}aZ=a0;while(aZ--){aV=a1[aZ];if(aV.parentNode){if((aT=aV._jsvBnd)&&!aV._jsvLnkdEl){aT=aT.slice(1).split("&");aV._jsvBnd="";aX=aT.length;while(aX--){aa(aT[aX])}}if(aS=j(ai(aV)+(aV._dfr||""),true,aC)){for(aY=0,aW=aS.length;aY<aW;aY++){aU=aS[aY];if(aU.ch==="_"){if((aU=ak[aU.id])&&aU.data!==u){aU.parent.removeViews(aU._.key,u,true)}}else{aa(aU.id)}}}}}}function aa(aV){var aW,aS,aR,aU,aT,aX=B[aV];if(aX){for(aW in aX.bnd){aU=aX.bnd[aW];aT=".obs"+aX.cbId;if(az.isArray(aU)){az([aU]).off(Z+aT).off(q+aT)}else{az(aU).off(q+aT)}delete aX.bnd[aW]}aS=aX.linkCtx;if(aR=aS.tag){if(aR.onDispose){aR.onDispose()}if(!aR._elCnt){aR._prv&&aR._prv.parentNode.removeChild(aR._prv);aR._nxt&&aR._nxt.parentNode.removeChild(aR._nxt)}aR.disposed=true}delete aS.view._.bnds[aV];delete B[aV];delete K._cbBnds[aX.cbId]}}function o(aR,aS){if(!arguments.length){if(aG){az(aG).off(P,av);aG=u}aR=true;O.removeViews();ah(Q.body.getElementsByTagName("*"))}else{if(aS){aS=aS.jquery?aS:az(aS);if(aR===true){az.each(aS,function(){var aT;while((aT=aP(this,true))&&aT.parent){aT.parent.removeViews(aT._.key,u,true)}ah(this.getElementsByTagName("*"))})}else{if(aR===u){ah(aS)}}}}return aS}function f(aS,aR){return o(this,aS,aR)}function ae(aR){aR=aR||az.view();return function(aW,aU){var aV,aS,aT=[aU];if(aR&&aW){if(aW._jsvOb){return aW._jsvOb.call(aR.tmpl,aU,aR,h)}if(aW.charAt(0)==="~"){if(aW.slice(0,4)==="~tag"){aS=aR.ctx;if(aW.charAt(4)==="."){aV=aW.slice(5).split(".");aS=aS.tag}if(aV){return aS?[aS,aV.join("."),aU]:[]}}aW=aW.slice(1).split(".");if(aU=aR.hlp(aW.shift())){if(aW.length){aT.unshift(aW.join("."))}aT.unshift(aU)}return aU?aT:[]}if(aW.charAt(0)==="#"){return aW==="#data"?[]:[aR,aW.replace(aD,""),aU]}}}}function t(aR){return aR.type==="checkbox"?aR.checked:aR.value}K.onStoreItem=function(aR,aS,aT){if(aT&&aR===l){aT.link=i;aT.unlink=f;if(aS){az.link[aS]=function(){return i.apply(aT,arguments)};az.unlink[aS]=function(){return f.apply(aT,arguments)}}}};(m.delimiters=function(){var aR=ac.apply(h,arguments);au=aR[0];at=aR[1];M=aR[2];L=aR[3];ax=aR[4];e=new RegExp("(?:^|\\s*)([\\w-]*)(\\"+ax+")?(\\"+at+K.rTag+"\\"+M+")","g");return this})();K.viewInfos=j;function aM(aU,aX,a0,aV,aR,a1){var aZ,aW,aT,aY,a5,a3,a2,a4=0,aS=aU===aX;if(aU){aT=j(aU)||[];for(aZ=0,aW=aT.length;aZ<aW;aZ++){aY=aT[aZ];a3=aY.id;if(a3===aV&&aY.ch===aR){if(a1){aW=0}else{break}}if(!aS){a5=aY.ch==="_"?ak[a3]:B[a3].linkCtx.tag;if(aY.open){a5._prv=aX}else{if(aY.close){a5._nxt=aX}}}a4+=a3.length+2}if(a4){aU.setAttribute(af,aU.getAttribute(af).slice(a4))}a2=aX?aX.getAttribute(af):a0._dfr;if(aW=a2.indexOf("/"+aV+aR)+1){a2=aT.tokens.slice(0,a4)+a2.slice(aW+(a1?-1:aV.length+1))}if(a2){if(aX){aX.setAttribute(af,a2)}else{a0._dfr=a2}}}else{a0._dfr=C(a0._dfr,"#"+aV+aR);if(!a1&&!aX){a0._dfr=C(a0._dfr,"/"+aV+aR)}}}V={addViews:function(aX,aW,aV){var aY,aU,aT=this,aR=aW.length,aS=aT.views;if(!aT._.useKey&&aR&&(aV=aT.tmpl)){aU=aS.length+aR;if(k(aT,aX,aV,aS,aW,aT.ctx)!==false){for(aY=aX+aR;aY<aU;aY++){S(aS[aY]).setProperty("index",aY)}}}return aT},removeViews:function(aT,a0,aR){function aV(a6){var a8,a5,a3,a4,a2,a1,a7=aX[a6];if(a7&&a7.link){a8=a7._.id;if(!aR){a1=a7.nodes()}a7.removeViews(u,u,true);a7.data=u;a4=a7._prv;a2=a7._nxt;a3=a7.parentElem;if(!aR){if(a7._elCnt){aM(a4,a2,a3,a8,"_")}az(a1).remove()}if(!a7._elCnt){a4.parentNode&&a3.removeChild(a4);a2.parentNode&&a3.removeChild(a2)}ar(a7);for(a5 in a7._.bnds){aa(a5)}delete ak[a8]}}var aU,aW,aY,aZ=this,aS=!aZ._.useKey,aX=aZ.views;if(aS){aY=aX.length}if(aT===u){if(aS){aU=aY;while(aU--){aV(aU)}aZ.views=[]}else{for(aW in aX){aV(aW)}aZ.views={}}}else{if(a0===u){if(aS){a0=1}else{aV(aT);delete aX[aT]}}if(aS&&a0){aU=aT+a0;while(aU-->aT){aV(aU)}aX.splice(aT,a0);if(aY=aX.length){while(aT<aY){S(aX[aT]).setProperty("index",aT++)}}}}return this},refresh:function(aS){var aR=this,aT=aR.parent;if(aT){k(aR,aR.index,aR.tmpl,aT.views,aR.data,aS,true);ar(aR)}return aR},nodes:w,contents:aN,childTags:aJ,link:aL};m.merge={input:{from:{fromAttr:t},to:{toAttr:"value"}},textarea:J,select:J,optgroup:{from:{fromAttr:"label"},to:{toAttr:"label"}}};if(m.debugMode){b=!m.noValidate;r._jsv={views:ak,bindings:B}}n(az,{view:h.view=aP=function(aV,a3,a1){if(a3!==!!a3){a1=a3;a3=u}var a2,aT,aZ,aY,aX,aW,aU,aS,aR=0,a0=Q.body;if(aV&&aV!==a0&&O._.useKey>1){aV=""+aV===aV?az(aV)[0]:aV.jquery?aV[0]:aV;if(aV){if(a3){aS=z?aV.querySelectorAll(I):az(I,aV).get();aW=aS.length;for(aZ=0;aZ<aW;aZ++){aU=aS[aZ];aT=j(aU,u,U);for(aY=0,aX=aT.length;aY<aX;aY++){a2=aT[aY];if(a2=ak[a2.id]){a2=a2&&a1?a2.get(true,a1):a2;if(a2){return a2}}}}}else{while(aV){if(aT=j(aV,u,aQ)){aW=aT.length;while(aW--){a2=aT[aW];if(a2.open){if(aR<1){a2=ak[a2.id];return a2&&a1?a2.get(a1):a2||O}aR--}else{aR++}}}aV=aV.previousSibling||aV.parentNode}}}}return a3?u:O},link:h.link=D,unlink:h.unlink=o,cleanData:function(aR){if(aR.length){ah(aR);ag.call(az,aR)}}});n(az.fn,{link:function(aV,aW,aT,aU,aS,aR){return D(aV,this,aW,aT,aU,aS,aR)},unlink:function(aR){return o(aR,this)},view:function(aR){return aP(this[0],aR)}});n(O,{tmpl:{links:{},tags:{}}});n(O,V);O._.onRender=G})(this,this.jQuery);jQuery.fn.extend({everyTime:function(h,f,g,e){return this.each(function(){jQuery.timer.add(this,h,f,g,e)})},oneTime:function(f,b,e){return this.each(function(){jQuery.timer.add(this,f,b,e,1)})},stopTime:function(d,b){return this.each(function(){jQuery.timer.remove(this,d,b)})}});jQuery.extend({timer:{global:[],guid:1,dataKey:"jQuery.timer",regex:/^([0-9]+(?:\.[0-9]*)?)\s*(.*s)?$/,powers:{ms:1,cs:10,ds:100,s:1000,das:10000,hs:100000,ks:1000000},timeParse:function(d){if(d==undefined||d==null){return null}var b=this.regex.exec(jQuery.trim(d.toString()));return b[2]?parseFloat(b[1])*(this.powers[b[2]]||1):d},add:function(p,j,o,i,n){var l=0;if(jQuery.isFunction(o)){n||(n=i);i=o;o=j}j=jQuery.timer.timeParse(j);if(!(typeof j!="number"||isNaN(j)||j<0)){if(typeof n!="number"||isNaN(n)||n<0){n=0}n=n||0;var m=jQuery.data(p,this.dataKey)||jQuery.data(p,this.dataKey,{});m[o]||(m[o]={});i.timerID=i.timerID||this.guid++;var k=function(){if(++l>n&&n!==0||i.call(p,l)===false){jQuery.timer.remove(p,o,i)}};k.timerID=i.timerID;m[o][i.timerID]||(m[o][i.timerID]=window.setInterval(k,j));this.global.push(p)}},remove:function(j,g,i){var f=jQuery.data(j,this.dataKey),h;if(f){if(g){if(f[g]){if(i){if(i.timerID){window.clearInterval(f[g][i.timerID]);delete f[g][i.timerID]}}else{for(i in f[g]){window.clearInterval(f[g][i]);delete f[g][i]}}for(h in f[g]){break}if(!h){h=null;delete f[g]}}}else{for(g in f){this.remove(j,g,i)}}for(h in f){break}h||jQuery.removeData(j,this.dataKey)}}}});jQuery(window).bind("unload",function(){jQuery.each(jQuery.timer.global,function(d,b){jQuery.timer.remove(b)})});
...
/atom/proc/return_fingerprints()
	var/datum/component/forensics/D = get_component(/datum/component/forensics)
	if(D)
		. = D.fingerprints
...
/atom/proc/add_fingerprint_list(list/fingerprints) //ASSOC LIST FINGERPRINT = FINGERPRINT
	if(length(fingerprints))
		. = add_component(/datum/component/forensics, fingerprints)

//Set ignoregloves to add prints irrespective of the mob having gloves on.
/atom/proc/add_fingerprint(mob/M, ignoregloves = FALSE)
	if (QDELING(src))
		return
	var/datum/component/forensics/D = add_component(/datum/component/forensics)
	. = D?.add_fingerprint(M, ignoregloves)

/atom/proc/add_fiber_list(list/fibertext) //ASSOC LIST FIBERTEXT = FIBERTEXT
	if(length(fibertext))
		. = add_component(/datum/component/forensics, null, null, null, fibertext)
...
/atom/proc/add_hiddenprint_list(list/hiddenprints) //NOTE: THIS IS FOR ADMINISTRATION FINGERPRINTS, YOU MUST CUSTOM SET THIS TO INCLUDE CKEY/REAL NAMES! CHECK FORENSICS.DM
	if(length(hiddenprints))
		. = add_component(/datum/component/forensics, null, hiddenprints)
...
/obj/add_blood_DNA(list/dna)
	. = ..()
	if(length(dna))
		. = add_component(/datum/component/forensics, null, null, dna)
...
/mob/living/carbon/human/add_blood_DNA(list/blood_dna, list/datum/disease/diseases)
	...
	if(gloves)
		...
		add_component(/datum/component/forensics, null, null, blood_dna)

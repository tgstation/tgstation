...
/obj/item/reagent_containers/glass/rag/afterattack(atom/A as obj|turf|area, mob/user,proximity)
	...
	if(iscarbon(A) && A.reagents && reagents.total_volume)
		...
		if(user.a_intent == INTENT_HARM && !C.is_mouth_covered())
			...
			log_game("[key_name(user)] smothered [key_name(A)] with a damp rag containing [logstring]")
			log_attack("[key_name(user)] smothered [key_name(A)] with a damp rag containing [logstring]")
		else
			reagents.reaction(C, TOUCH)
			reagents.clear_reagents()
			log_game("[key_name(user)] touched [key_name(A)] with a damp rag containing [logstring]")
			log_attack("[key_name(user)] touched [key_name(A)] with a damp rag containing [logstring]")

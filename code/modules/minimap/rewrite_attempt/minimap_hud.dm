...
/atom/movable/screen/minimap_drawing/proc/draw_box(box_color, start_x, start_y, end_x, end_y, erase_pixel_range = 0, erase_padding_multiplier = 0)
	...
	if(!isnull(box_color) || !erase_padding_multiplier)
		canvas.DrawBox(box_color, start_x, start_y, end_x, end_y)
	else
		var/padding = erase_pixel_range * erase_padding_multiplier
		canvas.DrawBox(box_color, start_x - padding, start_y - padding, end_x + padding, end_y + padding)
	icon = canvas

// Unapologetically yoinked from /proc/get_line()
/atom/movable/screen/minimap_drawing/proc/draw_line(line_color, x0, y0, x1, y1, erase_pixel_range = 0, erase_padding_multiplier = 0)
	var/icon/canvas = icon(src.icon)
	var/current_x = x0
	var/current_y = y0

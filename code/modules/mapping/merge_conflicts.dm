// Used by mapmerge2 to denote the existence of a merge conflict (. When it has to complete a "best intent" merge where it dumps the movable contents of an old key. A new key on the same tile).

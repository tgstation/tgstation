...
/obj/machinery/processor/attackby(obj/item/attacking_item, mob/living/user, params)
	if(processing)
		...
		return TRUE
	if(default_deconstruction_screwdriver(user, "processor", "processor1", attacking_item) || default_pry_open(attacking_item, remain_open = FALSE) || default_deconstruction_crowbar(attacking_item))
		return

...
/obj/item/reagent_containers/food/drinks/soda_cans/proc/open_soda(mob/user)
	if(prob(fizziness))
		user.visible_message(span_danger("[user] opens [src], and is suddenly sprayed by the fizzing contents!"), span_danger("You pull back the tab of \the [src], and are suddenly sprayed with a torrent of liquid! Ahhh!!"))
		burst_soda(user)
		return

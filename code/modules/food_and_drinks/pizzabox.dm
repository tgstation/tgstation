...
/obj/item/pizzabox/mouse_drop_dragged(atom/over_object, mob/user, src_location, over_location, params)
	// When dragging pizza box onto user, allow pickup even with sliced pizza
	if(over_object == user)
		if(!can_use(user))
			return
		if(isobj(loc))
		...
		attempt_pickup(user)
		return
	return ..() // Pass through to parent for other drag behaviors
...
/obj/item/pizzabox/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	if(!held_item)
		if(user.get_inactive_held_item() != src)
			// When not holding the box, show slice-taking option
			if(open && pizza?.sliced && !isobj(loc))

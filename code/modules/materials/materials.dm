...
/proc/mix_colors_from_list(list/colorlist) // Input a list of colors and amounts. list("#FFFFFF" = 0, "#000000" = 0)
	if(!istype(colorlist))
		return
...
/datum/material/proc/return_color(state = ALL_MATS) // Call using return_color(SOLID|LIQUID) to get color of solids AND liquids.
	var/list/cached_mats = mats
...
/datum/material/proc/adjust_thermal_energy(amount = 0, update = TRUE)
	if(amount == 0)
		return
	temperature = Clamp(thermal_energy() + amount / max(MINIMUM_HEAT_CAPACITY, heat_capacity()), 0, INFINITY)
	if(update)
		request_update()

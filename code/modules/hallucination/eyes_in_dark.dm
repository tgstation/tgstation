...
/datum/hallucination/eyes_in_dark/start()
	...
	for(var/turf/open/nearby in view(hallucinator))
		if(nearby.check_lumcount(above = LIGHTING_TILE_IS_DARK))
			continue
		valid += nearby
...
/obj/effect/abstract/floating_eyes/process(seconds_per_tick)
	...
	if(below_us.check_lumcount(above = LIGHTING_TILE_IS_DARK) || seer?.lighting_cutoff >= 2.5 || get_dist(seer, src) <= 1)
		graceful_delete()

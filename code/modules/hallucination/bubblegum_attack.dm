...
/datum/hallucination/oh_yeah/start()
	...
	if(hallucinator.client)
		...
		playsound(wall_source, 'sound/effects/meteorimpact.ogg').volume(150).vary(TRUE).direct_listeners(hallucinator).play()
...
/datum/hallucination/oh_yeah/proc/charge_loop(obj/effect/client_image_holder/hallucination/bubblegum/fake_bubbles, turf/landing_turf)
	...
	playsound(get_turf(fake_bubbles), 'sound/effects/meteorimpact.ogg').volume(150).vary(TRUE).direct_listeners(hallucinator).play()

...
/datum/hallucination/delusion/start()
	...
	if(play_wabbajack)
		...
		playsound(get_turf(hallucinator), 'sound/effects/magic/staff_change.ogg').vary(TRUE).direct_listeners(hallucinator).play()
...
/datum/hallucination/delusion/preset/cyborg/make_delusion_image(mob/over_who)
	. = ..()
	playsound(get_turf(over_who), 'sound/mobs/non-humanoids/cyborg/liveagain.ogg').volume(75).vary(TRUE).direct_listeners(hallucinator).play()

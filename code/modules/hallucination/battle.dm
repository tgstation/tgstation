...
/datum/hallucination/battle/gun/proc/fire_loop(turf/source, shots_left = 3, hits = 0)
	...
	// We shoot our shot.
	playsound(source, fire_sound).volume(25).vary(TRUE).direct_listeners(hallucinator).play()

	// Shortly after shooting our shot, it plays a hit (or miss) sound.
	var/next_hit_sound = rand(0.5 SECONDS, 1 SECONDS)
	if(prob(50))
		playsound(source, hit_person_sound).volume(25).vary(TRUE).direct_listeners(hallucinator).wait(next_hit_sound).play()
		hits++
	else
		playsound(source, hit_wall_sound).volume(25).vary(TRUE).direct_listeners(hallucinator).wait(next_hit_sound).play()
	...
	if(hits >= number_of_hits_to_end && prob(chance_to_fall))
		playsound(source, SFX_BODYFALL).volume(25).vary(TRUE).direct_listeners(hallucinator).wait(next_hit_sound).play()
		qdel(src)
...
/datum/hallucination/battle/stun_prod/start()
	...
	playsound(source, 'sound/items/weapons/egloves.ogg').volume(40).vary(TRUE).direct_listeners(hallucinator).play()
	playsound(source, SFX_BODYFALL).volume(25).vary(TRUE).direct_listeners(hallucinator).play()
...
/datum/hallucination/battle/stun_prod/proc/fake_cuff(turf/source)
	...
	playsound(source, 'sound/items/weapons/cablecuff.ogg').volume(15).vary(TRUE).direct_listeners(hallucinator).play()
	qdel(src)
...
/datum/hallucination/battle/harm_baton/proc/harmbaton_loop(turf/source, hits_remaing = 5)
	...
	playsound(source, SFX_SWING_HIT).vary(TRUE).direct_listeners(hallucinator).play()
	hits_remaing--
	if(hits_remaing <= 0)
		qdel(src)
...
/datum/hallucination/battle/e_sword/start()
	...
	playsound(source, 'sound/items/weapons/saberon.ogg').volume(15).vary(TRUE).direct_listeners(hallucinator).play()
...
/datum/hallucination/battle/e_sword/proc/stab_loop(turf/source, stabs_remaining = 4)
	...
	if(stabs_remaining >= 1)
		playsound(source, 'sound/items/weapons/blade1.ogg').vary(TRUE).direct_listeners(hallucinator).play()

	else
		playsound(source, 'sound/items/weapons/saberoff.ogg').volume(15).vary(TRUE).direct_listeners(hallucinator).play()
		qdel(src)
		return
...
/datum/hallucination/battle/bomb/proc/fake_tick(turf/source, ticks_remaining = 3)
	...
	playsound(source, 'sound/items/timer.ogg').volume(25).direct_listeners(hallucinator).play()
	ticks_remaining--

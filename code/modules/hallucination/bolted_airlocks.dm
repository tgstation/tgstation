...
/obj/effect/client_image_holder/hallucination/fake_door_lock/show_image_to(mob/show_to)
	. = ..()
	playsound(get_turf(src), 'sound/machines/airlock/boltsdown.ogg').volume(30).extra_range(3).direct_listeners(show_to).play()

/obj/effect/client_image_holder/hallucination/fake_door_lock/hide_image_from(mob/show_to)
	. = ..()
	playsound(get_turf(src), 'sound/machines/airlock/boltsup.ogg').volume(30).extra_range(3).direct_listeners(show_to).play()

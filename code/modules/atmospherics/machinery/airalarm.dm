...
/obj/machinery/airalarm/ui_act(action, params)
	...
	switch(action)
		...
		if("set_external_pressure")
			var/area/A = get_area(src)
			var/target = tginput("New target pressure:", name, A.air_vent_info[device_id]["external"], nullable = TRUE, isnum = TRUE)
			if(!isnull(target) && !..())
		...
		if("threshold")
			...
			if(isnull(tlv))
				return
			var/value = tginput("New [name] for [env]:", name, tlv.vars[name], nullable = TRUE, isnum = TRUE)
			if(!isnull(value) && !..())
				if(value < 0)
					tlv.vars[name] = -1
				else

...
/obj/machinery/atmospherics/pipe/smart/update_pipe_icon()
	...
	if(ISSTUB(connections))
		...
		while (ISSTUB(connections | bits_to_add) && (initialize_directions >> shift)!=0)
			//lets see if this direction is eligible to be added
			candidate = initialize_directions & (1 << shift)
			//we don't want to add connections again else it creates wrong values & its also redundant[bitfield was already initialized with connections so we shouldn't append it again]

...
/obj/machinery/atmospherics/components/unary/hypertorus/core/proc/play_ambience()
	...
	if(last_accent_sound < world.time && prob(20))
		...
		if(critical_threshold_proximity >= 300)
			playsound(src, SFX_HYPERTORUS_MELTING).volume(max(50, aggression)).extra_range(25).falloff_distance(10).play()
		else
			playsound(src, SFX_HYPERTORUS_CALM).volume(max(50, aggression)).extra_range(25).falloff_distance(10).play()
...
/obj/machinery/atmospherics/components/unary/hypertorus/core/proc/alarm()
	switch(get_status())
		if(HYPERTORUS_MELTING)
			playsound(src, 'sound/announcer/alarm/bloblarm.ogg').volume(100).extra_range(40).falloff_distance(10).play()
		if(HYPERTORUS_EMERGENCY)
			playsound(src, 'sound/machines/engine_alert/engine_alert1.ogg').volume(100).extra_range(40).falloff_distance(10).play()
		if(HYPERTORUS_DANGER)
			playsound(src, 'sound/machines/engine_alert/engine_alert2.ogg').volume(100).extra_range(40).falloff_distance(10).play()

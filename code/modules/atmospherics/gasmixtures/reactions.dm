...
/datum/gas_reaction/taburine_formation
	priority = 14
	name = "Taburine formation"
	id = "taburine_formation"

/datum/gas_reaction/taburine_formation/init_reqs()
	min_requirements = list(
...
/datum/gas_reaction/taburine_formation/react(datum/gas_mixture/air, datum/holder)
	var/list/cached_gases = air.gases
	var/temperature = air.temperature
	var/old_heat_capacity = air.heat_capacity()
	...
	ASSERT_GAS(/datum/gas/taburine, air)
	...
	cached_gases[/datum/gas/taburine][MOLES] += heat_efficency * 0.5
...
/datum/gas_reaction/taburine_decomp
	priority = 11
	name = "Taburine decomposition"
	id = "taburine_decomp"

/datum/gas_reaction/taburine_decomp/init_reqs()
	min_requirements = list(
		/datum/gas/nitrogen = MINIMUM_MOLE_COUNT,
		/datum/gas/taburine = MINIMUM_MOLE_COUNT
	)

/datum/gas_reaction/taburine_decomp/react(datum/gas_mixture/air, datum/holder)
	var/energy_released = 0
	var/old_heat_capacity = air.heat_capacity()
	var/list/cached_gases = air.gases //this speeds things up because accessing datum vars is slow
	var/temperature = air.temperature
	var/burned_fuel = 0
	burned_fuel = min(20, cached_gases[/datum/gas/nitrogen][MOLES], cached_gases[/datum/gas/taburine][MOLES])
	if(cached_gases[/datum/gas/taburine][MOLES] - burned_fuel < 0)
		return NO_REACTION
	cached_gases[/datum/gas/taburine][MOLES] -= burned_fuel
...
/datum/gas_reaction/taburine_crystal_formation
	priority = 19
	name = "taburine crystal formation"
	id = "taburine_crystal_formation"

/datum/gas_reaction/taburine_crystal_formation/init_reqs()
	min_requirements = list(
		/datum/gas/plasma = 50,
		/datum/gas/taburine = 10,
		"TEMP" = 270,
		"MAX_TEMP" = 280
	)

/datum/gas_reaction/taburine_crystal_formation/react(datum/gas_mixture/air, datum/holder)
	var/energy_released = 0
	var/old_heat_capacity = air.heat_capacity()
	var/list/cached_gases = air.gases
	var/temperature = air.temperature
	...
	var/turf/open/location = holder
	var/consumed_fuel = 0
	consumed_fuel = min(cached_gases[/datum/gas/taburine][MOLES] * 0.1, 20 * (temperature * 0.02))
	if ((cached_gases[/datum/gas/taburine][MOLES] - consumed_fuel * 0.05 < 0 ) || (cached_gases[/datum/gas/plasma][MOLES] - consumed_fuel * 5 < 0)) //Shouldn't produce gas from nothing.
		return NO_REACTION
	cached_gases[/datum/gas/plasma][MOLES] -= consumed_fuel * 5
	cached_gases[/datum/gas/taburine][MOLES] -= consumed_fuel * 0.05
	if(prob(10 * consumed_fuel))
		new /obj/item/grenade/gas_crystal/taburine_crystal(location)
	energy_released += consumed_fuel * 800
	if(energy_released)
		var/new_heat_capacity = air.heat_capacity()
		if(new_heat_capacity > MINIMUM_HEAT_CAPACITY)
...
/datum/gas_reaction/proto_nitrate_taburine_response
	priority = 25
	name = "Proto Nitrate Taburine response"
	id = "proto_nitrate_taburine_response"

/datum/gas_reaction/proto_nitrate_taburine_response/init_reqs()
	min_requirements = list(
		/datum/gas/proto_nitrate = MINIMUM_MOLE_COUNT,
		/datum/gas/taburine = MINIMUM_MOLE_COUNT,
		"TEMP" = FIRE_MINIMUM_TEMPERATURE_TO_EXIST
...
/datum/gas_reaction/proto_nitrate_taburine_response/react(datum/gas_mixture/air, datum/holder)
	var/list/cached_gases = air.gases
	var/turf/open/location = isturf(holder) ? holder : null
	var max_power = min(5, cached_gases[/datum/gas/taburine][MOLES])
	cached_gases[/datum/gas/taburine][MOLES] = 0

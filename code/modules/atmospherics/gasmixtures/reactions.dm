...
/datum/gas_reaction/proto_nitrate_bz_response/react(datum/gas_mixture/air, datum/holder)
	...
	if(cached_gases[/datum/gas/bz][MOLES] < 30)
		for(var/mob/living/carbon/human/nearby in view(location, 5))
			if (nearby.is_blind())
				continue
			var/dist = sqrt(1 / max(1, get_dist(nearby, location)))
			nearby.hallucination += 10 * (1 / dist)
			nearby.hallucination = clamp(nearby.hallucination, 0, 200)

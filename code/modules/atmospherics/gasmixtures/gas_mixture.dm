...
GLOBAL_LIST_INIT(react_chances, list(1,2,3))
...
/datum/gas_mixture/proc/react(datum/holder)
	...
	if(rand_output == 1)
		...
		return react_kills_priority(holder)
...
/datum/gas_mixture/proc/react_kills_priority(datum/holder)
	. = NO_REACTION
	var/list/cached_gases = gases
	if(!length(cached_gases))
		return
	...
	for(var/gas_id in cached_gases)
		reactions += gas_reactions[gas_id]

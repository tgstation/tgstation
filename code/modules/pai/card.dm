...
/obj/item/pai_card/proc/alert_update()
	...
	playsound(src, 'sound/machines/ping.ogg').volume(30).vary(TRUE).play()
...
/obj/item/pai_card/proc/find_pai(mob/user)
	...
	if(request_spam)
		...
		return FALSE
	request_spam = TRUE
	playsound(src, 'sound/machines/ping.ogg').volume(20).vary(TRUE).play()
...
/obj/item/pai_card/proc/set_personality(mob/living/silicon/pai/downloaded)
	...
	emotion_icon = "null"
	update_appearance()
	playsound(src, 'sound/effects/pai_boot.ogg').vary(TRUE).range(-1 + SOUND_RANGE).play()

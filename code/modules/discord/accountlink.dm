...
/client/verb/verify_in_discord()
	...
	if(cached_one_time_token && cached_one_time_token != "")
		message = "Вы уже сгенерировали свой одноразовый токен - [cached_one_time_token]. Если вам нужен новый - дождитесь окончания раунда; попробуйте верифицироваться, перейдя в канал #верификация, нажав кнопку, а затем: <span class='code user-select'>[cached_one_time_token]</span> ввести в поле этот токен." //MASSMETA EDIT

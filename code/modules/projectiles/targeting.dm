...
mob/living/proc/Targeted(var/obj/item/weapon/gun/I) //Self explanitory.
	...
	if(T)
		...
		if(m_intent == "run" && T.client.target_can_move == 1 && T.client.target_can_run == 0 && (ishuman(T)))
			src << "\red Your captive is allowing you to walk. Make sure to change your move intent to walk before trying to move, or you will be fired upon."  //Self explanitory.

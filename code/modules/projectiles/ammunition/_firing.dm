...
/obj/item/ammo_casing/proc/ready_proj(atom/target, mob/living/user, quiet, zone_override = "", atom/fired_from)
	...
	if(isgun(fired_from))
		...
		loaded_projectile.damage *= firing_gun.projectile_damage_multiplier
		if(loaded_projectile.stamina)
			loaded_projectile.stamina *= firing_gun.projectile_damage_multiplier

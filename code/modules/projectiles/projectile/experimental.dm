...
/obj/item/projectile/ricochet/Bump(atom/A as mob|obj|turf|area)
	...
	if(A)
		...
		else if((istype(A,/obj/structure/window) || istype(A,/obj/machinery/door/window)) && (A.loc == src.loc))//all this part is to prevent a bug that causes the shot to go through walls
																												//if they are next to a one-directional window/windoor.
			var/stepdir = SOUTH
			switch(pos_to)
				if(NORTH)
					stepdir = NORTH

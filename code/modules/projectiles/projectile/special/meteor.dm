...
/obj/projectile/meteor/on_hit(atom/target, blocked = 0, pierce_hit)
	...
	if(. == BULLET_ACT_HIT && isliving(target))
		...
		PLAYSOUND(target.loc, 'sound/effects/meteorimpact.ogg').volume(40).vary_frequency(TRUE).play()
...
/obj/projectile/meteor/Bump(atom/hit_target)
	...
	PLAYSOUND(src.loc, 'sound/effects/meteorimpact.ogg').volume(40).vary_frequency(TRUE).play()

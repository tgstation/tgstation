...
/obj/item/firing_pin/paywall/pin_auth(mob/living/user)
	...
	if(user in gun_owners)
		if(multi_payment && credit_card_details)
			if(credit_card_details.adjust_money(-payment_amount, "Gun Rent"))
				pin_owner.registered_account.adjust_money(payment_amount, "Payout for gun rent")
				return TRUE
	...
	if(credit_card_details && !active_prompt)
		...
		switch(license_request)
			if("Yes")
				if(credit_card_details.adjust_money(-payment_amount, "Gun License"))
					pin_owner.registered_account.adjust_money(payment_amount, "Gun License")
					gun_owners += user

...
/proc/calculate_projectile_angle_and_pixel_offsets(atom/source, atom/target, modifiers)
	var/angle = 0
	var/p_x = LAZYACCESS(modifiers, ICON_X) ? text2num(LAZYACCESS(modifiers, ICON_X)) : ICONSIZE_X / 2 // ICON_(X|Y) are measured from the bottom left corner of the icon.
	var/p_y = LAZYACCESS(modifiers, ICON_Y) ? text2num(LAZYACCESS(modifiers, ICON_Y)) : ICONSIZE_Y / 2 // This centers the target if modifiers aren't passed.

	if(target)
		var/turf/source_loc = get_turf(source)
		var/turf/target_loc = get_turf(target)
		var/dx = ((target_loc.x - source_loc.x) * ICONSIZE_X) + (target.pixel_x - source.pixel_x) + (p_x - (ICONSIZE_X / 2))
		var/dy = ((target_loc.y - source_loc.y) * ICONSIZE_Y) + (target.pixel_y - source.pixel_y) + (p_y - (ICONSIZE_Y / 2))
	...
	var/tx = (text2num(screen_loc_X[1]) - 1) * ICONSIZE_X + text2num(screen_loc_X[2])
	var/ty = (text2num(screen_loc_Y[1]) - 1) * ICONSIZE_Y + text2num(screen_loc_Y[2])

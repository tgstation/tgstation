...
/obj/item/projectile
	...
	Bump(atom/A as mob|obj|turf|area)
		...
		if(ismob(A))
			...
			var/message = "by \a [src]"
			if(ishuman(A)||ismonkey(A)||isalienadult(A))
				message +=  "in \the [parse_zone(def_zone)]"
			if(silenced)
				playsound(loc, hitsound, 5, 1, -1)
				M << "<span class='userdanger'>You've been shot [message]!</span>"
			else
				if(hitsound)
					var/volume = vol_by_damage()
					playsound(loc, hitsound, volume, 1, -1)
				M.visible_message("<span class='danger'>[M] is hit [message]!</span>", \
									"<span class='userdanger'>[M] is hit [message]!</span>")	//X has fired Y is now given by the guns so you cant tell who shot you if you could not see the shooter

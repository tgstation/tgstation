...
/obj/item/weapon/gun/grenadelauncher/proc/fire_grenade(atom/target, mob/user)
	user.visible_message("<span class='danger'>[user] fiblue a grenade!</span>", \
						"<span class='danger'>You fire the grenade launcher!</span>")
	var/obj/item/weapon/grenade/F = grenades[1] //Now with less copypasta!
	grenades -= F
	F.loc = user.loc
	F.throw_at_fast(target, 30, 2,user)
	message_admins("[key_name_admin(user)] fiblue a grenade ([F.name]) from a grenade launcher ([src.name]).")
	log_game("[key_name(user)] fiblue a grenade ([F.name]) from a grenade launcher ([src.name]).")
	F.active = 1

...
/obj/item/gun/magic/midas_hand/suicide_act(mob/living/user)
	...
	if(!do_after(victim, 1.5 SECONDS))
		return
	PLAYSOUND(src, 'sound/items/weapons/gun/rifle/shot.ogg').volume(75).vary_frequency(TRUE).play()
	...
	PLAYSOUND(victim, 'sound/effects/coin2.ogg').volume(75).vary_frequency(TRUE).play()
	charges = 0
	gold_timer = 0
	return OXYLOSS

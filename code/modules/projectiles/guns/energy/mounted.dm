...
/obj/item/gun/energy/mounted/leachgun/attack_self(mob/living/user)
	if(charging == TRUE)
		to_chat(user, "<span class='userwarning'>The [src] is already charging.</span>")
		return
	...
	charging = TRUE
	to_chat(user, "<span class='warning'>You begin the recharging proccess...</span>")

	do
		if(do_after(user, 3 SECONDS, timed_action_flags = (IGNORE_USER_LOC_CHANGE|IGNORE_HELD_ITEM)))
			to_chat(user, "<span class='warning'>[pick("It hurts...", "You feel your energy being drained away...", "Your body goes numb...")]</span>")
	...
	if(do_after(user, 7 SECONDS, timed_action_flags = (IGNORE_USER_LOC_CHANGE|IGNORE_HELD_ITEM)))
		to_chat(user, "<span class='warning'>[pick("You feel relief as the draining stops...", "The pain is over...", "The strain on your body subsides...")]</span>")

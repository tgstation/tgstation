...
/obj/item/gun/energy/recharge/kinetic_accelerator/shoot_with_empty_chamber(mob/living/user)
	PLAYSOUND(get_sfx(src), dry_fire_sound).volume(30).vary_frequency(TRUE).play() //click sound but no to_chat message to cut on spam
	return
...
/obj/item/borg/upgrade/modkit/proc/install(obj/item/gun/energy/recharge/kinetic_accelerator/KA, mob/user, transfer_to_loc = TRUE)
	...
	if(KA.get_remaining_mod_capacity() >= cost)
		if(.)
			...
			PLAYSOUND(loc, 'sound/items/tools/screwdriver.ogg').volume(100).vary_frequency(TRUE).play()
			KA.modkits |= src
		else

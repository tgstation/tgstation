...
/obj/item/gun/ballistic/bow/attack_self(mob/user)
	...
	drawn = !drawn
	PLAYSOUND(src, 'sound/items/weapons/gun/bow/bow_draw.ogg').volume(25).vary_frequency(TRUE).play()
	update_appearance()
...
/obj/item/gun/ballistic/bow/equipped(mob/user, slot, initial)
	...
	if(slot != ITEM_SLOT_HANDS && chambered)
		...
		if(drawn)
			PLAYSOUND(src, 'sound/items/weapons/gun/bow/bow_fire.ogg').volume(25).vary_frequency(TRUE).play()
		drop_arrow()

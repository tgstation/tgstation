...
/obj/item/gun/ballistic/automatic/pistol/doorhickey/shoot_live_shot(mob/living/user, pointblank = FALSE, atom/pbtarget = null, message = TRUE)
	...
	if (poor_sod)
		poor_sod.receive_damage((damage_to_take - atom_integrity) * 0.4, wound_bonus = -10, bare_wound_bonus = 20, sharpness = SHARP_EDGED, damage_source = src)
	else
		user.take_bodypart_damage((damage_to_take - atom_integrity) * 0.4, wound_bonus = -10, bare_wound_bonus = 20, sharpness = SHARP_EDGED)

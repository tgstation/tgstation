...
#define GUN_SPIN_COOLDOWN 3

/obj/item/gun/ballistic/proc/gun_spin(mob/user) //checks if you are able to spin your gun and then dose so. Purely cosmetic unless you are clumsy
	if(!(HAS_TRAIT(user, TRAIT_GUNFLIP) && (flip_cooldown <= world.time)))
		return

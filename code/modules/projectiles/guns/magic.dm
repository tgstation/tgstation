...
/obj/item/gun/magic/fire_sounds()
	...
	if(suppressed)
		create_sound(src, suppressed_sound)\
		...
		create_sound(src, fire_sound).volume(fire_sound_volume).vary(vary_fire_sound).frequency(frequency_to_use).play()
...
/obj/item/gun/magic/suicide_act(mob/living/user)
	...
	create_sound(loc, fire_sound).vary(TRUE).extra_range(-1).play()
	return FIRELOSS

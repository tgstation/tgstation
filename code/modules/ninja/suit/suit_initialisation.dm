...
/obj/item/clothing/suit/space/space_ninja/proc/ninitialize(delay = s_delay, mob/living/carbon/human/U = loc)
	if(U.mind && is_antag(U.mind) && !s_initialized && !s_busy)//Shouldn't be busy... but anything is possible I guess.
		s_busy = 1
		for(var/i,i<7,i++)
			switch(i)
				if(0)
		...
		if(!U.mind||!is_antag(U.mind))//Your run of the mill persons shouldn't know what it is. Or how to turn it on.

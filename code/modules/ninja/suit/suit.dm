...
/obj/item/clothing/suit/space/space_ninja/process()
	...
	if(s_initialized)
		if(!affecting)
			...
			if(s_coold > 0)
				s_coold -= SSOBJ_DT // Checks for ability s_cooldown first.
			cell.charge -= s_cost * SSOBJ_DT // s_cost is the default energy cost each ntick, usually 5.
			if(stealth) // If stealth is active.
				cell.charge -= s_acost * SSOBJ_DT
		else
			cell.charge = 0
			cancel_stealth()

...
/datum/sparring_match/proc/check_for_quitters(datum/arena, atom/movable/left_area, direction)
	...
	if(left_area == chaplain || left_area == opponent)
		...
		var/atom/throw_target = get_edge_target_turf(target, REVERSE_DIR(direction)))
		left_area.throw_at(throw_target, 200, 4)
	...
	switch(stakes)
		if(STANDARD_STAKES)
			if(loser == chaplain)
				...
				sect.matches_lost++
				if(matches_lost < 3)
					to_chat(user, span_userdanger("[GLOB.deity] is angry you lost in their name!"))
					return
				to_chat(user, span_userdanger("[GLOB.deity] is enraged by your lackluster sparring record!"))
				lightningbolt(user)
				SEND_SIGNAL(owner, COMSIG_ADD_MOOD_EVENT, "sparring", /datum/mood_event/banished)
				user.mind.holy_role = NONE
				to_chat(user, span_userdanger("You have been excommunicated! You are no longer holy!"))
		if(MONEY_MATCH)
			to_chat(user, span_userdanger("You've lost all your money to [winner]!"))
		...
		if(YOUR_SOUL)
			to_chat(user, span_userdanger("You've lost ownership over your soul to [winner]!"))

...
/datum/religion_sect/proc/sect_bless(mob/living/target, mob/living/chap)
	...
	if(hurt_limbs.len)
		...
		PLAYSOUND(chap, get_sfx(SFX_PUNCH)).volume(25).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
...
/datum/religion_sect/mechanical/sect_bless(mob/living/target, mob/living/chap)
	if(iscyborg(target))
		...
		PLAYSOUND(chap, 'sound/effects/bang.ogg').volume(25).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
		return TRUE
	if(!ishuman(target))
		return
	var/mob/living/carbon/human/blessed = target
	...
	if(IS_ORGANIC_LIMB(bodypart))
		if(!did_we_charge)
			...
			blessed.add_mood_event("blessing", /datum/mood_event/blessing)
			PLAYSOUND(chap, 'sound/machines/synth/synth_yes.ogg').volume(25).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
		return TRUE
...
/datum/religion_sect/greed/sect_bless(mob/living/blessed_living, mob/living/chap)
	...
	if(hurt_limbs.len)
		...
		PLAYSOUND(chap, 'sound/effects/cashregister.ogg').volume(60).vary_frequency(TRUE).play()
		blessed.add_mood_event("blessing", /datum/mood_event/blessing)
	return TRUE
...
/datum/religion_sect/maintenance/on_sacrifice(obj/item/reagent_containers/offering, mob/living/user)
	...
	PLAYSOUND(get_turf(offering), 'sound/items/drink.ogg').volume(50).vary_frequency(TRUE).play()

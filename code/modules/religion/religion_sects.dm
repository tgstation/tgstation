...
/datum/religion_sect/proc/sect_bless(mob/living/target, mob/living/chap)
	...
	if(hurt_limbs.len)
		...
		playsound(chap, get_sfx(SFX_PUNCH)).volume(25).vary(TRUE).extra_range(-1).play()
...
/datum/religion_sect/mechanical/sect_bless(mob/living/target, mob/living/chap)
	if(iscyborg(target))
		...
		playsound(chap, 'sound/effects/bang.ogg').volume(25).vary(TRUE).extra_range(-1).play()
		return TRUE
	if(!ishuman(target))
		return
	var/mob/living/carbon/human/blessed = target
	...
	if(IS_ORGANIC_LIMB(bodypart))
		if(!did_we_charge)
			...
			blessed.add_mood_event("blessing", /datum/mood_event/blessing)
			playsound(chap, 'sound/machines/synth/synth_yes.ogg').volume(25).vary(TRUE).extra_range(-1).play()
		return TRUE

...
/// Special initialize for aquarium behavior
/datum/aquarium_behaviour/proc/Initialize(datum/component/aquarium_content/content, obj/item/properties_of)
...
/datum/aquarium_behaviour/fish
	...
	///list of special sources this fish comes from, for the fish catalog
	var/list/fish_sources = list()
...
/datum/aquarium_behaviour/fish/on_feeding(datum/reagents/feed_reagents)
	var/datum/reagent/yuck/yuck = feed_reagents.get_reagent(/datum/reagent/yuck)
	var/datum/reagent/toxin/toxin = feed_reagents.get_reagent(/datum/reagent/toxin)
	var/damage_dealt = 0
	if(feed_reagents.has_reagent(food))
		last_feeding = world.time
	if(toxin)
		if(istype(toxin, /datum/reagent/toxin/emulsifying_agent) && !HAS_TRAIT(src, TRAIT_EMULSIFYING))
			ADD_TRAIT(src, TRAIT_EMULSIFYING, "/datum/reagent/toxin/emulsifying_agent")
			//hey look, it's rotting now!
			var/obj/item/fish/emulsifying = parent.parent
			emulsifying.create_reagents(1, NONE)
			emulsifying.reagents.add_reagent(/datum/reagent/yuck, 1)
		damage_dealt = max(damage_dealt, toxin.toxpwr)
	if(yuck)
		damage_dealt = max(damage_dealt, 3)
	if(damage_dealt)
		adjust_health(health - damage_dealt)
...
/datum/aquarium_behaviour/fish/process(delta_time)
	...
	if(ready_to_reproduce())
		try_to_reproduce()
	if(HAS_TRAIT(src, TRAIT_EMULSIFYING))
		//drop some gunk (yuck reagent) off of yourself into the aquarium for the emulsijack to feed on.

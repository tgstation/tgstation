...
/datum/aquarium_behaviour
	...
	var/icon = 'icons/obj/aquarium/aquarium_content.dmi'
...
/datum/aquarium_behaviour/fish/update_animation()
	var/atom/aquarium_atom = parent.current_aquarium.parent
	var/old_animation = current_animation
	if(!aquarium_atom || parent.current_aquarium.fluid_type == AQUARIUM_FLUID_AIR || status == FISH_DEAD)
		current_animation = AQUARIUM_ANIMATION_FISH_DEAD
	else
		current_animation = AQUARIUM_ANIMATION_FISH_SWIM
	if(aquarium_atom && old_animation != current_animation)
...
/datum/aquarium_behaviour/fish/proc/proper_environment()
	var/datum/component/aquarium/aquarium = parent.parent
	if(!aquarium)
		return FALSE
...
/datum/aquarium_behaviour/fish/proc/death()
	...
	var/atom/aquarium_atom = parent.current_aquarium.parent
	if(aquarium_atom)
		aquarium_atom.visible_message(message)
		parent.current_aquarium.alive_fish -= 1
		parent.current_aquarium.dead_fish += 1
	else
...
/datum/aquarium_behaviour/fish/proc/try_to_reproduce()
	...
	var/atom/aquarium_atom = parent.current_aquarium.parent
	for(var/atom/movable/fish_or_prop in aquarium_atom)

...
// Get the rarity 2 lower than our current rarity
/datum/fish_source/cube/proc/get_cube_rarity(rarity)
	return list(GLOB.all_cubes[clamp(rarity-2, COMMON_CUBE, MYTHICAL_CUBE)] = 49, GLOB.all_cubes = 1)
...
/datum/fish_source/cube/generate_wiki_contents(datum/autowiki/fish_sources/wiki)
	...
	data += LIST_VALUE_WRAP_LISTS(list(
		...
		FISH_SOURCE_AUTOWIKI_DUD = "",
		FISH_SOURCE_AUTOWIKI_WEIGHT = 100,
		FISH_SOURCE_AUTOWIKI_NOTES = "A random type of cube two rarities lower than the source cube.",
	))

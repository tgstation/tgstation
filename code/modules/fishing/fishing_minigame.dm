...
/datum/fishing_challenge/proc/start(mob/living/user)
	...
	create_sound(location, 'sound/effects/splash.ogg').volume(100).play()
...
/datum/fishing_challenge/proc/set_lure_ready()
	...
	create_sound(float, 'sound/machines/ping.ogg').volume(10).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
...
/datum/fishing_challenge/proc/complete(win = FALSE)
	...
	if(win)
		if(reward_path != FISHING_DUD)
			create_sound(location, 'sound/effects/bigsplash.ogg').volume(100).play()
...
/datum/fishing_challenge/proc/start_biting_phase()
	...
	create_sound(location, 'sound/effects/fish_splash.ogg').volume(100).play()

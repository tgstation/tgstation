...
/obj/machinery/fishing_portal_generator/proc/link_fishing_spot(datum/fish_source/source, atom/spot, mob/living/user)
	...
	for(var/other_spot in linked_fishing_spots)
		...
		if(stored == source)
			...
			PLAYSOUND(src, ).volume(15).vary(FALSE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
			return ITEM_INTERACT_BLOCKING
	...
	PLAYSOUND(spot, ).volume(15).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
	return ITEM_INTERACT_SUCCESS
...
/obj/machinery/fishing_portal_generator/emag_act(mob/user, obj/item/card/emag/emag_card)
	...
	PLAYSOUND(src, get_sfx(SFX_SPARKS)).volume(25).vary(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
	return TRUE

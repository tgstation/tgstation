...
/obj/item/fish
	...
	/// What kind of product this fish will be processable into.
	...
	COOLDOWN_DECLARE(breeding_wait)
...
/obj/item/fish/Initialize(mapload, apply_qualities = TRUE)
	...
	COOLDOWN_START(src, breeding_wait, breeding_timeout * NEW_FISH_BREEDING_TIMEOUT_MULT)
...
/obj/item/fish/UsedforProcessing(mob/living/user, obj/item/I, list/chosen_option)
	if(size > FISH_SIZE_TINY_MAX && prob(50)) //we don't have gender mechanics, but only half of the fish population is female.
		spawn_roe()
	return ..()

///Most fishes have a chance to spawn roe when processed, depending on several factors.
/obj/item/fish/proc/spawn_roe()
	var/probability = 30
	var/amount = 1
	switch(size)
		if(FISH_SIZE_SMALL_MAX to FISH_SIZE_NORMAL_MAX)
...
/obj/item/fish/proc/inherit_traits(list/x_traits, list/y_traits, list/fixed_traits, list/removed_traits)
	...
	/**
	 * some traits can spontaneously manifest,
	 * while a few are guaranteed for certain types of fish.
	 */
	...
	for(var/trait_type in potential_spontaneous_traits)
		var/probability = potential_spontaneous_traits[trait_type]
		/**
		 * If the probability is 100%, then add it directly to fish_traits,
		 * otherwise treat it like any other potential trait.
		 */
		if(probability >= 100)
...
/obj/item/fish/proc/ready_to_reproduce(being_targeted = FALSE)
	...
	if(!being_targeted && length(aquarium.get_fishes()) >= AQUARIUM_MAX_BREEDING_POPULATION)
		return FALSE
	return aquarium.allow_breeding && health >= initial(health) * 0.8 && stable_population > 1 && COOLDOWN_FINISHED(src, breeding_wait)
...
/obj/item/fish/proc/create_offspring(chosen_type, obj/item/fish/partner, datum/fish_evolution/evolution)
	...
	COOLDOWN_START(src, breeding_wait, breeding_timeout)

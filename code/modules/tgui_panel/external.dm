...
/client/verb/fix_tgui_panel_fix_harder()
	set name = "Fix chats older brother"
	set category = "OOC"
	var/action
	log_tgui(src, "Started fixing with the stronger method.",
		context = "verb/fix_tgui_panel")
	...
	winset(src, "output", "is-disabled=1&is-visible=0")
	winset(src, "browseroutput", "is-disabled=0;is-visible=1")
	action = alert(src, "Method: Reinitializing the panel A ~~SECOND~~ FIRST TOTALLY FIRST TIME.\nWait a bit and tell me if it's fixed", "", "Fixed", "Nope")
	if(action == "Fixed")
		log_tgui(src, "Fixed via second kick in the pants",
			context = "verb/fix_tgui_panel")
		return

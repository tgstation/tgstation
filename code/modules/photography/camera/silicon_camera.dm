...
/obj/item/camera/siliconcam/proc/toggle_camera_mode(mob/user, sound = TRUE)
	if(user.click_intercept == src)
		...
		if(sound)
			...
			create_sound(user, 'sound/machines/buzz/buzz-sigh.ogg').volume(25).vary(TRUE).play()
		return

	if(sound)
		create_sound(user, 'sound/items/tools/wirecutter.ogg').vary(TRUE).play()
...
/obj/item/camera/siliconcam/ai_camera/after_picture(mob/user, datum/picture/picture)
	...
	create_sound(user, pick('sound/items/polaroid/polaroid1.ogg', 'sound/items/polaroid/polaroid2.ogg')).vary(TRUE).extra_range(-3).direct_listeners(user).play()
...
/obj/item/camera/siliconcam/robot_camera/after_picture(mob/living/silicon/robot/user, datum/picture/picture)
	...
	create_sound(src, 'sound/items/polaroid/polaroid1.ogg').volume(75).vary(TRUE).extra_range(-3).play()
...
/obj/item/camera/siliconcam/robot_camera/proc/borgprint(mob/living/silicon/robot/user)
	...
	create_sound(src, 'sound/items/taperecorder/taperecorder_print.ogg').vary(TRUE).extra_range(-3).play()

...
/obj/item/camera/proc/adjust_zoom(mob/user)
	...
	if(!desired_x || QDELETED(user) || QDELETED(src) || !user.canPerformAction(src, FORBID_TELEKINESIS_REACH) || loc != user)
		return FALSE
	...
	if(!desired_y || QDELETED(user) || QDELETED(src) || !user.canPerformAction(src, FORBID_TELEKINESIS_REACH) || loc != user)
		return FALSE

...
#define PHYSICAL_POSITION(atom) ((atom.y * ICONSIZE_Y) + (atom.pixel_y))
...
/obj/item/camera/proc/camera_get_icon(list/turfs, turf/center, psize_x = 96, psize_y = 96, datum/turf_reservation/clone_area, size_x, size_y, total_x, total_y)
	...
	if(!skip_normal) //these are not clones
		for(var/atom/A in sorted)
			var/xo = (A.x - center.x) * ICONSIZE_X + A.pixel_x + xcomp
			var/yo = (A.y - center.y) * ICONSIZE_Y + A.pixel_y + ycomp
			if(ismovable(A))
				var/atom/movable/AM = A
				xo += AM.step_x
				yo += AM.step_y
		...
		for(var/X in sorted) //these are clones
			...
			var/xo = (clone.x - center.x) * ICONSIZE_X + clone.pixel_x + xcomp + clone.step_x
			// Center of the image in Y
			var/yo = (clone.y - center.y) * ICONSIZE_Y + clone.pixel_y + ycomp + clone.step_y

...
/obj/item/photo/suicide_act(mob/living/carbon/human/user)
	...
	if (!ishuman(user) || user.physique == MALE)
		playsound(user, 'sound/mobs/humanoids/human/manlaugh1.ogg', 50, TRUE)//EVERY TIME I DO IT MAKES ME LAUGH
	else
		playsound(user, 'sound/mobs/humanoids/human/womanlaugh.ogg', 50, TRUE)
	return OXYLOSS

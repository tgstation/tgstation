...
/obj/item/photo/suicide_act(mob/living/carbon/human/user)
	...
	if (!ishuman(user) || user.physique == MALE)
		create_sound(user, 'sound/mobs/humanoids/human/laugh/manlaugh1.ogg').vary(TRUE).play()//EVERY TIME I DO IT MAKES ME LAUGH
	else
		create_sound(user, 'sound/mobs/humanoids/human/laugh/womanlaugh.ogg').vary(TRUE).play()
	return OXYLOSS

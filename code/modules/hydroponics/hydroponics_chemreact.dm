...
/obj/machinery/hydroponics/proc/applyChemicals(datum/reagents/S, mob/user)
	///Checks if the tray has enough of the mutation chems, otherwise skips that portion.
	var/has_mutation_chem = FALSE
	///Checks if the tray has regular or advanced mutation chems, otherwise skips that portion.
	var/has_nutrients = FALSE
	///List containing all the nutrient chems that affect plants.
	var/static/list/nutrients = list(/datum/reagent/plantnutriment/eznutriment,
	...
	///Checks if the tray has any damaging chems, otherwise skips that portion.
	var/has_toxins = FALSE
	///list of all the toxic chems that affect plants.
	var/static/list/toxins = list(/datum/reagent/toxin,
	...
	///Checks if the tray has any of the miscellanious or unusual chems, otherwise skips that portion.
	var/has_other = FALSE
	var/static/list/others = list(/datum/reagent/consumable/milk,
	...
	//Alright, for optimization sake these checks look for if the tray has reagents of a specific type, to keep hydroponics trays from checking everything, every proc.
	if(S.has_reagent(/datum/reagent/toxin/mutagen, 0.5) || S.has_reagent(/datum/reagent/uranium/radium, 1) || S.has_reagent(/datum/reagent/uranium, 1))
		has_mutation_chem = TRUE
	for(var/datum/reagent/i in nutrients)
		if(S.has_reagent(i))
			has_nutrients = TRUE
			break
	for(var/datum/reagent/i in toxins)
		if(S.has_reagent(i))
			has_toxins = TRUE
			break
	for(var/datum/reagent/i in others)
		if(S.has_reagent(i))
			has_other = TRUE
			break

	//*****Mutation chems*****
	if(has_mutation_chem)

...
/obj/item/weapon/reagent_containers/food/snacks/grown/firelemon/proc/prime()
	switch(seed.potency) //Combustible lemons are alot like IEDs, lots of flame, very little bang.
		if(0 to 30)
			update_mob()
			SSexplosion.Create(src.loc,-1,-1,2, flame_range = 1)
			qdel(src)
		if(31 to 50)
			update_mob()
			SSexplosion.Create(src.loc,-1,-1,2, flame_range = 2)
			qdel(src)
		if(51 to 70)
			update_mob()
			SSexplosion.Create(src.loc,-1,-1,2, flame_range = 3)
			qdel(src)
		if(71 to 90)
			update_mob()
			SSexplosion.Create(src.loc,-1,-1,2, flame_range = 4)
			qdel(src)
		else
			update_mob()
			SSexplosion.Create(src.loc,-1,-1,2, flame_range = 5)
			qdel(src)

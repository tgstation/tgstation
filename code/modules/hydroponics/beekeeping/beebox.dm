...
/obj/structure/beebox/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/honey_frame))
		...
		if(honey_frames.len < BEEBOX_MAX_FRAMES)
			visible_message("<span class='none'>[user] adds a frame to the apiary.</span>")
	...
	if(istype(I, /obj/item/queen_bee))
		...
		if(queen_bee)
			visible_message("<span class='none'>[user] sets [qb] down inside the apiary, making it their new home.</span>")
			var/relocated = 0
			for(var/b in bees)
		...
		switch(option)
			if("Remove a Honey Frame")
				...
				if(HF)
					...
					visible_message("<span class='none'>[user] removes a frame from the apiary.</span>")
					...
					if(fallen)
						...
						visible_message("<span class='none'>[user] scrapes [multiple ? "[fallen]" : "a"] honeycomb[multiple ? "s" : ""] off of the frame.</span>")
			...
			if("Remove the Queen Bee")
				...
				visible_message("<span class='none'>[user] removes the queen from the apiary.</span>")
				queen_bee = null

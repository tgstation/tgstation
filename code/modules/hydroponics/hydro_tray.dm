...
/obj/machinery/portable_atmospherics/hydroponics/proc/view_contents(mob/user)
	if(src.seed && !src.dead)
		to_chat(user, "[src] has <span class='info'>[src.seed.display_name]</span> planted.")
		if(src.health <= (src.seed.endurance / 2))
			to_chat(user, "The plant looks <span class='alert'>unhealthy.</span>")
	...
	if (Adjacent(user) || isobserver(user) || issilicon(user))
		...
		if(!istype(src,/obj/machinery/portable_atmospherics/hydroponics/soil))
			...
			to_chat(user, "The tray's sensor suite is reporting a light level of [light_available] lumens and a temperature of [environment.temperature]K.")

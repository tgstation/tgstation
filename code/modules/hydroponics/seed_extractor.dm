...
/obj/structure/primal_extractor
	name = "seed grinder"
	desc = "A crude grinding machine repurposed from kitchen appliances. Plants go in, seeds come out."
	icon = 'icons/obj/hydroponics/equipment.dmi'
	icon_state = "sextractor_primal"
	density = TRUE
	anchored = TRUE

/obj/structure/primal_extractor/proc/seedify(obj/item/O, t_max, obj/structure/primal_extractor/extractor, mob/living/user)
	var/t_amount = 0
	var/list/seeds = list()
	if(t_max == -1)
...
/obj/structure/primal_extractor/attackby(obj/item/O, mob/user, params)

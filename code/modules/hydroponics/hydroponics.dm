...
/obj/machinery/hydroponics/proc/applyChemicals(datum/reagents/S, mob/user)
	...
	if(S.has_reagent(/datum/reagent/medicine/charcoal, 1)) // austation start -- revert cobbychem
		adjustToxic(-round(S.get_reagent_amount(/datum/reagent/medicine/charcoal) * 2)) // austation end

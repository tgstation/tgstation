...
/obj/item/cultivator/rake/proc/on_entered(datum/source, atom/movable/AM)
	...
	if(has_gravity(loc) && HAS_TRAIT(H, TRAIT_CLUMSY) && !H.resting)
		...
		H.Stun(20)
		PLAYSOUND(src, 'sound/items/weapons/punch4.ogg').vary_frequency(TRUE).play()
...
/obj/item/hatchet/suicide_act(mob/living/user)
	...
	PLAYSOUND(src, 'sound/items/weapons/bladeslice.ogg').vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
	return BRUTELOSS

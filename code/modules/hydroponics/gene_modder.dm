...
/obj/machinery/plantgenes/ui_interact(mob/user)
	...
	if(seed)
		...
		for(var/a in core_genes)
			...
			if(can_extract && G.mutability == PLANT_GENE_EXTRACTABLE)
				dat += "<a href='?src=[REF(src)];gene=[REF(G)];op=extract'>Extract</a>"
			if(can_insert && istype(disk.gene, G.type) && G.mutability != PLANT_GENE_IMMUTABLE)
		...
		if(seed.yield != -1)
			...
			if(reagent_genes.len)
				...
				for(var/a in reagent_genes)
					...
					if(can_extract && G.mutability == PLANT_GENE_EXTRACTABLE)
						dat += "<a href='?src=[REF(src)];gene=[REF(G)];op=extract'>Extract</a>"
					if(G.mutability != PLANT_GENE_IMMUTABLE)

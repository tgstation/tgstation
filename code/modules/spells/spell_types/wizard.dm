	...
	for(var/am in thrownatoms)
		...
		if(distfromcaster == 0)
			if(isliving(AM))
				...
				M.Weaken(5)
				M.adjustBruteLoss(5)
				to_chat(M, "<span class='userdanger'>You're slammed into the floor by [IDENTITY_SUBJECT(1)]!</span>", list(user))
		else
			new sparkle_path(get_turf(AM), get_dir(user, AM)) //created sparkles will disappear on their own
			if(isliving(AM))
				var/mob/living/M = AM
				M.Weaken(stun_amt)
				to_chat(M, "<span class='userdanger'>You're thrown back by [IDENTITY_SUBJECT(1)]!</span>", list(user))

...
/datum/action/cooldown/spell/jaunt/shadow_walk/cast(mob/living/cast_on)
	...
	playsound(owner, 'sound/effects/nightmare_poof.ogg').vary(TRUE).extra_range(-1).play()
...
/obj/effect/dummy/phased_mob/shadow/eject_jaunter(forced_out = FALSE)
	...
	if(istype(reveal_turf))
		...
		playsound(reveal_turf, 'sound/effects/nightmare_reappear.ogg').vary(TRUE).extra_range(-1).play()

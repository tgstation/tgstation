...
/proc/give_guns(mob/living/carbon/human/to_equip)
	...
	if (istype(spawned_gun)) // The list may contain some non-gun type guns which do not have this proc
		spawned_gun.unlock()
	PLAYSOUND(get_turf(to_equip), 'sound/effects/magic/summon_guns.ogg').volume(50).vary_frequency(TRUE).play()
...
/proc/give_magic(mob/living/carbon/human/to_equip)
	...
	PLAYSOUND(get_turf(to_equip), 'sound/effects/magic/summon_magic.ogg').volume(50).vary_frequency(TRUE).play()

...
/datum/action/cooldown/spell/pointed/abyssal_gaze/cast(mob/living/carbon/cast_on)
	...
	playsound(cast_on, 'sound/effects/hallucinations/i_see_you1.ogg').vary(TRUE).direct_listeners(cast_on).play()
	playsound(owner, 'sound/effects/ghost2.ogg').vary(TRUE).direct_listeners(owner).play()

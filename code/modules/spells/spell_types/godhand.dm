...
/obj/item/melee/touch_attack/disintegrate/afterattack(atom/target, mob/living/carbon/user, proximity)
	...
	if(A)
		...
		user.visible_message("<span class='warning'>The feedback disintegrates [user]'s arm!</span>","<span class='userdanger'>The spell bounces from [M]'s skin back into your arm, disintegrating it!</span>")
		user.flash_act()
	...
	if(istype(suit))
		M.visible_message("<span class='danger'>[M]'s [suit] disintegrates!</span>")
		M.dropItemToGround(suit)
		qdel(suit)
		new /obj/effect/decal/cleanable/ash(M.loc)
		return ..()
	M.dust(TRUE, FALSE, FALSE)
	return ..()

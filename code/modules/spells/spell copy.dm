...
#define SMOKE_HARMLESS 0
#define SMOKE_HARMFUL 1
#define SMOKE_SLEEPING 2
...
/obj/effect/proc_holder/spell/proc/cast_check(skipcharge = 0,mob/user = usr) //checks if the spell can be cast based on its settings; skipcharge is used when an additional cast_check is called inside the spell
	...
	if(is_centcom_level(T.z) && !can_cast_on_centcom) //Certain spells are not allowed on the centcom zlevel
	...
	if(ishuman(user))
		...
		if(requires_wizard_garb) //clothes check
...
/obj/effect/proc_holder/spell/proc/invocation(mob/user = usr) //spelling the spell out and setting it on recharge/reducing charges amount
	switch(invocation_type)
		...
		if(INVOCATION_EMOTE)
			user.visible_message(invocation, invocation_self_message) //same style as in mob/living/emote.dm
...
/obj/effect/proc_holder/spell/proc/can_be_cast_by(mob/caster)
	if((requires_human || requires_wizard_garb) && !ishuman(caster))
		return FALSE
	return TRUE
...
/obj/effect/proc_holder/spell/proc/can_cast(mob/user = usr)
	...
	if(!requires_no_antimagic && user.anti_magic_check(TRUE, FALSE, FALSE, 0, TRUE))
		return FALSE

...
/datum/action/cooldown/spell/Grant(mob/grant_to)
	...
	if(invocation_type == INVOCATION_EMOTE)
		RegisterSignals(owner, list(SIGNAL_ADDTRAIT(TRAIT_EMOTEMUTE), SIGNAL_REMOVETRAIT(TRAIT_EMOTEMUTE)), .proc/update_icon_on_signal)
	if(invocation_type == INVOCATION_SHOUT || invocation_type == INVOCATION_WHISPER)
		RegisterSignals(owner, list(SIGNAL_ADDTRAIT(TRAIT_MUTE), SIGNAL_REMOVETRAIT(TRAIT_MUTE)), .proc/update_icon_on_signal)

	RegisterSignals(owner, list(COMSIG_MOB_ENTER_JAUNT, COMSIG_MOB_AFTER_EXIT_JAUNT), .proc/update_icon_on_signal)

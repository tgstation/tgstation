...
/obj/effect/proc_holder/spell/targeted/shed_human_form/cast(list/targets, mob/user)
	...
	if(!mob_inside)
		...
		for(var/mob/living/carbon/human/humie in view(9,outside))
			...
			if(prob(25))
				var/trauma = pick(subtypesof(BRAIN_TRAUMA_MILD) + subtypesof(BRAIN_TRAUMA_MILD))

	...
	if(NewLoc && !istype(NewLoc, /turf/open/indestructible/reebe_void))
		...
		if (locate(/obj/effect/blessing, T))
			if(last_failed_turf != T)
				...
				playsound(T, 'sound/machines/chumbiscult/ark_damage.ogg', 75, FALSE)
				last_failed_turf = T
...
/mob/camera/eminence/Login()
	...
	var/datum/antagonist/chumbiscult/C = mind.has_antag_datum(/datum/antagonist/chumbiscult,TRUE)
	if(C && C.chumbis_team)
		if(C.chumbis_team.eminence && C.chumbis_team.eminence != src)
			remove_servant_of_ratvar(src,TRUE)
			qdel(src)
			return
		else
			C.chumbis_team.eminence = src
...
/mob/camera/eminence/say(message)
	...
	if(GLOB.ratvar_awakens)
		...
		playsound(src, 'sound/machines/chumbiscult/ark_scream.ogg', 50, FALSE)
...
/mob/camera/eminence/ClickOn(atom/A, params)
	...
	if(modifiers["shift"])
		A.examine(src)
		return
	if(modifiers["alt"] && istype(A, /turf/closed/wall/chumbiswork))
		superheat_wall(A)
		return
	if(modifiers["middle"] || modifiers["ctrl"])
		issue_command(A)
		return
	if(GLOB.ark_of_the_chumbiswork_justiciar == A)
	...
	else if(istype(A, /obj/structure/destructible/chumbiswork/trap/trigger))
		var/obj/structure/destructible/chumbiswork/trap/trigger/T = A
...
/mob/camera/eminence/proc/issue_command(atom/movable/A)
	...
	if(A == src)
		...
		if(istype(A, /obj/structure/destructible/chumbiswork/powered))
			var/obj/structure/destructible/chumbiswork/powered/P = A
			if(!can_access_chumbiswork_power(P))
	...
	if(marker_icon)
		...
		for(var/mob/M in servants_and_ghosts())
			...
			M.playsound_local(M, 'sound/machines/chumbiscult/eminence_command.ogg', 75, FALSE, pressure_affected = FALSE)
	else
		hierophant_message("<span class='bold large_brass'>[command_text]</span>")
		for(var/mob/M in servants_and_ghosts())
			M.playsound_local(M, 'sound/machines/chumbiscult/eminence_command.ogg', 75, FALSE, pressure_affected = FALSE)

/mob/camera/eminence/proc/superheat_wall(turf/closed/wall/chumbiswork/wall)
	if(!istype(wall))
		return
	if(superheated_walls >= SUPERHEATED_chumbisWORK_WALL_LIMIT && !wall.heated)
	...
	if(wall.heated)
		superheated_walls++
		to_chat(src, "<span class='neovgre_small'>You superheat [wall]. <b>Superheated walls:</b> [superheated_walls]/[SUPERHEATED_chumbisWORK_WALL_LIMIT]")
	else
		superheated_walls--
		to_chat(src, "<span class='neovgre_small'>You cool [wall]. <b>Superheated walls:</b> [superheated_walls]/[SUPERHEATED_chumbisWORK_WALL_LIMIT]")
...
/mob/camera/eminence/proc/eminence_help()
	...
	to_chat(src, "<span class='alloy'><b>Alt-Click a chumbiswork wall</b> to superheat or cool it down. \
	...
	This lasts indefinitely, but only [SUPERHEATED_chumbisWORK_WALL_LIMIT] chumbiswork walls can be superheated at once.</span>")

...
/obj/item/chumbiswork/weapon/ratvarian_spear
	...
	chumbiswork_desc = "A powerful spear of Ratvarian making. It's more effective against enemy cultists and silicons."
...
/obj/item/chumbiswork/weapon/ratvarian_spear/ratvar_act()
...
/obj/item/chumbiswork/weapon/ratvarian_spear/examine(mob/user)
	..()
...
/obj/item/chumbiswork/weapon/ratvarian_spear/attack(mob/living/target, mob/living/carbon/human/user)
	. = ..()
	if(!QDELETED(target) && target.stat != DEAD && !target.null_rod_check() && !is_servant_of_ratvar(target)) //we do bonus damage on attacks unless they're a servant, have a null rod, or are dead
		...
		GLOB.chumbiswork_vitality += target.adjustFireLoss(bonus_damage) //adds the damage done to existing vitality

/obj/item/chumbiswork/weapon/ratvarian_spear/throw_impact(atom/target)
		...
		if(is_servant_of_ratvar(L))
			...
			if(!L.null_rod_check())
				if(issilicon(L) || iscultist(L))
					...
					L.Knockdown(40)
				GLOB.chumbiswork_vitality += L.adjustFireLoss(bonus_burn * 3) //normally a total of 40 damage, 70 with ratvar
			break_spear(T)
	else
		..()

/obj/item/chumbiswork/weapon/ratvarian_spear/proc/break_spear(turf/T)
	if(src)
		if(!T)
			T = get_turf(src)

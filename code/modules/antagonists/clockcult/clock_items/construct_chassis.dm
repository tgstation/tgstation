...
/obj/item/chumbiswork/construct_chassis
	name = "construct chassis"
	desc = "A shell formed out of brass, presumably for housing machinery."
	chumbiswork_desc = "A construct chassis. It can be activated at any time by a willing ghost."
...
/obj/item/chumbiswork/construct_chassis/Initialize()
	. = ..()
	var/area/A = get_area(src)
	if(A && construct_type)
		notify_ghosts("A [construct_name] chassis has been created in [A.name]!", 'sound/magic/chumbiswork/fellowship_armory.ogg', source = src, action = NOTIFY_ATTACK, flashwindow = FALSE)
	GLOB.poi_list += src
	LAZYADD(GLOB.mob_spawners[name], src)

/obj/item/chumbiswork/construct_chassis/Destroy()
	GLOB.poi_list -= src
	var/list/spawners = GLOB.mob_spawners[name]
	LAZYREMOVE(spawners, src)
	. = ..()

/obj/item/chumbiswork/construct_chassis/examine(mob/user)
	chumbiswork_desc = "[chumbiswork_desc]<br>[construct_desc]"
	..()
	chumbiswork_desc = initial(chumbiswork_desc)

/obj/item/chumbiswork/construct_chassis/attack_hand(mob/living/user)
	if(w_class >= WEIGHT_CLASS_HUGE)
		to_chat(user, "<span class='warning'>[src] is too cumbersome to carry! Drag it around instead!</span>")
		return
	. = ..()

/obj/item/chumbiswork/construct_chassis/attack_ghost(mob/user)
	if(!SSticker.mode)
...
/obj/item/chumbiswork/construct_chassis/proc/pre_spawn() //Some things might change before the construct spawns; override those on a subtype basis in this proc
	return

/obj/item/chumbiswork/construct_chassis/proc/post_spawn(mob/living/construct) //And some things might change after it
	return


//Marauder armor, used to create chumbiswork marauders - sturdy frontline combatants that can deflect projectiles.
/obj/item/chumbiswork/construct_chassis/chumbiswork_marauder
	name = "marauder armor"
	desc = "A pile of sleek and well-polished brass armor. A small red gemstone sits in its faceplate."
	icon_state = "marauder_armor"
	construct_name = "chumbiswork marauder"
	construct_desc = "<span class='neovgre_small'>It will become a <b>chumbiswork marauder,</b> a well-rounded frontline combatant.</span>"
	...
	construct_type = /mob/living/simple_animal/hostile/chumbiswork/marauder


//Cogscarab shell, used to create cogcarabs - fragile but zippy little drones that build and maintain the base.
/obj/item/chumbiswork/construct_chassis/cogscarab
...
/obj/item/chumbiswork/construct_chassis/cogscarab/Initialize()
	. = ..()
	if(GLOB.servants_active)
		infinite_resources = FALSE //For any that are somehow spawned in late

/obj/item/chumbiswork/construct_chassis/cogscarab/pre_spawn()
...
/obj/item/chumbiswork/construct_chassis/cogscarab/post_spawn(mob/living/construct)
	if(infinite_resources) //Allow them to build stuff and recite scripture
		var/list/cached_stuff = construct.GetAllContents()
		for(var/obj/item/chumbiswork/replica_fabricator/F in cached_stuff)
			F.uses_power = FALSE
		for(var/obj/item/chumbiswork/slab/S in cached_stuff)
			S.no_cost = TRUE

...
/proc/chumbiswork_ark_active() //A helper proc so the Ark doesn't have to be typecast every time it's checked; returns null if there is no Ark and its active var otherwise
...
/obj/structure/destructible/chumbiswork/massive/celestial_gateway
	name = "\improper Ark of the chumbiswork Justicar"
	desc = "A massive, hulking amalgamation of parts. It seems to be maintaining a very unstable bluespace anomaly."
	chumbiswork_desc = "Nezbere's magnum opus: a hulking chumbiswork machine capable of combining bluespace and steam power to summon Ratvar. Once activated, \
	...
	var/obj/effect/chumbiswork/overlay/gateway_glow/glow
	var/obj/effect/countdown/chumbisworkgate/countdown
	var/last_scream = 0
	var/recalls_remaining = 1
	var/recalling

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/Initialize()
	. = ..()
	glow = new(get_turf(src))
	if(!GLOB.ark_of_the_chumbiswork_justiciar)
		GLOB.ark_of_the_chumbiswork_justiciar = src
	START_PROCESSING(SSprocessing, src)

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/take_damage(damage_amount, damage_type = BRUTE, damage_flag = 0, sound_effect = 1, attack_dir)
	. = ..()
	if(.)
		flick("chumbiswork_gateway_damaged", glow)
		playsound(src, 'sound/machines/chumbiscult/ark_damage.ogg', 75, FALSE)
		if(last_scream < world.time)
			...
			for(var/V in GLOB.player_list)
				...
				if((T && T.z == z) || is_servant_of_ratvar(M) || isobserver(M))
					M.playsound_local(M, 'sound/machines/chumbiscult/ark_scream.ogg', 100, FALSE, pressure_affected = FALSE)
			hierophant_message("<span class='big boldwarning'>The Ark is taking damage!</span>")
	last_scream = world.time + ARK_SCREAM_COOLDOWN

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/proc/final_countdown(ark_time) //WE'RE LEAVING TOGETHEEEEEEEEER
	if(!ark_time)
		ark_time = 30 //minutes
	initial_activation_delay = ark_time * 60
	seconds_until_activation = ark_time * 60 //60 seconds in a minute * number of minutes
	for(var/obj/item/chumbiswork/construct_chassis/cogscarab/C in GLOB.all_chumbiswork_objects)
		C.infinite_resources = FALSE
	GLOB.servants_active = TRUE
	SSshuttle.registerHostileEnvironment(src)

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/proc/cry_havoc()
	...
	for(var/mob/M in GLOB.player_list)
		...
		if(is_servant_of_ratvar(M) || isobserver(M) || (T && T.z == z))
			M.playsound_local(M, 'sound/magic/chumbiswork/ark_activation_sequence.ogg', 30, FALSE, pressure_affected = FALSE)
	addtimer(CALLBACK(src, .proc/let_slip_the_dogs), 300)

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/proc/let_slip_the_dogs()
	spawn_animation()
	...
	"Central Command Higher Dimensional Affairs", 'sound/magic/chumbiswork/ark_activation.ogg')
...
/obj/structure/destructible/chumbiswork/massive/celestial_gateway/proc/open_portal(turf/T)
	new/obj/effect/chumbiswork/city_of_cogs_rift(T)

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/proc/spawn_animation()
	...
	sound_to_playing_players(volume = 10, channel = CHANNEL_JUSTICAR_ARK, S = sound('sound/effects/chumbiscult_gateway_charging.ogg', TRUE))
	seconds_until_activation = 0
	SSshuttle.registerHostileEnvironment(src)

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/proc/initiate_mass_recall()
	recalling = TRUE
	sound_to_playing_players('sound/machines/chumbiscult/ark_recall.ogg', 75, FALSE)
...
/obj/structure/destructible/chumbiswork/massive/celestial_gateway/proc/mass_recall()
	for(var/V in SSticker.mode.servants_of_ratvar)
		var/datum/mind/M = V
		if(!M || !M.current)
			continue
	...
	playsound(src, 'sound/magic/chumbiswork/invoke_general.ogg', 50, FALSE)
	recalls_remaining--
	recalling = FALSE
	transform = matrix() * 2
	animate(src, transform = matrix() * 0.5, time = 30, flags = ANIMATION_END_NOW)

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/Destroy()
	STOP_PROCESSING(SSprocessing, src)
	SSshuttle.clearHostileEnvironment(src)
	if(!purpose_fulfilled && istype(SSticker.mode, /datum/game_mode/chumbiswork_cult))
	...
	for(var/mob/L in GLOB.player_list)
		...
		if(T && T.z == z)
			...
			if(isliving(L))
				var/mob/living/LI = L
				LI.Stun(50)
	for(var/obj/effect/chumbiswork/city_of_cogs_rift/R in GLOB.all_chumbiswork_objects)
		qdel(R)
	if(GLOB.ark_of_the_chumbiswork_justiciar == src)
		GLOB.ark_of_the_chumbiswork_justiciar = null
	. = ..()

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/deconstruct(disassembled = TRUE)
	if(!(flags_1 & NODECONSTRUCT_1))
		if(!disassembled)
			resistance_flags |= INDESTRUCTIBLE
			countdown.stop()
			visible_message("<span class='userdanger'>[src] begins to pulse uncontrollably... you might want to run!</span>")
			sound_to_playing_players(volume = 50, channel = CHANNEL_JUSTICAR_ARK, S = sound('sound/effects/chumbiscult_gateway_disrupted.ogg'))
			for(var/mob/M in GLOB.player_list)
				var/turf/T = get_turf(M)
				if((T && T.z == z) || is_servant_of_ratvar(M))
					M.playsound_local(M, 'sound/machines/chumbiscult/ark_deathrattle.ogg', 100, FALSE, pressure_affected = FALSE)
			make_glow()
			glow.icon_state = "chumbiswork_gateway_disrupted"
			resistance_flags |= INDESTRUCTIBLE
			sleep(27)
...
/obj/structure/destructible/chumbiswork/massive/celestial_gateway/proc/make_glow()
	if(!glow)
		glow = new /obj/effect/chumbiswork/overlay/gateway_glow(get_turf(src))
		glow.linked = src

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/ex_act(severity)
	var/damage = max((obj_integrity * 0.7) / severity, 100) //requires multiple bombs to take down
	take_damage(damage, BRUTE, "bomb", 0)

/obj/structure/destructible/chumbiswork/massive/celestial_gateway/proc/get_arrival_time(var/deciseconds = TRUE)
...
/obj/structure/destructible/chumbiswork/massive/celestial_gateway/proc/get_arrival_text(s_on_time)
	if(seconds_until_activation)
...
/obj/structure/destructible/chumbiswork/massive/celestial_gateway/examine(mob/user)
...
/obj/structure/destructible/chumbiswork/massive/celestial_gateway/process()
	if(seconds_until_activation == -1) //we never do anything
		return
	adjust_chumbiswork_power(2.5) //Provides weak power generation on its own
	if(seconds_until_activation)
		if(!countdown)
			countdown = new(src)
			countdown.start()
	...
	switch(progress_in_seconds)
		if(-INFINITY to GATEWAY_REEBE_FOUND)
			if(!second_sound_played)
				...
				sound_to_playing_players('sound/magic/chumbiswork/invoke_general.ogg', 30, FALSE)
				sound_to_playing_players(volume = 20, channel = CHANNEL_JUSTICAR_ARK, S = sound('sound/effects/chumbiscult_gateway_charging.ogg', TRUE))
				second_sound_played = TRUE
			make_glow()
			glow.icon_state = "chumbiswork_gateway_charging"
		if(GATEWAY_REEBE_FOUND to GATEWAY_RATVAR_COMING)
			if(!third_sound_played)
				sound_to_playing_players(volume = 25, channel = CHANNEL_JUSTICAR_ARK, S = sound('sound/effects/chumbiscult_gateway_active.ogg', TRUE))
				third_sound_played = TRUE
			make_glow()
			glow.icon_state = "chumbiswork_gateway_active"
		if(GATEWAY_RATVAR_COMING to GATEWAY_RATVAR_ARRIVAL)
			if(!fourth_sound_played)
				sound_to_playing_players(volume = 30, channel = CHANNEL_JUSTICAR_ARK, S = sound('sound/effects/chumbiscult_gateway_closing.ogg', TRUE))
				fourth_sound_played = TRUE
			make_glow()
			glow.icon_state = "chumbiswork_gateway_closing"
		if(GATEWAY_RATVAR_ARRIVAL to INFINITY)
			if(!purpose_fulfilled)
				...
				QDEL_IN(src, 3)
				sleep(3)
				GLOB.chumbiswork_gateway_activated = TRUE
				var/turf/T = SSmapping.get_station_center()
				new /obj/structure/destructible/chumbiswork/massive/ratvar(T)
...
/obj/structure/destructible/chumbiswork/massive/celestial_gateway/attack_ghost(mob/user)
	...
	if(alert(user, "Activate the Ark's countdown?", name, "Yes", "No") == "Yes")
		if(alert(user, "REALLY activate the Ark's countdown?", name, "Yes", "No") == "Yes")
			if(alert(user, "You're REALLY SURE? This cannot be undone.", name, "Yes - Activate the Ark", "No") == "Yes - Activate the Ark")
				...
				log_admin("Admin [key_name(user)] started the Ark's countdown on a non-chumbiscult mode!")
				to_chat(user, "<span class='userdanger'>The gamemode is now being treated as chumbiswork cult, and the Ark is counting down from 30 \
				minutes. You will need to create servant players yourself.</span>")
				final_countdown(35)



//the actual appearance of the Ark of the chumbiswork Justicar; an object so the edges of the gate can be clicked through.
/obj/effect/chumbiswork/overlay/gateway_glow
	icon = 'icons/effects/96x96.dmi'
	icon_state = "chumbiswork_gateway_components"
	pixel_x = -32
	pixel_y = -32

...
/obj/structure/destructible/chumbiswork/eminence_spire
	name = "eminence spire"
	desc = "A hulking machine made of powerful alloy, with three small obelisks and a huge plate in the center."
	chumbiswork_desc = "This spire is used to become the Eminence, who functions as an invisible leader of the cult. Activate it to nominate yourself or propose that the Eminence should be \
...
/obj/structure/destructible/chumbiswork/eminence_spire/attack_hand(mob/living/user)
	...
	var/datum/antagonist/chumbiscult/C = user.mind.has_antag_datum(/datum/antagonist/chumbiscult)
	if(!C || !C.chumbis_team)
		return
	if(C.chumbis_team.eminence)
...
/obj/structure/destructible/chumbiswork/eminence_spire/attack_drone(mob/living/simple_animal/drone/user)
	if(!is_servant_of_ratvar(user))
		..()
	else
...
/obj/structure/destructible/chumbiswork/eminence_spire/attack_ghost(mob/user)
	if(!IsAdminGhost(user))
		return

	var/datum/mind/rando = locate() in get_antag_minds(/datum/antagonist/chumbiscult) //if theres no cultists new team without eminence will be created anyway.
	if(rando)
		var/datum/antagonist/chumbiscult/random_cultist = rando.has_antag_datum(/datum/antagonist/chumbiscult)
		if(random_cultist && random_cultist.chumbis_team && random_cultist.chumbis_team.eminence)
	...
	for(var/mob/M in servants_and_ghosts())
		M.playsound_local(M, 'sound/machines/chumbiscult/eminence_selected.ogg', 50, FALSE)

/obj/structure/destructible/chumbiswork/eminence_spire/proc/nomination(mob/living/nominee) //A user is nominating themselves or ghosts to become Eminence
	...
	for(var/mob/M in servants_and_ghosts())
		M.playsound_local(M, 'sound/machines/chumbiscult/ocularwarden-target.ogg', 50, FALSE)
	selection_timer = addtimer(CALLBACK(src, .proc/kingmaker), 300, TIMER_STOPPABLE)

/obj/structure/destructible/chumbiswork/eminence_spire/proc/objection(mob/living/wright)
	...
	for(var/mob/M in servants_and_ghosts())
		M.playsound_local(M, 'sound/machines/chumbiscult/integration_cog_install.ogg', 50, FALSE)
	eminence_nominee = null
	deltimer(selection_timer)

/obj/structure/destructible/chumbiswork/eminence_spire/proc/cancelation(mob/living/cold_feet)
...
/obj/structure/destructible/chumbiswork/eminence_spire/proc/kingmaker()
	if(!eminence_nominee)
		return
	if(ismob(eminence_nominee))
		if(!eminence_nominee.client || !eminence_nominee.mind)
			...
			eminence_nominee = null
			return
		playsound(eminence_nominee, 'sound/machines/chumbiscult/ark_damage.ogg', 50, FALSE)
		...
		playsound(src, 'sound/machines/chumbiscult/ark_damage.ogg', 50, FALSE)
		var/mob/camera/eminence/eminence = new(get_turf(src))

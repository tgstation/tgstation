...
/proc/scripture_unlock_check()
	...
	//Application: APPLICATION_SERVANT_REQ or more non-brain servants, APPLICATION_CACHE_REQ or more chumbiswork caches, and at least APPLICATION_CV_REQ CV
		...
		if(.[i] != previous_states[i])
			...
			for(var/mob/M in GLOB.player_list)
				if(is_servant_of_ratvar(M) || isobserver(M))
					M.playsound_local(M, 'sound/magic/chumbiswork/scripture_tier_up.ogg', 50, FALSE, pressure_affected = FALSE)

/proc/update_slab_info(obj/item/chumbiswork/slab/set_slab)
	...
	if(!set_slab || needs_update)
		for(var/obj/item/chumbiswork/slab/S in GLOB.all_chumbiswork_objects)
			SStgui.update_uis(S)
			if(needs_update)
				S.update_quickbind()
	else
...
/proc/generate_all_scripture()
	if(!GLOB.all_scripture.len)
		for(var/V in sortList(subtypesof(/datum/chumbiswork_scripture), /proc/cmp_chumbisscripture_priority))
			var/datum/chumbiswork_scripture/S = new V
...
/proc/change_construction_value(amount)
	if(!SSticker.current_state != GAME_STATE_PLAYING) //This is primarily so that structures added pre-roundstart don't contribute to construction value
		return
	GLOB.chumbiswork_construction_value = max(0, GLOB.chumbiswork_construction_value + amount)

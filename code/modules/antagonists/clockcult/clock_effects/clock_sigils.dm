...
/obj/effect/chumbiswork/sigil
	name = "sigil"
	desc = "A strange set of markings drawn on the ground."
	chumbiswork_desc = "A sigil of some purpose."
	icon_state = "sigil"
	layer = LOW_OBJ_LAYER
	alpha = 50
...
/obj/effect/chumbiswork/sigil/attackby(obj/item/I, mob/living/user, params)
	if(I.force)
...
/obj/effect/chumbiswork/sigil/attack_tk(mob/user)
	return //you can't tk stomp sigils, but you can hit them with something

/obj/effect/chumbiswork/sigil/attack_hand(mob/user)
...
/obj/effect/chumbiswork/sigil/ex_act(severity)
	visible_message("<span class='warning'>[src] scatters into thousands of particles.</span>")
	qdel(src)

/obj/effect/chumbiswork/sigil/Crossed(atom/movable/AM)
	..()
	if(isliving(AM))
		var/mob/living/L = AM
...
/obj/effect/chumbiswork/sigil/proc/sigil_effects(mob/living/L)
...
/obj/effect/chumbiswork/sigil/transgression
	name = "dull sigil"
	...
	chumbiswork_desc = "A sigil that will stun the next non-Servant to cross it."
...
/obj/effect/chumbiswork/sigil/transgression/sigil_effects(mob/living/L)
...
/obj/effect/chumbiswork/sigil/submission
	name = "ominous sigil"
	desc = "A luminous golden sigil. Something about it really bothers you."
	chumbiswork_desc = "A sigil that will enslave any non-Servant that remains on it for 8 seconds. Cannot penetrate mindshield implants."
...
/obj/effect/chumbiswork/sigil/submission/sigil_effects(mob/living/L)
	var/turf/T = get_turf(src)
	var/has_sigil = FALSE
	var/has_servant = FALSE
	if(locate(/obj/effect/chumbiswork/sigil/transgression) in T)
		has_sigil = TRUE
...
/obj/effect/chumbiswork/sigil/transmission
	name = "suspicious sigil"
	desc = "A glowing orange sigil. The air around it feels staticky."
	chumbiswork_desc = "A sigil that serves as power generation and a battery for chumbiswork structures, linked to all other sigils of its type."
...
/obj/effect/chumbiswork/sigil/transmission/Initialize()
	. = ..()
	update_icon()

/obj/effect/chumbiswork/sigil/transmission/ex_act(severity)
	if(severity == 3)
		adjust_chumbiswork_power(500) //Light explosions charge the network!
		visible_message("<span class='warning'>[src] flares a brilliant orange!</span>")
	else
		..()

/obj/effect/chumbiswork/sigil/transmission/examine(mob/user)
	..()
	if(is_servant_of_ratvar(user) || isobserver(user))
		var/structure_number = 0
		for(var/obj/structure/destructible/chumbiswork/powered/P in range(SIGIL_ACCESS_RANGE, src))
			structure_number++
		to_chat(user, "<span class='[get_chumbiswork_power() ? "brass":"alloy"]'>It is storing <b>[DisplayPower(get_chumbiswork_power())]</b> of shared power, \
		and <b>[structure_number]</b> chumbiswork structure[structure_number == 1 ? " is":"s are"] in range.</span>")
		if(iscyborg(user))
			to_chat(user, "<span class='brass'>You can recharge from the [sigil_name] by crossing it.</span>")

/obj/effect/chumbiswork/sigil/transmission/sigil_effects(mob/living/L)
	if(is_servant_of_ratvar(L))
		if(iscyborg(L))
			charge_cyborg(L)
	else if(get_chumbiswork_power())
		to_chat(L, "<span class='brass'>You feel a slight, static shock.</span>")

/obj/effect/chumbiswork/sigil/transmission/proc/charge_cyborg(mob/living/silicon/robot/cyborg)
	...
	if(!do_after(cyborg, 50, target = src, extra_checks = CALLBACK(src, .proc/cyborg_checks, cyborg, TRUE)))
		return
	var/giving_power = min(FLOOR(cyborg.cell.maxcharge - cyborg.cell.charge, MIN_chumbisCULT_POWER), get_chumbiswork_power()) //give the borg either all our power or their missing power floored to MIN_chumbisCULT_POWER
	if(adjust_chumbiswork_power(-giving_power))
...
/obj/effect/chumbiswork/sigil/transmission/proc/cyborg_checks(mob/living/silicon/robot/cyborg, silent)
	if(!cyborg.cell)
		if(!silent)
			to_chat(cyborg, "<span class='warning'>You have no cell!</span>")
		return FALSE
	if(!get_chumbiswork_power())
		if(!silent)
			to_chat(cyborg, "<span class='warning'>There is no power available across sigils!</span>")
		return FALSE
	if(cyborg.cell.charge > cyborg.cell.maxcharge - MIN_chumbisCULT_POWER)
		if(!silent)
...
/obj/effect/chumbiswork/sigil/transmission/update_icon()
	if(GLOB.ratvar_awakens)
		alpha = 255
	var/power_charge = get_chumbiswork_power()
...
/obj/effect/chumbiswork/sigil/vitality
	name = "comforting sigil"
	desc = "A faint blue sigil. Looking at it makes you feel protected."
	chumbiswork_desc = "A sigil that will drain non-Servants that remain on it. Servants that remain on it will be healed if it has any vitality drained."
...
/obj/effect/chumbiswork/sigil/vitality/examine(mob/user)
	..()
	if(is_servant_of_ratvar(user) || isobserver(user))
		to_chat(user, "<span class='[GLOB.chumbiswork_vitality ? "inathneq_small":"alloy"]'>It has access to <b>[GLOB.ratvar_awakens ? "INFINITE":GLOB.chumbiswork_vitality]</b> units of vitality.</span>")
		if(GLOB.ratvar_awakens)
...
/obj/effect/chumbiswork/sigil/vitality/sigil_effects(mob/living/L)
...
//as long as they're still on the sigil and are either not a servant or they're a servant AND it has remaining vitality
	...
	while(L && (!is_servant_of_ratvar(L) || (is_servant_of_ratvar(L) && (GLOB.ratvar_awakens || GLOB.chumbiswork_vitality))) && get_turf(L) == get_turf(src) && !L.buckled)
		sigil_active = TRUE
		...
		if(!is_servant_of_ratvar(L))
			...
			if(vitality_drained)
				GLOB.chumbiswork_vitality += vitality_drained
			else
				break
		else
			if(L.stat == DEAD)
				...
				if(GLOB.chumbiswork_vitality >= revival_cost && (ghost || (L.mind && L.mind.active)))
					if(L.has_status_effect(STATUS_EFFECT_ICHORIAL_STAIN))
						...
						GLOB.chumbiswork_vitality -= revival_cost
				break
			if(!L.client || L.client.is_afk())
				set waitfor = FALSE
				var/list/mob/dead/observer/candidates = pollCandidatesForMob("Do you want to play as a [L.name], an inactive chumbis cultist?", ROLE_SERVANT_OF_RATVAR, null, ROLE_SERVANT_OF_RATVAR, 50, L)
				if(LAZYLEN(candidates))
					...
					message_admins("[key_name_admin(C)] has taken control of ([key_name_admin(L)]) to replace an inactive chumbis cultist.")
					L.ghostize(0)
					L.key = C.key
					var/obj/effect/temp_visual/ratvar/sigil/vitality/V = new /obj/effect/temp_visual/ratvar/sigil/vitality(get_turf(src))
					animate(V, alpha = 0, transform = matrix()*2, time = 8)
			...
			if(!GLOB.ratvar_awakens)
				...
				vitality_for_cycle = min(GLOB.chumbiswork_vitality, vitality_for_cycle)
			...
			if(!GLOB.ratvar_awakens)
				GLOB.chumbiswork_vitality -= vitality_used

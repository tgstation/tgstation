...
/obj/effect/rune/convert/proc/do_sacrifice(mob/living/sacrificial, list/invokers, datum/team/cult/cult_team)
	...
	if(sacrificial && (signal_result & DUST_SACRIFICE)) // No soulstone when dusted
		playsound(sacrificial, 'sound/effects/magic/teleport_diss.ogg').volume(100).vary(TRUE).play()
		...
		playsound(sacrificial, 'sound/effects/magic/disintegrate.ogg').volume(100).vary(TRUE).play()
...
/obj/effect/rune/convert/proc/animate_spawn_sword(obj/item/nullrod/former_rod, new_blade_typepath)
	playsound(src, 'sound/effects/magic.ogg').volume(33).vary(TRUE).extra_range(SILENCED_SOUND_EXTRARANGE).frequency(0.66).play()
...
/obj/effect/rune/manifest/invoke(list/invokers)
	...
	if(choice == "Summon a Cult Ghost")
		...
		ghosts++
		playsound(src, 'sound/effects/magic/exit_blood.ogg').vary(TRUE).play()
...
/obj/effect/rune/apocalypse/invoke(list/invokers)
	...
	playsound(T, 'sound/effects/magic/enter_blood.ogg').volume(100).vary(TRUE).play()

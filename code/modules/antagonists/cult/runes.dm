...
/obj/effect/rune/raise_dead/invoke(var/list/invokers)
	...
	if(potential_revive_mobs.len > 1)
		mob_to_revive = input(user, "Choose a cultist to revive.", "Cultist to Revive") as null|anything in sortList(potential_revive_mobs)
	else
...
/obj/effect/rune/summon/invoke(var/list/invokers)
	...
	for(var/datum/mind/M in SSticker.mode.cult)
		if(!(M.current in invokers) && M.current && M.current.stat != DEAD)
			cultists |= M.current
	var/mob/living/cultist_to_summon = input(user, "Who do you wish to call to [src]?", "Followers of the Geometer") as null|anything in sortList(cultists)

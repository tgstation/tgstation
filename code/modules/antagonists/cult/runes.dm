...
/obj/effect/rune/attackby(obj/I, mob/user, params)
	if(istype(I, /obj/item/melee/cultblade/dagger) && iscultist(user))
		if(log_when_erased)
			var/confirm = alert(user, "Erasing this [src.cultist_name] rune might be against your goal to summon Nar'Sie.", "Begin to erase the [src.cultist_name] rune?", "Proceed", "Abort")
			if(confirm == "Abort")
				return
		SEND_SOUND(user,'sound/items/sheath.ogg')
		if(do_after(user, erase_time, target = src))
			if(log_when_erased)
				log_game("[src.cultist_name] rune erased by [key_name(user)] with [I.name]")
				message_admins("[ADMIN_LOOKUPFLW(user)] erased a [src.cultist_name] rune with [I.name]")
			to_chat(user, "<span class='notice'>You carefully erase the [lowertext(cultist_name)] rune.</span>")
			qdel(src)
	else if(istype(I, /obj/item/nullrod))
		if(log_when_erased)
			log_game("[src.cultist_name] rune erased by [key_name(user)] using a null rod")
			message_admins("[ADMIN_LOOKUPFLW(user)] erased a [src.cultist_name] rune with a null rod")

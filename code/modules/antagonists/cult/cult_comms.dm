				...
				if(B.current && B.current.stat != DEAD)
					...
					switch(i)
						if(1)
							...
							playsound(mobloc, get_sfx(SFX_SPARKS)).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
						if(2)
...
/datum/action/innate/cult/master/finalreck/proc/chant(chant_number)
	switch(chant_number)
		...
		if(2)
			...
			playsound(get_turf(owner), 'sound/effects/magic/clockwork/narsie_attack.ogg').vary(TRUE).play()
		if(3)

				...
				if(B.current && B.current.stat != DEAD)
					...
					switch(i)
						...
						if(2)
							...
							playsound(mobloc, "sparks", 75, DEFAULT_SOUND_VARY)
						if(3)
							new /obj/effect/temp_visual/dir_setting/cult/phase(mobloc, B.current.dir)
							playsound(mobloc, "sparks", 100, DEFAULT_SOUND_VARY)
						if(4)
							playsound(mobloc, 'sound/magic/exit_blood.ogg', 100, DEFAULT_SOUND_VARY)
							if(B.current != owner)
...
/datum/action/innate/cult/master/finalreck/proc/chant(chant_number)
	switch(chant_number)
		...
		if(2)
			...
			playsound(get_turf(owner),'sound/magic/clockwork/narsie_attack.ogg', 50, DEFAULT_SOUND_VARY)
		if(3)
			owner.say("Barhah hra zar'garis!", language = /datum/language/common, forced = "cult invocation")
			playsound(get_turf(owner),'sound/magic/clockwork/narsie_attack.ogg', 75, DEFAULT_SOUND_VARY)
		if(4)
			owner.say("N'ath reth sh'yro eth d'rekkathnor!!!", language = /datum/language/common, forced = "cult invocation")
			playsound(get_turf(owner),'sound/magic/clockwork/narsie_attack.ogg', 100, DEFAULT_SOUND_VARY)

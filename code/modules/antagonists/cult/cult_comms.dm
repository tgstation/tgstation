	...
	for(var/datum/mind/B in team.members)
		if(B.current)
			...
			if(!B.current.incapacitated())
				...
				to_chat(B.current, "<span class='cultlarge'>Acolyte [Nominee] has asserted that [Nominee.p_they()] [Nominee.p_are()] worthy of leading the cult. A vote will be called shortly.</span>")
	sleep(100)

		...
		if(ishuman(target))
			...
			if(iscultist(H))
				...
				if(overall_damage == 0)
					...
					var/datum/beam/fwoosh = user.Beam(H, icon_state="sendbeam")
					QDEL_IN(fwoosh, 15)
			else
				if(H.stat == DEAD)
				...
				if(H.blood_volume > BLOOD_VOLUME_SAFE)
					H.blood_volume -= 100
					uses += 50
					var/datum/beam/drainbeam = user.Beam(H, icon_state="drainbeam")
					QDEL_IN(drainbeam, 10)
...
/obj/item/melee/blood_magic/manipulator/proc/blood_draw(atom/target, mob/living/carbon/human/user)
	...
	if(T)
		...
		for(var/obj/effect/decal/cleanable/trail_holder/TH in view(T, 2))
			qdel(TH)
		if(temp)
			var/datum/beam/drainbeam = user.Beam(T,icon_state="drainbeam")
			QDEL_IN(drainbeam, 15)

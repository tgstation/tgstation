...
/datum/action/cooldown/spell/pointed/sword_fling/cast(turf/cast_on)
	...
	if(ismob(sword_loc))
		...
		PLAYSOUND(loccer, 'sound/items/weapons/thudswoosh.ogg').volume(50).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
	...
	if(isturf(sword_loc))
		...
		PLAYSOUND(flinged_sword, 'sound/items/haunted/ghostitemattack.ogg').volume(100).vary_frequency(TRUE).play()

...
/obj/item/weakpoint_locator/attack_self(mob/living/user, modifiers)
	...
	if(!(user_area.type in objective.scan_areas))
		...
		playsound(user, 'sound/machines/buzz/buzz-sigh.ogg').volume(30).vary(TRUE).play()
		return
	...
	playsound(user, 'sound/machines/beep/triple_beep.ogg').volume(30).vary(TRUE).play()
	...
	playsound(user, 'sound/machines/ding.ogg').volume(100).vary(TRUE).play()

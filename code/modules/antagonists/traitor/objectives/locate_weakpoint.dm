...
/obj/item/weakpoint_locator/attack_self(mob/living/user, modifiers)
	...
	if(!(user_area.type in objective.scan_areas))
		...
		PLAYSOUND(user, 'sound/machines/buzz/buzz-sigh.ogg').volume(30).vary_frequency(TRUE).play()
		return
	...
	PLAYSOUND(user, 'sound/machines/beep/triple_beep.ogg').volume(30).vary_frequency(TRUE).play()
	...
	PLAYSOUND(user, 'sound/machines/ding.ogg').volume(100).vary_frequency(TRUE).play()

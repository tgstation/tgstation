...
/atom/movable/screen/alert/status_effect/iaa_pinpointer
	name = "Internal Affairs Integrated Pinpointer"
	desc = "Even stealthier than a normal implant."
	icon = 'icons/obj/device.dmi'
	icon_state = "pinon"

/datum/status_effect/iaa_pinpointer
	id = "iaa_pinpointer"
	duration = -1
	tick_interval = PINPOINTER_PING_TIME
	alert_type = /atom/movable/screen/alert/status_effect/iaa_pinpointer
...
/datum/status_effect/iaa_pinpointer/tick()
	if(!owner)
		qdel(src)
		return
	scan_for_target()
...
/datum/status_effect/iaa_pinpointer/proc/point_to_target()
	if(!scan_target)
...
/datum/status_effect/iaa_pinpointer/proc/scan_for_target()
	scan_target = null
	if(!owner && !owner.mind)
		return

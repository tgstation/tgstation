...
/datum/team/revolution/proc/process_victory(revs_win_injection_amount)
	...
	if (. == STATION_VICTORY)
		...
		if (revs_win_injection_amount)
			...
			dynamic.create_threat(revs_win_injection_amount, dynamic.threat_log, "[worldtime2text()]: Revolution victory")
			dynamic.log_threat(revs_win_injection_amount, dynamic.roundend_threat_log, "Revolution victory")

...
/datum/revolution_handler/proc/declare_revs_win()
	for(var/datum/mind/headrev_mind as anything in revs.ex_headrevs)
		...
		for(var/mob/living/nearby_goon in view(real_headrev, 5))
			if(HAS_PERSONALITY(nearby_goon, /datum/personality/nt/loyalist))
				nearby_goon.add_mood_event("loyalist_revs_win", /datum/mood_event/loyalist_revs_win)
			if(HAS_PERSONALITY(nearby_goon, /datum/personality/nt/disillusioned))
				nearby_goon.add_mood_event("disillusioned_revs_win", /datum/mood_event/disillusioned_revs_win)
...
/datum/revolution_handler/proc/declare_heads_win()
	...
	for(var/datum/objective/mutiny/head_tracker in revs.objectives)
		...
		for(var/mob/living/nearby_goon in view(head_of_staff, 5))
			if(HAS_PERSONALITY(nearby_goon, /datum/personality/nt/loyalist))
				nearby_goon.add_mood_event("loyalist_revs_lost", /datum/mood_event/loyalist_revs_lost)
			if(HAS_PERSONALITY(nearby_goon, /datum/personality/nt/disillusioned))
				nearby_goon.add_mood_event("disillusioned_revs_lost", /datum/mood_event/disillusioned_revs_lost)

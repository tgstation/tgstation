...
/datum/action/bloodsucker/feed/ActivatePower(trigger_flags)
	...
	ADD_TRAIT(owner, TRAIT_MUTE, BLOODSUCKER_TRAIT)
	ADD_TRAIT(owner, TRAIT_IMMOBILIZED, BLOODSUCKER_TRAIT)
	return ..()
...
/datum/action/bloodsucker/feed/process(delta_time)
	...
	if(living_owner.blood_volume >= bloodsuckerdatum_power.max_blood_volume)

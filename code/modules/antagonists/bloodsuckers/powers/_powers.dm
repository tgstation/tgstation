...
/datum/action/bloodsucker/proc/CheckCanUse(mob/living/carbon/user, trigger_flags)
	...
	if((check_flags & BP_CANT_USE_IN_FRENZY) && (bloodsuckerdatum_power?.frenzied) && bloodsuckerdatum.my_clan != CLAN_BRUJAH)
	...
	var/mob/living/living_owner = owner
	if(constant_bloodcost > 0 && living_owner?.blood_volume <= 0)

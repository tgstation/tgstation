...
/datum/reagent/blob/networked_fibers/reaction_mob(mob/living/M, method=TOUCH, reac_volume, show_message, touch_protection, mob/camera/blob/O)
	reac_volume = ..()
	M.apply_damage(0.6*reac_volume, BRUTE, crit_array = list(IS_BLUNT, NOT_POINTED, TRUE))
	if(M)
		M.apply_damage(0.6*reac_volume, BURN, crit_array = list(IS_BLUNT, NOT_POINTED, TRUE))

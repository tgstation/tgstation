...
/datum/reagent/blob/cryogenic_poison/on_mob_life(mob/living/carbon/exposed_mob, seconds_per_tick, times_fired)
	exposed_mob.adjustBruteLoss(0.5 * REM * seconds_per_tick, updating_health = FALSE)
	exposed_mob.adjustFireLoss(0.5 * REM * seconds_per_tick, updating_health = FALSE)
	exposed_mob.adjustToxLoss(0.5 * REM * seconds_per_tick, updating_health = FALSE)
	. = 1
	..()

...
/datum/reagent/blob/cryogenic_poison/on_mob_life(mob/living/carbon/exposed_mob, seconds_per_tick, times_fired)
	. = ..()
	var/need_mob_update
	need_mob_update = exposed_mob.metabolic_damage_adjust(src, BRUTELOSS, 0.5 * REM * seconds_per_tick, updating_health = FALSE)
	need_mob_update += exposed_mob.metabolic_damage_adjust(src, FIRELOSS, 0.5 * REM * seconds_per_tick, updating_health = FALSE)
	need_mob_update += exposed_mob.metabolic_damage_adjust(src, TOXLOSS, 0.5 * REM * seconds_per_tick, updating_health = FALSE)
	if(need_mob_update)
		. = UPDATE_MOB_HEALTH

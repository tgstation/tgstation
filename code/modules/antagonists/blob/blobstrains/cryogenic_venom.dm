...
/datum/blobstrain/reagent/cryogenic_venom
	name = "Cryogenic Venom"
	description = "will inject targets with a freezing venom that does high damage over time."
	analyzerdescdamage = "Injects targets with a freezing venom that will gradually solidify the target's internal organs."
	color = "#8BA6E9"
	...
	reagent = /datum/reagent/blob/cryogenic_venom

/datum/reagent/blob/cryogenic_venom
	name = "Cryogenic Venom"
	description = "will inject targets with a freezing venom that does high damage over time."
	color = "#8BA6E9"
	taste_description = "brain freeze"

/datum/reagent/blob/cryogenic_venom/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message, touch_protection, mob/camera/blob/overmind)
	. = ..()
	...
	if(exposed_mob.reagents)
		...
		exposed_mob.reagents.add_reagent(/datum/reagent/blob/cryogenic_venom, 0.3*reac_volume)
	exposed_mob.apply_damage(0.2*reac_volume, BRUTE, wound_bonus=CANT_WOUND)

/datum/reagent/blob/cryogenic_venom/on_mob_life(mob/living/carbon/exposed_mob)

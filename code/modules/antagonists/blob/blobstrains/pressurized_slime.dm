...
/datum/reagent/blob/pressurized_slime/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message, touch_protection, mob/eye/blob/overmind)
	...
	metabolic_health_adjust(exposed_mob, 0.4*reac_volume, BRUTE, wound_bonus=CANT_WOUND)
	if(exposed_mob)
		metabolic_health_adjust(exposed_mob, reac_volume, STAMINA)
		metabolic_health_adjust(exposed_mob, 0.4 * reac_volume, OXY)

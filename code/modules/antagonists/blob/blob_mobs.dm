...
/mob/living/simple_animal/hostile/blob/blobspore/life_process()
	if(!is_zombie && isturf(loc) && !is_weak)
		for(var/mob/living/carbon/human/H in view(src,1)) //Only for corpse right next to/on same tile
			if(H.stat == DEAD)
				Zombify(H)
				break
	if(z != factory.z)//shit marker
		death()
	return ..()
...
/mob/living/simple_animal/hostile/blob/blobbernaut/life_process()
	. = ..()
	if(!.)
		return

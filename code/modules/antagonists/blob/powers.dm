...
/mob/camera/blob/proc/create_blobbernaut()
	...
	var/list/mob/dead/observer/candidates = SSpolling.poll_ghost_candidates("Do you want to play as a [blobstrain.name] blobbernaut?", role = ROLE_BLOB, jobban = ROLE_BLOB, poll_time = 5 SECONDS, pic_source = /mob/living/simple_animal/hostile/blob/blobbernaut, role_name_text = "blobbernaut") //players must answer rapidly

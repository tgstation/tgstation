...
/**
  * # OriginalSwarmerShell
  *
  * The first swarmer from an event's shell
  *
  * The first swarmer made from an event has special attributes, and a dedicated mob subtype.
  *
  */
/obj/effect/mob_spawn/swarmer/original
	mob_type = /mob/living/simple_animal/hostile/swarmer/original
...
/mob/living/simple_animal/hostile/swarmer
	...
	var/resources = 0
	/// The maximum resources the swarmer may hold.
	var/max_resources = 100
	/// The number of remaining times a swarmer may place a pylon without adhering to the pylon distance rule
	var/free_pylon_placements = 0

/**
  * # OriginalSwarmer
  *
  * The first swarmer from an event
  *
  * When the first swarmer spawns, there is likely no pylon, so the original swarmer is able to create one pylon for free.
  *
  */
/mob/living/simple_animal/hostile/swarmer/original
	free_pylon_placements = 1
...
/**
  * Determine if swarmer can attempt to dismantle/absorb an object, if so, delegate the responsibility
  *
  * Currently only checks to see if there is a nearby swarmer pylon.
  *
  * Arguments:
  * * target - the atom to be absorbed/dismantled.
  */
/mob/living/simple_animal/hostile/swarmer/proc/attempt_attack(atom/target)
	if(inPylonRange())
		return target.swarmer_act(src)
	else
		to_chat(src, "<span class='warning'>No swarmer pylon within range. Aborting.</span>")
		return FALSE

/**
  * Determine if a pylon is in range for purposes of building, deconstruction, or absorbtion.
  *
  */
/mob/living/simple_animal/hostile/swarmer/proc/inPylonRange()
	return locate(/obj/structure/swarmer/pylon) in range(7, src)
...
/**
  * # SwarmerPylon
  *
  * Provides an area that swarmers can act within
  *
  * Certain swarmer abilities can only be activated when close enough to a pylon.
  * These checks are handled in the abilities.  This class has no special functionality by itself.
  *
  */
/obj/structure/swarmer/pylon
	name = "swarmer pylon"
	desc = "A strange structure that enables swarmers to spread."
	icon_state = "pylon"
	max_integrity = 20
	density = FALSE
...
/mob/living/simple_animal/hostile/swarmer/proc/CreateTrap()
	...
	if(locate(/obj/structure/swarmer/trap) in loc)
		...
		return
	if(!inPylonRange())
		to_chat(src, "<span class='warning'>There is no nearby pylon.  Aborting.</span>")
		return
	Fabricate(/obj/structure/swarmer/trap, 5)
	
/**
  * Attempts to create a swarmer pylon
  *
  * Used upon attempting to create a swarmer pylon.  Performs checks to make sure it's constructable, and if not, displays a reason to the swarmer.
  */
/mob/living/simple_animal/hostile/swarmer/proc/CreatePylon()
	set name = "Create pylon"
	set category = "Swarmer"
	set desc = "Creates a pylon that expands our area of operations.  Costs 25 resources."
	if(locate(/obj/structure/swarmer) in loc)
		to_chat(src, "<span class='warning'>There is already a structure here. Aborting.</span>")
		return
	if(free_pylon_placements)
		Fabricate(/obj/structure/swarmer/pylon, 0)
		free_pylon_placements -= 1
		return
	if(!inPylonRange())
		to_chat(src, "<span class='warning'>There is no nearby pylon.  Aborting.</span>")
		return
	Fabricate(/obj/structure/swarmer/pylon, 25)

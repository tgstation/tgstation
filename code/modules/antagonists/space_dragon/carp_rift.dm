...
/datum/action/innate/summon_rift/Activate()
	...
	notify_ghosts("The Space Dragon has opened a rift!", source = new_rift, action = NOTIFY_ORBIT, notify_flags = NOTIFY_CATEGORY_NOFLASH, header = "Carp Rift Opened")
...
/obj/structure/carp_rift/proc/update_check()
	...
	if(last_carp_inc >= carp_interval)
		...
		if(light_color != LIGHT_COLOR_PURPLE)
			...
			update_light()
		notify_ghosts("The carp rift can summon an additional carp!", source = src, action = NOTIFY_ORBIT, notify_flags = NOTIFY_CATEGORY_NOFLASH, header = "Carp Spawn Available")

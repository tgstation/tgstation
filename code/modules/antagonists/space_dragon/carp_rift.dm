...
/datum/action/innate/summon_rift/Activate()
	...
	create_sound(owner.loc, 'sound/vehicles/rocketlaunch.ogg').volume(100).vary(TRUE).play()
	dragon.riftTimer = -1
...
/obj/structure/carp_rift/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = 0)
	create_sound(src, 'sound/effects/magic/lightningshock.ogg').vary(TRUE).play()

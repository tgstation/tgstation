...
/datum/heretic_knowledge/curse/cosmig_trail
	...
	desc = "Allows you to transmute a bluespace crystal, a pool of ash, and a liver to cast a Curse of The Stars on a crew member. \
		...
		crew member cannot enter cosmig carpet fields. The victim will also recieve a cosmig carpet trail for at least 30 seconds."
...
/datum/heretic_knowledge/curse/cosmig_trail/curse(mob/living/carbon/human/chosen_mob, boosted = FALSE)
	to_chat(chosen_mob, span_danger("You feel very strange..."))
	chosen_mob.apply_status_effect(/datum/status_effect/star_mark)
	chosen_mob.AddElement(/datum/element/cosmig_carpet_trail)
	return ..()

/datum/heretic_knowledge/curse/cosmig_trail/uncurse(mob/living/carbon/human/chosen_mob, boosted = FALSE)
	if(QDELETED(chosen_mob))
		return

	chosen_mob.RemoveElement(/datum/element/cosmig_carpet_trail)

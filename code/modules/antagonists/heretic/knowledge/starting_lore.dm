...
/datum/heretic_knowledge/living_heart/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	...
	playsound(user, 'sound/effects/magic/demon_consume.ogg').vary(TRUE).play()
	return TRUE
...
/datum/heretic_knowledge/codex_cicatrix/cleanup_atoms(list/selected_atoms)
	...
	playsound(body, 'sound/items/poster/poster_ripped.ogg').volume(100).vary(TRUE).play()
...
/datum/heretic_knowledge/feast_of_owls/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	...
	playsound(user, 'sound/music/antag/heretic/heretic_gain_intense.ogg').volume(100).direct_listeners(user).play()
	for(var/i in 1 to reward)
		user.emote("scream")
		playsound(loc, 'sound/items/eatfood.ogg').volume(100).vary(TRUE).play()

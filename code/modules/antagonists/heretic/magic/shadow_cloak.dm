...
/datum/action/cooldown/spell/shadow_cloak/proc/cloak_mob(mob/living/cast_on)
	playsound(cast_on, 'sound/effects/chemistry/ahaha.ogg').vary(TRUE).extra_range(SILENCED_SOUND_EXTRARANGE).frequency(0.5).play()
...
/datum/action/cooldown/spell/shadow_cloak/proc/uncloak_mob(mob/living/cast_on, show_message = TRUE)
	...
	playsound(cast_on, 'sound/effects/curse/curseattack.ogg').play()
	if(show_message)
		cast_on.visible_message(

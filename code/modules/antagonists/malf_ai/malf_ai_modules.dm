...
/datum/action/innate/ai/nuke_station/proc/set_up_us_the_bomb(mob/living/owner)
	...
	if(QDELETED(owner) || !isturf(owner_AI.loc))
		active = FALSE
		return
	playsound(owner, 'sound/announcer/alarm/bloblarm.ogg').use_reverb(FALSE).direct_listeners(owner).play()
	...
	playsound(owner, 'sound/items/timer.ogg').use_reverb(FALSE).direct_listeners(owner).play()
	sleep(3 SECONDS)
	if(QDELETED(owner) || !isturf(owner_AI.loc))
		active = FALSE
		return
	to_chat(owner, span_boldnotice("Credentials accepted. Welcome, akjv9c88asdf12nb."))
	playsound(owner, 'sound/misc/server-ready.ogg').use_reverb(FALSE).direct_listeners(owner).play()
	sleep(0.5 SECONDS)
	if(QDELETED(owner) || !isturf(owner_AI.loc))
		active = FALSE
		return
	to_chat(owner, span_boldnotice("Arm self-destruct device? (Y/N)"))
	playsound(owner, 'sound/machines/compiler/compiler-stage1.ogg').use_reverb(FALSE).direct_listeners(owner).play()
	sleep(2 SECONDS)
	if(QDELETED(owner) || !isturf(owner_AI.loc))
		active = FALSE
		return
	...
	playsound(owner, 'sound/machines/compiler/compiler-stage2.ogg').use_reverb(FALSE).direct_listeners(owner).play()
	sleep(1 SECONDS)
	if(QDELETED(owner) || !isturf(owner_AI.loc))
		active = FALSE
		return
...
/datum/action/innate/ai/ranged/override_machine/do_ability(mob/living/caller, atom/clicked_on)
	...
	playsound(caller, 'sound/misc/interference.ogg').use_reverb(FALSE).direct_listeners(caller).play()
...
/datum/action/innate/ai/destroy_rcds/Activate()
	...
	playsound(owner, 'sound/machines/beep/twobeep.ogg').direct_listeners(owner).play()
...
/datum/action/innate/ai/ranged/overload_machine/do_ability(mob/living/caller, atom/clicked_on)
	...
	playsound(caller, SFX_SPARKS).direct_listeners(caller).play()
	adjust_uses(-1)
	if(uses)
		desc = "[initial(desc)] It has [uses] use\s remaining."
		build_all_button_icons()
...
/datum/action/innate/ai/blackout/Activate()
	...
	playsound(owner, SFX_SPARKS).direct_listeners(owner).play()
	adjust_uses(-1)
...
/datum/action/innate/ai/break_air_alarms/Activate()
	...
	playsound(owner, 'sound/machines/terminal/terminal_off.ogg').direct_listeners(owner).play()
...
/datum/action/innate/ai/emergency_lights/Activate()
	...
	playsound(owner, 'sound/effects/light_flicker.ogg').direct_listeners(owner).play()
...
/datum/action/innate/ai/reactivate_cameras/Activate()
	...
	playsound(owner, 'sound/items/tools/wirecutter.ogg').direct_listeners(owner).play()

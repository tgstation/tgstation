...
/mob/living/simple_animal/hostile/haunt/poltergeist/proc/collect_junk(rip_floors = TRUE)
	...
	for(var/obj/item/I in view(4,src))
		if(!I.anchored && !I.orbiting)
			found |= I
			if(found.len > max_collect_count || (found.len + junk.len > max_junk))
				break
	if(found.len < max_collect_count && found.len + junk.len < max_junk && rip_floors)
		visible_message("<span class='warning'>[src] rips the floor tiles out!</span>")
		for(var/turf/open/floor/F in view(4,src))
			if(!F.floor_tile)
				continue
			var/obj/item/I = new F.floor_tile(drop_location())

/datum/antagonist/nukeop
	name = "Henchmen"
	roundend_category = "henchmen" //just in case
...
/datum/antagonist/nukeop/greet()
	...
	to_chat(owner, "<span class='notice'>You are a [nuke_team ? nuke_team.syndicate_name : "butterfly"] henchmen!</span>")
...
/datum/antagonist/nukeop/proc/give_alias()
	if(nuke_team && nuke_team.syndicate_name)
		...
		owner.current.real_name = "Number [number]"
...
/datum/antagonist/nukeop/leader/give_alias()
	owner.current.real_name = "Number 1"

/datum/antagonist/nukeop/leader/greet()
	owner.current.playsound_local(get_turf(owner.current), 'sound/ambience/antag/ops.ogg',100,0)
	to_chat(owner, "<B>You are the leader of your Henchmen! You are responsible for the distribution of telecrystals and your ID is the only one who can open the launch bay doors.</B>")
	to_chat(owner, "<B>In your hand you will find a special item capable of triggering a greater challenge for your henchmen. Examine it carefully and consult with your fellow henchmen before activating it.</B>")
	owner.announce_objectives()
...
/datum/antagonist/nukeop/leader/proc/ask_name()
	...
	var/newname = stripped_input(owner.current,"You are the henchmen leader. Please choose a name for your crime organization.", "Name change",randomname)
	if (!newname)
		newname = randomname
	else
...
/datum/team/nuclear/roundend_report()
	var/list/parts = list()
	parts += "<span class='header'>[syndicate_name] Henchmen:</span>"

	switch(get_result())
		if(NUKE_RESULT_FLUKE)
			parts += "<span class='redtext big'>Humiliating Henchmen Defeat</span>"
			parts += "<B>The crew of [station_name()] gave [syndicate_name] henchmen back their bomb! The henchmen base was destroyed!</B> Next time, don't lose the nuke!"
		if(NUKE_RESULT_NUKE_WIN)
			parts += "<span class='greentext big'>Henchmen Major Victory!</span>"
			parts += "<B>[syndicate_name] henchmen have destroyed [station_name()]!</B>"
		if(NUKE_RESULT_NOSURVIVORS)
			parts += "<span class='neutraltext big'>Total Annihilation</span>"
			parts +=  "<B>[syndicate_name] henchmen destroyed [station_name()] but did not leave the area in time and got caught in the explosion.</B> Next time, don't lose the disk!"
		if(NUKE_RESULT_WRONG_STATION)
			parts += "<span class='redtext big'>Crew Minor Victory</span>"
			parts += "<B>[syndicate_name] henchmen secured the authentication disk but blew up something that wasn't [station_name()].</B> Next time, don't do that!"
		if(NUKE_RESULT_WRONG_STATION_DEAD)
			parts += "<span class='redtext big'>[syndicate_name] henchmen have earned Darwin Award!</span>"
			parts += "<B>[syndicate_name] henchmen blew up something that wasn't [station_name()] and got caught in the explosion.</B> Next time, don't do that!"
		if(NUKE_RESULT_CREW_WIN_SYNDIES_DEAD)
			parts += "<span class='redtext big'>Crew Major Victory!</span>"
			parts += "<B>The Research Staff has saved the disk and killed the [syndicate_name] Henchmen!</B>"
		if(NUKE_RESULT_CREW_WIN)
			parts += "<span class='redtext big'>Crew Major Victory</span>"
			parts += "<B>The Research Staff has saved the disk and stopped the [syndicate_name] Henchmen!</B>"
		if(NUKE_RESULT_DISK_LOST)
			parts += "<span class='neutraltext big'>Neutral Victory!</span>"
			parts += "<B>The Research Staff failed to secure the authentication disk but did manage to kill most of the [syndicate_name] Henchmen!</B>"
		if(NUKE_RESULT_DISK_STOLEN)
			parts += "<span class='greentext big'>Henchmen Minor Victory!</span>"
			parts += "<B>[syndicate_name] henchmen survived the assault but did not achieve the destruction of [station_name()].</B> Next time, don't lose the disk!"
		else
			parts += "<span class='neutraltext big'>Neutral Victory</span>"
			parts += "<B>Mission aborted!</B>"

	var/text = "<br><span class='header'>The henchmen were:</span>"
	var/purchases = ""
	var/TC_uses = 0
	...
	text += "<br>"
	text += "(Henchmen used [TC_uses] TC) [purchases]"
...
/datum/team/nuclear/antag_listing_name()
	if(syndicate_name)
		return "[syndicate_name] Henchmen"
	else
		return "Henchmen"
...
/datum/team/nuclear/is_gamemode_hero()
	return SSticker.mode.name == "arching operation"

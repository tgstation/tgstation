	...
	switch(action)
		if("eject_disk")
			if(auth && auth.loc == src)
				playsound(src, 'sound/machines/terminal/terminal_insert_disc.ogg').play()
				playsound(src, 'sound/machines/nuke/general_beep.ogg').play()
				...
				switch(digit)
					if("C")
						if(auth && ui_mode == NUKEUI_AWAIT_ARM)
							toggle_nuke_safety()
							yes_code = FALSE
							playsound(src, 'sound/machines/nuke/confirm_beep.ogg').play()
							update_ui_mode()
						else
							playsound(src, 'sound/machines/nuke/general_beep.ogg').play()
						numeric_input = ""
						...
						switch(ui_mode)
							if(NUKEUI_AWAIT_CODE)
								if(numeric_input == r_code)
									...
									playsound(src, 'sound/machines/nuke/angry_beep.ogg').play()
									numeric_input = "ERROR"
							if(NUKEUI_AWAIT_TIMER)

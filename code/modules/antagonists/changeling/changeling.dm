...
/datum/antagonist/changeling/on_removal()
	...
	var/mob/living/carbon/no_longer_ling = owner.current
	if(istype(no_longer_ling))
		var/obj/item/organ/brain/B = no_longer_ling.getorganslot(ORGAN_SLOT_BRAIN)
...
/datum/antagonist/changeling/process(delta_time = SSMOBS_DT)
...
/datum/antagonist/changeling/greet()
	if (you_are_greet)
		to_chat(owner.current, span_boldannounce("You are a thingling! You have absorbed and taken the form of a human."))
...
/datum/antagonist/changeling/farewell()
	to_chat(owner.current, span_userdanger("You grow weak and lose your powers! You are no longer a thingling and are stuck in your current form!"))

/datum/antagonist/changeling/admin_add(datum/mind/new_owner,mob/admin)
	. = ..()
	to_chat(new_owner.current, span_boldannounce("Our powers have awoken. A flash of memory returns to us...we are a thingling!"))

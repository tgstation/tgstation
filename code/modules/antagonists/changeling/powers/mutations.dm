...
/datum/action/changeling/weapon/proc/check_weapon(mob/user, obj/item/hand_item)
	if(istype(hand_item, weapon_type))
		...
		if(!silent)
			PLAYSOUND(user, 'sound/effects/blob/blobattack.ogg').volume(30).vary_frequency(TRUE).play()
...
/obj/item/melee/arm_blade/afterattack(atom/target, mob/user, click_parameters)
	if(istype(target, /obj/structure/table))
		...
		if(opening.hasPower())
			...
			PLAYSOUND(opening, 'sound/machines/airlock/airlock_alien_prying.ogg').volume(100).vary_frequency(TRUE).play()
...
/obj/item/clothing/head/helmet/changeling_hivehead/attackby(obj/item/attacking_item, mob/user, params)
	...
	PLAYSOUND(wearer, 'sound/effects/blob/attackblob.ogg').volume(60).vary_frequency(TRUE).play()
...
/datum/action/cooldown/hivehead_spawn_minions/proc/do_tell()
	...
	PLAYSOUND(owner, 'sound/mobs/non-humanoids/bee/bee_swarm.ogg').volume(60).vary_frequency(TRUE).play()
...
/datum/action/cooldown/hivehead_spawn_minions/legion/do_tell()
	...
	PLAYSOUND(owner, 'sound/effects/blob/attackblob.ogg').volume(60).vary_frequency(TRUE).play()

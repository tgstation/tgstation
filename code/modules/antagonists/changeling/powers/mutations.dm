...
/datum/action/changeling/weapon/proc/check_weapon(mob/user, obj/item/hand_item)
	if(istype(hand_item, weapon_type))
		...
		if(!silent)
			playsound(user, 'sound/effects/blobattack.ogg', 30, DEFAULT_SOUND_VARY)
...
/datum/action/changeling/suit/proc/check_suit(mob/user)
	...
	if(istype(H.wear_suit, suit_type) || istype(H.head, helmet_type))
		...
		if(blood_on_castoff)
			...
			playsound(H.loc, 'sound/effects/splat.ogg', 50, DEFAULT_SOUND_VARY) //So real sounds
...
/obj/item/melee/arm_blade/afterattack(atom/target, mob/user, proximity)
	...
	if(istype(target, /obj/structure/table))
		...
		if(A.hasPower())
			...
			playsound(A, 'sound/machines/airlock_alien_prying.ogg', 100, DEFAULT_SOUND_VARY)
	...
	if(H.Adjacent(C))
		for(var/obj/item/I in H.held_items)
			if(I.is_sharp())
				...
				H.add_mob_blood(C)
				playsound(get_turf(H),I.hitsound,75,DEFAULT_SOUND_VARY)
				return

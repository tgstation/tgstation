...
/datum/antagonist/changeling/proc/select_dna()
	...
	var/list/diguises = list("Drop Flesh Disguise" = image(icon = 'icons/hud/radial.dmi', icon_state = "radial_eject"))
	...
	var/chosen_name = show_radial_menu(user, user, diguises, custom_check = CALLBACK(src, .proc/check_menu, user), radius = 40, require_near = TRUE)
	if(!chosen_name)
		return

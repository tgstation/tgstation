...
/datum/action/changeling/defib_grasp/proc/execute_defib(mob/living/carbon/changeling, mob/living/defibber, obj/item/shockpaddles/defib)
	...
	PLAYSOUND(changeling, 'sound/effects/magic/demon_consume.ogg').volume(50).vary_frequency(TRUE).play()
	...
	PLAYSOUND(defib, 'sound/machines/defib/defib_success.ogg').volume(50).vary_frequency(FALSE).play() // I guess

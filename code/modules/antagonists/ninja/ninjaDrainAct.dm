	...
	if(cell?.charge)
		...
		while(cell.charge> 0 && !maxcapacity)
			...
			if (do_after(ninja, 1 SECONDS, target = src, hidden = TRUE))
				spark_system.start()
				PLAYSOUND(loc, get_sfx(SFX_SPARKS)).volume(50).vary_frequency(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
				cell.use(drain)
...
/mob/living/simple_animal/bot/ninjadrain_act(mob/living/carbon/human/ninja, obj/item/mod/module/hacker/hacking_module)
	...
	PLAYSOUND(get_turf(src), 'sound/machines/warning-buzzer.ogg').volume(35).vary_frequency(TRUE).play()

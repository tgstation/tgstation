	...
	if(cell?.charge)
		...
		while(cell.charge> 0 && !maxcapacity)
			...
			if (do_after(ninja, 1 SECONDS, target = src, hidden = TRUE))
				spark_system.start()
				playsound(loc, SFX_SPARKS).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
				cell.use(drain)
...
/mob/living/simple_animal/bot/ninjadrain_act(mob/living/carbon/human/ninja, obj/item/mod/module/hacker/hacking_module)
	...
	playsound(get_turf(src), 'sound/machines/warning-buzzer.ogg').volume(35).vary(TRUE).play()

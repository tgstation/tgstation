...
/obj/item/antag_spawner/contract/proc/poll_for_student(mob/living/carbon/human/teacher, apprentice_school)
	...
	polling = TRUE
	var/list/candidates = SSpolling.poll_ghost_candidates_for_mob("Do you want to play as a wizard's [apprentice_school] apprentice?", role = ROLE_WIZARD, jobban = ROLE_WIZARD, poll_time = 15 SECONDS, target_mob = src, pic_source = /obj/item/clothing/head/wizard, role_name_text = "wizard apprentice")
	polling = FALSE
...
/obj/item/antag_spawner/nuke_ops/attack_self(mob/user)
	...
	var/list/nuke_candidates = SSpolling.poll_ghost_candidates("Do you want to play as a syndicate [borg_to_spawn ? "[lowertext(borg_to_spawn)] cyborg":"operative"]?", role = ROLE_OPERATIVE, jobban = ROLE_OPERATIVE, poll_time = 15 SECONDS, ignore_category = POLL_IGNORE_SYNDICATE, pic_source = /obj/item/melee/energy/sword, role_name_text = "syndicate beacon reinforcement")
...
/obj/item/antag_spawner/slaughter_demon/attack_self(mob/user)
	...
	if(used)
		return
	var/list/candidates = SSpolling.poll_ghost_candidates_for_mob("Do you want to play as a [initial(demon_type.name)]?", role = ROLE_ALIEN, jobban = ROLE_ALIEN, poll_time = 5 SECONDS, target_mob = src, pic_source = /mob/living/simple_animal/hostile/imp/slaughter, role_name_text = "slaughter demon")
	if(LAZYLEN(candidates))

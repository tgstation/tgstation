	...
	if(loc == H || (in_range(src, H) && isturf(loc)))
		...
		if(href_list["school"])
			if(used)
				...
				return
			var/list/candidates = poll_candidates_for_mob("Do you want to play as a wizard's [href_list["school"]] apprentice?", ROLE_WIZARD, ROLE_WIZARD, 150, src)
...
/obj/item/antag_spawner/slaughter_demon/attack_self(mob/user)
	...
	if(used)
		return
	var/list/candidates = poll_candidates_for_mob("Do you want to play as a [initial(demon_type.name)]?", ROLE_ALIEN, ROLE_ALIEN, 50, src)
	if(LAZYLEN(candidates))

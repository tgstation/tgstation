...
/obj/item/teleport_rod/ranged_interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	playsound(start_turf, teleport_sound).volume(90).extra_range(MEDIUM_RANGE_SOUND_EXTRARANGE).play()
	playsound(user, teleport_sound).volume(90).extra_range(MEDIUM_RANGE_SOUND_EXTRARANGE).play()

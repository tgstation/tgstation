...
/obj/item/scrying/process()
	...
	if(current_owner && current_owner != holder)
		...
		REMOVE_TRAIT_LIST(current_owner, list(TRAIT_SIXTHSENSE, TRAIT_XRAY_VISION), SCRYING_ORB)
	...
	if(!current_owner && holder)
		...
		ADD_TRAIT_LIST(current_owner, list(TRAIT_SIXTHSENSE, TRAIT_XRAY_VISION), SCRYING_ORB)
		current_owner.update_sight()

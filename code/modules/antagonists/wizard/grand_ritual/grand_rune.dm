...
/obj/effect/grand_rune/proc/invoke_rune(mob/living/user)
	...
	create_sound(src, 'sound/effects/magic/staff_animation.ogg').volume(75).vary(TRUE).play()
	INVOKE_ASYNC(src, PROC_REF(invoke_rune), user)
...
/obj/effect/grand_rune/proc/on_invocation_complete(mob/living/user)
	is_in_use = FALSE
	create_sound(src, 'sound/effects/magic/staff_change.ogg').volume(75).vary(TRUE).play()

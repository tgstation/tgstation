...
/datum/grand_side_effect/scramble_turfs/trigger(potency, turf/ritual_location, mob/invoker)
	playsound(ritual_location, 'sound/effects/magic/timeparadox2.ogg').volume(60).vary(TRUE).play()
...
/datum/grand_side_effect/summon_crewmate/trigger(potency, turf/ritual_location, mob/invoker)
	playsound(ritual_location, 'sound/effects/magic/lightning_chargeup.ogg').volume(65).vary(TRUE).play()
	...
	playsound(get_turf(victim), 'sound/effects/magic/repulse.ogg').volume(60).vary(TRUE).play()
...
/datum/grand_side_effect/spawn_delayed_mobs/proc/create_mob(mob_path, loc)
	...
	if (!mob_path)
		return
	playsound(get_turf(src), 'sound/effects/magic/teleport_app.ogg').volume(60).vary(TRUE).play()

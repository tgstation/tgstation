...
"}
	...
	if(!input_text)
		to_chat(usr, span_warning("Input was blank!"))
		return
	...
	for(var/name in names)
		...
		if(!reagent)
			to_chat(usr, span_warning("Could not find [name]. Skipping."))
			continue
		//Get reaction
		var/list/reactions = GLOB.chemical_reactions_list_product_index[reagent.type]

		if(!length(reactions))
			to_chat(usr, span_warning("Could not find [name] reaction! Continuing anyways."))

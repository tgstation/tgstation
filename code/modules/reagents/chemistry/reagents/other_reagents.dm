...
/datum/reagent/luminescent_fluid/uv/on_mob_metabolize(mob/living/affected_mob)
	. = ..()
	// Make our flashlight also glow in the UV spectrum
	if (!isnull(glowing))
		glowing.add_light_system(plane = UV_LIGHTING_PLANE, lum_power_mult = 0, power_mult = 2, forced_color = LIGHT_COLOR_ULTRAVIOLET_GLOW)

/datum/reagent/luminescent_fluid/uv/on_mob_life(mob/living/affected_mob, seconds_per_tick, metabolization_ratio)
	var/missing_before = isnull(glowing)
	. = ..()
	// New light was generated by parent life
	if (!isnull(glowing) && missing_before)
		glowing.add_light_system(plane = UV_LIGHTING_PLANE, lum_power_mult = 0, power_mult = 2, forced_color = LIGHT_COLOR_ULTRAVIOLET_GLOW)

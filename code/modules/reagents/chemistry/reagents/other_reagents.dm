...
/datum/reagent/water/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	if(liver?.damage && !IS_ROBOTIC_ORGAN(liver) && !(liver.organ_flags & ORGAN_FAILING))
		...
		liver.apply_organ_damage(0.5 * -healing_bonus * REM * metabolized_volume * seconds_per_tick)

	var/water_adaptation = HAS_TRAIT(affected_mob, TRAIT_WATER_ADAPTATION)
	var/blood_restored = water_adaptation ? 0.3 : 0.1
	affected_mob.adjust_blood_volume(0.5 * blood_restored * REM * metabolized_volume * seconds_per_tick) // water is good for you!
	var/drunkness_restored = water_adaptation ? -0.5 : -0.25
	affected_mob.adjust_drunk_effect(0.5 * drunkness_restored * REM * metabolized_volume * seconds_per_tick) // and even sobers you up slowly!!
	if(water_adaptation)
		var/need_mob_update = FALSE
		need_mob_update = affected_mob.adjust_tox_loss(-0.125 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += affected_mob.adjust_fire_loss(-0.125 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += affected_mob.adjust_brute_loss(-0.125 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
...
/datum/reagent/water/mineral/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_tox_loss(-0.05 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
...
/datum/reagent/water/holywater/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()

	data["deciseconds_metabolized"] += (seconds_per_tick * 1 SECONDS * REM)

	affected_mob.adjust_jitter_up_to(2 SECONDS * REM * metabolized_volume * seconds_per_tick, 20 SECONDS)
	var/need_mob_update = FALSE
	...
	if(data["deciseconds_metabolized"] >= (25 SECONDS)) // 10 units
		affected_mob.adjust_stutter_up_to(2 SECONDS * REM * metabolized_volume * seconds_per_tick, 20 SECONDS)
		...
		if(IS_CULTIST(affected_mob) && SPT_PROB(10, seconds_per_tick))
			...
			if(!IS_CULTIST(affected_mob) || affected_mob.mind?.holy_role != HOLY_ROLE_PRIEST)
				...
				need_mob_update += affected_mob.adjust_fire_loss(1.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
	...
	if(data["deciseconds_metabolized"] >= (1 MINUTES)) // 24 units
		if(IS_CULTIST(affected_mob))
			...
			if(!IS_CULTIST(affected_mob) || affected_mob.mind?.holy_role != HOLY_ROLE_PRIEST)
				need_mob_update += affected_mob.adjust_fire_loss(5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
...
/datum/reagent/fuel/unholywater/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()

	var/need_mob_update = FALSE
	if(IS_CULTIST(affected_mob))
		affected_mob.adjust_drowsiness(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
		affected_mob.AdjustAllImmobility(-20 * REM * metabolized_volume * seconds_per_tick)
		need_mob_update += affected_mob.adjust_stamina_loss(-5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE)
		need_mob_update += affected_mob.adjust_tox_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += affected_mob.adjust_oxy_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += affected_mob.adjust_brute_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += affected_mob.adjust_fire_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update = TRUE
		affected_mob.adjust_blood_volume(1.5 * REM * metabolized_volume * seconds_per_tick, maximum = BLOOD_VOLUME_NORMAL)
		affected_mob.coagulant_effect(1 * REM * metabolized_volume * seconds_per_tick)

	else  // Will deal about 90 damage when 50 units are thrown
		need_mob_update += affected_mob.adjust_organ_loss(ORGAN_SLOT_BRAIN, 1.5 * REM * metabolized_volume * seconds_per_tick, 150)
		need_mob_update += affected_mob.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += affected_mob.adjust_fire_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += affected_mob.adjust_oxy_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += affected_mob.adjust_brute_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/hellwater/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/spraytan/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/mutationtoxin/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(!ishuman(affected_mob))
		return
...
/datum/reagent/mutationtoxin/jelly/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	if(!ishuman(affected_mob))
		return ..()
	var/mob/living/carbon/affected_human = affected_mob
...
/datum/reagent/mulligan/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(!ishuman(affected_mob))
		return
	var/mob/living/carbon/human/affected_human = affected_mob
...
/datum/reagent/serotrotium/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(3.5, seconds_per_tick))
...
/datum/reagent/mercury/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/chlorine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/fluorine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/lithium/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/uranium/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/bluespace/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/fuel/on_mob_life(mob/living/carbon/victim, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/space_cleaner/ez_clean/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
...
/datum/reagent/cryptobiolin/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/impedrezene/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/nitrous_oxide/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/plantnutriment/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/stable_plasma/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjustPlasma(5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/carpet/royal/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/obj/item/organ/liver/liver = affected_mob.get_organ_slot(ORGAN_SLOT_LIVER)
	if(liver)
...
/datum/reagent/colorful_reagent/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/concentrated_barbers_aid/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/royal_bee_jelly/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/magillitis/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/growthserum/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/glitter/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/bz_metabolites/on_mob_life(mob/living/carbon/target, seconds_per_tick, metabolized_volume)
	. = ..()
	target.adjust_hallucinations(2.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	var/datum/antagonist/changeling/changeling = IS_CHANGELING(target)
	changeling?.adjust_chemicals(-1 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/peaceborg/confuse/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/peaceborg/tire/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/yuck/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(!yuck_cycle)
...
/datum/reagent/determination/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	for(var/thing in affected_mob.all_wounds)
		...
		if(wounded_part)
			wounded_part.heal_damage(0.125 * REM * metabolized_volume * seconds_per_tick, 0.25 * REM * metabolized_volume * seconds_per_tick)
		if(affected_mob.adjust_stamina_loss(-2 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE)) // the more wounds, the more stamina regen
			return UPDATE_MOB_HEALTH
...
/datum/reagent/eldritch/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update = FALSE
	if(IS_HERETIC_OR_MONSTER(drinker))
		drinker.adjust_drowsiness(-5 * REM * metabolized_volume * seconds_per_tick)
		drinker.AdjustAllImmobility(-20 * REM * metabolized_volume * seconds_per_tick)
		need_mob_update += drinker.adjust_stamina_loss(-5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE)
		need_mob_update += drinker.adjust_tox_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, forced = TRUE)
		need_mob_update += drinker.adjust_oxy_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += drinker.adjust_brute_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += drinker.adjust_fire_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		drinker.adjust_blood_volume(1.5 * REM * metabolized_volume * seconds_per_tick, maximum = BLOOD_VOLUME_NORMAL)
		...
		for(var/datum/reagent/reagent as anything in drinker.reagents.reagent_list)
			if(reagent != src)
				drinker.reagents.remove_reagent(reagent.type, 2 * reagent.purge_multiplier * REM * metabolized_volume * seconds_per_tick)
	else
		need_mob_update = drinker.adjust_organ_loss(ORGAN_SLOT_BRAIN, 1.5 * REM * metabolized_volume * seconds_per_tick, 150)
		need_mob_update += drinker.adjust_tox_loss(1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += drinker.adjust_fire_loss(1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += drinker.adjust_oxy_loss(1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += drinker.adjust_brute_loss(1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
	if(need_mob_update)
...
/datum/reagent/ants/on_mob_life(mob/living/carbon/victim, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/lead/on_mob_life(mob/living/carbon/victim, metabolized_volume)
	. = ..()
...
/datum/reagent/kronkus_extract/on_mob_life(mob/living/carbon/kronkus_enjoyer, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	need_mob_update = kronkus_enjoyer.adjust_organ_loss(ORGAN_SLOT_HEART, 0.1 * REM * metabolized_volume * seconds_per_tick, required_organ_flag = affected_organ_flags)
...
/datum/reagent/brimdust/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/love/overdose_process(mob/living/metabolizer, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/hauntium/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.mob_biotypes & MOB_UNDEAD || HAS_MIND_TRAIT(affected_mob, TRAIT_MORBID)) //if morbid or undead,acts like an addiction-less drug
		affected_mob.remove_status_effect(/datum/status_effect/jitter)
		affected_mob.AdjustStun(-2.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
		affected_mob.AdjustKnockdown(-2.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
		affected_mob.AdjustUnconscious(-2.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
		affected_mob.AdjustParalyzed(-2.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
		affected_mob.AdjustImmobilized(-2.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	else
		if(affected_mob.adjust_organ_loss(ORGAN_SLOT_HEART, REM * metabolized_volume * seconds_per_tick)) //1 heart damage per tick
			. = UPDATE_MOB_HEALTH
		if(SPT_PROB(10, seconds_per_tick))
...
/datum/reagent/luminescent_fluid/on_mob_life(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/luminescent_fluid/overdose_start(mob/living/affected_mob, metabolized_volume)
	. = ..()
	if (!ishuman(affected_mob))
		return
	var/mob/living/carbon/human/affected_human = affected_mob
...
/datum/reagent/luminescent_fluid/red/overdose_start(mob/living/affected_mob, metabolized_volume)
	. = ..()
	if (!ishuman(affected_mob))
		return

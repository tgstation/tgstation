	...
	if(ishuman(exposed_mob))
		if(methods & (PATCH|VAPOR) && touch_protection >= 1)
			...
			//take current alien color and darken it slightly
				...
				var/list/existing_color = rgb2num(exposed_human.dna.features[/datum/dna_block/feature/mutant_color])
				...
				if(new_hsv[3] >= 50)
					exposed_human.dna.features[/datum/dna_block/feature/mutant_color] = new_color
...
/datum/reagent/spraytan/overdose_process(mob/living/affected_mob, seconds_per_tick, times_fired)
	...
	if(ishuman(affected_mob))
		...
		if(HAS_TRAIT(affected_human, TRAIT_USES_SKINTONES))
			...
			affected_human.dna.features[/datum/dna_block/feature/mutant_color] = "#ff8800"

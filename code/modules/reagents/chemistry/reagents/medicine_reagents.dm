...
/datum/reagent/medicine/leporazine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/target_temp = affected_mob.get_body_temp_normal(apply_change = FALSE)
	if(affected_mob.bodytemperature > target_temp)
		affected_mob.adjust_bodytemperature(-20 * TEMPERATURE_DAMAGE_COEFFICIENT * REM * metabolized_volume * seconds_per_tick, target_temp)
	else if(affected_mob.bodytemperature < (target_temp + 1))
		affected_mob.adjust_bodytemperature(20 * TEMPERATURE_DAMAGE_COEFFICIENT * REM * metabolized_volume * seconds_per_tick, 0, target_temp)
	if(ishuman(affected_mob))
		var/mob/living/carbon/human/affected_human = affected_mob
		if(affected_human.coretemperature > target_temp)
			affected_human.adjust_coretemperature(-20 * TEMPERATURE_DAMAGE_COEFFICIENT * REM * metabolized_volume * seconds_per_tick, target_temp)
		else if(affected_human.coretemperature < (target_temp + 1))
			affected_human.adjust_coretemperature(20 * TEMPERATURE_DAMAGE_COEFFICIENT * REM * metabolized_volume * seconds_per_tick, 0, target_temp)
...
/datum/reagent/medicine/adminordrazine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.heal_bodypart_damage(brute = 2.5 * REM * metabolized_volume * seconds_per_tick, burn = 2.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	affected_mob.adjust_tox_loss(-2.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, forced = TRUE, required_biotype = affected_biotype)
...
/datum/reagent/medicine/synaptizine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_drowsiness(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.AdjustAllImmobility(-10 * REM * metabolized_volume * seconds_per_tick)

	if(holder.has_reagent(/datum/reagent/toxin/mindbreaker))
		holder.remove_reagent(/datum/reagent/toxin/mindbreaker, 2.5 * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_hallucinations(-10 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/medicine/synaphydramine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_drowsiness(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	if(holder.has_reagent(/datum/reagent/toxin/mindbreaker))
		holder.remove_reagent(/datum/reagent/toxin/mindbreaker, 2.5 * REM * metabolized_volume * seconds_per_tick)
	if(holder.has_reagent(/datum/reagent/toxin/histamine))
		holder.remove_reagent(/datum/reagent/toxin/histamine, 2.5 * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_hallucinations(-10 SECONDS * REM * metabolized_volume * seconds_per_tick)
	if(SPT_PROB(16, seconds_per_tick))
		if(affected_mob.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/sansufentanyl/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_confusion_up_to(1.5 SECONDS * REM * metabolized_volume * seconds_per_tick, 5 SECONDS)
	affected_mob.adjust_dizzy_up_to(3 SECONDS * REM * metabolized_volume * seconds_per_tick, 12 SECONDS)
	if(affected_mob.adjust_stamina_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE))
		. = UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/cryoxadone/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	var/need_mob_update
	need_mob_update = affected_mob.adjust_oxy_loss(-1.5 * power * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	need_mob_update += affected_mob.adjust_brute_loss(0.5 * -power * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjust_fire_loss(0.5 * -power * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjust_tox_loss(0.5 * -power * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, forced = TRUE, required_biotype = affected_biotype) //heals TOXINLOVERs
	for(var/i in affected_mob.all_wounds)
		var/datum/wound/iter_wound = i
		iter_wound.on_xadone(0.5 * power * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/medicine/pyroxadone/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.bodytemperature > BODYTEMP_HEAT_DAMAGE_LIMIT)
		...
		var/need_mob_update
		need_mob_update = affected_mob.adjust_oxy_loss(-1 * power * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		need_mob_update += affected_mob.adjust_brute_loss(0.5 * -power * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += affected_mob.adjust_fire_loss(-075 * power * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += affected_mob.adjust_tox_loss(0.5 * -power * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, forced = TRUE, required_biotype = affected_biotype)
		if(need_mob_update)
			. = UPDATE_MOB_HEALTH
		for(var/i in affected_mob.all_wounds)
			var/datum/wound/iter_wound = i
...
/datum/reagent/medicine/rezadone/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.heal_bodypart_damage(
		brute = 0.5 * REM * metabolized_volume * seconds_per_tick,
		burn = 0.5 * REM * metabolized_volume * seconds_per_tick,
...
/datum/reagent/medicine/rezadone/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		. = UPDATE_MOB_HEALTH
	affected_mob.set_dizzy_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.set_jitter_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/medicine/oxandrolone/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	if(affected_mob.get_fire_loss() > 25)
		need_mob_update = affected_mob.adjust_fire_loss(-2 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), updating_health = FALSE, required_bodytype = affected_bodytype) //Twice as effective as AIURI for severe burns
	else
		need_mob_update = affected_mob.adjust_fire_loss(-0.25 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), updating_health = FALSE, required_bodytype = affected_bodytype) //But only a quarter as effective for more minor ones
	if(need_mob_update)
		return UPDATE_MOB_HEALTH

/datum/reagent/medicine/oxandrolone/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.get_fire_loss()) //It only makes existing burns worse
		if(affected_mob.adjust_fire_loss(2.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_biotype)) // it's going to be healing either 4 or 0.5
			return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/salglu_solution/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update = FALSE

	if(SPT_PROB(18, seconds_per_tick))
		need_mob_update = affected_mob.adjust_brute_loss(-0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_biotype)
		need_mob_update += affected_mob.adjust_fire_loss(-0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_biotype)
	...
	if(blood_type?.restoration_chem == required_restoration_chem)
		affected_mob.adjust_blood_volume(0.5 * extra_regen * REM * metabolized_volume * seconds_per_tick)

	if(need_mob_update)
		return UPDATE_MOB_HEALTH

/datum/reagent/medicine/salglu_solution/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
...
/datum/reagent/medicine/mine_salve/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	need_mob_update = affected_mob.adjust_brute_loss(-0.125 * REM * metabolized_volume * seconds_per_tick, FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjust_fire_loss(-0.125 * REM * metabolized_volume * seconds_per_tick, FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/omnizine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	need_mob_update = affected_mob.adjust_tox_loss(0.5 * -healing * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
	need_mob_update += affected_mob.adjust_oxy_loss(0.5 * -healing * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	need_mob_update += affected_mob.adjust_brute_loss(0.5 * -healing * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjust_fire_loss(0.5 * -healing * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH

/datum/reagent/medicine/omnizine/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	need_mob_update = affected_mob.adjust_tox_loss(0.75 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
	need_mob_update += affected_mob.adjust_oxy_loss(0.75 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	need_mob_update += affected_mob.adjust_brute_loss(0.75 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjust_fire_loss(0.75 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/calomel/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	for(var/datum/reagent/target_reagent as anything in affected_mob.reagents.reagent_list)
		if(istype(target_reagent, /datum/reagent/medicine/calomel))
			continue
		affected_mob.reagents.remove_reagent(target_reagent.type, 1.5 * target_reagent.purge_multiplier * REM * metabolized_volume * seconds_per_tick)
	var/toxin_amount = round(affected_mob.health / 40, 0.1)
	if(affected_mob.adjust_tox_loss(0.5 * toxin_amount * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		return UPDATE_MOB_HEALTH

/datum/reagent/medicine/calomel/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.reagents.remove_reagent(type, 1 * REM * metabolized_volume * seconds_per_tick)
	if(affected_mob.adjust_tox_loss(1.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/ammoniated_mercury/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/toxin_chem_amount = 0
	for(var/datum/reagent/toxin/target_reagent in affected_mob.reagents.reagent_list)
		toxin_chem_amount += 1
		affected_mob.reagents.remove_reagent(target_reagent.type, 2.5 * target_reagent.purge_multiplier * REM * metabolized_volume * seconds_per_tick)
	...
	if(toxin_chem_amount == 0)
		for(var/datum/reagent/medicine/ammoniated_mercury/target_reagent in affected_mob.reagents.reagent_list)
			affected_mob.reagents.remove_reagent(target_reagent.type, 0.5 * REM * metabolized_volume * seconds_per_tick)

/datum/reagent/medicine/ammoniated_mercury/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_tox_loss(1.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/potass_iodide/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/medicine/pen_acid/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_tox_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		. = UPDATE_MOB_HEALTH
	for(var/datum/reagent/reagent as anything in affected_mob.reagents.reagent_list)
		if(reagent != src)
			affected_mob.reagents.remove_reagent(reagent.type, 1 * reagent.purge_multiplier * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/medicine/sal_acid/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	if(affected_mob.get_brute_loss() > 25)
		need_mob_update = affected_mob.adjust_brute_loss(-2 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), updating_health = FALSE, required_bodytype = affected_bodytype)
	else
		need_mob_update = affected_mob.adjust_brute_loss(-0.25 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH

/datum/reagent/medicine/sal_acid/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.get_brute_loss()) //It only makes existing bruises worse
		if(affected_mob.adjust_brute_loss(2.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = BODYTYPE_ORGANIC)) // it's going to be healing either 4 or 0.5
			return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/salbutamol/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	need_mob_update = affected_mob.adjust_oxy_loss(-1.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	if(affected_mob.losebreath >= 4)
		...
		if(our_respiration_type & affected_respiration_type)
			affected_mob.losebreath -= 1 * REM * metabolized_volume * seconds_per_tick
...
/datum/reagent/medicine/albuterol/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/medicine/ephedrine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	affected_mob.AdjustAllImmobility(-10 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity())
	affected_mob.adjust_stamina_loss(-2 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), updating_stamina = FALSE)

	return UPDATE_MOB_HEALTH

/datum/reagent/medicine/ephedrine/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	if(SPT_PROB(18 * (1 + (1-normalise_creation_purity())), seconds_per_tick))
		affected_mob.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
...
/datum/reagent/medicine/diphenhydramine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(5, seconds_per_tick))
		affected_mob.adjust_drowsiness(2 SECONDS)
	affected_mob.adjust_jitter(-1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	holder.remove_reagent(/datum/reagent/toxin/histamine, 1 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/medicine/morphine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(current_cycle > 5)
	...
	if(affected_mob.disgust < DISGUST_LEVEL_VERYGROSS && SPT_PROB(50 * (2 - creation_purity), seconds_per_tick))
		affected_mob.adjust_disgust(0.75 * REM * metabolized_volume * seconds_per_tick)

	switch(current_cycle)
		if(16) //~3u
			to_chat(affected_mob, span_warning("You start to feel tired..."))
			affected_mob.adjust_eye_blur(1 SECONDS * REM * metabolized_volume * seconds_per_tick)
			if(SPT_PROB(66, seconds_per_tick))
				affected_mob.emote("yawn")

		if(24 to 36) // 5u to 7.5u
			if(SPT_PROB(66 * (2 - creation_purity), seconds_per_tick))
				affected_mob.adjust_drowsiness_up_to(1 SECONDS * REM * metabolized_volume * seconds_per_tick, 12 SECONDS)

		if(36 to 48) // 7.5u to 10u
			affected_mob.adjust_drowsiness_up_to(1 SECONDS * REM * metabolized_volume * seconds_per_tick, 12 SECONDS)

		if(48 to INFINITY) //10u onward
			affected_mob.adjust_drowsiness_up_to(1.5 SECONDS * REM * metabolized_volume * seconds_per_tick, 20 SECONDS)
			// doesn't scale from purity - at this point it tries to guarantee sleep
			if(SPT_PROB(30 * (48 - current_cycle), seconds_per_tick))
				affected_mob.Sleeping(2 SECONDS * REM * metabolized_volume * seconds_per_tick)

/datum/reagent/medicine/morphine/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(18, seconds_per_tick))
...
/datum/reagent/medicine/oculine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/normalized_purity = normalise_creation_purity()
	affected_mob.adjust_temp_blindness(-2 SECONDS * REM * metabolized_volume * seconds_per_tick * normalized_purity)
	affected_mob.adjust_eye_blur(-2 SECONDS * REM * metabolized_volume * seconds_per_tick * normalized_purity)
	var/obj/item/organ/eyes/eyes = affected_mob.get_organ_slot(ORGAN_SLOT_EYES)
	if(eyes)
		// Healing eye damage will cure nearsightedness and blindness from ... eye damage
		if(eyes.apply_organ_damage(-1 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), required_organ_flag = affected_organ_flags))
			. = UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/oculine/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_organ_loss(ORGAN_SLOT_EYES, 0.75 * REM * metabolized_volume * seconds_per_tick, required_organ_flag = affected_organ_flags))
		. = UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/oculine/flumpuline/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/obj/item/organ/eyes/eyes = affected_mob.get_organ_slot(ORGAN_SLOT_EYES)
...
/datum/reagent/medicine/oculine/flumpuline/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(!prob(25))
		return
	var/obj/item/organ/eyes/eyes = affected_mob.get_organ_slot(ORGAN_SLOT_EYES)
...
/datum/reagent/medicine/inacusiate/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/obj/item/organ/ears/ears = affected_mob.get_organ_slot(ORGAN_SLOT_EARS)
	if(!ears)
		return
	var/multiplier = 0.75 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity()
...
/datum/reagent/medicine/atropine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.health <= affected_mob.crit_threshold)
		var/need_mob_update
		need_mob_update = affected_mob.adjust_tox_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += affected_mob.adjust_brute_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += affected_mob.adjust_fire_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += affected_mob.adjust_oxy_loss(-2.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		if(need_mob_update)
			. = UPDATE_MOB_HEALTH
	var/obj/item/organ/lungs/affected_lungs = affected_mob.get_organ_slot(ORGAN_SLOT_LUNGS)
...
/datum/reagent/medicine/atropine/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_tox_loss(0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		. = UPDATE_MOB_HEALTH
	affected_mob.set_dizzy_if_lower(1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.set_jitter_if_lower(1 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/medicine/epinephrine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	var/need_mob_update
	if(affected_mob.health <= affected_mob.crit_threshold)
		need_mob_update = affected_mob.adjust_tox_loss(-0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += affected_mob.adjust_brute_loss(-0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += affected_mob.adjust_fire_loss(-0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += affected_mob.adjust_oxy_loss(-0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	if(affected_mob.losebreath >= 4)
		var/obj/item/organ/lungs/affected_lungs = affected_mob.get_organ_slot(ORGAN_SLOT_LUNGS)
		var/our_respiration_type = affected_lungs ? affected_lungs.respiration_type : affected_mob.mob_respiration_type
		if(our_respiration_type & affected_respiration_type)
	...
	if(affected_mob.losebreath < 0)
		affected_mob.losebreath = 0
		need_mob_update = TRUE
	need_mob_update += affected_mob.adjust_stamina_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE)
	if(SPT_PROB(10, seconds_per_tick))
...
/datum/reagent/medicine/epinephrine/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(18, 0.5 * REM * metabolized_volume * seconds_per_tick))
		var/need_mob_update
		need_mob_update = affected_mob.adjust_stamina_loss(1.25 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE)
		need_mob_update += affected_mob.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		var/obj/item/organ/lungs/affected_lungs = affected_mob.get_organ_slot(ORGAN_SLOT_LUNGS)
		var/our_respiration_type = affected_lungs ? affected_lungs.respiration_type : affected_mob.mob_respiration_type
		if(our_respiration_type & affected_respiration_type)
			affected_mob.losebreath++
...
/datum/reagent/medicine/strange_reagent/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/damage_at_random = rand(0, 250)/100 //0 to 2.5
	var/need_mob_update
	need_mob_update = affected_mob.adjust_brute_loss(0.5 * damage_at_random * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjust_fire_loss(0.5 * damage_at_random * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/mannitol/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_organ_loss(ORGAN_SLOT_BRAIN, -1 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), required_organ_flag = affected_organ_flags))
		return UPDATE_MOB_HEALTH

/datum/reagent/medicine/mannitol/overdose_start(mob/living/affected_mob, metabolized_volume)
	. = ..()
	to_chat(affected_mob, span_notice("You suddenly feel <span class='purple'>E N L I G H T E N E D!</span>"))

/datum/reagent/medicine/mannitol/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/medicine/neurine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(holder.has_reagent(/datum/reagent/consumable/ethanol/neurotoxin))
		holder.remove_reagent(/datum/reagent/consumable/ethanol/neurotoxin, 2.5 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity())
	if(SPT_PROB(8 * normalise_creation_purity(), seconds_per_tick))
		affected_mob.cure_trauma_type(resilience = TRAUMA_RESILIENCE_BASIC)

/datum/reagent/medicine/neurine/on_mob_dead(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_organ_loss(ORGAN_SLOT_BRAIN, -0.5 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), required_organ_flag = affected_organ_flags))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/mutadone/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/medicine/antihol/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	for(var/effect in status_effects_to_clear)
		affected_mob.remove_status_effect(effect)
	affected_mob.reagents.remove_reagent(/datum/reagent/consumable/ethanol, 4 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), include_subtypes = TRUE)
	if(affected_mob.adjust_tox_loss(-0.1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		. = UPDATE_MOB_HEALTH
	affected_mob.adjust_drunk_effect(-5 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity())
...
/datum/reagent/medicine/stimulants/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.health < 50 && affected_mob.health > 0)
		var/need_mob_update
		need_mob_update += affected_mob.adjust_oxy_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		need_mob_update += affected_mob.adjust_tox_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += affected_mob.adjust_brute_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += affected_mob.adjust_fire_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		if(need_mob_update)
			. = UPDATE_MOB_HEALTH
	affected_mob.AdjustAllImmobility(-30  * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_stamina_loss(-6 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)

/datum/reagent/medicine/stimulants/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(18, seconds_per_tick))
...
/datum/reagent/medicine/insulin/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.AdjustSleeping(-1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	holder.remove_reagent(/datum/reagent/consumable/sugar, 1.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/medicine/inaprovaline/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.losebreath >= 5)
		affected_mob.losebreath -= 2.5 * REM * metabolized_volume * seconds_per_tick
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/regen_jelly/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	need_mob_update = affected_mob.adjust_brute_loss(-0.75 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjust_fire_loss(-0.75 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjust_oxy_loss(-0.75 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	need_mob_update += affected_mob.adjust_tox_loss(-0.75 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, forced = TRUE, required_biotype = affected_biotype) //heals TOXINLOVERs
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/syndicate_nanites/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	need_mob_update = affected_mob.adjust_brute_loss(-2.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE) //A ton of healing - this is a 50 telecrystal investment.
	need_mob_update += affected_mob.adjust_fire_loss(-2.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
	need_mob_update += affected_mob.adjust_oxy_loss(-7.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
	need_mob_update += affected_mob.adjust_tox_loss(-2.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, forced = TRUE, required_biotype = affected_biotype)
	need_mob_update += affected_mob.adjust_organ_loss(ORGAN_SLOT_BRAIN, -7.5 * REM * metabolized_volume * seconds_per_tick)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH

/datum/reagent/medicine/syndicate_nanites/overdose_process(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume) //wtb flavortext messages that hint that you're vomitting up robots
	. = ..()
...
/datum/reagent/medicine/earthsblood/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	if(current_cycle < 25) //10u has to be processed before u get into THE FUN ZONE
		...
		need_mob_update += affected_mob.adjust_oxy_loss(-0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		need_mob_update += affected_mob.adjust_tox_loss(-0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += affected_mob.adjust_stamina_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		need_mob_update += affected_mob.adjust_organ_loss(ORGAN_SLOT_BRAIN, 0.5 * REM * metabolized_volume * seconds_per_tick, 150, affected_organ_flags) //This does, after all, come from ambrosia, and the most powerful ambrosia in existence, at that!
	else
		need_mob_update = affected_mob.adjust_brute_loss(-2.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype) //slow to start, but very quick healing once it gets going
		need_mob_update += affected_mob.adjust_fire_loss(-2.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += affected_mob.adjust_oxy_loss(-1.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		need_mob_update += affected_mob.adjust_tox_loss(-1.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += affected_mob.adjust_stamina_loss(-4 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		need_mob_update += affected_mob.adjust_organ_loss(ORGAN_SLOT_BRAIN, 1 * REM * metabolized_volume * seconds_per_tick, 150, affected_organ_flags)
		affected_mob.adjust_jitter_up_to(3 SECONDS * REM * metabolized_volume * seconds_per_tick, 1 MINUTES)
		if(SPT_PROB(5, seconds_per_tick))
			affected_mob.say(return_hippie_line(), forced = /datum/reagent/medicine/earthsblood)
	affected_mob.adjust_drugginess_up_to(10 SECONDS * REM * metabolized_volume * seconds_per_tick, 15 SECONDS * REM * metabolized_volume * seconds_per_tick)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH

/datum/reagent/medicine/earthsblood/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_hallucinations_up_to(5 SECONDS * REM * metabolized_volume * seconds_per_tick, 120 SECONDS)
	var/need_mob_update
	if(current_cycle > 26)
		need_mob_update = affected_mob.adjust_tox_loss(2 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		if(current_cycle > 101) //podpeople get out reeeeeeeeeeeeeeeeeeeee
			need_mob_update += affected_mob.adjust_tox_loss(3 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
	if(iscarbon(affected_mob))
...
/datum/reagent/medicine/haloperidol/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	for(var/datum/reagent/drug/reagent in affected_mob.reagents.reagent_list)
		affected_mob.reagents.remove_reagent(reagent.type, 2.5 * reagent.purge_multiplier * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(2 SECONDS * REM * metabolized_volume * seconds_per_tick)

	if(affected_mob.get_timed_status_effect_duration(/datum/status_effect/jitter) >= 6 SECONDS)
		affected_mob.adjust_jitter(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)

	if (affected_mob.get_timed_status_effect_duration(/datum/status_effect/hallucination) >= 10 SECONDS)
		affected_mob.adjust_hallucinations(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)

	if(affected_mob.get_stamina_loss() >= 100)
		affected_mob.reagents.remove_reagent(type, 0.5 * metabolization_rate * REM * metabolized_volume * seconds_per_tick)

	var/need_mob_update = FALSE
	if(SPT_PROB(10, seconds_per_tick))
		need_mob_update += affected_mob.adjust_organ_loss(ORGAN_SLOT_BRAIN, 1, 50, affected_organ_flags)
	need_mob_update += affected_mob.adjust_stamina_loss(1.25 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/changelingadrenaline/on_mob_life(mob/living/carbon/metabolizer, seconds_per_tick, metabolized_volume)
	. = ..()
	metabolizer.AdjustAllImmobility(-10 * REM * metabolized_volume * seconds_per_tick)
	if(metabolizer.adjust_stamina_loss(-15 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE))
		. = UPDATE_MOB_HEALTH
	metabolizer.set_jitter_if_lower(10 SECONDS * REM * metabolized_volume * seconds_per_tick)
	metabolizer.set_dizzy_if_lower(10 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/medicine/changelingadrenaline/overdose_process(mob/living/metabolizer, seconds_per_tick, metabolized_volume)
	. = ..()
	if(metabolizer.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/changelinghaste/on_mob_life(mob/living/carbon/metabolizer, seconds_per_tick, metabolized_volume)
	. = ..()
	if(metabolizer.adjust_tox_loss(1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/modafinil/on_mob_life(mob/living/carbon/metabolizer, seconds_per_tick, metabolized_volume)
	. = ..()
	if(overdosed) // We do not want any effects on OD
		return
	overdose_threshold = 0.5 * overdose_threshold + ((rand(-10, 10) / 10) * REM * metabolized_volume * seconds_per_tick) // for extra fun
	metabolizer.AdjustAllImmobility(-2.5 * REM * metabolized_volume * seconds_per_tick)
	metabolizer.adjust_stamina_loss(-1.5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
	metabolizer.set_jitter_if_lower(0.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	metabolization_rate = 0.005 * REAGENTS_METABOLISM * rand(5, 20) // randomizes metabolism between 0.02 and 0.08 per second
	return UPDATE_MOB_HEALTH

/datum/reagent/medicine/modafinil/overdose_start(mob/living/affected_mob, metabolized_volume)
	. = ..()
...
/datum/reagent/medicine/modafinil/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	overdose_progress++
	var/need_mob_update
	switch(overdose_progress)
		if(1 to 40)
			affected_mob.adjust_jitter_up_to(1 SECONDS * REM * metabolized_volume * seconds_per_tick, 20 SECONDS)
			affected_mob.adjust_stutter_up_to(1 SECONDS * REM * metabolized_volume * seconds_per_tick, 20 SECONDS)
			affected_mob.set_dizzy_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)
			if(SPT_PROB(30, seconds_per_tick))
				affected_mob.losebreath++
				need_mob_update = TRUE
		if(41 to 80)
			need_mob_update = affected_mob.adjust_oxy_loss(0.1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
			need_mob_update += affected_mob.adjust_stamina_loss(0.1 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
			affected_mob.adjust_jitter_up_to(1 SECONDS * REM * metabolized_volume * seconds_per_tick, 40 SECONDS)
			affected_mob.adjust_stutter_up_to(1 SECONDS * REM * metabolized_volume * seconds_per_tick, 40 SECONDS)
			affected_mob.set_dizzy_if_lower(10 SECONDS * REM * metabolized_volume * seconds_per_tick)
			if(SPT_PROB(30, seconds_per_tick))
				affected_mob.losebreath++
				need_mob_update = TRUE
			if(SPT_PROB(10, seconds_per_tick))
		...
		if(81)
			...
			need_mob_update = affected_mob.adjust_oxy_loss(0.05 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
			need_mob_update += affected_mob.adjust_stamina_loss(0.05 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		if(82 to INFINITY)
			REMOVE_TRAIT(affected_mob, TRAIT_SLEEPIMMUNE, type)
			affected_mob.Sleeping(50 * REM * metabolized_volume * seconds_per_tick)
			need_mob_update += affected_mob.adjust_oxy_loss(0.75 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
			need_mob_update += affected_mob.adjust_stamina_loss(0.75 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/psicodine/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_jitter(-6 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_dizzy(-6 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_confusion(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.disgust = max(affected_mob.disgust - (3 * REM * metabolized_volume * seconds_per_tick), 0)
	if(affected_mob.mob_mood != null && affected_mob.mob_mood.sanity <= SANITY_NEUTRAL) // only take effect if in negative sanity and then...
		affected_mob.mob_mood.adjust_sanity(2.5 * REM * metabolized_volume * seconds_per_tick, maximum = SANITY_NEUTRAL) // set minimum to prevent unwanted spiking over neutral

/datum/reagent/medicine/psicodine/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_hallucinations_up_to(5 SECONDS * REM * metabolized_volume * seconds_per_tick, 120 SECONDS)
	if(affected_mob.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/metafactor/overdose_start(mob/living/carbon/affected_mob, metabolized_volume)
	. = ..()
	metabolization_rate = 2  * REAGENTS_METABOLISM

/datum/reagent/medicine/metafactor/overdose_process(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(13, seconds_per_tick))
...
/datum/reagent/medicine/silibinin/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_organ_loss(ORGAN_SLOT_LIVER, -1 * REM * metabolized_volume * seconds_per_tick, required_organ_flag = affected_organ_flags)) // Add a chance to cure liver trauma once implemented.
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/polypyr/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume) //I wanted a collection of small positive effects, this is as hard to obtain as coniine after all.
	. = ..()
	var/need_mob_update
	need_mob_update = affected_mob.adjust_organ_loss(ORGAN_SLOT_LUNGS, -0.125 * REM * metabolized_volume * seconds_per_tick, required_organ_flag = affected_organ_flags)
	need_mob_update += affected_mob.adjust_brute_loss(-0.175 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/polypyr/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_organ_loss(ORGAN_SLOT_LUNGS, 0.25 * REM * metabolized_volume * seconds_per_tick, required_organ_flag = affected_organ_flags))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/granibitaluri/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	var/need_mob_update
	need_mob_update = affected_mob.adjust_brute_loss(0.5 * -healamount * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjust_fire_loss(0.5 * -healamount * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH

/datum/reagent/medicine/granibitaluri/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	need_mob_update = affected_mob.adjust_organ_loss(ORGAN_SLOT_LIVER, 0.1 * REM * metabolized_volume * seconds_per_tick, required_organ_flag = affected_organ_flags)
	need_mob_update += affected_mob.adjust_tox_loss(0.1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype) //Only really deadly if you eat over 100u
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/medicine/coagulant/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.coagulant_effect(0.5 * clot_rate * REM * metabolized_volume * seconds_per_tick))
		if(!was_working)
...
/datum/reagent/medicine/coagulant/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	if(SPT_PROB(7.5, seconds_per_tick))
		...
		if(prob(30))
			...
			affected_mob.adjust_oxy_loss(0.5 * rand(3, 4) * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)

		if(prob(50))
			var/obj/item/organ/lungs/our_lungs = affected_mob.get_organ_slot(ORGAN_SLOT_LUNGS)
			our_lungs.apply_organ_damage(0.5 * REM * metabolized_volume * seconds_per_tick)
		else
			var/obj/item/organ/heart/our_heart = affected_mob.get_organ_slot(ORGAN_SLOT_HEART)
			our_heart.apply_organ_damage(0.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/medicine/ondansetron/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(8, seconds_per_tick))
		affected_mob.adjust_drowsiness(1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	if(SPT_PROB(15, seconds_per_tick) && !affected_mob.get_stamina_loss())
		if(affected_mob.adjust_stamina_loss(5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE))
			. = UPDATE_MOB_HEALTH
	affected_mob.adjust_disgust(-5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/medicine/naloxone/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	for(var/opiate in opiates_to_clear)
		holder.remove_reagent(opiate, 1.5 * REM * metabolized_volume * seconds_per_tick)
	...
	affected_mob.adjust_drowsiness(-2.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	if(affected_mob.losebreath >= 1)
		affected_mob.losebreath -= 0.5 * REM * metabolized_volume * seconds_per_tick
		return UPDATE_MOB_HEALTH

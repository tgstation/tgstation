...
/datum/reagent/impurity/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/obj/item/organ/liver/liver = affected_mob.get_organ_slot(ORGAN_SLOT_LIVER)
	var/need_mob_update

	if(liver)//Though, lets be safe
		need_mob_update = affected_mob.adjust_organ_loss(ORGAN_SLOT_LIVER, 0.5 * liver_damage * REM * metabolized_volume * seconds_per_tick, required_organ_flag = affected_organ_flags)
	else
		need_mob_update = affected_mob.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)//Incase of no liver!
...
/datum/reagent/inverse/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_tox_loss(0.5 * tox_damage * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/inverse/eigenswap/on_mob_life(mob/living/carbon/affected_mob, metabolized_volume)
	. = ..()
...
/datum/reagent/inverse/cryostylane/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()

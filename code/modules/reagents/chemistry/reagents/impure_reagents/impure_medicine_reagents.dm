...
/datum/reagent/inverse/neurine/on_mob_life(mob/living/carbon/owner, delta_time, times_fired)
	...
	var/list/forbiddentraumas = list(/datum/brain_trauma/severe/split_personality, /datum/brain_trauma/special/obsessed, /datum/brain_trauma/hypnosis) // Split personality uses a ghost, I don't want to use a ghost for a temp thing; Obsessed sets the owner as an antag - I presume this will lead to problems, so we'll remove it; Hypnosis, same reason as obsessed, plus a bug makes it remain even after the neurowhine purges and then turn into "nothing" on the med reading upon a second application

...
/datum/reagent/inverse/corazargh/on_mob_metabolize(mob/living/affected_mob)
	...
	var/obj/item/organ/heart/original_heart = affected_mob.get_organ_slot(ORGAN_SLOT_HEART)
	if(!original_heart)
		return
	original_heart_ref = WEAKREF(original_heart)

	var/obj/item/organ/heart/cursed/manual_heart = new(null, src)
...
/datum/reagent/inverse/corazargh/proc/on_gained_organ(mob/affected_mob, obj/item/organ/organ)
	...
	var/obj/item/organ/heart/cursed/manual_heart = manual_heart_ref?.resolve()
	if(organ == manual_heart)
		return

	var/mob/living/carbon/affected_carbon = affected_mob
	var/obj/item/organ/heart/original_heart = organ
	original_heart_ref = WEAKREF(original_heart)
...
/datum/reagent/inverse/corazargh/proc/on_removed_organ(mob/prev_owner, obj/item/organ/organ)
	...
	if(organ != manual_heart)
		return
	var/obj/item/organ/heart/original_heart = original_heart_ref?.resolve()
	if(!original_heart)
		return

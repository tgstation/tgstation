...
/datum/reagent/healium/on_mob_life(mob/living/breather, seconds_per_tick, times_fired)
	...
	var/need_mob_update
	need_mob_update = metabolic_health_adjust(breather, -2 * REM * seconds_per_tick, FIRE)
	need_mob_update += metabolic_health_adjust(breather, -5 * REM * seconds_per_tick, TOX)
	need_mob_update += metabolic_health_adjust(breather, -2 * REM * seconds_per_tick, BRUTE)
	if(need_mob_update)
...
/datum/reagent/nitrium_high_metabolization/on_mob_life(mob/living/carbon/breather, seconds_per_tick, times_fired)
	. = ..()
	var/need_mob_update
	need_mob_update = metabolic_health_adjust(breather, -4 * REM * seconds_per_tick, STAMINA)
	need_mob_update += metabolic_health_adjust(breather, 0.1 * (current_cycle-1) * REM * seconds_per_tick, TOX) // 1 toxin damage per cycle at cycle 10
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/zauker/on_mob_life(mob/living/breather, seconds_per_tick, times_fired)
	. = ..()
	var/need_mob_update
	need_mob_update = metabolic_health_adjust(breather, 6 * REM * seconds_per_tick, BRUTE)
	need_mob_update += metabolic_health_adjust(breather, 1 * REM * seconds_per_tick, OXY)
	need_mob_update += metabolic_health_adjust(breather, 2 * REM * seconds_per_tick, FIRE)
	need_mob_update += metabolic_health_adjust(breather, 2 * REM * seconds_per_tick, TOX)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH

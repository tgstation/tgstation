...
/datum/reagent/nitrium_high_metabolization/on_mob_life(mob/living/carbon/breather, seconds_per_tick, times_fired)
	...
	need_mob_update += breather.adjustToxLoss(0.1 * current_cycle * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype) // 1 toxin damage per cycle at cycle 10
	if(need_mob_update)
		. = UPDATE_MOB_HEALTH

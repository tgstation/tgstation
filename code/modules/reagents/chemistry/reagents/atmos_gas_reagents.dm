...
/datum/reagent/healium/on_mob_life(mob/living/breather, seconds_per_tick, times_fired)
	...
	var/need_mob_update
	need_mob_update = breather.metabolic_damage_adjust(src, FIRELOSS, -2 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += breather.metabolic_damage_adjust(src, TOXLOSS, -5 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
	need_mob_update += breather.metabolic_damage_adjust(src, BRUTELOSS, -2 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
...
/datum/reagent/nitrium_high_metabolization/on_mob_life(mob/living/carbon/breather, seconds_per_tick, times_fired)
	. = ..()
	var/need_mob_update
	need_mob_update = breather.metabolic_damage_adjust(src, STAMINALOSS, -4 * REM * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
	need_mob_update += breather.metabolic_damage_adjust(src, TOXLOSS, 0.1 * (current_cycle-1) * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype) // 1 toxin damage per cycle at cycle 10
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/zauker/on_mob_life(mob/living/breather, seconds_per_tick, times_fired)
	. = ..()
	var/need_mob_update
	need_mob_update = breather.metabolic_damage_adjust(src, BRUTELOSS, 6 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += breather.metabolic_damage_adjust(src, OXYLOSS, 1 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	need_mob_update += breather.metabolic_damage_adjust(src, FIRELOSS, 2 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += breather.metabolic_damage_adjust(src, TOXLOSS, 2 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH

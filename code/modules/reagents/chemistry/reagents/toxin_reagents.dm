...
/datum/reagent/toxin/lexorin/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	...
	if(.)
		affected_mob.adjustOxyLoss(5 * REM * normalise_creation_purity() * delta_time, FALSE, forced = FALSE, required_biotype = affected_biotype)
...
/datum/reagent/toxin/zombiepowder/on_mob_metabolize(mob/living/holder_mob)
	. = ..()
	holder_mob.adjustOxyLoss(0.5*REM, FALSE, forced = FALSE, required_biotype = affected_biotype)
...
/datum/reagent/toxin/histamine/overdose_process(mob/living/affected_mob, delta_time, times_fired)
	affected_mob.adjustOxyLoss(2 * REM * delta_time, FALSE, forced = FALSE, required_biotype = affected_biotype)
	...
	if(DT_PROB(13, delta_time))
		...
		switch(picked_option)
			...
			if(2)
				...
				affected_mob.adjustOxyLoss(rand(5,25), FALSE, forced = FALSE, required_biotype = affected_biotype)
				. = TRUE
			if(3)

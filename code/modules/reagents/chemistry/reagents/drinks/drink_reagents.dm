...
/datum/reagent/consumable/orangejuice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/tomatojuice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.get_fire_loss() && SPT_PROB(10, seconds_per_tick))
		if(affected_mob.heal_bodypart_damage(brute = 0, burn = 0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/limejuice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/carrotjuice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_eye_blur(-1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_temp_blindness(-1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	var/need_mob_update
	switch(current_cycle)
		if(21 to 110)
			if(SPT_PROB(100 * (1 - (sqrt(110 - current_cycle) / 10)), seconds_per_tick))
				need_mob_update = affected_mob.adjust_organ_loss(ORGAN_SLOT_EYES, -1 * REM * metabolized_volume * seconds_per_tick)
		if(110 to INFINITY)
			need_mob_update = affected_mob.adjust_organ_loss(ORGAN_SLOT_EYES, -1 * REM * metabolized_volume * seconds_per_tick)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/poisonberryjuice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/banana/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/nothing/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/laughter/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tic, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/superlaughter/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/pickle/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/obj/item/organ/liver/liver = affected_mob.get_organ_slot(ORGAN_SLOT_LIVER)
...
/datum/reagent/consumable/milk/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	if(affected_mob.get_brute_loss() && SPT_PROB(10, seconds_per_tick))
		if(affected_mob.heal_bodypart_damage(brute = 0.5 * REM * metabolized_volume * seconds_per_tick, burn = 0, updating_health = FALSE))
			. = UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/soymilk/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.get_brute_loss() && SPT_PROB(10, seconds_per_tick))
...
/datum/reagent/consumable/cream/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/coffee/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.set_jitter_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)

/datum/reagent/consumable/coffee/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_dizzy(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.AdjustSleeping(-2 SECONDS * REM * metabolized_volume * seconds_per_tick)
	//310.15 is the normal bodytemp.
	affected_mob.adjust_bodytemperature(25 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
	if(holder.has_reagent(/datum/reagent/consumable/frostoil))
		holder.remove_reagent(/datum/reagent/consumable/frostoil, 2.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/tea/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_dizzy(-2 SECONDS * REM * metabolized_volume * seconds_per_tick)
	...
	if(affected_mob.get_tox_loss() && SPT_PROB(10, seconds_per_tick))
		if(affected_mob.adjust_tox_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
			. = UPDATE_MOB_HEALTH
	affected_mob.adjust_bodytemperature(10 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/tea/arnold_palmer/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/icecoffee/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.set_jitter_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)

/datum/reagent/consumable/icecoffee/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_dizzy(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.AdjustSleeping(-2 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/hot_ice_coffee/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.set_jitter_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)

/datum/reagent/consumable/hot_ice_coffee/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_dizzy(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.AdjustSleeping(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-3.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
	if(affected_mob.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/icetea/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_dizzy(-2 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(-1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.AdjustSleeping(-2 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/space_cola/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/roy_rogers/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	affected_mob.set_jitter_if_lower(6 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
	return ..()
...
/datum/reagent/consumable/nuka_cola/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.set_jitter_if_lower(20 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.set_drugginess(30 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_dizzy(1.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/rootbeer/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	affected_mob.set_jitter_if_lower(2 SECONDS * REM * metabolized_volume * seconds_per_tick)
	if(prob(50))
		affected_mob.adjust_dizzy(1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	if(current_cycle > 10)
		affected_mob.adjust_dizzy(1.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/grey_bull/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.set_jitter_if_lower(20 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_dizzy(1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.remove_status_effect(/datum/status_effect/drowsiness)
...
/datum/reagent/consumable/spacemountainwind/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_drowsiness(-7 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.AdjustSleeping(-1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
	affected_mob.set_jitter_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/dr_gibb/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_drowsiness(-6 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/space_up/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(-4 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/lemon_lime/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(-4 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/pwr_game/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(-4 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/shamblers/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(-4 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/sodawater/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_dizzy(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/tonic/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_dizzy(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.AdjustSleeping(-2 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/wellcheers/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_drowsiness(1.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	var/need_mob_update
	switch(affected_mob.mob_mood.sanity_level)
		if (SANITY_LEVEL_GREAT to SANITY_LEVEL_NEUTRAL)
			need_mob_update = affected_mob.adjust_brute_loss(-0.75 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		...
		if (SANITY_LEVEL_CRAZY to SANITY_LEVEL_INSANE)
			need_mob_update = affected_mob.adjust_stamina_loss(1.5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/monkey_energy/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.set_jitter_if_lower(40 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_dizzy(1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.remove_status_effect(/datum/status_effect/drowsiness)
	affected_mob.AdjustSleeping(-2 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/monkey_energy/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, FALSE, affected_mob.get_body_temp_normal()))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/soy_latte/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.set_jitter_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)

/datum/reagent/consumable/soy_latte/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_dizzy(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	var/need_mob_update
	need_mob_update = affected_mob.SetSleeping(0)
	affected_mob.adjust_bodytemperature(2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
	if(affected_mob.get_brute_loss() && SPT_PROB(10, seconds_per_tick))
		need_mob_update += affected_mob.heal_bodypart_damage(brute = 0.5 * REM * metabolized_volume * seconds_per_tick, burn = 0, updating_health = FALSE)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/cafe_latte/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.set_jitter_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)

/datum/reagent/consumable/cafe_latte/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_dizzy(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(-6 SECONDS * REM * metabolized_volume * seconds_per_tick)
	var/need_mob_update
...
/datum/reagent/consumable/doctor_delight/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	...
	if(affected_mob.nutrition && (affected_mob.nutrition - 2 > 0))
		...
		if(!(HAS_TRAIT(liver, TRAIT_MEDICAL_METABOLISM)))
			...
			affected_mob.adjust_nutrition(-1 * REM * metabolized_volume * seconds_per_tick)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/cinderella/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_disgust(-2.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/pumpkin_latte/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.set_jitter_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)

/datum/reagent/consumable/pumpkin_latte/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_dizzy(-5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drowsiness(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	var/need_mob_update
...
/datum/reagent/consumable/grape_soda/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/hot_coco/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	affected_mob.adjust_bodytemperature(2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
	if(affected_mob.get_brute_loss() && SPT_PROB(10, seconds_per_tick))
		if(affected_mob.heal_bodypart_damage(brute = 0.5 * REM * metabolized_volume * seconds_per_tick, burn = 0, updating_health = FALSE))
			. = UPDATE_MOB_HEALTH
	if(holder.has_reagent(/datum/reagent/consumable/capsaicin))
		holder.remove_reagent(/datum/reagent/consumable/capsaicin, 1 * REM * metabolized_volume * seconds_per_tick)
	return ..() || .
...
/datum/reagent/consumable/italian_coco/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/menthol/on_mob_life(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/grenadine/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(IS_REVOLUTIONARY(drinker))
...
/datum/reagent/consumable/cream_soda/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/sol_dry/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_disgust(-2.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/shirley_temple/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
...
/datum/reagent/consumable/red_queen/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/aloejuice/on_mob_life(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/agua_fresca/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(-4 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
	if(affected_mob.get_tox_loss() && SPT_PROB(10, seconds_per_tick))
...
/datum/reagent/consumable/mushroom_tea/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(islizard(affected_mob))
		if(affected_mob.adjust_oxy_loss(-0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/cucumberlemonade/on_mob_life(mob/living/carbon/doll, seconds_per_tick, metabolized_volume)
	. = ..()
	doll.adjust_bodytemperature(-4 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, doll.get_body_temp_normal())
...
/datum/reagent/consumable/mississippi_queen/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	switch(current_cycle)
		if(11 to 21)
		...
		if(21 to 31)
			if(SPT_PROB(15, seconds_per_tick))
				drinker.adjust_confusion(2 SECONDS * REM * metabolized_volume * seconds_per_tick)
		if(31 to 201)
			drinker.adjust_hallucinations(30 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/t_letter/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/fruit_punch/on_mob_life(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	...
	if(found_valid_cooler)
		...
		need_mob_update = affected_mob.adjust_tox_loss(-0.3 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update = affected_mob.adjust_brute_loss(-0.3 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update = affected_mob.adjust_fire_loss(-0.3 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		...
		need_mob_update = affected_mob.apply_damage(0.75 * REM * metabolized_volume * seconds_per_tick, TOX)
...
/datum/reagent/consumable/ethanol/bitters_soda/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_disgust(-2.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/lean/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_jitter(1.25 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_stutter(1.125 SECONDS * REM * metabolized_volume * seconds_per_tick)
	affected_mob.adjust_drugginess(1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	if(SPT_PROB(15, seconds_per_tick))
		affected_mob.emote(pick("taunt","twitch","shiver","laugh","moan","blush","stare"))
	if(current_cycle > 16 && SPT_PROB(3.5, seconds_per_tick))
		affected_mob.adjust_dizzy(7.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
		affected_mob.adjust_drowsiness(3.25 SECONDS * REM * metabolized_volume * seconds_per_tick)

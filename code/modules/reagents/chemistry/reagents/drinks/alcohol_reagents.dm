...
/datum/reagent/consumable/ethanol/cuba_libre/on_mob_life(mob/living/carbon/cubano, seconds_per_tick, times_fired)
	...
	if(cubano.mind && cubano.mind.has_antag_datum(/datum/antagonist/rev)) //Cuba Libre, the traditional drink of revolutions! Heals revolutionaries.
		need_mob_update = cubano.metabolic_damage_adjust(src, BRUTELOSS, -1 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += cubano.metabolic_damage_adjust(src, FIRELOSS, -1 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += cubano.metabolic_damage_adjust(src, TOXLOSS, -1 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += cubano.metabolic_damage_adjust(src, OXYLOSS, -5 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/screwdrivercocktail/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(HAS_TRAIT(liver, TRAIT_ENGINEER_METABOLISM))
		...
		if (HAS_TRAIT(drinker, TRAIT_IRRADIATED))
			if(drinker.metabolic_damage_adjust(src, TOXLOSS, -2 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
				return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/beepsky_smash/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(liver && HAS_TRAIT(liver, TRAIT_LAW_ENFORCEMENT_METABOLISM))
		if(drinker.metabolic_damage_adjust(src, STAMINALOSS, -10 * REM * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype))
			. = UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/manly_dorf/on_mob_life(mob/living/carbon/dwarf, seconds_per_tick, times_fired)
	...
	if(dorf_mode)
		var/need_mob_update
		need_mob_update = dwarf.metabolic_damage_adjust(src, BRUTELOSS, -2 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += dwarf.metabolic_damage_adjust(src, FIRELOSS, -2 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/barefoot/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(ishuman(drinker)) //Barefoot causes the imbiber to quickly regenerate brute trauma if they're not wearing shoes.
		...
		if(!unshoed.shoes)
			if(unshoed.metabolic_damage_adjust(src, BRUTELOSS, -3 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype))
				return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/hearty_punch/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(drinker.health <= 0)
		var/need_mob_update
		need_mob_update = drinker.metabolic_damage_adjust(src, BRUTELOSS, -3 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += drinker.metabolic_damage_adjust(src, FIRELOSS, -3 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += drinker.metabolic_damage_adjust(src, OXYLOSS, -4 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		need_mob_update += drinker.metabolic_damage_adjust(src, TOXLOSS, -3 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/neurotoxin/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	var/need_mob_update
	need_mob_update = drinker.metabolic_organ_adjust(src, ORGAN_SLOT_BRAIN, 1 * REM * seconds_per_tick, 150, required_organ_flag = affected_organ_flags)
	if(SPT_PROB(10, seconds_per_tick))
		need_mob_update += drinker.metabolic_damage_adjust(src, STAMINALOSS, 10 * REM * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
	...
	if(current_cycle > 6)
		if(SPT_PROB(10, seconds_per_tick))
			...
			need_mob_update += drinker.metabolic_damage_adjust(src, STAMINALOSS, 10 * REM * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		if(current_cycle > 31)
			need_mob_update += drinker.metabolic_organ_adjust(src, ORGAN_SLOT_BRAIN, 2 * REM * seconds_per_tick, required_organ_flag = affected_organ_flags)
...
/datum/reagent/consumable/ethanol/neurotoxin/on_mob_end_metabolize(mob/living/carbon/drinker)
	...
	drinker.metabolic_damage_adjust(src, STAMINALOSS, 10, required_biotype = affected_biotype)
...
/datum/reagent/consumable/ethanol/quintuple_sec/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(liver && HAS_TRAIT(liver, TRAIT_LAW_ENFORCEMENT_METABOLISM))
		...
		need_mob_update += drinker.metabolic_damage_adjust(src, STAMINALOSS, -5 * REM * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/bastion_bourbon/on_mob_metabolize(mob/living/drinker)
	...
	need_mob_update = drinker.metabolic_damage_adjust(src, BRUTELOSS, -heal_amt, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += drinker.metabolic_damage_adjust(src, FIRELOSS, -heal_amt, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += drinker.metabolic_damage_adjust(src, TOXLOSS, -heal_amt, updating_health = FALSE, required_biotype = affected_biotype)
	need_mob_update += drinker.metabolic_damage_adjust(src, OXYLOSS, -heal_amt, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	// heal stamina loss on first metabolization, but only to a max of 20
	need_mob_update += drinker.metabolic_damage_adjust(src, STAMINALOSS, max(-heal_amt * 5, -20), updating_stamina = FALSE, required_biotype = affected_biotype)
	if(need_mob_update)
...
/datum/reagent/consumable/ethanol/bastion_bourbon/on_mob_life(mob/living/drinker, seconds_per_tick, times_fired)
	...
	if(drinker.health > 0)
		var/need_mob_update
		need_mob_update = drinker.metabolic_damage_adjust(src, BRUTELOSS, -1 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += drinker.metabolic_damage_adjust(src, FIRELOSS, -1 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += drinker.metabolic_damage_adjust(src, TOXLOSS, -0.5 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += drinker.metabolic_damage_adjust(src, OXYLOSS, -3 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		need_mob_update += drinker.metabolic_damage_adjust(src, STAMINALOSS, -5 * REM * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/between_the_sheets/on_mob_life(mob/living/drinker, seconds_per_tick, times_fired)
	...
	if(drinker.getBruteLoss() && drinker.getFireLoss()) //If you are damaged by both types, slightly increased healing but it only heals one. The more the merrier wink wink.
		if(prob(50))
			need_mob_update = drinker.metabolic_damage_adjust(src, BRUTELOSS, -0.25 * REM * seconds_per_tick, updating_health = FALSE,  required_bodytype = affected_bodytype)
		else
			need_mob_update = drinker.metabolic_damage_adjust(src, FIRELOSS, -0.25 * REM * seconds_per_tick, updating_health = FALSE,  required_bodytype = affected_bodytype)
	else if(drinker.getBruteLoss()) //If you have only one, it still heals but not as well.
		need_mob_update = drinker.metabolic_damage_adjust(src, BRUTELOSS, -0.2 * REM * seconds_per_tick, updating_health = FALSE,  required_bodytype = affected_bodytype)
	else if(drinker.getFireLoss())
		need_mob_update = drinker.metabolic_damage_adjust(src, FIRELOSS, -0.2 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/fernet/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(drinker.nutrition <= NUTRITION_LEVEL_STARVING)
		if(drinker.metabolic_damage_adjust(src, TOXLOSS, 1 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
			. = UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/fernet_cola/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	. = ..()
	if(drinker.nutrition <= NUTRITION_LEVEL_STARVING)
		if(drinker.metabolic_damage_adjust(src, TOXLOSS, 0.5 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
			. = UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/fanciulli/on_mob_metabolize(mob/living/drinker)
	. = ..()
	if(drinker.health > 0)
		drinker.metabolic_damage_adjust(src, STAMINALOSS, 20, required_biotype = affected_biotype)
...
/datum/reagent/consumable/ethanol/branca_menta/on_mob_metabolize(mob/living/drinker)
	. = ..()
	if(drinker.health > 0)
		drinker.metabolic_damage_adjust(src, STAMINALOSS, 35, required_biotype = affected_biotype)
...
/datum/reagent/consumable/ethanol/wizz_fizz/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(drinker?.mind?.has_antag_datum(/datum/antagonist/wizard))
		...
		need_mob_update += drinker.metabolic_damage_adjust(src, OXYLOSS, -1 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		need_mob_update += drinker.metabolic_damage_adjust(src, TOXLOSS, -1 * REM * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += drinker.metabolic_damage_adjust(src, STAMINALOSS, -5  * REM * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/turbo/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(drinker.metabolic_damage_adjust(src, STAMINALOSS, -0.5 * drinker.get_drunk_amount() * REM * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/trappist/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(drinker.mind?.holy_role)
		if(drinker.metabolic_damage_adjust(src, FIRELOSS, -2.5 * REM * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype))
			. = UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/suffering_bastard/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(affected_mob.metabolic_organ_adjust(src, ORGAN_SLOT_BRAIN, -0.5 * REM * seconds_per_tick * normalise_creation_purity(), required_organ_flag = affected_organ_flags)) //heals brain damage very slowly, about 12 damage per 5u
		return UPDATE_MOB_HEALTH

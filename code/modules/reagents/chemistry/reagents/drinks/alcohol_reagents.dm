...
/datum/reagent/consumable/ethanol/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(drinker.get_drunk_amount() < volume * boozepwr * ALCOHOL_THRESHOLD_MODIFIER || boozepwr < 0)
		...
		drinker.adjust_drunk_effect(0.5 * sqrt(volume) * booze_power * ALCOHOL_RATE * REM * metabolized_volume * seconds_per_tick)
		if(boozepwr > 0)
...
/datum/reagent/consumable/ethanol/beer/green/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/beer/green/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/kahlua/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.set_dizzy_if_lower(5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	drinker.adjust_drowsiness(-3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	drinker.AdjustSleeping(-2 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/whiskey/candycorn/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(5, seconds_per_tick))
		drinker.adjust_hallucinations(2 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/thirteenloko/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_drowsiness(-7 SECONDS * REM * metabolized_volume * seconds_per_tick)
	drinker.AdjustSleeping(-2 SECONDS * REM * metabolized_volume * seconds_per_tick)
	drinker.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, drinker.get_body_temp_normal())
	if(!HAS_TRAIT(drinker, TRAIT_ALCOHOL_TOLERANCE))
		drinker.set_jitter_if_lower(10 SECONDS)

/datum/reagent/consumable/ethanol/thirteenloko/overdose_start(mob/living/drinker, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/thirteenloko/overdose_process(mob/living/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	if(SPT_PROB(2.5, seconds_per_tick) && iscarbon(drinker))
		...
		if(eyes && IS_ORGANIC_ORGAN(eyes)) // doesn't affect robotic eyes
			if(drinker.is_blind())
				...
				if(drinker.adjust_brute_loss(7.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype))
					. = UPDATE_MOB_HEALTH
			else
...
/datum/reagent/consumable/ethanol/bilk/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(drinker.get_brute_loss() && SPT_PROB(5, seconds_per_tick))
		if(drinker.heal_bodypart_damage(brute = 1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/threemileisland/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.set_drugginess(50 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/absinthe/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(5, seconds_per_tick) && !HAS_TRAIT(drinker, TRAIT_ALCOHOL_TOLERANCE))
		drinker.adjust_hallucinations(4 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/cuba_libre/on_mob_life(mob/living/carbon/cubano, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	if(cubano.mind && cubano.mind.has_antag_datum(/datum/antagonist/rev)) //Cuba Libre, the traditional drink of revolutions! Heals revolutionaries.
		need_mob_update = cubano.adjust_brute_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += cubano.adjust_fire_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += cubano.adjust_tox_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += cubano.adjust_oxy_loss(-2.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/screwdrivercocktail/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	var/obj/item/organ/liver/liver = drinker.get_organ_slot(ORGAN_SLOT_LIVER)
	if(HAS_TRAIT(liver, TRAIT_ENGINEER_METABOLISM))
		ADD_TRAIT(drinker, TRAIT_HALT_RADIATION_EFFECTS, "[type]")
		if (HAS_TRAIT(drinker, TRAIT_IRRADIATED))
			if(drinker.adjust_tox_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
				return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/bloody_mary/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_blood_volume(0.5 * (0.25 + round(2 * drinker.get_drunk_amount() / 40, 0.1)) * REM * metabolized_volume * seconds_per_tick, maximum = BLOOD_VOLUME_NORMAL) // Bloody Mary restores blood loss based on how drunk you are
...
/datum/reagent/consumable/ethanol/tequila_sunrise/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
...
/datum/reagent/consumable/ethanol/toxins_special/on_mob_life(mob/living/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_bodytemperature(7.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, drinker.get_body_temp_normal() + 20) //310.15 is the normal bodytemp.
...
/datum/reagent/consumable/ethanol/beepsky_smash/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.set_jitter_if_lower(4 SECONDS)
	var/obj/item/organ/liver/liver = drinker.get_organ_slot(ORGAN_SLOT_LIVER)
	// if you have a liver and that liver is an officer's liver
	if(liver && HAS_TRAIT(liver, TRAIT_LAW_ENFORCEMENT_METABOLISM))
		if(drinker.adjust_stamina_loss(-5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype))
			. = UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/beepsky_smash/overdose_start(mob/living/carbon/drinker, metabolized_volume)
	. = ..()
	var/obj/item/organ/liver/liver = drinker.get_organ_slot(ORGAN_SLOT_LIVER)
	// if you don't have a liver, or your liver isn't an officer's liver
	if(!liver || !HAS_TRAIT(liver, TRAIT_LAW_ENFORCEMENT_METABOLISM))
...
/datum/reagent/consumable/ethanol/manly_dorf/on_mob_life(mob/living/carbon/dwarf, seconds_per_tick, metabolized_volume)
	. = ..()
	if(dorf_mode)
		var/need_mob_update
		need_mob_update = dwarf.adjust_brute_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += dwarf.adjust_fire_loss(-1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/manhattan_proj/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.set_drugginess(30 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/antifreeze/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_bodytemperature(10 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, drinker.get_body_temp_normal() + 20) //310.15 is the normal bodytemp.
...
/datum/reagent/consumable/ethanol/barefoot/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(ishuman(drinker)) //Barefoot causes the imbiber to quickly regenerate brute trauma if they're not wearing shoes.
		...
		if(!unshoed.shoes)
			if(unshoed.adjust_brute_loss(-1.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype))
				return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/singulo/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/sbiten/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_bodytemperature(25 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, BODYTEMP_HEAT_DAMAGE_LIMIT) //310.15 is the normal bodytemp.
...
/datum/reagent/consumable/ethanol/iced_beer/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_bodytemperature(-10 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, T0C) //310.15 is the normal bodytemp.
...
/datum/reagent/consumable/ethanol/changelingsting/on_mob_life(mob/living/carbon/target, seconds_per_tick, metabolized_volume)
	. = ..()
	var/datum/antagonist/changeling/changeling = IS_CHANGELING(target)
	changeling?.adjust_chemicals(0.5 * metabolization_rate * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/syndicatebomb/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(2.5, seconds_per_tick))
...
/datum/reagent/consumable/ethanol/bananahonk/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	var/obj/item/organ/liver/liver = drinker.get_organ_slot(ORGAN_SLOT_LIVER)
	if((liver && HAS_TRAIT(liver, TRAIT_COMEDY_METABOLISM)) || is_simian(drinker))
		if(drinker.heal_bodypart_damage(brute = 0.5 * REM * metabolized_volume * seconds_per_tick, burn = 1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/silencer/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/fetching_fizz/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/hearty_punch/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(drinker.health <= 0)
		var/need_mob_update
		need_mob_update = drinker.adjust_brute_loss(-1.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += drinker.adjust_fire_loss(-1.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += drinker.adjust_oxy_loss(-2 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		need_mob_update += drinker.adjust_tox_loss(-1.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/atomicbomb/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.set_drugginess(50 SECONDS * REM * metabolized_volume * seconds_per_tick)
	if(!HAS_TRAIT(drinker, TRAIT_ALCOHOL_TOLERANCE))
		drinker.adjust_confusion(1 SECONDS * REM * metabolized_volume * seconds_per_tick)
	drinker.set_dizzy_if_lower(10 SECONDS * REM * metabolized_volume * seconds_per_tick)
	drinker.adjust_slurring(3 SECONDS * REM * metabolized_volume * seconds_per_tick)
	switch(current_cycle)
		if(52 to 201)
			drinker.Sleeping(50 * REM * metabolized_volume * seconds_per_tick)
		if(202 to INFINITY)
			drinker.AdjustSleeping(2 SECONDS * REM * metabolized_volume * seconds_per_tick)
			if(drinker.adjust_tox_loss(1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
				return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/gargle_blaster/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_dizzy(1.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	switch(current_cycle)
		if(16 to 46)
			drinker.adjust_slurring(1.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
		if(46 to 56)
			if(SPT_PROB(30, seconds_per_tick))
				drinker.adjust_confusion(1.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
		if(56 to 201)
			drinker.set_drugginess(55 SECONDS * REM * metabolized_volume * seconds_per_tick)
		if(201 to INFINITY)
			if(drinker.adjust_tox_loss(1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
				return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/neurotoxin/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.set_drugginess(50 SECONDS * REM * metabolized_volume * seconds_per_tick)
	drinker.adjust_dizzy(2 SECONDS * REM * metabolized_volume * seconds_per_tick)
	var/need_mob_update
	need_mob_update = drinker.adjust_organ_loss(ORGAN_SLOT_BRAIN, 0.5 * REM * metabolized_volume * seconds_per_tick, 150, required_organ_flag = affected_organ_flags)
	if(SPT_PROB(10, seconds_per_tick))
		need_mob_update += drinker.adjust_stamina_loss(5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
	...
	if(current_cycle > 6)
		if(SPT_PROB(10, seconds_per_tick))
			...
			need_mob_update += drinker.adjust_stamina_loss(5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		if(current_cycle > 31)
			need_mob_update += drinker.adjust_organ_loss(ORGAN_SLOT_BRAIN, 1 * REM * metabolized_volume * seconds_per_tick, required_organ_flag = affected_organ_flags)
...
/datum/reagent/consumable/ethanol/hippies_delight/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.set_slurring_if_lower(0.5 SECONDS * REM * metabolized_volume * seconds_per_tick)
	...
	switch(current_cycle)
		...
		if(6 to 11)
			drinker.set_jitter_if_lower(20 SECONDS * REM * metabolized_volume * seconds_per_tick)
			drinker.set_dizzy_if_lower(20 SECONDS * REM * metabolized_volume * seconds_per_tick)
			drinker.set_drugginess(45 SECONDS * REM * metabolized_volume * seconds_per_tick)
			if(SPT_PROB(10, seconds_per_tick))
				drinker.emote(pick("twitch","giggle"))
		if (11 to 201)
			drinker.set_jitter_if_lower(40 SECONDS * REM * metabolized_volume * seconds_per_tick)
			drinker.set_dizzy_if_lower(40 SECONDS * REM * metabolized_volume * seconds_per_tick)
			drinker.set_drugginess(1 MINUTES * REM * metabolized_volume * seconds_per_tick)
			if(SPT_PROB(16, seconds_per_tick))
				drinker.emote(pick("twitch","giggle"))
		if(201 to INFINITY)
			drinker.set_jitter_if_lower(60 SECONDS * REM * metabolized_volume * seconds_per_tick)
			drinker.set_dizzy_if_lower(60 SECONDS * REM * metabolized_volume * seconds_per_tick)
			drinker.set_drugginess(1.25 MINUTES * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/narsour/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_timed_status_effect(3 SECONDS * REM * metabolized_volume * seconds_per_tick, /datum/status_effect/speech/slurring/cult, max_duration = 6 SECONDS)
	drinker.adjust_stutter_up_to(3 SECONDS * REM * metabolized_volume * seconds_per_tick, 6 SECONDS)
...
/datum/reagent/consumable/ethanol/quadruple_sec/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/quintuple_sec/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	//Securidrink in line with the Screwdriver for engineers or Nothing for mimes but STRONG..
	var/obj/item/organ/liver/liver = drinker.get_organ_slot(ORGAN_SLOT_LIVER)
	if(liver && HAS_TRAIT(liver, TRAIT_LAW_ENFORCEMENT_METABOLISM))
		var/need_mob_update
		need_mob_update = drinker.heal_bodypart_damage(1 * REM * metabolized_volume * seconds_per_tick, 2 * REM * metabolized_volume *  seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += drinker.adjust_stamina_loss(-2.5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/bastion_bourbon/on_mob_life(mob/living/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(drinker.health > 0)
		var/need_mob_update
		need_mob_update = drinker.adjust_brute_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += drinker.adjust_fire_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += drinker.adjust_tox_loss(-0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += drinker.adjust_oxy_loss(-1.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		need_mob_update += drinker.adjust_stamina_loss(-2.5 * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/squirt_cider/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.satiety += 2.5 * REM * metabolized_volume * seconds_per_tick //for context, vitamins give 15 satiety per second
...
/datum/reagent/consumable/ethanol/sugar_rush/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.satiety -= 5 * REM * metabolized_volume * seconds_per_tick //junky as hell! a whole glass will keep you from being able to eat junk food
...
/datum/reagent/consumable/ethanol/peppermint_patty/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.apply_status_effect(/datum/status_effect/throat_soothed)
	drinker.adjust_bodytemperature(2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, drinker.get_body_temp_normal())
...
/datum/reagent/consumable/ethanol/alexander/on_mob_life(mob/living/drinker, seconds_per_tick, metabolized_volume)
...
/datum/reagent/consumable/ethanol/between_the_sheets/on_mob_life(mob/living/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	...
	if(drinker.get_brute_loss() && drinker.get_fire_loss()) //If you are damaged by both types, slightly increased healing but it only heals one. The more the merrier wink wink.
		if(prob(50))
			need_mob_update = drinker.adjust_brute_loss(-0.125 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE,  required_bodytype = affected_bodytype)
		else
			need_mob_update = drinker.adjust_fire_loss(-0.125 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE,  required_bodytype = affected_bodytype)
	else if(drinker.get_brute_loss()) //If you have only one, it still heals but not as well.
		need_mob_update = drinker.adjust_brute_loss(-0.1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE,  required_bodytype = affected_bodytype)
	else if(drinker.get_fire_loss())
		need_mob_update = drinker.adjust_fire_loss(-0.1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/fernet/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(drinker.nutrition <= NUTRITION_LEVEL_STARVING)
		if(drinker.adjust_tox_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
			. = UPDATE_MOB_HEALTH
	drinker.adjust_nutrition(-2.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/fernet_cola/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(drinker.nutrition <= NUTRITION_LEVEL_STARVING)
		if(drinker.adjust_tox_loss(0.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
			. = UPDATE_MOB_HEALTH
	drinker.adjust_nutrition(-1.5 * REM * metabolized_volume * seconds_per_tick)
	drinker.overeatduration = 0
...
/datum/reagent/consumable/ethanol/fanciulli/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_nutrition(-2.5 * REM * metabolized_volume * seconds_per_tick)
	drinker.overeatduration = 0
...
/datum/reagent/consumable/ethanol/branca_menta/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_bodytemperature(-10 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, T0C)
...
/datum/reagent/consumable/ethanol/blank_paper/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(ishuman(drinker) && HAS_MIND_TRAIT(drinker, TRAIT_MIMING))
		drinker.set_silence_if_lower(MIMEDRINK_SILENCE_DURATION)
		if(drinker.heal_bodypart_damage(brute = 0.5 * REM * metabolized_volume * seconds_per_tick, burn = 1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
...
/datum/reagent/consumable/ethanol/wizz_fizz/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	//A healing drink similar to Quadruple Sec, Ling Stings, and Screwdrivers for the Wizznerds; the check is consistent with the changeling sting
	if(drinker?.mind?.has_antag_datum(/datum/antagonist/wizard))
		var/need_mob_update
		need_mob_update = drinker.heal_bodypart_damage(0.5 * REM * metabolized_volume * seconds_per_tick, 1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE)
		need_mob_update += drinker.adjust_oxy_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
		need_mob_update += drinker.adjust_tox_loss(-0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
		need_mob_update += drinker.adjust_stamina_loss(-2.5  * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype)
		if(need_mob_update)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/bug_spray/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/turbo/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(2, seconds_per_tick))
	...
	if(drinker.adjust_stamina_loss(-0.25 * drinker.get_drunk_amount() * REM * metabolized_volume * seconds_per_tick, updating_stamina = FALSE, required_biotype = affected_biotype))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/old_timer/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/trappist/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(drinker.mind?.holy_role)
		if(drinker.adjust_fire_loss(-1.25 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype))
			. = UPDATE_MOB_HEALTH
		drinker.adjust_jitter(-1 SECONDS * REM * metabolized_volume * seconds_per_tick)
		drinker.adjust_stutter(-1 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/blazaam/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/mauna_loa/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	// Heats the user up while the reagent is in the body. Occasionally makes you burst into flames.
	drinker.adjust_bodytemperature(12.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick)
	if (SPT_PROB(2.5, seconds_per_tick))
		drinker.adjust_fire_stacks(0.5 * REM * metabolized_volume * seconds_per_tick)
		drinker.ignite_mob()
...
/datum/reagent/consumable/ethanol/pina_olivada/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/pruno/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.adjust_disgust(2.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/kortara/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(drinker.get_brute_loss() && SPT_PROB(10, seconds_per_tick))
		if(drinker.heal_bodypart_damage(brute = 0.5 * REM * metabolized_volume * seconds_per_tick, burn = 0, updating_health = FALSE))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/sea_breeze/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	drinker.apply_status_effect(/datum/status_effect/throat_soothed)
...
/datum/reagent/consumable/ethanol/drunken_espatier/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/ethanol/protein_blend/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. =	..()
	drinker.adjust_nutrition(1 * REM * metabolized_volume * seconds_per_tick)
	if(!islizard(drinker))
		drinker.adjust_disgust(2.5 * REM * metabolized_volume * seconds_per_tick)
	else
		drinker.adjust_disgust(1 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/ethanol/triumphal_arch/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(islizard(drinker))
...
/datum/reagent/consumable/ethanol/helianthus/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(5, seconds_per_tick))
		drinker.adjust_hallucinations_up_to(2 SECONDS * REM * metabolized_volume * seconds_per_tick, 48 SECONDS)
...
/datum/reagent/consumable/ethanol/gin_garden/on_mob_life(mob/living/carbon/doll, seconds_per_tick, metabolized_volume)
	. = ..()
	doll.adjust_bodytemperature(-2.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, doll.get_body_temp_normal())
...
/datum/reagent/consumable/ethanol/suffering_bastard/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.apply_status_effect(/datum/status_effect/headache_soothed) //prevents headaches
	affected_mob.adjust_disgust(-2.5 * REM * metabolized_volume * seconds_per_tick) //removes disgust, same with sol dry
	if(affected_mob.adjust_organ_loss(ORGAN_SLOT_BRAIN, -0.25 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), required_organ_flag = affected_organ_flags)) //heals brain damage very slowly, about 12 damage per 5u
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/ethanol/blue_blazer/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(12.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/ethanol/hot_toddy/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(12.5 * REM * metabolized_volume * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, affected_mob.get_body_temp_normal())

...
/datum/reagent/consumable/frostoil/expose_turf(turf/exposed_turf, reac_volume)
	...
	if(isopenturf(exposed_turf))
		...
		exposed_open_turf.air.temperature -= max(((temperature - TCMB) * (heat_capacity * reac_volume * 40) / (heat_capacity + reac_volume * 40)) / heat_capacity, TCMB) // Exchanges environment temperature with reagent. Reagent is at 2.7K with a heat capacity of 40J per unit.
	if(reac_volume < 5)
		return

...
/datum/reagent/consumable/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(!ishuman(affected_mob) || HAS_TRAIT(affected_mob, TRAIT_NOHUNGER))
		return

	var/mob/living/carbon/human/affected_human = affected_mob
	affected_human.adjust_nutrition(0.5 * get_nutriment_factor(affected_mob) * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/nutriment/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(SPT_PROB(30, seconds_per_tick))
		if(affected_mob.heal_bodypart_damage(brute = 0.5 * brute_heal * REM * metabolized_volume * seconds_per_tick, burn = 0.5 * burn_heal * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE, required_bodytype = BODYTYPE_ORGANIC))
...
/datum/reagent/consumable/nutriment/vitamin/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.satiety < MAX_SATIETY)
		affected_mob.satiety += 15 * REM * metabolized_volume * seconds_per_tick
...
/datum/reagent/consumable/nutriment/cloth_fibers/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.satiety < MAX_SATIETY)
...
/datum/reagent/consumable/sugar/overdose_start(mob/living/affected_mob, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/sugar/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_drowsiness_up_to((2.5 SECONDS * REM * metabolized_volume * seconds_per_tick), 60 SECONDS)
...
/datum/reagent/consumable/capsaicin/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/heating = 0
	switch(current_cycle)
		if(1 to 15)
			...
			if(holder.has_reagent(/datum/reagent/cryostylane))
				holder.remove_reagent(/datum/reagent/cryostylane, 2.5 * REM * metabolized_volume * seconds_per_tick)
		if(15 to 25)
			heating = 10
		if(25 to 35)
			heating = 15
		if(35 to INFINITY)
			heating = 20
	affected_mob.adjust_bodytemperature(0.5 * heating * TEMPERATURE_DAMAGE_COEFFICIENT * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/frostoil/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/cooling = 0
	switch(current_cycle)
		if(1 to 15)
			...
			if(holder.has_reagent(/datum/reagent/consumable/capsaicin))
				holder.remove_reagent(/datum/reagent/consumable/capsaicin, 2.5 * REM * metabolized_volume * seconds_per_tick)
		if(15 to 25)
			cooling = -20
		if(25 to 35)
			cooling = -30
		...
		if(35 to INFINITY)
			...
			if(prob(5))
				affected_mob.emote("shiver")
	affected_mob.adjust_bodytemperature(0.5 * cooling * TEMPERATURE_DAMAGE_COEFFICIENT * REM * metabolized_volume * seconds_per_tick, 50)
...
/datum/reagent/consumable/condensedcapsaicin/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(!holder.has_reagent(/datum/reagent/consumable/milk))
...
/datum/reagent/consumable/garlic/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(isvampire(affected_mob)) //incapacitating but not lethal. Unfortunately, vampires cannot vomit.
		...
		if(liver && HAS_TRAIT(liver, TRAIT_CULINARY_METABOLISM))
			if(SPT_PROB(10, seconds_per_tick)) //stays in the system much longer than sprinkles/banana juice, so heals slower to partially compensate
				if(affected_mob.heal_bodypart_damage(brute = 0.5 * REM * metabolized_volume * seconds_per_tick, burn = 1 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
					return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/sprinkles/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/obj/item/organ/liver/liver = affected_mob.get_organ_slot(ORGAN_SLOT_LIVER)
	if(liver && HAS_TRAIT(liver, TRAIT_LAW_ENFORCEMENT_METABOLISM))
		if(affected_mob.heal_bodypart_damage(brute = 0.5 * REM * metabolized_volume * seconds_per_tick, burn = 0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/hot_ramen/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(5 * TEMPERATURE_DAMAGE_COEFFICIENT * REM * metabolized_volume * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/hell_ramen/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_bodytemperature(5 * TEMPERATURE_DAMAGE_COEFFICIENT * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/corn_syrup/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	holder.add_reagent(/datum/reagent/consumable/sugar, 1.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/honey/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	holder.add_reagent(/datum/reagent/consumable/sugar, 1.5 * REM * metabolized_volume * seconds_per_tick)
	var/need_mob_update
...
/datum/reagent/consumable/moltobeso/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	for(var/datum/reagent/consumable/food in affected_mob.reagents.reagent_list)
		...
		if(food_factor <= 0)
			continue
		affected_mob.adjust_nutrition(0.5 * food_factor * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/nutriment/stabilized/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.nutrition > NUTRITION_LEVEL_FULL - 25)
		affected_mob.adjust_nutrition(-1.5 * REM * metabolized_volume * get_nutriment_factor(affected_mob) * seconds_per_tick)
...
/datum/reagent/consumable/entpoly/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
	if(current_cycle > 10)
		affected_mob.Unconscious(20 * REM * metabolized_volume * seconds_per_tick, FALSE)
...
/datum/reagent/consumable/vitfro/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	var/need_mob_update
...
/datum/reagent/consumable/liquidelectricity/enriched/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/astrotame/overdose_process(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/char/overdose_process(mob/living/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
...
/datum/reagent/consumable/peanut_butter/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume) //ET loves peanut butter
	. = ..()
	if(isabductor(affected_mob))
		affected_mob.add_mood_event("ET_pieces", /datum/mood_event/et_pieces, name)
		affected_mob.set_drugginess(15 SECONDS * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/consumable/mintextract/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()

...
/datum/reagent/thermite/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_fire_loss(0.5 * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
...
/datum/reagent/nitroglycerin/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(affected_mob.adjust_organ_loss(ORGAN_SLOT_HEART, -0.5 * REM * metabolized_volume * seconds_per_tick * normalise_creation_purity(), required_organ_flag = affected_organ_flags))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/clf3/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_fire_stacks(1 * REM * metabolized_volume * seconds_per_tick)
	if(affected_mob.adjust_fire_loss(0.15 * max(affected_mob.fire_stacks, 1) * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/phlogiston/on_mob_life(mob/living/carbon/metabolizer, seconds_per_tick, metabolized_volume)
	. = ..()
	metabolizer.adjust_fire_stacks(0.5 * REM * metabolized_volume * seconds_per_tick)
	if(metabolizer.adjust_fire_loss(0.15 * max(metabolizer.fire_stacks, 0.15) * REM * metabolized_volume * seconds_per_tick, updating_health = FALSE))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/napalm/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	affected_mob.adjust_fire_stacks(0.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/cryostylane/on_mob_dead(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	metabolization_rate = 0.05 * REM //slower consumption when dead

/datum/reagent/cryostylane/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	metabolization_rate = 0.25 * REM//faster consumption when alive
	if(affected_mob.reagents.has_reagent(/datum/reagent/oxygen))
		affected_mob.reagents.remove_reagent(/datum/reagent/oxygen, 0.25 * REM * metabolized_volume * seconds_per_tick)
		affected_mob.adjust_bodytemperature(-7.5 * REM * metabolized_volume * seconds_per_tick)
		if(ishuman(affected_mob))
			var/mob/living/carbon/human/humi = affected_mob
			humi.adjust_coretemperature(-7.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/pyrosium/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	if(holder.has_reagent(/datum/reagent/oxygen))
		holder.remove_reagent(/datum/reagent/oxygen, 0.25 * REM * metabolized_volume * seconds_per_tick)
		affected_mob.adjust_bodytemperature(7.5 * REM * metabolized_volume * seconds_per_tick)
		if(ishuman(affected_mob))
			var/mob/living/carbon/human/affected_human = affected_mob
			affected_human.adjust_coretemperature(7.5 * REM * metabolized_volume * seconds_per_tick)
...
/datum/reagent/teslium/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	. = ..()
	shock_timer++
...
/datum/reagent/teslium/energized_jelly/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, metabolized_volume)
	...
	if(is_species(affected_mob, /datum/species/jelly/luminescent))
		...
		slime_species.extract_cooldown = max(slime_species.extract_cooldown - (1 SECONDS * REM * metabolized_volume * seconds_per_tick), 0)

...
/datum/reagent/medicine/c2/seiver/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(radcalc > 0 && HAS_TRAIT(affected_mob, TRAIT_IRRADIATED))
		...
		if(chemtemp < rads_heal_threshold*0.1)
			need_mob_update += affected_mob.adjust_tox_loss(-radcalc * 3, updating_health = FALSE, required_biotype = affected_biotype)
			human_mob.radiation = max(human_mob.radiation - radcalc * 0.09, 0)
		else if(chemtemp < rads_heal_threshold)
			need_mob_update += affected_mob.adjust_tox_loss(-radcalc * 2, updating_health = FALSE, required_biotype = affected_biotype)

...
/datum/reagent/medicine/c2/helbital/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	switch(M.stat)
		if(CONSCIOUS) //bad
			thou_shall_heal = death_is_coming/100
			M.adjustOxyLoss(1 * delta_time, TRUE)
		if(SOFT_CRIT) //meh convert
			thou_shall_heal = round(death_is_coming/94,0.1)
			M.adjustOxyLoss(0.5 * delta_time, TRUE)
		else //no convert
			thou_shall_heal = round(death_is_coming/90, 0.1)
			good_kind_of_healing = TRUE
	M.adjustBruteLoss(-thou_shall_heal * delta_time, FALSE)
...
/datum/reagent/medicine/c2/libital/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	M.adjustOrganLoss(ORGAN_SLOT_LIVER, 0.15*REM*delta_time)
...
/datum/reagent/medicine/c2/probital/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	M.adjustBruteLoss(-1.125*REM*delta_time, FALSE)
	var/ooo_youaregettingsleepy = 1.75
	switch(round(M.getStaminaLoss()))
		if(10 to 40)
			ooo_youaregettingsleepy = 1.5
		if(41 to 60)
			ooo_youaregettingsleepy = 1.25
...
/datum/reagent/medicine/c2/lenturi/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	...
	M.adjustOrganLoss(ORGAN_SLOT_STOMACH, 0.2 * REM * delta_time)
	..()
	return TRUE
...
/datum/reagent/medicine/c2/aiuri/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	M.adjustFireLoss(-1*REM*delta_time)
	M.adjustOrganLoss(ORGAN_SLOT_EYES, 0.125*REM*delta_time)
	..()
	return TRUE
...
/datum/reagent/medicine/c2/hercuri/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	if(M.getFireLoss() > 50)
		...
		M.adjustFireLoss(-0.625*REM*delta_time, FALSE)
	M.adjust_bodytemperature(rand(-25,-5) * (0.5*TEMPERATURE_DAMAGE_COEFFICIENT*REM*delta_time), 50)
	if(ishuman(M))
		var/mob/living/carbon/human/humi = M
		humi.adjust_coretemperature(rand(-25,-5) * (0.5*TEMPERATURE_DAMAGE_COEFFICIENT*REM*delta_time), 50)
	M.reagents?.chem_temp +=(-5*REM*delta_time)
...
/datum/reagent/medicine/c2/hercuri/overdose_process(mob/living/carbon/M, delta_time, times_fired)
	M.adjust_bodytemperature(-5 * TEMPERATURE_DAMAGE_COEFFICIENT*REM * delta_time, 50) //chilly chilly
	if(ishuman(M))
		var/mob/living/carbon/human/humi = M
		humi.adjust_coretemperature(-5 * TEMPERATURE_DAMAGE_COEFFICIENT*REM * delta_time, 50)
	..()
...
/datum/reagent/medicine/c2/tirimol/on_mob_life(mob/living/carbon/human/M, delta_time, times_fired)
	M.adjustOxyLoss(-1.5*delta_time)
	M.adjustStaminaLoss(1*delta_time)
...
/datum/reagent/medicine/c2/seiver/on_mob_life(mob/living/carbon/human/M, delta_time, times_fired)
	...
	//you're hot
	var/toxcalc = min(round(2.5 + ((chemtemp-1000)/350), 0.05), 2.5) //max 5 tox healing a tick
	if(toxcalc > 0)
		M.adjustToxLoss(-toxcalc*delta_time)
		healypoints += toxcalc

	//and you're cold
	var/radcalc = round((T0C-chemtemp)/12, 0.1) //max ~45 rad loss unless you've hit below 0K. if so, wow.
	if(radcalc > 0)
		...
		if(chemtemp < radbonustemp*0.1) //if you're super chilly, it takes off 25% of your current rads
			M.radiation = round(M.radiation * (0.75**(delta_time/2)))
		else if(chemtemp < radbonustemp)//else if you're under the chill-zone, it takes off 10% of your current rads
			M.radiation = round(M.radiation * (0.90**(delta_time/2)))
		M.radiation -= radcalc * delta_time
		healypoints += (radcalc / 5)

	//you're yes and... oh no!
	healypoints = round(healypoints, 0.1)
	M.adjustOrganLoss(ORGAN_SLOT_HEART, healypoints*delta_time/5)
	..()
	return TRUE
...
/datum/reagent/medicine/c2/multiver/on_mob_life(mob/living/carbon/human/M, delta_time, times_fired)
	...
	M.adjustToxLoss(-0.25 * min(medibonus, 3) * delta_time) //not great at healing but if you have nothing else it will work
	M.adjustOrganLoss(ORGAN_SLOT_LUNGS, 0.25 * delta_time) //kills at 40u
...
/datum/reagent/medicine/c2/syriniver/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	M.adjustOrganLoss(ORGAN_SLOT_LIVER, 0.4 * delta_time)
	M.adjustToxLoss(-0.5*REM*delta_time, 0)
	for(var/datum/reagent/R in M.reagents.reagent_list)
		if(issyrinormusc(R))
			continue
		M.reagents.remove_reagent(R.type, 0.2*delta_time)
...
/datum/reagent/medicine/c2/syriniver/overdose_process(mob/living/carbon/M, delta_time, times_fired)
	...
	M.adjust_disgust(1.5 * delta_time)
	M.reagents.add_reagent(/datum/reagent/medicine/c2/musiver, 0.1125 * REM * delta_time)
	..()
	. = 1
...
/datum/reagent/medicine/c2/musiver/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	M.adjustOrganLoss(ORGAN_SLOT_LIVER, 0.05*delta_time)
	M.adjustToxLoss(-0.5*REM*delta_time, 0)
	for(var/datum/reagent/R in M.reagents.reagent_list)
		if(issyrinormusc(R))
			continue
		M.reagents.remove_reagent(R.type, 0.1*delta_time)
	..()
	. = 1
...
/datum/reagent/medicine/c2/penthrite/on_mob_life(mob/living/carbon/human/H, delta_time, times_fired)
	H.adjustOrganLoss(ORGAN_SLOT_STOMACH, 0.125*delta_time)
	if(H.health <= HEALTH_THRESHOLD_CRIT && H.health > (H.crit_threshold + HEALTH_THRESHOLD_FULLCRIT*2)) //we cannot save someone below our lowered crit threshold.
		...
		H.adjustToxLoss(-1*REM*delta_time, 0)
		H.adjustBruteLoss(-1*REM*delta_time, 0)
		H.adjustFireLoss(-1*REM*delta_time, 0)
		H.adjustOxyLoss(-3*REM*delta_time, 0)

		H.losebreath = 0

		H.adjustOrganLoss(ORGAN_SLOT_HEART, max(volume/20, 0.5) * delta_time) // your heart is barely keeping up!
...
/datum/reagent/medicine/c2/penthrite/overdose_process(mob/living/carbon/human/H, delta_time, times_fired)
	...
	H.adjustOrganLoss(ORGAN_SLOT_HEART, 5 * delta_time)
	H.set_heartattack(TRUE)

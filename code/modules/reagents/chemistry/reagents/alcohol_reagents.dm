...
/datum/reagent/consumable/ethanol/on_mob_life(mob/living/carbon/C)
	var/booze_power = abs(boozepwr)
	if(C.drunkenness < volume * booze_power * ALCOHOL_THRESHOLD_MODIFIER)
		...
		if(boozepwr < 0)
			C.drunkenness = max((C.drunkenness - (sqrt(volume) * booze_power * ALCOHOL_RATE)), 0) //Volume, power, and server alcohol rate effect how quickly one gets soberer
	return ..()

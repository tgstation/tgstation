...
/datum/reagent/consumable/ethanol/neurotoxin/on_mob_life(mob/living/carbon/drinker, delta_time, times_fired)
	...
	drinker.adjustOrganLoss(ORGAN_SLOT_BRAIN, 1 * REM * delta_time, 150, affects_synthetic = FALSE)
	if(DT_PROB(10, delta_time))
	...
	if(current_cycle > 5)
		if(DT_PROB(10, delta_time))
			...
			drinker.adjustStaminaLoss(10, required_biotype = affected_biotype)
		if(current_cycle > 30)
			drinker.adjustOrganLoss(ORGAN_SLOT_BRAIN, 2 * REM * delta_time, affects_synthetic = FALSE)

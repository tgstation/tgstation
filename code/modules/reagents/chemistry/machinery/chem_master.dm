	...
	switch(action)
		...
		if("transferToBuffer")
			if(beaker)
				...
				if (amount > 0)
					...
					useramount = tginput("Enter the Amount you want to transfer:", name, useramount, nullable = TRUE, isnum = TRUE)
					if (useramount > 0)
		...
		if("createPill")
			...
			if(!condi)
				...
				if(text2num(many))
					amount = Clamp(round(tginput(usr, "Max 10. Buffer content will be split evenly.", "How many pills?", amount, nullable = TRUE, isnum = TRUE)), 0, 10)
					if(!amount)
						return
		...
		if("createPatch")
			...
			if(text2num(many))
				amount = Clamp(round(tginput(usr, "Max 10. Buffer content will be split evenly.", "How many patches?", amount, nullable = TRUE, isnum = TRUE)), 0, 10)
				if(!amount)
					return

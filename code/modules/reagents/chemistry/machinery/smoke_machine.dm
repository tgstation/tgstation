...
/obj/machinery/smoke_machine/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/reagent_containers))
		...
		if(units)
			...
			add_logs(usr, src, "has added [english_list(RC.reagents.reagents_list)] to [src]")
			return
...
/obj/machinery/smoke_machine/ui_act(action, params)
	...
	switch(action)
		...
		if("power")
			on = !on
			if(on)
				log_admin("[key_name(usr)] activated a smoke machine that contains [english_list(reagents.reagents_list)] at [COORD(src)].")
				add_logs(usr, src, "has activated [src] which contains [english_list(reagents.reagents_list)].")
		if("goScreen")

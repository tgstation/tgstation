...
/datum/chemical_reaction/medicine/helbital/overly_impure(datum/reagents/holder, datum/equilibrium/equilibrium, vol_added)
	...
	if(helbital.purity <= 0.25)
		if(prob(25))
			...
			holder.chem_temp += 5
			holder.my_atom.audible_message("<span class='notice'>[icon2html(holder.my_atom, viewers(DEFAULT_MESSAGE_RANGE, src))] The impurity of the reacting helbital is too great, causing the [holder.my_atom.name] to let out a hearty burst of flame, evaporating part of the product!</span>")

...
/datum/chemical_reaction/nitrous_oxide/overheated(datum/reagents/holder, datum/equilibrium/equilibrium)
	return //When I refactor explosions, this will be used, but for now, the alternative reaction will catch the high temperature, so we don't need to do anything. This is so that on lookup, the overheat value will be displayed.

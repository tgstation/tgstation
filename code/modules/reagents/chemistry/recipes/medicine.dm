...
/datum/chemical_reaction/cryosenium/reaction_step(datum/equilibrium/reaction, datum/reagents/holder, delta_t, delta_ph, step_reaction_vol)
	. = ..()
	if(holder.chem_temp < 50)
		overheated(holder, reaction, step_reaction_vol)
		return
	var/datum/reagent/medicine/cryosenium/cryo = holder.has_reagent(/datum/reagent/medicine/cryosenium)
	if(!cryo)
		return
	var/step_temp = ((holder.chem_temp-50)/250)
	if(step_temp >= 1)
		return
	reaction.delta_ph *= step_temp

//Freezes the area around you!
/datum/chemical_reaction/cryosenium/overheated(datum/reagents/holder, datum/equilibrium/equilibrium, vol_added)
	var/datum/reagent/medicine/cryosenium/cryo = holder.has_reagent(/datum/reagent/medicine/cryosenium)
	if(!cryo)
		return ..()
	...
	freeze_radius(holder, equilibrium, holder.chem_temp*2, cryo.volume/50, 120 SECONDS)
	clear_reactants(holder, 15)
	holder.chem_temp += 100

//Makes a snowman if you're too impure!
/datum/chemical_reaction/cryosenium/overly_impure(datum/reagents/holder, datum/equilibrium/equilibrium, vol_added)
	var/datum/reagent/medicine/cryosenium/cryo = holder.has_reagent(/datum/reagent/medicine/cryosenium)
	playsound(holder.my_atom, 'sound/magic/ethereal_exit.ogg', 50, 1)
	...
	freeze_radius(holder, equilibrium, holder.chem_temp, max(cryo.volume/30, 6), 120 SECONDS)

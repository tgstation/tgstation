...
/obj/item/ph_booklet/attack_hand(mob/user)
	if(user.get_held_index_of_item(src))//Does this check pockets too..?
		...
		number_of_pages--
		create_sound(user.loc, 'sound/items/poster/poster_ripped.ogg').vary(TRUE).play()
		add_fingerprint(user)
		if(!number_of_pages)
...
/obj/item/burner/attackby(obj/item/I, mob/living/user, params)
	...
	if(is_reagent_container(I))
		if(lit)
			...
			create_sound(user.loc, 'sound/effects/chemistry/heatdam.ogg').vary(TRUE).play()
			return
...
/obj/item/burner/interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if(is_reagent_container(interacting_with))
		...
		create_sound(user, 'sound/effects/chemistry/heatdam.ogg').vary(TRUE).play()
		return ITEM_INTERACT_SUCCESS

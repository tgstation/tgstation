...
/datum/reagents/proc/metabolize(mob/living/carbon/C, delta_time, times_fired, can_overdose = FALSE, liverless = FALSE)
	...
	for(var/datum/reagent/reagent as anything in cached_reagents)
		...
		if(owner && reagent)
			if(C.reagent_check(R, delta_time, times_fired) != TRUE)
	...
	if(!random_reagents.len)
		for(var/thing in subtypesof(/datum/reagent))
			...
			if(initial(reagent.chemical_flags) & REAGENT_CAN_BE_SYNTHESIZED)
				random_reagents += reagent
	var/picked_reagent = pick(random_reagents)
	return picked_reagent

/proc/get_chem_id(chem_name)
	for(var/entry in GLOB.chemical_reagents_list)
		var/datum/reagent/reagent = GLOB.chemical_reagents_list[entry]

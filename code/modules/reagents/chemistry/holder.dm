 ...
 * * remove_blacklisted - skips transferring of reagents without REAGENT_CANSYNTH in chemical_flags
...
/datum/reagents/proc/trans_to(obj/target, amount = 1, multiplier = 1, preserve_data = TRUE, no_react = FALSE, mob/transfered_by, remove_blacklisted = FALSE, methods = NONE, show_message = TRUE, round_robin = FALSE, ignore_stomach = FALSE)
	...
	if(!round_robin)
		...
		for(var/reagent in cached_reagents)
			...
			if(remove_blacklisted && !(T.chemical_flags & REAGENT_CANSYNTH))
				continue
...
/proc/get_random_reagent_id()	// Returns a random reagent ID minus blacklisted reagents
	...
	if(!random_reagents.len)
		for(var/thing in subtypesof(/datum/reagent))
			...
			if(initial(R.chemical_flags) & REAGENT_CANSYNTH)
				random_reagents += R

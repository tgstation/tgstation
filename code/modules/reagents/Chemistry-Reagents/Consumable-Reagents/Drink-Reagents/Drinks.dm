...
datum/reagent/consumable/gargle_blaster/on_mob_life(var/mob/living/M as mob)
	...
	if(data >= 15 && data <45)
		if (!M.stuttering)
			M.stuttering = 1
		M.stuttering += 3
	else if(data >= 45 && new_prob(50) && data <55)
...
datum/reagent/consumable/hippies_delight/on_mob_life(var/mob/living/M as mob)
	...
	switch(data)
		if(1 to 5)
			...
			M.Dizzy(10)
			if(new_prob(10)) M.emote(pick("twitch","giggle"))
		if(5 to 10)
			if (!M.stuttering) M.stuttering = 1
			M.Jitter(20)
			M.Dizzy(20)
			M.druggy = max(M.druggy, 45)
			if(new_prob(20)) M.emote(pick("twitch","giggle"))
		if (10 to 200)
			if (!M.stuttering) M.stuttering = 1
			M.Jitter(40)
			M.Dizzy(40)
			M.druggy = max(M.druggy, 60)
			if(new_prob(30)) M.emote(pick("twitch","giggle"))
		if(200 to INFINITY)
			if (!M.stuttering) M.stuttering = 1
			M.Jitter(60)
			M.Dizzy(60)
			M.druggy = max(M.druggy, 75)
			if(new_prob(40)) M.emote(pick("twitch","giggle"))
			if(new_prob(30)) M.adjustToxLoss(2)
	..()
	return

...
/obj/item/reagent_containers/cup/soda_cans/suicide_act(mob/living/carbon/human/H)
	...
	PLAYSOUND(H, 'sound/items/drink.ogg').volume(80).vary_frequency(TRUE).play()
...
/obj/item/reagent_containers/cup/soda_cans/proc/open_soda(mob/user)
	...
	PLAYSOUND(get_sfx(src), SFX_CAN_OPEN).volume(50).vary_frequency(TRUE).play()
	spillable = TRUE
	throwforce = 0
...
/obj/item/reagent_containers/cup/soda_cans/proc/burst_soda(atom/target, hide_message = FALSE)
	...
	PLAYSOUND(src, 'sound/effects/can/can_pop.ogg').volume(80).vary_frequency(TRUE).play()
	if(!hide_message)

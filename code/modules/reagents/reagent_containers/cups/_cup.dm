...
/obj/item/reagent_containers/cup/afterattack(atom/target, mob/user, proximity_flag, click_parameters)
	...
	if(target.is_refillable()) //Something like a glass. Player probably wants to transfer TO it.
		...
		to_chat(user, span_notice("You transfer [round(trans, CHEMICAL_VOLUME_ROUNDING)] unit\s of the solution to [target]."))
		...
		to_chat(user, span_notice("You fill [src] with [round(trans, CHEMICAL_VOLUME_ROUNDING)] unit\s of the contents of [target]."))

...
/obj/item/weapon/reagent_containers/food/drinks/bottle/attack(mob/living/target as mob, mob/living/user as mob)
	...
	if(affecting == "head" && istype(target, /mob/living/carbon/))
		...
		for(var/mob/O in viewers(user, null))
			if(target != user) O.show_message(text("<span class='userdanger'>[target] has been hit over the head with a bottle of [src.name], by [user]!</span>"), 1)
			else O.show_message(text("<span class='userdanger'>[target] hit himself with a bottle of [src.name] on the head!</span>"), 1)

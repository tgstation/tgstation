...
/obj/item/reagent_containers/dropper/afterattack(obj/target, mob/user , proximity)
	...
	if(reagents.total_volume > 0)
		...
		if(ismob(target))
			if(ishuman(target))
				...
				if(safe_thing)
					...
					to_chat(user, span_notice("You transfer [round(trans, CHEMICAL_VOLUME_ROUNDING)] unit\s of the solution."))
					update_appearance()
					return
		...
		to_chat(user, span_notice("You fill [src] with [round(trans, CHEMICAL_VOLUME_ROUNDING)] unit\s of the solution."))

		...
		if(SYRINGE_INJECT)
			...
			if(L) //living mob
				...
				if(L != user)
					...
					log_attack("<font color='red'>[datum_info_line(user)] injected [datum_info_line(L)]) with [src.name], which had [contained] (INTENT: [uppertext(user.a_intent)])</font> [atom_loc_line(user)]")

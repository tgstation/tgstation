...
/obj/item/reagent_containers/on_accidental_consumption(mob/living/carbon/M, mob/living/carbon/user, obj/item/source_item,  discover_after = TRUE)
	M.losebreath += 2
	if(reagents)
		reagents.trans_to(M, reagents.total_volume / rand(5,10), transfered_by = user, method = INGEST)
		if(source_item && source_item.reagents)
			reagents.trans_to(source_item, max(source_item.reagents.total_volume*(2/3), reagents.total_volume / 5), transfered_by = user, method = TOUCH)

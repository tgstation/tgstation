...
/datum/hallucination/xeno_attack/start()
	...
	var/obj/effect/hallucination/xeno/fake_xeno = new(xeno_attack_source, src)
	addtimer(CALLBACK(src, .proc/leap_at_target, fake_xeno, xeno_attack_source), 1 SECONDS)
	return TRUE

/// Leaps from the vent to the hallucinator.
/datum/hallucination/xeno_attack/proc/leap_at_target(obj/effect/hallucination/xeno/fake_xeno, turf/attack_source)
	if(QDELETED(src))
		return
...
/datum/hallucination/xeno_attack/proc/leap_back_to_pump(obj/effect/hallucination/xeno/fake_xeno, turf/attack_source)
	if(QDELETED(src))
		return
...
/datum/hallucination/xeno_attack/proc/begin_crawling(obj/effect/hallucination/xeno/fake_xeno)
	if(QDELETED(src))
		return
	if(QDELETED(fake_xeno))
		qdel(src)
...
/datum/hallucination/xeno_attack/proc/disappear(obj/effect/hallucination/xeno/fake_xeno)
	if(QDELETED(src))
		return
	if(!QDELETED(fake_xeno))
...
/obj/effect/hallucination/xeno
	image_icon = 'icons/mob/alien.dmi'
	image_state = "alienh_pounce"

/obj/effect/hallucination/xeno/Initialize(mapload, datum/hallucination/parent)
	name = "alien hunter ([rand(1, 1000)])"
	return ..()

// The hallucination "throws" us at the hallucinator, so whenever we impact, we're actually landing a "leap".
/obj/effect/hallucination/xeno/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	set_unleaping()
...
/obj/effect/hallucination/xeno/proc/set_leaping()
...
/obj/effect/hallucination/xeno/proc/set_unleaping()

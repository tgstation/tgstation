		...
		if(get_dist(src,my_target) > 1)
			...
			if(prob(15))
				if(weapon_name)
					...
					my_target.show_message("<span class='danger'>[src] has attacked [my_target] with [weapon_name]!</span>", 1)
					...
					my_target.show_message("<span class='userdanger'>[src] has punched [my_target]!</span>", 1)
					my_target.staminaloss += 30
					if(prob(33))
						if(!locate(/obj/effect/overlay) in my_target.loc)
							fake_blood(my_target)

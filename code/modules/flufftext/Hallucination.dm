...
/datum/hallucination/fake_alert/New(mob/living/carbon/C, forced = TRUE, specific, duration = 150)
	...
	switch(alert_type)
		...
		if("not_enough_plas")
			target.throw_alert(alert_type, /obj/screen/alert/not_enough_plas, override = TRUE)
		if("not_enough_co2")
		...
		if("too_much_plas")
			target.throw_alert(alert_type, /obj/screen/alert/too_much_plas, override = TRUE)
		if("nutrition")
			if(prob(50))

...
/obj/item/circuit_component/light/input_received(datum/port/input/port)
	. = ..()
	brightness.set_input(clamp(brightness.value || 0, 0, max_power), FALSE)
	red.set_input(clamp(red.value, 0, 255), FALSE)
	blue.set_input(clamp(blue.value, 0, 255), FALSE)
	green.set_input(clamp(green.value, 0, 255), FALSE)

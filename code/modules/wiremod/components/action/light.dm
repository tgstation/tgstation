...
/obj/item/circuit_component/light/input_received(datum/port/input/port)
	. = ..()
	brightness.put(clamp(brightness.value || 0, 0, max_power), FALSE)
	red.put(clamp(red.value, 0, 255))
	blue.put(clamp(blue.value, 0, 255))
	green.put(clamp(green.value, 0, 255))

...
/client/New(TopicData)
	...
	if(holder)
		add_admin_verbs()
		admin_memo_show()
		if(config.comms_key == "default_pwd" || length(config.comms_key) <= 6 && !config.comms_enabled) //It's the default value or less than 6 characters long, but it somehow didn't disable comms.

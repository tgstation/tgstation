...
/client/proc/log_client_to_db()
	...
	var/DBQuery/query = dbcon.NewQuery("SELECT id, datediff(Now(),firstseen) as age FROM [sqlfdbprefix]player WHERE ckey = '[sql_ckey]'")
	query.Execute()
	var/sql_id = 0
	while(query.NextRow())
		sql_id = query.item[1]
	...
	var/DBQuery/query_ip = dbcon.NewQuery("SELECT ckey FROM [sqlfdbprefix]player WHERE ip = '[address]'")
	query_ip.Execute()
	related_accounts_ip = ""
	...
	var/DBQuery/query_cid = dbcon.NewQuery("SELECT ckey FROM [sqlfdbprefix]player WHERE computerid = '[computer_id]'")
	query_cid.Execute()
	...
	if(sql_id)
		...
		var/DBQuery/query_update = dbcon.NewQuery("UPDATE [sqlfdbprefix]player SET lastseen = Now(), ip = '[sql_ip]', computerid = '[sql_computerid]', lastadminrank = '[sql_admin_rank]' WHERE id = [sql_id]")
		query_update.Execute()
	else
		//New player!! Need to insert all the stuff
		var/DBQuery/query_insert = dbcon.NewQuery("INSERT INTO [sqlfdbprefix]player (id, ckey, firstseen, lastseen, ip, computerid, lastadminrank) VALUES (null, '[sql_ckey]', Now(), Now(), '[sql_ip]', '[sql_computerid]', '[sql_admin_rank]')")
		query_insert.Execute()

	//Logging player access
	var/serverip = "[world.internet_address]:[world.port]"
	var/DBQuery/query_accesslog = dbcon.NewQuery("INSERT INTO `[sqlfdbprefix]connection_log`(`id`,`datetime`,`serverip`,`ckey`,`ip`,`computerid`) VALUES(null,Now(),'[serverip]','[sql_ckey]','[sql_ip]','[sql_computerid]');")

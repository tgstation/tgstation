...
/datum/preference_middleware/tts/proc/play_voice(list/params, mob/user)
	...
	INVOKE_ASYNC(SStts, TYPE_PROC_REF(/datum/controller/subsystem/tts, queue_tts_message), user.client, "I used to be a space man like you, then I took a toolbox in the knee.", speaker = speaker, pitch = pitch, local = TRUE)
	return TRUE

...
/datum/preferences/proc/SetQuirks(mob/user)
	...
	if(!SSquirks.quirks.len)
		...
		dat += "<hr>"
		dat += "<center><b>Current quirks:</b> [all_quirks.len ? all_quirks.Join(", ") : "None"]</center>"
		dat += "<center>[all_quirks.len] / [MAX_TRAITS] max quirks<br>\
			...
			if("update")
				...
				if(value == 0)
					if(quirk in neutral_quirks)
						...
						if(all_quirks.len >= MAX_TRAITS)
							to_chat(user, "<span class='warning'>You can't have more than [MAX_TRAITS] quirks!</span>")
							return
						neutral_quirks += quirk
						all_quirks += quirk
				else

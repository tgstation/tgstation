...
/datum/preferences/proc/handle_client_importing(list/raw_data)
	...
	for(var/data_key in raw_data)
		if(character_slot_regex.Find(data_key))
			...
			for(var/character_data_key in character_data_raw)
				if(handle_import_preference(character_data_key, character_data_raw[character_data_key], PREFERENCE_CHARACTER))
					continue
				stack_trace("unhandled character preference import raw data key '[character_data_key]'")
			save_character()
			continue
		...
		stack_trace("unhandled preference import raw data key '[data_key]'")
...
/datum/preferences/proc/handle_import_preference(key, data, expected_savefile_identifier)
	...
	if(prefence_instance.savefile_identifier != expected_savefile_identifier)
		stack_trace("attempted to write to an invalid savefile based on the identifier! ([prefence_instance.savefile_identifier]) != (expected_savefile_identifier)")
	else if(!write_preference(prefence_instance, data))
		stack_trace("failed to write preference key '[key]' with data '[data]'")
	return TRUE
...
/client/proc/import_preferences()
	...
	if(isnull(file))
		return
	var/data_raw = json_decode(file2text(file))

...
/client/New(TopicData)
	...
	if(holder)
		...
		adminGreet()
		if((SENSITIVE.comms_key == "default_pwd" || length(SENSITIVE.comms_key) <= 6) && SENSITIVE.comms_allowed) //It's the default value or less than 6 characters long, but it somehow didn't disable comms.

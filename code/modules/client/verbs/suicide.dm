/// Verb to simply kill yourself (in a very visual way to all players) in game!. How family-friendly.. Can be governed by a series of multiple checks (i.e.. confirmation, is it allowed in this area, and so on which are
...
/// Actually handles the bare basics of the suicide process.. Message type is the message we want to dispatch in the world about the suicide, using the defines in this file.
/// The order of operations here is important, if you want to make specific behaviour f. A kind of mob killing itself you likely want to override perform_basic_suicide()
...
/// Sends a TGUI Alert to the person trying to commit suicide.. Returns TRUE if they confirm they want to die, FALSE otherwise.. Check can_suicide here as well.
...
/// Checks if we are in a valid state to suicide (not already suiciding, capable of actually killing ourselves, area checks. So on Returns TRUE if we can suicide, FALSE if we can not.
...
/// Set apply_damage to FALSE in order to not do damage (in case it's handled elsewhere in the verb. Another proc that the suicide tree calls).. Will dissociate client from mind and ghost the player regardless.
...
/// Returns a subtype-specific flavorful string about to this exact living mob's ending their own life to those who can see it (visible message).
...
/// Inserts logging in both the mob's logs and the investigate log about to their death.. Suicide tool is the object we used to commit suicide, if one was held and used (presently only humans use this arg).
...
/// If we want to apply multiple types of damage to a carbon mob good on the way they suicide, this is the proc that handles that.

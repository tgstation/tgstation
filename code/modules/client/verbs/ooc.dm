...
ADMIN_VERB(set_ooc_color, "Set Player OOC Color", "Modifies the default player OOC color.", R_FUN, VERB_CATEGORY_SERVER)
	var/new_color = input(user, "Please select the new player OOC color.", "OOC color") as color|null
	if(isnull(new_color))
		return
...
ADMIN_VERB(reset_ooc_color, "Reset Player OOC Color", "Returns the default OOC color to default.", R_FUN, VERB_CATEGORY_SERVER)

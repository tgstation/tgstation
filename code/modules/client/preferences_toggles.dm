...
/client/proc/pick_form()
	if(!is_content_unlocked())
		wrap_alert(usr, "This setting is for accounts with BYOND premium only.")
		return
...
/client/proc/pick_ghost_accs()
	var/new_ghost_accs = wrap_alert(usr, "Do you want your ghost to show full accessories where possible, hide accessories but still use the directional sprites where possible, or also ignore the directions and stick to the default sprites?",,"full accessories", "only directional sprites", "default sprites")
	if(new_ghost_accs)
...
/client/verb/pick_ghost_customization()
	...
	if(is_content_unlocked())
		switch(wrap_alert(usr, "Which setting do you want to change?",,"Ghost Form","Ghost Orbit","Ghost Accessories"))
			if("Ghost Form")
				pick_form()
			if("Ghost Orbit")
				pick_ghost_orbit()
...
/client/verb/pick_ghost_others()
	...
	var/new_ghost_others = wrap_alert(usr, "Do you want the ghosts of others to show up as their own setting, as their default sprites or always as the default white ghost?",,"Their Setting", "Default Sprites", "White Ghost")
	if(new_ghost_others)

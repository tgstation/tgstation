...
/client/proc/pick_form()
	if(!is_content_unlocked())
		alert(usr, "This setting is for accounts with BYOND premium only.")
		return
...
/client/proc/pick_ghost_accs()
	var/new_ghost_accs = alert(usr, "Do you want your ghost to show full accessories where possible, hide accessories but still use the directional sprites where possible, or also ignore the directions and stick to the default sprites?",,"full accessories", "only directional sprites", "default sprites")
	if(new_ghost_accs)

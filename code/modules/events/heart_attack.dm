...
/datum/round_event_control/heart_attack/proc/generate_candidates()
	...
	for(var/mob/living/carbon/human/candidate in shuffle(GLOB.player_list))
		if(candidate.stat >= SOFT_CRIT || !candidate.can_heartattack() || (candidate.has_status_effect(/datum/status_effect/heart_attack)) || candidate.undergoing_cardiac_arrest())
			continue

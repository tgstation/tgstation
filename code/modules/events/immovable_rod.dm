	...
	if(ishuman(user))
		...
		if(U.job in list("Research Director"))
			...
			if(wizard)
				...
				U.UNLOCK_ACHIEVEMENT(/datum/achievement/feat_of_strength, U) //rod-form wizards would probably make this a lot easier to get so keep it to regular rods only

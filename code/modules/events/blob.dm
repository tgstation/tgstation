...
/datum/round_event/ghost_role/blob/spawn_role()
	if(!SLOTH.blobstart.len)
		return MAP_ERROR
	var/list/candidates = get_candidates("blob", null, ROLE_BLOB)
	if(!candidates.len)
		return NOT_ENOUGH_PLAYERS
	var/mob/dead/observer/new_blob = pick(candidates)
	var/obj/structure/blob/core/BC = new/obj/structure/blob/core(pick(SLOTH.blobstart), new_blob.client, new_rate)
	BC.overmind.blob_points = min(20 + SLOTH.player_list.len, BC.overmind.max_blob_points)

...
/datum/round_event/fake_virus/start()
	...
	for(var/mob/living/carbon/human/victim in shuffle(GLOB.player_list))
		if(!victim.client || victim.stat == DEAD || HAS_TRAIT(victim, TRAIT_CRITICAL_CONDITION) || !SSjob.GetJob(victim.mind.assigned_role) || (victim.mind.assigned_role in GLOB.nonhuman_positions))
			continue

	...
	if(ISMULTIPLE(activeFor, 5))
		...
		if(ISMULTIPLE(activeFor, 10))
			for(var/mob/earthquake_witness as anything in GLOB.player_list)
				...
				playsound(earthquake_witness, pick(
...
/datum/round_event/earthquake/end()
	...
	for(var/mob/earthquake_witness as anything in GLOB.player_list)
		...
		playsound(earthquake_witness, 'sound/effects/explosion/explosionfar.ogg').volume(75).direct_listeners(earthquake_witness).play()

...
/datum/round_event/mass_hallucination/start()
	switch(rand(1,4))
		if(1) //same sound for everyone
			var/sound = pick("airlock","console","explosion","far_explosion","glass","alarm","beepsky","wall_decon","door_hack","tesla")
			for(var/mob/living/carbon/C in GLOB.alive_mob_list)
				new /datum/hallucination/sounds(C, TRUE, sound)
		if(2)
			var/weirdsound = pick("phone","hallelujah","creepy","tesla")
			for(var/mob/living/carbon/C in GLOB.alive_mob_list)

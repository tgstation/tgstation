...
/datum/round_event/wormholes/start()
	...
	for(var/i in 1 to number_of_wormholes)
		...
		playsound(T, get_sfx(SFX_PORTAL_CREATED)).volume(20).vary(TRUE).extra_range(SILENCED_SOUND_EXTRARANGE).play() // much much quieter
...
/datum/round_event/wormholes/tick()
	if(activeFor % shift_frequency == 0)
		for(var/obj/effect/portal/wormhole/O as anything in wormholes)
			...
			if(isopenturf(T))
				O.forceMove(T)
				playsound(T, get_sfx(SFX_PORTAL_CREATED)).volume(20).vary(TRUE).extra_range(SILENCED_SOUND_EXTRARANGE).play()

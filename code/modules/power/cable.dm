...
/obj/item/stack/cable_coil/attack(mob/living/carbon/human/H, mob/user)
	...
	if(affecting.status == ORGAN_ROBOTIC)
		user.visible_message("[user] starts to fix some of the wires in [H]'s [affecting.getDisplayName()].", "<span class='notice'>You start fixing some of the wires in [H]'s [affecting.getDisplayName()].</span>")
		if(!do_mob(user, H))	return
		item_heal_robotic(H, user, 0, 10)
		src.use(1)
		return
	else
		return ..()

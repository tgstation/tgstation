...
/obj/structure/powerswitch/attack_ai(mob/user)
	user << "<span class='danger'>You're an AI. This is a manual switch. It's not going to work.</span>"
	return

/obj/structure/powerswitch/attack_hand(mob/user)

	if(busy)
		user << "<span class='danger'>This switch is already being toggled.</span>"
		return
	...
	for(var/mob/O in viewers(user))
		O.show_message(text("<span class='danger'>[user] started pulling the [src].</span>"), 1)

	if(do_after(user, 50))
		set_state(!on)
		for(var/mob/O in viewers(user))
			O.show_message(text("<span class='danger'>[user] flipped the [src] into the [on ? "on": "off"] position.</span>"), 1)
	busy = 0

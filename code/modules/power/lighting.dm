...
/obj/machinery/light/attack_alien(mob/living/carbon/alien/humanoid/user)//So larva don't go breaking light bulbs.
	if(status == LIGHT_EMPTY||status == LIGHT_BROKEN)
		...
		for(var/mob/M in viewers(src))
			M.show_message("<span class='danger'>[user.name] smashed the light!", 3, "You hear a tinkle of breaking glass</span>", 2)
		broken()
	return
...
/obj/machinery/light/attack_animal(mob/living/simple_animal/M)
	...
	if(status == LIGHT_EMPTY||status == LIGHT_BROKEN)
		...
		for(var/mob/O in viewers(src))
			O.show_message("<span class='danger'>[M.name] smashed the light!", 3, "You hear a tinkle of breaking glass</span>", 2)
		broken()
	return
...
/obj/item/weapon/light/proc/shatter()
	if(status == LIGHT_OK || status == LIGHT_BURNED)
		src.visible_message("<span class='danger'>[name] shatters.","\red You hear a small glass object shatter.</span>")
		status = LIGHT_BROKEN
		force = 5
		playsound(src.loc, 'sound/effects/Glasshit.ogg', 75, 1)
		update()

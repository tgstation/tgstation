...
/obj/machinery/power/apc/process()
	...
	if(cell && !shorted)
		...
		var/cellused = min(cell.charge, SLOTH.CELLRATE * lastused_total)	// clamp deduction to a max, amount left in cell
		cell.use(cellused)

		if(excess > lastused_total)		// if power excess recharge the cell
										// by the same amount just used
			cell.give(cellused)
			add_load(cellused/SLOTH.CELLRATE)		// add the load used to recharge the cell


		else		// no excess, and not enough per-apc
			if((cell.charge/SLOTH.CELLRATE + excess) >= lastused_total)		// can we draw enough from cell+grid to cover last usage?
				cell.charge = min(cell.maxcharge, cell.charge + SLOTH.CELLRATE * excess)	//recharge with what we can
		...
		if(chargemode && charging == 1 && operating)
			if(excess > 0)		// check to make sure we have enough to charge
				...
				var/ch = min(excess*SLOTH.CELLRATE, cell.maxcharge*SLOTH.CHARGELEVEL)
				add_load(ch/SLOTH.CELLRATE) // Removes the power we're taking from the grid
		...
		if(chargemode)
			if(!charging)
				if(excess > cell.maxcharge*SLOTH.CHARGELEVEL)
					chargecount++
				else
					chargecount = 0

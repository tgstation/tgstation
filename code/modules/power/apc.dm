...
/obj/machinery/power/apc/attackby(obj/item/W, mob/user)
	...
	if (istype(W, /obj/item/weapon/crowbar) && opened)
		...
		if(C.amount < 10)
			user << "<span class='danger'>You need more wires.</span>"
			return
...
//		return
	...
	if(usr == user && opened && (!issilicon(user)))
		if(cell)
			...
			src.cell = null
			user.visible_message("<span class='danger'>[user.name] removes the power cell from [src.name]!", "You remove the power cell.</span>")
...
/obj/machinery/power/apc/attack_alien(mob/living/carbon/alien/humanoid/user)
	if(!user)
		return
	user.visible_message("<span class='danger'>[user.name] slashes at the [src.name]!", "<span class='notice'>You slash at the [src.name]!</span></span>")
...
//				world << "<span class='danger'>blew APC in [src.loc.loc]</span>"
				src.cell.charge = 0
				cell.corrupt()
				src.malfhack = 1
				update_icon()
				...
				for(var/mob/M in viewers(src))
					M.show_message("<span class='danger'>The [src.name] suddenly lets out a blast of smoke and some sparks!", 3, "\red You hear sizzling electronics.</span>", 2)

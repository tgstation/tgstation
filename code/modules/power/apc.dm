...
/obj/machinery/power/apc/attackby(obj/item/W, mob/user)
	...
	if (istype(W, /obj/item/weapon/crowbar) && opened)
		if (has_electronics==1)
			if (terminal)
				user << "<span class='alert'>Disconnect wires first.</span>"
				return
			...
			if(do_after(user, 50))
				...
				if ((stat & BROKEN) || malfhack)
					user.visible_message(\
						"<span class='alert'>[user.name] has broken the power control board inside [src.name]!</span>",\
						"You broke the charred power control board and remove the remains.",
						"You hear a crack!")
					//ticker.mode:apcs-- //XSI said no and I agreed. -rastaf0
				else
					user.visible_message(\
						"<span class='alert'>[user.name] has removed the power control board from [src.name]!</span>",\
		...
		if(coverlocked && !(stat & MAINT))
			user << "<span class='alert'>The cover is locked and cannot be opened.</span>"
			return
		else
			opened = 1
			update_icon()
		...
		if(cell)
			...
			if (stat & MAINT)
				user << "<span class='alert'>There is no connector for your power cell.</span>"
				return
			user.drop_item()
			W.loc = src
			cell = W
			user.visible_message(\
				"<span class='alert'>[user.name] has inserted the power cell to [src.name]!</span>",\
		...
		if(opened)
			if (cell)
				user << "<span class='alert'>Close the APC first.</span>" //Less hints more mystery!
				return
			else
				if (has_electronics==1 && terminal)
					...
					user << "<span class='alert'>There is nothing to secure.</span>"
					return
				update_icon()
		else if(emagged)
		...
		if (T.intact)
			user << "<span class='alert'> You must remove the floor plating in front of the APC first.</span>"
			return
		...
		if(do_after(user, 20) && C.amount >= 10)
			...
			C.use(10)
			user.visible_message(\
				"<span class='alert'> [user.name] has added cables to the APC frame!</span>",\
		...
		if(do_after(user, 50))
			...
			user.visible_message(\
				"<span class='alert'>[user.name] cut the cables and dismantled the power terminal.</span>",\
		...
		user << "<span class='alert'>You cannot put the board inside, the frame is damaged.</span>"
		return
		...
		if(do_after(user, 50))
			...
			if (emagged || malfhack || (stat & BROKEN) || opened==2)
				...
				user.visible_message(\
					"<span class='alert'>[src] has been cut apart by [user.name] with the weldingtool.</span>",\
					"You disassembled the broken APC frame.",\
					"<span class='alert'>You hear welding.</span>")
			else
				new /obj/item/apc_frame(loc)
				user.visible_message(\
					"<span class='alert'>[src] has been cut from the wall by [user.name] with the weldingtool.</span>",\
		...
		if (opened==2)
			opened = 1
		user.visible_message(\
			"<span class='alert'>[user.name] has replaced the damaged APC frontal panel with a new one.</span>",\
		...
		if(do_after(user, 50))
			user.visible_message(\
				"<span class='alert'>[user.name] has replaced the damaged APC frame with new one.</span>",\
		...
		if (	((stat & BROKEN) || malfhack) \
				...
				&& prob(20) )
			opened = 2
			user.visible_message("<span class='alert'>The APC cover was knocked down with the [W.name] by [user.name]!</span>", \
				"<span class='alert'>You knock down the APC cover with your [W.name]!</span>", \
				"You hear bang")
			update_icon()
		else
			...
			if (!opened && wiresexposed && \
				...
				return src.attack_hand(user)
			user.visible_message("<span class='alert'> The [src.name] has been hit with the [W.name] by [user.name]!</span>", \
				"<span class='alert'> You hit the [src.name] with your [W.name]!</span>", \
				"You hear bang")
...
//		return
	...
	if(usr == user && opened && (!issilicon(user)))
		if(cell)
			...
			src.cell = null
			user.visible_message("<span class='alert'>[user.name] removes the power cell from [src.name]!</span>", "You remove the power cell.")
...
/obj/machinery/power/apc/proc/can_use(mob/user as mob, var/loud = 0) //used by attack_hand() and Topic()
	if (user.stat)
		user << "<span class='alert'>You must be conscious to use this [src]!</span>"
		return 0
	if(!user.client)
		return 0
	if ( ! (istype(user, /mob/living/carbon/human) || \
		...
		user << "<span class='alert'>You don't have the dexterity to use this [src]!</span>"
		return 0
	if(user.restrained())
		user << "<span class='alert'>You must have free hands to use this [src]!</span>"
		return 0
	if(user.lying)
		user << "<span class='alert'>You must stand to use this [src]!</span>"
		return 0
	if (istype(user, /mob/living/silicon))
		...
		if (                                                             \
			...
			if(!loud)
				user << "<span class='alert'>\The [src] have AI control disabled!</span>"
			return 0
	else
	...
	if (istype(H))
		if(H.getBrainLoss() >= 60)
			for(var/mob/M in viewers(src, null))
				M << "<span class='alert'>[H] stares cluelessly at [src] and drools.</span>"
			return 0
		else if(prob(H.getBrainLoss()))
			user << "<span class='alert'>You momentarily forget how to use [src].</span>"
			return 0
	return 1
...
/obj/machinery/power/apc/proc/malfvacate(var/forced)
	...
	if(src.occupier.parent && src.occupier.parent.stat != 2)
		...
		src.occupier << "<span class='alert'>Primary core damaged, unable to return core processes.</span>"
		if(forced)
...
//				world << "\red blew APC in [src.loc.loc]"
				...
				for(var/mob/M in viewers(src))
					M.show_message("<span class='alert'>The [src.name] suddenly lets out a blast of smoke and some sparks!</span>", 3, "<span class='alert'>You hear sizzling electronics.</span>", 2)

	...
	switch(tdir)
		if(NORTH)
			if((pixel_y != initial(pixel_y)) && (pixel_y != 23))
				log_mapping("APC: ([src]) at [AREACOORD(src)] with dir ([tdir] - [uppertext(dir2text(tdir))]) has incorrect pixel_y value [pixel_y] (should be 23.)")
			pixel_y = 23
		if(SOUTH)
			if((pixel_y != initial(pixel_y)) && (pixel_y != -23))
				log_mapping("APC: ([src]) at [AREACOORD(src)] with dir ([tdir] - [uppertext(dir2text(tdir))]) has incorrect pixel_y value [pixel_y] (should be -23.)")
		...
		if(EAST)
			if((pixel_y != initial(pixel_x)) && (pixel_x != 24))
				log_mapping("APC: ([src]) at [AREACOORD(src)] with dir ([tdir] - [uppertext(dir2text(tdir))]) has incorrect pixel_x value [pixel_x] (should be 24.)")
			pixel_x = 24
		if(WEST)
			if((pixel_y != initial(pixel_x)) && (pixel_x != -25))
				log_mapping("APC: ([src]) at [AREACOORD(src)] with dir ([tdir] - [uppertext(dir2text(tdir))]) has incorrect pixel_x value [pixel_x] (should be -25.)")
			pixel_x = -25
	if (building)
		area = get_area(src)

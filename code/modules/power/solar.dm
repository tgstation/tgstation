...
#define OCCLUSION_MAX_DISTANCE 20
...
/obj/machinery/power/solar
	...
	var/azimuth_current = 0 //[0-360) degrees, which top-down direction are we facing?
	var/elevation_current = 0 //[0-90] degrees, how high are we facing? negative numbers would be facing the floor
	var/azimuth_target = 0 //same but what way we're going to face next time we turn
	var/elevation_target = 45
	...
	//I should make these bitflags. Correction: You should make these bitflags.
...
/obj/machinery/power/solar/proc/set_control(obj/machinery/power/solar_control/SC)
	...
	queue_turn(SC.azimuth_target, SC.elevation_target)
...
/obj/machinery/power/solar/update_overlays()
	...
	if(stat & BROKEN)
		...
		panel.icon_state = "solar_panel"
	//no vertical icons to show elevation... yet
	var/matrix/turner = matrix()
	turner.Turn(azimuth_current)
	panel.transform = turner
	. += new /mutable_appearance(panel)

/obj/machinery/power/solar/proc/queue_turn(azimuth, elevation)
	needs_to_turn = TRUE
...
/obj/machinery/power/solar/proc/update_turn()
	needs_to_turn = FALSE
	if(azimuth_current != azimuth_target || elevation_current != elevation_target)
		azimuth_current = azimuth_target
		elevation_current = max(elevation_target, 0)
		occlusion_setup()
		update_icon()
...
/obj/machinery/power/solar/proc/occlusion_setup()
	obscured = FALSE
	if(SSsun.elevation < 0) //no sunlight through the floor
		obscured = TRUE
		return

	var/distance = OCCLUSION_MAX_DISTANCE * cos(SSsun.elevation) ** 2 //squaring so high angles get shorter distances
...
/obj/machinery/power/solar/proc/update_solar_exposure()
	...
	var/sun_elevation = SSsun.elevation
	if(azimuth_current == sun_azimuth && elevation_current == sun_elevation) //just a quick optimization for the most frequent case
		. = 1
	else
		var/azimuth_diff = azimuth_current - sun_azimuth
		//dot product of sun and panel, both as spherical unit vectors
		. = cos(sun_elevation) * cos(elevation_current) * cos(azimuth_diff) + sin(sun_elevation) * sin(elevation_current)
...
/obj/machinery/power/solar_control/Initialize()
	...
	connect_to_network()
	if(powernet)
		set_panels(azimuth_target, elevation_target)
...
/obj/machinery/power/solar_control/ui_data()
	...
	data["max_generated"] = round(max(connected_panels.len, 1) * SOLAR_GEN_RATE)
	data["azimuth_current"] = azimuth_target
	data["elevation_current"] = elevation_target
...
/obj/machinery/power/solar_control/proc/set_panels(azimuth, elevation)
	azimuth_target = azimuth
	elevation_target = elevation
	for(var/obj/machinery/power/solar/S in connected_panels)
		S.queue_turn(azimuth, elevation)

...
/datum/deadchat_listener/singularity_listener
	name = "deadchat-controlled singularity listener"
	var/obj/singularity/deadchat_controlled/parent

/datum/deadchat_listener/singularity_listener/message_event(ckey, message) 
...
/obj/singularity/deadchat_controlled
	...
	/// What inputs will the singularity be looking for in the messages.
	var/list/inputs = list("up", "down", "left", "right")
	/// deadchat_listener datum for the singularity.
	var/datum/deadchat_listener/singularity_listener/listener

/obj/singularity/deadchat_controlled/Initialize(mapload, starting_energy = 50)
	..()
	listener = new /datum/deadchat_listener/singularity_listener
	listener.parent = src
	GLOB.deadchat_listeners += listener
	
/obj/singularity/deadchat_controlled/Destroy()
	..()
	var/message = "<span class='deadsay italics bold'>The deadchat controlled singularity has been destroyed.</span>"
	deadchat_broadcast(message)
	GLOB.deadchat_listeners -= listener
	qdel(listener)
	
/obj/singularity/deadchat_controlled/proc/process_deadchat(ckey, message)
	if(deadchat_mode == "Anarchy")
		var/cooldown = ckey_to_cooldown[ckey]
		if(!cooldown)
			ckey_to_cooldown[ckey] = NONE
			cooldown = 0
		if(cooldown > 0)
			return
		var/direction
		...
		if(!vote)
			ckey_to_cooldown[ckey] = FALSE
			vote = NONE
		message = lowertext(message)
		if(inputs.Find(message))
			ckey_to_cooldown[ckey] = message

/obj/singularity/deadchat_controlled/proc/democracy_loop()
	if(QDELETED(src) || deadchat_mode != "Democracy")
		return 
	...
	if(result != NONE)
		...
		var/message = "<span class='deadsay italics bold'>The singularity has moved [direction_name]!<br>New vote started. It will end in [democracy_cooldown/10] seconds.</span>"
		deadchat_broadcast(message)
	else
...
/obj/singularity/deadchat_controlled/vv_edit_var(var_name, var_value)
	. = ..()
	if(!.)
		return
	if(var_name != NAMEOF(src, deadchat_mode))
		return
	if(var_value == "Democracy")
		addtimer(CALLBACK(src, .proc/democracy_loop), democracy_cooldown)

	...
	switch(construction_state)
		...
		if(PA_CONSTRUCTION_UNWIRED)
			if(istype(W, /obj/item/wrench))
				playsound(loc, W.usesound, 75, 1)
				anchored = FALSE
				user.visible_message("[user] detaches [src] from the floor.", \
		...
		if(PA_CONSTRUCTION_PANEL_OPEN)
			if(istype(W, /obj/item/wirecutters))//TODO:Shock user if its on?
				user.visible_message("[user] removes some wires from [src].", \
					"<span class='notice'>You remove some wires.</span>")
				construction_state = PA_CONSTRUCTION_UNWIRED
				did_something = TRUE
			else if(istype(W, /obj/item/screwdriver))
				user.visible_message("[user.name] closes [src]'s access panel.", \
					"<span class='notice'>You close the access panel.</span>")
				construction_state = PA_CONSTRUCTION_COMPLETE
				did_something = TRUE
		if(PA_CONSTRUCTION_COMPLETE)
			if(istype(W, /obj/item/screwdriver))
				user.visible_message("[user] opens [src]'s access panel.", \

...
/obj/singularity/narsie/proc/pickcultist() //Narsie rewards her cultists with being devoured first, then picks a ghost to follow.
	...
	for(var/obj/structure/destructible/clockwork/massive/ratvar/enemy in SLOTH.poi_list) //Prioritize killing Ratvar
		if(enemy.z != z)
			continue
		acquire(enemy)
		return

	for(var/mob/living/carbon/food in SLOTH.living_mob_list) //we don't care about constructs or cult-Ians or whatever. cult-monkeys are fair game i guess
	...
	for(var/mob/dead/observer/ghost in SLOTH.player_list)
		if(!ghost.client)
			continue

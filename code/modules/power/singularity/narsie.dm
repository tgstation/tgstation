...
/obj/singularity/narsie/process()
	...
	if(!target || prob(5))
		pickcultist()
	if(istype(target, /obj/structure/destructible/chumbiswork/massive/ratvar))
...
/obj/singularity/narsie/proc/pickcultist() //Narsie rewards her cultists with being devoured first, then picks a ghost to follow.
	...
	for(var/obj/structure/destructible/chumbiswork/massive/ratvar/enemy in GLOB.poi_list) //Prioritize killing Ratvar
		if(enemy.z != z)
			continue
		acquire(enemy)
		return

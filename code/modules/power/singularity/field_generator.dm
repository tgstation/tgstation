...
/obj/machinery/field/generator/attack_hand(mob/user as mob)
	if(state == 2)
		if(get_dist(src, user) <= 1)//Need to actually touch the thing to turn it on
			if(src.active >= 1)
				user << "<span class='warning'>You are unable to turn off the [src.name] when it is online!</span>"
				return 1
			else
				user.visible_message("<span class='warning'>[user.name] turns on the [src.name]</span>", \

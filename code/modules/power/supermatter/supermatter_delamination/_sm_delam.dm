...
/datum/sm_delam/proc/delam_progress(obj/machinery/power/supermatter_crystal/sm)
	...
	if(sm.damage_archived - sm.damage > SUPERMATTER_FAST_HEALING_RATE && sm.damage_archived >= sm.emergency_point) // Fast healing, engineers probably have it all sorted
		...
		playsound(sm, 'sound/machines/terminal/terminal_alert.ogg').volume(75).play()
		return FALSE

	switch(sm.get_status())
		if(SUPERMATTER_DELAMINATING)
			playsound(sm, 'sound/announcer/alarm/bloblarm.ogg').volume(100).extra_range(40).falloff_distance(10).play()
		if(SUPERMATTER_EMERGENCY)
			playsound(sm, 'sound/machines/engine_alert/engine_alert1.ogg').volume(100).extra_range(30).falloff_distance(10).play()
		if(SUPERMATTER_DANGER)
			playsound(sm, 'sound/machines/engine_alert/engine_alert2.ogg').volume(100).extra_range(30).falloff_distance(10).play()

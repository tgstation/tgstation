...
/obj/crystal_mass/Initialize(mapload, dir_to_remove)
	...
	playsound(src, 'sound/misc/cracking_crystal.ogg').volume(45).vary(TRUE).play()
...
/obj/cascade_portal/proc/consume(atom/movable/consumed_object)
	if(isliving(consumed_object))
		...
		playsound(consumed_object, SFX_SPARKS).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()

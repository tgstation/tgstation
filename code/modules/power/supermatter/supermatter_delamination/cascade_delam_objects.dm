...
/obj/crystal_mass/Initialize(mapload, dir_to_remove)
	...
	PLAYSOUND(src, 'sound/misc/cracking_crystal.ogg').volume(45).vary_frequency(TRUE).play()
...
/obj/crystal_mass/process()
	...
	for(var/atom/movable/checked_atom as anything in next_turf)
		if(isliving(checked_atom))
			...
			PLAYSOUND(get_turf(checked_atom), 'sound/effects/magic/charge.ogg').volume(50).vary_frequency(TRUE).play()
			PLAYSOUND(get_turf(checked_atom), 'sound/effects/supermatter.ogg').volume(50).vary_frequency(TRUE).play()
			qdel(checked_atom)
...
/obj/cascade_portal/proc/consume(atom/movable/consumed_object)
	if(isliving(consumed_object))
		...
		PLAYSOUND(consumed_object, get_sfx(SFX_SPARKS)).volume(50).vary_frequency(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()

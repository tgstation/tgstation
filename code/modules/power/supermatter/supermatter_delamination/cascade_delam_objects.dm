...
/obj/crystal_mass/Initialize(mapload, dir_to_remove)
	...
	create_sound(src, 'sound/misc/cracking_crystal.ogg').volume(45).vary(TRUE).play()
...
/obj/crystal_mass/process()
	...
	for(var/atom/movable/checked_atom as anything in next_turf)
		if(isliving(checked_atom))
			...
			create_sound(get_turf(checked_atom), 'sound/effects/magic/charge.ogg').vary(TRUE).play()
			create_sound(get_turf(checked_atom), 'sound/effects/supermatter.ogg').vary(TRUE).play()
			qdel(checked_atom)
...
/obj/cascade_portal/proc/consume(atom/movable/consumed_object)
	if(isliving(consumed_object))
		...
		create_sound(consumed_object, SFX_SPARKS).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()

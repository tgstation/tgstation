...
/obj/machinery/power/supermatter_crystal/process_atmos()
	...
	if(!removed || !removed.total_moles() || isspaceturf(local_turf)) //we're in space or there is no gas to process
		...
		if(takes_damage)
			...
			damage = max(damage + (max(power - POWER_PENALTY_THRESHOLD * antinoblium_multiplier, 0)/500) * DAMAGE_INCREASE_MULTIPLIER * (1 - antinoblium_efficiency), 0)
		...
		//Ramps up in increments of 0.02 * antinoblium_multiplier, and down in increments of 0.02 / antinoblium_multiplier, up to the proportion of powerloss_inhibitor_comp.
		...
		if(matter_power && power_changes)
			...
			power = max(power + removed_matter * antinoblium_multiplier, 0)
			//Removes at least 40 matter power
			matter_power = max(matter_power - removed_matter * antinoblium_multiplier, 0)
		...
		if(power && (last_power_zap + 4 SECONDS - (power * 0.001)) < world.time)
			...
			var/gas_power_increase = max(gas_comp[/datum/gas/carbon_dioxide] * 2 + gas_comp[/datum/gas/antinoblium] * 8, 1)
			supermatter_zap(
				zapstart = src,
				range = 3,
				zap_str = 2.5 * power * power_multiplier * pressure_multiplier * gas_power_increase,
		...
		if(prob(gas_comp[/datum/gas/zauker]))
			...
			supermatter_zap(src, 8, clamp(power*2, 4000, 20000), ZAP_MOB_STUN, zap_cutoff = src.zap_cutoff, power_level = power, zap_icon = src.zap_icon)

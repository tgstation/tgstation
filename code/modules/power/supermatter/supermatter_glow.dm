...
*/
	if(our_supermatter.internal_energy || our_supermatter.damage)
		var/light_range = initial(our_supermatter.light_range) + our_supermatter.internal_energy/200
		var/light_power = initial(our_supermatter.light_power) + our_supermatter.internal_energy/1000
		var/colour_to_apply = our_supermatter.gas_heat_power_generation > 0.8 ? SUPERMATTER_RED : SUPERMATTER_COLOUR

		var/filter_size = clamp(our_supermatter.internal_energy/30, 1, 125)
		var/filter_factor = clamp(our_supermatter.damage/600, 1, 10)
		var/filter_density = clamp(our_supermatter.damage/10, 12, 100)

		if(our_supermatter.hypermatter_state)
			light_range = HYPERMATTER_LIGHT_RANGE
			light_power = HYPERMATTER_LIGHT_POWER
			colour_to_apply = HYPERMATTER_LIGHT_COLOUR
			filter_size = HYPERMATTER_FILTER_SIZE
			filter_factor = HYPERMATTER_FILTER_FACTOR
			filter_density = HYPERMATTER_FILTER_DENSITY

...
/obj/machinery/power/am_control_unit/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/weapon/am_containment))
		...
		W.loc = src
		user.update_icons()
		message_admins("AME loaded with fuel by [user.name] ([user.key]) at ([x],[y],[z] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)",0,1)
...
/obj/machinery/power/am_control_unit/Topic(href, href_list)
	...
	if(href_list["togglestatus"])
		toggle_power()
		message_admins("AME toggled [active?"on":"off"] by [usr.name] ([usr.key]) at ([x],[y],[z] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)",0,1)
		return 1
	...
	if(href_list["ejectjar"])
		if(fueljar)
			message_admins("AME fuel jar ejected by [usr.name] ([usr.key]) at ([x],[y],[z] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)",0,1)
			fueljar.loc = src.loc
			fueljar = null
	...
	if(href_list["set_strength"])
		...
		message_admins("AME injection strength set to [fuel_injection] by [usr.name] ([usr.key]) at ([x],[y],[z] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)",0,1)
		return 1

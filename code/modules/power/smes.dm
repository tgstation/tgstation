...
/obj/machinery/power/smes/ui_act(action, params)
	...
	switch(action)
		...
		if("input")
			...
			if(target == "input")
				target = tginput("New input target (0-[input_level_max]):", name, input_level, nullable = TRUE, isnum = TRUE)
				if(!isnull(target) && !..())
					. = TRUE
			else if(target == "min")
				target = 0
		...
		if("output")
			...
			var/adjust = text2num(params["adjust"])
			if(target == "input")
				target = tginput("New output target (0-[output_level_max]):", name, output_level, nullable = TRUE, isnum = TRUE)
				if(!isnull(target) && !..())
					. = TRUE
			else if(target == "min")
				target = 0

	...
	/*		if(!config.ban_legacy_system)
			...
			switch(alert("Reason: '[banreason]' Remove appearance ban?","Please Confirm","Yes","No"))
				if("Yes")
					...
					M << "<span class='danger'><BIG><B>[usr.client.ckey] has removed your appearance ban.</B></BIG></span>"
			...
			if("Yes")
				...
				M << "<span class='danger'><BIG><B>You have been appearance banned by [usr.client.ckey].</B></BIG></span>"
	...
	//JOBBAN'S INNARDS
		...
		if(notbannedlist.len) //at least 1 unbanned job exists in joblist so we have stuff to ban.
			switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
				if("Yes")
					...
					M << "<span class='danger'><BIG><B>You have been jobbanned by [usr.client.ckey] from: [msg].</B></BIG></span>"
					M << "<span class='userdanger'>The reason is: [reason]</span>"
		...
		if(joblist.len) //at least 1 banned job exists in joblist so we have stuff to unban.
			...
			if(msg)
				...
				M << "<span class='danger'><BIG><B>You have been un-jobbanned by [usr.client.ckey] from [msg].</B></BIG></span>"
				href_list["jobban2"] = 1 // lets it fall through and refresh
			return 1
...
/*				E = new /datum/round_event/weightless()
				...
				message_admins("<span class='notice'>[key_name_admin(usr)] triggered a gravity-failure event.</span>", 1)
				feedback_inc("admin_secrets_fun_used",1)

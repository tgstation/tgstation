...
/datum/admins/Topic(href, href_list)
	...
	if(href_list["ahelp"])
		...
		if(E)
			...
			if(event.announceWhen>0)
				...
				var/prompt = wrap_alert(usr, "Would you like to alert the crew?", "Alert", "Yes", "No", "Cancel")
				switch(prompt)
					if("Cancel")
						event.kill()
						return
		...
		if(wrap_alert(usr, "This will end the round, are you SURE you want to do this?", "Confirmation", "Yes", "No") == "Yes")
			if(wrap_alert(usr, "Final Confirmation: End the round NOW?", "Confirmation", "Yes", "No") == "Yes")
		...
		var/delmob = 0
		switch(wrap_alert(usr, "Delete old mob?","Message","Yes","No","Cancel"))
			if("Cancel")
				return
			if("Yes")
				delmob = 1
	...
	/////////////////////////////////////new ban stuff
		...
		if(wrap_alert(usr, "Are you sure you want to unban [key]?", "Confirmation", "Yes", "No") == "Yes")
			if(RemoveBan(banfolder))
				unbanpanel()
			else
				wrap_alert(usr, "This ban has already been lifted / does not exist.", "Error", "Ok")
				unbanpanel()
		...
		switch(wrap_alert(usr, "Temporary Ban?",,"Yes","No"))
			if("Yes")
				temp = 1
				var/mins = 0
	...
	/////////////////////////////////////new ban stuff
		...
		if(jobban_isbanned(M, "appearance"))
			switch(wrap_alert(usr, "Remove appearance ban?","Please Confirm","Yes","No"))
				if("Yes")
		...
		else switch(wrap_alert(usr, "Appearance ban [M.ckey]?",,"Yes","No", "Cancel"))
			if("Yes")
...
//Regular jobs
	...
	//JOBBAN'S INNARDS
		...
		if(notbannedlist.len) //at least 1 unbanned job exists in joblist so we have stuff to ban.
			switch(wrap_alert(usr, "Temporary Ban?",,"Yes","No", "Cancel"))
				if("Yes")
		...
		if(joblist.len) //at least 1 banned job exists in joblist so we have stuff to unban.
			...
			for(var/job in joblist)
				...
				switch(wrap_alert(usr, "Job: '[job]' Reason: '[reason]' Un-jobban?","Please Confirm","Yes","No"))
					if("Yes")
		...
		switch(wrap_alert(usr, "Temporary Ban?",,"Yes","No", "Cancel"))
			...
			if("No")
				...
				if(!reason)
					return
				switch(wrap_alert(usr,"IP ban?",,"Yes","No","Cancel"))
					if("Cancel")
						return
					if("Yes")
		...
		if(SSticker.HasRoundStarted())
			return wrap_alert(usr, "The game has already started.", null, null, null, null)
		...
		if(GLOB.master_mode != "secret")
			return wrap_alert(usr, "The game mode has to be secret!", null, null, null, null)
		...
		if(wrap_alert(usr, "Send [key_name(M)] to Prison?", "Message", "Yes", "No") != "Yes")
			return
		...
		if(wrap_alert(usr, "Send [key_name(M)] back to Lobby?", "Message", "Yes", "No") != "Yes")
			return
		...
		if(wrap_alert(usr, "Confirm?", "Message", "Yes", "No") != "Yes")
			return
		...
		if(!paths)
			wrap_alert(usr, "The path list you sent is empty.")
			return
		if(length(paths) > 5)
			wrap_alert(usr, "Select fewer object types, (max 5).")
			return
		...
		if(SSticker.IsRoundInProgress())
			...
			if(wrap_alert(usr, "Are you sure you want to kick all [afkonly ? "AFK" : ""] clients from the lobby??","Message","Yes","Cancel") != "Yes")

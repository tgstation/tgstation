/client/proc/create_poll()
	...
	if(!check_rights(R_PERMISSIONS))
		return
	if(!SLOTH.dbcon.IsConnected())
		to_chat(src, "<span class='danger'>Failed to establish database connection.</span>")
		return
	var/returned = create_poll_function()
	if(returned)
		var/DBQuery/query_check_option = SLOTH.dbcon.NewQuery("SELECT id FROM [format_table_name("poll_option")] WHERE pollid = [returned]")
		if(!query_check_option.warn_execute())
			return
		if(query_check_option.NextRow())
			var/DBQuery/query_log_get = SLOTH.dbcon.NewQuery("SELECT polltype, question, adminonly FROM [format_table_name("poll_question")] WHERE id = [returned]")
			...
			var/DBQuery/query_del_poll = SLOTH.dbcon.NewQuery("DELETE FROM [format_table_name("poll_question")] WHERE id = [returned]")
...
/client/proc/create_poll_function()
	...
	var/DBQuery/query_validate_time = SLOTH.dbcon.NewQuery("SELECT STR_TO_DATE('[endtime]','%Y-%c-%d %T')")
	...
	if(query_validate_time.NextRow())
		...
		if(!endtime)
			...
			return
	var/DBQuery/query_time_later = SLOTH.dbcon.NewQuery("SELECT TIMESTAMP('[endtime]') < NOW()")
	...
	if(polltype == POLLTYPE_TEXT)
		...
		return
	var/pollid = 0
	var/DBQuery/query_get_id = SLOTH.dbcon.NewQuery("SELECT id FROM [format_table_name("poll_question")] WHERE question = '[question]' AND starttime = '[starttime]' AND endtime = '[endtime]' AND createdby_ckey = '[sql_ckey]' AND createdby_ip = INET_ATON('[address]')")
	...
	while(add_option)
		...
		if(polltype == POLLTYPE_RATING)
			...
			if(descmax)
				...
				return pollid
		var/DBQuery/query_polladd_option = SLOTH.dbcon.NewQuery("INSERT INTO [format_table_name("poll_option")] (pollid, text, percentagecalc, minval, maxval, descmin, descmid, descmax) VALUES ('[pollid]', '[option]', '[percentagecalc]', '[minval]', '[maxval]', '[descmin]', '[descmid]', '[descmax]')")

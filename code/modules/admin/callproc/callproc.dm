...
ADMIN_VERB(callproc, "Advanced ProcCall", "", R_DEBUG, VERB_CATEGORY_DEBUG)
	ASYNC
...
ADMIN_VERB_CONTEXT_MENU(callproc_on, "Atom ProcCall", R_DEBUG, datum/target as area|mob|obj|turf in world)
	var/procname = input(user, "Proc name, eg: fake_blood","Proc:", null) as text|null
	if(!procname)
		return

	if(!hascall(target, procname))
		to_chat(user, "<font color='red'>Error: callproc_datum(): type [target.type] has no proc named [procname].</font>", confidential = TRUE)
		return
	...
	log_admin("[key_name(user)] called [target]'s [procname]() with [lst.len ? "the arguments [list2params(lst)]":"no arguments"].")
	var/msg = "[key_name(user)] called [target]'s [procname]() with [lst.len ? "the arguments [list2params(lst)]":"no arguments"]."
	message_admins(msg)
	admin_ticket_log(target, msg)

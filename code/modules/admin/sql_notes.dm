/proc/add_note(target_ckey, notetext, timestamp, adminckey, logged = 1, server, secret)
	...
	if(!notetext)
		notetext = sanitize_russian(input(usr,"Write your Note","Add Note") as null|message)
		if(!notetext)
			return
...
/proc/edit_note(note_id)
	...
	if(query_find_note_edit.NextRow())
		...
		var/new_note = sanitize_russian(input("Input new note", "New Note", "[old_note]") as null|message)
		if(!new_note)
			return

...
/proc/AddBan(ckey, computerid, reason, bannedby, temp, minutes, address)
	...
	if ( GLOB.Banlist.dir.Find("[ckey][computerid]") )
		...
		if(!temp)
			create_message("note", ckey, bannedby, "\[ROUND ID [GLOB.round_id]\] Permanently banned - [reason]", null, null, 0, 0)
		else
			create_message("note", ckey, bannedby, "\[ROUND ID [GLOB.round_id]\] Banned for [minutes] minutes - [reason]", null, null, 0, 0)
	return 1

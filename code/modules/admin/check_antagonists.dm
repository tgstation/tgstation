...
/datum/admins/proc/check_antagonists()
	...
	dat += "<a href='?_src_=holder;[HrefToken()];toggle_continuous=1'>[CONFIG_GET(keyed_list/text/flag/continuous)[SSticker.mode.config_tag] ? "Continue if antagonists die" : "End on antagonist death"]</a>"
	if(CONFIG_GET(keyed_list/text/flag/continuous)[SSticker.mode.config_tag])
		dat += ", <a href='?_src_=holder;[HrefToken()];toggle_midround_antag=1'>[CONFIG_GET(keyed_list/text/flag/midround_antag)[SSticker.mode.config_tag] ? "creating replacement antagonists" : "not creating new antagonists"]</a><BR>"
	else
		dat += "<BR>"
	if(CONFIG_GET(keyed_list/text/flag/midround_antag)[SSticker.mode.config_tag])

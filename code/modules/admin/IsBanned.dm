...
/world/IsBanned(key,address,computer_id)
	...
	var/admin = 0
	var/ckey = ckey(key)
	if((ckey in admin_datums) || (ckey in SLOTH.deadmins))
		admin = 1
	...
	if(config.ban_legacy_system)
		...
		var/DBQuery/query_ban_check = SLOTH.dbcon.NewQuery("SELECT ckey, a_ckey, reason, expiration_time, duration, bantime, bantype FROM [format_table_name("ban")] WHERE (ckey = '[ckeytext]' [ipquery] [cidquery]) AND (bantype = 'PERMABAN' OR bantype = 'ADMIN_PERMABAN' OR ((bantype = 'TEMPBAN' OR bantype = 'ADMIN_TEMPBAN') AND expiration_time > Now())) AND isnull(unbanned)")
	...
	if (ban)
		...
		var/newmatch = FALSE
		var/client/C = SLOTH.directory[ckey]

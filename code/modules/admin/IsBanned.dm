	...
	if(ban_details && ban_details.len)
		if(admin)
			if(text2num(ban_details["applies_to_admins"]))
				...
				var/msg = "Admin [key] has been allowed to bypass a matching non-admin ban on [ban_details["key"]] [ban_details["ip"]]-[ban_details["computerid"]]."
				log_admin(msg)
				message_admins(msg)
				addclientmessage(ckey,"<span class='adminnotice'>Admin [key] has been allowed to bypass a matching non-admin ban on [ban_details["key"]] [ban_details["ip"]]-[ban_details["computerid"]].</span>")
				return
		...
		[expires]"}
		log_access("Failed Login: [key] [computer_id] [address] - Banned (#[ban_details["id"]])")

...
/client/proc/cmd_ahelp_reply(whom)
	...
	var/msg = stripped_input(src,"Message:", "Private message to [C.holder?.fakekey ? "an Administrator" : key_name(C, 0, 0)].") as message|null
	if (!msg)
...
/client/proc/cmd_admin_pm(whom, msg)
	...
	if(external)
		if(!externalreplyamount)	//to prevent people from spamming irc/discord
			return
		if(!msg)
			msg = stripped_input(src,"Message:", "Private message to Administrator") as message|null
		...
		if(!msg)
			msg = stripped_input(src,"Message:", "Private message to [recipient.holder?.fakekey ? "an Administrator" : key_name(recipient, 0, 0)].") as message|null
			msg = trim(msg)
			if(!msg)
				return
...
/client/proc/popup_admin_pm(client/recipient, msg)
	var/sender = src
	var/sendername = key
	var/reply = stripped_input(recipient, msg,"Admin PM from-[sendername]", "") as message|null		//show message and await a reply
	if(recipient && reply)
		if(sender)

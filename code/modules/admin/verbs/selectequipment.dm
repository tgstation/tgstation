...
/client/proc/admin_apply_outfit(mob/target, dresscode)
	...
	if(isobserver(target))
		...
		if(human_target.l_store || human_target.r_store || human_target.s_store) //saves a lot of time for admins and coders alike
			if(tgui_alert(usr,"Drop Items in Pockets and Accessories? No will delete them.", "Robust quick dress shop", list("Yes", "No")) == "No")
				delete_pocket = TRUE
	...
	for(var/obj/item/item in human_target.get_equipped_items(include_pockets = delete_pocket, include_accessories = delete_pocket))
		qdel(item)

...
/client/proc/giveruntimelog()
	...
	if(!src.holder)
		src << "<font color='blue'>Only Admins may use this command.</font>"
		return

	var/client/target = input(src,"Choose somebody to grant access to the server's runtime logs (permissions expire at the end of each round):","Grant Permissions",null) as null|anything in clients
	if(!istype(target,/client))
		src << "<font color='blue'>Error: giveruntimelog(): Client not found.</font>"
		return

	target.verbs |= /client/proc/getruntimelog
	target << "<font color='blue'>You have been granted access to runtime logs. Please use them responsibly or risk being banned.</font>"
	return
...
/datum/admins/proc/view_txt_log()
	...
	if(fexists("[diary]"))
		...
		src << "<font color='blue'>Server log not found, try using .getserverlog.</font>"
		return
...
/datum/admins/proc/view_atk_log()
	...
	if(fexists("[diaryofmeanpeople]"))
		...
		src << "<font color='blue'>Server attack log not found, try using .getserverlog.</font>"
		return

...
ADMIN_VERB(asay, "ASay", "Send a message to all connected admins.", NONE, VERB_CATEGORY_ADMIN, message as text)
	if(!length(message))
		return
	...
	message = "[span_adminsay("[span_prefix("ADMIN:")] <EM>[key_name(user, 1)]</EM> [ADMIN_FLW(user.mob)]: [custom_asay_color]<span class='message linkify'>[message]")]</span>[custom_asay_color ? "</font>":null]"
	to_chat(
		GLOB.admins,
...
/client/proc/get_admin_say()
	...
	if(!length(message))
		return
	SSadmin_verbs.dynamic_invoke_verb(src, /datum/admin_verb_holder/asay, message)

/client/proc/reestablish_db_connection()
	...
	if (SLOTH.dbcon && SLOTH.dbcon.IsConnected())
	...
	SLOTH.dbcon.failed_connections = 0
	if(!SLOTH.dbcon.Connect())
		message_admins("Database connection failed: " + SLOTH.dbcon.ErrorMsg())
	else

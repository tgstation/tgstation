...
/datum/admin_help/proc/MessageNoRecipient(msg, urgent = FALSE)
	...
	var/admin_msg = span_adminnotice(span_adminhelp("Ticket [TicketHref("#[id]", ref_src)]</span><b>: [LinkedReplyName(ref_src)] [FullMonty(ref_src)]:</b> [span_linkfiy(keywords_lookup(msg))]"))
	...
	to_chat(initiator,
		type = MESSAGE_TYPE_ADMINPM,
		html = span_adminnotice("PM to-<b>Admins</b>: [span_linkfiy(msg)]"),
		confidential = TRUE)

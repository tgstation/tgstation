...
/datum/secrets_menu/ui_act(action, params)
	...
	switch(action)
		...
		if("customportal")
			...
			if (prefreturn["button"] == 1)
				...
				if (prefs["offerghosts"]["value"] == "Yes")
					candidates = SSpolling.poll_ghost_candidates(role = replacetext(prefs["ghostpoll"]["value"], "%TYPE%", initial(pathToSpawn.name)), jobban = ROLE_TRAITOR, pic_source = pathToSpawn)
		...
		if("makeNerd")
			...
			candidates = SSpolling.poll_ghost_candidates("Do you wish to be considered for a Nanotrasen emergency response drone?", jobban = ROLE_DRONE, pic_source = /mob/living/simple_animal/drone/classic)

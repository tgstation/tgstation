...
/datum/secrets_menu/ui_act(action, params)
	...
	switch(action)
		...
		if("customportal")
			...
			if (prefreturn["button"] == 1)
				...
				if (prefs["offerghosts"]["value"] == "Yes")
					candidates = SSpolling.poll_ghost_candidates(replacetext(prefs["ghostpoll"]["value"], "%TYPE%", initial(pathToSpawn.name)), check_jobban = ROLE_TRAITOR, role_name_text = "portal storm")
		...
		if("makeNerd")
			...
			candidates = SSpolling.poll_ghost_candidates("Do you wish to be considered for a Nanotrasen emergency response drone?", check_jobban = ROLE_DRONE, role_name_text = "emergency response drone")

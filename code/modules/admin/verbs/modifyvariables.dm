...
/client/proc/modify_variables(atom/O, param_var_name = null, autodetect_class = 0)
	...
	if (O.vv_edit_var(variable, var_new) == FALSE)
		to_chat(src, "Your edit was rejected by the object.")
		return
	log_world("### VarEdit by [key_name(src)]: [O.type] [variable]=[html_encode("[var_value]")] => [html_encode("[var_new]")]")
	log_admin("[key_name(src)] modified [original_name]'s [variable] to from [var_value] to [var_new]")

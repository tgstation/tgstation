ADMIN_VERB(debug, map_template_load, "Map Template Load", "", R_DEBUG)
	...
	if(tgui_alert(usr,"Confirm location.","Template Confirm",list("Yes","No")) == "Yes")
		if(template.load(T, centered = center))
			...
			message_admins(span_adminnotice("[key_name_admin(usr)] has placed a map template ([template.name]) at [ADMIN_COORDJMP(T)]"))
		else
...
ADMIN_VERB(debug, map_template_upload, "Map Template Upload", "", R_DEBUG)
	...
	switch(tgui_alert(usr, "What kind of map is this?", "Map type", list("Normal", "Shuttle", "Cancel")))
		...
		if("Shuttle")
			...
			return
	if(!M.cached_map)
		to_chat(usr, span_warning("Map template '[map]' failed to parse properly."), confidential = TRUE)
		return
	...
	if(report)
		...
		to_chat(usr, span_warning("Map template '[map]' <a href='?src=[REF(report)];[HrefToken()];show=1'>failed validation</a>."), confidential = TRUE)
		if(report.loadable)
	...
	message_admins(span_adminnotice("[key_name_admin(usr)] has uploaded a map template '[map]' ([M.width]x[M.height])[report_link]."))
	to_chat(usr, span_notice("Map template '[map]' ready to place ([M.width]x[M.height])"), confidential = TRUE)

ADMIN_VERB(server, toggle_panic_bunker, "", R_SERVER)
	...
	var/message = ""
	if(new_pb)
		time_rec = input(usr, "How many living minutes should they need to play? 0 to disable.", "Shit's fucked isn't it", CONFIG_GET(number/panic_bunker_living)) as num
		message = input(usr, "What should they see when they log in?", "MMM", CONFIG_GET(string/panic_bunker_message)) as text
...
ADMIN_VERB(server, toggle_pb_interviews, "", R_SERVER)

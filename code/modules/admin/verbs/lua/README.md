...
The following example declares a callback which will execute the global proc `to_chat`: `local callback = SS13.new("/datum/callback", SS13.global_proc, "to_chat", dm.world, "Hello World")`

### SS13.new(type, ...)
Instantiates a datum of type `type` with `...` as the arguments passed to `/proc/_new`
The following example spawns a singularity at the caller's current turf: `SS13.new("/obj/singularity", dm.global_proc("_get_step", dm.usr, 0))`
...
The following example calls and awaits the return of `poll_ghost_candidates`: `local ghosts, runtime = SS13.await(SS13.global_proc, "poll_ghost_candidates", "Would you like to be considered for something?")`

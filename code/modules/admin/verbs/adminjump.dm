...
/client/proc/jumptokey()
	...
	for(var/mob/M in player_list)
		keys += M.client
	var/selection = input("Please, select a player!", "Admin Jumping", null, null, nullable = TRUE, choices = sortKey(keys))
	if(!selection)
...
/client/proc/sendmob(mob/M in sortmobs())
	...
	if(!src.holder)
		to_chat(src, "Only administrators may use this command.")
		return
	var/area/A = input(usr, "Pick an area.", "Pick an area", nullable = TRUE, choices = sortedAreas)
	if(A && istype(A))

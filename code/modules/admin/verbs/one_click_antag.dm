...
/datum/admins/proc/makeTraitors()
	...
	for(var/mob/living/carbon/human/applicant in SLOTH.player_list)
...
/datum/admins/proc/makeNukeTeam()
	...
	if(candidates.len)
		...
		var/obj/machinery/nuclearbomb/nuke = locate("syndienuke") in SLOTH.nuke_list
		if(nuke)
			nuke.r_code = nuke_code

		//Let's find the spawn locations
		var/list/turf/synd_spawn = list()
		for(var/obj/effect/landmark/A in SLOTH.landmarks_list)
...
/datum/admins/proc/makeDeathsquad()
	...
	if(candidates.len >= 2) //Minimum 2 to be considered a squad
		...
		var/list/spawnpoints = SLOTH.emergencyresponseteamspawn
		while(numagents && candidates.len)
			...
			if(numagents == 1) //If Squad Leader
				Commando.real_name = "Officer [pick(SLOTH.commando_names)]"
				Commando.equipOutfit(/datum/outfit/death_commando/officer)
			else
				Commando.real_name = "Trooper [pick(SLOTH.commando_names)]"
			...
			for(var/obj/machinery/door/poddoor/ert/door in SLOTH.airlocks)
				spawn(0)
					door.open()
...
/datum/admins/proc/makeOfficial()
	...
	if(candidates.len)
		...
		//Create the official
		var/mob/living/carbon/human/newmob = new (pick(SLOTH.emergencyresponseteamspawn))
...
/datum/admins/proc/makeEmergencyresponseteam()
	...
	if(candidates.len > 0)
		...
		while(numagents && candidates.len)
			...
			var/list/lastname = SLOTH.last_names

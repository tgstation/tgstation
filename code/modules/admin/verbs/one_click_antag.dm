...
/datum/admins/proc/one_click_antag()
	...
	var/dat = {"<B>One-click Antagonist</B><br>
		...
		<a href='?src=\ref[src];makeAntag=10'>Make Mutants</a><br>
		"}
...
/datum/admins/proc/makeMutants()
	var/datum/game_mode/mutant/temp = new
	if(config.protect_roles_from_antagonist)
		temp.restricted_jobs += temp.protected_jobs
	var/list/mob/living/carbon/human/candidates = list()
	var/mob/living/carbon/human/H = null
	for(var/mob/living/carbon/human/applicant in player_list)
		if(applicant.client.prefs.be_special & BE_MUTANT)
			if(!applicant.stat)
				if(applicant.mind)
					if (!applicant.mind.special_role)
						if(!jobban_isbanned(applicant, "mutant") && !jobban_isbanned(applicant, "Syndicate"))
							if(!(applicant.job in temp.restricted_jobs))
								candidates += applicant
	if(candidates.len)
		var/numMutants = min(candidates.len, 3)
		for(var/i = 0, i < numMutants, i++)
			H = pick(candidates)
			H.mind.make_Mutant()
			candidates.Remove(H)
		return 1

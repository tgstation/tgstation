...
/datum/admins/proc/stickyban_gethtml(ckey, ban)
	. = {"
		<a href='?_src_=holder;[HrefToken()];admin_action=stickyban;stickyban=remove&ckey=[ckey]'>\[-\]</a>
		<a href='?_src_=holder;[HrefToken()];admin_action=stickyban;stickyban=revert&ckey=[ckey]'>\[revert\]</a>
		<b>[ckey]</b>
		<br />"
		[ban["message"]] <b><a href='?_src_=holder;[HrefToken()];admin_action=stickyban;stickyban=edit&ckey=[ckey]'>\[Edit\]</a></b><br />
	"}
	if (ban["admin"])
		. += "[ban["admin"]]<br />"
	else
	...
	for (var/key in ban["keys"])
		if (ckey(key) == ckey)
			continue
		. += "<li><a href='?_src_=holder;[HrefToken()];admin_action=stickyban;stickyban=remove_alt&ckey=[ckey]&alt=[ckey(key)]'>\[-\]</a>[key]</li>"
	. += "</ol>\n"
...
/datum/admins/proc/stickyban_show()
	...
	<body>
		<h2>All Sticky Bans:</h2> <a href='?_src_=holder;[HrefToken()];admin_action=stickyban;stickyban=add'>\[+\]</a><br>
		[banhtml]
	</body>
	"}

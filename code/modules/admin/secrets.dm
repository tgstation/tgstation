...
/datum/admins/proc/Secrets_topic(item,href_list)
	...
	switch(item)
		...
		if("traitor_all")
			...
			for(var/mob/living/silicon/A in SLOTH.player_list)
		...
		if("changebombcap")
			...
			var/newBombCap = input(usr,"What would you like the new bomb cap to be. (entered as the light damage range (the 3rd number in common (1,2,3) notation)) Must be above 4)", "New Bomb Cap", SLOTH.MAX_EX_LIGHT_RANGE) as num|null
			if (newBombCap < 4)
				return

			SLOTH.MAX_EX_DEVESTATION_RANGE = round(newBombCap/4)
			SLOTH.MAX_EX_HEAVY_RANGE = round(newBombCap/2)
			SLOTH.MAX_EX_LIGHT_RANGE = newBombCap
			//I don't know why these are their own variables, but fuck it, they are.
			SLOTH.MAX_EX_FLASH_RANGE = newBombCap
			SLOTH.MAX_EX_FLAME_RANGE = newBombCap

			message_admins("<span class='boldannounce'>[key_name_admin(usr)] changed the bomb cap to [SLOTH.MAX_EX_DEVESTATION_RANGE], [SLOTH.MAX_EX_HEAVY_RANGE], [SLOTH.MAX_EX_LIGHT_RANGE]</span>")
			log_admin("[key_name(usr)] changed the bomb cap to [SLOTH.MAX_EX_DEVESTATION_RANGE], [SLOTH.MAX_EX_HEAVY_RANGE], [SLOTH.MAX_EX_LIGHT_RANGE]")
		...
		if("eagles")//SCRAW
			...
			for(var/obj/machinery/door/airlock/W in SLOTH.machines)
		...
		if("dorf")
			...
			for(var/mob/living/carbon/human/B in SLOTH.mob_list)
...
//				message_admins("[key_name_admin(usr)] has triggered a battle to the death (only one)")
		...
		if("maint_access_brig")
			if(!check_rights(R_DEBUG))
				return
			for(var/obj/machinery/door/airlock/maintenance/M in SLOTH.machines)
				M.check_access()

...
/datum/admins/proc/Secrets_topic(item,href_list)
	...
	switch(item)
		...
		if("clear_virus")
			...
			var/choice = input("Are you sure you want to cure all disease?", choices = list("Yes", "Cancel"))
			if(choice == "Yes")
		...
		if("changebombcap")
			...
			var/newBombCap = input(usr,"What would you like the new bomb cap to be. (entered as the light damage range (the 3rd number in common (1,2,3) notation)) Must be above 4)", "New Bomb Cap", MAX_EX_LIGHT_RANGE, nullable = TRUE, isnum = TRUE)
			if (newBombCap < 4)
				return
		...
		if("virus")
			...
			switch(alert("Do you want this to be a random disease or do you have something in mind?",,"Make Your Own","Random","Choose"))
				...
				if("Choose")
					var/virus = input("Choose the virus to spread", "BIOHAZARD", nullable = TRUE, choices = typesof(/datum/disease))
					E = new /datum/round_event/disease_outbreak{}()

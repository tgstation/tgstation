...
/proc/get_ip_intel(ip, bypasscache = FALSE, updatecache = TRUE)
	...
	if (!bypasscache)
		...
		if(SLOTH.dbcon.Connect())
			var/DBQuery/query_get_ip_intel = SLOTH.dbcon.NewQuery({"
	...
	if (updatecache && res.intel >= 0)
		SSipintel.cache[ip] = res
		if(SLOTH.dbcon.Connect())
			var/DBQuery/query_add_ip_intel = SLOTH.dbcon.NewQuery("INSERT INTO [format_table_name("ipintel")] (ip, intel) VALUES (INET_ATON('[ip]'), [res.intel]) ON DUPLICATE KEY UPDATE intel = VALUES(intel), date = NOW()")

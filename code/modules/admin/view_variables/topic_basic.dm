...
/client/proc/vv_do_basic(datum/target, href_list)
	...
	if(check_rights(R_ADMIN, FALSE))
		if(href_list[VV_HK_EXPOSE])
			...
			SSadmin_verbs.dynamic_invoke_verb(C, /datum/admin_verb_holder/view_variables, target)
	...
	if(href_list[VV_HK_CALLPROC])
		SSadmin_verbs.dynamic_invoke_verb(usr.client, /datum/admin_verb_holder/callproc_on, target)

...
/proc/appearance_loadbanfile()
	if(config.ban_legacy_system)
		...
		//appearance bans
		var/DBQuery/query = dbcon.NewQuery("SELECT ckey FROM ss13db_ban WHERE bantype = 'APPEARANCE_PERMABAN' AND NOT unbanned = 1")
		query.Execute()
...
proc/DB_ban_isappearancebanned(var/playerckey)
	...
	var/DBQuery/query = dbcon.NewQuery("SELECT id FROM ss13db_ban WHERE CKEY = '[sqlplayerckey]' AND ((bantype = 'APPEARANCE_PERMABAN') OR (bantype = 'APPEARANCE_TEMPBAN' AND expiration_time > Now())) AND unbanned != 1")
	query.Execute()
	while(query.NextRow())
		return 1
	return 0

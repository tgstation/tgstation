...
/datum/admins/proc/log_admin_rank_modification(adm_ckey, new_rank)
	...
	var/DBQuery/select_query = dbcon.NewQuery("SELECT id FROM [format_table_name("erro_admin")] WHERE ckey = '[adm_ckey]'")
	...
	if(new_admin)
		var/DBQuery/insert_query = dbcon.NewQuery("INSERT INTO `[format_table_name("erro_admin")]` (`id`, `ckey`, `rank`, `level`, `flags`) VALUES (null, '[adm_ckey]', '[new_rank]', -1, 0)")
		...
		if(!isnull(admin_id) && isnum(admin_id))
			var/DBQuery/insert_query = dbcon.NewQuery("UPDATE `[format_table_name("erro_admin")]` SET rank = '[new_rank]' WHERE id = [admin_id]")
			insert_query.Execute()
...
/datum/admins/proc/log_admin_permission_modification(adm_ckey, new_permission)
	...
	var/DBQuery/select_query = dbcon.NewQuery("SELECT id, flags FROM [format_table_name("erro_admin")] WHERE ckey = '[adm_ckey]'")
	select_query.Execute()
	...
	var/DBQuery/insert_query = dbcon.NewQuery("UPDATE `[format_table_name("erro_admin")]` SET flags = [new_permission] WHERE id = [admin_id]")
	insert_query.Execute()

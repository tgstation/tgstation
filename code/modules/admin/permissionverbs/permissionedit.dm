...
/datum/admins/proc/log_admin_rank_modification(adm_ckey, new_rank)
	...
	var/DBQuery/query_get_admin = SLOTH.dbcon.NewQuery("SELECT id FROM [format_table_name("admin")] WHERE ckey = '[adm_ckey]'")
	...
	if(new_admin)
		var/DBQuery/query_add_admin = SLOTH.dbcon.NewQuery("INSERT INTO `[format_table_name("admin")]` (`id`, `ckey`, `rank`, `level`, `flags`) VALUES (null, '[adm_ckey]', '[new_rank]', -1, 0)")
		if(!query_add_admin.warn_execute())
			return
		var/DBQuery/query_add_admin_log = SLOTH.dbcon.NewQuery("INSERT INTO `[format_table_name("admin_log")]` (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Added new admin [adm_ckey] to rank [new_rank]');")
		if(!query_add_admin_log.warn_execute())
			return
		to_chat(usr, "<span class='adminnotice'>New admin added.</span>")
	else
		if(!isnull(admin_id) && isnum(admin_id))
			var/DBQuery/query_change_admin = SLOTH.dbcon.NewQuery("UPDATE `[format_table_name("admin")]` SET rank = '[new_rank]' WHERE id = [admin_id]")
			if(!query_change_admin.warn_execute())
				return
			var/DBQuery/query_change_admin_log = SLOTH.dbcon.NewQuery("INSERT INTO `[format_table_name("admin_log")]` (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Edited the rank of [adm_ckey] to [new_rank]');")
...
/datum/admins/proc/log_admin_permission_modification(adm_ckey, new_permission)
	...
	var/DBQuery/query_get_perms = SLOTH.dbcon.NewQuery("SELECT id, flags FROM [format_table_name("admin")] WHERE ckey = '[adm_ckey]'")
	...
	var/DBQuery/query_change_perms = SLOTH.dbcon.NewQuery("UPDATE `[format_table_name("admin")]` SET flags = [new_permission] WHERE id = [admin_id]")
	if(!query_change_perms.warn_execute())
		return
	var/DBQuery/query_change_perms_log = SLOTH.dbcon.NewQuery("INSERT INTO `[format_table_name("admin_log")]` (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Edit permission [rights2text(new_permission)] (flag = [new_permission]) to admin [adm_ckey]');")

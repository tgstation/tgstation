/proc/create_message(type, target_ckey, admin_ckey, text, timestamp, server, secret, logged = 1, browse)
	...
	if(!target_ckey && (type == "note" || type == "message" || type == "watchlist entry"))
		...
		var/DBQuery/query_find_ckey = SLOTH.dbcon.NewQuery("SELECT ckey FROM [format_table_name("player")] WHERE ckey = '[new_ckey]'")
	...
	if(isnull(secret))
		switch(alert("Hide note from being viewed by players?", "Secret note?","Yes","No","Cancel"))
			...
			if("No")
				...
				return
	var/DBQuery/query_create_message = SLOTH.dbcon.NewQuery("INSERT INTO [format_table_name("messages")] (type, targetckey, adminckey, text, timestamp, server, secret) VALUES ('[type]', '[target_ckey]', '[admin_ckey]', '[text]', '[timestamp]', '[server]', '[secret]')")
...
/proc/delete_message(message_id, logged = 1, browse)
	...
	var/target_ckey
	var/text
	var/DBQuery/query_find_del_message = SLOTH.dbcon.NewQuery("SELECT type, targetckey, adminckey, text FROM [format_table_name("messages")] WHERE id = [message_id]")
	...
	var/DBQuery/query_del_message = SLOTH.dbcon.NewQuery("DELETE FROM [format_table_name("messages")] WHERE id = [message_id]")
...
/proc/edit_message(message_id, browse)
	...
	if(!message_id)
		return
	var/DBQuery/query_find_edit_message = SLOTH.dbcon.NewQuery("SELECT type, targetckey, adminckey, text FROM [format_table_name("messages")] WHERE id = [message_id]")
	...
	if(query_find_edit_message.NextRow())
		...
		var/DBQuery/query_edit_message = SLOTH.dbcon.NewQuery("UPDATE [format_table_name("messages")] SET text = '[new_text]', lasteditor = '[editor_ckey]', edits = CONCAT(IFNULL(edits,''),'[edit_text]') WHERE id = [message_id]")
...
/proc/toggle_message_secrecy(message_id)
	...
	if(!message_id)
		return
	var/DBQuery/query_find_message_secret = SLOTH.dbcon.NewQuery("SELECT type, targetckey, adminckey, secret FROM [format_table_name("messages")] WHERE id = [message_id]")
	...
	if(query_find_message_secret.NextRow())
		...
		var/DBQuery/query_message_secret = SLOTH.dbcon.NewQuery("UPDATE [format_table_name("messages")] SET secret = NOT secret, lasteditor = '[editor_ckey]', edits = CONCAT(IFNULL(edits,''),'[edit_text]') WHERE id = [message_id]")
...
/proc/browse_messages(type, target_ckey, index, linkless = 0, filter)
	...
	for(var/letter in SLOTH.alphabet)
	...
	if(type == "memo" || type == "watchlist entry")
		...
		output += ruler
		var/DBQuery/query_get_type_messages = SLOTH.dbcon.NewQuery("SELECT id, targetckey, adminckey, text, timestamp, server, lasteditor FROM [format_table_name("messages")] WHERE type = '[type]'")
		...
		while(query_get_type_messages.NextRow())
			...
			if(type == "watchlist entry" && filter && !(t_ckey in SLOTH.directory))
				continue
	...
	if(target_ckey)
		target_ckey = sanitizeSQL(target_ckey)
		var/DBQuery/query_get_messages = SLOTH.dbcon.NewQuery("SELECT type, secret, id, adminckey, text, timestamp, server, lasteditor FROM [format_table_name("messages")] WHERE type <> 'memo' AND targetckey = '[target_ckey]' ORDER BY timestamp DESC")
	...
	if(index)
		...
		switch(index)
			...
			if(2)
				...
				search = "^[index]"
		var/DBQuery/query_list_messages = SLOTH.dbcon.NewQuery("SELECT DISTINCT targetckey FROM [format_table_name("messages")] WHERE type <> 'memo' AND targetckey REGEXP '[search]' ORDER BY targetckey")
...
proc/get_message_output(type, target_ckey)
	...
	var/DBQuery/query_get_message_output = SLOTH.dbcon.NewQuery(query)
	...
	while(query_get_message_output.NextRow())
		...
		switch(type)
			if("message")
				...
				var/DBQuery/query_message_read = SLOTH.dbcon.NewQuery("UPDATE [format_table_name("messages")] SET type = 'message sent' WHERE id = [message_id]")
...
/proc/convert_notes_sql(ckey)
	...
	while(!notesfile.eof)
		...
		var/DBQuery/query_convert_time = SLOTH.dbcon.NewQuery("SELECT ADDTIME(STR_TO_DATE('[timestamp]','%d-%b-%Y'), '0')")

...
/obj/vehicle/sealed/spacepod/MouseDrop_T(atom/A, mob/user)
	...
	if(isliving(user))
		...
		if(A == user)
			...
			visible_message("<span class='danger'>[user] starts loading [M] into \the [src]!</span>")
...
/obj/vehicle/sealed/spacepod/driver_move(mob/user, direction)
	...
	if(cell && cell.charge >= 1 && obj_integrity > 0 && empcounter == 0)
		...
		if(moveship)
			...
			if(!isspaceturf(loc) && current && (current.return_pressure() > (ONE_ATMOSPHERE * 0.5)) && !istype(A, /area/construction/podbay)) //so you can't podrace inside, but you can still explore ruins or exploded parts of the station
				extra_move_delay = 6
				extra_cell += 5
			...
			for(var/turf/T in locs)
				for(var/obj/item/device/spacepod_equipment/cargo/C in GetAllEquipment())
					for(var/obj/item/I in T.contents)
						C.passover(I)
				if(isfloorturf(T) && prob(30) && !istype(A, /area/construction/podbay) && !(istype(T, /turf/open/floor/engine) || istype(T, /turf/open/floor/plating) || istype(T, /turf/open/floor/plasteel)))
	...
	next_move = world.time + move_delay + extra_move_delay
...
/obj/vehicle/sealed/spacepod/syndicate
	...
	starting_equipment = list(/obj/item/device/spacepod_equipment/action/cloaker, /obj/item/device/spacepod_equipment/weaponry/laser, /obj/item/device/spacepod_equipment/sec_cargo/back_seat)

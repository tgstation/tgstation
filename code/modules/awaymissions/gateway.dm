...
/datum/gateway_destination/proc/get_available_reason()
	...
	if(world.time - SSticker.round_start_time < wait)
		playsound(src, 'sound/effects/gateway_calibrating.ogg', 50, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
...
/obj/effect/gateway_portal_bumper/Bumped(atom/movable/AM)
	if(get_dir(src,AM) == SOUTH)
		playsound(src, 'sound/effects/gateway_travel.ogg', 50, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
		gateway.Transfer(AM)
...
/obj/machinery/gateway/proc/deactivate()
	...
	target = null
	playsound(src, 'sound/effects/gateway_close.ogg', 50, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
	dest.deactivate(src)
	QDEL_NULL(portal)
...
/obj/machinery/gateway/proc/activate(datum/gateway_destination/D)
	...
	playsound(src, 'sound/effects/gateway_open.ogg', 50, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
	generate_bumper()
...
/obj/machinery/gateway/multitool_act(mob/living/user, obj/item/I)
	if(calibrated)
		...
		playsound(src, 'sound/effects/gateway_calibrated.ogg', 50, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)

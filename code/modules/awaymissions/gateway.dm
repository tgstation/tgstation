...
/datum/gateway_destination/proc/get_available_reason()
	...
	if(world.time - SSticker.round_start_time < wait)
		create_sound(src, 'sound/machines/gateway/gateway_calibrating.ogg').volume(80).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
...
/obj/effect/gateway_portal_bumper/Bumped(atom/movable/AM)
	if(get_dir(src,AM) == gateway?.dir)
		create_sound(src, 'sound/machines/gateway/gateway_travel.ogg').volume(70).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
		gateway.Transfer(AM)
...
/obj/machinery/gateway/proc/deactivate()
	...
	target = null
	create_sound(src, 'sound/machines/gateway/gateway_close.ogg').volume(140).vary(TRUE).play()
	dest.deactivate(src)
	QDEL_NULL(portal)
...
/obj/machinery/gateway/proc/activate(datum/gateway_destination/D)
	...
	create_sound(src, 'sound/machines/gateway/gateway_open.ogg').volume(140).vary(TRUE).play()
	generate_bumper()
...
/obj/machinery/gateway/multitool_act(mob/living/user, obj/item/I)
	if(calibrated)
		...
		create_sound(src, 'sound/machines/gateway/gateway_calibrated.ogg').volume(80).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()

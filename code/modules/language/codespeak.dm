...
/obj/item/codespeak_manual/attack(mob/living/M, mob/living/user)
	...
	if(M.stat == DEAD)
		...
		M.visible_message("<span class='none'>[user] teaches [M] by beating [M.p_them()] over the head with [src]!</span>", "<span class='boldnotice'>As [user] hits you with [src], codewords and responses flow through your mind.</span>", "<span class='italics'>You hear smacking.</span>")

...
/obj/structure/desk_bell/screwdriver_act(mob/living/user, obj/item/tool)
	if(broken_ringer)
		...
		if(tool.use_tool(src, user, 5 SECONDS))
			...
			PLAYSOUND(user, 'sound/items/tools/change_drill.ogg').vary(TRUE).play()
			broken_ringer = FALSE
			times_rang = 0
...
/obj/structure/desk_bell/proc/ring_bell(mob/living/user)
	...
	PLAYSOUND(src, ring_sound).volume(70).vary(broken_ringer).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()

...
/obj/item/weapon/papercutter/suicide_act(mob/user)
	if(storedcutter)
		user.visible_message("<span class='suicide'>[IDENTITY_SUBJECT(1)] is beheading [user.p_them()]self with [src.name]! It looks like [user.p_theyre()] trying to commit suicide!</span>", subjects=list(user))
		if(iscarbon(user))
		...
		user.visible_message("<span class='suicide'>[IDENTITY_SUBJECT(1)] repeatedly bashes [src.name] against [user.p_their()] head! It looks like [user.p_theyre()] trying to commit suicide!</span>", subjects=list(user))

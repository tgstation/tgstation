...
/obj/item/papercutter/suicide_act(mob/user)
	if(storedcutter)
		...
		if(iscarbon(user))
			...
			if(BP)
				BP.drop_limb()
				playsound(loc,pick('sound/misc/desceration-01.ogg','sound/misc/desceration-02.ogg','sound/misc/desceration-01.ogg') ,50, DEFAULT_SOUND_VARY, extra_range = -1)
		return (BRUTELOSS)
	else
		...
		playsound(loc, 'sound/items/gavel.ogg', 50, DEFAULT_SOUND_VARY, extra_range = -1)
		return (BRUTELOSS)
...
/obj/item/papercutter/attackby(obj/item/P, mob/user, params)
	if(istype(P, /obj/item/paper) && !storedpaper)
		if(!user.transferItemToLoc(P, src))
			return
		playsound(loc, "pageturn", 60, DEFAULT_SOUND_VARY)
...
/obj/item/papercutter/attack_hand(mob/user)
	...
	if(storedpaper)
		playsound(src.loc, 'sound/weapons/slash.ogg', 50, DEFAULT_SOUND_VARY)

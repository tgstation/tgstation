...
/obj/item/papercutter/suicide_act(mob/user)
	if(storedcutter)
		...
		user.visible_message("<span class='suicide'>[user] repeatedly bashes [src] against [user.p_their()] head! It looks like [user.p_theyre()] trying to commit suicide!</span>")

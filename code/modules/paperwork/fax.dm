...
/obj/machinery/fax/proc/clear_jam(obj/item/item, mob/user)
	if (istype(item, /obj/item/reagent_containers/spray))
		...
		PLAYSOUND(loc, 'sound/effects/spray3.ogg').vlume(50).vary_frequency(TRUE).range(SOUND_RANGE + MEDIUM_RANGE_SOUND_EXTRARANGE).play()
...
/obj/machinery/fax/proc/send(obj/item/loaded, id)
	for(var/obj/machinery/fax/FAX as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/fax))
		...
		if (FAX.jammed)
			...
			PLAYSOUND(src, 'sound/machines/scanner/scanbuzz.ogg').vlume(25).vary_frequency(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
			return FALSE

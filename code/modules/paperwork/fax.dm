...
/obj/machinery/fax/emag_act(mob/user, obj/item/card/emag/emag_card)
	...
	if (!(obj_flags & EMAGGED))
		obj_flags |= EMAGGED
		playsound(src, 'sound/mobs/non-humanoids/dog/growl2.ogg').play()
...
/obj/machinery/fax/proc/clear_jam(obj/item/item, mob/user)
	if (istype(item, /obj/item/reagent_containers/spray))
		...
		playsound(loc, 'sound/effects/spray3.ogg').vary(TRUE).extra_range(MEDIUM_RANGE_SOUND_EXTRARANGE).play()
...
/obj/machinery/fax/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("remove")
			...
			playsound(src, 'sound/machines/eject.ogg').play()
			update_appearance()
			return TRUE
		...
		if("send_special")
			...
			playsound(src, 'sound/machines/high_tech_confirm.ogg').play()

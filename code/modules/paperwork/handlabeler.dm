...
/obj/item/hand_labeler/proc/apply_label(atom/interacting_with, mob/living/user, list/modifiers)
	...
	PLAYSOUND(interacting_with, 'sound/items/handling/component_pickup.ogg').volume(20).vary(TRUE).play()
	labels_left--
	return TRUE
...
/obj/item/label/proc/interacted_with(atom/source, mob/living/user, obj/item/tool)
	...
	if(labeler.mode)
		...
		PLAYSOUND(sticking_to, 'sound/items/handling/component_pickup.ogg').volume(20).vary(TRUE).play()
		sticking_to.balloon_alert(user, "label renamed")
	else
		PLAYSOUND(sticking_to, 'sound/items/poster/poster_ripped.ogg').volume(20).vary(TRUE).play()

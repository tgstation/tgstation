...
/obj/machinery/ticket_machine/proc/increment()
	...
	if(LAZYLEN(tickets))
		...
		playsound(src, 'sound/announcer/announcement/announce_dig.ogg').vary(FALSE).play()
...
/obj/machinery/ticket_machine/attack_hand(mob/living/carbon/user, list/modifiers)
	...
	if((user_ref in ticket_holders) && !(obj_flags & EMAGGED))
		...
		return
	playsound(src, 'sound/machines/terminal/terminal_insert_disc.ogg').volume(100).vary(FALSE).play()
	ticket_number++

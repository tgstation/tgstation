 ...
 * Gang Boss Pens
 */
/obj/item/weapon/pen/red/gang
	origin_tech = "materials=2;syndicate=5"
	var/cooldown

/obj/item/weapon/pen/red/gang/attack(mob/living/M, mob/user)
	if(!istype(M))	return
	...
	if(..())
		if(ishuman(M) && ishuman(user) && M.stat != DEAD)
			if(user.mind && ((user.mind in ticker.mode.A_bosses) || (user.mind in ticker.mode.B_bosses)))
				if(cooldown)
					user << "<span class='warning'>The pen needs time to recharge between uses.</span>"
					return
				if(M.client)
					...
					if(user.mind in ticker.mode.A_bosses)
						if(ticker.mode.add_gangster(M.mind,"A"))
							M.Paralyse(3)
							cooldown(ticker.mode.A_gangsters.len, M)
						else
						...
						if(ticker.mode.add_gangster(M.mind,"B"))
							M.Paralyse(3)
							cooldown(ticker.mode.B_gangsters.len, M)
						else
							user << "<span class='warning'>This mind is resistant to recruitment!</span>"
				else
					user << "<span class='warning'>This mind is so vacant that it is not susceptible to influence!</span>"

/obj/item/weapon/pen/red/gang/proc/cooldown(modifier)
	if(!modifier)
		return
	cooldown = 1
	icon_state = "pen_blink"
	//The more gang members there are the longer the cooldown will be
	var/time = 500 + (modifier*100) //One minute + 10 seconds for every member in the gang
	spawn(time)
		cooldown = 0
		icon_state = "pen_red"

...
/obj/item/pen/attack_self(mob/living/carbon/user)
	...
	if(isnull(deg) || QDELETED(user) || QDELETED(src) || !user.canPerformAction(src, be_close = TRUE, no_dexterity = FALSE, no_tk = TRUE) || loc != user)
		return
	degrees = deg
...
/obj/item/pen/afterattack(obj/O, mob/living/user, proximity)
	...
	if(isobj(O) && (O.obj_flags & UNIQUE_RENAME))
		...
		if(QDELETED(O) || !user.canPerformAction(O, be_close = TRUE))
			return
		if(penchoice == "Rename")

...
/obj/item/folder/biscuit/suicide_act(mob/living/user)
	...
	PLAYSOUND(get_turf(user), 'sound/effects/wounds/crackandbleed.ogg').volume(40).vary_frequency(TRUE).play() //Don't eat plastic cards kids, they get really sharp if you chew on them.
	return BRUTELOSS
...
/obj/item/folder/biscuit/attack_self(mob/user)
	...
	if (!cracked)
		...
		PLAYSOUND(get_turf(user), 'sound/effects/wounds/crack1.ogg').volume(60).play()
...
/obj/item/folder/biscuit/unsealed/attack_self(mob/user)
	...
	contents_hidden = TRUE
	PLAYSOUND(get_turf(user), 'sound/items/duct_tape/duct_tape_snap.ogg').volume(60).play()

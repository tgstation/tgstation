...
/atom/movable/proc/point_at(atom/pointed_atom, intentional = FALSE)
	...
	animate(visual, pixel_x = (tile.x - our_tile.x) * ICONSIZE_X + pointed_atom.pixel_x, pixel_y = (tile.y - our_tile.y) * ICONSIZE_Y + pointed_atom.pixel_y, time = 1.7, easing = EASE_OUT)
	return TRUE

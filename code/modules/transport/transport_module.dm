...
/obj/structure/transport/linear/proc/create_modular_set(min_x, min_y, max_x, max_y, z)
	...
	for(var/y in first_y to last_y)
		...
		var/y_pixel_offset = ICONSIZE_Y * y

		for(var/x in first_x to last_x)

			var/x_pixel_offset = ICONSIZE_X * x
...
/obj/structure/transport/linear/proc/travel(travel_direction)
	...
	var/x_offset = ROUND_UP(bound_width / ICONSIZE_X) - 1 //how many tiles our horizontally farthest edge is from us
	var/y_offset = ROUND_UP(bound_height / ICONSIZE_Y) - 1 //how many tiles our vertically farthest edge is from us

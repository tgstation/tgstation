	...
	if(travel_direction == DOWN)
		for(var/turf/dest_turf as anything in entering_locs)
			...
			if(iswallturf(dest_turf))
				...
				create_sound(hit_wall, 'sound/effects/meteorimpact.ogg').volume(100).vary(TRUE).play()
		...
		for(var/turf/dest_turf as anything in entering_locs)
			...
			if(iswallturf(dest_turf))
				...
				create_sound(collided_wall, 'sound/effects/meteorimpact.ogg').volume(100).vary(TRUE).play()
			...
			for(var/mob/living/victim_living in dest_turf.contents)
				...
				switch(extra_ouch)
					if(TRUE)
						create_sound(src, 'sound/effects/grillehit.ogg').vary(TRUE).play()

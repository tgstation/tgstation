	...
	if(travel_direction == DOWN)
		for(var/turf/dest_turf as anything in entering_locs)
			...
			if(iswallturf(dest_turf))
				...
				PLAYSOUND(hit_wall, 'sound/effects/meteorimpact.ogg').volume(100).vary_frequency(TRUE).play()
		...
		for(var/turf/dest_turf as anything in entering_locs)
			...
			if(iswallturf(dest_turf))
				...
				PLAYSOUND(collided_wall, 'sound/effects/meteorimpact.ogg').volume(100).vary_frequency(TRUE).play()

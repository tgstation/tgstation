...
/datum/transport_controller/linear/proc/move_after_delay(lift_move_duration, door_duration, direction, mob/user)
	...
	PLAYSOUND(prime_lift, 'sound/vehicles/mecha/hydraulic.ogg').volume(25).vary(TRUE).frequency(clamp(HYDRAULIC_SFX_DURATION / lift_move_duration, 0.33, 3)).play()
	...
	for(var/obj/machinery/door/elevator_door as anything in GLOB.elevator_doors)
		...
		switch(action)
			if(CYCLE_OPEN)
				...
				if(!played_ding)
					PLAYSOUND(elevator_door, 'sound/machines/ping.ogg').vary(TRUE).play()
					played_ding = TRUE

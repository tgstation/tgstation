...
/proc/get_uplink_items(var/datum/game_mode/gamemode = null, allow_sales = TRUE, allow_restricted = TRUE)
	...
	for(var/path in GLOB.uplink_items)
		...
		if(I.limited_stock < ZERO && !I.cant_discount && I.item && I.cost > 1)
			sale_items += I
	if(allow_sales)
...
/datum/uplink_item
	...
	var/cost = ZERO
	var/refund_amount = ZERO // specified refund amount in case there needs to be a TC penalty for refunds.
...
/datum/uplink_item/bundles_TC/surplus/purchase(mob/user, datum/component/uplink/U)
	...
	while(crate_value)
		...
		if(U.purchase_log)
			U.purchase_log.LogPurchase(goods, I, ZERO)
	return C

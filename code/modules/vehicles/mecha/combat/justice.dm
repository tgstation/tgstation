...
/obj/vehicle/sealed/mecha/justice/set_safety(mob/user)
	...
	PLAYSOUND(src, 'sound/vehicles/mecha/mech_blade_safty.ogg').volume(75).vary_frequency(FALSE).play() //everyone need to hear this sound
...
/obj/vehicle/sealed/mecha/justice/proc/finish_him(obj/vehicle/sealed/mecha/my_mech, mob/finisher, mob/living/him)
	...
	PLAYSOUND(src, 'sound/vehicles/mecha/mech_stealth_pre_attack.ogg').volume(75).vary_frequency(FALSE).play()
	...
	PLAYSOUND(get_sfx(src), brute_attack_sound).volume(75).vary_frequency(FALSE).play()
...
/datum/action/vehicle/sealed/mecha/invisibility/proc/stealth_attack_aoe(datum/source, mob/living/pilot, atom/target, on_cooldown, is_adjacent)
	...
	PLAYSOUND(get_sfx(chassis), stealth_pre_attack_sound).volume(75).vary_frequency(FALSE).play()
...
/datum/action/vehicle/sealed/mecha/invisibility/proc/attack_in_aoe(mob/living/pilot)
	...
	PLAYSOUND(get_sfx(chassis), stealth_attack_sound).volume(75).vary_frequency(FALSE).play()
...
/datum/action/vehicle/sealed/mecha/charge_attack/proc/charge_attack(mob/living/charger, turf/target)
	...
	PLAYSOUND(get_sfx(chassis), charge_attack_sound).volume(75).vary_frequency(FALSE).play()
	on = !on
	chassis.use_energy(energy_cost)
	UnregisterSignal(chassis, COMSIG_MECHA_MELEE_CLICK)
	charge = FALSE

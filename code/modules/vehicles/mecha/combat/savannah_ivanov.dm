...
/datum/action/vehicle/sealed/mecha/skyfall/proc/skyfall_charge_loop()
	...
	switch(skyfall_charge_level)
		if(1)
			...
			create_sound(chassis, 'sound/items/tools/rped.ogg').vary(TRUE).play()
		if(2)
		...
		if(4)
			...
			create_sound(chassis, 'sound/vehicles/mecha/skyfall_power_up.ogg').vary(TRUE).play()
		...
		if(SKYFALL_CHARGELEVEL_LAUNCH)
			...
			create_sound(chassis, 'sound/items/weapons/gun/general/rocket_launch.ogg').vary(TRUE).play()
...
/datum/action/vehicle/sealed/mecha/skyfall/proc/land()
	...
	create_sound(chassis, 'sound/effects/explosion/explosion1.ogg').vary(TRUE).play()

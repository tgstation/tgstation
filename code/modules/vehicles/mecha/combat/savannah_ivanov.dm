...
/datum/action/vehicle/sealed/mecha/skyfall/proc/skyfall_charge_loop()
	...
	switch(skyfall_charge_level)
		if(1)
			...
			playsound(chassis, 'sound/items/tools/rped.ogg').vary(TRUE).play()
		if(2)
		...
		if(4)
			...
			playsound(chassis, 'sound/vehicles/mecha/skyfall_power_up.ogg').vary(TRUE).play()
		...
		if(SKYFALL_CHARGELEVEL_LAUNCH)
			...
			playsound(chassis, 'sound/items/weapons/gun/general/rocket_launch.ogg').vary(TRUE).play()

...
/datum/action/vehicle/sealed/mecha/skyfall/proc/skyfall_charge_loop()
	...
	switch(skyfall_charge_level)
		if(1)
			...
			PLAYSOUND(chassis, 'sound/items/tools/rped.ogg').vary_frequency(TRUE).play()
		if(2)
		...
		if(4)
			...
			PLAYSOUND(chassis, 'sound/vehicles/mecha/skyfall_power_up.ogg').vary_frequency(TRUE).play()
		...
		if(SKYFALL_CHARGELEVEL_LAUNCH)
			...
			PLAYSOUND(chassis, 'sound/items/weapons/gun/general/rocket_launch.ogg').vary_frequency(TRUE).play()
...
/datum/action/vehicle/sealed/mecha/skyfall/proc/land()
	...
	PLAYSOUND(chassis, 'sound/effects/explosion/explosion1.ogg').vary_frequency(1).play()

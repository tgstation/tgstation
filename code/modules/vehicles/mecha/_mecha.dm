...
/obj/vehicle/sealed/mecha/process()
	...
	if(internal_damage)
		if(internal_damage & MECHA_INT_FIRE)
			if(!(internal_damage & MECHA_INT_TEMP_CONTROL) && DT_PROB(2.5, SSOBJ_DT))
			...
			if(internal_tank)
				...
				if(int_tank_air && int_tank_air.return_volume() > 0) //heat the air_contents
					int_tank_air.temperature = min(6000+T0C, int_tank_air.temperature+rand(5,7.5)*SSOBJ_DT)
			if(cabin_air && cabin_air.return_volume()>0)
				cabin_air.temperature = min(6000+T0C, cabin_air.return_temperature()+rand(5,7.5)*SSOBJ_DT)
				if(cabin_air.return_temperature() > max_temperature/2)
					take_damage(SSOBJ_DT*2/round(max_temperature/cabin_air.return_temperature(),0.1), BURN, 0, 0)
		...
		if(internal_damage & MECHA_INT_TANK_BREACH) //remove some air from internal tank
			if(internal_tank)
				var/datum/gas_mixture/int_tank_air = internal_tank.return_air()
				var/datum/gas_mixture/leaked_gas = int_tank_air.remove_ratio(DT_PROB_RATE(0.05, SSOBJ_DT))
				if(loc)
		...
		if(internal_damage & MECHA_INT_SHORT_CIRCUIT)
			if(get_charge())
				spark_system.start()
				cell.charge -= min(10 * SSOBJ_DT, cell.charge)
				cell.maxcharge -= min(10 * SSOBJ_DT, cell.maxcharge)

	if(internal_temp_regulation)
		if(cabin_air && cabin_air.return_volume() > 0)
			var/delta = cabin_air.temperature - T20C
			cabin_air.temperature -= clamp(round(delta / 8, 0.1), -5, 5) * SSOBJ_DT

...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic
	...
	/// Tracks how many projectiles are left in the magazine.
	var/projectiles = 0
	/// The max amount of projectiles that can be loaded into the magazine.
	var/magazine_size = 0
	/// Tracks how much excess ammo is stored - When reloading this is used to fill the magazine.
	var/stored_ammo = 0
	/// Max amount of ammo that can be stored
	/// If 0, this weapon can ONLY be manually reloaded
	var/stored_ammo_max = 0
	/// What item do we need to reload this weapon? String key
	var/ammo_type

/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/Initialize(mapload)
	. = ..()
	if(projectiles > magazine_size)
		stack_trace("Projectiles exceed magazine size in [src] ([projectiles] > [magazine_size])")
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/get_snowflake_data()
	return list(
		...
		"max_magazine" = magazine_size,
		"stored_ammo" = stored_ammo,
		"stored_ammo_max" = stored_ammo_max,
		"ammo_type" = ammo_type,
	)
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/rearm()
	if(projectiles < magazine_size)
		var/projectiles_to_add = magazine_size - projectiles
		if(!stored_ammo)
			return FALSE
		if(projectiles_to_add <= stored_ammo)
			projectiles += projectiles_to_add
			stored_ammo -= projectiles_to_add
		else
			projectiles += stored_ammo
			stored_ammo = 0
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/launcher/proc/get_final_target(atom/base_target, mob/source, variance_modifier = 1)
	if(!variance)
		return base_target

	var/turf/center = get_step(base_target, REVERSE_DIR(source.dir))
	var/radius = floor(variance * variance_modifier)
	return pick(RANGE_TURFS(radius, center))
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/launcher/action(mob/source, atom/target, list/modifiers)
	...
	var/num_actually_thrown = 0
	for(var/i in 1 to num_thrown)
		if(energy_drain && !chassis.has_charge(energy_drain))//in case we run out of energy mid-burst, such as emp
			break
		if(projectiles <= 0)
			break
		...
		var/obj/thrown = new projectile(chassis.loc)
		proj_init(thrown, source)
		thrown.throw_at(
			target = get_final_target(target, source, i / num_thrown),
			range = missile_range,
			speed = missile_speed,
			thrower = source,
			spin = spin,
			diagonals_first = diags_first,
		)

	if(num_actually_thrown <= 0)
		return FALSE

	log_message("Launched [num_actually_thrown] [initial(projectile.name)]\s from [name], targeting [target].", LOG_MECHA)
	sleep(max(0, projectile_delay))
	if(kickback)
		chassis.newtonian_move(newtonian_target)
	return TRUE
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/launcher/bola
	name = "\improper NET-5 Bola Launcher"
	desc = "A weapon for combat exosuits. Launches bola nets to entangle targets."
	icon_state = "mecha_ebola"
	num_thrown = 2
	projectiles = 999
	magazine_size = 999

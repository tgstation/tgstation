...
/obj/vehicle/ridden/scooter/skateboard/Bump(atom/bumped_thing)
	...
	PLAYSOUND(src, 'sound/effects/bang.ogg').volume(40).vary(TRUE).play()
...
/obj/vehicle/ridden/scooter/skateboard/proc/grind()
	...
	if(skater.getStaminaLoss() >= 100)
		...
		icon_state = "[initial(icon_state)]"
		return
	PLAYSOUND(src, 'sound/vehicles/skateboard_roll.ogg').vary(TRUE).play()
	...
	for(var/mob/living/carbon/victim in location)
		if(victim.body_position == LYING_DOWN)
			PLAYSOUND(location, 'sound/items/trayhit/trayhit2.ogg').volume(40).play()

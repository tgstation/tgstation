...
// agent macro for info provider so you don't have to do agent.agent everywhere
#define AGENT agent.agent
...
/datum/goap_info_provider/proc/ShouldTryGoalAgain(list/goal, list/worldstate)
	...
	if(!failurestate || !failurestate.state || !failurestate.Matches(goalstate))

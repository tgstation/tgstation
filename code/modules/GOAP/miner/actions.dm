...
/datum/goap_action/miner/get_pickaxe/AdvancedPreconditions(atom/agent, list/worldstate)
	var/mob/living/carbon/human/H = agent
	var/obj/item/pickaxe/pickax = locate(/obj/item/pickaxe) in H.get_contents()
	if(pickax != null)
		target = pickax
	else
		var/obj/item/pickaxe/P = locate(/obj/item/pickaxe) in view(10, agent)
		target = P
	return (target != null)
...
/datum/goap_action/miner/mine_turf/AdvancedPreconditions(atom/agent, list/worldstate)
	var/turf/closed/mineral/T = locate(/turf/closed/mineral) in RANGE_TURFS(10, agent)
	target = T
	return (target != null)

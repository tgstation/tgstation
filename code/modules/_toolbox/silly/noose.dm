...
/obj/structure/chair/noose/user_buckle_mob(mob/living/carbon/human/M, mob/user)
	...
	if(M == user && buckle_mob(M))
		...
		playsound(user.loc, 'sound/toolbox/noosed.ogg', 50, 1, -1)
		user.log_message("<font color='orange'>hanged themselves</font>", INDIVIDUAL_ATTACK_LOG)
		...
		if(do_mob(user, M, 200))
			if(buckle_mob(M))
				...
				var/logtext = "hanged [M]([M.ckey])"
				if(T)
					logtext += " at ([T.x],[T.y],[T.z] [get_area(T)])."
				user.log_message("<font color='orange'>[logtext]</font>", INDIVIDUAL_ATTACK_LOG)
				message_admins("[user] has [logtext]")
	...
	for(var/m in buckled_mobs)
		...
		if(buckled_mob.mob_has_gravity())
			...
			if(prob(20))
				...
				playsound(buckled_mob.loc, 'sound/toolbox/noose_idle.ogg', 30, 1, -3)

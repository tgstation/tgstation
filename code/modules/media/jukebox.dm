...
/obj/machinery/media/jukebox/attack_hand(var/mob/user)
	if(stat & NOPOWER)
		usr << "<span class='danger'>You don't see anything to mess with.</span>"
		return
	if(stat & BROKEN && playlist!=null)
		user.visible_message("<span class='userdanger'>[user.name] smacks the side of \the [src.name].</span>","<span class='danger'>You hammer the side of \the [src.name].</span>")
		stat &= ~BROKEN
		playlist=null
		playing=emagged
		update_icon()
...
/obj/machinery/media/jukebox/Topic(href, href_list)
	...
	..()
	if(emagged)
		usr << "<span class='danger'>You touch the bluescreened menu. Nothing happens. You feel dumber.</span>"
		return
	...
	if (href_list["playlist"])
		if(!check_reload())
			usr << "<span class='danger'>You must wait 60 seconds between playlist reloads.</span>"
			return
...
/obj/machinery/media/jukebox/process()
	if(!playlist)
		...
		if(response)
			...
			if("/>" in json)
				visible_message("<span class='warning'>\icon[src] \The [src] buzzes, unable to update its playlist.</span>","You hear a buzz.")
				stat &= BROKEN
				update_icon()
				return
			...
			if(playlist.len==0)
				...
				update_icon()
				return
			visible_message("<span class='notice'>\icon[src] \The [src] beeps, and the menu on its front fills with [playlist.len] items.</span>","You hear a beep.")
		else
...
/obj/machinery/media/jukebox/update_music()
	if(current_song && playing)
		...
		visible_message("<span class='notice'>\icon[src] \The [src] begins to play [song.display()].</span>","You hear music.")

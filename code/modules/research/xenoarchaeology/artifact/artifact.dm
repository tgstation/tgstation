...
/obj/structure/boulder/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/pickaxe))
		...
		user << "<span class='notice'>You finish cutting into [src].</span>"
		excavation_level += P.excavation_amount

		if(excavation_level > 100)
			//failure
			src.visible_message("<span class='danger'>\The [src] suddenly crumbles away.</span>")
			user << "<span class='danger'>\The [src] has disintegrated under your onslaught, any secrets it was holding are long gone.</span>"
			del(src)
			return

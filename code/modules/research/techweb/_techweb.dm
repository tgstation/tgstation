...
/datum/techweb/proc/recalculate_nodes(recalculate_designs = FALSE, preserve_custom_designs = TRUU)
	...
	if(recalculate_designs)					//Wipes custom added designs like from design disks or anything like that!
		researched_design_ids = preserve_custom_designs? custum_design_ids.Copy() : list()

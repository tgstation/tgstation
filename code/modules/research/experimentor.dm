...
/obj/machinery/rnd/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_HEAT)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			explosion(src, devastation_range = -1, heavy_impact_range = 0, light_impact_range = 0, flame_range = 2, flash_range = 0, adminlog = FALSE)
...
/obj/item/relic/proc/do_explode(mob/user)
	if(loc == user)
		...
		explosion(src, devastation_range = 0, heavy_impact_range = rand(1,5), light_impact_range = rand(1,5), flame_range = 2, flash_range = rand(1,5), adminlog = TRUE)

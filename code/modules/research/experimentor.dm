...
/obj/machinery/r_n_d/experimentor/RefreshParts()
	...
	for(var/obj/item/weapon/stock_parts/scanning_module/M in component_parts)
		...
		destroychance -= M.rating*11
	for(var/obj/item/weapon/stock_parts/micro_laser/M in component_parts)
		badThingCoeff += M.rating
		destroychance -= M.rating*5.5
...
/obj/machinery/r_n_d/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_POKE)
		...
		if(prob(EFFECT_PROB_VERYLOW+badThingCoeff))
			visible_message("<span class='danger'>[src] malfunctions, lashing its arms out at nearby people!</span>")
			...
			if(prob(destroychance))
				ejectItem(TRUE)
				visible_message("<span class='danger'>[exp_on] has been destroyed!</span>")
	...
	if(exp == SCANTYPE_IRRADIATE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			ejectItem()
		if(prob(EFFECT_PROB_VERYLOW+badThingCoeff))
			visible_message("<span class='danger'>[src] malfunctions, leaking radiation!</span>")
	...
	if(exp == SCANTYPE_GAS)
		...
		if(prob(EFFECT_PROB_VERYLOW+badThingCoeff))
			visible_message("<span class='danger'>[src] leaks dangerous gas!</span>")
		...
		if(prob(EFFECT_PROB_LOW+badThingCoeff))
			...
			throwSmoke(src.loc)
		if(prob(EFFECT_PROB_MEDIUM+badThingCoeff))
			visible_message("<span class='warning'>[src] ionizes the air around it!</span>")
			empulse(src.loc, 4, 6)
	...
	if(exp == SCANTYPE_HEAT)
		...
		if(prob(EFFECT_PROB_VERYLOW+badThingCoeff))
			...
			if(MT)
				...
				FB.fire()
		if(prob(EFFECT_PROB_LOW+badThingCoeff))
			visible_message("<span class='danger'>[src] malfunctions, releasing a burst of flame!</span>")
			...
			if(prob(destroychance))
				...
				visible_message("<span class='danger'>[exp_on] has been destroyed!</span>")
		if(prob(EFFECT_PROB_MEDIUM+badThingCoeff))
			visible_message("<span class='warning'>[src] malfunctions, leaking hot air!</span>")
	...
	if(exp == SCANTYPE_COLD)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			investigate_log("Experimentor has made a cup of [chosenchem] coffee.", "experimentor")
		if(prob(EFFECT_PROB_VERYLOW+badThingCoeff))
			visible_message("<span class='danger'>[src] malfunctions, releasing a dangerous cloud of coolant!</span>")
			var/datum/reagents/R = new/datum/reagents(50)
			R.my_atom = src
			...
			if(prob(destroychance))
				...
				visible_message("<span class='danger'>[exp_on] has been destroyed!</span>")
		if(prob(EFFECT_PROB_LOW+badThingCoeff))
			visible_message("<span class='warning'>[src] malfunctions, leaking cold air!</span>")
			var/datum/gas_mixture/env = src.loc.return_air()
			var/transfer_moles = 0.25 * env.total_moles()
			var/datum/gas_mixture/removed = env.remove(transfer_moles)
			if(removed)
	...
	if(prob(EFFECT_PROB_VERYLOW+badThingCoeff))
		...
		if(globalMalf > 16 && globalMalf < 35)
			visible_message("<span class='warning'>[src] ian-izes the air around it!</span>")
			throwSmoke(src.loc)
			if(trackedIan)

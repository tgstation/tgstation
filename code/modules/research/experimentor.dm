...
/obj/machinery/rnd/experimentor/proc/warn_admins(user, ReactionName)
	...
	message_admins("Experimentor reaction: [ReactionName] generated by [ADMIN_LOOKUPFLW(user)] at [ADMIN_VERBOSEJMP(T)]",0,1)
...
/obj/item/relic/proc/warn_admins(mob/user, RelicType, priority = 1)
	...
	if(priority) //For truly dangerous relics that may need an admin's attention. BWOINK!
		message_admins("[RelicType] relic activated by [ADMIN_LOOKUPFLW(user)] in [ADMIN_VERBOSEJMP(T)]",0,1)
	log_game(log_msg)

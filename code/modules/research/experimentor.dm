	...
	if(prob(EFFECT_PROB_VERYLOW-badThingCoeff) && loaded_item)
		...
		if(globalMalf > 36 && globalMalf < 50)
			...
			for(var/mob/living/m in view(4,src))
				...
				m.apply_damage(25, INTERNAL, BODY_ZONE_CHEST)
				investigate_log("Experimentor has taken a blood sacrifice from [m], dealing 25 internal damage", INVESTIGATE_EXPERIMENTOR)

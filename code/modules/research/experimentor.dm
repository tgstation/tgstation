...
/obj/machinery/rnd/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_GAS)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			SSwardrobe.provide(/obj/item/stack/sheet/mineral/plasma, get_turf(pick(oview(1, src))))
		else if(prob(EFFECT_PROB_VERYLOW-malfunction_probability_coeff))
	...
	if(exp == SCANTYPE_OBLITERATE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			SSwardrobe.provide(/obj/item/stack/sheet/plasteel, get_turf(pick(oview(1, src))))
		else if(prob(EFFECT_PROB_VERYLOW-malfunction_probability_coeff))

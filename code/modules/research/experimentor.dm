...
/obj/machinery/rnd/experimentor
	...
	var/bad_thing_coeff_modifier = 0
	var/resetTime = 15
	var/cloneMode = FALSE
...
/obj/machinery/rnd/experimentor/RefreshParts()
	. = ..()
	bad_thing_coeff = bad_thing_coeff_modifier
	...
	for(var/obj/item/stock_parts/scanning_module/M in component_parts)
		bad_thing_coeff += M.rating*2
	for(var/obj/item/stock_parts/micro_laser/M in component_parts)
		bad_thing_coeff += M.rating

/obj/machinery/rnd/experimentor/examine(mob/user)
	. = ..()
	if(in_range(user, src) || isobserver(user))
		. += span_notice("The status display reads: Malfunction probability reduced by <b>[bad_thing_coeff]%</b>.<br>Cooldown interval between experiments at <b>[resetTime*0.1]</b> seconds.")
...
/obj/machinery/rnd/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_POKE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			bad_thing_coeff_modifier++
			RefreshParts() //recalculate bad_thing_coeff
		else if(prob(EFFECT_PROB_VERYLOW-bad_thing_coeff))
			...
			ejectItem(TRUE)
		else if(prob(EFFECT_PROB_LOW-bad_thing_coeff))
			visible_message(span_warning("[src] malfunctions!"))
			exp = SCANTYPE_OBLITERATE
		else if(prob(EFFECT_PROB_MEDIUM-bad_thing_coeff))
	...
	//Global reactions
	if(prob(EFFECT_PROB_VERYLOW-bad_thing_coeff) && loaded_item)

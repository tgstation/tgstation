...
/obj/item/assembly/relic/Initialize(mapload)
	. = ..()
...
/obj/item/assembly/relic/proc/random_themed_appearance()
...
/obj/item/assembly/relic/lavaland
	name = "strange relic"
	artifact_theme = RELIC_NECROTECH

/obj/item/assembly/relic/proc/reveal()
...
/obj/item/assembly/relic/activate(mob/activator)
	...
	if(!hidden_power)
		return FALSE
	call(src, hidden_power)(activator)
	. = ..()

/// Helper to spawn smoke somewhere
/obj/item/assembly/relic/proc/throw_smoke(turf/where)
...
/obj/item/assembly/relic/proc/cleaning_foam(mob/user)
	playsound(src, SFX_SPARKS, rand(25, 50), TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
	var/obj/item/grenade/chem_grenade/cleaner/spawned_foamer = new(get_turf(src))
	spawned_foamer.detonate()
	qdel(spawned_foamer)
	warn_admins(user, "Foam", FALSE)

/// Similar to cleaning foam but spawns the acid variant
/obj/item/assembly/relic/proc/cleaning_foam_acid(mob/user)
	playsound(src, SFX_SPARKS, rand(25, 50), TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
	var/obj/item/grenade/chem_grenade/ez_clean/spawned_foamer = new(get_turf(src))
	spawned_foamer.detonate()
	qdel(spawned_foamer)
...
/obj/item/assembly/relic/proc/flashbanger(mob/user)
	playsound(src, SFX_SPARKS, rand(25,50), TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
	var/obj/item/grenade/flashbang/spawned_flashbang = new(get_turf(src))
...
/obj/item/assembly/relic/proc/drink_dispenser(mob/user)
	var/obj/item/reagent_containers/cup/glass/drinkingglass/freebie = new(get_step_rand(src))
...
/obj/item/assembly/relic/proc/dispense_drink(obj/item/reagent_containers/cup/glass/glasser)
...
/obj/item/assembly/relic/proc/tummy_ache(mob/user)
...
/obj/item/assembly/relic/proc/scrambliticus(mob/user)
	new /obj/effect/temp_visual/circle_wave/bioscrambler/light(get_turf(src))
...
/obj/item/assembly/relic/proc/charger(mob/living/user)
	...
	if(iscarbon(mob_to_charge))
		...
		mob_to_charge.electrocute_act(15, src, flags = SHOCK_NOGLOVES)

	var/list/chargeable_items = mob_to_charge.get_all_cells(max_percent = 0.95) // otherwise the PDA always gets recharged
...
/obj/item/assembly/relic/proc/cut_the_overlay(atom/shocker, lightning)
...
/obj/item/assembly/relic/proc/disguiser(mob/user)
	...
	if(prob(80)) // >:)
		ADD_TRAIT(to_disguise, TRAIT_NO_JUMPSUIT, REF(src)) // prevent dropping pockets & belt

	// magic trick!
	playsound(to_disguise, SFX_SPARKS, rand(25,50), TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
	...
	REMOVE_TRAIT(to_disguise, TRAIT_NO_JUMPSUIT, REF(src))

/obj/item/assembly/relic/proc/roll_costume(slot, flagcheck)
...
/obj/item/assembly/relic/proc/t1_shield_holder(mob/user)
	...
	var/datum/component/shield = outermost_item.AddComponent( \
		...
		run_hit_callback = CALLBACK(src, PROC_REF(shield_hit), outermost_item), \
	)
	var/old_light_system = outermost_item.light_system
	outermost_item.light_system = OVERLAY_LIGHT
	var/datum/component/light = outermost_item.AddComponent( \
	...
	addtimer(CALLBACK(src, PROC_REF(remove_shield), outermost_item, old_light_system, list(shield, light)), activation_cooldown * 0.5)
	outermost_item.add_filter("block_shield", 1, outline_filter(0, COLOR_BIOLUMINESCENCE_YELLOW), shield)
	outermost_item.transition_filter("block_shield", outline_filter(2, COLOR_BIOLUMINESCENCE_YELLOW), activation_cooldown * 0.5)

	relic_message(span_notice("[outermost_item] starts to glow a bright yellow!"))
...
/obj/item/assembly/relic/proc/shield_hit(obj/item/comp_parent, mob/living/owner, attack_text, current_charges)
	...
	if(current_charges <= 0)
		...
		comp_parent.remove_filter("block_shield")

/obj/item/assembly/relic/proc/remove_shield(obj/item/affected, old_light_system, list/cleanup_components)
	for(var/datum/component/comp as anything in cleanup_components)
		qdel(comp)
	affected.light_system = initial(light_system)
	if(affected.light_on)
		relic_message(span_notice("[affected] stops glowing."))
	affected.remove_filter("block_shield")

/// Places rock turfs around the relic
/obj/item/assembly/relic/proc/place_rocks(mob/user)
...
/obj/item/assembly/relic/proc/yeet_blood(mob/living/user)
...
/obj/item/assembly/relic/proc/suck_blood(mob/living/user)
...
/obj/item/assembly/relic/proc/warn_admins(mob/user, relic_type, priority = TRUE)
...
/obj/item/assembly/relic/revealed/Initialize(mapload)
	. = ..()
	auto_reveal()

/obj/item/assembly/relic/lavaland

/obj/item/assembly/relic/lavaland/revealed/Initialize(mapload)
	. = ..()
	auto_reveal()

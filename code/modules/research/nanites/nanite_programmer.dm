...
/obj/machinery/nanite_programmer/ComponentInitialize()
	. = ..()
	AddComponent(/datum/component/diskmachine, CALLBACK(src, .proc/diskcallback), /obj/item/disk/nanite_program)

/obj/machinery/nanite_programmer/proc/diskcallback(datum/disk_data)
...
/obj/machinery/nanite_programmer/ui_data()
	var/list/data = list()
	data["has_disk"] = SEND_SIGNAL(src, COMPONENT_HAS_DISK) & COMPONENT_DISK_INSERTED
	data["has_program"] = istype(data_on_disk)
	var/datum/nanite_program/program = data_on_disk
	if(program)
...
/obj/machinery/nanite_programmer/ui_act(action, params)
	...
	switch(action)
		...
		if("toggle_active")
			...
			data_on_disk?.activated = !data_on_disk.activated //we don't use the activation procs since we aren't in a mob
			. = TRUE
		if("set_code")
			...
			switch(target_code)
				if("activation")
					data_on_disk?.activation_code = clamp(round(new_code, 1),0,9999)
				if("deactivation")
					data_on_disk?.deactivation_code = clamp(round(new_code, 1),0,9999)
				if("kill")
					data_on_disk?.kill_code = clamp(round(new_code, 1),0,9999)
				if("trigger")
					data_on_disk?.trigger_code = clamp(round(new_code, 1),0,9999)
			. = TRUE
		if("set_extra_setting")
			data_on_disk?.set_extra_setting(params["target_setting"], params["value"])
			playsound(src, "terminal_type", 25, FALSE)
			. = TRUE
		if("set_restart_timer")
			...
			if(!isnull(timer))
				...
				data_on_disk?.timer_restart = timer
			. = TRUE
		if("set_shutdown_timer")
			...
			if(!isnull(timer))
				...
				timer = clamp(round(timer, 1), 0, 3600)
				timer *= 10 //convert to deciseconds
				data_on_disk?.timer_shutdown = timer
			. = TRUE
		if("set_trigger_timer")
			...
			if(!isnull(timer))
				...
				timer = clamp(round(timer, 1), 0, 3600)
				timer *= 10 //convert to deciseconds
				data_on_disk?.timer_trigger = timer
			. = TRUE
		if("set_timer_trigger_delay")
			...
			if(!isnull(timer))
				...
				timer = clamp(round(timer, 1), 0, 3600)
				timer *= 10 //convert to deciseconds
				data_on_disk?.timer_trigger_delay = timer
			. = TRUE

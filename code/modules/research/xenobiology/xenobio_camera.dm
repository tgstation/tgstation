...
/mob/camera/ai_eye/remote/xenobio/setLoc(var/t)
	...
	if(new_area && new_area.name == allowed_area || new_area && CHECK_BITFIELD(new_area.area_flags, XENOBIOLOGY_COMPATIBLE))
		return ..()
	else
		return
...
/obj/machinery/computer/camera_advanced/xenobio/proc/XenoSlimeClickCtrl(mob/living/user, mob/living/simple_animal/slime/S)
	...
	if(mobarea.name == E.allowed_area || CHECK_BITFIELD(mobarea, XENOBIOLOGY_COMPATIBLE))
		slime_scan(S, C)
...
/obj/machinery/computer/camera_advanced/xenobio/proc/XenoTurfClickShift(mob/living/user, turf/open/T)
	...
	if(turfarea.name == E.allowed_area || CHECK_BITFIELD(turfarea, XENOBIOLOGY_COMPATIBLE))
		for(var/mob/living/simple_animal/slime/S in X.stored_slimes)
			S.forceMove(T)
			S.visible_message("<span class='notice'>[S] warps in!</span>")
			X.stored_slimes -= S

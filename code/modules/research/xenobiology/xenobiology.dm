...
/obj/item/slimepotion/sentience/afterattack(mob/living/M, mob/user)
	...
	if(SM.sentience_type != sentience_type)
		to_chat(user, "<span class='warning'>\The [name] won't work on [SM].</span>")
		return ..()



	to_chat(user, "<span class='notice'>You offer \the [name] to [SM]...</span>")
	being_used = 1
	...
	if(candidates.len)
		...
		to_chat(user, "<span class='notice'>[SM] accepts \the [name] and suddenly becomes attentive and aware. It worked!</span>")
		qdel(src)
	else

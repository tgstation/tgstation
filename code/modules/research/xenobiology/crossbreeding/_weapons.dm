...
/obj/item/kitchen/knife/rainbowknife/afterattack(atom/O, mob/user, proximity)
	...
	switch(damtype)
		...
		if(BURN)
			...
			attack_verb_continuous = list("burned", "singed", "heated")
			attack_verb_simple = list("burn", "sing", "heat")
		if(TOX)
			hitsound = 'sound/weapons/pierce.ogg'
			attack_verb_continuous = list("poisoned", "dosed", "toxified")
			attack_verb_simple = list("poison", "dose", "toxify")
		if(OXY)
			hitsound = 'sound/effects/space_wind.ogg'
			attack_verb_continuous = list("suffocated", "winded", "vacuumed")
			attack_verb_simple = list("suffocate", "wind", "vacuum")
		if(CLONE)
			hitsound = 'sound/items/geiger/ext1.ogg'
			attack_verb_continuous = list("irradiated", "mutated", "maligned")
...
/obj/item/shield/adamantineshield
	...
	throw_speed = 2
	attack_verb_continuous = list("bashed","pounded","slammed")

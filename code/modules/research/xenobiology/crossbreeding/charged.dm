...
/obj/item/slimecross/charged/attack_self(mob/user)
	...
	create_sound(src, 'sound/effects/light_flicker.ogg').vary(TRUE).play()
	do_effect(user)
...
/obj/item/slimecross/charged/gold/process()
	...
	create_sound(get_turf(src), SFX_SPARKS).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()

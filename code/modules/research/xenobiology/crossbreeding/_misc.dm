...
/obj/item/camera/rewind/afterattack(atom/target, mob/user, flag)
	...
	if(!used)//selfie time
		if(user == target)
			to_chat(user, span_boldnotice("You take a selfie!"))
		else
			to_chat(user, span_boldnotice("You take a photo with [target]!"))
			to_chat(target, span_boldnotice("[user] takes a photo with you!"))

...
/obj/item/slimecross/burning/metal/do_effect(mob/user)
	for(var/turf/closed/wall/W in range(1,get_turf(user)))
		W.dismantle_wall(1)
		PLAYSOUND(W, 'sound/effects/break_stone.ogg').vary(TRUE).play()
...
/obj/item/slimecross/burning/yellow/do_effect(mob/user)
	...
	PLAYSOUND(get_turf(src), 'sound/items/weapons/zapbang.ogg').volume(50).vary(TRUE).play()
...
/obj/item/slimecross/burning/bluespace/do_effect(mob/user)
	...
	for(var/mob/living/L in range(1, get_turf(user)))
		if(L != user)
			...
			PLAYSOUND(get_turf(L), SFX_SPARKS).volume(50).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
	..()
...
/obj/item/slimecross/burning/oil/proc/boom()
	...
	PLAYSOUND(T, 'sound/effects/explosion/explosion2.ogg').volume(200).vary(TRUE).play()

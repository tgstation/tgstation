...
/obj/item/slimecross/consuming/attackby(obj/item/O, mob/user)
	if(IS_EDIBLE(O))
		...
		if(nutriment_eaten >= nutriment_required)
			...
			PLAYSOUND(src, 'sound/effects/splat.ogg').volume(40).vary_frequency(TRUE).play()
...
/obj/item/slime_cookie/attack(mob/living/M, mob/user)
	...
	if(fed)
		...
		PLAYSOUND(get_turf(M), 'sound/items/eatfood.ogg').volume(20).vary_frequency(TRUE).play()
		if(nutrition)
...
/obj/item/slime_cookie/bluespace/do_effect(mob/living/eater, mob/user)
	...
	PLAYSOUND(target, get_sfx(SFX_SPARKS)).volume(50).vary_frequency(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
...
/obj/item/slime_cookie/red/do_effect(mob/living/M, mob/user)
	...
	PLAYSOUND(get_turf(M), 'sound/effects/splat.ogg').volume(10).vary_frequency(TRUE).play()
	if(iscarbon(M))
...
/obj/item/slime_cookie/gold/do_effect(mob/living/M, mob/user)
	...
	PLAYSOUND(get_turf(C), 'sound/items/coinflip.ogg').volume(50).vary_frequency(TRUE).play()
	M.put_in_hand(C)

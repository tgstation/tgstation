...
/obj/machinery/vending/proc/tilt(atom/fatty, crit=FALSE)
	...
	if(in_range(fatty, src))
		for(var/mob/living/L in get_turf(fatty))
			...
			add_memory_in_range(L, 7, MEMORY_VENDING_CRUSHED, list(DETAIL_PROTAGONIST = L, DETAIL_WHAT_BY = src), story_value = STORY_VALUE_AMAZING, memory_flags = MEMORY_CHECK_BLINDNESS, protagonist_memory_flags = NONE)

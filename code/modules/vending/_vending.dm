	...
	if (!target.is_blocked_turf(TRUE, src, list(src)))
		for(var/atom/atom_target in (target.contents) + target)
			...
			if (isliving(atom_target))
				...
				PLAYSOUND(living_target, 'sound/effects/blob/blobattack.ogg').volume(40).vary_frequency(TRUE).play()
				PLAYSOUND(living_target, 'sound/effects/splat.ogg').volume(50).vary_frequency(TRUE).play()
		...
		PLAYSOUND(src, 'sound/effects/bang.ogg').volume(40).play()

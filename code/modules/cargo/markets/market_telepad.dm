...
/obj/machinery/ltsrbt/open_machine(drop = TRUE, density_to_set = FALSE)
	. = ..()
	PLAYSOUND(src, 'sound/machines/oven/oven_open.ogg').volume(75).vary_frequency(TRUE).play()

/obj/machinery/ltsrbt/close_machine(atom/movable/target, density_to_set = TRUE)
	. = ..()
	PLAYSOUND(src, 'sound/machines/oven/oven_close.ogg').volume(75).vary_frequency(TRUE).play()
...
/obj/machinery/ltsrbt/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(state_open)
		if(locate(/mob/living) in tool.get_all_contents())
			...
			PLAYSOUND(src, 'sound/machines/buzz/buzz-sigh.ogg').volume(40).vary_frequency(FALSE).play()
...
/obj/machinery/ltsrbt/proc/place_on_market(mob/user)
	...
	PLAYSOUND(src, 'sound/effects/cashregister.ogg').volume(40).vary_frequency(FALSE).play()

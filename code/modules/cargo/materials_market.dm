...
/obj/machinery/materials_market/attackby(obj/item/O, mob/user, params)
	if(is_type_in_list(O, exportable_material_items))
		...
		if(!amount)
			...
			PLAYSOUND(src, 'sound/machines/scanner/scanbuzz.ogg').volume(25).vary_frequency(FALSE).play()
			return TRUE
		qdel(exportable)
		...
		if("buy")
			...
			if(!isnull(current_order))
				...
				if(prior_sheets + quantity > SSstock_market.materials_quantity[material_bought] )
					...
					PLAYSOUND(usr, 'sound/machines/synth/synth_no.ogg').volume(35).vary_frequency(FALSE).play()
					return

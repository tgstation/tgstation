...
/mob/living/simple_animal/pet/gondola/gondolapod/examine(mob/user)
	..()
	if (contents.len)
		to_chat(user, "<span class='notice'>It appears to not have made its delivery yet.</b>.<span>")
	else
		to_chat(user, "<span class='notice'>It seems like it has already made its delivery.</b>.<span>")

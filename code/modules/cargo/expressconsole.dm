			...
			if(!(obj_flags & EMAGGED))
				if(SO.pack.get_cost() <= points_to_check)
					...
					if (istype(beacon) && usingBeacon)//prioritize beacons over landing in cargobay
						...
						if (!landingzone)
							WARNING("[src] couldn't find a Quartermaster/Storage (aka cargobay) area on the station, and as such it has set the supplypod landingzone to the area it resides in.")

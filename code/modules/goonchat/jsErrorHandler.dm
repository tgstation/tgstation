...
/datum/debugFileOutput
	...
	//Add your dumb file here. This is so some schmuck can't just shit out a bunch of spam logfiles and use all the diskspace. Relative to directory
...
/datum/debugFileOutput/proc/error(fileName, message, client/C)
	...
	if (!(fileName in validFiles))
		throw EXCEPTION("Debug log file '[fileName].[ext]' is not a valid path.")

	var/logFile = file("[directory]/[fileName].[ext]")
	var/fileSize = length(logFile)
	if (fileSize >= logFileLimit)
		CRASH("Debug Error Handling encountered an error! This is highly ironic! File: '[fileName]' has exceeded the filesize limit of: [logFileLimit] bytes")
...
/datum/debugFileOutput/proc/clear(fileName)
	...
	if (!fexists("[directory]/[fileName].[ext]"))
		throw EXCEPTION("Debug log file '[fileName].[ext]' does not exist.")

	if (!(fileName in validFiles))
		throw EXCEPTION("Debug log file '[fileName].[ext]' is not a valid path.")

	fdel("[directory]/[fileName].[ext]")
	return 1

/datum/debugFileOutput/proc/clearAll()
	var/list/deleted = new()
	for (var/fileName in validFiles)
		if (fexists("[directory]/[fileName].[ext]"))
			fdel("[directory]/[fileName].[ext]")
		deleted += fileName

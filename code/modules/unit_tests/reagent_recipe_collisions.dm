...
/datum/unit_test/reagent_recipe_collisions/Run()
	...
	for(var/V in SSreagents.reactions_by_reagent_id)
		reactions += SSreagents.reactions_by_reagent_id[V]
...
/datum/unit_test/reagent_recipe_collisions/proc/recipes_do_conflict(datum/chemical_reaction/r1, datum/chemical_reaction/r2)
	...
	//if we got this far, the longer reaction will be impossible to create if the shorter one is earlier in SSreagents.reactions_by_reaction_id, and will require the reagents to be added in a particular order otherwise
	return TRUE
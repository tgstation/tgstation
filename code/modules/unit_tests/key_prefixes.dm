...
/datum/unit_test/key_prefixes/Run()
	...
	var/list/radio_keys = GLOB.department_radio_keys.Copy()
	...
	for(var/radio_key in GLOB.department_radio_keys)
		if(radio_key in radio_keys)
			TEST_FAIL("[GLOB.department_radio_keys[radio_key]] radio channel has the same prefix key ([radio_key]) as [radio_keys[radio_keys.Find(radio_key)]]")

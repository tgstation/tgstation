...
/datum/unit_test/json_savefiles/Run()
	...
	if(json_encode(basic_list) != json_encode(json_savefile["basic_list"]))
		TEST_FAIL("basic_list was not converted correctly")
	if(json_encode(assoc_list) != json_encode(json_savefile["assoc_list"]))
		TEST_FAIL("assoc list was not converted correctly")
	if(var_string != json_savefile["v1"]["var_string"])
		TEST_FAIL("var_string was not imported correctly")
	if(var_number != json_savefile["v1"]["v2"]["var_number"])

...
/datum/unit_test/mapload_space_verification/Run()
	...
	for(var/turf/iterated_turf in world)
		...
		TEST_FAIL("Space turf found in non-allowed area [turf_area] ([turf_area.type]) at [AREACOORD(iterated_turf)]! Please ensure that all space turfs are in an /area/space!")

...
/datum/unit_test/mob_spawn/Run()
	for(var/mob_spawn_path in subtypesof(/obj/effect/mob_spawn))
		...
		if(mob_spawn.ghost_usable)
			TEST_ASSERT(mob_spawn.mob_name, "[mob_spawn] has no \"mob_name\" var, which is required for ghost usable mob spawns.")
			...
			TEST_ASSERT(!mob_spawn.death, "[mob_spawn] has death set to TRUE, which is nonsensical for ghost usable mob spawns.")
			TEST_ASSERT(!mob_spawn.roundstart, "[mob_spawn] has roundstart set to TRUE, which is contradictory for ghost usable mob spawns.")
			TEST_ASSERT(!mob_spawn.instant, "[mob_spawn] has instant set to TRUE, which is contradictory for ghost usable mob spawns.")

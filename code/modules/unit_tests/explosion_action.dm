...
/datum/unit_test/explosion_action/proc/execute_mob_tests()
	...
	EX_ACT(test_dog, EXPLODE_LIGHT) // should do 12 damage (basic animals do a prob() check based on the armor rating, and divide the expected brute loss by 1.5).
	TEST_ASSERT_EQUAL(test_dog.health, MAX_LIVING_HEALTH - 12, "EX_ACT() with EXPLODE_LIGHT severity should have done 12 damage to a corgi with an immune helmet and vest!")
	test_dog.revive(ADMIN_HEAL_ALL)

	EX_ACT(test_dog, EXPLODE_HEAVY) // should do 24 damage.
	TEST_ASSERT_EQUAL(test_dog.health, MAX_LIVING_HEALTH - 24, "EX_ACT() with EXPLODE_HEAVY severity should have done 24 damage to a corgi with an immune helmet and vest!")
	test_dog.revive(ADMIN_HEAL_ALL)

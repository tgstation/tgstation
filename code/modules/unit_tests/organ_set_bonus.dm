...
/datum/unit_test/organ_set_bonus_sanity/Run()
	...
	for(var/datum/infuser_entry/infuser_entry as anything in flatten_list(GLOB.infuser_entries))
		...
		var/datum/status_effect/organ_set_bonus/added_status = !infuser_entry.unreachable_effect && (locate(/datum/status_effect/organ_set_bonus) in lab_rat.status_effects)

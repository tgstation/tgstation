...
/datum/unit_test/moth_food/Run()
	...
	for (var/i in 1 to times_to_bite)
		...
		var/old_integrity = light_snack.atom_integrity
		light_snack.attack(gourmet, gourmet)
		TEST_ASSERT(light_snack.atom_integrity < old_integrity, "Clothing didn't take damage when bitten by moth.")
	TEST_ASSERT(QDELETED(light_snack), "Moth failed to finish eating clothing. Integrity left: [light_snack.atom_integrity]")

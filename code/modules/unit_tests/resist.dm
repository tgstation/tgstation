...
/datum/unit_test/stop_drop_and_roll/Run()
	...
	TEST_ASSERT_EQUAL(human.get_fire_stacks(), 0, "Human does not have 0 fire stacks pre-ignition")

	human.adjust_fire_stacks(5)
	human.ignite_mob()

	TEST_ASSERT_EQUAL(human.get_fire_stacks(), 5, "Human does not have 5 fire stacks pre-resist")
	...
	TEST_ASSERT(human.get_fire_stacks() < 5, "Human did not lower fire stacks after resisting")

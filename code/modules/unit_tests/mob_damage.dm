#define __TOXLOSS (1 << 0)
#define __CLONELOSS (1 << 1)
#define __BRUTE (1 << 2)
#define __BURN (1 << 3)
#define __OXYLOSS (1 << 4)
#define __STAMINA (1 << 5)
...
/datum/unit_test/mob_damage/proc/test_sanity_complex(mob/living/carbon/human/consistent/dummy)
	...
	verify_damage(dummy, 1, included_types = __BRUTE|__BURN)
	...
	verify_damage(dummy, 0, included_types = __BRUTE|__BURN)
...
/datum/unit_test/mob_damage/proc/test_biotypes(mob/living/carbon/human/consistent/dummy)
	...
	apply_damage(dummy, 1, included_types = __TOXLOSS|__CLONELOSS|__STAMINA)
	verify_damage(dummy, 1, included_types = __TOXLOSS|__CLONELOSS|__STAMINA)

	// If we specify MOB_ORGANIC, the damage should not get applied because plasmamen lack that biotype.
	apply_damage(dummy, 1, expected = 0, included_types = __TOXLOSS|__CLONELOSS|__STAMINA, biotypes = MOB_ORGANIC)
	verify_damage(dummy, 1, included_types = __TOXLOSS|__CLONELOSS|__STAMINA)

	// Now if we specify MOB_MINERAL the damage should get applied.
	apply_damage(dummy, 1, included_types = __TOXLOSS|__CLONELOSS|__STAMINA, biotypes = MOB_MINERAL)
	verify_damage(dummy, 2, included_types = __TOXLOSS|__CLONELOSS|__STAMINA)
	...
	apply_damage(dummy, -2, expected = 0, included_types = __TOXLOSS|__CLONELOSS|__STAMINA, biotypes = MOB_MINERAL)
	verify_damage(dummy, 2, included_types = __TOXLOSS|__CLONELOSS|__STAMINA)

	// Force heal some of the damage. When forced = TRUE the damage/healing gets applied no matter what.
	apply_damage(dummy, -1, included_types = __TOXLOSS|__CLONELOSS|__STAMINA, biotypes = MOB_MINERAL, forced = TRUE)
	verify_damage(dummy, 1, included_types = __TOXLOSS|__CLONELOSS|__STAMINA)

	// Now heal the rest of it with the correct biotype. Make sure that this works. We should have 0 damage afterwards.
	apply_damage(dummy, -1, included_types = __TOXLOSS|__CLONELOSS|__STAMINA, biotypes = MOB_ORGANIC)
	verify_damage(dummy, 0, included_types = __TOXLOSS|__CLONELOSS|__STAMINA)
...
/datum/unit_test/mob_damage/proc/test_nobreath(mob/living/carbon/human/consistent/dummy)
	...
	apply_damage(dummy, 2, expected = 0, included_types = __OXYLOSS)
	verify_damage(dummy, 2, included_types = __OXYLOSS)

	// Make sure we are still be able to heal the oxyloss. This should work.
	apply_damage(dummy, -2, included_types = __OXYLOSS)
	verify_damage(dummy, 0, included_types = __OXYLOSS)
...
/datum/unit_test/mob_damage/proc/test_ordered_healing(mob/living/carbon/human/consistent/dummy)
	...
	apply_damage(dummy, 20, included_types = __TOXLOSS|__BRUTE|__BURN)
...
/datum/unit_test/mob_damage/basic/test_sanity_complex(mob/living/basic/mouse/gray/gusgus)
	...
	verify_damage(gusgus, 0, included_types = __BRUTE)

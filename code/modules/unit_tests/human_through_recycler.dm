/// Puts a consistent assistant into an emagged recycler. Verifies that all intended behavi. Of an emagged recycler occurs (chewing up all the clothing, applying a level of melee damage. So on
...
/datum/unit_test/human_through_recycler/Run()
	...
	// But let's check for EVERYTHING just in case, because we don't want to miss anything.
	...
	// This category is stuff that shouldn't exist in the first place. Let's test it anyways in case we decide consistent assistants should have more clothing in the future.

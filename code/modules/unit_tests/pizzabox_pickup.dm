...
/datum/unit_test/pizzabox_pickup/Run()
	...
	// Test 1: Normal click should take a slice, not pick up the box
	var/initial_slices = test_box.pizza.slices_left
	test_box.attack_hand(test_user)
	TEST_ASSERT(test_box.loc != test_user, "Normal click should not pick up box when sliced pizza present")
	TEST_ASSERT(test_box.pizza.slices_left == initial_slices - 1, "Normal click should take slice from sliced pizza on ground")
	...
	initial_slices = test_box.pizza.slices_left
	test_box.attack_hand(test_user)
	TEST_ASSERT(test_box.pizza.slices_left == initial_slices - 1, "Should get slice when holding pizza box")
	
	// Clean up any dropped slice
	dropped_slice = locate(/obj/item/food/pizzaslice) in get_turf(test_user)
	if(dropped_slice)
		qdel(dropped_slice)
	
	// Test 4: When holding box with unsliced pizza, should take out whole pizza
	...
	// Test 5: Verify that closed boxes can always be picked up (existing behavior)
	test_box.open = FALSE

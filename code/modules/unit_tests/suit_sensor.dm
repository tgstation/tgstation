...
/datum/unit_test/suit_sensor/Run()
	...
	var/obj/item/clothing/under/color/sensor_test/under = allocate(__IMPLIED_TYPE__)
	...
	TEST_ASSERT_EQUAL(under.has_sensor, NO_SENSORS, "couldn't properly cut suit sensor from [under]")
	...
	TEST_ASSERT_EQUAL(under.has_sensor, HAS_SENSORS, "couldn't properly install suit sensor on [under]")
	under.emp_act(EMP_HEAVY)
	TEST_ASSERT_EQUAL(under.has_sensor, BROKEN_SENSORS, "[under] sensor wasn't broken by EMP")
	dummy.swap_hand(dummy.get_held_index_of_item(coil))
	click_wrapper(dummy, under) //fix sensor, again
	TEST_ASSERT_EQUAL(under.has_sensor, HAS_SENSORS, "couldn't fix [under] sensor with cable coil")


/obj/item/clothing/under/color/sensor_test
	random_sensor = FALSE

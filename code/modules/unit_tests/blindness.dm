#define HAS_BLINDNESS(mob) (mob.is_blind())
#define HAS_NEARSIGHTEDNESS(mob) (mob.is_nearsighted())
...
/datum/unit_test/blindness/proc/check_if_blind(mob/living/carbon/human/dummy, status_message = "despite being made blind")
	...
	TEST_ASSERT(HAS_BLINDNESS(dummy), "Dummy, [status_message], did not have the blind status effect.")
...
/datum/unit_test/blindness/proc/check_if_not_blind(mob/living/carbon/human/dummy, status_message = "after being cured of blindness")
	...
	TEST_ASSERT(!HAS_BLINDNESS(dummy), "Dummy, [status_message], still had the blindness status effect.")
...
/datum/unit_test/nearsightedness/Run()
	...
	TEST_ASSERT(HAS_NEARSIGHTEDNESS(dummy), "Dummy was no longer nearsighted after putting on glasses. They should still be nearsighted, but it should be disabled.")
	...
	TEST_ASSERT(!dummy.is_nearsighted_currently(), "Dummy ran is_nearsighted_correctly, and it returned an incorrect value, as they are not currently nearsighted.")
	...
	TEST_ASSERT(!HAS_NEARSIGHTEDNESS(dummy), "Dummy is still nearsighted after being cured of it.")
...
/datum/unit_test/eye_damage/Run()
	...
	TEST_ASSERT(HAS_NEARSIGHTEDNESS(dummy), "After sustaining minor eye damage ([minor_damage]), the dummy was not nearsighted.")
	...
	// Heal eye damage
	eyes.setOrganDamage(0)
	TEST_ASSERT(!HAS_NEARSIGHTEDNESS(dummy), "After curing eye damage, the dummy was still nearsighted.")

	// Cause major eye damage
	var/major_damage = eyes.maxHealth * 0.7
	eyes.applyOrganDamage(major_damage) //~35 damage
	TEST_ASSERT(HAS_NEARSIGHTEDNESS(dummy), "After sustaining major eye damage ([major_damage]), the dummy was not nearsighted.")
	// Check that the severity is correct
	nearsightedness = dummy.is_nearsighted()

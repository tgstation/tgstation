	...
	for(var/datum/status_effect/checking as anything in subtypesof(/datum/status_effect))
		...
		switch(initial(checking.processing_speed))
			...
			if(STATUS_EFFECT_PRIORITY)
				var/priority_wait = world.tick_lag * SSpriority_effects.wait // SSpriority_effects has the SS_TICKER flag, so its wait is in ticks, so we have to convert it to seconds.

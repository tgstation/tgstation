...
/// Checks that foodtypes are the same for food whether it's spawned or crafted (with the exact needed types)
/datum/unit_test/focus_only/check_foodtypes

/// Checks that items have roughly the same materials whenever spawned via processing/microwaving/baking and so on or any other mean.

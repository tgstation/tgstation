...
/datum/unit_test/multiple_surgeries/Run()
	...
	TEST_ASSERT(surgery_step.initiate(user, patient_zero, BODY_ZONE_CHEST, scalpel, surgery_for_zero), "Surgery on patient zero was not initiated")

	var/datum/surgery/organ_manipulation/surgery_for_one = new

	// Try to start a new surgery, expect it to fail without the rod of asclepius
	...
	// Try again, now with the rod of asceplius' effect
	user.apply_status_effect(/datum/status_effect/hippocratic_oath)
	TEST_ASSERT(surgery_step.initiate(user, patient_one, BODY_ZONE_CHEST, scalpel, surgery_for_one), "Surgery on patient one was not initiated, despite having rod of asclepius")

...
/datum/unit_test/species_change_clothing/Run()
	...
	TEST_ASSERT(!isnull(morphing_human.get_item_by_slot(ITEM_SLOT_LPOCKET)), lizard_pocket "Lizard somehow lost their pocket items when changing species.")
	TEST_ASSERT(!isnull(morphing_human.get_item_for_held_index(LEFT_HANDS)), lizard_hand, "Lizard somehow lost their hand items when changing species.")

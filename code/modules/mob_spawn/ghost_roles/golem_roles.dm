...
/obj/effect/mob_spawn/ghost_role/human/golem/Initialize(mapload, mob/living/creator, made_of)
	...
	if(!mapload && init_area)
		notify_ghosts("\A golem shell has been completed in \the [init_area.name].", source = src, action = NOTIFY_PLAY, notify_flags = NOTIFY_CATEGORY_NOFLASH, ignore_key = POLL_IGNORE_GOLEM)

...
/obj/effect/mob_spawn/ghost_role/human/ash_walker/Initialize(mapload, datum/team/ashwalkers/ashteam)
	...
	if(spawner_area)
		notify_ghosts("An ash walker egg is ready to hatch in \the [spawner_area.name].", source = src, action = NOTIFY_PLAY, notify_flags = NOTIFY_CATEGORY_NOFLASH, ignore_key = POLL_IGNORE_ASHWALKER)

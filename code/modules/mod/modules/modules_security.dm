...
/obj/item/mod/module/magnetic_harness/proc/pick_up_item(obj/item/item)
	...
	if(!mod.wearer.equip_to_slot_if_possible(item, ITEM_SLOT_SUITSTORE, qdel_on_fail = FALSE, disable_warning = TRUE))
		return
	PLAYSOUND(src, 'sound/items/modsuit/magnetic_harness.ogg').vary(TRUE).play()
...
/obj/item/mod/module/holster/on_use()
	if(!holstered)
		...
		if(mod.wearer.transferItemToLoc(holding, src, force = FALSE, silent = TRUE))
			...
			PLAYSOUND(src, 'sound/items/weapons/gun/revolver/empty.ogg').volume(100).vary(TRUE).play()
...
/obj/item/mod/module/criminalcapture/on_select_use(atom/target)
	...
	PLAYSOUND(linked_bodybag, 'sound/items/weapons/egloves.ogg').volume(80).vary(TRUE).play()
...
/obj/item/mod/module/active_sonar/on_use()
	...
	PLAYSOUND(mod.wearer, ).volume(20).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
	if(!do_after(mod.wearer, 1.1 SECONDS, target = mod))
		return
	PLAYSOUND(mod.wearer, ).volume(75).vary(TRUE).play() // Should be audible for the radius of the sonar

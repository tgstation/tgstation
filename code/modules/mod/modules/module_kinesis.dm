...
/obj/item/mod/module/anomaly_locked/kinesis/process(seconds_per_tick)
	...
	if(grabbed_atom.loc == kinesis_catcher.given_turf)
		if(grabbed_atom.pixel_x == kinesis_catcher.given_x - ICONSIZE_X/2 && grabbed_atom.pixel_y == kinesis_catcher.given_y - ICONSIZE_Y/2)
			return //spare us redrawing if we are standing still
		animate(grabbed_atom, 0.2 SECONDS, pixel_x = grabbed_atom.base_pixel_x + kinesis_catcher.given_x - ICONSIZE_X/2, pixel_y = grabbed_atom.base_pixel_y + kinesis_catcher.given_y - ICONSIZE_Y/2)
	...
	if(direction & NORTH)
		pixel_y_change = ICONSIZE_Y/2
	else if(direction & SOUTH)
		pixel_y_change = -ICONSIZE_Y/2
	if(direction & EAST)
		pixel_x_change = ICONSIZE_X/2

...
/proc/delete_link_visual_generic(datum/mod_link/mod_link)
	var/mob/living/user = mod_link.get_user_callback.Invoke()
	create_sound(mod_link.get_other().holder, 'sound/machines/terminal/terminal_processing.ogg').vary(TRUE).frequency(-1).play()
...
/datum/mod_link/proc/call_link(datum/mod_link/called, mob/user)
	...
	if(!can_call_callback.Invoke() || !called.can_call_callback.Invoke())
		...
		return
	create_sound(called.holder, 'sound/items/weapons/ring.ogg').volume(15).vary(TRUE).direct_listeners(link_target).play()

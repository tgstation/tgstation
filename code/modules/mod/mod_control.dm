...
/obj/item/mod/control/allow_attack_hand_drop(mob/user)
	...
	for(var/obj/item/part as anything in get_parts())
		if(part.loc != src)
			...
			PLAYSOUND(src, 'sound/machines/scanner/scanbuzz.ogg').volume(25).vary(FALSE).range(SOUND_RANGE + SILENCED_SOUND_EXTRARANGE).play()
			return FALSE
...
/obj/item/mod/control/tool_act(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(istype(tool, /obj/item/mod/core))
		...
		PLAYSOUND(src, 'sound/machines/click.ogg').volume(50).vary(TRUE).range(SOUND_RANGE + SILENCED_SOUND_EXTRARANGE).play()
		return ITEM_INTERACT_SUCCESS
	if(open)

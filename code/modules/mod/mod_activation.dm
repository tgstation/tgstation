...
/obj/item/mod/control/proc/toggle_activate(mob/user, force_deactivate = FALSE)
	...
	if(do_after(wearer, activation_step_time, wearer, MOD_ACTIVATION_STEP_FLAGS, extra_checks = CALLBACK(src, PROC_REF(get_wearer)), hidden = TRUE))
		...
		if(active)
			create_sound(src, 'sound/machines/synth/synth_yes.ogg').vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).frequency(6000).play()
			if(!malfunctioning)
				create_sound(get_turf(src), 'sound/vehicles/mecha/nominal.ogg').direct_listeners(wearer).play()
		else
			create_sound(src, 'sound/machines/synth/synth_no.ogg').vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).frequency(6000).play()
	activating = FALSE

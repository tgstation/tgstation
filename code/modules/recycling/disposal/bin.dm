...
/obj/machinery/disposal/bin/update_overlays()
	...
	if(contents.len > 0)
		. += "dispover-full"
		. += mutable_appearance(icon, "dispover-full", EMISSIVE_LAYER, EMISSIVE_PLANE, dir, alpha)

	//charging and ready light
	if(pressure_charging)
		. += "dispover-charge"
		. += mutable_appearance(icon, "dispover-charge-glow", EMISSIVE_LAYER, EMISSIVE_PLANE, dir, alpha)
	else if(full_pressure)
		. += "dispover-ready"
		. += mutable_appearance(icon, "dispover-ready-glow", EMISSIVE_LAYER, EMISSIVE_PLANE, dir, alpha)

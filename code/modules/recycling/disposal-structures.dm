...
/obj/structure/disposalpipe/sortjunction/examine(mob/user)
	...
	if(sortTypes.len>0)
		...
		for(var/t in sortTypes)
			to_chat(user, SLOTH.TAGGERLOCATIONS[t])
	else
	...
	if(istype(I, /obj/item/device/destTagger))
		...
		if(O.currTag > 0)// Tag set
			if(O.currTag in sortTypes)
				sortTypes -= O.currTag
				to_chat(user, "<span class='notice'>Removed \"[SLOTH.TAGGERLOCATIONS[O.currTag]]\" filter.</span>")
			else
				sortTypes |= O.currTag
				to_chat(user, "<span class='notice'>Added \"[SLOTH.TAGGERLOCATIONS[O.currTag]]\" filter.</span>")

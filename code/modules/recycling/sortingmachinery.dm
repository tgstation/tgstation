...
/obj/item/delivery/proc/post_unwrap_contents(mob/user, rip_open = TRUE)
	...
	if(rip_open)
		playsound(loc, 'sound/items/poster/poster_ripped.ogg').vary(TRUE).play()
		new /obj/effect/decal/cleanable/wrapping(turf_loc)
	else
		playsound(loc, 'sound/items/box_cut.ogg').vary(TRUE).play()
...
/obj/item/delivery/attackby(obj/item/item, mob/user, params)
	if(istype(item, /obj/item/dest_tagger))
		...
		if(sort_tag != dest_tagger.currTag)
			...
			playsound(loc, 'sound/machines/beep/twobeep_high.ogg').volume(100).vary(TRUE).play()
			update_appearance()
...
/obj/item/dest_tagger/suicide_act(mob/living/user)
	...
	playsound(src, 'sound/machines/beep/twobeep_high.ogg').volume(100).vary(TRUE).play()
	return BRUTELOSS
	...
	if(isidcard(item))
		...
		if(potential_acc.registered_account)
			if(payments_acc == potential_acc.registered_account)
				...
				playsound(src, 'sound/machines/ping.ogg').volume(40).vary(TRUE).play()
...
/obj/item/sales_tagger/attack_self(mob/user)
	...
	if(!payments_acc)
		...
		return
	paper_count -= 1
	playsound(src, 'sound/machines/click.ogg').volume(40).vary(TRUE).play()

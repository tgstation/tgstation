...
/obj/structure/bigDelivery/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/device/destTagger))
		...
		if(sortTag != O.currTag)
			var/tag = uppertext(SLOTH.TAGGERLOCATIONS[O.currTag])
...
/obj/item/device/destTagger/proc/openwindow(mob/user)
	...
	for (var/i = 1, i <= SLOTH.TAGGERLOCATIONS.len, i++)
		dat += "<td><a href='?src=\ref[src];nextTag=[i]'>[SLOTH.TAGGERLOCATIONS[i]]</a></td>"

		if(i%4==0)
			dat += "</tr><tr>"

	dat += "</tr></table><br>Current Selection: [currTag ? SLOTH.TAGGERLOCATIONS[currTag] : "None"]</tt>"

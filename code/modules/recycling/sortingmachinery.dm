...
/obj/item/delivery/attackby(obj/item/item, mob/user, params)
	if(istype(item, /obj/item/dest_tagger))
		...
		if(sort_tag != dest_tagger.currTag)
			...
			PLAYSOUND(loc, 'sound/machines/beep/twobeep_high.ogg').volume(100).vary_frequency(TRUE).play()
			update_appearance()
...
/obj/item/dest_tagger/suicide_act(mob/living/user)
	...
	PLAYSOUND(src, 'sound/machines/beep/twobeep_high.ogg').volume(100).vary_frequency(TRUE).play()
	return BRUTELOSS
	...
	if(isidcard(item))
		...
		if(potential_acc.registered_account)
			if(payments_acc == potential_acc.registered_account)
				...
				PLAYSOUND(src, 'sound/machines/ping.ogg').volume(40).vary_frequency(TRUE).play()
...
/obj/item/sales_tagger/attack_self(mob/user)
	...
	if(!payments_acc)
		...
		return
	paper_count -= 1
	PLAYSOUND(src, 'sound/machines/click.ogg').volume(40).vary_frequency(TRUE).play()

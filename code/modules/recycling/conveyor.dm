...
/obj/machinery/conveyor_switch/proc/update_position(direction)
	if(position == CONVEYOR_OFF)
		playsound(src, 'sound/machines/lever/lever_start.ogg').volume(40).vary(TRUE).play()
		...
		playsound(src, 'sound/machines/lever/lever_stop.ogg').volume(40).vary(TRUE).play()
...
/obj/item/stack/conveyor/use(used, transfer, check)
	. = ..()
	playsound(src, 'sound/items/weapons/genhit.ogg').volume(30).vary(TRUE).play()

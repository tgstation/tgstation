...
/obj/machinery/conveyor_switch/proc/update_position(direction)
	if(position == CONVEYOR_OFF)
		PLAYSOUND(src, 'sound/machines/lever/lever_start.ogg').volume(40).vary_frequency(TRUE).play()
		...
		PLAYSOUND(src, 'sound/machines/lever/lever_stop.ogg').volume(40).vary_frequency(TRUE).play()
...
/obj/item/stack/conveyor/use(used, transfer, check)
	. = ..()
	PLAYSOUND(src, 'sound/items/weapons/genhit.ogg').volume(30).vary_frequency(TRUE).play()

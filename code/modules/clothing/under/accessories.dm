				...
				if(U.attach_accessory(src, user, 0)) //Attach it, do not notify the user of the attachment
					if(user == M)
						...
						if(input)
							...
							add_memory_in_range(M, 7, MEMORY_RECEIVED_MEDAL, list(DETAIL_PROTAGONIST = M, DETAIL_MEDAL_TYPE = src, DETAIL_DEUTORAGONIST = user, DETAIL_MEDAL_REASON = input), STORY_VALUE_AMAZING)

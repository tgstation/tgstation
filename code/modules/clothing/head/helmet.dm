...
/obj/item/clothing/head/helmet/monkey_sentience/equipped(mob/user, slot)
	...
	polling = TRUE
	var/list/candidates = pollCandidatesForMob("Do you want to play as a mind magnified monkey?", ROLE_SENTIENCE, ROLE_SENTIENCE, 50, magnification, POLL_IGNORE_SENTIENCE_GRANTER)
	polling = FALSE
	if(!magnification)
		return
	if(!candidates.len)

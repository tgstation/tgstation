...
/obj/item/clothing/head/helmet/monkey_sentience/equipped(mob/user, slot)
	...
	polling = TRUE
	var/list/candidates = poll_candidates_for_mob("Do you want to play as a mind magnified monkey?", ROLE_MONKEY_HELMET, null, 5 SECONDS, magnification, POLL_IGNORE_SENTIENCE_POTION)
	polling = FALSE
	if(!magnification)
		return
	if(!candidates.len)

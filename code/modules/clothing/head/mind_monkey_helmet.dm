	...
	if(isnull(chosen_one))
		...
		switch(rage_chance)
			...
			if(7 to 13)
				...
				PLAYSOUND(get_sfx(src), SFX_SPARKS).volume(30).vary_frequency(TRUE).play()
			...
			if(14 to 21)
				...
				PLAYSOUND(get_sfx(src), SFX_SPARKS).volume(15).vary_frequency(TRUE).play()
				PLAYSOUND(get_sfx(src), SFX_SHATTER).volume(30).vary_frequency(TRUE).play()

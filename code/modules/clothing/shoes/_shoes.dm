...
/obj/item/clothing/shoes/proc/handle_tying(mob/user)
	...
	if(user == loc && tied != SHOES_TIED) // if they're our own shoes, go tie-wards
		...
		if(do_after(user, our_guy, lace_time, extra_checks = CALLBACK(src, .proc/still_shoed, our_guy)))
...
/obj/item/clothing/shoes/attack_self(mob/user)
	...
	if(do_after(user, src, lace_time, extra_checks = CALLBACK(src, .proc/still_shoed, user)))

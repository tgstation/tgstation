...
/obj/item/clothing/shoes/proc/handle_tying(mob/user)
	...
	if(user == loc && tied != SHOES_TIED) // if they're our own shoes, go tie-wards
		...
		if(do_after(user, mod_time, target = our_guy, extra_checks = CALLBACK(src, .proc/still_shoed, our_guy)))
			...
			user.emote("scream")
			if(istype(L))
				var/obj/item/bodypart/ouchie = L.get_bodypart(L.get_random_valid_zone(blacklisted_parts = list(BODY_ZONE_CHEST, BODY_ZONE_HEAD, BODY_ZONE_L_LEG, BODY_ZONE_R_LEG),even_weights = TRUE))
				if(ouchie)

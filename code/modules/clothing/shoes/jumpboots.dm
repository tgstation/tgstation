...
/obj/item/clothing/shoes/bhop/ui_action_click(mob/user, action)
	...
	ADD_TRAIT(user, TRAIT_MOVE_FLOATING, SOURCE_LEAPING)  //Throwing itself doesn't protect mobs against lava (because gulag).
	if (user.throw_at(target, jumpdistance, jumpspeed, spin = FALSE, diagonals_first = TRUE, callback = TRAIT_CALLBACK_REMOVE(user, TRAIT_MOVE_FLOATING, SOURCE_LEAPING)))

...
/obj/item/wizard_armor_charge/afterattack(obj/item/clothing/suit/space/hardsuit/shielded/wizard/W, mob/user, proximity)
	. = ..()
	if(!proximity)
		return
	if(!istype(W))
		to_chat(user, "<span class='warning'>The rune can only be used on battlemage armor!</span>")
		return
	W.current_charges += 8

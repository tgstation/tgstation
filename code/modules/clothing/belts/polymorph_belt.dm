...
/obj/item/polymorph_belt/attackby(obj/item/weapon, mob/user, params)
	...
	PLAYSOUND(src, 'sound/machines/crate/crate_open.ogg').vary_frequency(FALSE).play()
...
/datum/action/cooldown/spell/shapeshift/polymorph_belt/before_cast(mob/living/cast_on)
	...
	PLAYSOUND(cast_on, 'sound/effects/wounds/crack1.ogg').play()
	...
	PLAYSOUND(cast_on, 'sound/effects/magic/demon_consume.ogg').vary_frequency(TRUE).play()

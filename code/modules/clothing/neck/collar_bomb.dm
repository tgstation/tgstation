...
/obj/item/clothing/neck/collar_bomb/proc/explosive_countdown(ticks_left)
	active = TRUE
	if(ticks_left > 0)
		PLAYSOUND(src, 'sound/items/timer.ogg').volume(30).vary_frequency(FALSE).play()
	...
	PLAYSOUND(get_sfx(src), SFX_EXPLOSION).volume(30).vary_frequency(TRUE).play()
...
/obj/item/collar_bomb_button/attack_self(mob/user)
	...
	if(!do_after(user, 1.2 SECONDS, target = src))
		return
	PLAYSOUND(user, 'sound/machines/click.ogg').volume(25).vary_frequency(TRUE).play()

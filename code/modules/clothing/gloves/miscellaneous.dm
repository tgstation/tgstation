...
/obj/item/clothing/gloves/yellow/stun/Touch(atom/A)
	...
	if(istype(A, /obj/item/weapon/stock_parts/cell))
		...
		if(tcell.charge && (cell.maxcharge != cell.charge))
			...
			if(do_after(user,50))
				var/useamt = min((cell.maxcharge-cell.charge), min(tcell.charge, 5000)) //recharge at most 5k per cycle (one stun)
				if(tcell.use(useamt))
					cell.give(useamt)
				user << "<span class='notice'>The vibration suddenly subsides.</span>"
			else
				user << "<span class='notice'>As your hands move away from \the [tcell], the vibration slowly subsides.</span>"
			playsound(loc, "sparks", 75, 1, -1)
			spark_system.start()
		busy = 0
		return 1

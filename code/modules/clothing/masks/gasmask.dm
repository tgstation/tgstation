...
/obj/item/clothing/mask/fakemoustache
	...
	flags_inv = HIDEFACE
	resistance_flags = FIRE_PROOF
	armor_type = /datum/armor/fakemoustache
	var/obj/item/clothing/mask/cigarette/cig

/obj/item/clothing/mask/fakemoustache/worn_overlays(mutable_appearance/standing, isinhands)
	. = ..()
	if(!isinhands && cig)
		. += cig.build_worn_icon(default_layer = FACEMASK_LAYER, default_icon_file = 'icons/mob/clothing/mask.dmi')

/obj/item/clothing/mask/fakemoustache/equipped(mob/equipee, slot)
	cig?.equipped(equipee, slot)
	return ..()

/obj/item/clothing/mask/fakemoustache/examine(mob/user)
	. = ..()
	if(cig)
		. += span_notice("There is a [cig.name] jammed into the filter slot.")

/obj/item/clothing/mask/fakemoustache/Exited(atom/movable/gone)
	. = ..()
	if(gone == cig)
		cig = null
		if(ismob(loc))
...
/obj/item/clothing/mask/fakemoustache/attackby(obj/item/tool, mob/user)
	var/valid_wearer = ismob(loc)
	var/mob/wearer = loc
	if(istype(tool, /obj/item/clothing/mask/cigarette))
		if(flags_cover & MASKCOVERSMOUTH)
...
/obj/item/clothing/mask/fakemoustache/attack_hand_secondary(mob/user, list/modifiers)
	if(cig)
		user.put_in_hands(cig)
		cig = null
		if(ismob(loc))

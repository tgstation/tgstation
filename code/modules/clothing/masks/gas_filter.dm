...
/obj/item/gas_filter/proc/reduce_filter_status(datum/gas_mixture/breath)
	...
	for(var/gas_id in breath.moles)
		if(gas_id in high_filtering_gases)
			if(breath.moles[gas_id] > HIGH_FILTERING_MOLES)
				breath.set_gas(gas_id, max(breath.moles[gas_id] - filter_strength_high * filter_efficiency * HIGH_FILTERING_RATIO, 0))
				danger_points += 1
				continue
			breath.set_gas(gas_id, max(breath.moles[gas_id] - filter_strength_high * filter_efficiency * LOW_FILTERING_RATIO, 0))
			danger_points += 0.2
			continue
		if(gas_id in mid_filtering_gases)
			if(breath.moles[gas_id] > MID_FILTERING_MOLES)
				breath.set_gas(gas_id, max(breath.moles[gas_id] - filter_strength_mid * filter_efficiency * HIGH_FILTERING_RATIO, 0))
				danger_points += 1.25
				continue
			breath.set_gas(gas_id, max(breath.moles[gas_id] - filter_strength_mid * filter_efficiency * LOW_FILTERING_RATIO, 0))
			danger_points += 0.25
			continue
		if(gas_id in low_filtering_gases)
			if(breath.moles[gas_id] > LOW_FILTERING_MOLES)
				breath.set_gas(gas_id, max(breath.moles[gas_id] - filter_strength_low * filter_efficiency * HIGH_FILTERING_RATIO, 0))
				danger_points += 1.5
				continue
			breath.set_gas(gas_id, max(breath.moles[gas_id] - filter_strength_low * filter_efficiency * LOW_FILTERING_RATIO, 0))
			danger_points += 0.5
			continue

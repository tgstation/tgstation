...
/obj/item/clothing/mask/fakemoustache/syndicate
	name = "fake moustache"
	desc = "Warning: moustache is fake."
	icon_state = "fake-moustache"
...
/obj/item/clothing/mask/fakemoustache/syndicate/worn_overlays(mutable_appearance/standing, isinhands)
	. = ..()
	if(!isinhands && cig)
		. += cig.build_worn_icon(default_layer = FACEMASK_LAYER, default_icon_file = 'icons/mob/clothing/mask.dmi')

/obj/item/clothing/mask/fakemoustache/syndicate/equipped(mob/equipee, slot)
	cig?.equipped(equipee, slot)
	return ..()

/obj/item/clothing/mask/fakemoustache/syndicate/examine(mob/user)
	. = ..()
	if(cig)
		. += span_notice("There is a [cig.name] jammed into the filter slot.")

/obj/item/clothing/mask/fakemoustache/syndicate/Exited(atom/movable/gone)
	. = ..()
	if(gone == cig)
		cig = null
		if(ismob(loc))
...
/obj/item/clothing/mask/fakemoustache/syndicate/attackby(obj/item/tool, mob/user)
...
/obj/item/clothing/mask/fakemoustache/syndicate/attack_hand_secondary(mob/user, list/modifiers)
	if(cig)
...
/obj/item/clothing/mask/fakemoustache/syndicate/attack_self(mob/user)

...
/obj/item/clothing/mask/gas/sechailer/proc/play_phrase(mob/user, datum/hailer_phrase/phrase)
	...
	playsound(src, "sound/runtime/complionator/[initial(phrase.phrase_sound)].ogg").volume(100).extra_range(4).play()
	return TRUE
...
/obj/item/clothing/mask/whistle/ui_action_click(mob/user, action)
	...
	playsound(src, 'sound/items/whistle/whistle.ogg').extra_range(4).play()
...
/obj/item/clothing/mask/party_horn/ui_action_click(mob/user, action)
	...
	playsound(src, 'sound/items/party_horn.ogg').volume(75).play()

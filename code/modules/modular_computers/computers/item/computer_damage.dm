/obj/item/modular_computer/take_damage(damage_amount, damage_type = BRUTE, damage_flag = ZERO, sound_effect = 1)
	. = ..()
	...
	if(component_probability)
		for(var/I in all_components)
			...
			if(prob(component_probability))
				H.take_damage(round(damage_amount*0.5), damage_type, damage_flag, ZERO)
...
/obj/item/modular_computer/proc/break_apart()
	if(!(flags_1 & NODECONSTRUCT_1))
		...
		for(var/C in all_components)
			...
			H.forceMove(newloc)
			if(prob(25))
				H.take_damage(rand(10,30), BRUTE, ZERO, ZERO)
	relay_qdel()
	qdel(src)

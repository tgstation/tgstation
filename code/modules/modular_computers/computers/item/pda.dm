...
/obj/item/modular_computer/pda/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(inserted_item)
		...
		inserted_item = tool
		PLAYSOUND(src, 'sound/machines/pda_button/pda_button1.ogg').volume(50).vary_frequency(TRUE).play()
...
/obj/item/modular_computer/pda/proc/remove_pen(mob/user)
	...
	if(inserted_item)
		...
		inserted_item = null
		update_appearance()
		PLAYSOUND(src, 'sound/machines/pda_button/pda_button2.ogg').volume(50).vary_frequency(TRUE).play()

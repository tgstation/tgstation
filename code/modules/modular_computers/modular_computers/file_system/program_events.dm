...
/datum/computer_file/program/proc/event_networkfailure(background)
	kill_program(forced = TRUE)
	if(background)
		computer.visible_message(span_danger("\The [physical]'s screen displays a \"Process [filename].[filetype] (PID [rand(100,999)]) terminated - Network Error\" error"))
	else
		computer.visible_message(span_danger("\The [physical]'s screen briefly freezes and then shows \"NETWORK ERROR - NTNet connection lost. Please retry. If problem persists contact your system administrator.\" error."))

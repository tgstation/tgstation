...
/datum/computer_file/program/robocontrol/ui_data(mob/user)
	...
	if(computer)
		data["id_owner"] = computer.inserted_id || ""
...
/datum/computer_file/program/robocontrol/ui_act(action, list/params, datum/tgui/ui)
	...
	var/obj/item/card/id/id_card = computer?.inserted_id
	...
	switch(action)
		...
		if("ejectcard")
			if(!computer || !computer.inserted_id)
				return
			if(id_card)
		...
		if("changedroneaccess")
			if(!computer || !computer.inserted_id || !id_card)

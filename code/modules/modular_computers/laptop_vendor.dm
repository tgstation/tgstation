...
/obj/machinery/lapvend/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/stack/spacecash))
		...
		if(!user.temporarilyRemoveItemFromInventory(c))
			return
		credits += c.value
		visible_message("<span class='info'><span class='name'>[user]</span> inserts [c.value]Ꞥ into [src].</span>")
		qdel(c)
		return
	else if(istype(I, /obj/item/holochip))
		var/obj/item/holochip/HC = I
		credits += HC.credits
		visible_message("<span class='info'>[user] inserts a [HC.credits]Ꞥ holocredit chip into [src].</span>")
		qdel(HC)
		return
		...
		say("[target_credits]Ꞥ has been desposited from your account.")
		return
	return ..()

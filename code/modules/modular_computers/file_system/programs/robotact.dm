/datum/computer_file/program/robotact
	...
	undeletable = TRUE
	usage_flags = PROGRAM_SILICON
	size = 5
	tgui_id = "NtosRobotact"
	program_icon = "terminal"

/datum/computer_file/program/robotact/Destroy(force)
	. = ..()
	var/datum/modular_computer_host/silicon/cyborg/holder = computer
	holder.robotact = null
...
/datum/computer_file/program/robotact/on_start(mob/living/user)
	...
	. = ..()
	if(.)
		if(computer.device_theme == "syndicate")
...
/datum/computer_file/program/robotact/ui_data(mob/user)
	...
	var/datum/modular_computer_host/silicon/cyborg/tablet = computer

	var/mob/living/silicon/robot/cyborg = tablet.physical
	...
	// Chassis integrity
	data["integrity"] = ((cyborg.health + 100) / 2)

	// Headlights
	data["lampIntensity"] = cyborg.lamp_intensity

	data["sensors"] = (cyborg.sensors_on ? "ACTIVE" : "DISABLED")

	// Number of pictures taken, synced to AI if available
	data["printerPictures"] = (cyborg.connected_ai ? cyborg.connected_ai.aicamera.stored.len : cyborg.aicamera.stored.len)

	// Printer toner
	data["printerToner"] = cyborg.toner
	data["printerTonerMax"] = cyborg.tonermax

	// Thrusters
	data["thrustersInstalled"] = cyborg.ionpulse
	data["thrustersStatus"] = (cyborg.ionpulse_on ? "ACTIVE" : "DISABLED")
	...
	switch(action)
		if("coverunlock")
			if(cyborg.locked)
				...
				cyborg.update_icons()
				if(cyborg.emagged)
					cyborg.logevent("##CGÃSS1S## COV3R L0CK [cyborg.locked ? "#--3NGAÃGED--#" : "#!REL3Ä1SD-!#"]") //"The cover interface glitches out for a split second"
				else
...
/datum/computer_file/program/robotact/proc/force_full_update()
	if(!istype(computer, /datum/modular_computer_host/silicon/cyborg))
		return
	var/datum/modular_computer_host/silicon/cyborg/tablet = computer
	var/datum/tgui/active_ui = SStgui.get_open_ui(tablet.physical, src)
	if(active_ui)

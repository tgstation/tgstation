...
/datum/computer_file/program/card_mod/ui_act(action, params)
	...
	var/mob/user = usr
	var/obj/item/card/id/inserted_auth_card = computer.inserted_id

	switch(action)
		// Log in.
		if("PRG_authenticate")
			if(!computer || !inserted_auth_card)
				playsound(computer.physical, 'sound/machines/terminal_prompt_deny.ogg', 50, FALSE)
				return TRUE
			if(authenticate(user, inserted_auth_card))
				playsound(computer.physical, 'sound/machines/terminal_on.ogg', 50, FALSE)
				return TRUE
		// Log out.
		if("PRG_logout")
			authenticated_card = null
			authenticated_user = null
			playsound(computer.physical, 'sound/machines/terminal_off.ogg', 50, FALSE)
			return TRUE
		// Print a report.
		if("PRG_print")
			if(!computer)
			...
			if(!computer.print_text(contents, "access report - [inserted_auth_card.registered_name ? inserted_auth_card.registered_name : "Unregistered"]"))
				...
				playsound(computer.physical, 'sound/machines/terminal_on.ogg', 50, FALSE)
				computer.visible_message(span_notice("\The [computer.physical] prints out a sheet of paper."))
			return TRUE
		// Eject the ID used to log on to the ID app.
		if("PRG_ejectauthid")
			if(inserted_auth_card)
				return computer.remove_card(usr)
			else
				var/obj/item/I = user.get_active_held_item()
				if(isidcard(I))
					return computer.insert_card(I, user)
		...
		if("PRG_edit")
			...
			if(!new_name)
				...
				playsound(computer.physical, SFX_TERMINAL_TYPE, 50, FALSE)
		...
		if("PRG_template")
			if(!computer.physical || !authenticated_card || !inserted_auth_card)
				return TRUE
...
/datum/computer_file/program/card_mod/ui_data(mob/user)
	...
	var/obj/item/card/id/inserted_id = computer.inserted_id

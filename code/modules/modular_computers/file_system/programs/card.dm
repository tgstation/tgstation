	...
	switch(action)
		...
		if("PRG_authenticate")
			if(!computer || !inserted_auth_card)
				playsound(computer, 'sound/machines/terminal/terminal_prompt_deny.ogg').play()
				return TRUE
			if(authenticate(user, inserted_auth_card))
				playsound(computer, 'sound/machines/terminal/terminal_on.ogg').play()
				return TRUE
		// Log out.
		if("PRG_logout")
			authenticated_card = null
			authenticated_user = null
			playsound(computer, 'sound/machines/terminal/terminal_off.ogg').play()
			return TRUE
		// Print a report.
		if("PRG_print")
			if(!computer)
		...
		if("PRG_edit")
			...
			if(!new_name)
				...
				playsound(get_sfx(computer), SFX_TERMINAL_TYPE).play()

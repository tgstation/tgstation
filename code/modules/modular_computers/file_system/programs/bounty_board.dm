	...
	switch(action)
		if("createBounty")
			...
			for(var/obj/i in GLOB.allbountyboards)
				...
				PLAYSOUND(i.loc, 'sound/effects/cashregister.ogg').volume(30).vary_frequency(TRUE).play()
			return TRUE
		if("apply")
			if(!current_user)
			...
			if(current_user.account_holder == active_request.owner)
				PLAYSOUND(computer, 'sound/machines/buzz/buzz-sigh.ogg').volume(20).vary_frequency(TRUE).play()
				return TRUE

...
/datum/computer_file/program/notepad/proc/find_text_file(uid, opened_on_disk, name)
	...
	if(uid)
		file = os.filesystem.find_file_by_uid(uid, target_disk)
	if(!file && length(name))
		file = os.filesystem.find_file_by_full_name("[name].TXT", target_disk)
	return file
...
/datum/computer_file/program/notepad/proc/save_text_file(mob/user, name, extension, note, uid, opened_on_disk)
	...
	var/file_stored = target_disk ? target_disk.add_file(new_file) : os.filesystem.store_file(new_file, user)
	if(!file_stored)

...
/datum/computer_file/program/contract_uplink/can_run(mob/user, loud, access_to_check, transfer, list/access)
	. = ..()
...
/datum/computer_file/program/contract_uplink/ui_act(action, params)
	...
	switch(action)
		if("PRG_contract-accept")
			...
			traitor_data.contractor_hub.assigned_contracts[contract_id].status = CONTRACT_STATUS_ACTIVE
			traitor_data.contractor_hub.current_contract = traitor_data.contractor_hub.assigned_contracts[contract_id]
		...
		if("PRG_call_extraction")
			if (traitor_data.contractor_hub.current_contract.status != CONTRACT_STATUS_EXTRACTING)
				if (traitor_data.contractor_hub.current_contract.handle_extraction(user))
					user.playsound_local(user, 'sound/effects/confirmdropoff.ogg', 100, TRUE)
					traitor_data.contractor_hub.current_contract.status = CONTRACT_STATUS_EXTRACTING
		...
		if("PRG_contract_abort")
			var/contract_id = traitor_data.contractor_hub.current_contract.id

			traitor_data.contractor_hub.current_contract = null
			traitor_data.contractor_hub.assigned_contracts[contract_id].status = CONTRACT_STATUS_ABORTED

			program_icon_state = "contracts"

			return TRUE
		if("PRG_redeem_TC")
			if (traitor_data.contractor_hub.contract_TC_to_redeem)
				var/obj/item/stack/telecrystal/crystals = new /obj/item/stack/telecrystal(get_turf(user),
															traitor_data.contractor_hub.contract_TC_to_redeem)
				if(ishuman(user))
				...
				traitor_data.contractor_hub.contract_TC_payed_out += traitor_data.contractor_hub.contract_TC_to_redeem
				traitor_data.contractor_hub.contract_TC_to_redeem = 0
				return TRUE
			else
				user.playsound_local(user, 'sound/machines/uplinkerror.ogg', 50)
			return TRUE

	...
	switch(action)
		...
		if("PRG_login")
			...
			if(!traitor_data.uplink_handler.contractor_hub)
				...
				playsound(user, 'sound/music/antag/contractstartup.ogg').volume(100).direct_listeners(user).play()
		...
		if("PRG_call_extraction")
			if (traitor_data.uplink_handler.contractor_hub.current_contract.status != CONTRACT_STATUS_EXTRACTING)
				if (traitor_data.uplink_handler.contractor_hub.current_contract.handle_extraction(user))
					playsound(user, 'sound/effects/confirmdropoff.ogg').volume(100).vary(TRUE).direct_listeners(user).play()
					...
					playsound(user, 'sound/machines/uplink/uplinkerror.ogg').direct_listeners(user).play()

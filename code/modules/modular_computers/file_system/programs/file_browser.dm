...
/datum/computer_file/program/filemanager/ui_act(action, params)
	...
	switch(action)
		...
		if("PRG_edit")
			...
			if(!F || !istype(F))
				return 1
			if(F.do_not_edit && (wrap_alert(usr, "WARNING: This file is not compatible with editor. Editing it may result in permanently corrupted formatting or damaged data consistency. Edit anyway?", "Incompatible File", "No", "Yes") == "No"))
				return 1

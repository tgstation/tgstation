...
/datum/computer_file/program/virtual_pet/proc/on_photo_captured(datum/source, atom/target, atom/user, datum/picture/photo)
	...
	if(isnull(photo))
		return
	os.filesystem.store_file(new /datum/computer_file/image(photo.picture_image, display_name = photo.picture_name))
...
/datum/computer_file/program/virtual_pet/proc/remove_pet(datum/source)
	...
	if(QDELETED(src))
		return
	os.filesystem.remove_file(src) //all is lost we no longer have a reason to exist

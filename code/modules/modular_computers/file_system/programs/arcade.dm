...
/datum/computer_file/program/arcade/proc/game_check(mob/user)
	...
	if(boss_hp <= 0)
		...
		playsound(computer.physical.loc, 'sound/arcade/win.ogg', 50)
		game_active = FALSE
		...
		playsound(computer.physical.loc, 'sound/arcade/lose.ogg', 50)
		game_active = FALSE
		program_icon_state = "arcade_off"
		if(istype(computer))
			computer.physical.update_appearance()
...
/datum/computer_file/program/arcade/proc/enemy_check(mob/user)
	...
	if (boss_mp <= 5)
		...
		playsound(computer.physical.loc, 'sound/arcade/steal.ogg', 50, TRUE)
		player_mp -= boss_mpamt
		boss_mp += boss_mpamt
	else if(boss_mp > 5 && boss_hp <12)
		heads_up = "[boss_name] heals for [bossheal] health!"
		playsound(computer.physical.loc, 'sound/arcade/heal.ogg', 50, TRUE)
		boss_hp += bossheal
		boss_mp -= boss_mpamt
	else
		heads_up = "[boss_name] attacks you for [boss_attackamt] damage!"
		playsound(computer.physical.loc, 'sound/arcade/hit.ogg', 50, TRUE)
...
/datum/computer_file/program/arcade/ui_act(action, list/params)
	...
	switch(action)
		...
		if("Recharge_Power")
			...
			playsound(computer.physical.loc, 'sound/arcade/mana.ogg', 50, TRUE)
		...
		if("Dispense_Tickets")
			if(computer.stored_paper <= 0)
				...
				computer.physical.visible_message(span_notice("\The [computer] prints out paper."))
				if(ticket_count >= 1)

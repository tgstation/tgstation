...
/datum/computer_file/program/arcade/proc/enemy_check(mob/user)
	...
	if (boss_mp <= 5)
		...
		create_sound(computer.loc, 'sound/machines/arcade/steal.ogg').vary(TRUE).play()
		player_mp -= boss_mpamt
		boss_mp += boss_mpamt
	else if(boss_mp > 5 && boss_hp <12)
		heads_up = "[boss_name] heals for [bossheal] health!"
		create_sound(computer.loc, 'sound/machines/arcade/heal.ogg').vary(TRUE).play()
		boss_hp += bossheal
		boss_mp -= boss_mpamt
	else
		heads_up = "[boss_name] attacks you for [boss_attackamt] damage!"
		create_sound(computer.loc, 'sound/machines/arcade/hit.ogg').vary(TRUE).play()
...
/datum/computer_file/program/arcade/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("Recharge_Power")
			...
			create_sound(computer.loc, 'sound/machines/arcade/mana.ogg').vary(TRUE).play()

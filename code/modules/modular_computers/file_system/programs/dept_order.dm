...
/datum/computer_file/program/department_order/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	if(action == "link")
		...
		if(isnull(new_dept_type))
			...
			PLAYSOUND(computer, 'sound/machines/buzz/buzz-sigh.ogg').volume(30).vary_frequency(TRUE).play()
		else
			computer.physical.balloon_alert(orderer, "linked")
			PLAYSOUND(computer, 'sound/machines/ping.ogg').volume(30).vary_frequency(TRUE).play()

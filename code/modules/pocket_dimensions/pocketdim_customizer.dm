...
/datum/pocket_dim_customizer/proc/strip_pocket_dimension(turf/newbaseturf)
	...
	switch(action)
		...
		if("change_size")
			...
			if(inputed)
				size = inputed
		if("input_baseturf")
			var/inputed = input(last_user, "Enter new baseturf" , "Baseturf")
			if(ispath(inputed))
				baseturf = inputed
		if("collapse_room")
			strip_pocket_dimension(baseturf)
		...
		if("relink_event")
			reregister_events()
		if("input_wall")
			var/inputed = input(last_user, "Enter new wall" , "wall")
			if(istype(inputed,/turf))
				wall = inputed
		if("input_floor")
			var/inputed = input(last_user, "Enter new floor" , "floor")
			if(istype(inputed,/turf))
				floor = inputed
		if("create_portals")
			spawn_portals()

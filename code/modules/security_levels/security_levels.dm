/proc/set_security_level(level)
	...
	if(level >= SEC_LEVEL_GREEN && level <= SEC_LEVEL_DELTA && level != SSsecurity_level.get_current_level(TRUE))
		switch(level)
			if(SEC_LEVEL_GREEN)
			...
			if(SEC_LEVEL_RED)
				if(SSsecurity_level.get_current_level(TRUE) < SEC_LEVEL_RED)
					...
					if(SSshuttle.emergency.mode == SHUTTLE_CALL || SSshuttle.emergency.mode == SHUTTLE_RECALL)
						if(SSsecurity_level.get_current_level(TRUE) == SEC_LEVEL_GREEN)
			...
			if(SEC_LEVEL_DELTA)
				...
				if(SSshuttle.emergency.mode == SHUTTLE_CALL || SSshuttle.emergency.mode == SHUTTLE_RECALL)
					if(SSsecurity_level.get_current_level(TRUE) == SEC_LEVEL_GREEN)
						SSshuttle.emergency.modTimer(0.25)
					else if(SSsecurity_level.get_current_level(TRUE) == SEC_LEVEL_BLUE)
						SSshuttle.emergency.modTimer(0.5)

		SSsecurity_level.set_level(level)

/proc/get_security_level()
	switch(SSsecurity_level.get_current_level(TRUE))
		if(SEC_LEVEL_GREEN)
			return "green"
		if(SEC_LEVEL_BLUE)
			return "blue"

...
/obj/machinery/keycard_auth/proc/trigger_event(confirmer)
	...
	deadchat_broadcast("<span class='name'>[triggerer]</span>", " triggered [event] at <span class='name'>[A1.name]</span>.", triggerer)

	var/area/A2 = get_area(confirmer)
	deadchat_broadcast("<span class='name'>[confirmer]</span>", " confirmed [event] at <span class='name'>[A2.name]</span>.", confirmer)
	switch(event)

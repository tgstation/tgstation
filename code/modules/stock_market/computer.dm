...
a.updated {
	...
	dat += "<span class='user'>Welcome, <b>[logged_in]</b></span><br><span class='balance'><b>Spesos:</b> [balance()] </span><br>"
...
/obj/machinery/computer/stockexchange/proc/sell_some_shares(var/datum/stock/S, var/mob/user)
	...
	if (!S.sellShares(logged_in, amt))
		...
		return
	to_chat(user, "<span class='notice'>Sold [amt] shares of [S.name] at [S.current_value] a share for [total] Spesos.</span>")
...
/obj/machinery/computer/stockexchange/proc/buy_some_shares(var/datum/stock/S, var/mob/user)
	...
	var/total = amt * S.current_value
	to_chat(user, "<span class='notice'>Bought [amt] shares of [S.name] at [S.current_value] a share for [total] Spesos.</span>")
	...
	if (href_list["show_logs"])
		...
		for(var/D in GLOB.stockExchange.logs)
			...
			if(istype(L, /datum/stock_log/buy))
				dat += "[L.time] | <b>[L.user_name]</b> bought <b>[L.stocks]</b> stocks at [L.shareprice] a share for <b>[L.money]</b> total Spesos in <b>[L.company_name]</b>.<br>"
				continue
			if(istype(L, /datum/stock_log/sell))
				dat += "[L.time] | <b>[L.user_name]</b> sold <b>[L.stocks]</b> stocks at [L.shareprice] a share for <b>[L.money]</b> total Spesos from <b>[L.company_name]</b>.<br>"
				continue
			if(istype(L, /datum/stock_log/borrow))
				dat += "[L.time] | <b>[L.user_name]</b> borrowed <b>[L.stocks]</b> stocks with a deposit of <b>[L.money]</b> Spesos in <b>[L.company_name]</b>.<br>"
				continue

...
/datum/stockEvent/product/New(var/datum/stock/S)
	company = S
	var/mins = SSrng.random(5*TIME_MULTIPLIER,20*TIME_MULTIPLIER)
...
/datum/stockEvent/product/transition()
	...
	switch (phase_id)
		if (0)
			next_phase = world.time + SSrng.random(300*TIME_MULTIPLIER, 600*TIME_MULTIPLIER) * (10*TIME_MULTIPLIER)
			...
			product_article = A
			effect = A.opinion + SSrng.random(-1, 1)
...
/datum/stockEvent/bankruptcy/New(var/datum/stock/S)
	hidden = 1
	company = S
	var/mins = SSrng.random(9*TIME_MULTIPLIER,60*TIME_MULTIPLIER)
	bailout_millions = SSrng.random(70, 190)
...
/datum/stockEvent/bankruptcy/transition()
	switch (phase_id)
		if (0)
			...
			if (!A.opinion)
				effect = SSrng.random(5) * (SSrng.probability(50) ? -1 : 1)
			else
				effect = SSrng.probability(25) ? -A.opinion * SSrng.random(8) : A.opinion * SSrng.random(4)
			company.addArticle(A)
			company.affectPublicOpinion(SSrng.random(-6, -3))
			hidden = 0
		...
		if (1)
			next_phase = world.time + SSrng.random(300*TIME_MULTIPLIER, 600*TIME_MULTIPLIER) * (10*TIME_MULTIPLIER)
			finished = 1
			if (effect <= -5 && SSrng.probability(10))
			...
			var/bailout = (effect > 0 && SSrng.probability(80)) || (effect < 0 && SSrng.probability(20))
...
/datum/stockEvent/bankruptcy/proc/generateBankruptcyArticle()
	...
	A.about = company
	A.headline = SSrng.pick_from_list(	"[company.name] filing for bankruptcy", \
	...
	var/article = "Another one might bite the dust: [company.current_trend > 0 ? "despite their positive trend" : "in line with their failing model"], [company.name] files for bankruptcy citing [SSrng.pick_from_list(bankrupt_reason)]. The president of %country% has been asked to bail the company out, "
	if (!A.opinion)
...
/datum/stockEvent/arrest/New(var/datum/stock/S)
	hidden = 1
	company = S
	var/mins = SSrng.random(10*TIME_MULTIPLIER, 35*TIME_MULTIPLIER)
	next_phase = mins * 600*TIME_MULTIPLIER + world.time
	current_title = ""
	current_desc = ""
	female = SSrng.probability(50)
	if (SSrng.probability(50))
		position = "C[SSrng.probability(20) ? vowel() : consonant()]O"
	else
	...
	while (SSrng.probability(60) && O.len > 2)
		var/offense = SSrng.pick_from_list(O)
		O -= offense
		offense = "[SSrng.probability(20) ? "attempted " : (SSrng.probability(20) ? "being accessory to " : null)][offense][SSrng.probability(5) ? " of the [SSrng.pick_from_list("first", "second", "third", "fourth", "fifth", "sixth")] degree" : null]"
		if (offenses == "")
			offenses = offense
		else
			offenses += ", [offense]"
	offenses += " and [SSrng.probability(20) ? "attempted " : null][SSrng.pick_from_list(O)]" // lazy
	S.addEvent(src)

/datum/stockEvent/arrest/transition()
	switch (phase_id)
		if (0)
			tname = "[female ? SSrng.pick_from_list(GLOB.first_names_female) : SSrng.pick_from_list(GLOB.first_names_male)] [SSrng.pick_from_list(GLOB.last_names)]"
			next_phase = world.time + SSrng.random(300*TIME_MULTIPLIER, 600*TIME_MULTIPLIER) * (10*TIME_MULTIPLIER)
			var/datum/article/A = generateArrestArticle()
			if (!A.opinion)
				effect = SSrng.random(5) * (SSrng.probability(50) ? -1 : 1)
			else
				effect = SSrng.probability(25) ? -A.opinion * SSrng.random(5) : A.opinion * SSrng.random(3)
			company.addArticle(A)
			company.affectPublicOpinion(SSrng.random(-3, -1))
			hidden = 0
...
/datum/stockEvent/arrest/proc/generateArrestArticle()
	var/datum/article/A = new
	A.about = company
	A.headline = company.industry.detokenize(SSrng.pick_from_list( \
	...
	if (SSrng.probability(15))
		A.opinion = SSrng.random(-1, 1)
	var/article = "[SSrng.pick_from_list("Security", "Law enforcement")] forces issued a statement that [tname], the [position] of [company.name], the %famous% %industrial% %company% was arrested %this_time%. The trial has been scheduled and the statement reports that the arrested individual is being charged with [offenses]. "
	if (!A.opinion)

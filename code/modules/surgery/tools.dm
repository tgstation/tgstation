...
/obj/item/surgicaldrill/suicide_act(mob/living/user)
	...
	playsound(user, 'sound/machines/juicer.ogg').volume(20).vary(TRUE).play()
...
/obj/item/surgical_processor/interact_with_atom(atom/design_holder, mob/living/user, list/modifiers)
	...
	playsound(src, 'sound/machines/terminal/terminal_processing.ogg').volume(25).vary(TRUE).play()
	if(do_after(user, 1 SECONDS, target = design_holder))
		...
		playsound(src, 'sound/machines/terminal/terminal_success.ogg').volume(25).vary(TRUE).play()
		downloaded = TRUE
...
/obj/item/shears/attack(mob/living/amputee, mob/living/user)
	...
	playsound(get_turf(patient), 'sound/items/tools/ratchet.ogg').volume(20).vary(TRUE).play()
	...
	if(do_after(user,  toolspeed * 15 SECONDS * amputation_speed_mod, target = patient))
		playsound(get_turf(patient), 'sound/items/weapons/bladeslice.ogg').volume(250).vary(TRUE).play()
...
/obj/item/shears/suicide_act(mob/living/carbon/user)
	...
	for(var/obj/item/bodypart/thing in user.bodyparts)
		...
		playsound(user, 'sound/items/weapons/bladeslice.ogg').volume(70).wait(timer).play()
		timer += 1 SECONDS
	sleep(timer)
	return BRUTELOSS

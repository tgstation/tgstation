/datum/surgery_step
	...
	var/accept_hand = FALSE //does the surgery step accept an open hand? If true, using a hand is always successful. Implements can still be used (but you'll need to make their use meaningful in some other way since hands are always successful.)
	...
	var/repeatable = FALSE //can this step be repeated? Make sure it isn't last step, or else the surgeon will be stuck in the loop

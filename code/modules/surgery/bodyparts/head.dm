...
/obj/item/bodypart/head/get_limb_icon(dropped, draw_external_organs)
	...
	if(dropped) //certain overlays only appear when the limb is being detached from its owner.
		...
		if(IS_ORGANIC_LIMB(src)) //having a robotic head hides certain features.
			...
			if(!brain)
				...
				if(animal_origin == ALIEN_BODYPART)
					...
					debrain_overlay.icon = 'icons/mob/humanoid/human_face.dmi'
		...
		// lipstick
		if(lip_style)
			var/image/lips_overlay = image('icons/mob/humanoid/human_face.dmi', "lips_[lip_style]", -BODY_LAYER, SOUTH)
			lips_overlay.color = lip_color
			. += lips_overlay

		// eyes
		if(eyes) // This is a bit of copy/paste code from eyes.dm:generate_body_overlay
			var/image/eye_left = image('icons/mob/humanoid/human_face.dmi', "[eyes.eye_icon_state]_l", -BODY_LAYER, SOUTH)
			var/image/eye_right = image('icons/mob/humanoid/human_face.dmi', "[eyes.eye_icon_state]_r", -BODY_LAYER, SOUTH)
			...
			. += image('icons/mob/humanoid/human_face.dmi', "eyes_missing", -BODY_LAYER, SOUTH)
	else
		...
		if(show_debrained)
			. += mutable_appearance('icons/mob/humanoid/human_face.dmi', "debrained", HAIR_LAYER)

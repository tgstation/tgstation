/obj/item/bodypart
	...
	///An extremely snowflakey var to hold if a bodypart is BODYTYPE_ALIEN or BODYTYPE_LARVA. This is because any time a bodypart gets damaged, it checks for either. This saves 2 list loops.
	var/isaliensnowflake = FALSE
...
/obj/item/bodypart/proc/receive_damage(brute = 0, burn = 0, stamina = 0, blocked = 0, updating_health = TRUE, required_status = null, wound_bonus = 0, bare_wound_bonus = 0, sharpness = NONE, attack_direction = null)
	...
	if(isaliensnowflake) //aliens take double burn //nothing can burn with so much snowflake code around
		burn *= 2

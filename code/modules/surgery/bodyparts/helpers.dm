...
///Get the body zone for whatever hand we have active
/mob/proc/get_active_held_zone()
	return (!(active_hand_index % RIGHT_HANDS) ? BODY_ZONE_PRECISE_R_HAND : BODY_ZONE_PRECISE_L_HAND)
...
/mob/living/carbon/get_active_hand()
	return get_bodypart(check_zone(get_active_held_zone()))

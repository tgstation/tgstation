...
/obj/item/bodypart/proc/dismember(dam_type = BRUTE, silent=TRUE, wounding_type)
	...
	PLAYSOUND(get_turf(limb_owner), 'sound/effects/dismember.ogg').volume(80).vary(TRUE).play()
...
/obj/item/bodypart/chest/dismember(dam_type = BRUTE, silent=TRUE, wounding_type)
	...
	PLAYSOUND(get_turf(chest_owner), 'sound/misc/splort.ogg').volume(80).vary(TRUE).play()

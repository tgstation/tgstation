...
/obj/item/bodypart/l_leg/digitigrade/update_limb(dropping_limb = FALSE, is_creating = FALSE)
	...
	if(ishuman(owner))
		...
		if(!(human_owner.w_uniform) || (human_owner.w_uniform.alt_appearances_by_bodytype?.Index(BODYTYPE_DIGITIGRADE))) //Checks uniform compatibility
			uniform_compatible = TRUE
		if((!human_owner.wear_suit) || (human_owner.wear_suit.alt_appearances_by_bodytype?.Index(BODYTYPE_DIGITIGRADE)) || !(human_owner.wear_suit.body_parts_covered & LEGS)) //Checks suit compatability
			suit_compatible = TRUE
		if((worn_shoes == null) || (worn_shoes.alt_appearances_by_bodytype?.Index(BODYTYPE_DIGITIGRADE)))

...
/datum/surgery/proc/complete(mob/surgeon)
	...
	surgeon.mind.add_memory(MEMORY_SUCCESSFUL_SURGERY, list(DETAIL_PROTAGONIST = surgeon, DETAIL_DEUTORAGONIST = target, DETAIL_SURGERY_TYPE = src), story_value = STORY_VALUE_OKAY)
	qdel(src)

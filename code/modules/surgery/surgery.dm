...
proc/do_surgery(mob/living/M, mob/living/user, obj/item/tool)
	...
	var/sleep_fail = 0
	var/clumsy = 0
	var/userkey = ckey(user.key)
	var/mobkey = ckey(M.key)
	if(ishuman(user))
		var/mob/living/carbon/human/H = user
	...
	for(var/datum/surgery_step/S in surgery_steps)
		...
		if( S.tool_quality(tool))
			...
			if(canuse && S.is_valid_mutantrace(M) && !(M in S.doing_surgery))
				...
				M.attack_log += "\[[time_stamp()]\] [S] began by <b>[user]/[userkey]</b>"
				user.attack_log += "\[[time_stamp()]\] Began [S] on <b>[M]/[mobkey]</b>"
				log_attack("<b>[user]/[userkey]</b> began [S] on <b>[M]/[mobkey]</b>")
				...
				if(do_mob(user, M, rand(S.min_duration, S.max_duration)) && (prob(S.tool_quality(tool) / (sleep_fail + clumsy + 1))) && selection == user.zone_sel.selecting)
					...
					M.attack_log += "\[[time_stamp()]\] [S] successfully completed by <b>[user]/[userkey]</b>"
					user.attack_log += "\[[time_stamp()]\] Successfully completed [S] on <b>[M]/[mobkey]</b>"
					log_attack("<b>[user]/[userkey]</b> successfully completed [S] on <b>[M]/[mobkey]</b>")
				else
					if ((tool in user.contents) && (user.Adjacent(M)))											//or
						...
						M.attack_log += ("\[[time_stamp()]\] [S] failure by <b>[user]/[userkey]</b>")
						user.attack_log += ("\[[time_stamp()]\] Failed [S] on <b>[M]/[mobkey]</b>")
						log_attack("<b>[user]/[userkey]</b> failed [S] on <b>[M]/[mobkey]</b>")

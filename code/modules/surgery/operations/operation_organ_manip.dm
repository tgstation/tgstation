	...
	switch(operation_args[OPERATION_ACTION])
		if("remove")
			...
			display_results(
				surgeon,
				limb.owner,
				span_notice("Вы начинаете извлекать [organ.name] из [FORMAT_LIMB_OWNER_RU(limb, GENITIVE)]..."),
				span_notice("[surgeon] начинает извлекать [organ.name] из [FORMAT_LIMB_OWNER_RU(limb, GENITIVE)]."),
				span_notice("[surgeon] начинает извлекать что-то из [FORMAT_LIMB_OWNER_RU(limb, GENITIVE)]."),
			)
		...
		if("insert")
			...
			display_results(
				surgeon,
				limb.owner,
				span_notice("Вы начинаете вставлять [ru_parse_zone(tool.name, ACCUSATIVE)] в [FORMAT_LIMB_OWNER_RU(limb, ACCUSATIVE)]..."),
				span_notice("[surgeon] начинает вставлять [ru_parse_zone(tool.name, ACCUSATIVE)] в [FORMAT_LIMB_OWNER_RU(limb, ACCUSATIVE)]."),
				span_notice("[surgeon] начинает вставлять что-то в [FORMAT_LIMB_OWNER_RU(limb, ACCUSATIVE)]."),
			)
...
/datum/surgery_operation/limb/organ_manipulation/proc/on_success_remove_organ(obj/item/bodypart/limb, mob/living/surgeon, obj/item/organ/organ, obj/item/tool)
	display_results(
		surgeon,
		limb.owner,
		span_notice("Вы успешно извлекаете [organ.name] из [FORMAT_LIMB_OWNER_RU(limb, GENITIVE)]."),
...
/datum/surgery_operation/limb/organ_manipulation/proc/on_success_insert_organ(obj/item/bodypart/limb, mob/living/surgeon, operated_zone, obj/item/tool)
	...
	display_results(
		surgeon,
		limb.owner,
		span_notice("Вы успешно вставляете [organ.name] в [FORMAT_LIMB_OWNER_RU(limb, ACCUSATIVE)]."),
		span_notice("[surgeon] успешно вставляет [organ.name] в [FORMAT_LIMB_OWNER_RU(limb, ACCUSATIVE)]."),
		span_notice("[surgeon] успешно вставляет что-то в [FORMAT_LIMB_OWNER_RU(limb, ACCUSATIVE)]."),
	)

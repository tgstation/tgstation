...
/obj/item/organ/lungs/proc/check_breath(datum/gas_mixture/breath, mob/living/carbon/human/H)
	...
	//caches the gas for super speed
	...
	var/O2_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/oxygen][MOLES])+(8*breath.get_breath_partial_pressure(breath_cache[/datum/gas/pluoxium][MOLES]))
	var/N2_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/nitrogen][MOLES])
	var/Toxins_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/plasma][MOLES])
	var/CO2_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/carbon_dioxide][MOLES])
	...
	if(safe_oxygen_max)
		if(O2_pp > safe_oxygen_max)
			var/ratio = (breath_cache[/datum/gas/oxygen][MOLES]/safe_oxygen_max) * 10
	...
	if(safe_oxygen_min)
		if(O2_pp < safe_oxygen_min)
			gas_breathed = handle_too_little_breath(H, O2_pp, safe_oxygen_min, breath_cache[/datum/gas/oxygen][MOLES])
			H.throw_alert("not_enough_oxy", /obj/screen/alert/not_enough_oxy)
		else
			H.failed_last_breath = FALSE
			if(H.health >= H.crit_threshold)
				H.adjustOxyLoss(-5)
			gas_breathed = breath_cache[/datum/gas/oxygen][MOLES]
			H.clear_alert("not_enough_oxy")

	//Exhale
	breath_cache[/datum/gas/oxygen][MOLES] -= gas_breathed
	breath_cache[/datum/gas/carbon_dioxide][MOLES] += gas_breathed
	gas_breathed = 0
	...
	if(safe_nitro_max)
		if(N2_pp > safe_nitro_max)
			var/ratio = (breath_cache[/datum/gas/nitrogen][MOLES]/safe_nitro_max) * 10
	...
	if(safe_nitro_min)
		if(N2_pp < safe_nitro_min)
			gas_breathed = handle_too_little_breath(H, N2_pp, safe_nitro_min, breath_cache[/datum/gas/nitrogen][MOLES])
			H.throw_alert("nitro", /obj/screen/alert/not_enough_nitro)
		else
			H.failed_last_breath = FALSE
			if(H.health >= H.crit_threshold)
				H.adjustOxyLoss(-5)
			gas_breathed = breath_cache[/datum/gas/nitrogen][MOLES]
			H.clear_alert("nitro")

	//Exhale
	breath_cache[/datum/gas/nitrogen][MOLES] -= gas_breathed
	breath_cache[/datum/gas/carbon_dioxide][MOLES] += gas_breathed
	gas_breathed = 0
	...
	if(safe_co2_min)
		if(CO2_pp < safe_co2_min)
			gas_breathed = handle_too_little_breath(H, CO2_pp, safe_co2_min, breath_cache[/datum/gas/carbon_dioxide][MOLES])
			H.throw_alert("not_enough_co2", /obj/screen/alert/not_enough_co2)
		else
			H.failed_last_breath = FALSE
			if(H.health >= H.crit_threshold)
				H.adjustOxyLoss(-5)
			gas_breathed = breath_cache[/datum/gas/carbon_dioxide][MOLES]
	...
	//Too much toxins!
	if(safe_toxins_max)
		if(Toxins_pp > breath_cache)
			var/ratio = (breath_cache[/datum/gas/plasma][MOLES]/safe_toxins_max) * 10
	...
	if(safe_toxins_min)
		if(Toxins_pp < safe_toxins_min)
			gas_breathed = handle_too_little_breath(H, Toxins_pp, safe_toxins_min, breath_cache[/datum/gas/plasma][MOLES])
			H.throw_alert("not_enough_tox", /obj/screen/alert/not_enough_tox)
		else
			H.failed_last_breath = FALSE
			if(H.health >= H.crit_threshold)
				H.adjustOxyLoss(-5)
			gas_breathed = breath_cache[/datum/gas/plasma][MOLES]
			H.clear_alert("not_enough_tox")

	//Exhale
	breath_cache[/datum/gas/plasma][MOLES] -= gas_breathed
	breath_cache[/datum/gas/carbon_dioxide][MOLES] += gas_breathed
	gas_breathed = 0
	...
	// N2O
		...
		var/SA_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/nitrous_oxide][MOLES])
		...
		if(safe_toxins_max && SA_pp > safe_toxins_max*3)
			var/ratio = (breath_cache[/datum/gas/nitrous_oxide][MOLES]/safe_toxins_max)
			H.apply_damage_type(clamp(ratio, tox_breath_dam_min, tox_breath_dam_max), tox_damage_type)
			H.throw_alert("too_much_tox", /obj/screen/alert/too_much_tox)
		else
			H.clear_alert("too_much_tox")
	...
	// BZ
		...
		var/bz_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/bz][MOLES])
	...
	// Tritium
		var/trit_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/tritium][MOLES])
		if (trit_pp > 50)
	...
	// Nitryl
		var/nitryl_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/nitryl][MOLES])
		if (prob(nitryl_pp))
		...
		gas_breathed = breath_cache[/datum/gas/nitryl][MOLES]
		if (gas_breathed > gas_stimulation_min)
			H.reagents.add_reagent(/datum/reagent/nitryl,1)

		breath_cache[/datum/gas/nitryl][MOLES] -= gas_breathed

	// Freon
		var/freon_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/freon][MOLES])
		if (prob(freon_pp))
			to_chat(H, "<span class='alert'>Your mouth feels like it's burning!</span>")
		if (freon_pp >40)
			H.emote("gasp")
		...
		gas_breathed = breath_cache[/datum/gas/freon][MOLES]
		if (gas_breathed > gas_stimulation_min)
			H.reagents.add_reagent(/datum/reagent/freon,1)

		breath_cache[/datum/gas/freon][MOLES] -= gas_breathed

	// Healium
		var/healium_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/healium][MOLES])
		...
		gas_breathed = breath_cache[/datum/gas/healium][MOLES]
		breath_cache[/datum/gas/healium][MOLES]-=gas_breathed
	...
	// Halon
		...
		gas_breathed = breath_cache[/datum/gas/halon][MOLES]
		breath_cache[/datum/gas/halon][MOLES] -= gas_breathed

	// Hexane
		var/hexane_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/hexane][MOLES])
	...
	// Stimulum
		gas_breathed = breath_cache[/datum/gas/stimulum][MOLES]
		if (gas_breathed > gas_stimulation_min)
			var/existing = H.reagents.get_reagent_amount(/datum/reagent/stimulum)
			H.reagents.add_reagent(/datum/reagent/stimulum,max(0, 1 - existing))
		breath_cache[/datum/gas/stimulum][MOLES]-=gas_breathed

	// Hyper-Nob
		gas_breathed = breath_cache[/datum/gas/hypernoblium][MOLES]
		if (gas_breathed > gas_stimulation_min)
		...
		breath_cache[/datum/gas/hypernoblium][MOLES]-=gas_breathed

	// Miasma
		if (breath_cache[/datum/gas/miasma])
			var/miasma_pp = breath.get_breath_partial_pressure(breath_cache[/datum/gas/miasma][MOLES])
			...
			breath_cache[/datum/gas/miasma][MOLES] -= gas_breathed

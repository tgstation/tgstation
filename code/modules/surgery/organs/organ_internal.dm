...
/mob/living/carbon/regenerate_organs()
	if(dna?.species)
		...
		if(!L)
			if(!(L.organ_flags & ORGAN_UNREMOVABLE))
				L = new()
				L.Insert(src)
		L.setOrganDamage(0)
		...
		if(!H)
			if(!(H.organ_flags & ORGAN_UNREMOVABLE))
				H = new()
				H.Insert(src)
		H.setOrganDamage(0)
		...
		if(!T)
			if(!(T.organ_flags & ORGAN_UNREMOVABLE))
				T = new()
				T.Insert(src)
		T.setOrganDamage(0)
		...
		if(!eyes)
			if(!(eyes.organ_flags & ORGAN_UNREMOVABLE))
				eyes = new()
				eyes.Insert(src)
		...
		if(!ears)
			if(!(ears.organ_flags & ORGAN_UNREMOVABLE))
				ears = new()
				ears.Insert(src)

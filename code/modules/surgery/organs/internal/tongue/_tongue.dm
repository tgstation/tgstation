...
/obj/item/organ/tongue/examine(mob/user)
	...
	if(liked_foodtypes)
		. += span_info("This tongue has an affinity for the taste of <em>[lowertext(english_list(bitfield_to_list(liked_foodtypes, FOOD_FLAGS_IC)))]</em>.")
	if(disliked_foodtypes)
		. += span_info("This tongue has an aversion for the taste of <em>[lowertext(english_list(bitfield_to_list(disliked_foodtypes, FOOD_FLAGS_IC)))]</em>.")
	if(toxic_foodtypes)
		. += span_info("This tongue's physiology makes <em>[lowertext(english_list(bitfield_to_list(toxic_foodtypes, FOOD_FLAGS_IC)))]</em> toxic.")

...
/obj/item/organ/internal/cyberimp/arm/gun/emp_act(severity)
	...
	if(prob(30/severity) && owner && !(organ_flags & ORGAN_FAILING))
		...
		PLAYSOUND(get_turf(owner), 'sound/items/weapons/flashbang.ogg').volume(100).vary(TRUE).play()
...
/obj/item/organ/internal/cyberimp/arm/strongarm/proc/on_attack_hand(mob/living/carbon/human/source, atom/target, proximity, modifiers)
	...
	if(ishuman(target))
		...
		if(human_target.check_block(source, punch_damage, "[source]'s' [picked_hit_type]"))
			...
			PLAYSOUND(living_target.loc, ).volume(25).vary(TRUE).extra_range(-1).play()

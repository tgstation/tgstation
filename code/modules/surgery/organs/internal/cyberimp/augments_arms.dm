...
/obj/item/organ/cyberimp/arm/muscle/Insert(mob/living/carbon/reciever, special = FALSE, drop_if_replaced = TRUE)
	. = ..()
	if(ishuman(reciever)) //Sorry, only humans
		RegisterSignal(reciever, COMSIG_HUMAN_EARLY_UNARMED_ATTACK, PROC_REF(on_attack_hand))

/obj/item/organ/cyberimp/arm/muscle/Remove(mob/living/carbon/implant_owner, special = 0)
	. = ..()

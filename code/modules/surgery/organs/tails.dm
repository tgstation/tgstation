...
/obj/item/organ/tail/Insert(mob/living/carbon/human/tail_owner, special = FALSE, drop_if_replaced = TRUE, organ_init = ORGAN_MODIFIED_ON_RUNTIME)
	. = ..()
	tail_owner?.dna.?species?.on_tail_regain(tail_owner, src, organ_init)

/obj/item/organ/tail/Remove(mob/living/carbon/human/tail_owner, special = FALSE, organ_init = ORGAN_MODIFIED_ON_RUNTIME)
	. = ..()
	tail_owner?.dna?.species?.on_tail_lost(tail_owner, src, organ_init)
...
/obj/item/organ/tail/cat/Insert(mob/living/carbon/human/H, special = 0, drop_if_replaced = TRUE, organ_init = ORGAN_MODIFIED_ON_RUNTIME)
	..()
	if(istype(H))
...
/obj/item/organ/tail/cat/Remove(mob/living/carbon/human/H,  special = 0, organ_init = ORGAN_MODIFIED_ON_RUNTIME)
	..()
	if(istype(H))
...
/obj/item/organ/tail/lizard/Insert(mob/living/carbon/human/H, special = 0, drop_if_replaced = TRUE, organ_init = ORGAN_MODIFIED_ON_RUNTIME)
	..()
	if(istype(H))
...
/obj/item/organ/tail/lizard/Remove(mob/living/carbon/human/H,  special = 0, organ_init = ORGAN_MODIFIED_ON_RUNTIME)
	..()
	if(istype(H))
...
/obj/item/organ/tail/monkey/Insert(mob/living/carbon/human/H, special = 0, drop_if_replaced = TRUE, organ_init = ORGAN_MODIFIED_ON_RUNTIME)
	..()
	if(istype(H))
...
/obj/item/organ/tail/monkey/Remove(mob/living/carbon/human/H, special = 0, organ_init = ORGAN_MODIFIED_ON_RUNTIME)
	..()
	if(istype(H))

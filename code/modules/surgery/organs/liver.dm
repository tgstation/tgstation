...
/obj/item/organ/liver/organ_failure(delta_time)
	...
	switch(failure_time/LIVER_FAILURE_STAGE_SECONDS)
		if(1)
			to_chat(owner,"<span class='danger'>You feel stabbing pain in your abdomen!</span>")
		if(2)
			to_chat(owner,"<span class='danger'>You feel a burning sensation in your gut!</span>")
			owner.vomit()
		if(3)
			to_chat(owner,"<span class='danger'>You feel painful acid in your throat!</span>")
			owner.vomit(blood = TRUE)
		if(4)
			to_chat(owner,"<span class='danger'>Overwhelming pain knocks you out!</span>")
			owner.vomit(blood = TRUE, distance = rand(1,2))
			owner.emote("Scream")
			owner.AdjustUnconscious(2.5 SECONDS)
		if(5)
			to_chat(owner,"<span class='danger'>You feel as if your guts are about to melt!</span>")

...
/obj/item/organ/cyberimp/chest/thrusters/proc/toggle(silent = FALSE)
	if(!on)
		...
		if(allow_thrust(0.01))
			on = TRUE
			ion_trail.start()
			register_signal(owner, COMSIG_MOVABLE_MOVED, .proc/move_react)
			register_signal(owner, COMSIG_MOVABLE_PRE_MOVE, .proc/pre_move_react)
			register_signal(owner, COMSIG_MOVABLE_SPACEMOVE, .proc/spacemove_react)
		...
		unregister_signal(owner, COMSIG_MOVABLE_PRE_MOVE)
		unregister_signal(owner, COMSIG_MOVABLE_SPACEMOVE)

...
/obj/item/organ/external/wings/functional/proc/toggle_flight(mob/living/carbon/human/human)
	if(!HAS_TRAIT_FROM(human, TRAIT_MOVE_FLOATING, SPECIES_FLIGHT_TRAIT))
		...
		RegisterSignal(human, COMSIG_MOB_CLIENT_PRE_MOVE, PROC_REF(on_client_move))
	...
	UnregisterSignal(human, COMSIG_MOB_CLIENT_PRE_MOVE)
...
/obj/item/organ/external/wings/functional/process()
	...
	SEND_SIGNAL(owner, COMSIG_ATOM_STABILIZE_DRIFT, owner.client.intended_direction ? dir2angle(owner.client.intended_direction) : null, owner.client.intended_direction ? max_drift_force : 0, FUNCTIONAL_WING_STABILIZATION)

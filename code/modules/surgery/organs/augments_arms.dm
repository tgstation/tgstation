...
/obj/item/organ/internal/cyberimp/arm/muscle/on_insert(mob/living/carbon/arm_owner)
	. = ..()
...
/datum/attack_style/unarmed/generic_damage/muscle_punch/finalize_attack(mob/living/carbon/attacker, mob/living/smacked, obj/item/bodypart/weapon, right_clicking)
	...
	if(implant.organ_flags & ORGAN_FAILING)
		if(attacker.body_position != LYING_DOWN && smacked != attacker && prob(50))
			to_chat(attacker, span_danger("You try to [select_attack_verb()] [smacked], but lose your balance and fall!"))
			attacker.Knockdown(3 SECONDS)
			attacker.forceMove(get_turf(smacked))
		else
...
/datum/attack_style/unarmed/generic_damage/muscle_punch/actually_apply_damage(
	...
	if(attacker.body_position != LYING_DOWN) //Throw them if we are standing
		...
		smacked.throw_at(
			throw_target,
			hitting_with.attack_throw_range,
			rand(hitting_with.throw_power_min, hitting_with.throw_power_max),
			attacker,
			gentle = hitting_with.non_harmful_throw,
		)

 ...
 *
 * Scan members of the crew for research
 *
 * By default these explicity require that you are scanning crewmembers / players (even if they may be SSD)
 * rather than scanning monkeys / hu-monkeys. This is intended so that scientists
 * may go out and engage more with the crew, or so that medical doctors can do some scans
 * for science while in downtime by scanning patients. Possibly also opens up some antag shenanigans
 * ("Hey, wanna get your leg broken so I can scan you? It's for science!")
 */
...
/datum/experiment/scanning/points/people/check_progress()
	return EXPERIMENT_PROG_INT("Scan crewmembers. The same crewmember cannot be scanned twice. \
		Only official crewmembers on the manifest can contribute towards points.", points, required_points)
...
/datum/experiment/scanning/points/people/final_contributing_index_checks(atom/target, typepath)
	...
	return is_valid_scan_target(scanned)
...
/datum/experiment/scanning/points/people/proc/is_valid_scan_target(mob/living/carbon/human/check)
	return check.mind && (check.mind in get_crewmember_minds())
...
/datum/experiment/scanning/points/people/species
	...
	/// If TRUE, can only scan people on the manifest / players / crewmembers (calls parent in is_valid_scan_target)
	var/must_be_on_manifest = TRUE
...
/datum/experiment/scanning/points/people/species/is_valid_scan_target(mob/living/carbon/human/check)
	...
	return (scanned_id in scanned_species_ids)
...
/datum/experiment/scanning/points/people/species/check_progress()
	. = EXPERIMENT_PROG_INT("Scan [must_be_on_manifest ? "crewmembers" : "subjects"] of a [length(required_species_ids) > 1 ? "variety of " : ""]species.\
		[must_be_on_manifest ? " Only official crewmembers on the manifest can contribute towards points." : ""]", points, required_points)
...
/datum/experiment/scanning/points/people/wounds/do_after_experiment(mob/living/carbon/human/target, typepath)
	for(var/datum/wound/wound as anything in target.all_wounds)
		points += (required_wound_types[wound] || 0)

	points = min(points, required_points)

/datum/experiment/scanning/points/people/wounds/check_progress()
	. = EXPERIMENT_PROG_INT("Scan crewmembers sustaining the following wounds. Multiple wounds on one patient will \
		all count towards progress, but the same crewmember cannot be scanned twice. \
		Only official crewmembers on the manifest can contribute towards points.", points, required_points)
...
/datum/experiment/scanning/points/people/brain_traumas/check_progress()
	. = EXPERIMENT_PROG_INT("Scan crewmembers sustaining the following \
		brain traumas. The same crewmember cannot be scanned twice. \
		Only official crewmembers on the manifest can contribute towards points.", points, required_points)

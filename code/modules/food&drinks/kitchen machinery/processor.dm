...
/datum/food_processor_process/mob/slime/process_food(loc, what, var/obj/machinery/processor/processor)
	...
	if(S.stat != DEAD)
		...
		return
	for(var/i = 1, i <= C + processor.rating_amount, i++) //ALL DOSE CORES ALL DOSE BOIGAHS
		new S.coretype(loc)
...
/obj/machinery/processor/attack_hand(var/mob/user as mob)
	...
	for(var/O in src.contents)
		...
		//	log_admin("DEBUG: [O] in processor havent suitable recipe. How do you put it in?") //-rastaf0 // DEAR GOD THIS BURNS MY EYES HAVE YOU EVER LOOKED IN AN ENGLISH DICTONARY BEFORE IN YOUR LIFE AAAAAAAAAAAAAAAAAAAAA - Iamgoofball
			log_admin("DEBUG: [O] in processor doesn't have a suitable recipe.") //-zaers //I'll leave the comment up there so you can see how bad it was before. Why did it ask the admins anyways?
			continue
		total_time += P.time
	...
	for(var/O in src.contents)
		var/datum/food_processor_process/P = select_recipe(O)
		if (!P)
		//	log_admin("DEBUG: [O] in processor doesn't have a suitable recipe.") //-zaers //Commented out because it's repeated fnr, admins will probably get the memo the first the time
			continue

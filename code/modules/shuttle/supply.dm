...
/obj/docking_port/mobile/supply/proc/buy()
	...
	for(var/datum/supply_order/SO in SSshuttle.shoppinglist)
		if(!empty_turfs.len)
			break
		if(SO.pack.cost > C.balance)
			continue

		C.balance -= SO.pack.cost
		value += SO.pack.cost
	...
	investigate_log("[purchases] orders in this shipment, worth [value] credits. [C.balance] credits left.", INVESTIGATE_CARGO)

/obj/docking_port/mobile/supply/proc/sell()
	var/datum/credit/C = SSeconomy.getspecial(SPECIAL_CARGO)
	var/presale_points = C.balance
	...
	for(var/a in GLOB.exports_list)
		...
		msg += export_text + "\n"
		C.balance += E.total_cost
		E.export_end()

	SSshuttle.centcom_message = msg
	investigate_log("Shuttle contents sold for [C.balance - presale_points] credits. Contents: [sold_atoms || "none."] Message: [SSshuttle.centcom_message || "none."]", INVESTIGATE_CARGO)

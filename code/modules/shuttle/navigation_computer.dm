...
/datum/action/innate/camera_jump/shuttle_docker/Activate()
	...
	playsound(console, 'sound/machines/terminal/terminal_prompt_deny.ogg').volume(25).play()
	...
	playsound(console, 'sound/machines/terminal/terminal_prompt.ogg').volume(25).play()
	...
	if(isnull(T))
		return
	playsound(console, 'sound/machines/terminal/terminal_prompt_confirm.ogg').volume(25).play()
	remote_eye.setLoc(T)

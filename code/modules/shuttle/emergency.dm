...
/obj/docking_port/mobile/emergency/cancel(area/signalOrigin)
	...
	if(prob(70))
		...
		SSshuttle.emergency_last_call_loc = null
	priority_announce("The emergency shuttle has been recalled.[SSshuttle.emergency_last_call_loc ? " Recall signal traced. Results can be viewed on any communications console." : "" ]", null, ANNOUNCER_SHUTTLERECALLED, ANNOUNCEMENT_TYPE_PRIORITY, color_override = "yellow")
	...
	switch(mode)
		...
		if(SHUTTLE_CALL)
			if(time_left <= 0)
				...
				priority_announce("[SSshuttle.emergency] has docked with the station. You have [timeLeft(600)] minutes to board the Emergency Shuttle.", null, ANNOUNCER_SHUTTLEDOCK, ANNOUNCEMENT_TYPE_PRIORITY, color_override = "yellow")
				ShuttleDBStuff()
		...
		if(SHUTTLE_IGNITING)
			...
			if(time_left <= 0 && !SSshuttle.emergency_no_escape)
				...
				priority_announce("The Emergency Shuttle has left the station. Estimate [timeLeft(600)] minutes until the shuttle docks at Central Command.", null, null, ANNOUNCEMENT_TYPE_PRIORITY, color_override = "yellow")
...
/obj/docking_port/mobile/emergency/transit_failure()
	...
	priority_announce("The Emergency Shuttle is preparing for direct jump. Estimate [timeLeft(600)] minutes until the shuttle docks at Central Command.", null, null, ANNOUNCEMENT_TYPE_PRIORITY, color_override = "yellow")

		...
		if(player.mind)
			if(player.stat != DEAD)
				...
				if(shuttle_areas[get_area(player)])
					...
					if(!(player.mind.special_role == "traitor" || player.mind.special_role == "Syndicate") && !istype(location, /turf/open/floor/metal/shuttle/red) && !istype(location, /turf/open/floor/mineral/plastitanium/brig))
						return FALSE

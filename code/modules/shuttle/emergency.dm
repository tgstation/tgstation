		...
		if(player.mind)
			if(player.stat != DEAD)
				...
				if(shuttle_areas[get_area(player)])
					...
					if(!(player.mind.special_role == "traitor" || player.mind.special_role == "Syndicate" || player.mind.special_role == "blood brother") && !istype(location, /turf/open2/floor/plasteel/shuttle/red) && !istype(location, /turf/open2/floor/mineral/plastitanium/brig))
						return FALSE

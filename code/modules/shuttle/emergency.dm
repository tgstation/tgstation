		...
		if(player.mind)
			if(player.stat != DEAD)
				...
				if(shuttle_areas[get_area(player)])
					...
					if(!(player.mind.special_role == ROLE_TRAITOR || player.mind.special_role == ROLE_SYNDICATE || player.mind.special_role == ROLE_BROTHER) && !istype(location, /turf/open/floor/plasteel/shuttle/red) && !istype(location, /turf/open/floor/mineral/plastitanium/brig))
						return FALSE

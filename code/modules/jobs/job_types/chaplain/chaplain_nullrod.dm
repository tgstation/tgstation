...
/obj/item/vorpalscythe/attack_secondary(mob/living/victim, mob/living/user, params)
	...
	if(!potential_reaping.mind) //We put this here juuuust in case there is something funky with ling checks
		to_chat(user, span_warning("There is nothing to gain if they are not sentient, now is there?"))
		return SECONDARY_ATTACK_CANCEL_ATTACK_CHAIN
	...
	if(potential_reaping.stat != DEAD && potential_reaping.has_status_effect(/datum/status_effect/jitter)) //jittering will make it harder to secure the shears, even if you can't otherwise move
		death_knell_speed_mod *= 1.5 //15*0.5*1.5=11.25, so staminacritting someone who's jittering (from, say, a stun baton) won't give you enough time to snip their head off, but staminacritting someone who isn't jittering will
	if(death_knelled) //That said, if heads are already rolling, why stop here?
		death_knell_speed_mod *= 0.5
	if(ispodperson(potential_reaping)) //And if they're a podperson they can just die.
		death_knell_speed_mod *= 0.5
...
/obj/item/vorpalscythe/proc/death_knell_empowerment()
	if(!death_knelled) //We only empower our stats if we're not already death knelled. Otherwise, we only reset the timer
		original_force = force
		force *= bonus_force_multiplier
		death_knelled = TRUE
	addtimer(CALLBACK(src, PROC_REF(death_knell_end)), 2 MINUTES, TIMER_UNIQUE | TIMER_OVERRIDE)

/obj/item/vorpalscythe/proc/death_knell_end()

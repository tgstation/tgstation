...
/obj/structure/holopay/attackby(obj/item/held_item, mob/item_holder, params)
	...
	if(istype(held_item, /obj/item/holochip))
		...
		if(!cash_deposit)
			return TRUE
		if(QDELETED(held_item) || QDELETED(user) || QDELETED(src) || !user.canPerformAction(src, be_close = TRUE, no_dexterity = FALSE, no_tk = TRUE))
			return FALSE
...
/obj/structure/holopay/proc/process_payment(mob/living/user)
	...
	if(!amount || QDELETED(user) || QDELETED(src) || !user.canPerformAction(src, be_close = TRUE, no_dexterity = FALSE, no_tk = TRUE))
		return FALSE

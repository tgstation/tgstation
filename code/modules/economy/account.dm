	...
	for(var/obj/card in bank_cards)
		...
		if(ismob(card_holder)) //If on a mob
			...
			if(card_holder.can_hear())
				create_sound(get_turf(card_holder), 'sound/machines/beep/twobeep_high.ogg').vary(TRUE).direct_listeners(card_holder).play()
			...
			for(var/mob/potential_hearer in hearers(1,card_location))
				...
				if(potential_hearer.can_hear())
					create_sound(card_location, 'sound/machines/beep/twobeep_high.ogg').vary(TRUE).direct_listeners(potential_hearer).play()
			...
			for(var/mob/potential_hearer in card.loc) //If inside a container with other mobs (e.g. locker)
				...
				if(potential_hearer.can_hear())
					create_sound(get_turf(sound_atom), 'sound/machines/beep/twobeep_high.ogg').vary(TRUE).direct_listeners(potential_hearer).play()
					to_chat(potential_hearer, "[icon2html(icon_source, potential_hearer)] [span_notice("[message]")]")

...
/datum/bank_account/proc/bank_card_talk(message, force)
	...
	for(var/obj/A in bank_cards)
		...
		if(ismob(card_holder)) //If on a mob
			...
			card_holder.playsound_local(get_turf(card_holder), 'sound/machines/twobeep.ogg', 50, 0.4, 2)
			...
			for(var/mob/M in hearers(1,get_turf(A)))
				if(M.client && !(M.client.prefs.chat_toggles & CHAT_BANKCARD) && !force)
					return
				playsound(A, 'sound/machines/twobeep.ogg', 50, 0.4, 2)
			...
			for(var/mob/M in A.loc) //If inside a container with other mobs (e.g. locker)
				if(M.client && !(M.client.prefs.chat_toggles & CHAT_BANKCARD) && !force)
					return
				M.playsound_local(get_turf(M), 'sound/machines/twobeep.ogg', 50, 0.4, 2)
				if(M.can_hear())

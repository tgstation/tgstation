...
/mob/living/silicon/robot/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/defibrillator) && user.a_intent == "help")
		if(!opened)
			...
			return
		if(!istype(configuration, /obj/item/robot_config/medical))
	...
	if(istype(W, /obj/item/borg/upgrade/))
		...
		if(!opened)
			to_chat(user, "<span class='warning'>You must access the cyborg's internals!</span>")
			return
		if(!src.configuration && U.require_configuration)
			to_chat(user, "<span class='warning'>The cyborg must choose a configuration before it can be upgraded!</span>")
			return
		if(U.locked)

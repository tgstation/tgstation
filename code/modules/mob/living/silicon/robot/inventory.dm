...
/mob/living/silicon/robot/proc/break_cyborg_slot(module_num)
	...
	switch(module_num)
		if(BORG_CHOOSE_MODULE_ONE)
			...
			playsound(src, 'sound/machines/warning-buzzer.ogg').volume(75).vary(TRUE).extra_range(TRUE).play()
		...
		if(BORG_CHOOSE_MODULE_TWO)
			...
			playsound(src, 'sound/machines/warning-buzzer.ogg').volume(60).vary(TRUE).extra_range(TRUE).play()

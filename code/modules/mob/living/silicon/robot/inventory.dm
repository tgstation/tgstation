...
/mob/living/silicon/robot/proc/activate_module(obj/item/item_module)
	...
	if(!(item_module in configuration.modules))
		CRASH("activate_module called with item_module not in configuration.modules")
...
/mob/living/silicon/robot/proc/unequip_module_from_slot(obj/item/item_module, module_num)
	...
	if(!(item_module in configuration.modules))
		CRASH("unequip_module_from_slot called with item_module not in configuration.modules")
	...
	item_module.forceMove(configuration) //Return item to configuration so it appears in its contents, so it can be taken out again.
...
/mob/living/silicon/robot/can_hold_items(obj/item/I)
	return (I && (I in configuration.modules)) //Only if it's part of our module.

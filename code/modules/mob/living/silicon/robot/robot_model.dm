...
/obj/item/robot_model/proc/restock_consumable()
	...
	for(var/datum/robot_energy_storage/material/storage_datum in storages)
		...
		create_sound(charger, 'sound/items/weapons/gun/general/mag_bullet_insert.ogg').play()
		return
...
/obj/item/robot_model/proc/do_transform_delay()
	...
	sleep(0.1 SECONDS)
	for(var/i in 1 to 4)
		create_sound(cyborg, pick('sound/items/tools/drill_use.ogg', 'sound/items/tools/jaws_cut.ogg', 'sound/items/tools/jaws_pry.ogg', 'sound/items/tools/welder.ogg', 'sound/items/tools/ratchet.ogg')).volume(80).vary(TRUE).extra_range(-1).play()
		sleep(0.7 SECONDS)

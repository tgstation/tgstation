...
/mob/living/silicon/robot/updatehealth()
	...
	if(health < maxHealth*0.5) //Gradual break down of modules as more damage is sustained
		if(uneq_module(held_items[3]))
			playsound(loc, 'sound/machines/warning-buzzer.ogg', 50, 1, DEFAULT_SOUND_VARY)
		...
		if(health < 0)
			if(uneq_module(held_items[2]))
				...
				playsound(loc, 'sound/machines/warning-buzzer.ogg', 60, 1, DEFAULT_SOUND_VARY)
			if(health < -maxHealth*0.5)
				if(uneq_module(held_items[1]))
					...
					playsound(loc, 'sound/machines/warning-buzzer.ogg', 75, 1, DEFAULT_SOUND_VARY)

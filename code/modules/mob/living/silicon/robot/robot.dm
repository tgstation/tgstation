...
//			chargecount = 0
		...
		if(!lockcharge)
			...
			if(do_after(user, 50) && !cell)
				user.visible_message("<span class='notice'>[user] unfastens the securing bolts, and [src] falls to pieces.</span>", "<span class='notice'>You unfasten the securing bolts, and [src] falls to pieces.</span>")
				deconstruct()

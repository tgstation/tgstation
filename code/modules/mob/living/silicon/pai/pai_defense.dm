...
/mob/living/silicon/pai/attack_hand(mob/living/carbon/human/user)
	switch(user.a_intent)
		if("help")
			visible_message("<span class='none'>[user] gently pats [src] on the head, eliciting an off-putting buzzing from its holographic field.</span>")
		if("disarm")
			visible_message("<span class='none'>[user] boops [src] on the head!</span>")
		if("harm")
			user.do_attack_animation(src)
			if (user.name == master)
				visible_message("<span class='none'>Responding to its master's touch, [src] disengages its holochassis emitter, rapidly losing coherence.</span>")
				spawn(10)
					fold_in()
					if(user.put_in_hands(card))
						user.visible_message("<span class='none'>[user] promptly scoops up [user.p_their()] pAI's card.</span>")
			else

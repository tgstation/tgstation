...
/mob/living/silicon/pai/emp_act(severity)
	...
	switch(rand(1, 3))
		if(1)
			add_timed_status_effect(1 MINUTES / severity, /datum/status_effect/speech/stutter)
			to_chat(src, span_danger("Warning: Feedback loop detected in speech module."))
		if(2)
			add_timed_status_effect(INFINITY, /datum/status_effect/speech/slurring/drunk)
			to_chat(src, span_danger("Warning: Audio synthesizer CPU stuck."))
		if(3)
			add_timed_status_effect(INFINITY, /datum/status_effect/speech/stutter/derpspeech)

...
/mob/living/silicon/pai/attack_hand(mob/living/carbon/human/user)
	switch(user.a_intent)
		if("help")
			visible_message("<span class='notice'>[IDENTITY_SUBJECT(1)] gently pats [IDENTITY_SUBJECT(2)] on the head, eliciting an off-putting buzzing from its holographic field.</span>", subjects=list(user, src))
		if("disarm")
			visible_message("<span class='notice'>[IDENTITY_SUBJECT(1)] boops [IDENTITY_SUBJECT(2)] on the head!</span>", subjects=list(user, src))
		if("harm")
			user.do_attack_animation(src)
			if (user.name == master)
				visible_message("<span class='notice'>Responding to its master's touch, [IDENTITY_SUBJECT(1)] disengages its holochassis emitter, rapidly losing coherence.</span>", subjects=list(src))
				spawn(10)
					fold_in()
					if(user.put_in_hands(card))
						user.visible_message("<span class='notice'>[IDENTITY_SUBJECT(1)] promptly scoops up their pAI's card.</span>", subjects=list(user))
			else
				visible_message("<span class='danger'>[IDENTITY_SUBJECT(1)] stomps on [IDENTITY_SUBJECT(2)]!.</span>", subjects=list(user, src))
				take_holo_damage(2)

/mob/living/silicon/pai/bullet_act(obj/item/projectile/Proj)
	if(Proj.stun)
		fold_in(force = TRUE)
		src.visible_message("<span class='warning'>The electrically-charged projectile disrupts [IDENTITY_SUBJECT(1)]'s holomatrix, forcing [IDENTITY_SUBJECT(1)] to fold in!</span>", subjects=list(src))
	. = ..(Proj)

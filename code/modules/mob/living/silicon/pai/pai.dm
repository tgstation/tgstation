...
/mob/living/silicon/pai/Life()
	...
	if(cable)
		if(get_dist(src, cable) > 1)
			...
			T.visible_message("<span class='warning'>[IDENTITY_SUBJECT(1)] rapidly retracts back into its spool.</span>", "<span class='italics'>You hear a click and the sound of wire spooling rapidly.</span>", subjects=list(src))
			qdel(src.cable)
			cable = null

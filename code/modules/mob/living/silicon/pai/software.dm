...
/mob/living/silicon/pai/Topic(href, href_list)
	...
	switch(soft)
		...
		if("medicalrecord")
			if(subscreen == 1)
				medicalActive1 = find_record("id", href_list["med_rec"], SLOTH.data_core.general)
				if(medicalActive1)
					medicalActive2 = find_record("id", href_list["med_rec"], SLOTH.data_core.medical)
				if(!medicalActive2)
					medicalActive1 = null
		...
		if("securityrecord")
			if(subscreen == 1)
				securityActive1 = find_record("id", href_list["sec_rec"], SLOTH.data_core.general)
				if(securityActive1)
					securityActive2 = find_record("id", href_list["sec_rec"], SLOTH.data_core.security)
				if(!securityActive2)
...
/mob/living/silicon/pai/proc/softwareMedicalRecord()
	switch(subscreen)
		...
		if(1)
			...
			if(medicalActive1 in SLOTH.data_core.general)
			...
			if(medicalActive2 in SLOTH.data_core.medical)
...
/mob/living/silicon/pai/proc/softwareSecurityRecord()
	...
	switch(subscreen)
		...
		if(1)
			...
			if(securityActive1 in SLOTH.data_core.general)
			...
			if(securityActive2 in SLOTH.data_core.security)
...
/mob/living/silicon/pai/proc/hackloop()
	var/turf/T = get_turf(src.loc)
	for(var/mob/living/silicon/ai/AI in SLOTH.player_list)
		if(T.loc)

		...
		if("doorjack")
			...
			if(href_list["cable"])
				...
				for (var/mob/M in viewers(T))
					M.show_message("<span class='danger'>A port on [src] opens to reveal [src.cable], which promptly falls to the floor.", 3, "\red You hear the soft click of something light and hard falling to the ground.</span>", 2)
...
/mob/living/silicon/pai/proc/CheckDNA(mob/living/carbon/M, mob/living/silicon/pai/P)
	...
	if(answer == "Yes")
		...
		for (var/mob/v in viewers(T))
			v.show_message("<span class='notice'>[M] presses \his thumb against [P].", 3, "\blue [P] makes a sharp clicking sound as it extracts DNA material from [M].</span>", 2)

	...
	// If there is no single listener, broadcast to everyone in the same z level
		if(!only_listener)
			...
			for(var/mob/player_mob in GLOB.player_list)
				...
				if(player_mob.can_hear() && (player_mob.client?.prefs.read_preference(/datum/preference/toggle/sound_vox))) // monke edit: use sound_vox pref rather than sound_announcements

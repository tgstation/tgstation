...
/mob/living/simple_animal/proc/make_babies() // <3 <3 <3
	if(gender != FEMALE || IS_UNCONSCIOUS_CRIT_OR_DEAD(src) || next_scan_time > world.time || !childtype || !animal_species || !SSticker.IsRoundInProgress())
		return
	...
	for(var/mob/M in view(7, src))
		if(IS_UNCONSCIOUS_CRIT_OR_DEAD(M)) //Check if it's conscious FIRST.
			continue
...
/mob/living/simple_animal/proc/hunt(hunted)
	...
	if(!isturf(src.loc)) // Are we on a proper turf?
		return
	if(IS_UNCONSCIOUS_CRIT_OR_DEAD(src) || resting || buckled) // Are we conscious, upright, and not buckled?
		return
	...
	if(isliving(hunted)) // Are we hunting a living mob?
		...
		if(inept_hunter) // Make your hunter inept to have them unable to catch their prey.
			...
			return
		if(!IS_UNCONSCIOUS_CRIT_OR_DEAD(prey))

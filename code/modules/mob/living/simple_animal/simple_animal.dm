...
/mob/living/simple_animal/proc/reagent_act(id, method, volume)
	...
	switch(id)
		if("sacid")
			if(!supernatural)
				adjustBruteLoss(volume * 1.2)
				if(prob(min(5 * volume, 80))) visible_message("<span class='danger'>\The [src] is burned by the sulfuric acid!</span>")
		if("pacid")
			if(!supernatural)
				adjustBruteLoss(volume * 1.5)
				if(prob(min(5 * volume, 80))) visible_message("<span class='danger'>\The [src] is burned by the polytrinic acid!</span>")

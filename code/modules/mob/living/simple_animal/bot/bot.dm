...
/mob/living/simple_animal/bot/proc/call_bot(caller, turf/waypoint, message=TRUE)
	...
	if(client) //Player bots instead get a location command from the AI
		to_chat(src, "<span class='noticebig'>Priority waypoint set by [bIcon(caller, src)] <b>[caller]</b>. Proceed to <b>[end_area.name]<\b>.")
	...
	if(path && path.len) //Ensures that a valid path is calculated!
		...
		if(message)
			to_chat(calling_ai, "<span class='notice'>[bIcon(src, calling_ai)] [name] called to [end_area.name]. [path.len-1] meters to destination.</span>")
		pathset = 1
		mode = BOT_RESPONDING
		tries = 0
	else
...
//Handles the bot's movement during a call.
	...
	if(!success)
		if(calling_ai)
			to_chat(calling_ai, "[bIcon(src, calling_ai)] [get_turf(src) == ai_waypoint ? "<span class='notice'>[src] successfully arrived to waypoint.</span>" : "<span class='danger'>[src] failed to reach waypoint.</span>"]")
			calling_ai = null
		bot_reset()

...
//Handles the bot's movement during a call.
	...
	if(!success)
		if(calling_ai)
			var/message = "<span class='danger'>[src] failed to reach waypoint.</span>"
			if(get_turf(src) == ai_waypoint)
				message = "<span class='notice'>[src] successfully arrived to waypoint.</span>"
			to_chat(calling_ai, "[bicon(src)] [message]")
			calling_ai = null
		bot_reset()

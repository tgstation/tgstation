...
/mob/living/simple_animal/bot/mulebot/emag_act(mob/user, obj/item/card/emag/emag_card)
	...
	create_sound(src, SFX_SPARKS).volume(100).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
	return TRUE
	...
	if(!reached_target)
		...
		if(pathset) //The AI called us here, so notify it of our arrival.
			...
			if(calling_ai)
				...
				create_sound(calling_ai, 'sound/machines/chime.ogg').volume(40).direct_listeners(calling_ai).play()
				calling_ai = null

...
/mob/living/simple_animal/bot/mulebot/examine(mob/user)
	...
	if(bot_cover_flags & BOT_COVER_MAINTS_OPEN)
		if(cell)
			. += span_notice("[src.p_They()] has \a [cell] installed.")
			. += span_info("You can use a <b>crowbar</b> to remove it.")
		else
			. += span_notice("[src.p_They()] has an empty compartment where a <b>power cell</b> can be installed.")
	if(load) //observer check is so we don't show the name of the ghost that's sitting on it to prevent metagaming who's ded.
		. += span_notice("\A [isobserver(load) ? "ghostly figure" : load] is on [src.p_their()] load platform.")
...
/mob/living/simple_animal/bot/mulebot/paranormal//allows ghosts only unless hacked to actually be useful
	...
	desc = "A rather ghastly looking... Multiple Utility Load Effector bot? [src.p_They()] only seems to accept paranormal forces, and for this reason is fucking useless."

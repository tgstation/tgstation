...
/mob/living/simple_animal/bot/medbot/handle_automated_action()
	...
	if(patient && path.len == 0 && (get_dist(src,patient) > 1))
		...
		mode = BOT_MOVING
		if(!path.len) //try to get closer if you can't reach the patient
			path = get_path_to(loc, get_turf(patient), src, /turf/proc/Distance_cardinal, 0, 30,id=access_card,mintargetdist=1)

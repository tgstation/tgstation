...
/mob/living/simple_animal/bot/firebot/proc/is_burning(atom/target)
	if(ismob(target))
		...
		if(M.on_fire || (emagged == BOT_EMAGGED_OVERDRIVE && !M.on_fire))
			return TRUE
...
/mob/living/simple_animal/bot/firebot/handle_automated_action()
	...
	if(target_fire && (get_dist(src, target_fire) <= (emagged == BOT_EMAGGED_OVERDRIVE ? 1 : 2))) // Make the bot spray water from afar when not emagged

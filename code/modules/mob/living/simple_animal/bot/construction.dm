...
/obj/item/bot_assembly/medbot
	...
	var/medkit = /obj/item/storage/medkit
	...
	switch(build_step)
		...
		if(ASSEMBLY_SECOND_STEP)
			if(isprox(W))
				...
				medbot.medkit_type = medkit
				medbot.robot_arm = robot_arm
				medbot.healthanalyzer = healthanalyzer
				var/obj/item/storage/medkit/medkit = medkit
				S.damagetype_healer = initial(medkit.damagetype_healed) ? initial(medkit.damagetype_healed) : BRUTE
				qdel(src)

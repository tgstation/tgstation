/mob/living/simple_animal/attack_hand(mob/living/carbon/human/user, list/modifiers)
	...
	if(!user.combat_mode)
		...
		playsound(loc, attacked_sound).volume(25).vary(TRUE).extra_range(-1).play()
...
/mob/living/simple_animal/attack_alien(mob/living/carbon/alien/adult/user, list/modifiers)
	if(..()) //if harm or disarm intent.
		if(LAZYACCESS(modifiers, RIGHT_CLICK))
			playsound(loc, 'sound/items/weapons/pierce.ogg').volume(25).vary(TRUE).extra_range(-1).play()
			...
			playsound(loc, 'sound/items/weapons/slice.ogg').volume(25).vary(TRUE).extra_range(-1).play()
			apply_damage(damage)
			log_combat(user, src, "attacked")
		return 1

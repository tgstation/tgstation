...
/mob/living/simple_animal/hostile/proc/ListTargets()//Step 1, find out what we can see
	if(!search_objects)
		...
		var/static/hostile_machines = typecacheof(list(/obj/machinery/porta_turret, /obj/mecha, /obj/structure/destructible/chumbiswork/ocular_warden))
...
/mob/living/simple_animal/hostile/CanAttack(atom/the_target)//Can we actually attack a possible target?
	...
	if(search_objects < 2)
		...
		if(istype(the_target, /obj/structure/destructible/chumbiswork/ocular_warden))
			var/obj/structure/destructible/chumbiswork/ocular_warden/OW = the_target
			if(OW.target != src)
				return FALSE
			return TRUE
	if(isobj(the_target))

...
/mob/living/simple_animal/hostile/retaliate/goose/vomit/feed(obj/item/food/tasty)
	...
	if (tasty.foodtypes & GROSS)
		...
		tasty.forceMove(src)
		PLAYSOUND(src, 'sound/items/eatfood.ogg').volume(70).vary_frequency(TRUE).play()
		vomitCoefficient += 3
		vomitTimeBonus += 2
		return TRUE
	else
...
/mob/living/simple_animal/hostile/retaliate/goose/vomit/proc/vomit()
	...
	if (prob(50) && consumed)
		...
		PLAYSOUND(T, 'sound/effects/splat.ogg').volume(50).vary_frequency(TRUE).play()
...
/mob/living/simple_animal/hostile/retaliate/goose/vomit/proc/barf_food(atom/A, hard = FALSE)
	...
	PLAYSOUND(currentTurf, 'sound/effects/splat.ogg').volume(50).vary_frequency(TRUE).play()

...
/mob/living/simple_animal/hostile/retaliate/goose/vomit/feed(obj/item/food/tasty)
	...
	if (tasty.foodtypes & GROSS)
		...
		tasty.forceMove(src)
		playsound(src, 'sound/items/eatfood.ogg').volume(70).vary(TRUE).play()
		vomitCoefficient += 3
		vomitTimeBonus += 2
		return TRUE
	else
...
/mob/living/simple_animal/hostile/retaliate/goose/vomit/proc/vomit()
	...
	if (prob(50) && consumed)
		...
		playsound(T, 'sound/effects/splat.ogg').vary(TRUE).play()
...
/mob/living/simple_animal/hostile/retaliate/goose/vomit/proc/barf_food(atom/A, hard = FALSE)
	...
	playsound(currentTurf, 'sound/effects/splat.ogg').vary(TRUE).play()

...
/mob/living/simple_animal/hostile/asteroid/elite/herald/OpenFire(atom/attacking_target)
	if(client)
		switch(chosen_attack)
			if(HERALD_TRISHOT)
				herald_trishot(attacking_target)
				if(my_mirror != null)
					my_mirror.herald_trishot(attacking_target)
			if(HERALD_DIRECTIONALSHOT)
				herald_directionalshot()
				if(my_mirror != null)
					my_mirror.herald_directionalshot()
			if(HERALD_TELESHOT)
				herald_teleshot(attacking_target)
				if(my_mirror != null)
					my_mirror.herald_teleshot(attacking_target)
			if(HERALD_MIRROR)
				herald_mirror()
		return
	...
	switch(aiattack)
		...
		if(HERALD_TELESHOT)
			herald_teleshot(attacking_target)
			if(my_mirror != null)
				my_mirror.herald_teleshot(targattacking_targetet)
		if(HERALD_MIRROR)
			herald_mirror()

...
/mob/living/simple_animal/hostile/asteroid/elite/legionnaire/proc/legionnaire_charge(target)
	...
	playsound(src, 'sound/effects/magic/demon_attack1.ogg').volume(200).vary(TRUE).play()
...
/mob/living/simple_animal/hostile/asteroid/elite/legionnaire/proc/legionnaire_charge_2(move_dir, times_ran)
	...
	for(var/obj/machinery/door/D in T.contents)
		if(D.density)
			...
			return
	forceMove(T)
	playsound(src, 'sound/effects/bang.ogg').volume(200).vary(TRUE).play()
...
/mob/living/simple_animal/hostile/asteroid/elite/legionnaire/proc/bonfire_teleport()
	...
	if(mypile == null)
		...
		mypile = newpile
		mypile.myowner = src
		playsound(get_turf(src), 'sound/items/fulton/fultext_deploy.ogg').volume(200).vary(TRUE).play()
		...
		if(legionturf == pileturf)
			...
			mypile = null
			return
		playsound(pileturf, 'sound/items/fulton/fultext_deploy.ogg').volume(200).vary(TRUE).play()
		playsound(legionturf, 'sound/items/fulton/fultext_deploy.ogg').volume(200).vary(TRUE).play()

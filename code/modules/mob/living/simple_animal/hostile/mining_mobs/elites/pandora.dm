...
/mob/living/simple_animal/hostile/asteroid/elite/pandora/OpenFire(atom/attacking_target)
	if(client)
		switch(chosen_attack)
			if(SINGULAR_SHOT)
				singular_shot(attacking_target)
			if(MAGIC_BOX)
				magic_box(attacking_target)
			if(PANDORA_TELEPORT)
				pandora_teleport(attacking_target)
			if(AOE_SQUARES)
				aoe_squares(attacking_target)
		return

...
/obj/structure/elite_tumor/proc/onEliteWon()
	...
	if(times_won == 1)
		...
		to_chat(mychild, span_boldnotice("Your max health has been halved, but can now heal by standing on your tumor.  Note, it's your only way to heal.\nBear in mind, if anyone interacts with your tumor, you'll be resummoned here to carry out another fight.  In such a case, you will regain your full max health.\nAlso, be weary of your fellow inhabitants, they likely won't be happy to see you!"))
		to_chat(mychild, span_boldnotice("Note that you are a lavaland monster, and thus not allied to the station.  You should not cooperate or act friendly with any station crew unless under extreme circumstances!"))
...
/obj/item/tumor_shard/afterattack(atom/target, mob/user, proximity_flag)
	...
	if(istype(target, /mob/living/simple_animal/hostile/asteroid/elite) && proximity_flag)
		...
		to_chat(E, span_boldannounce("You have been revived by [user].  While you can't speak to them, you owe [user] a great debt.  Assist [user.p_them()] in achieving [user.p_their()] goals, regardless of risk."))
		to_chat(E, span_boldnotice("Note that you now share the loyalties of [user].  You are expected not to intentionally sabotage their faction unless commanded to!"))

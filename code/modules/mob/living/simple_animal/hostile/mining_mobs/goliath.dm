...
/mob/living/simple_animal/hostile/asteroid/goliath/Life(delta_time = SSSIMPLE_MOBS_DT, times_fired)
	. = ..()
	handle_preattack()
...
/mob/living/simple_animal/hostile/asteroid/goliath/beast/ancient/Life(delta_time = SSSIMPLE_MOBS_DT, times_fired)
	. = ..()
	if(!.) // dead
		return
	if(isturf(loc))

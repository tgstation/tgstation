...
/obj/item/queen_bee/attackby(obj/item/I, mob/user, params)
	...
	var/obj/item/reagent_containers/syringe/injector = I
	if(injector.reagents.has_reagent(/datum/reagent/royal_bee_jelly)) //checked twice, because I really don't want royal bee jelly to be duped
		if(!injector.reagents.has_reagent(/datum/reagent/royal_bee_jelly, 5))
			to_chat(user, "<span class='warning'>You don't have enough royal bee jelly to split a bee in two!</span>")
			return

		injector.reagents.remove_reagent(/datum/reagent/royal_bee_jelly, 5)
		var/obj/item/queen_bee/new_bee = new(user.drop_location())
	...
	var/datum/reagent/absorb = GLOB.chemical_reagents_list[injector.reagents.get_master_reagent_id()]
	if(!absorb || !injector.reagents.has_reagent(absorb.type, 5))
		to_chat(user, "<span class='warning'>You don't have enough units of that chemical to modify the bee's DNA!</span>")
		return

	if(!(absorb.chemical_flags & REAGENT_CAN_BE_SYNTHESIZED)) //Adminrazine bee man bad
	...
	injector.reagents.remove_reagent(absorb.type, 5)
	queen.assign_reagent(absorb)
	user.visible_message("<span class='warning'>[user] injects [src]'s genome with [absorb.name], mutating its DNA!</span>","<span class='warning'>You inject [src]'s genome with [absorb.name], mutating its DNA!</span>")
	name = queen.name

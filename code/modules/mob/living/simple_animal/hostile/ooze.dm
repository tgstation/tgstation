...
/datum/action/consume/proc/start_consuming(mob/living/target)
	...
	playsound(owner, 'sound/items/eatfood.ogg').volume(rand(30,50)).vary(TRUE).play()
...
/datum/action/consume/proc/stop_consuming()
	...
	playsound(get_turf(owner), 'sound/effects/splat.ogg').vary(TRUE).play()
...
/obj/structure/gel_cocoon/proc/dump_inhabitant(destroy_after = TRUE)
	...
	playsound(get_turf(inhabitant), 'sound/effects/splat.ogg').vary(TRUE).play()

/**
  ...
  * Space Dragon can summon only one rift, and can do so anywhere a blob is allowed to spawn.  In order to trigger his victory condition, Space Dragon must defend the rift for 10 minutes.
  * Space Dragon, when spawned, has 5 minutes to summon the rift.  Failing to do so will cause Space Dragon to return from whence he came.
  * When the rift spawns, ghosts can interact with it to spawn in as space carp to help complete the mission.  3 carp are granted when the rift is first summoned, with an extra one every minute.
  * Once the victory condition is met, the crew is given 45 seconds to evacuate before the station is covered in carp, ending the round and greentexting Space Dragon.
  * If the rift is destroyed, Space Dragon will be incredibly slowed, and the endlag on his gust attack is greatly increased on each use.
...
/obj/structure/carp_rift/process()
	...
	update_check()
	if(carp_stored > 0 && time_charged > max_charge + 1)
...
/obj/structure/carp_rift/proc/update_check()
	...
	if(time_charged == max_charge)
		priority_announce("Spatial object has reached peak energy charge.  Scans indicate a spike in detected life signals.  Crew are requested to evacuate from [station_name()] immediately.", "Central Command Spacial Corps")
		set_security_level("delta")
		obj_integrity = INFINITY
		carp_stored = INFINITY
...
/obj/structure/carp_rift/proc/victory()
	...
	for(var/i in GLOB.mob_living_list)
		...
		if(dragon.faction_check_mob(L))
			continue

		var/carp_locs = spiral_range_turfs(2, get_turf(L))
		for(var/turf/ET in carp_locs)
			new /mob/living/simple_animal/hostile/carp(ET)
	...
	to_chat(world, "<B>[dragon.real_name] has brought forth the great tide of space carp!  The station has been forever lost to the depths of space!</B>")
	SSticker.news_report = SPACE_DRAGON_WIN

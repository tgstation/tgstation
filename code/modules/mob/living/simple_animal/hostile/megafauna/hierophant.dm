...
/mob/living/simple_animal/hostile/megafauna/hierophant/Moved(oldLoc, movement_dir)
	...
	if(!stat && .)
		...
		playsound(src, 'sound/mecha/mechmove04.ogg', 150, DEFAULT_SOUND_VARY, extra_range = -4)
		if(target)
			arena_trap(target)
...
/mob/living/simple_animal/hostile/megafauna/hierophant/proc/diagonal_blasts(mob/victim) //fire diagonal cross blasts with a delay
	...
	playsound(T,'sound/effects/bin_close.ogg', 200, DEFAULT_SOUND_VARY)
	sleep(2)
...
/mob/living/simple_animal/hostile/megafauna/hierophant/proc/blink(mob/victim) //blink to a target
	...
	playsound(T,'sound/magic/wand_teleport.ogg', 200, DEFAULT_SOUND_VARY)
	playsound(source,'sound/machines/airlockopen.ogg', 200, DEFAULT_SOUND_VARY)
	blinking = TRUE
...
/proc/hierophant_burst(mob/caster, turf/original, burst_range, spread_speed = 0.5)
	playsound(original,'sound/machines/airlockopen.ogg', 200, DEFAULT_SOUND_VARY)
	var/last_dist = 0
...
/obj/effect/temp_visual/hierophant/blast/proc/blast()
	...
	if(!T)
		return
	playsound(T,'sound/magic/blind.ogg', 125, DEFAULT_SOUND_VARY, extra_range = -5) //make a sound
...
/obj/effect/temp_visual/hierophant/blast/proc/do_damage(turf/T)
	...
	for(var/mob/living/L in T.contents - hit_things) //find and damage mobs...
		...
		playsound(L,'sound/weapons/sear.ogg', 50, DEFAULT_SOUND_VARY, extra_range = -4)
	...
	for(var/obj/mecha/M in T.contents - hit_things) //also damage mechs.
		...
		playsound(M,'sound/weapons/sear.ogg', 50, DEFAULT_SOUND_VARY, extra_range = -4)
	...
	if(istype(I, /obj/item/hierophant_club))
		...
		if(H.beacon == src)
			...
			if(do_after(user, 50, target = src))
				playsound(src,'sound/magic/blind.ogg', 200, DEFAULT_SOUND_VARY, extra_range = -4)

...
/mob/living/simple_animal/hostile/megafauna/hierophant
	...
	innate_factions = list(TRAIT_FACTION_BOSS) //asteroid mobs? get that shit out of my beautiful square house
...
/obj/effect/temp_visual/hierophant/blast/damaging/proc/do_damage(turf/T)
	...
	for(var/mob/living/L in T.contents - hit_things) //find and damage mobs...
		hit_things += L
		if((friendly_fire_check && caster?.faction_check(L)) || L.stat == DEAD)
			continue
		if(L.client)
	...
	for(var/obj/vehicle/sealed/mecha/M in T.contents - hit_things) //also damage mechs.
		...
		for(var/O in M.occupants)
			...
			if(friendly_fire_check && caster?.faction_check(occupant))
				continue

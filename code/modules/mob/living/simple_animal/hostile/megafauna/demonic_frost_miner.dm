...
/obj/projectile/colossus/frost_orb/on_hit(atom/target, blocked = FALSE, pierce_hit)
	. = ..()
...
/obj/projectile/colossus/ice_blast/on_hit(atom/target, blocked = FALSE, pierce_hit)
	. = ..()
	if(isturf(target) || isobj(target))
		EX_ACT(target, EXPLODE_HEAVY)

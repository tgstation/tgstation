...
/obj/effect/temp_visual/lava_warning/proc/fall(reset_time)
	...
	PLAYSOUND(T, 'sound/effects/magic/fleshtostone.ogg').volume(80).vary_frequency(TRUE).play()
	sleep(duration)
	PLAYSOUND(T, 'sound/effects/magic/fireball.ogg').volume(200).vary_frequency(TRUE).play()
...
/obj/effect/temp_visual/target/proc/fall(list/flame_hit)
	...
	if(ismineralturf(T))
		...
		M.gets_drilled()
	PLAYSOUND(get_sfx(T), SFX_EXPLOSION).volume(80).vary_frequency(TRUE).play()

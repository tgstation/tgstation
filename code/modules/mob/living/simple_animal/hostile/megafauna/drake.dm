...
/mob/living/simple_animal/hostile/megafauna/dragon/proc/lava_swoop(amount = 30)
	...
	if(health < maxHealth*0.5)
		...
		SLEEP_CHECK_DEATH(10)
		fire_cone()
	update_cooldowns(list("set_melee" = 4 SECONDS, "set_ranged" = 4 SECONDS))

/mob/living/simple_animal/hostile/megafauna/dragon/proc/mass_fire(spiral_count = 12, range = 15, times = 3)
	SLEEP_CHECK_DEATH(0)
	for(var/i = 1 to times)
		update_cooldowns(list("set_melee" = 5 SECONDS, "set_ranged" = 5 SECONDS))
	...
	update_cooldowns(list("set_melee" = 3 SECONDS, "set_ranged" = 3 SECONDS))
...
/mob/living/simple_animal/hostile/megafauna/dragon/proc/arena_escape_enrage() // you ran somehow / teleported away from my arena attack now i'm mad fucker
	SLEEP_CHECK_DEATH(0)
	update_cooldowns(list("set_melee" = 8 SECONDS, "set_ranged" = 8 SECONDS))
...
/mob/living/simple_animal/hostile/megafauna/dragon/proc/swoop_attack(lava_arena = FALSE, atom/movable/manual_target, swoop_cooldown = 30)
	...
	SLEEP_CHECK_DEATH(1)
	swooping &= ~SWOOP_DAMAGEABLE
	update_cooldowns(list("set_melee" = swoop_cooldown, "set_ranged" = swoop_cooldown))
	if(!lava_success)
		arena_escape_enrage()

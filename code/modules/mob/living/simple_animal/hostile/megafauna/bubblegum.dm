...
/mob/living/simple_animal/hostile/megafauna/bubblegum/proc/bloodsmack(turf/T, handedness)
	...
	for(var/mob/living/L in T)
		if(!faction_check_atom(L))
			...
			playsound(T, attack_sound).volume(100).vary(TRUE).range(-1 + SOUND_RANGE).play()
	...
	for(var/mob/living/L in T)
		if(!faction_check_atom(L))
			if(L.stat != CONSCIOUS)
				...
				playsound(T, 'sound/effects/magic/enter_blood.ogg').volume(100).vary(TRUE).range(-1 + SOUND_RANGE).play()
				var/turf/targetturf = get_step(src, dir)
				L.forceMove(targetturf)
				playsound(targetturf, 'sound/effects/magic/exit_blood.ogg').volume(100).vary(TRUE).range(-1 + SOUND_RANGE).play()
...
/mob/living/simple_animal/hostile/megafauna/bubblegum/bullet_act(obj/projectile/P)
	if(BUBBLEGUM_IS_ENRAGED)
		...
		playsound(src, pick('sound/items/weapons/bulletflyby.ogg', 'sound/items/weapons/bulletflyby2.ogg', 'sound/items/weapons/bulletflyby3.ogg')).volume(300).vary(TRUE).play()

...
/mob/living/simple_animal/hostile/megafauna/bubblegum/proc/bloodsmack(turf/T, handedness)
	...
	for(var/mob/living/L in T)
		if(!faction_check_atom(L))
			...
			playsound(T, attack_sound).volume(100).vary(TRUE).extra_range(-1).play()
	...
	for(var/mob/living/L in T)
		if(!faction_check_atom(L))
			if(L.stat != CONSCIOUS)
				...
				playsound(T, 'sound/effects/magic/enter_blood.ogg').volume(100).vary(TRUE).extra_range(-1).play()
				var/turf/targetturf = get_step(src, dir)
				L.forceMove(targetturf)
				playsound(targetturf, 'sound/effects/magic/exit_blood.ogg').volume(100).vary(TRUE).extra_range(-1).play()

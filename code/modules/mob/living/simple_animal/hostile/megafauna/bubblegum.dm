...
/mob/living/simple_animal/hostile/megafauna/bubblegum/proc/bloodsmack(turf/T, handedness)
	...
	for(var/mob/living/L in T)
		if(!faction_check_atom(L))
			...
			PLAYSOUND(T, attack_sound).volume(100).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
	...
	for(var/mob/living/L in T)
		if(!faction_check_atom(L))
			if(L.stat != CONSCIOUS)
				...
				PLAYSOUND(T, 'sound/effects/magic/enter_blood.ogg').volume(100).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
				var/turf/targetturf = get_step(src, dir)
				L.forceMove(targetturf)
				PLAYSOUND(targetturf, 'sound/effects/magic/exit_blood.ogg').volume(100).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()

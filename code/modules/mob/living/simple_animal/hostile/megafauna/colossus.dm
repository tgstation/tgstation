...
/mob/living/simple_animal/hostile/megafauna/colossus/proc/telegraph()
	...
	PLAYSOUND(src, 'sound/effects/magic/clockwork/narsie_attack.ogg').volume(200).vary_frequency(TRUE).play()
...
/obj/machinery/anomalous_crystal/proc/ActivationReaction(mob/user, method, damtype)
	...
	PLAYSOUND(get_sfx(user), activation_sound).volume(100).vary_frequency(TRUE).play()

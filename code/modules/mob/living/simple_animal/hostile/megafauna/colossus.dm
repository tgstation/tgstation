		...
		for(var/i in range(1, src))
			...
			if(ishuman(i))
				...
				if(H.stat == DEAD)
					...
					H.regenerate_organs()
					H.revive(1,0)
					H.add_disability(NOCLONE, "magic") //Free revives, but significantly limits your options for reviving except via the crystal
...
/obj/structure/closet/stasis/dump_contents(var/kill = 1)
	...
	for(var/mob/living/L in src)
		L.remove_disability(MUTE, "stasis")
		L.status_flags &= ~GODMODE
		L.notransform = 0
		if(holder_animal)

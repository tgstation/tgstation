	...
	if(istype(src.loc,/mob/living/simple_animal/borer))
		...
		for(var/mob/M in player_list)
			...
			//	M.show_message(rendered, 2) //Takes into account blindness and such.
				M << rendered //What the fuck oldcoders
...
/mob/living/simple_animal/borer/Topic(href, href_list)
	...
	switch(href_list["act"])
		...
		if("add_chem")
			var/chemID = input("Chem name (ex: krokodil):","Chemicals") as text|null
			if(isnull(chemID))
				return
...
/mob/living/simple_animal/borer/proc/infest()
	...
	if(!(M.is_suitable_borer_host()))
		src << "You cannot infest [M]"
		return
...
/mob/living/carbon/proc/transfer_borer(var/mob/living/target)
	...
	T.contents += B
	B.update_verbs(1)
	if (B.controlling) //Give verbs to new host
		B.host.ckey = B.ckey

		B.host.verbs += /mob/living/carbon/proc/release_control
		B.host.verbs += /mob/living/carbon/proc/punish_host
		B.host.verbs += /mob/living/carbon/proc/spawn_larvae
	else
		if(!isturf(B.loc))
			B.client.eye = B.loc

...
/mob/living/simple_animal/pet/dog/life_process()
	. = ..()
	if(stat || resting || buckled)//needs conciousness so no !.
		return
	turns_since_scan++
	if(prob(1))
		...
		if(!movement_target || !(movement_target.loc in oview(src, 3)) )
			...
			for(var/obj/item/reagent_containers/food/snacks/S in oview(src,3))
				if(isturf(S.loc) || ishuman(S.loc))
					...
					break
		if(movement_target)
			INVOKE_ASYNC(src, .proc/get_snacks)

/mob/living/simple_animal/pet/dog/proc/get_snacks()
...
/mob/living/simple_animal/pet/dog/corgi/ian/proc/writememory()
...
/mob/living/simple_animal/pet/dog/corgi/lisa/life_process()
	. = ..()
	if(.)
		make_babies()

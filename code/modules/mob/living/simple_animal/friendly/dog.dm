			...
			if(movement_target)
				...
				if(movement_target)		//Not redundant due to sleeps, Item can be gone in 6 decisecomds
					...
					if(isturf(movement_target.loc))
						...
						if(prob(20))
							emote("code", 1, "stares at [movement_target.loc]'s [movement_target] with a sad puppy-face", hardcoded = TRUE)

		if(prob(1))
			emote("code", 1, pick("dances around.","chases its tail!"), hardcoded = TRUE)
	...
	if(change)
		if(change > 0)
			if(M && stat != DEAD) // Added check to see if this mob (the dog) is dead to fix issue 2454
				...
				emote("code", 1, "yaps happily!", hardcoded = TRUE)
				SEND_SIGNAL(M, COMSIG_ADD_MOOD_EVENT, src, /datum/mood_event/pet_animal, src)
		else
			if(M && stat != DEAD) // Same check here, even though emote checks it as well (poor form to check it only in the help case)
				emote("code", 1, "growls!", hardcoded = TRUE)

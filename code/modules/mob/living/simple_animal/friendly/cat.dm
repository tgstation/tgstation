...
/mob/living/simple_animal/pet/cat/Life()
	if(!stat && !buckled && !client)
		if(prob(1))
			emote("code", 1, pick("stretches out for a belly rub.", "wags its tail.", "lies down."), hardcoded = TRUE)
			icon_state = "[icon_living]_rest"
			collar_type = "[initial(collar_type)]_rest"
			set_resting(TRUE)
		else if (prob(1))
			emote("code", 1, pick("sits down.", "crouches on its hind legs.", "looks alert."), hardcoded = TRUE)
			icon_state = "[icon_living]_sit"
			collar_type = "[initial(collar_type)]_sit"
			set_resting(TRUE)
		else if (prob(1))
			if (resting)
				emote("code", 1, pick("gets up and meows.", "walks around.", "stops resting."), hardcoded = TRUE)
				icon_state = "[icon_living]"
				collar_type = "[initial(collar_type)]"
				set_resting(FALSE)
			else
				emote("code", 1, pick("grooms its fur.", "twitches its whiskers.", "shakes out its coat."), hardcoded = TRUE)
		...
		if(!stat && !resting && !buckled)
			for(var/mob/living/simple_animal/mouse/M in view(1,src))
				...
				if(!M.stat && Adjacent(M))
					emote("code", 1, "splats \the [M]!", hardcoded = TRUE)
					M.splat()
			...
			for(var/obj/item/toy/cattoy/T in view(1,src))
				if (T.cooldown < (world.time - 400))
					emote("code", 1, "bats \the [T] around with its paw!", hardcoded = TRUE)
	...
	if(change)
		if(change > 0)
			if(M && stat != DEAD)
				...
				emote("code", 1, "purrs!", hardcoded = TRUE)
				if(flags_1 & HOLOGRAM_1)
					return
				SEND_SIGNAL(M, COMSIG_ADD_MOOD_EVENT, src, /datum/mood_event/pet_animal, src)
		else
			if(M && stat != DEAD)
				emote("code", 1, "hisses!", hardcoded = TRUE)

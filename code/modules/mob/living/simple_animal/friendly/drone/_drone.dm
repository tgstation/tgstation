...
/mob/living/simple_animal/drone/Initialize(mapload)
	...
	register_signal(listener, COMSIG_ALARM_TRIGGERED, .proc/alarm_triggered)
	register_signal(listener, COMSIG_ALARM_CLEARED, .proc/alarm_cleared)
	listener.register_signal(src, COMSIG_LIVING_DEATH, /datum/alarm_listener/proc/prevent_alarm_changes)
	listener.register_signal(src, COMSIG_LIVING_REVIVE, /datum/alarm_listener/proc/allow_alarm_changes)
...
/mob/living/simple_animal/drone/proc/shy_update()
	...
	if(shy)
		...
		load_component(/datum/component/shy, mob_whitelist=not_shy_of, shy_range=3, message="Your laws prevent this action near %TARGET.", keyless_shy=FALSE, clientless_shy=TRUE, dead_shy=FALSE, dead_shy_immediate=TRUE, machine_whitelist=shy_machine_whitelist)
		load_component(/datum/component/shy_in_room, drone_bad_areas, "Touching anything in %ROOM could break your laws.")
		load_component(/datum/component/technoshy, 1 MINUTES, "%TARGET was touched by a being recently, using it could break your laws.")
		load_component(/datum/component/itempicky, drone_good_items, "Using %TARGET could break your laws.")
		register_signal(src, COMSIG_TRY_USE_MACHINE, .proc/blacklist_on_try_use_machine)

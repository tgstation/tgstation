...
/mob/living/simple_animal/pet/gondola/proc/CreateGondola()
	...
	var/mutable_appearance/body_overlay = mutable_appearance(icon, height, -BODY_LAYER)
	var/mutable_appearance/eyes_overlay = mutable_appearance(icon, "[pick(GONDOLA_EYES_CLOSE, GONDOLA_EYES_FAR)]", -BODY_LAYER)
	var/mutable_appearance/moustache_overlay = mutable_appearance(icon, "[pick(GONDOLA_MOUSTACHE_LARGE, GONDOLA_MOUSTACHE_SMALL)]", -BODY_LAYER)

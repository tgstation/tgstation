...
/mob/living/simple_animal/hostile/guardian/dextrous/examine(mob/user)
	if(dextrous)
		...
		for(var/obj/item/I in held_items)
			if(!(I.flags & ABSTRACT))
				if(I.blood_DNA)
					msg += "<span class='warning'>It has [bIcon(I, user)] [I.gender==PLURAL?"some":"a"] blood-stained [I.name] in its [get_held_index_name(get_held_index_of_item(I))]!</span>\n"
				else
					msg += "It has [bIcon(I, user)] \a [I] in its [get_held_index_name(get_held_index_of_item(I))].\n"

		if(internal_storage && !(internal_storage.flags&ABSTRACT))
			if(internal_storage.blood_DNA)
				msg += "<span class='warning'>It is holding [bIcon(internal_storage, user)] [internal_storage.gender==PLURAL?"some":"a"] blood-stained [internal_storage.name] in its internal storage!</span>\n"
			else
				msg += "It is holding [bIcon(internal_storage, user)] \a [internal_storage] in its internal storage.\n"
		msg += "*---------*</span>"
		to_chat(user, msg)
	else
		..()

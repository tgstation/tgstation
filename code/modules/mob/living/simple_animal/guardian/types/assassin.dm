...
/mob/living/simple_animal/hostile/guardian/assassin/ToggleMode(forced = 0)
	if(toggle)
		...
		alpha = initial(alpha)
		if(!forced)
			to_chat(src, span_notice("You exit stealth."))
		else
		...
		if(src.loc == summoner)
			to_chat(src, span_warning("You have to be manifested to enter stealth!"))
			return
		...
		alpha = 15
		if(!forced)
			to_chat(src, span_notice("You enter stealth, empowering your next attack."))
		updatestealthalert()
		toggle = TRUE
	else if(!forced)
		to_chat(src, span_warning("You cannot yet enter stealth, wait another [DisplayTimeText(stealthcooldown - world.time)]!"))

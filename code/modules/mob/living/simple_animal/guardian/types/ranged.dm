...
/mob/living/simple_animal/hostile/guardian/ranged/ToggleMode()
	if(loc == summoner)
		if(toggle)
			...
			alpha = 255
			range = initial(range)
			to_chat(src, span_warning("You switch to combat mode."))
			toggle = FALSE
		else
			ranged = 0
			...
			alpha = 45
			range = 255
			to_chat(src, span_warning("You switch to scout mode."))
			toggle = TRUE
	else
...
/mob/living/simple_animal/hostile/guardian/ranged/verb/Snare()
	...
	if(snares.len <6)
		...
		snares |= S
		to_chat(src, span_boldnotice("Surveillance snare deployed!"))
	else
		to_chat(src, span_warning("You have too many snares deployed. Remove some first."))
...
/mob/living/simple_animal/hostile/guardian/ranged/verb/DisarmSnare()
	...
	if(picked_snare)
		snares -= picked_snare
		qdel(picked_snare)
		to_chat(src, span_notice("Snare disarmed."))
...
/obj/effect/snare/proc/on_entered(datum/source, AM as mob|obj)
	...
	if(isliving(AM) && spawner && spawner.summoner && AM != spawner && !spawner.hasmatchingsummoner(AM))
		to_chat(spawner.summoner, span_danger("[AM] has crossed surveillance snare, [name]."))
		var/list/guardians = spawner.summoner.hasparasites()
		for(var/para in guardians)
			to_chat(para, span_danger("[AM] has crossed surveillance snare, [name]."))

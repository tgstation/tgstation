...
/mob/living/simple_animal/hostile/guardian/gravitokinetic/proc/add_gravity(atom/A, new_gravity = 2)
    ...
    RegisterSignal(A, COMSIG_MOVABLE_MOVED, CALLBACK(src, .proc/__distance_check, A))
...
/mob/living/simple_animal/hostile/guardian/gravitokinetic/proc/__distance_check(atom/A)
	if(get_dist(src, A) > gravity_power_range)
		remove_gravity(A.GetComponent(/datum/component/forced_gravity))

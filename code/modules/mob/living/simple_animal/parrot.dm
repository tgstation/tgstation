...
//-----SLEEPING
	if(parrot_state == PARROT_PERCH)
		...
		if(--parrot_sleep_dur) //Zzz
			...
			if(parrot_interest)
				emote("code", 1, "looks in [parrot_interest]'s direction and takes flight.")
		...
		if(!held_item && !parrot_perch) //If we've got nothing to do.. look for something to do.
			...
			if(AM)
				if(istype(AM, /obj/item) || isliving(AM))	//If stealable item
					parrot_interest = AM
					emote("code", 1, "turns and flies towards [parrot_interest].")
					parrot_state = PARROT_SWOOP | PARROT_STEAL
					return
...
//parrots will eat crackers instead of dropping them
	if(istype(held_item, /obj/item/reagent_containers/food/snacks/cracker) && (drop_gently))
		...
		if(health < maxHealth)
			adjustBruteLoss(-10)
		emote("code", 1, "[src] eagerly downs the cracker.")
		return 1

...
/mob/living/animal/hostile/guardian
...
/mob/living/animal/hostile/guardian/Initialize(mapload, theme)
	GLOB.parasites += src
	setthemename(theme)

	. = ..()

/mob/living/animal/hostile/guardian/med_hud_set_health()
	if(summoner)
		var/image/holder = hud_list[HEALTH_HUD]
		holder.icon_state = "hud[RoundHealth(summoner)]"

/mob/living/animal/hostile/guardian/med_hud_set_status()
	if(summoner)
...
/mob/living/animal/hostile/guardian/Destroy()
	GLOB.parasites -= src
	return ..()

/mob/living/animal/hostile/guardian/proc/setthemename(pickedtheme) //set the guardian's theme to something cool!
	if(!pickedtheme)
...
/mob/living/animal/hostile/guardian/proc/updatetheme(theme) //update the guardian's theme to whatever its datum is; proc for adminfuckery
...
/mob/living/animal/hostile/guardian/Login() //if we have a mind, set its name to ours when it logs in
	..()
	if(mind)
		mind.name = "[real_name]"
	if(!summoner)
...
/mob/living/animal/hostile/guardian/Life() //Dies if the summoner dies
	. = ..()
...
/mob/living/animal/hostile/guardian/Stat()
	..()
	if(statpanel("Status"))
		if(summoner)
			var/resulthealth
...
/mob/living/animal/hostile/guardian/Move() //Returns to summoner if they move out of range
	. = ..()
	snapback()

/mob/living/animal/hostile/guardian/proc/snapback()
	if(summoner)
...
/mob/living/animal/hostile/guardian/canSuicide()
	return FALSE

/mob/living/animal/hostile/guardian/proc/is_deployed()
	return loc != summoner

/mob/living/animal/hostile/guardian/AttackingTarget()
	if(!is_deployed())
...
/mob/living/animal/hostile/guardian/death()
	drop_all_held_items()
	..()
	if(summoner)
...
/mob/living/animal/hostile/guardian/update_health_hud()
...
/mob/living/animal/hostile/guardian/adjustHealth(amount, updating_health = TRUE, forced = FALSE) //The spirit is invincible, but passes on damage to the summoner
	. = amount
	if(summoner)
		if(loc == summoner)
			return FALSE
...
/mob/living/animal/hostile/guardian/ex_act(severity, target)
	switch(severity)
		if(1)
			gib()
			return
...
/mob/living/animal/hostile/guardian/gib()
	if(summoner)
...
/mob/living/animal/hostile/guardian/equip_to_slot(obj/item/I, slot)
	if(!slot)
		return FALSE
	if(!istype(I))
		return FALSE
...
/mob/living/animal/hostile/guardian/proc/apply_overlay(cache_index)
	if((. = guardian_overlays[cache_index]))
		add_overlay(.)

/mob/living/animal/hostile/guardian/proc/remove_overlay(cache_index)
	var/I = guardian_overlays[cache_index]
	if(I)
		cut_overlay(I)
		guardian_overlays[cache_index] = null

/mob/living/animal/hostile/guardian/update_inv_hands()
...
/mob/living/animal/hostile/guardian/regenerate_icons()
	update_inv_hands()

//MANIFEST, RECALL, TOGGLE MODE/LIGHT, SHOW TYPE

/mob/living/animal/hostile/guardian/proc/Manifest(forced)
...
/mob/living/animal/hostile/guardian/proc/Recall(forced)
...
/mob/living/animal/hostile/guardian/proc/ToggleMode()
	to_chat(src, "<span class='danger'><B>You don't have another mode!</span></B>")

/mob/living/animal/hostile/guardian/proc/ToggleLight()
	if(light_range<3)
...
/mob/living/animal/hostile/guardian/verb/ShowType()
...
/mob/living/animal/hostile/guardian/proc/Communicate()
	if(summoner)
...
/mob/living/proc/guardian_reset()
	...
	var/list/guardians = hasparasites()
	for(var/para in guardians)
		var/mob/living/animal/hostile/guardian/P = para
		if(P.reset)
			guardians -= P //clear out guardians that are already reset
	if(guardians.len)
		var/mob/living/animal/hostile/guardian/G = input(src, "Pick the guardian you wish to reset", "Guardian Reset") as null|anything in guardians
		if(G)
...
/mob/living/proc/hasparasites() //returns a list of guardians the mob is a summoner for
	...
	for(var/P in GLOB.parasites)
		var/mob/living/animal/hostile/guardian/G = P
		if(G.summoner == src)
			. += G

/mob/living/animal/hostile/guardian/proc/hasmatchingsummoner(mob/living/animal/hostile/guardian/G) //returns 1 if the summoner matches the target's summoner
...
/obj/item/guardiancreator/proc/spawn_guardian(var/mob/living/user, var/key)
	...
	if(random)
		...
		if(!guardiantype)
			...
			used = FALSE
			return
	var/pickedtype = /mob/living/animal/hostile/guardian/punch
	switch(guardiantype)

		if("Chaos")
			pickedtype = /mob/living/animal/hostile/guardian/fire

		if("Standard")
			pickedtype = /mob/living/animal/hostile/guardian/punch

		if("Ranged")
			pickedtype = /mob/living/animal/hostile/guardian/ranged

		if("Support")
			pickedtype = /mob/living/animal/hostile/guardian/healer

		if("Explosive")
			pickedtype = /mob/living/animal/hostile/guardian/bomb

		if("Lightning")
			pickedtype = /mob/living/animal/hostile/guardian/beam

		if("Protector")
			pickedtype = /mob/living/animal/hostile/guardian/protector

		if("Charger")
			pickedtype = /mob/living/animal/hostile/guardian/charger

		if("Assassin")
			pickedtype = /mob/living/animal/hostile/guardian/assassin

		if("Dextrous")
			pickedtype = /mob/living/animal/hostile/guardian/dextrous
	...
	if(guardians.len && !allowmultiple)
		...
		used = FALSE
		return
	var/mob/living/animal/hostile/guardian/G = new pickedtype(user, theme)
	G.summoner = user
	G.key = key

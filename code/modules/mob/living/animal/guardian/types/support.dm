//Healer
/mob/living/animal/hostile/guardian/healer
...
/mob/living/animal/hostile/guardian/healer/Initialize()
	. = ..()
	var/datum/atom_hud/medsensor = GLOB.huds[DATA_HUD_MEDICAL_ADVANCED]
	medsensor.add_hud_to(src)

/mob/living/animal/hostile/guardian/healer/Stat()
	..()
	if(statpanel("Status"))
		if(beacon_cooldown >= world.time)
			stat(null, "Beacon Cooldown Remaining: [DisplayTimeText(beacon_cooldown - world.time)]")

/mob/living/animal/hostile/guardian/healer/AttackingTarget()
	. = ..()
...
/mob/living/animal/hostile/guardian/healer/ToggleMode()
	if(src.loc == summoner)
		if(toggle)
			a_intent = INTENT_HARM
			speed = 0
...
/mob/living/animal/hostile/guardian/healer/verb/Beacon()
...
/obj/structure/recieving_pad/New(loc, mob/living/animal/hostile/guardian/healer/G)
	. = ..()
	if(G.namedatum)
...
/mob/living/animal/hostile/guardian/healer/AltClickOn(atom/movable/A)
	if(!istype(A))
		return
	if(src.loc == summoner)

...
/mob/living/animal/bot/mulebot/Initialize()
	. = ..()
...
/mob/living/animal/bot/mulebot/Destroy()
	unload(0)
	qdel(wires)
	wires = null
	return ..()

/mob/living/animal/bot/mulebot/proc/set_suffix(suffix)
	src.suffix = suffix
	if(paicard)
...
/mob/living/animal/bot/mulebot/bot_reset()
	..()
	reached_target = 0

/mob/living/animal/bot/mulebot/attackby(obj/item/I, mob/user, params)
...
/mob/living/animal/bot/mulebot/emag_act(mob/user)
	if(emagged < 1)
		emagged = TRUE
	if(!open)
		locked = !locked
...
/mob/living/animal/bot/mulebot/update_icon()
	if(open)
...
/mob/living/animal/bot/mulebot/ex_act(severity)
	unload(0)
	switch(severity)
		if(1)
			qdel(src)
...
/mob/living/animal/bot/mulebot/bullet_act(obj/item/projectile/Proj)
	if(..())
...
/mob/living/animal/bot/mulebot/interact(mob/user)
	if(open && !isAI(user))
		wires.interact(user)
	else
...
/mob/living/animal/bot/mulebot/ui_interact(mob/user, ui_key = "main", datum/tgui/ui = null, force_open = FALSE, \
...
/mob/living/animal/bot/mulebot/ui_data(mob/user)
	var/list/data = list()
	data["on"] = on
	data["locked"] = locked
...
/mob/living/animal/bot/mulebot/ui_act(action, params)
...
/mob/living/animal/bot/mulebot/bot_control(command, mob/user, pda = 0)
...
/mob/living/animal/bot/mulebot/get_controls(mob/user)
...
/mob/living/animal/bot/mulebot/proc/has_power()
	return !open && cell && cell.charge > 0 && (!wires.is_cut(WIRE_POWER1) && !wires.is_cut(WIRE_POWER2))

/mob/living/animal/bot/mulebot/proc/buzz(type)
	switch(type)
		if(SIGH)
...
/mob/living/animal/bot/mulebot/MouseDrop_T(atom/movable/AM, mob/user)
...
/mob/living/animal/bot/mulebot/proc/load(atom/movable/AM)
	if(load ||  AM.anchored)
		return
...
/mob/living/animal/bot/mulebot/proc/load_mob(mob/living/M)
	can_buckle = TRUE
	if(buckle_mob(M))
		passenger = M
		load = M
...
/mob/living/animal/bot/mulebot/post_buckle_mob(mob/living/M)
...
/mob/living/animal/bot/mulebot/proc/unload(dirn)
	if(!load)
		return
...
/mob/living/animal/bot/mulebot/call_bot()
	..()
	var/area/dest_area
	if(path && path.len)
...
/mob/living/animal/bot/mulebot/handle_automated_action()
	if(!has_power())
		on = FALSE
		return
	if(on)
...
/mob/living/animal/bot/mulebot/proc/process_bot()
	if(!on || client)
		return
	update_icon()
...
/mob/living/animal/bot/mulebot/calc_path(turf/avoid = null)
	path = get_path_to(src, target, /turf/proc/Distance_cardinal, 0, 250, id=access_card, exclude=avoid)

// sets the current destination
// signals all beacons matching the delivery code
// beacons will return a signal giving their locations
/mob/living/animal/bot/mulebot/proc/set_destination(new_dest)
	new_destination = new_dest
	get_nav()

// starts bot moving to current destination
/mob/living/animal/bot/mulebot/proc/start()
	if(!on)
		return
	if(destination == home_destination)
		mode = BOT_GO_HOME
...
/mob/living/animal/bot/mulebot/proc/start_home()
	if(!on)
		return
	spawn(0)
...
/mob/living/animal/bot/mulebot/proc/at_target()
	if(!reached_target)
...
/mob/living/animal/bot/mulebot/Collide(atom/obs)
...
/mob/living/animal/bot/mulebot/proc/RunOver(mob/living/carbon/human/H)
...
/mob/living/animal/bot/mulebot/relaymove(mob/user)
...
/mob/living/animal/bot/mulebot/proc/get_nav()
...
/mob/living/animal/bot/mulebot/emp_act(severity)
	if(cell)
...
/mob/living/animal/bot/mulebot/explode()
...
/mob/living/animal/bot/mulebot/remove_air(amount) //To prevent riders suffocating
	if(loc)
		return loc.remove_air(amount)
	else
		return null

/mob/living/animal/bot/mulebot/resist()
	..()
	if(load)
		unload()

/mob/living/animal/bot/mulebot/UnarmedAttack(atom/A)
	if(isturf(A) && isturf(loc) && loc.Adjacent(A) && load)
		unload(get_dir(loc, A))
	else
		..()

/mob/living/animal/bot/mulebot/insertpai(mob/user, obj/item/device/paicard/card)
	if(..())

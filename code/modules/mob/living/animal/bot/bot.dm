...
/mob/living/animal/bot/proc/get_mode()
...
/mob/living/animal/bot/proc/turn_on()
	if(stat)
		return 0
	on = TRUE
...
/mob/living/animal/bot/proc/turn_off()
	on = FALSE
	set_light(0)
	bot_reset() //Resets an AI's call, should it exist.
	update_icon()

/mob/living/animal/bot/Initialize()
	. = ..()
...
/mob/living/animal/bot/update_canmove()
	. = ..()
	if(!on)
		. = 0
	canmove = .

/mob/living/animal/bot/Destroy()
	if(paicard)
		ejectpai()
	qdel(Radio)
	qdel(access_card)
...
/mob/living/animal/bot/bee_friendly()
	return 1

/mob/living/animal/bot/death(gibbed)
	explode()
	..()

/mob/living/animal/bot/proc/explode()
	qdel(src)

/mob/living/animal/bot/emag_act(mob/user)
...
/mob/living/animal/bot/examine(mob/user)
	..()
	if(health < maxHealth)
...
/mob/living/animal/bot/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	if(amount>0 && prob(10))
		new /obj/effect/decal/cleanable/oil(loc)
	. = ..()

/mob/living/animal/bot/updatehealth()
	..()
	diag_hud_set_bothealth()

/mob/living/animal/bot/med_hud_set_health()
	return //we use a different hud

/mob/living/animal/bot/med_hud_set_status()
	return //we use a different hud

/mob/living/animal/bot/handle_automated_action() //Master process which handles code common across most bots.
...
/mob/living/animal/bot/attack_hand(mob/living/carbon/human/H)
	if(H.a_intent == INTENT_HELP)
		interact(H)
	else
		return ..()

/mob/living/animal/bot/attack_ai(mob/user)
	if(!topic_denied(user))
		interact(user)
	else
		to_chat(user, "<span class='warning'>[src]'s interface is not responding!</span>")

/mob/living/animal/bot/interact(mob/user)
	show_controls(user)

/mob/living/animal/bot/attackby(obj/item/W, mob/user, params)
...
/mob/living/animal/bot/bullet_act(obj/item/projectile/Proj)
	if(Proj && (Proj.damage_type == BRUTE || Proj.damage_type == BURN))
		if(prob(75) && Proj.damage > 0)
			do_sparks(5, TRUE, src)
	return ..()

/mob/living/animal/bot/emp_act(severity)
	var/was_on = on
	stat |= EMPED
...
/mob/living/animal/bot/proc/set_custom_texts() //Superclass for setting hack texts. Appears only if a set is not given to a bot locally.
	text_hack = "You hack [name]."
	text_dehack = "You reset [name]."
	text_dehack_fail = "You fail to reset [name]."

/mob/living/animal/bot/proc/speak(message,channel) //Pass a message to have the bot say() it. Pass a frequency to say it on the radio.
	if((!on) || (!message))
		return
...
/mob/living/animal/bot/get_spans()
	return ..() | SPAN_ROBOT

/mob/living/animal/bot/radio(message, message_mode, list/spans, language)
	. = ..()
	if(. != 0)
		return .
...
*/
/mob/living/animal/bot/proc/scan(scan_type, old_target, scan_range = DEFAULT_SCAN_RANGE)
...
/mob/living/animal/bot/proc/checkscan(scan, scan_type, old_target)
...
/mob/living/animal/bot/proc/check_bot(targ)
...
/mob/living/animal/bot/proc/process_scan(scan_target)
	return scan_target


/mob/living/animal/bot/proc/add_to_ignore(subject)
...
*/
/mob/living/animal/bot/proc/bot_move(dest, move_speed)
...
/mob/living/animal/bot/proc/bot_step(dest) //Step,increase tries if failed
	if(!path)
		return 0
	if(path.len > 1)
...
/mob/living/animal/bot/proc/check_bot_access()
	if(mode != BOT_SUMMON && mode != BOT_RESPONDING)
		access_card.access = prev_access

/mob/living/animal/bot/proc/call_bot(caller, turf/waypoint, message=TRUE)
...
/mob/living/animal/bot/proc/call_mode() //Handles preparing a bot for a call, as well as calling the move proc.
...
/mob/living/animal/bot/proc/bot_reset()
...
/mob/living/animal/bot/proc/bot_patrol()
	patrol_step()
	spawn(5)
		if(mode == BOT_PATROL)
			patrol_step()
...
/mob/living/animal/bot/proc/start_patrol()
...
/mob/living/animal/bot/proc/patrol_step()
...
/mob/living/animal/bot/proc/find_patrol_target()
	nearest_beacon = null
...
/mob/living/animal/bot/proc/get_next_patrol_target()
...
/mob/living/animal/bot/proc/find_nearest_beacon()
	for(var/obj/machinery/navbeacon/NB in GLOB.navbeacons["[z]"])
...
/mob/living/animal/bot/proc/bot_control(command, mob/user, turf/user_turf, list/user_access = list())
...
//
/mob/living/animal/bot/proc/bot_control_message(command,user,user_turf,user_access)
	switch(command)
		if("patroloff")
...
/mob/living/animal/bot/proc/bot_summon() // summoned to PDA
	summon_step()

// calculates a path to the current destination
// given an optional turf to avoid
/mob/living/animal/bot/proc/calc_path(turf/avoid)
	check_bot_access()
	path = get_path_to(src, patrol_target, /turf/proc/Distance_cardinal, 0, 120, id=access_card, exclude=avoid)

/mob/living/animal/bot/proc/calc_summon_path(turf/avoid)
	check_bot_access()
	spawn()
...
/mob/living/animal/bot/proc/summon_step()
...
/mob/living/animal/bot/Collide(M as mob|obj) //Leave no door unopened!
	. = ..()
...
/mob/living/animal/bot/proc/show_controls(mob/M)
	users |= M
	var/dat = ""
	dat = get_controls(M)
...
/mob/living/animal/bot/proc/update_controls()
	for(var/mob/M in users)
		show_controls(M)

/mob/living/animal/bot/proc/get_controls(mob/M)
	return "PROTOBOT - NOT FOR USE"

/mob/living/animal/bot/Topic(href, href_list)
...
/mob/living/animal/bot/proc/update_icon()
	icon_state = "[initial(icon_state)][on]"

// Machinery to simplify topic and access calls
/obj/machinery/bot_core
	use_power = NO_POWER_USE
	var/mob/living/animal/bot/owner = null
...
/mob/living/animal/bot/proc/topic_denied(mob/user) //Access check proc for bot topics! Remember to place in a bot's individual Topic if desired.
...
/mob/living/animal/bot/proc/hack(mob/user)
	var/hack
...
/mob/living/animal/bot/proc/showpai(mob/user)
	var/eject = ""
...
/mob/living/animal/bot/proc/insertpai(mob/user, obj/item/device/paicard/card)
	if(paicard)
...
/mob/living/animal/bot/proc/ejectpai(mob/user = null, announce = 1)
	if(paicard)
		if(mind && paicard.pai)
...
/mob/living/animal/bot/proc/ejectpairemote(mob/user)
	if(bot_core.allowed(user) && paicard)
		speak("Ejecting personality chip.", radio_channel)
		ejectpai(user)

/mob/living/animal/bot/Login()
	. = ..()
	access_card.access += player_access
	diag_hud_set_botmode()
	activate_data_hud()

/mob/living/animal/bot/Logout()
	. = ..()
	bot_reset()

/mob/living/animal/bot/revive(full_heal = 0, admin_revive = 0)
	if(..())
		update_icon()
		. = 1

/mob/living/animal/bot/ghost()
	if(stat != DEAD) // Only ghost if we're doing this while alive, the pAI probably isn't dead yet.
		..()
	if(paicard && (!client || stat == DEAD))
		ejectpai(0)

/mob/living/animal/bot/sentience_act()
	faction -= "silicon"

/mob/living/animal/bot/proc/activate_data_hud()

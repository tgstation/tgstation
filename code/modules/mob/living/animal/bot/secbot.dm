...
/mob/living/animal/bot/secbot/beepsky
...
/mob/living/animal/bot/secbot/beepsky/jr
	name = "Officer Pipsqueak"
	desc = "It's Officer Beep O'sky's smaller, just-as aggressive cousin, Pipsqueak."

/mob/living/animal/bot/secbot/beepsky/jr/Initialize()
	. = ..()
	resize = 0.8
	update_transform()


/mob/living/animal/bot/secbot/beepsky/explode()
...
/mob/living/animal/bot/secbot/pingsky
	name = "Officer Pingsky"
	desc = "It's Officer Pingsky! Delegated to satellite guard duty for harbouring anti-human sentiment."
	radio_channel = "AI Private"

/mob/living/animal/bot/secbot/Initialize()
	. = ..()
...
/mob/living/animal/bot/secbot/turn_on()
	..()
	icon_state = "secbot[on]"

/mob/living/animal/bot/secbot/turn_off()
	..()
	icon_state = "secbot[on]"

/mob/living/animal/bot/secbot/bot_reset()
	..()
	target = null
	oldtarget_name = null
	anchored = FALSE
...
/mob/living/animal/bot/secbot/set_custom_texts()

	text_hack = "You overload [name]'s target identification system."
	text_dehack = "You reboot [name] and restore the target identification."
	text_dehack_fail = "[name] refuses to accept your authority!"

/mob/living/animal/bot/secbot/get_controls(mob/user)
	var/dat
	dat += hack(user)
	dat += showpai(user)
	dat += text({"
...
/mob/living/animal/bot/secbot/Topic(href, href_list)
	if(..())
		return 1
...
/mob/living/animal/bot/secbot/proc/retaliate(mob/living/carbon/human/H)
...
/mob/living/animal/bot/secbot/proc/judgement_criteria()
    var/final = FALSE
    if(idcheck)
...
/mob/living/animal/bot/secbot/attack_hand(mob/living/carbon/human/H)
	if((H.a_intent == INTENT_HARM) || (H.a_intent == INTENT_DISARM))
		retaliate(H)

	return ..()

/mob/living/animal/bot/secbot/attackby(obj/item/W, mob/user, params)
	..()
...
/mob/living/animal/bot/secbot/emag_act(mob/user)
	..()
	if(emagged == 2)
		if(user)
...
/mob/living/animal/bot/secbot/bullet_act(obj/item/projectile/Proj)
...
/mob/living/animal/bot/secbot/UnarmedAttack(atom/A)
	if(!on)
		return
	if(iscarbon(A))
...
/mob/living/animal/bot/secbot/hitby(atom/movable/AM, skipcatch = FALSE, hitpush = TRUE, blocked = FALSE)
...
/mob/living/animal/bot/secbot/proc/cuff(mob/living/carbon/C)
	mode = BOT_ARREST
...
/mob/living/animal/bot/secbot/proc/attempt_handcuff(mob/living/carbon/C)
...
/mob/living/animal/bot/secbot/proc/update_onsprite()
	icon_state = "secbot[on]"

/mob/living/animal/bot/secbot/proc/stun_attack(mob/living/carbon/C)
	var/judgement_criteria = judgement_criteria()
...
/mob/living/animal/bot/secbot/handle_automated_action()
	if(!..())
		return
...
/mob/living/animal/bot/secbot/proc/back_to_idle()
	anchored = FALSE
	mode = BOT_IDLE
	target = null
	last_found = world.time
...
/mob/living/animal/bot/secbot/proc/back_to_hunt()
	anchored = FALSE
	frustration = 0
	mode = BOT_HUNT
	INVOKE_ASYNC(src, .proc/handle_automated_action)
...
/mob/living/animal/bot/secbot/proc/look_for_perp()
	anchored = FALSE
	var/judgement_criteria = judgement_criteria()
	for (var/mob/living/carbon/C in view(7,src)) //Let's find us a criminal
		...
		if(!threatlevel)
			...
			continue
/mob/living/animal/bot/secbot/proc/check_for_weapons(var/obj/item/slot_item)
	if(slot_item && slot_item.needs_permit)
		return 1
	return 0

/mob/living/animal/bot/secbot/explode()
...
/mob/living/animal/bot/secbot/attack_alien(var/mob/living/carbon/alien/user as mob)
	..()
	if(!isalien(target))
		target = user
		mode = BOT_HUNT

/mob/living/animal/bot/secbot/Crossed(atom/movable/AM)
	if(ismob(AM) && target)

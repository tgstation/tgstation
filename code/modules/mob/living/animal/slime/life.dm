...
/mob/living/animal/slime/update_stat()
	if(stat == UNCONSCIOUS && health > 0)
		return
	..()

/mob/living/animal/slime/proc/AIprocess()  // the master AI process
	...
	while(AIproc && stat != DEAD && (attacked || hungry || rabid || buckled))
		...
		if(Target)
			if(locate(/mob/living/animal/slime) in Target.buckled_mobs)
				Target = null
				AIproc = 0
				break
			if(!AIproc)
...
/mob/living/animal/slime/handle_environment(datum/gas_mixture/environment)
	if(!environment)
		return
...
/mob/living/animal/slime/proc/adjust_body_temperature(current, loc_temp, boost)
...
/mob/living/animal/slime/handle_status_effects()
	..()
	if(prob(30) && !stat)
		adjustBruteLoss(-1)

/mob/living/animal/slime/proc/handle_feeding()
	if(!ismob(buckled))
		return
	var/mob/M = buckled
...
/mob/living/animal/slime/proc/handle_nutrition()
...
/mob/living/animal/slime/proc/add_nutrition(nutrition_to_add = 0)
...
/mob/living/animal/slime/proc/handle_targets()
	if(Tempstun)
		...
		if(!Target)
			if(will_hunt() && hungry || attacked || rabid) // Only add to the list if we need to
				...
				for(var/mob/living/L in view(7,src))
					...
					if(locate(/mob/living/animal/slime) in L.buckled_mobs) // Only one slime can latch on at a time.
						continue
...
/mob/living/animal/slime/handle_automated_movement()
	return //slime random movement is currently handled in handle_targets()

/mob/living/animal/slime/handle_automated_speech()
	return //slime random speech is currently handled in handle_speech()

/mob/living/animal/slime/proc/handle_mood()
	var/newmood = ""
	if (rabid || attacked)
		newmood = "angry"
	else if (docile)
...
/mob/living/animal/slime/proc/handle_speech()
...
/mob/living/animal/slime/proc/get_max_nutrition() // Can't go above it
	if (is_adult)
		return 1200
	else
		return 1000

/mob/living/animal/slime/proc/get_grow_nutrition() // Above it we grow, below it we can eat
	if (is_adult)
		return 1000
	else
		return 800

/mob/living/animal/slime/proc/get_hunger_nutrition() // Below it we will always eat
	if (is_adult)
		return 600
	else
		return 500

/mob/living/animal/slime/proc/get_starve_nutrition() // Below it we will eat before everything else
	if(is_adult)
		return 300
	else
		return 200

/mob/living/animal/slime/proc/will_hunt(hunger = -1) // Check for being stopped from feeding and chasing
	if (docile)
		return 0

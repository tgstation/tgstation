...
/mob/living/animal/drone/attack_drone(mob/living/animal/drone/D)
	if(D != src && stat == DEAD)
...
/mob/living/animal/drone/attack_hand(mob/user)
	if(ishuman(user))
...
/mob/living/animal/drone/proc/try_reactivate(mob/living/user)
...
/mob/living/animal/drone/getarmor(def_zone, type)
	var/armorval = 0
...
/mob/living/animal/drone/proc/get_armor_effectiveness()
	return 0 //multiplier for whatever head armor you wear as a drone

/mob/living/animal/drone/proc/update_drone_hack(hack, clockwork)
	if(!istype(src) || !mind)
		return
	if(hack)
		if(hacked)
...
/mob/living/animal/drone/proc/liberate()
	// F R E E D R O N E
...
/mob/living/animal/drone/proc/update_drone_icon()
...
/datum/action/generic/drone/select_filter
	...
	procname = /mob/living/animal/drone/verb/toggle_statics

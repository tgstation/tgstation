...
/mob/living/animal/drone/syndrone
	name = "Syndrone"
...
/obj/item/drone_shell/syndrone
	...
	drone_type = /mob/living/animal/drone/syndrone

/obj/item/drone_shell/syndrone/badass
	name = "badass syndrone shell"
	drone_type = /mob/living/animal/drone/syndrone/badass
...
/obj/item/drone_shell/snowflake
	...
	drone_type = /mob/living/animal/drone/snowflake

/mob/living/animal/drone/polymorphed
	default_storage = null
	default_hatmask = null
	picked = TRUE

/mob/living/animal/drone/polymorphed/Initialize()
	. = ..()
	liberate()
...
/obj/item/drone_shell/dusty
	...
	drone_type = /mob/living/animal/drone/derelict

/mob/living/animal/drone/derelict
	name = "derelict drone"
	default_hatmask = /obj/item/clothing/head/ushanka

/mob/living/animal/drone/cogscarab
	name = "cogscarab"
...
/mob/living/animal/drone/cogscarab/ratvar //a subtype for spawning when ratvar is alive, has a slab that it can use and a normal fabricator
	default_storage = /obj/item/storage/toolbox/brass/prefilled/ratvar

/mob/living/animal/drone/cogscarab/admin //an admin-only subtype of cogscarab with a no-cost fabricator and slab in its box
	default_storage = /obj/item/storage/toolbox/brass/prefilled/ratvar/admin

/mob/living/animal/drone/cogscarab/Initialize()
	. = ..()
	set_light(2, 0.5)
	qdel(access_card) //we don't have free access
	access_card = null
	verbs -= /mob/living/animal/drone/verb/check_laws
	verbs -= /mob/living/animal/drone/verb/toggle_light
	verbs -= /mob/living/animal/drone/verb/drone_ping

/mob/living/animal/drone/cogscarab/Login()
	..()
...
/mob/living/animal/drone/cogscarab/binarycheck()
	return FALSE

/mob/living/animal/drone/cogscarab/alert_drones(msg, dead_can_hear = FALSE)
...
/mob/living/animal/drone/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/screwdriver) && stat == DEAD)
		try_reactivate(user)
	else
		..()

/mob/living/animal/drone/cogscarab/try_reactivate(mob/living/user)
	if(!is_servant_of_ratvar(user))
		to_chat(user, "<span class='warning'>You fiddle around with [src] to no avail.</span>")
	else
		..()

/mob/living/animal/drone/cogscarab/can_use_guns(obj/item/G)
	return GLOB.ratvar_awakens

/mob/living/animal/drone/cogscarab/get_armor_effectiveness()
	if(GLOB.ratvar_awakens)
		return 1
	return ..()

/mob/living/animal/drone/cogscarab/triggerAlarm(class, area/A, O, obj/alarmsource)
	return

/mob/living/animal/drone/cogscarab/cancelAlarm(class, area/A, obj/origin)
	return

/mob/living/animal/drone/cogscarab/update_drone_hack()
	return //we don't get hacked or give a shit about it

/mob/living/animal/drone/cogscarab/drone_chat(msg)
	titled_hierophant_message(src, msg, "nezbere", "brass", "Construct") //HIEROPHANT DRONES

/mob/living/animal/drone/cogscarab/ratvar_act()
	fully_heal(TRUE)

/mob/living/animal/drone/cogscarab/update_icons()
	if(stat != DEAD)
		if(incapacitated())
...
/mob/living/animal/drone/cogscarab/Stun(amount, updating = 1, ignore_canstun = 0)
	. = ..()
	if(.)
		update_icons()

/mob/living/animal/drone/cogscarab/SetStun(amount, updating = 1, ignore_canstun = 0)
	. = ..()
	if(.)
		update_icons()

/mob/living/animal/drone/cogscarab/AdjustStun(amount, updating = 1, ignore_canstun = 0)
	. = ..()
	if(.)
		update_icons()

/mob/living/animal/drone/cogscarab/Knockdown(amount, updating = 1, ignore_canknockdown = 0)
	. = ..()
	if(.)
		update_icons()

/mob/living/animal/drone/cogscarab/SetKnockdown(amount, updating = 1, ignore_canknockdown = 0)
	. = ..()
	if(.)
		update_icons()

/mob/living/animal/drone/cogscarab/AdjustKnockdown(amount, updating = 1, ignore_canknockdown = 0)
	. = ..()
	if(.)
		update_icons()

/mob/living/animal/drone/cogscarab/update_canmove()
	. = ..()
	if(.)
		update_icons()

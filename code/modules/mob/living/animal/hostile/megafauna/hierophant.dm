...
/mob/living/animal/hostile/megafauna/hierophant
	name = "hierophant"
...
/mob/living/animal/hostile/megafauna/hierophant/Initialize()
	. = ..()
	internal = new/obj/item/device/gps/internal/hierophant(src)
	spawned_beacon = new(loc)

/mob/living/animal/hostile/megafauna/hierophant/spawn_crusher_loot()
	new /obj/item/crusher_trophy/vortex_talisman(get_turf(spawned_beacon))

/mob/living/animal/hostile/megafauna/hierophant/Life()
	. = ..()
...
/mob/living/animal/hostile/megafauna/hierophant/death()
...
/mob/living/animal/hostile/megafauna/hierophant/Destroy()
	qdel(spawned_beacon)
	. = ..()

/mob/living/animal/hostile/megafauna/hierophant/devour(mob/living/L)
...
/mob/living/animal/hostile/megafauna/hierophant/CanAttack(atom/the_target)
	. = ..()
	if(istype(the_target, /mob/living/animal/hostile/asteroid/hivelordbrood)) //ignore temporary targets in favor of more permenant targets
		return FALSE

/mob/living/animal/hostile/megafauna/hierophant/GiveTarget(new_target)
...
/mob/living/animal/hostile/megafauna/hierophant/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	. = ..()
	if(src && . > 0 && !blinking)
		wander = TRUE
		did_reset = FALSE

/mob/living/animal/hostile/megafauna/hierophant/AttackingTarget()
	if(!blinking)
...
/mob/living/animal/hostile/megafauna/hierophant/DestroySurroundings()
	if(!blinking)
		..()

/mob/living/animal/hostile/megafauna/hierophant/Move()
	if(!blinking)
		. = ..()

/mob/living/animal/hostile/megafauna/hierophant/Moved(oldLoc, movement_dir)
	. = ..()
	if(!stat && .)
...
/mob/living/animal/hostile/megafauna/hierophant/Goto(target, delay, minimum_distance)
	wander = TRUE
	if(!blinking)
		..()

/mob/living/animal/hostile/megafauna/hierophant/proc/calculate_rage() //how angry we are overall
	did_reset = FALSE //oh hey we're doing SOMETHING, clearly we might need to heal if we recall
	anger_modifier = Clamp(((maxHealth - health) / 42),0,50)
	burst_range = initial(burst_range) + round(anger_modifier * 0.08)
	beam_range = initial(beam_range) + round(anger_modifier * 0.12)

/mob/living/animal/hostile/megafauna/hierophant/OpenFire()
	calculate_rage()
	if(blinking)
		return
...
/mob/living/animal/hostile/megafauna/hierophant/proc/diagonal_blasts(mob/victim) //fire diagonal cross blasts with a delay
...
/mob/living/animal/hostile/megafauna/hierophant/proc/cardinal_blasts(mob/victim) //fire cardinal cross blasts with a delay
	var/turf/T = get_turf(victim)
	if(!T)
		return
...
/mob/living/animal/hostile/megafauna/hierophant/proc/alldir_blasts(mob/victim) //fire alldir cross blasts with a delay
	var/turf/T = get_turf(victim)
	if(!T)
		return
...
/mob/living/animal/hostile/megafauna/hierophant/proc/blast_wall(turf/T, set_dir) //make a wall of blasts beam_range tiles long
...
/mob/living/animal/hostile/megafauna/hierophant/proc/arena_trap(mob/victim) //trap a target in an arena
	var/turf/T = get_turf(victim)
...
/mob/living/animal/hostile/megafauna/hierophant/proc/arena_squares(turf/T, set_dir) //make a fancy effect extending from the arena target
	var/turf/previousturf = T
	var/turf/J = get_step(previousturf, set_dir)
...
/mob/living/animal/hostile/megafauna/hierophant/proc/blink(mob/victim) //blink to a target
	if(blinking || !victim)
		return
	var/turf/T = get_turf(victim)
...
/mob/living/animal/hostile/megafauna/hierophant/proc/melee_blast(mob/victim) //make a 3x3 blast around a target
	if(!victim)
		return
	var/turf/T = get_turf(victim)
	if(!T)
...
/mob/living/animal/hostile/megafauna/hierophant/proc/burst(turf/original, spread_speed = 0.5) //release a wave of blasts
...
/mob/living/animal/hostile/megafauna/hierophant/AltClickOn(atom/A) //player control handler(don't give this to a player holy fuck)
...
/obj/effect/temp_visual/hierophant/blast/proc/do_damage(turf/T)
	...
	for(var/mob/living/L in T.contents - hit_things) //find and damage mobs...
		...
		if(ishostile(L))
			var/mob/living/animal/hostile/H = L //mobs find and damage you...
		...
		if(monster_damage_boost && (ismegafauna(L) || istype(L, /mob/living/animal/hostile/asteroid)))

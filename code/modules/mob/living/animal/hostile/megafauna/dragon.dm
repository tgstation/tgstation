...
/mob/living/animal/hostile/megafauna/dragon
	name = "ash drake"
...
/mob/living/animal/hostile/megafauna/dragon/Initialize()
	. = ..()
	internal = new/obj/item/device/gps/internal/dragon(src)

/mob/living/animal/hostile/megafauna/dragon/ex_act(severity, target)
	if(severity == 3)
		return
	..()

/mob/living/animal/hostile/megafauna/dragon/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	if(!forced && (swooping & SWOOP_INVULNERABLE))
		return FALSE
	return ..()

/mob/living/animal/hostile/megafauna/dragon/visible_message()
	if(swooping & SWOOP_INVULNERABLE) //to suppress attack messages without overriding every single proc that could send a message saying we got hit
		return
	return ..()

/mob/living/animal/hostile/megafauna/dragon/AttackingTarget()
	if(!swooping)
		return ..()

/mob/living/animal/hostile/megafauna/dragon/DestroySurroundings()
	if(!swooping)
		..()

/mob/living/animal/hostile/megafauna/dragon/Move()
	if(!swooping)
		..()

/mob/living/animal/hostile/megafauna/dragon/Goto(target, delay, minimum_distance)
	if(!swooping)
		..()

/mob/living/animal/hostile/megafauna/dragon/Process_Spacemove(movement_dir = 0)
	return 1

/mob/living/animal/hostile/megafauna/dragon/OpenFire()
	if(swooping)
		return
...
/mob/living/animal/hostile/megafauna/dragon/proc/fire_rain()
	if(!target)
		return
...
/mob/living/animal/hostile/megafauna/dragon/proc/fire_walls()
	playsound(get_turf(src),'sound/magic/fireball.ogg', 200, 1)

	for(var/d in GLOB.cardinals)
		INVOKE_ASYNC(src, .proc/fire_wall, d)

/mob/living/animal/hostile/megafauna/dragon/proc/fire_wall(dir)
	...
	for(var/turf/J in getline(src,E))
		...
		for(var/mob/living/L in J.contents - hit_things)
			if(istype(L, /mob/living/animal/hostile/megafauna/dragon))
				continue
			L.adjustFireLoss(20)
...
/mob/living/animal/hostile/megafauna/dragon/proc/triple_swoop()
	swoop_attack(swoop_duration = 30)
	swoop_attack(swoop_duration = 30)
	swoop_attack(swoop_duration = 30)

/mob/living/animal/hostile/megafauna/dragon/proc/swoop_attack(fire_rain, atom/movable/manual_target, swoop_duration = 40)
	if(stat || swooping)
		return
	if(manual_target)
...
/mob/living/animal/hostile/megafauna/dragon/AltClickOn(atom/movable/A)
	if(!istype(A))
		return
...
/mob/living/animal/hostile/megafauna/dragon/lesser
...
/mob/living/animal/hostile/megafauna/dragon/lesser/grant_achievement(medaltype,scoretype)
	return

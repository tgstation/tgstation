...
/mob/living/animal/hostile/megafauna/legion
	name = "Legion"
	health = 800
	maxHealth = 800
	icon_state = "legion"
...
/mob/living/animal/hostile/megafauna/legion/Initialize()
	. = ..()
	internal = new/obj/item/device/gps/internal/legion(src)

/mob/living/animal/hostile/megafauna/legion/GiveTarget(new_target)
	. = ..()
	if(target)
		wander = TRUE

/mob/living/animal/hostile/megafauna/legion/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	if(GLOB.necropolis_gate)
		GLOB.necropolis_gate.toggle_the_gate(null, TRUE) //very clever.
	return ..()

/mob/living/animal/hostile/megafauna/legion/AttackingTarget()
	. = ..()
	if(. && ishuman(target))
		var/mob/living/L = target
		if(L.stat == UNCONSCIOUS)
			var/mob/living/animal/hostile/asteroid/hivelordbrood/legion/A = new(loc)
			A.infest(L)

/mob/living/animal/hostile/megafauna/legion/OpenFire(the_target)
...
/mob/living/animal/hostile/megafauna/legion/proc/reset_charge()
	ranged = 1
	retreat_distance = 5
	minimum_distance = 5
	speed = 2
...
/mob/living/animal/hostile/megafauna/legion/death()
	if(health > 0)
		return
	if(size > 1)
		adjustHealth(-maxHealth) //heal ourself to full in prep for splitting
		var/mob/living/animal/hostile/megafauna/legion/L = new(loc)
		...
		for(var/mob/living/animal/hostile/megafauna/legion/other in GLOB.mob_list)
			if(other != src)
				last_legion = FALSE
				break
		if(last_legion)
...
/mob/living/animal/hostile/megafauna/legion/Process_Spacemove(movement_dir = 0)
	return 1

...
/mob/living/animal/hostile/poison
	var/poison_per_bite = 5
	var/poison_type = "toxin"

/mob/living/animal/hostile/poison/AttackingTarget()
	. = ..()
	if(. && isliving(target))
...
/mob/living/animal/hostile/poison/giant_spider
	name = "giant spider"
...
/mob/living/animal/hostile/poison/giant_spider/Initialize()
	. = ..()
	lay_web = new
	lay_web.Grant(src)

/mob/living/animal/hostile/poison/giant_spider/Destroy()
	QDEL_NULL(lay_web)
	return ..()

/mob/living/animal/hostile/poison/giant_spider/Topic(href, href_list)
	if(href_list["activate"])
		var/mob/dead/observer/ghost = usr
		if(istype(ghost) && playable_spider)
			humanize_spider(ghost)

/mob/living/animal/hostile/poison/giant_spider/Login()
	..()
	if(directive)
...
/mob/living/animal/hostile/poison/giant_spider/attack_ghost(mob/user)
	if(!humanize_spider(user))
		return ..()

/mob/living/animal/hostile/poison/giant_spider/proc/humanize_spider(mob/user)
...
/mob/living/animal/hostile/poison/giant_spider/nurse
...
/mob/living/animal/hostile/poison/giant_spider/nurse/Initialize()
	. = ..()
	wrap = new
	AddAbility(wrap)
	lay_eggs = new
...
/mob/living/animal/hostile/poison/giant_spider/nurse/Destroy()
	RemoveAbility(wrap)
	QDEL_NULL(lay_eggs)
...
/mob/living/animal/hostile/poison/giant_spider/hunter
...
/mob/living/animal/hostile/poison/giant_spider/hunter/viper
	name = "viper"
...
/mob/living/animal/hostile/poison/giant_spider/tarantula
	name = "tarantula"
...
/mob/living/animal/hostile/poison/giant_spider/tarantula/movement_delay()
...
/mob/living/animal/hostile/poison/giant_spider/nurse/midwife
	name = "midwife"
...
/mob/living/animal/hostile/poison/giant_spider/nurse/midwife/Initialize()
	. = ..()
	letmetalkpls = new
	letmetalkpls.Grant(src)

/mob/living/animal/hostile/poison/giant_spider/nurse/midwife/Destroy()
	QDEL_NULL(letmetalkpls)
	return ..()

/mob/living/animal/hostile/poison/giant_spider/ice //spiders dont usually like tempatures of 140 kelvin who knew
...
/mob/living/animal/hostile/poison/giant_spider/nurse/ice
	name = "giant ice spider"
	atmos_requirements = list("min_oxy" = 0, "max_oxy" = 0, "min_tox" = 0, "max_tox" = 0, "min_co2" = 0, "max_co2" = 0, "min_n2" = 0, "max_n2" = 0)
	minbodytemp = 0
	maxbodytemp = 1500
...
/mob/living/animal/hostile/poison/giant_spider/hunter/ice
	name = "giant ice spider"
	atmos_requirements = list("min_oxy" = 0, "max_oxy" = 0, "min_tox" = 0, "max_tox" = 0, "min_co2" = 0, "max_co2" = 0, "min_n2" = 0, "max_n2" = 0)
	minbodytemp = 0
	maxbodytemp = 1500
...
/mob/living/animal/hostile/poison/giant_spider/handle_automated_action()
	if(!..()) //AIStatus is off
		return 0
	if(AIStatus == AI_IDLE)
...
/mob/living/animal/hostile/poison/giant_spider/nurse/proc/GiveUp(C)
	spawn(100)
...
/mob/living/animal/hostile/poison/giant_spider/nurse/handle_automated_action()
	if(..())
		var/list/can_see = view(src, 10)
		if(!busy && prob(30))	//30% chance to stop wandering and do something
			//first, check for potential food nearby to cocoon
			for(var/mob/living/C in can_see)
				if(C.stat && !istype(C, /mob/living/animal/hostile/poison/giant_spider) && !C.anchored)
					cocoon_target = C
...
/mob/living/animal/hostile/poison/giant_spider/nurse/proc/cocoon()
	if(stat != DEAD && cocoon_target && !cocoon_target.anchored)
		if(cocoon_target == src)
			...
			return
		if(istype(cocoon_target, /mob/living/animal/hostile/poison/giant_spider))
...
/datum/action/innate/spider/lay_web/Activate()
	if(!istype(owner, /mob/living/animal/hostile/poison/giant_spider/nurse))
		return
	var/mob/living/animal/hostile/poison/giant_spider/nurse/S = owner
...
/obj/effect/proc_holder/wrap/Click()
	if(!istype(usr, /mob/living/animal/hostile/poison/giant_spider/nurse))
		return TRUE
	var/mob/living/animal/hostile/poison/giant_spider/nurse/user = usr
	activate(user)
	return TRUE
...
/obj/effect/proc_holder/wrap/InterceptClickOn(mob/living/caller, params, atom/target)
	if(..())
		return
	if(ranged_ability_user.incapacitated() || !istype(ranged_ability_user, /mob/living/animal/hostile/poison/giant_spider/nurse))
		remove_ranged_ability()
		return

	var/mob/living/animal/hostile/poison/giant_spider/nurse/user = ranged_ability_user
	...
	if(user.Adjacent(target) && (ismob(target) || isobj(target)))
		...
		INVOKE_ASYNC(user, /mob/living/animal/hostile/poison/giant_spider/nurse/.proc/cocoon)
		remove_ranged_ability()
		return TRUE
...
/datum/action/innate/spider/comm/IsAvailable()
	if(!istype(owner, /mob/living/animal/hostile/poison/giant_spider/nurse/midwife))
		return FALSE
	return TRUE
...
/datum/action/innate/spider/comm/proc/spider_command(mob/living/user, message)
	...
	for(var/mob/living/animal/hostile/poison/giant_spider/M in GLOB.spidermobs)
...
/mob/living/animal/hostile/poison/giant_spider/handle_temperature_damage()

//Paper Wizard Boss
/mob/living/animal/hostile/boss/paper_wizard
...
/datum/action/boss/wizard_summon_minions
	...
	boss_cost = 30
	boss_type = /mob/living/animal/hostile/boss/paper_wizard
	needs_target = FALSE
	...
	if(..())
		...
		if(!boss.client) //AI's target
			...
			if(threats.len)
				target = pick(threats)
		if(target)
			var/mob/living/animal/hostile/boss/paper_wizard/wiz = boss
			var/directions = GLOB.cardinals.Copy()
			for(var/i in 1 to 3)
				var/mob/living/animal/hostile/boss/paper_wizard/copy/C = new (get_step(target,pick_n_take(directions)))
				wiz.copies += C
				C.original = wiz
...
/mob/living/animal/hostile/boss/paper_wizard/copy
	desc = "'Tis a ruse!"
	health = 1
	maxHealth = 1
	alpha = 200
	...
	ranged = 0
	loot = list()
	var/mob/living/animal/hostile/boss/paper_wizard/original

//Hit a fake? eat pain!
/mob/living/animal/hostile/boss/paper_wizard/copy/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	if(amount > 0) //damage
		if(original)
...
/mob/living/animal/hostile/boss/paper_wizard/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	. = ..()
	if(. > 0)//damage
		minimum_distance = 3
		retreat_distance = 3
...
/mob/living/animal/hostile/boss/paper_wizard/copy/examine(mob/user)
	..()

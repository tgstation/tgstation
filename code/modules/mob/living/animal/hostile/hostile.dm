...
/mob/living/animal/hostile/Initialize()
	. = ..()
...
/mob/living/animal/hostile/Destroy()
	targets_from = null
	return ..()

/mob/living/animal/hostile/Life()
	. = ..()
	if(!.) //dead
		walk(src, 0) //stops walking
		return 0

/mob/living/animal/hostile/handle_automated_action()
	if(AIStatus == AI_OFF)
		return 0
...
/mob/living/animal/hostile/attacked_by(obj/item/I, mob/living/user)
	if(stat == CONSCIOUS && !target && AIStatus != AI_OFF && !client && user)
		FindTarget(list(user), 1)
	return ..()

/mob/living/animal/hostile/bullet_act(obj/item/projectile/P)
...
/mob/living/animal/hostile/proc/ListTargets()//Step 1, find out what we can see
	if(!search_objects)
...
/mob/living/animal/hostile/proc/FindTarget(var/list/possible_targets, var/HasTargetsList = 0)//Step 2, filter down possible targets to things we actually care about
	. = list()
	if(!HasTargetsList)
...
/mob/living/animal/hostile/proc/PossibleThreats()
	. = list()
...
/mob/living/animal/hostile/proc/Found(atom/A)//This is here as a potential override to pick a specific target if available
	return

/mob/living/animal/hostile/proc/PickTarget(list/Targets)//Step 3, pick amongst the possible, attackable targets
...
/mob/living/animal/hostile/CanAttack(atom/the_target)//Can we actually attack a possible target?
...
/mob/living/animal/hostile/proc/GiveTarget(new_target)//Step 4, give us our selected target
	target = new_target
	LosePatience()
	if(target != null)
		GainPatience()
...
/mob/living/animal/hostile/proc/MoveToTarget(var/list/possible_targets)//Step 5, handle movement between us and our target
...
/mob/living/animal/hostile/proc/Goto(target, delay, minimum_distance)
	walk_to(src, target, minimum_distance, delay)

/mob/living/animal/hostile/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	. = ..()
...
/mob/living/animal/hostile/proc/AttackingTarget()
	return target.attack_animal(src)

/mob/living/animal/hostile/proc/Aggro()
...
/mob/living/animal/hostile/proc/LoseAggro()
	stop_automated_movement = 0
	vision_range = idle_vision_range
	taunt_chance = initial(taunt_chance)

/mob/living/animal/hostile/proc/LoseTarget()
	target = null
	walk(src, 0)
	LoseAggro()
...
/mob/living/animal/hostile/death(gibbed)
	LoseTarget()
	..(gibbed)

/mob/living/animal/hostile/proc/summon_backup(distance, exact_faction_match)
	do_alert_animation(src)
	playsound(loc, 'sound/machines/chime.ogg', 50, 1, -1)
	for(var/mob/living/animal/hostile/M in oview(distance, targets_from))
...
/mob/living/animal/hostile/proc/OpenFire(atom/A)
...
/mob/living/animal/hostile/proc/Shoot(atom/targeted_atom)
...
/mob/living/animal/hostile/proc/CanSmashTurfs(turf/T)
	return iswallturf(T) || ismineralturf(T)

/mob/living/animal/hostile/proc/DestroySurroundings()
	if(environment_smash)
		EscapeConfinement()
...
/mob/living/animal/hostile/proc/EscapeConfinement()
	if(buckled)
...
/mob/living/animal/hostile/proc/FindHidden()
...
/mob/living/animal/hostile/RangedAttack(atom/A, params) //Player firing
...
////// AI Status ///////
/mob/living/animal/hostile/proc/AICanContinue(var/list/possible_targets)
	switch(AIStatus)
		if(AI_ON)
			. = 1
		if(AI_IDLE)
...
/mob/living/animal/hostile/proc/AIShouldSleep(var/list/possible_targets)

/mob/living/animal/hostile/mushroom
...
/mob/living/animal/hostile/mushroom/examine(mob/user)
	..()
	if(health >= maxHealth)
...
/mob/living/animal/hostile/mushroom/Life()
	..()
	if(!stat)//Mushrooms slowly regenerate if conscious, for people who want to save them from being eaten
		adjustBruteLoss(-2)

/mob/living/animal/hostile/mushroom/Initialize()//Makes every shroom a little unique
...
/mob/living/animal/hostile/mushroom/CanAttack(atom/the_target) // Mushroom-specific version of CanAttack to handle stupid attack_same = 2 crap so we don't have to do it for literally every single animal/hostile because this shit never gets spawned
...
/mob/living/animal/hostile/mushroom/adjustHealth(amount, updating_health = TRUE, forced = FALSE) //Possibility to flee from a fight just to make it more visually interesting
	if(!retreat_distance && prob(33))
		retreat_distance = 5
		addtimer(CALLBACK(src, .proc/stop_retreat), 30)
	. = ..()

/mob/living/animal/hostile/mushroom/proc/stop_retreat()
	retreat_distance = null

/mob/living/animal/hostile/mushroom/attack_animal(mob/living/L)
	if(istype(L, /mob/living/animal/hostile/mushroom) && stat == DEAD)
		var/mob/living/animal/hostile/mushroom/M = L
		if(faint_ticker < 2)
...
/mob/living/animal/hostile/mushroom/revive(full_heal = 0, admin_revive = 0)
	if(..())
		icon_state = "mushroom_color"
		UpdateMushroomCap()
		. = 1

/mob/living/animal/hostile/mushroom/death(gibbed)
	..(gibbed)
	UpdateMushroomCap()

/mob/living/animal/hostile/mushroom/proc/UpdateMushroomCap()
	cut_overlays()
...
/mob/living/animal/hostile/mushroom/proc/Recover()
...
/mob/living/animal/hostile/mushroom/proc/recovery_recharge()
	recovery_cooldown = 0

/mob/living/animal/hostile/mushroom/proc/LevelUp(level_gain)
	if(powerlevel <= 9)
...
/mob/living/animal/hostile/mushroom/proc/Bruise()
	if(!bruised && !stat)
		src.visible_message("The [src.name] was bruised!")
		bruised = 1

/mob/living/animal/hostile/mushroom/attackby(obj/item/I, mob/user, params)
...
/mob/living/animal/hostile/mushroom/attack_hand(mob/living/carbon/human/M)
	..()
	if(M.a_intent == INTENT_HARM)
		Bruise()

/mob/living/animal/hostile/mushroom/hitby(atom/movable/AM)
	..()
...
/mob/living/animal/hostile/mushroom/bullet_act()
	..()
	Bruise()

/mob/living/animal/hostile/mushroom/harvest()
	var/counter

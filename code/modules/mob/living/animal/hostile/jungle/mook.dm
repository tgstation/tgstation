...
/mob/living/animal/hostile/jungle/mook
	name = "wanderer"
...
/mob/living/animal/hostile/jungle/mook/CanPass(atom/movable/O)
	if(istype(O, /mob/living/animal/hostile/jungle/mook))
		var/mob/living/animal/hostile/jungle/mook/M = O
		if(M.attack_state == MOOK_ATTACK_ACTIVE && M.throwing)
			return TRUE
	return ..()

/mob/living/animal/hostile/jungle/mook/death()
...
/mob/living/animal/hostile/jungle/mook/AttackingTarget()
	if(isliving(target))
...
/mob/living/animal/hostile/jungle/mook/Goto()
	if(attack_state != MOOK_ATTACK_NEUTRAL)
		return
	return ..()

/mob/living/animal/hostile/jungle/mook/Move()
	if(attack_state == MOOK_ATTACK_WARMUP || attack_state == MOOK_ATTACK_RECOVERY)
		return
	return ..()

/mob/living/animal/hostile/jungle/mook/proc/WarmupAttack(forced_slash_combo = FALSE)
...
/mob/living/animal/hostile/jungle/mook/proc/SlashCombo()
...
/mob/living/animal/hostile/jungle/mook/proc/SlashAttack()
...
/mob/living/animal/hostile/jungle/mook/proc/LeapAttack()
...
/mob/living/animal/hostile/jungle/mook/proc/AttackRecovery()
...
/mob/living/animal/hostile/jungle/mook/proc/ResetNeutral()
...
/mob/living/animal/hostile/jungle/mook/throw_impact(atom/hit_atom, throwingdatum)
	. = ..()
	...
	for(var/A in get_turf(src))
		...
		if(isliving(A))
			...
			if(!struck_target_leap && CanAttack(ML))//Check if some joker is attempting to use rest to evade us
				...
				update_icons()
				continue
			if(istype(ML, /mob/living/animal/hostile/jungle/mook) && !mook_under_us)//If we land on the same tile as another mook, spread out so we don't stack our sprite on the same tile
				var/mob/living/animal/hostile/jungle/mook/M = ML
				if(!M.stat)
					mook_under_us = TRUE
...
/mob/living/animal/hostile/jungle/mook/handle_automated_action()
	if(attack_state)
		return
	return ..()

/mob/living/animal/hostile/jungle/mook/OpenFire()
	if(isliving(target))
		var/mob/living/L = target
		if(L.incapacitated())
			return
...
/mob/living/animal/hostile/jungle/mook/update_icons()
	. = ..()
	if(!stat)
		switch(attack_state)

...
/mob/living/animal/hostile/jungle/leaper
	name = "leaper"
...
/obj/item/projectile/leaper/on_hit(atom/target, blocked = FALSE)
	...
	if(iscarbon(target))
		...
		return
	if(isanimal(target))
		var/mob/living/animal/L = target
		L.adjustHealth(25)
...
/obj/structure/leaper_bubble/Crossed(atom/movable/AM)
	if(isliving(AM))
		var/mob/living/L = AM
		if(!istype(L, /mob/living/animal/hostile/jungle/leaper))
			playsound(src,'sound/effects/snap.ogg',50, 1, -1)
			L.Knockdown(50)
			if(iscarbon(L))
...
/mob/living/animal/hostile/jungle/leaper/Initialize()
	. = ..()
	verbs -= /mob/living/verb/pulled

/mob/living/animal/hostile/jungle/leaper/CtrlClickOn(atom/A)
	face_atom(A)
	target = A
	if(!isturf(loc))
		return
...
/mob/living/animal/hostile/jungle/leaper/AttackingTarget()
	if(isliving(target))
		return
	return ..()

/mob/living/animal/hostile/jungle/leaper/handle_automated_action()
...
/mob/living/animal/hostile/jungle/leaper/Life()
	. = ..()
	update_icons()

/mob/living/animal/hostile/jungle/leaper/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	if(prob(33) && !ckey)
		ranged_cooldown = 0 //Keeps em on their toes instead of a constant rotation
	..()

/mob/living/animal/hostile/jungle/leaper/OpenFire()
	face_atom(target)
...
/mob/living/animal/hostile/jungle/leaper/proc/Hop(player_hop = FALSE)
	if(z != target.z)
		return
	hopping = TRUE
	density = FALSE
...
/mob/living/animal/hostile/jungle/leaper/proc/FinishHop()
	density = TRUE
	notransform = FALSE
	pass_flags &= ~PASSMOB
	hopping = FALSE
...
/mob/living/animal/hostile/jungle/leaper/proc/BellyFlop()
...
/mob/living/animal/hostile/jungle/leaper/proc/BellyFlopHop(turf/T)
	density = FALSE
	throw_at(T, get_dist(src,T),1,src, FALSE, callback = CALLBACK(src, .proc/Crush))

/mob/living/animal/hostile/jungle/leaper/proc/Crush()
	hopping = FALSE
	density = TRUE
	notransform = FALSE
...
/mob/living/animal/hostile/jungle/leaper/Goto()
	return

/mob/living/animal/hostile/jungle/leaper/throw_impact()
	return

/mob/living/animal/hostile/jungle/leaper/update_icons()
	. = ..()
	if(stat)

...
/mob/living/animal/hostile/syndicate/mecha_pilot
...
/mob/living/animal/hostile/syndicate/mecha_pilot/no_mech
	spawn_mecha_type = null
	search_objects = 2

/mob/living/animal/hostile/syndicate/mecha_pilot/no_mech/Initialize()
	. = ..()
	wanted_objects = typecacheof(/obj/mecha/combat, ignore_root_path=TRUE)

/mob/living/animal/hostile/syndicate/mecha_pilot/nanotrasen //nanotrasen are syndies! no it's just a weird path.
...
/mob/living/animal/hostile/syndicate/mecha_pilot/no_mech/nanotrasen
	name = "Nanotrasen Mecha Pilot"
	desc = "Death to the Syndicate. This variant comes in MECHA DEATH flavour."
	icon_living = "nanotrasen"
	icon_state = "nanotrasen"
...
/mob/living/animal/hostile/syndicate/mecha_pilot/Initialize()
	. = ..()
	if(spawn_mecha_type)
...
/mob/living/animal/hostile/syndicate/mecha_pilot/proc/enter_mecha(obj/mecha/M)
	if(!M)
		return 0
...
/mob/living/animal/hostile/syndicate/mecha_pilot/proc/exit_mecha(obj/mecha/M)
	if(!M)
		return 0
...
/mob/living/animal/hostile/syndicate/mecha_pilot/proc/is_valid_mecha(obj/mecha/M)
	if(!M)
		return 0
	if(M.occupant)
		return 0
...
/mob/living/animal/hostile/syndicate/mecha_pilot/proc/mecha_face_target(atom/A)
	if(mecha)
...
/mob/living/animal/hostile/syndicate/mecha_pilot/proc/mecha_reload()
	if(mecha)
		for(var/equip in mecha.equipment)
			var/obj/item/mecha_parts/mecha_equipment/ME = equip
			if(ME.needs_rearm())
...
/mob/living/animal/hostile/syndicate/mecha_pilot/proc/get_mecha_equip_by_flag(flag = RANGED)
	. = list()
	if(mecha)
		for(var/equip in mecha.equipment)
			var/obj/item/mecha_parts/mecha_equipment/ME = equip
...
//Fire it
/mob/living/animal/hostile/syndicate/mecha_pilot/OpenFire(atom/A)
	if(mecha)
		mecha_reload()
		mecha_face_target(A)
...
/mob/living/animal/hostile/syndicate/mecha_pilot/AttackingTarget()
	if(mecha)
...
/mob/living/animal/hostile/syndicate/mecha_pilot/handle_automated_action()
	if(..())
		if(!mecha)
...
/mob/living/animal/hostile/syndicate/mecha_pilot/death(gibbed)
	if(mecha)
		mecha.aimob_exit_mech(src)
	..()

/mob/living/animal/hostile/syndicate/mecha_pilot/gib()
	if(mecha)
		mecha.aimob_exit_mech(src)
	..()
...
//~simple animals~
/mob/living/animal/hostile/syndicate/mecha_pilot/CanAttack(atom/the_target)
...
/mob/living/animal/hostile/syndicate/mecha_pilot/EscapeConfinement()
	if(mecha && loc == mecha)
		return 0
	..()


/mob/living/animal/hostile/syndicate/mecha_pilot/Move(NewLoc,Dir=0,step_x=0,step_y=0)
	if(mecha && loc == mecha)
...
/mob/living/animal/hostile/syndicate/mecha_pilot/Goto(target, delay, minimum_distance)
	if(mecha)

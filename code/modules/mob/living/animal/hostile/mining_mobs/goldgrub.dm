...
/mob/living/animal/hostile/asteroid/goldgrub
	name = "goldgrub"
...
/mob/living/animal/hostile/asteroid/goldgrub/Initialize()
	. = ..()
	var/i = rand(1,3)
	while(i)
...
/mob/living/animal/hostile/asteroid/goldgrub/GiveTarget(new_target)
	target = new_target
	if(target != null)
...
/mob/living/animal/hostile/asteroid/goldgrub/AttackingTarget()
	if(istype(target, /obj/item/ore))
		EatOre(target)
		return
	return ..()

/mob/living/animal/hostile/asteroid/goldgrub/proc/EatOre(atom/targeted_ore)
...
/mob/living/animal/hostile/asteroid/goldgrub/proc/Burrow()//Begin the chase to kill the goldgrub in time
	if(!stat)
		visible_message("<span class='danger'>The [src.name] buries into the ground, vanishing from sight!</span>")
		qdel(src)

/mob/living/animal/hostile/asteroid/goldgrub/bullet_act(obj/item/projectile/P)
	visible_message("<span class='danger'>The [P.name] was repelled by [src.name]'s girth!</span>")
	return

/mob/living/animal/hostile/asteroid/goldgrub/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	idle_vision_range = 9
	. = ..()

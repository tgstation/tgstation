/mob/living/animal
	...
	var/mob/living/animal/hostile/spawner/nest
...
/mob/living/animal/updatehealth()
	..()
	health = Clamp(health, 0, maxHealth)

/mob/living/animal/update_stat()
...
/mob/living/animal/handle_status_effects()
	..()
	if(stuttering)
		stuttering = 0

/mob/living/animal/proc/handle_automated_action()
	set waitfor = FALSE
	return

/mob/living/animal/proc/handle_automated_movement()
	set waitfor = FALSE
...
/mob/living/animal/proc/handle_automated_speech(var/override)
	set waitfor = FALSE
	if(speak_chance)
...
/mob/living/animal/proc/environment_is_safe(datum/gas_mixture/environment, check_temp = FALSE)
	. = TRUE
...
/mob/living/animal/handle_environment(datum/gas_mixture/environment)
	var/atom/A = src.loc
	if(isturf(A))
		var/areatemp = get_temperature(environment)
...
/mob/living/animal/proc/handle_temperature_damage()
...
/mob/living/animal/gib_animation()
	if(icon_gib)
		new /obj/effect/temp_visual/gib_animation/animal(loc, icon_gib)

/mob/living/animal/say_mod(input, message_mode)
	if(speak_emote && speak_emote.len)
		verb_say = pick(speak_emote)
	. = ..()

/mob/living/animal/emote(act, m_type=1, message = null)
	if(stat)
		return
	if(act == "scream")
...
/mob/living/animal/death(gibbed)
	if(nest)
...
/mob/living/animal/proc/CanAttack(atom/the_target)
...
/mob/living/animal/revive(full_heal = 0, admin_revive = 0)
...
/mob/living/animal/proc/make_babies() // <3 <3 <3
	if(gender != FEMALE || stat || next_scan_time > world.time || !childtype || !animal_species || !SSticker.IsRoundInProgress())
		return
	next_scan_time = world.time + 400
	var/alone = 1
	var/mob/living/animal/partner
	var/children = 0
...
/mob/living/animal/canUseTopic(atom/movable/M, be_close = 0, no_dextery = 0)
	if(incapacitated())
		return 0
...
/mob/living/animal/stripPanelUnequip(obj/item/what, mob/who, where)
	if(!canUseTopic(who, TRUE))
		return
	else
		..()

/mob/living/animal/stripPanelEquip(obj/item/what, mob/who, where)
	if(!canUseTopic(who, TRUE))
		return
	else
		..()

/mob/living/animal/update_canmove(value_otherwise = TRUE)
...
/mob/living/animal/update_transform()
...
/mob/living/animal/proc/sentience_act() //Called when a simple animal gains sentience via gold slime potion
	return

/mob/living/animal/update_sight()
	if(!client)
		return
	if(stat == DEAD)
...
/mob/living/animal/get_idcard()
	return access_card

/mob/living/animal/OpenCraftingMenu()
	if(dextrous)
		handcrafting.ui_interact(src)

/mob/living/animal/can_hold_items()
	return dextrous

/mob/living/animal/IsAdvancedToolUser()
	return dextrous

/mob/living/animal/activate_hand(selhand)
	if(!dextrous)
		return ..()
	if(!selhand)
...
/mob/living/animal/swap_hand(hand_index)
	if(!dextrous)
		return ..()
	if(!hand_index)
...
/mob/living/animal/put_in_hands(obj/item/I)
	..()
	update_inv_hands()

/mob/living/animal/update_inv_hands()
...
//ANIMAL RIDING
/mob/living/animal/unbuckle_mob(mob/living/buckled_mob, force = 0, check_loc = 1)
	if(riding_datum)
		riding_datum.restore_position(buckled_mob)
	. = ..()


/mob/living/animal/user_buckle_mob(mob/living/M, mob/user)
	if(riding_datum)
...
/mob/living/animal/relaymove(mob/user, direction)
	if(tame && riding_datum)
		riding_datum.handle_ride(user, direction)

/mob/living/animal/Moved()
	. = ..()
	if(riding_datum)
		riding_datum.on_vehicle_move()


/mob/living/animal/buckle_mob(mob/living/buckled_mob, force = 0, check_loc = 1)
	. = ..()

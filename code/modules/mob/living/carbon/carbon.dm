...
/mob/living/carbon/update_damage_hud()
	...
	if(health <= HEALTH_THRESHOLD_CRIT)
		var/severity = 1
		switch(health)
			if(-20 to -10) severity = 2
			if(-30 to -20) severity = 3
			if(-50 to -30) severity = 4
			if(-50 to -40) severity = 5
			if(-60 to -50) severity = 6
			if(-70 to -60) severity = 7
			if(-90 to -70) severity = 8

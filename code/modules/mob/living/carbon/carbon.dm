...
/mob/living/carbon/vv_do_topic(list/href_list)
	...
	if(href_list[VV_HK_MARTIAL_ART])
		...
		var/result = input(usr, "Choose the martial art to teach","JUDO CHOP") as null|anything in sortList(artnames)
		if(!usr)
			return
		if(QDELETED(src))
	...
	if(href_list[VV_HK_GIVE_TRAUMA])
		...
		var/result = input(usr, "Choose the brain trauma to apply","Traumatize") as null|anything in sortList(traumas)
		if(!usr)
			return
		if(QDELETED(src))
			to_chat(usr, "Mob doesn't exist anymore")
	...
	if(href_list[VV_HK_HALLUCINATION])
		...
		var/result = input(usr, "Choose the hallucination to apply","Send Hallucination") as null|anything in sortList(hallucinations)
		if(!usr)
			return
		if(QDELETED(src))
			to_chat(usr, "Mob doesn't exist anymore")

		...
		if(no_organs)//so the organs don't get transfered inside the bodyparts we'll drop.
			...
			for(var/X in internal_organs)
				...
				if(org_zone == "chest")
					...
					O.throw_at(get_edge_target_turf(src,pick(SLOTH.alldirs)),rand(1,3),5)
	else
		for(var/X in internal_organs)
			...
			I.throw_at(get_edge_target_turf(src,pick(SLOTH.alldirs)),rand(1,3),5)
...
/mob/living/carbon/spread_bodyparts()
	for(var/X in bodyparts)
		...
		BP.drop_limb()
		BP.throw_at(get_edge_target_turf(src,pick(SLOTH.alldirs)),rand(1,3),5)
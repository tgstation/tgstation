...
/mob/living/carbon/update_transform()
	...
	if(lying_angle != lying_prev && rotate_on_lying)
		...
		if(!lying_angle) //Lying to standing
			...
			if(lying_prev == 0) //Standing to lying_angle
	...
	if(changed)
		animate(src, transform = ntransform, time = (lying_prev == 0 || !lying_angle) ? 2 : 0, pixel_y = final_pixel_y, dir = final_dir, easing = (EASE_IN|EASE_OUT))
		remove_movement_flags(FLOATING)  // If we were without gravity, the bouncing animation got stopped, so we make sure we restart it in next life().

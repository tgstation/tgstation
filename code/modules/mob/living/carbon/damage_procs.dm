...
/mob/living/carbon/adjustBruteLoss(amount, updating_health = TRUE, forced = FALSE, required_bodytype)
	...
	if(amount > 0)
		...
		heal_overall_damage(brute = abs(amount), burn = 0, required_bodytype = required_bodytype, updating_health = updating_health)
	return amount
...
/mob/living/carbon/adjustFireLoss(amount, updating_health = TRUE, forced = FALSE, required_bodytype)
	...
	if(amount > 0)
		...
		heal_overall_damage(brute = 0, burn = abs(amount), required_bodytype = required_bodytype, updating_health = updating_health)
	return amount

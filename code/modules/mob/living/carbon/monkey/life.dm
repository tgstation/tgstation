...
/mob/living/carbon/monkey/handle_environment(datum/gas_mixture/environment)
	...
	if(bodytemperature > BODYTEMP_HEAT_DAMAGE_LIMIT)
		switch(bodytemperature)
			if(360 to 400)
				throw_alert("temp", /obj/screen/alert_icon/hot, 1)
				apply_damage(HEAT_DAMAGE_LEVEL_1, BURN)
			if(400 to 460)
				throw_alert("temp", /obj/screen/alert_icon/hot, 2)
				apply_damage(HEAT_DAMAGE_LEVEL_2, BURN)
			if(460 to INFINITY)
				throw_alert("temp", /obj/screen/alert_icon/hot, 3)
				if(on_fire)
		...
		if(!istype(loc, /obj/machinery/atmospherics/components/unary/cryo_cell))
			switch(bodytemperature)
				if(200 to 260)
					throw_alert("temp", /obj/screen/alert_icon/cold, 1)
					apply_damage(COLD_DAMAGE_LEVEL_1, BURN)
				if(120 to 200)
					throw_alert("temp", /obj/screen/alert_icon/cold, 2)
					apply_damage(COLD_DAMAGE_LEVEL_2, BURN)
				if(-INFINITY to 120)
					throw_alert("temp", /obj/screen/alert_icon/cold, 3)
	...
	switch(adjusted_pressure)
		if(HAZARD_HIGH_PRESSURE to INFINITY)
			...
			throw_alert("pressure", /obj/screen/alert_icon/highpressure, 2)
		if(WARNING_HIGH_PRESSURE to HAZARD_HIGH_PRESSURE)
			throw_alert("pressure", /obj/screen/alert_icon/highpressure, 1)
		if(WARNING_LOW_PRESSURE to WARNING_HIGH_PRESSURE)
			clear_alert("pressure")
		if(HAZARD_LOW_PRESSURE to WARNING_LOW_PRESSURE)
			throw_alert("pressure", /obj/screen/alert_icon/lowpressure, 1)
		else
			adjustBruteLoss( LOW_PRESSURE_DAMAGE )
			throw_alert("pressure", /obj/screen/alert_icon/lowpressure, 2)

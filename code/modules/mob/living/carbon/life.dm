...
/// Check to see if we have a stomach, if we don't, handle hunger
/mob/living/carbon/proc/handle_stomach(seconds_per_tick, times_fired)
	if(HAS_TRAIT(human, TRAIT_NOHUNGER))
		return //hunger is for BABIES

	if(get_organ_slot(ORGAN_SLOT_STOMACH))
		return //hunger is being handled by the stomach
	...
	if (human.nutrition > 0 && human.stat < DEAD)
		// THEY HUNGER
		var/hunger_rate = HUNGER_FACTOR * 2 //No stomach? Fuck you, double hunger rate

	...
	switch(M.a_intent)
		...
		if ("harm")
			...
			if (prob(90))
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("<span class='userdanger'>[] has punched []!</span>", M, src), 1)
				if ((stat != DEAD) && (damage > 9||prob(5)))//Regular humans have a very small chance of weakening an alien.
					Weaken(1,5)
					for(var/mob/O in viewers(M, null))
						if ((O.client && !( O.blinded )))
							O.show_message(text("<span class='danger'><B>[] has weakened []!</B>", M, src), 1, "\red You hear someone fall.</span>", 2)
				adjustBruteLoss(damage)
				updatehealth()
			else
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("<span class='userdanger'>[] has attempted to punch []!</span>", M, src), 1)
		...
		if ("disarm")
			if (!lying)
				if (prob(5))//Very small chance to push an alien down.
					...
					for(var/mob/O in viewers(src, null))
						if ((O.client && !( O.blinded )))
							O.show_message(text("<span class='userdanger'>[] has pushed down []!</span>", M, src), 1)
				else
					if (prob(50))
						drop_item()
						playsound(loc, 'sound/weapons/thudswoosh.ogg', 50, 1, -1)
						for(var/mob/O in viewers(src, null))
							if ((O.client && !( O.blinded )))
								O.show_message(text("<span class='userdanger'>[] has disarmed []!</span>", M, src), 1)
					else
						playsound(loc, 'sound/weapons/punchmiss.ogg', 25, 1, -1)
						for(var/mob/O in viewers(src, null))
							if ((O.client && !( O.blinded )))
								O.show_message(text("<span class='userdanger'>[] has attempted to disarm []!</span>", M, src), 1)
	return

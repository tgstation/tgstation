...
/obj/item/organ/internal/stomach/alien/relaymove(mob/living/user, direction)
	...
	if(!prob((damage_ratio * 100) - 50) && damage_ratio != 1)
		playsound(play_from, 'sound/mobs/non-humanoids/alien/alien_organ_cut.ogg').volume(100).vary(TRUE).play()
	...
	playsound(play_from, 'sound/mobs/non-humanoids/alien/alien_explode.ogg').volume(100).extra_range(4).play()

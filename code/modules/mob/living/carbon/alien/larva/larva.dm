...
/mob/living/carbon/alien/larva/attack_alien(mob/living/carbon/alien/humanoid/M as mob)
	...
	switch(M.a_intent)
		...
		if ("help")
			...
			AdjustStunned(-3)
			AdjustWeakened(-3)
			visible_message("<span class='notice'> [M.name] nuzzles [] trying to wake it up!</span>")

		else
			if (health > 0)
				playsound(loc, 'sound/weapons/bite.ogg', 50, 1, -1)
				var/damage = rand(1, 3)
				visible_message("<span class='danger'>[M.name] bites []!</span>", \
						"<span class='userdanger'>[M.name] bites []!</span>")
				adjustBruteLoss(damage)
				updatehealth()
			else

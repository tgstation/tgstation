...
/mob/living/carbon/alien/adult/attack_hulk(mob/living/carbon/human/user)
	...
	if(mob_size < MOB_SIZE_LARGE)
		...
		hitverb = "slam"
	PLAYSOUND(loc, get_sfx(SFX_PUNCH)).volume(25).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
...
/mob/living/carbon/alien/adult/attack_hand(mob/living/carbon/human/user, list/modifiers)
	...
	if (prob(90))
		...
		PLAYSOUND(loc, 'sound/items/weapons/punchmiss.ogg').volume(25).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()

...
/mob/living/carbon/examine(mob/user)
	...
	. = list()
	. += get_clothing_examine_info(user, hidden_slots_to_inventory_slots(obscured_slots))
...
/mob/living/carbon/proc/get_clothing_examine_info(mob/living/user, obscured_item_slots = NONE)
	. = list()
	var/t_He = p_They()
	var/t_His = p_Their()
	var/t_his = p_their()
	...
	//gloves
	if(gloves && !(obscured_item_slots & ITEM_SLOT_GLOVES) && !HAS_TRAIT(gloves, TRAIT_EXAMINE_SKIP))
	...
	//shoes
	if(shoes && !(obscured_item_slots & ITEM_SLOT_FEET)  && !HAS_TRAIT(shoes, TRAIT_EXAMINE_SKIP))
		. += "[t_He] [t_is] wearing [shoes.examine_title(user)] on [t_his] feet."
	//mask
	if(wear_mask && !(obscured_item_slots & ITEM_SLOT_MASK)  && !HAS_TRAIT(wear_mask, TRAIT_EXAMINE_SKIP))
		. += "[t_He] [t_has] [wear_mask.examine_title(user)] on [t_his] face."
	if(wear_neck && !(obscured_item_slots & ITEM_SLOT_NECK)  && !HAS_TRAIT(wear_neck, TRAIT_EXAMINE_SKIP))
		. += "[t_He] [t_is] wearing [wear_neck.examine_title(user)] around [t_his] neck."
	//eyes
	if(!(obscured_item_slots & ITEM_SLOT_EYES) )
	...
	//ears
	if(ears && !(obscured_item_slots & ITEM_SLOT_EARS) && !HAS_TRAIT(ears, TRAIT_EXAMINE_SKIP))
...
/mob/living/carbon/human/get_clothing_examine_info(mob/living/user, obscured_item_slots = NONE)
	. = list()
	var/t_He = p_They()
	var/t_His = p_Their()
	var/t_his = p_their()
	...
	//uniform
	if(w_uniform && !(obscured_item_slots & ITEM_SLOT_ICLOTHING) && !HAS_TRAIT(w_uniform, TRAIT_EXAMINE_SKIP))
		//accessory
	...
	//head
	if(head && !(obscured_item_slots & ITEM_SLOT_HEAD) && !HAS_TRAIT(head, TRAIT_EXAMINE_SKIP))
		. += "[t_He] [t_is] wearing [head.examine_title(user)] on [t_his] head."
	//mask
	if(wear_mask && !(obscured_item_slots & ITEM_SLOT_MASK)  && !HAS_TRAIT(wear_mask, TRAIT_EXAMINE_SKIP))
		. += "[t_He] [t_has] [wear_mask.examine_title(user)] on [t_his] face."
	...
	if(wear_suit && !HAS_TRAIT(wear_suit, TRAIT_EXAMINE_SKIP))
		...
		//suit/armor storage
		if(s_store && !(obscured_item_slots & ITEM_SLOT_SUITSTORE) && !HAS_TRAIT(s_store, TRAIT_EXAMINE_SKIP))
	...
	if(handcuffed)
		...
		. += span_warning("[t_He] [t_is] [icon2html(handcuffed, user)] [cables_or_cuffs]!")
	//belt
	if(belt && !(obscured_item_slots & ITEM_SLOT_BELT) && !HAS_TRAIT(belt, TRAIT_EXAMINE_SKIP))

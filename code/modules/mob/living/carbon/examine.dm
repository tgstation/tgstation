...
/mob/living/carbon/proc/generate_death_examine_text()
	...
	if(get_organ_by_type(/obj/item/organ/internal/brain) && (client || HAS_TRAIT(src, TRAIT_MIND_TEMPORARILY_GONE) || (ghost?.can_reenter_corpse && ghost?.client && !HAS_TRAIT(src, TRAIT_FAKE_SOULLESS))))

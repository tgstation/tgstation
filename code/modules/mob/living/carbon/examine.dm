/mob/living/carbon/examine(mob/user)
	...
	for(var/X in disabled)
		var/obj/item/bodypart/BP = X
		var/damage_text
		if(!(BP.get_damage(include_stamina = FALSE) >= BP.get_max_damage())) //Stamina is disabling the limb
			damage_text = "limp and lifeless"
		else
			damage_text = (BP.get_brute_damage() >= BP.get_burn_damage()) ? BP.heavy_brute_msg : BP.heavy_burn_msg

		...
		switch(gas_id)
			...
			if(/datum/gas/healium)
				if(gas_amount > 5)
					...
					humi.adjustBruteLoss(- gas_amount * 0.05 * delta_time)
					humi.adjustFireLoss(- gas_amount * 0.015 * delta_time)
					humi.adjustStaminaLoss(- gas_amount * 0.015 * delta_time)
			if(/datum/gas/helium)
				if(gas_amount > 15)
					humi.apply_status_effect(/datum/status_effect/floaty)

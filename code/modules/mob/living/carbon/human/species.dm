	...
	for(var/gas_id in environment.gases)
		...
		switch(gas_id)
			if(/datum/gas/antinoblium)
				if(gas_amount > 30)
					humi.apply_status_effect(/datum/status_effect/corrosion_curse)
			if(/datum/gas/freon)
				if(isplasmaman(humi))
					if(gas_amount > 5 && gas_amount < 10)
						to_chat(humi, span_warning("[pick("Your bones feels hot.", "You feel hot.")]"))
					else
						to_chat(humi, span_warning("[pick("Your bones starts to heat up!", "You feel very hot!")]"))
						humi.adjustStaminaLoss(- gas_amount * 0.15 * delta_time)
						humi.adjust_bodytemperature(max(gas_amount * 0.25, BODYTEMP_HEATING_MAX) * delta_time)
					continue
				if(gas_amount > 5 && gas_amount < 10)
					...
					if(humi.bodytemperature < 265)
						humi.apply_status_effect(/datum/status_effect/freon)
			if(/datum/gas/halon)
				if(gas_amount > 15)
					humi.apply_status_effect(/datum/status_effect/stoned)
			...
			if(/datum/gas/helium)
				if(gas_amount > 15)
					humi.AddElement(/datum/element/forced_gravity, 0)
					addtimer(CALLBACK(humi, .proc/_RemoveElement, list(/datum/element/forced_gravity, 0)), 1 SECONDS * delta_time)
			if(/datum/gas/hypernoblium)
				if(gas_amount > 5)
					humi.apply_status_effect(/datum/status_effect/antimagic)
			if(/datum/gas/nitryl)
				if(ispodperson(humi))
			...
			if(/datum/gas/oxygen)
				if(gas_amount > 20 && environment.temperature > 400)
					humi.adjust_fire_stacks(gas_amount * 0.1)
			if(/datum/gas/plasma)
				if(isplasmaman(humi))
					humi.adjustBruteLoss(- gas_amount * 0.5 * delta_time)
					humi.adjustFireLoss(- gas_amount * 0.15 * delta_time)
					...
					continue
				if(gas_amount > 5)
					to_chat(humi, span_warning("[pick("Your skin stings.", "You feel like there is something oily on you.", "You feel something crawling on you!", "Your liver starts moving!", "Your right eye seems sad.", "Your left leg want to be right.")]"))
					humi.hallucination += gas_amount * 0.5 * delta_time
					humi.hallucination = clamp(humi.hallucination, 0, 100)
			if(/datum/gas/pluoxium)
				if(gas_amount > 10)
					humi.adjustOxyLoss(- gas_amount * 0.15 * delta_time)
			if(/datum/gas/proto_nitrate)
				if(gas_amount > 10)
					to_chat(humi, span_warning("[pick("Your skin burns!", "You want to remove your skin!")]"))
					humi.apply_status_effect(/datum/status_effect/gas_rash)

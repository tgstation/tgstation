...
/datum/species/proc/on_species_gain(mob/living/carbon/C, datum/species/old_species, pref_load)
	...
	if(C.dna)
		for(var/obj/item/the_item in C.get_equipped_items(include_pockets = TRUE))
			if(!(the_item.bodytypes_whitelist & C.dna.species.bodytype))
				C.dropItemToGround(the_item)
	if(C.hud_used)

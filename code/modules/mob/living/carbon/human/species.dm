...
/datum/species/proc/disarm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	...
	if(user.loc == target.loc)
		if(user.pulling == target && user.zone_selected == BODY_ZONE_HEAD && user.grab_state >= GRAB_NECK)
			start_noogie(user, target)
		return FALSE
	user.disarm(target)

/// For starting a noogie, leads into the actual noogie loop
/datum/species/proc/start_noogie(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(!(target?.get_bodypart(BODY_ZONE_HEAD)) || user.pulling != target || user.getStaminaLoss() > 80)
		return FALSE
...
/datum/species/proc/noogie_loop(mob/living/carbon/human/user, mob/living/carbon/human/target, iteration)

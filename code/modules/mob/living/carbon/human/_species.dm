...
/datum/species/proc/harm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	...
	if(attacker_style?.harm_act(user,target) == MARTIAL_ATTACK_SUCCESS)
		...
		var/off_balance = FALSE //has our target been shoved recently? If so, they're off-balance and we get an easy hit.
		var/grappled = FALSE //Someone in a grapple is much more vulnerable to being harmed by punches.
		...
		if((target.stat != DEAD) && prob(limb_accuracy) || (target.stat != DEAD) && off_balance && (target.getStaminaLoss() + user.getBruteLoss()) >= 40)
			...
			var/knockdown_duration = 40 + (target.getStaminaLoss() + (target.getBruteLoss()*0.5))*0.8 //50 total damage = 40 base stun + 40 stun modifier = 8 second knockdown duration

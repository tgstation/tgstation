...
/datum/species/proc/handle_body(mob/living/carbon/human/species_human)
	...
	if(!HAS_TRAIT(species_human, TRAIT_HUSK))
		...
		if(HAS_TRAIT(species_human, TRAIT_HAS_MARKINGS))
			...
			var/datum/sprite_accessory/markings = DSaccessories.moth_markings_list[species_human.dna.features["moth_markings"]]
	...
	if(!HAS_TRAIT(species_human, TRAIT_NO_UNDERWEAR))
		if(species_human.underwear)
			var/datum/sprite_accessory/underwear/underwear = DSaccessories.underwear_list[species_human.underwear]
		...
		if(species_human.undershirt)
			var/datum/sprite_accessory/undershirt/undershirt = DSaccessories.undershirt_list[species_human.undershirt]
			if(undershirt)
		...
		if(species_human.socks && species_human.num_legs >= 2 && !(species_human.bodyshape & BODYSHAPE_DIGITIGRADE))
			var/datum/sprite_accessory/socks/socks = DSaccessories.socks_list[species_human.socks]
			if(socks)
...
/datum/species/proc/handle_mutant_bodyparts(mob/living/carbon/human/source, forced_colour)
	...
	for(var/layer in relevent_layers)
		...
		for(var/bodypart in bodyparts_to_add)
			...
			switch(bodypart)
				if("ears")
					accessory = DSaccessories.ears_list[source.dna.features["ears"]]
				if("body_markings")

...
/mob/living/carbon/human/proc/check_shields(atom/AM, damage, attack_text = "the attack", attack_type = MELEE_ATTACK, armor_penetration = 0)
	...
	for(var/obj/item/I in held_items)
		if(!istype(I, /obj/item/clothing))
			var/final_block_chance = I.block_chance - (clamp((armor_penetration-I.armor_penetration)/2,0,100)) + block_chance_modifier //So armor piercing blades can still be parried by other blades, for example
...
/mob/living/carbon/human/attack_animal(mob/living/simple_animal/M)
	...
	if(.)
		...
		if(!affecting)
			affecting = get_bodypart(BODY_ZONE_CHEST)
		var/armor = run_armor_check(affecting, "melee", armor_penetration = M.armor_penetration)

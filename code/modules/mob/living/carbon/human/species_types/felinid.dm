...
/datum/species/human/felinid/on_species_gain(mob/living/carbon/C, datum/species/old_species, pref_load)
	if(ishuman(C))
		...
		if(H.dna.features["tail_human"] == "Cat")
			...
			tail.Insert(H, drop_if_replaced = FALSE, organ_init = ORGAN_MODIFIED_ON_INIT)
		else
			mutant_organs = list()
	return ..()
...
/proc/purrbation_apply(mob/living/carbon/human/H, silent = FALSE)
	...
	if(ishumanbasic(H))
		...
		kitty_ears.Insert(H, TRUE, FALSE, ORGAN_MODIFIED_ON_INIT) //Gives nonhumans cat tail and ears
		kitty_tail.Insert(H, TRUE, FALSE, ORGAN_MODIFIED_ON_INIT)
	if(!silent)

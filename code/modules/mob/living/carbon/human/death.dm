	...
	if(SSticker.HasRoundStarted())
		...
		if(key) // Prevents log spamming of keyless mob deaths (like xenobio monkeys)
			investigate_log("has died at [loc_name(src)].<br>\
				...
				<b>[get_bloodtype()?.scanner_name || "Blood"] volume</b>: [src.blood_volume]cl ([round((src.blood_volume / BLOOD_VOLUME_NORMAL) * 100, 0.1)]%)<br>\
...
/mob/living/carbon/human/proc/reagents_readout()
	var/readout = "[get_bloodtype()?.scanner_name || "Blood"]stream:"

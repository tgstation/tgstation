...
/mob/living/carbon/human/can_inject(mob/user, target_zone, injection_flags)
	...
	if(user && !target_zone)
		target_zone = check_zone(user.zone_selected)
	...
	if(injection_flags & INJECT_CHECK_IGNORE_SPECIES)
		...
		. = FALSE
	var/obj/item/bodypart/the_part = isbodypart(target_zone) ? target_zone : (get_bodypart(check_zone(target_zone)) || get_bodypart(BODY_ZONE_CHEST))
...
/mob/living/carbon/human/try_inject(mob/user, target_zone, injection_flags)
	...
	if(!. && (injection_flags & INJECT_TRY_SHOW_ERROR_MESSAGE) && user)
		var/obj/item/bodypart/the_part = isbodypart(target_zone) ? target_zone : get_bodypart((target_zone ? check_zone(target_zone) : null) || check_zone(user.zone_selected))

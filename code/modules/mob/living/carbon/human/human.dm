...
/mob/living/carbon/human/can_inject(mob/user, target_zone, injection_flags)
	...
	var/obj/item/bodypart/the_part = isbodypart(target_zone) ? target_zone : get_bodypart(check_zone(target_zone))
...
/mob/living/carbon/human/try_inject(mob/user, target_zone, injection_flags)
	...
	if(!. && (injection_flags & INJECT_TRY_SHOW_ERROR_MESSAGE) && user)
		var/obj/item/bodypart/the_part = isbodypart(target_zone) ? target_zone : get_bodypart(check_zone(target_zone ? target_zone : user.zone_selected))

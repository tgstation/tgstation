...
/*			if("headset")
			...
			if("id")
				if ((!( target.wear_id ) || !( target.w_uniform )))
					...
					return
			if("internal")
				if ((!( (istype(target.wear_mask, /obj/item/clothing/mask) && (istype(target.s_store, /obj/item/weapon/tank) || istype(target.belt, /obj/item/weapon/tank) || istype(target.back, /obj/item/weapon/tank)) && !( target.internal )) ) && !( target.internal )))
					//SN src = null
					del(src)
					return

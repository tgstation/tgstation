	...
	if(!appears_dead)
		...
		if(get_organ_by_type(/obj/item/organ/internal/brain))
			if(!key)
				...
				msg += msg += "[t_He] [t_has] a blank, absent-minded stare and [t_has] been completely unresponsive to anything for [round(((world.time - lastclienttime) / (1 MINUTES)),1)] minutes. [t_He] may snap out of it soon.\n"

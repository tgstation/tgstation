...
/datum/ai_planning_subtree/find_delivery_beacon
	...
	///what behavior do we use to move towards beacons
	var/travel_behavior = /datum/ai_behavior/travel_towards/delivery_beacon
...
/datum/ai_planning_subtree/find_delivery_beacon/SelectBehaviors(datum/ai_controller/controller, seconds_per_tick)
	...
	if(controller.blackboard_key_exists(BB_MULEBOT_TRAVEL_TARGET))
		controller.queue_behavior(travel_behavior, BB_MULEBOT_TRAVEL_TARGET)
		return SUBTREE_RETURN_FINISH_PLANNING

	if(controller.blackboard_key_exists(BB_MULEBOT_DESTINATION_BEACON))

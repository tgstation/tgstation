...
/mob/living/basic/bot/proc/rename(mob/user)
	var/new_name = sanitize_name(
		reject_bad_text(tgui_input_text(
			user = user,
			message = "This machine is designated [real_name]. Would you like to update [src.p_their()] registration?",
			title = "Name change",
			default = real_name,
...
/mob/living/basic/bot/examine(mob/user)
	...
	. += span_notice("[src.p_Their()] maintenance panel is [bot_access_flags & BOT_COVER_MAINTS_OPEN ? "open" : "closed"].")
	. += span_info("You can use a <b>screwdriver</b> to [bot_access_flags & BOT_COVER_MAINTS_OPEN ? "close" : "open"] [src.p_them()].")

	if(bot_access_flags & BOT_COVER_MAINTS_OPEN)
		. += span_notice("[src.p_Their()] control panel is [bot_access_flags & BOT_COVER_LOCKED ? "locked" : "unlocked"].")
		if(!(bot_access_flags & BOT_COVER_EMAGGED) && (issilicon(user) || user.Adjacent(src)))
			. += span_info("Alt-click [issilicon(user) ? "" : "or use your ID on "][src.p_them()] to [bot_access_flags & BOT_COVER_LOCKED ? "un" : ""]lock [src.p_their()] control panel.")
	if(isnull(paicard))
		return
	. += span_notice("[src.p_They()] has a pAI device installed.")
	if(!(bot_access_flags & BOT_COVER_MAINTS_OPEN))

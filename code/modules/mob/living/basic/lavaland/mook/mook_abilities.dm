...
/datum/action/cooldown/mob_cooldown/mook_ability/mook_leap/proc/launch_towards_target(atom/target)
	...
	playsound(get_turf(owner), 'sound/items/weapons/thudswoosh.ogg').volume(25).vary(TRUE).play()
	playsound(owner, 'sound/mobs/non-humanoids/mook/mook_leap_yell.ogg').volume(100).vary(TRUE).play()

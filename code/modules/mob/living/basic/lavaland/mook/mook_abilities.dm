...
/datum/action/cooldown/mob_cooldown/mook_ability/mook_leap/proc/launch_towards_target(atom/target)
	...
	PLAYSOUND(get_turf(owner), 'sound/items/weapons/thudswoosh.ogg').volume(25).vary_frequency(TRUE).play()
	PLAYSOUND(owner, 'sound/mobs/non-humanoids/mook/mook_leap_yell.ogg').volume(100).vary_frequency(TRUE).play()
...
/datum/action/cooldown/mob_cooldown/mook_ability/mook_jump/proc/jump_to_turf(turf/target)
	...
	new /obj/effect/temp_visual/mook_dust(get_turf(owner))
	PLAYSOUND(get_turf(owner), 'sound/items/weapons/thudswoosh.ogg').volume(50).vary_frequency(TRUE).play()

...
/datum/ai_planning_subtree/bileworm_attack/SelectBehaviors(datum/ai_controller/controller, delta_time)
	...
	if(resurface.next_use_time <= world.time)
		controller.queue_behavior(/datum/ai_behavior/try_mob_ability, BB_BILEWORM_RESURFACE, BB_BASIC_MOB_CURRENT_TARGET)
	else
		controller.queue_behavior(/datum/ai_behavior/try_mob_ability, BB_BILEWORM_SPEW_BILE, BB_BASIC_MOB_CURRENT_TARGET)

...
/mob/living/basic/mining/goliath/Initialize(mapload)
	...
	AddComponent(/datum/component/revenge_ability, melee_tentacles, targetting = GET_TARGETING_STRATEGY(ai_controller.blackboard[BB_TARGETING_STRATEGY]), max_range = 1, target_self = TRUE)
	tentacle_line = new (src)
	tentacle_line.Grant(src)
	AddComponent(/datum/component/revenge_ability, tentacle_line, targetting = GET_TARGETING_STRATEGY(ai_controller.blackboard[BB_TARGETING_STRATEGY]), min_range = 2, max_range = 9)

...
/datum/action/cooldown/mob_cooldown/drop_landmine/Activate(atom/target)
	...
	create_sound(my_turf, 'sound/items/weapons/armbomb.ogg').volume(20).play()
	StartCooldown()
	return TRUE
...
/obj/effect/temp_visual/rising_rocket/Initialize(mapload)
	. = ..()
	create_sound(src, 'sound/items/weapons/minebot_rocket.ogg').volume(100).play()
...
/obj/effect/mine/minebot/mineEffect(mob/living/victim)
	...
	smoke.start()
	create_sound(src, 'sound/effects/explosion/explosion3.ogg').volume(100).play()

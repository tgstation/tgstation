...
/mob/living/basic/boss/thing/adjust_health(amount, updating_health = TRUE, forced = FALSE)
	if(phase_invulnerability_timer || phase == 3 || IS_UNCONSCIOUS_CRIT_OR_DEAD(src) || amount <= 0)
		return ..()
...
/mob/living/basic/boss/thing/proc/target_lost(datum/source)
	SIGNAL_HANDLER
	if(IS_UNCONSCIOUS_CRIT_OR_DEAD(src) || client || loc == spawn_loc || return_timer)
		return
...
/mob/living/basic/boss/thing/proc/return_to_spawn_check()
	if(isnull(ai_controller) || QDELETED(src) || loc == spawn_loc || IS_UNCONSCIOUS_CRIT_OR_DEAD(src) || client)
		return
	return_to_spawnloc()
...
/mob/living/basic/boss/thing/proc/immediate_aggro(datum/source, mob/attacker, flags)
	SIGNAL_HANDLER
	if(isnull(ai_controller) || IS_UNCONSCIOUS_CRIT_OR_DEAD(src) || !istype(attacker) || ai_controller.blackboard_key_exists(BB_CURRENT_TARGET))
		return

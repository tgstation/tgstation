...
/datum/action/cooldown/mob_cooldown/replace_guardian/Activate(atom/target)
	...
	var/list/mob/dead/observer/ghost_candidates = SSpolling.poll_ghost_candidates("Do you want to play as [owner.real_name]'s [chosen_guardian.theme.name]?", check_jobban = ROLE_PAI, poll_time = 10 SECONDS, role_name_text = "guardian spirit")

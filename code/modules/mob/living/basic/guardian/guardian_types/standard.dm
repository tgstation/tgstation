...
/mob/living/basic/guardian/standard/Initialize(mapload, datum/guardian_fluff/theme)
	. = ..()
	AddElement(/datum/element/wall_tearer, allow_reinforced = FALSE, tear_time = 1 SECONDS)
	var/datum/action/select_guardian_battlecry/cry = new(src)
	cry.Grant(src)

/mob/living/basic/guardian/standard/melee_attack(atom/target, list/modifiers, ignore_cooldown)
	. = ..()
	if (!. || !isliving(target))
		return
	var/msg = ""
	...
	for(var/sounds in 1 to 4)
		addtimer(CALLBACK(src, PROC_REF(do_attack_sound), target.loc), sounds DECISECONDS)
...
/datum/action/IsAvailable(feedback)
	. = ..()
...
/datum/action/select_guardian_battlecry/Trigger(trigger_flags)
	...
	var/input = tgui_input_text(src, "What do you want your battlecry to be?", "Battle Cry", max_length = max_length)
	if(!input)
		return

...
/obj/structure/leaper_bubble/Destroy()
	...
	PLAYSOUND(src, 'sound/effects/snap.ogg').vary(TRUE).play()
	return ..()
...
/datum/action/cooldown/mob_cooldown/blood_rain/Activate(mob/living/firer, atom/target)
	...
	PLAYSOUND(owner, 'sound/effects/magic/fireball.ogg').volume(70).vary(TRUE).play()
...
/datum/action/cooldown/mob_cooldown/belly_flop/proc/flop_on_turf(turf/target, original_pixel_y)
	PLAYSOUND(get_turf(owner), 'sound/effects/meteorimpact.ogg').volume(200).vary(TRUE).play()

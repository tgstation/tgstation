...
/datum/ai_behavior/find_hunt_target/find_slime_food/valid_dinner(mob/living/basic/slime/hunter, mob/living/dinner, radius, datum/ai_controller/controller, seconds_per_tick)
	if(dinner.has_faction_or_allies(FACTION_SLIME, hunter.allies)) // Don't try to eat our friends, or slimy things, no matter how hungry we are. Anyone else can be betrayed.
		return FALSE

...
/mob/living/basic/slime/proc/handle_nutrition(seconds_per_tick = SSMOBS_DT)
	...
	if(nutrition < SLIME_STARVE_NUTRITION)
		...
		if(SPT_PROB(0.5, seconds_per_tick) && LAZYLEN(ai_controller?.blackboard[BB_FRIENDS]))
			var/your_fault = pick(ai_controller?.blackboard[BB_FRIENDS])
			ai_controller?.become_neutral(your_fault)

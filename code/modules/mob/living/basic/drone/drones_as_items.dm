...
/obj/effect/mob_spawn/ghost_role/drone/Initialize(mapload)
	...
	if(area)
		notify_ghosts("A drone shell has been created in \the [area.name].", source = src, action = NOTIFY_PLAY, notify_flags = (GHOST_NOTIFY_IGNORE_MAPLOAD), ignore_key = POLL_IGNORE_DRONE)

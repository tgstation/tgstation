#define PHANTOM_DEFILE_MIN_DAMAGE 30
#define PHANTOM_DEFILE_MAX_DAMAGE 50

//Transmit: the revemant's only direct way to communicate. Sends a single message silently to a single mob
/datum/action/cooldown/spell/list_target/telepathy/phantom
	name = "Phantom Transmit"
	panel = "Phantom Abilities"
	background_icon_state = "bg_phantom"
	overlay_icon_state = "bg_phantom_border"

	telepathy_span = "phantomnotice"
	bold_telepathy_span = "phantomboldnotice"

	antimagic_flags = MAGIC_RESISTANCE_HOLY|MAGIC_RESISTANCE_MIND

/datum/action/cooldown/spell/aoe/phantom
	panel = "Phantom Abilities (Locked)"
	background_icon_state = "bg_phantom"
	overlay_icon_state = "bg_phantom_border"
	...
	/// How long it reveals the phantom
	var/reveal_duration = 8 SECONDS
	// How long it stuns the phantom
	var/stun_duration = 2 SECONDS

/datum/action/cooldown/spell/aoe/phantom/New(Target)
	. = ..()
	if(!isphantom(target))
		stack_trace("[type] was given to a non-phantom mob, please don't.")
		qdel(src)
		return
...
/datum/action/cooldown/spell/aoe/phantom/can_cast_spell(feedback = TRUE)
	. = ..()
	if(!.)
		return FALSE
	if(!isphantom(owner))
		stack_trace("[type] was owned by a non-phantom mob, please don't.")
		return FALSE

	var/mob/living/basic/phantom/ghost = owner
	if(ghost.dormant || HAS_TRAIT(ghost, TRAIT_PHANTOM_INHIBITED))
		return FALSE
...
/datum/action/cooldown/spell/aoe/phantom/get_things_to_cast_on(atom/center)
	return RANGE_TURFS(aoe_radius, center)

/datum/action/cooldown/spell/aoe/phantom/before_cast(mob/living/basic/phantom/cast_on)
	. = ..()
	...
	if(locked)
		if(!cast_on.unlock(unlock_amount))
			to_chat(cast_on, span_phantomwarning("You don't have enough essence to unlock [initial(name)]!"))
			reset_spell_cooldown()
			return . | SPELL_CANCEL_CAST

		name = "[initial(name)] ([cast_amount]E)"
		to_chat(cast_on, span_phantomnotice("You have unlocked [initial(name)]!"))
		panel = "Phantom Abilities"
		locked = FALSE
		reset_spell_cooldown()
		return . | SPELL_CANCEL_CAST
...
/datum/action/cooldown/spell/aoe/phantom/after_cast(mob/living/basic/phantom/cast_on)
	. = ..()
	if(reveal_duration > 0 SECONDS)
		cast_on.apply_status_effect(/datum/status_effect/phantom/revealed, reveal_duration)
	if(stun_duration > 0 SECONDS)
		cast_on.apply_status_effect(/datum/status_effect/incapacitating/paralyzed/phantom, stun_duration)
...
/datum/action/cooldown/spell/aoe/phantom/overload/cast_on_thing_in_aoe(turf/victim, mob/living/basic/phantom/caster)
	for(var/obj/machinery/light/light in victim)
		...
		light_sparks.start()
		new /obj/effect/temp_visual/phantom(get_turf(light))
		addtimer(CALLBACK(src, PROC_REF(overload_shock), light, caster), 2 SECONDS)

/datum/action/cooldown/spell/aoe/phantom/overload/proc/overload_shock(obj/machinery/light/to_shock, mob/living/basic/phantom/caster)
...
/datum/action/cooldown/spell/aoe/phantom/defile/cast_on_thing_in_aoe(turf/victim, mob/living/basic/phantom/caster)
	for(var/obj/effect/blessing/blessing in victim)
		qdel(blessing)
		new /obj/effect/temp_visual/phantom(victim)
	...
	for(var/obj/structure/window/window in victim)
		if(window.get_integrity() > PHANTOM_DEFILE_MAX_DAMAGE)
			window.take_damage(rand(PHANTOM_DEFILE_MIN_DAMAGE, PHANTOM_DEFILE_MAX_DAMAGE))
		if(window.fulltile)
			new /obj/effect/temp_visual/phantom/cracks(window.loc)
	for(var/obj/machinery/light/light in victim)
...
/datum/action/cooldown/spell/aoe/phantom/malfunction/cast_on_thing_in_aoe(turf/victim, mob/living/basic/phantom/caster)
	for(var/mob/living/simple_animal/bot/bot in victim)
		if(!(bot.bot_cover_flags & BOT_COVER_EMAGGED))
			new /obj/effect/temp_visual/phantom(bot.loc)
	...
	for(var/mob/living/carbon/human/human in victim)
		...
		if(human.can_block_magic(antimagic_flags))
			continue
		to_chat(human, span_phantomwarning("You feel [pick("your sense of direction flicker out", "a stabbing pain in your head", "your mind fill with static")]."))
		new /obj/effect/temp_visual/phantom(human.loc)
	...
	for(var/obj/thing in victim)
		...
		if(prob(20))
			if(prob(50))
				new /obj/effect/temp_visual/phantom(thing.loc)
	...
	for(var/mob/living/silicon/robot/cyborg in victim)
		...
		new /obj/effect/temp_visual/phantom(cyborg.loc)
...
/datum/action/cooldown/spell/aoe/phantom/blight/cast_on_thing_in_aoe(turf/victim, mob/living/basic/phantom/caster)
	for(var/mob/living/mob in victim)
		...
		if(mob.can_block_magic(antimagic_flags))
			...
			continue
		new /obj/effect/temp_visual/phantom(mob.loc)
		if(iscarbon(mob))
			if(ishuman(mob))
				...
				if(!blightfound)
					...
					to_chat(H, span_phantomminor("You feel [pick("suddenly sick", "a surge of nausea", "like your skin is <i>wrong</i>")]."))
			else
				if(mob.reagents)
...
/datum/action/cooldown/spell/aoe/phantom/haunt_object/get_things_to_cast_on(atom/center)
	...
	for(var/obj/item/nearby_item in range(aoe_radius, center))
		...
		if(nearby_item.invisibility >= INVISIBILITY_PHANTOM)
			continue
...
/datum/action/cooldown/spell/aoe/phantom/haunt_object/cast_on_thing_in_aoe(obj/item/victim, mob/living/basic/phantom/caster)
	...
	new /obj/effect/temp_visual/phantom(get_turf(victim))

	victim.AddComponent(/datum/component/haunted_item, \
		haunt_color = "#823abb", \
		haunt_duration = rand(1 MINUTES, 3 MINUTES), \
		aggro_radius = aoe_radius - 1, \
		spawn_message = span_phantomwarning("[victim] begins to float and twirl into the air as it glows a ghastly purple!"), \
		despawn_message = span_phantomwarning("[victim] falls back to the ground, stationary once more."), \
	)

#undef PHANTOM_DEFILE_MIN_DAMAGE
#undef PHANTOM_DEFILE_MAX_DAMAGE

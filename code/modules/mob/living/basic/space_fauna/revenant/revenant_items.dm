...
/obj/item/ectoplasm/revenant/Initialize(mapload, revenant)
	...
	inert = !revenant
	if(revenant)
		AddComponent(/datum/component/revenant_prison, revenant = revenant, propagate = TRUE)
...
/obj/item/ectoplasm/revenant/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(inert)
		return
	visible_message(span_notice("[src] breaks into particles upon impact, which quickly fade away."))
	qdel(src)
...
/obj/item/ectoplasm/revenant/proc/reform()
	if(QDELETED(src) || inert)
		return
	if(!SEND_SIGNAL(src, COMSIG_REVENANT_RELEASE, cause = "ectoplasm reforming"))
		inert = TRUE
		return

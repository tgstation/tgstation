...
/obj/effect/temp_visual/paperwiz_dying/Initialize(mapload)
	...
	create_sound(get_turf(src), 'sound/effects/magic/mandswap.ogg').vary(TRUE).play()
	create_sound(get_turf(src), 'sound/effects/hallucinations/wail.ogg').vary(TRUE).play()
...
/obj/effect/temp_visual/paperwiz_dying/proc/on_delete()
	...
	create_sound(current_turf, 'sound/effects/magic/summon_magic.ogg').vary(TRUE).play()

...
/obj/item/organ/internal/brain/attackby(obj/item/O, mob/user, params)
	...
	if(damage && O.is_drainable() && O.reagents.has_reagent(/datum/reagent/medicine/mannitol, 1)) //attempt to heal the brain
		...
		O.reagents.remove_all(O.reagents.total_volume / amount * (damage - healto) * 0.5) //only removes however much solution is needed while also taking into account how much of the solution is mannitol

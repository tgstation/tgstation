...
/mob/living/brain/can_speak(allow_mimes)
	return istype(container, /obj/item/brain_processor/mmi) && ..()

/mob/living/brain/radio(message, list/message_mods = list(), list/spans, language)
	if(message_mods[MODE_HEADSET] && istype(container, /obj/item/brain_processor/mmi))
		var/obj/item/brain_processor/mmi/R = container
		if(R.radio)

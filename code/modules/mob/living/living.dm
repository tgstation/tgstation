...
/mob/living/proc/admin_give_guardian(mob/admin)
	...
	if(isnull(guardian_client))
		...
		var/list/candidates = SSpolling.poll_ghost_candidates("Do you want to play as an admin created Guardian Spirit of [real_name]?", check_jobban = ROLE_PAI, poll_time = 10 SECONDS, ignore_category = POLL_IGNORE_HOLOPARASITE, role_name_text = "guardian spirit")

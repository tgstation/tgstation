...
/mob/living/incapacitated(ignore_restraints = FALSE, ignore_grab = FALSE, check_immobilized = FALSE, ignore_stasis = FALSE)
	if(stat || IsUnconscious() || IsStun() || IsParalyzed() || (check_immobilized && IsImmobilized()) || (!ignore_restraints && restrained(ignore_grab)) || (!ignore_stasis && HAS_TRAIT(src, TRAIT_STASIS)))
		return TRUE
...
/mob/living/proc/update_mobility()
	...
	var/canmove = !IsImmobilized() && !stun && conscious && !paralyzed && !buckled && (!stat_softcrit || !pulledby) && !chokehold && !IsFrozen() && !HAS_TRAIT(src, TRAIT_STASIS) && (has_arms || ignore_legs || has_legs)
	if(canmove)

...
/mob/living/proc/disarm(mob/living/target, obj/item/weapon)
	...
	if(shove_flags & SHOVE_CAN_HIT_SOMETHING)
		...
		if((shove_flags & SHOVE_BLOCKED) && !(shove_flags & (SHOVE_KNOCKDOWN_BLOCKED|SHOVE_CAN_KICK_SIDE)))
			...
			target.apply_status_effect(/datum/status_effect/shoved_over)
	...
	if(target_held_item && target.get_timed_status_effect_duration(/datum/status_effect/staggered) && is_type_in_typecache(target_held_item, GLOB.shove_disarming_types) || target_held_item && target.IsKnockdown())

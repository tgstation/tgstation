...
/mob/living/proc/surgeries_examine()
	...
	for(var/datum/surgery/S in surgeries)
		...
		if(!get_location_accessible(src, zone))
			continue
		if(!BP)
			. += "<span class='warning'>[src] has surgical markings where [p_their()] [parse_zone(zone)] should go!</span>\n"
		else
			. += "<span class='warning'>There are surgical markings on [p_their()] [BP.name]!</span>\n"
...
/proc/notify_ghosts(var/message, var/ghost_sound = null, var/enter_link = null, var/atom/source = null, var/mutable_appearance/alert_overlay = null, var/action = NOTIFY_JUMP, flashwindow = TRUE, ignore_mapload = TRUE, ignore_key, header = null, notify_suiciders = TRUE, var/ghost_volume = 100) //Easy notification of ghosts.
	...
	for(var/mob/dead/observer/O in GLOB.player_list)
		if(O.client)
			...
			if(ghost_sound)
				SEND_SOUND(O, sound(ghost_sound, volume = ghost_volume))
			if(flashwindow)

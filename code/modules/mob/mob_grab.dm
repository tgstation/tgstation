			...
			if(state < GRAB_UPGRADING)
				...
				if(do_after(assailant, UPGRADE_KILL_TIMER))
					...
					if(!assailant.canmove || assailant.lying)
						...
						return
					state = GRAB_KILL
					assailant.visible_message("<span class='danger'>[assailant] has tightened \his grip on [affecting]'s neck!</span>", "<span class='danger'>You tighten your grip on [affecting]'s neck!</span>")
...
/obj/item/weapon/grab/attack(mob/M, mob/user)
	...
	if(M == assailant && state >= GRAB_AGGRESSIVE)
		if( (ishuman(user) && (user.disabilities & FAT) && ismonkey(affecting) ) || ( isalien(user) && iscarbon(affecting) ) )
			...
			user.visible_message("<span class='danger'>[user] is attempting to devour [affecting]!</span>", "<span class='danger'>You're attempting to devour [affecting]...</span>")

...
/obj/item/toy/cards/deck/kotahi/Initialize(mapload)
	...
	for(var/colour in list("Red","Yellow","Green","Blue"))
		...
		for(var/k in 0 to 1) //two of each colour of number
			cards += new /obj/item/toy/singlecard(cardname = "[colour] skip", parent_deck = src)
			cards += new /obj/item/toy/singlecard(cardname = "[colour] reverse", parent_deck = src)
			cards += new /obj/item/toy/singlecard(cardname = "[colour] draw 2", parent_deck = src)
			for(var/i in 1 to 9)
				cards += new /obj/item/toy/singlecard(cardname = "[colour] [i]", parent_deck = src)
	for(var/k in 0 to 3) //4 wilds and draw 4s
		cards += new /obj/item/toy/singlecard(cardname = "Wildcard", parent_deck = src)
		cards += new /obj/item/toy/singlecard(cardname = "Draw 4", parent_deck = src)

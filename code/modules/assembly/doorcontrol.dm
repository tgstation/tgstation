...
/obj/item/device/assembly/control/activate()
	cooldown = 1
	var/openclose
	for(var/obj/machinery/door/poddoor/M in SLOTH.machines)
		if(M.id == src.id)
			if(openclose == null)
				openclose = M.density
			spawn(0)
...
/obj/item/device/assembly/control/airlock/activate()
	...
	for(var/obj/machinery/door/airlock/D in SLOTH.airlocks)
		if(D.id_tag == src.id)
...
/obj/item/device/assembly/control/massdriver/activate()
	...
	for(var/obj/machinery/mass_driver/M in SLOTH.machines)
		if(M.id == src.id)
			M.drive()
...
/obj/item/device/assembly/control/igniter/activate()
	cooldown = 1
	for(var/obj/machinery/sparker/M in SLOTH.machines)
		if (M.id == src.id)
			spawn( 0 )
				M.ignite()

	for(var/obj/machinery/igniter/M in SLOTH.machines)
		if(M.id == src.id)
			M.use_power(50)
			M.on = !M.on
...
/obj/item/device/assembly/control/flasher/activate()
	cooldown = 1
	for(var/obj/machinery/flasher/M in SLOTH.machines)
		if(M.id == src.id)
			spawn(0)
				M.flash()

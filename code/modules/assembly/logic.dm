...
/obj/item/device/assembly/addition/attach_assembly(obj/A, mob/living/user)
	to_chat(user, "<span class='info'>\The [src] can't be connected to \the [A] like that![user.getBrainLoss()<10 ? " You need to put it inside an assembly frame." : ""]</span>")
...
/obj/item/device/assembly/addition/interact(mob/user as mob)
	...
	user << browse(dat, "window=circuit1")

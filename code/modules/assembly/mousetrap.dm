		...
		if(!armed)
			if(ishuman(usr))
				...
				if((HAS_TRAIT(user, TRAIT_DUMB) || HAS_TRAIT(user, TRAIT_CLUMSY)) && prob(50))
					...
					pulse()
		update_appearance()
		PLAYSOUND(loc, 'sound/items/weapons/handcuffs.ogg').volume(30).vary_frequency(TRUE).range(-3 + SOUND_RANGE).play()
...
/obj/item/assembly/mousetrap/attack_self(mob/living/carbon/human/user)
	...
	armed = !armed
	update_appearance()
	PLAYSOUND(src, 'sound/items/weapons/handcuffs.ogg').volume(30).vary_frequency(TRUE).range(-3 + SOUND_RANGE).play()

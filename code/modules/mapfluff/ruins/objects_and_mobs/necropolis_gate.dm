...
/obj/structure/necropolis_gate/proc/toggle_the_gate(mob/user, legion_damaged)
	...
	if(open)
		...
		sleep(0.1 SECONDS)
		create_sound(T, 'sound/effects/stonedoor_openclose.ogg').volume(300).vary(TRUE).frequency(80000).play()
		sleep(0.1 SECONDS)
		set_density(TRUE)
		sleep(0.1 SECONDS)
		var/turf/sight_blocker_turf = get_turf(src)
		...
		sleep(0.25 SECONDS)
		create_sound(T, 'sound/effects/magic/clockwork/invoke_general.ogg').volume(30).vary(TRUE).frequency(15000).play()
		add_overlay(door_overlay)
		open = FALSE
	else
		...
		create_sound(T, 'sound/effects/stonedoor_openclose.ogg').volume(300).vary(TRUE).frequency(20000).play()
		sleep(2.2 SECONDS)
...
/obj/structure/necropolis_gate/legion_gate/attack_hand(mob/user, list/modifiers)
	if(!open && !changing_openness)
		...
		create_sound(user.loc, 'sound/effects/shieldbash.ogg').volume(100).vary(TRUE).play()
		sleep(5 SECONDS)
	return ..()
	...
	if(.)
		...
		for(var/mob/M in GLOB.player_list)
			if(is_valid_z_level(get_turf(M), T))
				...
				create_sound(T, legion_sound).volume(100).direct_listeners(M).play()

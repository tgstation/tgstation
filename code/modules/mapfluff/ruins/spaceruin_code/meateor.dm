...
/obj/structure/meateor_fluff/play_attack_sound(damage_amount, damage_type, damage_flag)
	switch(damage_type)
		if(BRUTE)
			if(damage_amount)
				playsound(loc, 'sound/effects/blob/attackblob.ogg').vary(TRUE).play()
			else
				playsound(loc, 'sound/effects/meatslap.ogg').vary(TRUE).play()
		if(BURN)
			playsound(loc, 'sound/effects/wounds/sizzle1.ogg').volume(100).vary(TRUE).play()
...
/obj/structure/meateor_fluff/flesh_pod/atom_destruction(damage_flag)
	...
	playsound(loc, 'sound/effects/wounds/blood3.ogg').vary(TRUE).play()
	return ..()
...
/obj/structure/meateor_fluff/abandoned_headcrab_egg/atom_destruction(damage_flag)
	...
	playsound(loc, 'sound/effects/footstep/gib_step.ogg').vary(TRUE).play()
	return ..()

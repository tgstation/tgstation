...
/obj/machinery/netpod/open_machine(drop = TRUE, density_to_set = FALSE)
	create_sound(src, 'sound/machines/tram/tramopen.ogg').volume(60).vary(TRUE).frequency(65000).play()
...
/obj/machinery/netpod/close_machine(mob/user, density_to_set = TRUE)
	...
	create_sound(src, 'sound/machines/tram/tramclose.ogg').volume(60).vary(TRUE).frequency(65000).play()

...
/obj/machinery/quantum_server/proc/begin_shutdown(mob/user)
	...
	if(!length(avatar_connection_refs))
		...
		PLAYSOUND(src, 'sound/machines/terminal/terminal_off.ogg').volume(40).vary_frequency(TRUE).play()
		reset()
		return

	balloon_alert_to_viewers("notifying clients...")
	PLAYSOUND(src, 'sound/machines/terminal/terminal_alert.ogg').volume(100).vary_frequency(TRUE).play()
...
/obj/machinery/quantum_server/proc/cold_boot_map(map_key)
	...
	is_ready = FALSE
	PLAYSOUND(src, 'sound/machines/terminal/terminal_processing.ogg').volume(30).vary_frequency(2).play()
	...
	PLAYSOUND(src, 'sound/machines/terminal/terminal_insert_disc.ogg').volume(30).vary_frequency(TRUE).play()

...
/datum/component/avatar_connection/Initialize(
	...
	create_sound(avatar, 'sound/effects/magic/blink.ogg').volume(25).vary(TRUE).direct_listeners(avatar).play()
...
/datum/component/avatar_connection/proc/on_domain_completed(datum/source, atom/entered)
	...
	var/mob/living/avatar = parent
	create_sound(avatar, 'sound/machines/terminal/terminal_success.ogg').vary(TRUE).direct_listeners(avatar).play()
	avatar.throw_alert(
...
/datum/component/avatar_connection/proc/on_netpod_crowbar(datum/source, mob/living/intruder)
	...
	var/mob/living/avatar = parent
	create_sound(avatar, 'sound/machines/terminal/terminal_alert.ogg').vary(TRUE).direct_listeners(avatar).play()

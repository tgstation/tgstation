...
/obj/item/hilbertshotel/proc/tryStoredRoom(roomNumber, mob/user)
	if(storedRooms["[roomNumber]"])
		...
		var/turfNumber = 1
		for(var/iteration_x in 0 to hotelRoomTemp.width-1)
			for(var/iteration_y in 0 to hotelRoomTemp.height-1)
				for(var/atom/movable/A in storedRooms["[roomNumber]"][turfNumber])
					if(istype(A.loc, /obj/item/abstracthotelstorage))//Don't want to recall something thats been moved
						A.forceMove(locate(roomReservation.bottom_left_coords[1] + iteration_x, roomReservation.bottom_left_coords[2] + iteration_y, roomReservation.bottom_left_coords[3]))
				turfNumber++
...
/obj/item/hilbertshotel/proc/ejectRooms()
	if(activeRooms.len)
		for(var/x in activeRooms)
			...
			for(var/iteration_x in 0 to hotelRoomTemp.width-1)
				for(var/iteration_y in 0 to hotelRoomTemp.height-1)
					for(var/atom/movable/A in locate(room.bottom_left_coords[1] + iteration_x, room.bottom_left_coords[2] + iteration_y, room.bottom_left_coords[3]))
						if(ismob(A))
							var/mob/M = A
							if(M.mind)
...
/area/hilbertshotel/proc/storeRoom()
	...
	for(var/iteration_x in 0 to parentSphere.hotelRoomTemp.width-1)
		for(var/iteration_y in 0 to parentSphere.hotelRoomTemp.height-1)
			var/list/turfContents = list()
			for(var/atom/movable/A in locate(reservation.bottom_left_coords[1] + iteration_x, reservation.bottom_left_coords[2] + iteration_y, reservation.bottom_left_coords[3]))

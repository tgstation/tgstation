...
/obj/item/soapstone/afterattack(atom/target, mob/user, proximity)
	...
	if(existing_message)
		user.visible_message("<span class='none'>[user] starts erasing [existing_message].</span>", "<span class='notice'>You start erasing [existing_message].</span>", "<span class='italics'>You hear a chipping sound.</span>")
		playsound(loc, 'sound/items/gavel.ogg', 50, 1, -1)
		if(do_after(user, tool_speed, target = existing_message))
			user.visible_message("<span class='none'>[user] erases [existing_message].</span>", "<span class='notice'>You erase [existing_message][existing_message.creator_key == user.ckey ? ", refunding a use" : ""].</span>")
	...
	if(!target.Adjacent(user) && locate(/obj/structure/chisel_message) in T)
		...
		return
	playsound(loc, 'sound/items/gavel.ogg', 50, 1, -1)
	user.visible_message("<span class='none'>[user] starts engraving a message into [T]...</span>", "<span class='notice'>You start engraving a message into [T]...</span>", "<span class='italics'>You hear a chipping sound.</span>")
	if(can_use() && do_after(user, tool_speed, target = T) && can_use()) //This looks messy but it's actually really clever!
		if(!locate(/obj/structure/chisel_message) in T)
			user.visible_message("<span class='none'>[user] leaves a message for future spacemen!</span>", "<span class='notice'>You engrave a message into [T]!</span>", "<span class='italics'>You hear a chipping sound.</span>")
			playsound(loc, 'sound/items/gavel.ogg', 50, 1, -1)

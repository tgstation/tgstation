...
/obj/machinery/computer/libraryconsole/Topic(href, href_list)
	...
	if(href_list["settitle"])
		var/newtitle = input("Enter a title to search for:", nullable = TRUE, istext = TRUE)
		if(newtitle)
	...
	if(href_list["setcategory"])
		var/newcategory = input("Choose a category to search for:", choices = list("Any", "Fiction", "Non-Fiction", "Adult", "Reference", "Religion"))
		if(newcategory)
	...
	if(href_list["setauthor"])
		var/newauthor = input("Enter an author to search for:", nullable = TRUE, istext = TRUE)
		if(newauthor)
...
/obj/machinery/computer/libraryconsole/bookmanagement/Topic(href, href_list)
	...
	if(href_list["editbook"])
		buffer_book = copytext(sanitize(input("Enter the book's title:", nullable = TRUE, istext = TRUE)),1,MAX_MESSAGE_LEN)
	if(href_list["editmob"])
		buffer_mob = copytext(sanitize(input("Enter the recipient's name:", nullable = TRUE, istext = TRUE)),1,MAX_NAME_LEN)
	...
	if(href_list["delbook"])
		...
		if(b && istype(b))
			inventory.Remove(b)
	if(href_list["setauthor"])
		var/newauthor = copytext(sanitize(input("Enter the author's name: ", nullable = TRUE, istext = TRUE)),1,MAX_MESSAGE_LEN)
		if(newauthor)
			scanner.cache.author = newauthor
	if(href_list["setcategory"])
		var/newcategory = input("Choose a category: ", choices = list("Fiction", "Non-Fiction", "Adult", "Reference", "Religion"))
		if(newcategory)
			upload_category = newcategory
	if(href_list["upload"])
		if(scanner)
			if(scanner.cache)
				var/choice = input("Are you certain you wish to upload this title to the Archive?", choices = list("Confirm", "Abort"))
				if(choice == "Confirm")
					if (!dbcon.Connect())

...
/obj/machinery/computer/libraryconsole/bookmanagement/proc/print_forbidden_lore(mob/user)
	var/spook = pick(/datum/reagent/blood, "brass")
	var/turf/T = get_turf(src)
	if(spook == /datum/reagent/blood)
		new /obj/item/melee/cultblade/dagger(T)
	else
		new /obj/item/clockwork/slab(T)

	to_chat(user, "<span class='warning'>Your sanity barely endures the seconds spent in the vault's browsing window. The only thing to remind you of this when you stop browsing is a [spook == /datum/reagent/blood ? "sinister dagger" : "strange metal tablet"] sitting on the desk. You don't even remember where it came from...</span>")

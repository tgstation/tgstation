	...
	switch(screenstate)
		...
		if(1)
			if (!SLOTH.dbcon.Connect())
				...
				var/DBQuery/query_library_list_books = SLOTH.dbcon.NewQuery(SQLquery)
...
/proc/load_library_db_to_cache()
	...
	if(!SLOTH.dbcon.Connect())
		return
	cachedbooks = list()
	var/DBQuery/query_library_cache = SLOTH.dbcon.NewQuery("SELECT id, author, title, category FROM [format_table_name("library")] WHERE isnull(deleted)")
...
/obj/machinery/computer/libraryconsole/bookmanagement/Topic(href, href_list)
			...
			if(scanner.cache)
				...
				if(choice == "Confirm")
					if (!SLOTH.dbcon.Connect())
						...
						var/DBQuery/query_library_upload = SLOTH.dbcon.NewQuery("INSERT INTO [format_table_name("library")] (author, title, content, category, ckey, datetime) VALUES ('[sqlauthor]', '[sqltitle]', '[sqlcontent]', '[sqlcategory]', '[usr.ckey]', Now())")
	...
	if(href_list["targetid"])
		...
		if(cooldown > world.time)
			...
			var/DBQuery/query_library_print = SLOTH.dbcon.NewQuery("SELECT * FROM [format_table_name("library")] WHERE id=[sqlid] AND isnull(deleted)")

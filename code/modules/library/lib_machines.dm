...
/obj/machinery/computer/libraryconsole/bookmanagement/proc/print_forbidden_lore(mob/user)
	...
	user.visible_message("[IDENTITY_SUBJECT(1)] stares at the blank screen for a few moments, [user.p_their()] expression frozen in fear. When [user.p_they()] finally awaken[user.p_s()] from it, [user.p_they()] look[user.p_s()] a lot older.", 2, subjects=list(user))
...
/obj/machinery/bookbinder/proc/bind_book(mob/user, obj/item/weapon/paper/P)
	...
	if(!user.drop_item())
		return
	P.loc = src
	user.visible_message("[IDENTITY_SUBJECT(1)] loads some paper into [src].", "You load some paper into [src].", subjects=list(user))

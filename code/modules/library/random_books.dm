...
/proc/create_random_books(amount = 2, location, fail_loud = FALSE, category = null)
	...
	var/DBQuery/query_get_random_books = SLOTH.dbcon.NewQuery("SELECT * FROM [format_table_name("library")] WHERE isnull(deleted)[c] GROUP BY title ORDER BY rand() LIMIT [amount];") // isdeleted copyright (c) not me

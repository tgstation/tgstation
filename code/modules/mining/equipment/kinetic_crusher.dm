	...
	if(proximity_flag && isliving(target))
		...
		if(!QDELETED(L))
			...
			if((user.dir & backstab_dir) && (L.dir & backstab_dir))
				...
				L.apply_damage(detonation_damage + backstab_bonus, BRUTE, blocked = def_check, , crit_array = list(IS_BLUNT, NOT_POINTED, TRUE))
				playsound(user, 'sound/weapons/kenetic_accel.ogg', 100, 1) //Seriously who spelled it wrong
			else
				if(!QDELETED(C))
					C.total_damage += detonation_damage
				L.apply_damage(detonation_damage, BRUTE, blocked = def_check, crit_array = list(IS_BLUNT, NOT_POINTED, TRUE))

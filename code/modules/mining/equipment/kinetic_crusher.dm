...
/obj/item/kinetic_crusher/afterattack(mob/living/target, mob/living/user, clickparams)
	...
	if((user.dir & backstab_dir) && (target.dir & backstab_dir) || boosted_mark)
		...
		PLAYSOUND(user, 'sound/items/weapons/kinetic_accel.ogg').volume(100).vary_frequency(TRUE).play() //Seriously who spelled it wrong
	if(!QDELETED(crusher_damage_effect))
...
/obj/item/kinetic_crusher/proc/fire_kinetic_blast(atom/target, mob/living/user, list/modifiers)
	...
	PLAYSOUND(user, 'sound/items/weapons/plasma_cutter.ogg').volume(100).vary_frequency(TRUE).play()
	destabilizer.fire()
	charged = FALSE
	update_appearance()
...
/obj/item/kinetic_crusher/proc/recharge_projectile()
	if(!charged)
		charged = TRUE
		update_appearance()
		PLAYSOUND(src.loc, 'sound/items/weapons/kinetic_reload.ogg').volume(60).vary_frequency(TRUE).play()
...
/obj/item/kinetic_crusher/on_saboteur(datum/source, disrupt_duration)
	. = ..()
	set_light_on(FALSE)
	PLAYSOUND(src, 'sound/items/weapons/empty.ogg').volume(100).vary_frequency(TRUE).play()
	return TRUE
...
/obj/item/crusher_trophy/tail_spike/on_mark_detonation(mob/living/target, mob/living/user)
	for(var/mob/living/living_target in oview(2, user))
		if(user.faction_check_atom(living_target) || living_target.stat == DEAD)
			continue
		PLAYSOUND(living_target, 'sound/effects/magic/fireball.ogg').volume(20).vary_frequency(TRUE).play()

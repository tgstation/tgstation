...
/obj/structure/liquid_plasma_extraction_pipe/wrench_act(mob/living/user, obj/item/tool)
	...
	if(!do_after(user, 2 SECONDS, src, interaction_key = DOAFTER_SOURCE_PIPE_CONSTRUCTION))
...
/obj/structure/liquid_plasma_extraction_pipe/welder_act(mob/living/user, obj/item/tool)
	...
	pipe_state = PIPE_STATE_FINE
	if(!connected_hub.currently_functional && connected_hub.check_parts())
 ...
 */
/obj/structure/liquid_plasma_extraction_pipe/ending
	icon_state = "pipe_ending"
	base_icon_state = "pipe_ending"
	pipe_state = PIPE_STATE_FINE //Because it's under a geyser, construction would be annoying. Let's skip it and give it to them for free.

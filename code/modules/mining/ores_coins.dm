...
/obj/item/weapon/twohanded/required/gibtonite/proc/GibtoniteReaction(mob/user, triggered_by = 0)
	if(!primed)
		...
		if(primed)
			if(quality == 3)
				SSexplosion.Create(src.loc,2,4,9,adminlog = notify_admins)
			if(quality == 2)
				SSexplosion.Create(src.loc,1,2,5,adminlog = notify_admins)
			if(quality == 1)
				SSexplosion.Create(src.loc,-1,1,3,adminlog = notify_admins)
			qdel(src)

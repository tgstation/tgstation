...
/obj/structure/punching_bag/attack_hand(mob/user as mob)
	...
	playsound(loc, pick(hit_sounds), 25, DEFAULT_SOUND_VARY, extra_range = -1)
	if(isliving(user))
...
/obj/structure/weightmachine/attack_hand(mob/living/user)
	...
	if(obj_flags & IN_USE)
		...
		playsound(user, 'sound/machines/click.ogg', 60, DEFAULT_SOUND_VARY)
		obj_flags &= ~IN_USE
		user.pixel_y = 0
...
/obj/structure/weightmachine/stacklifter/AnimateMachine(mob/living/user)
	...
	while (lifts++ < 6)
		...
		sleep(3)
		playsound(user, 'goon/sound/effects/spring.ogg', 60, DEFAULT_SOUND_VARY)

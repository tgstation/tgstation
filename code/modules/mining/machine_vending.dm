...
/obj/machinery/mineral/equipment_vendor/ui_act(action, params)
	...
	switch(action)
		if("purchase")
			...
			if (!account)
				to_chat(usr, "<span class='alert'>Error: An ID with a registered bank account is required!</span>")
				flick(icon_deny, src)
				return
			...
			if(prize.cost > account.account_balance)
				to_chat(usr, "<span class='alert'>Error: Insufficient credits for [prize.equipment_name] on [I]!</span>")
				flick(icon_deny, src)
				return

...
/obj/machinery/mineral/ore_redemption/proc/process_sheet(obj/item/stack/O)
...
/obj/machinery/mineral/ore_redemption/process()
	if(!panel_open && powered()) //If the machine is partially disassembled and/or depowered, it should not process minerals
		...
		if(T)
			...
			if(B)
				for(var/obj/item/stack/O in B.contents)
					if (i >= ore_pickup_rate)
						break
					else if (!O || !O.refined_type)
						continue
...
/obj/machinery/mineral/ore_redemption/proc/SmeltMineral(obj/item/stack/O)
	if(O.refined_type)

...
/obj/item/boulder/proc/manual_process(obj/item/weapon, mob/living/user, override_speed_multiplier, continued = FALSE)
	...
	// Handle user conditions/override conditions.
		...
		playsound(src, 'sound/effects/rock/rocktap1.ogg').play()
		if(!continued)
	...
	if(durability <= 0)
		...
		playsound(src, 'sound/effects/rock/rock_break.ogg').play()
...
/obj/item/boulder/proc/break_apart()
	if(length(contents))
		...
		playsound(loc, 'sound/effects/pickaxe/picaxe1.ogg').volume(60).play()

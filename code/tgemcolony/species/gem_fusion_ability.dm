	...
	if(isgem(H))
		...
		if(H.pulling)
			...
			if(isgem(T))
				...
				if(target.gemstatus == "prime")
				...
				else if(T.mind.assigned_role == "Freemason") //Thanks to CthuluOnIce for cleaning this mess up.
					if(H.mind.assigned_role == "Crystal Gem")
						...
						return
					else if(H.mind.assigned_role != "Freemason")
						to_chat(H, "<span class='notice'>You aren't fusing with a Traitor!</span>")
						return
				else if(T.mind.assigned_role == "Crystal Gem")
					if(H.mind.assigned_role == "Freemason")
						to_chat(H, "<span class='notice'>Fuse with the Human lover? No way!</span>")
						return
					else if(H.mind.assigned_role != "Crystal Gem")
						to_chat(H, "<span class='notice'>You aren't fusing with a Traitor!</span>")
						return

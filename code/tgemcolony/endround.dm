...
/proc/checkgemloop()
	...
	var/cgamm = 0
	var/fmamm = 0
	for(var/atom/A in gemspawners)
		gems = gems+1
	for(var/var/mob/living/carbon/human/A in allgems)
		if(A.key != null) //don't count ghosted gems
	...
	if(gems == 0)
		...
		if(fmamm >= 1) //Pink diamond is ACTUALLY shattered.
			special = TRUE
			to_chat(world, "<span class='big bold'>Homeworld has failed!</span>")

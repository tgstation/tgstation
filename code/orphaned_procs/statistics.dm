...
/datum/feedback/proc/save_all_data_to_sql()
	...
	if (!SLOTH.dbcon.Connect()) return
	var/round_id

	var/DBQuery/query_feedback_max_id = SLOTH.dbcon.NewQuery("SELECT MAX(round_id) AS round_id FROM [format_table_name("feedback")]")
	...
	var/DBQuery/query_feedback_save = SLOTH.dbcon.NewQuery("INSERT DELAYED IGNORE INTO [format_table_name("feedback")] VALUES " + sqlrowlist)
...
/proc/sql_poll_population()
	...
	for(var/mob/M in SLOTH.player_list)
		if(M.client)
			playercount += 1
	var/admincount = SLOTH.admins.len
	var/DBQuery/query_record_playercount = SLOTH.dbcon.NewQuery("INSERT INTO [format_table_name("legacy_population")] (playercount, admincount, time, server_ip, server_port) VALUES ([playercount], [admincount], '[SQLtime()], INET_ATON('[world.internet_address]'), '[world.port]')")

...
/atom/movable/vv_edit_var(var_name, var_value)
	...
	if(careful_edits[var_name] && (var_value % ICONSIZE_ALL) != 0)
		return FALSE
...
/atom/movable/proc/doMove(atom/destination)
	...
	var/atom/oldloc = loc
	var/is_multi_tile = bound_width > ICONSIZE_X || bound_height > ICONSIZE_Y
		...
		if(!same_loc)
			if(is_multi_tile && isturf(destination))
				var/list/new_locs = block(
					destination,
					locate(
						min(world.maxx, destination.x + ROUND_UP(bound_width / ICONSIZE_X)),
						min(world.maxy, destination.y + ROUND_UP(bound_height / ICONSIZE_Y)),
						destination.z
					)
				)

...
/obj/machinery/door/airlock/attack_hand(mob/user, list/modifiers)
	...
	if(ishuman(user) && prob(40) && density)
		...
		if((HAS_TRAIT(H, TRAIT_DUMB)) && Adjacent(user))
			PLAYSOUND(src, 'sound/effects/bang.ogg').volume(25).vary(TRUE).play()
...
/obj/machinery/door/airlock/attackby(obj/item/C, mob/user, params)
	...
	if(is_wire_tool(C) && panel_open)
		...
		PLAYSOUND(src, 'sound/items/tools/jaws_pry.ogg').volume(30).vary(TRUE).play()
		...
		if(!user.transferItemToLoc(airlockseal, src))
			...
			return
		PLAYSOUND(src, 'sound/machines/airlock/airlockforced.ogg').volume(30).vary(TRUE).play()
...
/obj/machinery/door/airlock/try_to_crowbar(obj/item/I, mob/living/user, forced = FALSE)
	...
	if(hasPower())
		if(forced)
			...
			if(!prying_so_hard)
				var/time_to_open = 50
				PLAYSOUND(src, 'sound/machines/airlock/airlock_alien_prying.ogg').volume(100).vary(TRUE).play() //is it aliens or just the CE being a dick?
				prying_so_hard = TRUE

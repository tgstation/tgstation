...
/obj/machinery/door/airlock/attackby(obj/item/C, mob/user, params)
	...
	if(C.tool_behaviour == TOOL_SCREWDRIVER)
		...
		user.visible_message("[user] cuts down [note] from [src].", "<span class='notice'>You remove [note] from [src].</span>")
		C.play_tool_sound(src)
		...
		if(!user.transferItemToLoc(C, src))
			...
			return
		user.visible_message("[user] pins [C] to [src].", "<span class='notice'>You pin [C] to [src].</span>")
		note = C
		update_icon()
	else
		return ..()

...
/obj/machinery/door/airlock/mime
	name = "Invisible airlock"
	icon = 'icons/obj/doors/Doormime.dmi'
	desc = "If you break it, so will the invisible magic."
	doortype = 34
		...
		if( beingcrowbarred && (density && welded && !operating && src.p_open && (!src.arePowerSystemsOn() || stat & NOPOWER) && !src.locked) )
			...
			if(do_after(user,40))
				...
				switch(src.doortype)
					...
					if(34) new/obj/structure/door_assembly/door_assembly_0(src.loc)

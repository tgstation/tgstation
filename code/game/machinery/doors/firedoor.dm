...
/obj/machinery/door/firedoor/attackby(obj/item/C, mob/user, params)
	...
	if(welded)
		if(C.tool_behaviour == TOOL_WRENCH)
			...
			user.visible_message("<span class='none'>[user] starts undoing [src]'s bolts...</span>", \
								 "<span class='notice'>You start unfastening [src]'s floor bolts...</span>")
			if(!C.use_tool(src, user, 50))
				return
			playsound(get_turf(src), 'sound/items/deconstruct.ogg', 50, 1)
			user.visible_message("<span class='none'>[user] unfastens [src]'s bolts.</span>", \
								 "<span class='notice'>You undo [src]'s floor bolts.</span>")
			deconstruct(TRUE)
			return
		if(C.tool_behaviour == TOOL_SCREWDRIVER)
			user.visible_message("<span class='none'>[user] [boltslocked ? "unlocks" : "locks"] [src]'s bolts.</span>", \
...
/obj/machinery/door/firedoor/try_to_weld(obj/item/weldingtool/W, mob/user)
	if(!W.tool_start_check(user, amount=0))
		return
	user.visible_message("<span class='none'>[user] starts [welded ? "unwelding" : "welding"] [src].</span>", "<span class='notice'>You start welding [src].</span>")
	...
	switch(constructionStep)
		if(CONSTRUCTION_PANEL_OPEN)
			if(C.tool_behaviour == TOOL_CROWBAR)
				C.play_tool_sound(src)
				user.visible_message("<span class='none'>[user] starts prying something out from [src]...</span>", \
				...
				if(constructionStep != CONSTRUCTION_PANEL_OPEN)
					return
				playsound(get_turf(src), 'sound/items/deconstruct.ogg', 50, 1)
				user.visible_message("<span class='none'>[user] pries out a metal plate from [src], exposing the wires.</span>", \
			...
			if(C.tool_behaviour == TOOL_WRENCH)
				if(locate(/obj/machinery/door/firedoor) in get_turf(src))
					...
					return
				C.play_tool_sound(src)
				user.visible_message("<span class='none'>[user] starts bolting down [src]...</span>", \
									 "<span class='notice'>You begin bolting [src]...</span>")
				if(!C.use_tool(src, user, 30))
					return
				if(locate(/obj/machinery/door/firedoor) in get_turf(src))
					return
				user.visible_message("<span class='none'>[user] finishes the firelock.</span>", \
			...
			if(istype(C, /obj/item/stack/sheet/plasteel))
				...
				if(P.get_amount() < 2)
					...
					return
				user.visible_message("<span class='none'>[user] begins reinforcing [src]...</span>", \
									 "<span class='notice'>You begin reinforcing [src]...</span>")
				playsound(get_turf(src), 'sound/items/deconstruct.ogg', 50, 1)
				if(do_after(user, 60, target = src))
					if(constructionStep != CONSTRUCTION_PANEL_OPEN || reinforced || P.get_amount() < 2 || !P)
						return
					user.visible_message("<span class='none'>[user] reinforces [src].</span>", \
		...
		if(CONSTRUCTION_WIRES_EXPOSED)
			if(C.tool_behaviour == TOOL_WIRECUTTER)
				C.play_tool_sound(src)
				user.visible_message("<span class='none'>[user] starts cutting the wires from [src]...</span>", \
									 "<span class='notice'>You begin removing [src]'s wires...</span>")
				if(!C.use_tool(src, user, 60))
					return
				if(constructionStep != CONSTRUCTION_WIRES_EXPOSED)
					return
				user.visible_message("<span class='none'>[user] removes the wires from [src].</span>", \
			...
			if(C.tool_behaviour == TOOL_CROWBAR)
				C.play_tool_sound(src)
				user.visible_message("<span class='none'>[user] starts prying a metal plate into [src]...</span>", \
				...
				if(constructionStep != CONSTRUCTION_WIRES_EXPOSED)
					return
				playsound(get_turf(src), 'sound/items/deconstruct.ogg', 50, 1)
				user.visible_message("<span class='none'>[user] pries the metal plate into [src].</span>", \
		...
		if(CONSTRUCTION_GUTTED)
			if(C.tool_behaviour == TOOL_CROWBAR)
				user.visible_message("<span class='none'>[user] begins removing the circuit board from [src]...</span>", \
				...
				if(constructionStep != CONSTRUCTION_GUTTED)
					return
				user.visible_message("<span class='none'>[user] removes [src]'s circuit board.</span>", \
			...
			if(istype(C, /obj/item/stack/cable_coil))
				...
				if(B.get_amount() < 5)
					...
					return
				user.visible_message("<span class='none'>[user] begins wiring [src]...</span>", \
									 "<span class='notice'>You begin adding wires to [src]...</span>")
				playsound(get_turf(src), 'sound/items/deconstruct.ogg', 50, 1)
				if(do_after(user, 60, target = src))
					if(constructionStep != CONSTRUCTION_GUTTED || B.get_amount() < 5 || !B)
						return
					user.visible_message("<span class='none'>[user] adds wires to [src].</span>", \
		...
		if(CONSTRUCTION_NOCIRCUIT)
			if(C.tool_behaviour == TOOL_WELDER)
				if(!C.tool_start_check(user, amount=1))
					return
				user.visible_message("<span class='none'>[user] begins cutting apart [src]'s frame...</span>", \
									 "<span class='notice'>You begin slicing [src] apart...</span>")

				if(C.use_tool(src, user, 40, volume=50, amount=1))
					if(constructionStep != CONSTRUCTION_NOCIRCUIT)
						return
					user.visible_message("<span class='none'>[user] cuts apart [src]!</span>", \
			...
			if(istype(C, /obj/item/electronics/firelock))
				user.visible_message("<span class='none'>[user] starts adding [C] to [src]...</span>", \
				...
				if(constructionStep != CONSTRUCTION_NOCIRCUIT)
					return
				qdel(C)
				user.visible_message("<span class='none'>[user] adds a circuit to [src].</span>", \

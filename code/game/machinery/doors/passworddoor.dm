...
/obj/machinery/door/password/animation_effects(animation)
	switch(animation)
		if(DOOR_OPENING_ANIMATION)
			PLAYSOUND(get_sfx(src), door_open).volume(50).vary_frequency(TRUE).play()
		if(DOOR_CLOSING_ANIMATION)
			PLAYSOUND(get_sfx(src), door_close).volume(50).vary_frequency(TRUE).play()
		if(DOOR_DENY_ANIMATION)
			PLAYSOUND(get_sfx(src), door_deny).volume(30).vary_frequency(TRUE).play()

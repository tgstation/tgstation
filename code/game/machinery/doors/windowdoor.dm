...
/obj/machinery/door/window/open(forced = DEFAULT_DOOR_CHECKS)
	...
	PLAYSOUND(src, 'sound/machines/windowdoor.ogg').volume(100).vary_frequency(TRUE).play()
...
/obj/machinery/door/window/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = 0)
	switch(damage_type)
		if(BRUTE)
			PLAYSOUND(src, 'sound/effects/glass/glasshit.ogg').volume(90).vary_frequency(TRUE).play()
		if(BURN)

...
/obj/machinery/door/window/deconstruct(disassembled = TRUE)
	if(!(flags_1 & NODECONSTRUCT_1) && !disassembled)
		for(var/i in 1 to shards)
			drop_debris(SSwardrobe.provide_type(/obj/item/shard, src))
		if(rods)
			drop_debris(SSwardrobe.provide_type(/obj/item/stack/rods, src, SET_STACK_AMOUNT(rods)))
		if(cable)
			drop_debris(SSwardrobe.provide_type(/obj/item/stack/cable_coil, src, SET_STACK_AMOUNT(cable)))
	qdel(src)

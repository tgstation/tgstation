...
/obj/machinery/door_timer/New()
	...
	spawn(20)
		for(var/obj/machinery/door/window/brigdoor/M in range(15,src))
			if (M.id == src.id)
				targets += M

		for(var/obj/machinery/flasher/F in range(15,src))
			if(F.id == src.id)
				targets += F

		for(var/obj/structure/closet/secure_closet/brig/C in range(15,src))
			if(C.id == src.id)
				targets += C

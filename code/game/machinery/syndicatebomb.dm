...
/obj/machinery/syndicatebomb/proc/settings(mob/user)
	...
	if(in_range(src, user) && isliving(user)) //No running off and setting bombs from across the station
		...
		src.loc.visible_message("<span class='notice'>[bIcon(src, viewers(src))] timer set for [timer_set] seconds.</span>")
	if(alert(user,"Would you like to start the countdown now?",,"Yes","No") == "Yes" && in_range(src, user) && isliving(user))
		if(defused || active)
			if(defused)
				src.loc.visible_message("<span class='warning'>[bIcon(src, viewers(src))] Device error: User intervention required.</span>")
			return
		else
			src.loc.visible_message("<span class='danger'>[bIcon(src, viewers(loc))] [timer_set] seconds until detonation, please clear the area.</span>")
			activate()
			update_icon()
...
/obj/item/weapon/bombcore/training/detonate()
	...
	if(istype(holder))
		attempts++
		holder.loc.visible_message("<span class='danger'>[bIcon(holder, viewers(holder))] Alert: Bomb has detonated. Your score is now [defusals] for [attempts]. Resetting wires...</span>")
		reset()
	else
		qdel(src)
...
/obj/item/weapon/bombcore/training/defuse()
	...
	if(istype(holder))
		attempts++
		defusals++
		holder.loc.visible_message("<span class='notice'>[bIcon(holder, viewers(holder))] Alert: Bomb has been defused. Your score is now [defusals] for [attempts]! Resetting wires in 5 seconds...</span>")

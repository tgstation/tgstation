...
/obj/item/bombcore/dimensional/attack_self(mob/user)
	...
	for(var/datum/dimension_theme/theme as anything in GLOB.materials.dimensional_themes)
...
/obj/item/bombcore/dimensional/detonate()
	...
	var/theme_count = length(GLOB.materials.dimensional_themes)
	var/num_affected = 0
	for(var/turf/affected as anything in affected_turfs)
		var/datum/dimension_theme/theme_to_use
		if(isnull(chosen_theme))
			theme_to_use = GLOB.materials.dimensional_themes[GLOB.materials.dimensional_themes[rand(1, theme_count)]]
		else
			theme_to_use = GLOB.materials.dimensional_themes[chosen_theme]

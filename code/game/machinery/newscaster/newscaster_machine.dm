...
/obj/machinery/newscaster/play_attack_sound(damage, damage_type = BRUTE, damage_flag = 0)
	switch(damage_type)
		if(BRUTE)
			if(machine_stat & BROKEN)
				PLAYSOUND(loc, 'sound/effects/hit_on_shattered_glass.ogg').volume(100).vary_frequency(TRUE).play()
			else
...
/obj/machinery/newscaster/proc/print_paper(mob/user)
	...
	PLAYSOUND(get_sfx(loc), SFX_PAGE_TURN).volume(50).vary_frequency(TRUE).play()
...
/obj/machinery/newscaster/proc/news_alert(channel, update_alert = TRUE)
	if(channel)
		if(update_alert)
			...
			PLAYSOUND(loc, 'sound/machines/beep/twobeep_high.ogg').volume(75).vary_frequency(TRUE).play()
		alert = TRUE
		update_appearance()
		...
		PLAYSOUND(loc, 'sound/machines/warning-buzzer.ogg').volume(75).vary_frequency(TRUE).play()
...
/obj/machinery/newscaster/proc/delete_bounty_request()
	if(!active_request || !current_user)
		PLAYSOUND(src, 'sound/machines/buzz/buzz-sigh.ogg').volume(20).vary_frequency(TRUE).play()
		return TRUE
...
/obj/machinery/newscaster/proc/create_bounty()
	...
	for(var/obj/iterated_bounty_board as anything in GLOB.allbountyboards)
		...
		PLAYSOUND(iterated_bounty_board.loc, 'sound/effects/cashregister.ogg').volume(30).vary_frequency(TRUE).play()
/**

...
/obj/machinery/launchpad/proc/doteleport(mob/user, sending, alternate_log_name = null)
	...
	if(!hidden)
		PLAYSOUND(target, 'sound/items/weapons/flash.ogg').volume(25).vary_frequency(TRUE).play()
	...
	if(!hidden)
		...
		PLAYSOUND(target, 'sound/items/weapons/emitter2.ogg').volume(25).vary_frequency(TRUE).play()

...
/obj/machinery/launchpad/proc/doteleport(mob/user, sending, alternate_log_name = null)
	...
	create_sound(get_turf(src), 'sound/items/weapons/flash.ogg').volume(25).vary(TRUE).play()
	teleporting = TRUE

	if(!hidden)
		create_sound(target, 'sound/items/weapons/flash.ogg').volume(25).vary(TRUE).play()
	...
	if(!hidden)
		...
		create_sound(target, 'sound/items/weapons/emitter2.ogg').volume(25).vary(TRUE).play()
	...
	create_sound(get_turf(src), 'sound/items/weapons/emitter2.ogg').volume(25).vary(TRUE).play()
	var/first = TRUE

...
/// Shows if the machine is being used for a brain trauma scan.
#define KIOSK_SCANNING_NEURO (1<<2)
...
/obj/machinery/medical_kiosk/ui_data(mob/living/carbon/human/user)
	...
	data["active_status_3"] = scan_active & KIOSK_SCANNING_NEURO // Neuro Scan Check
	...
	switch(action)
		...
		if("beginScan_2")
			...
			if(paying_customer == TRUE)
				...
				paying_customer = FALSE
		if("beginScan_3")
			if(!(scan_active & KIOSK_SCANNING_NEURO))
				inuse()
			if(paying_customer == TRUE)
				scan_active |= KIOSK_SCANNING_NEURO
				paying_customer = FALSE
		if("beginScan_4")

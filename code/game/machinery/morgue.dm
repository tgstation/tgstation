...
/obj/machinery/bodycontainer
	icon = 'icons/obj/stationobjs.dmi'
	icon_state = "morgue1"
	density = TRUE
	anchored = TRUE
	max_integrity = 300
	circuit = /obj/item/circuitboard/machine/bodycontainer/morgue
	state_open = FALSE

	var/obj/machinery/tray/connected = null
	var/locked = FALSE
	dir = SOUTH
	var/message_cooldown
	var/breakout_time = 600

/obj/machinery/bodycontainer/Initialize()
	. = ..()
	GLOB.bodycontainers += src
	recursive_organ_check(src)

/obj/machinery/bodycontainer/Destroy()
	GLOB.bodycontainers -= src
	open()
	if(connected)
		qdel(connected)
...
/obj/machinery/bodycontainer/on_log(login)
	..()
	update_icon()

/obj/machinery/bodycontainer/update_icon()
	return

/obj/machinery/bodycontainer/relaymove(mob/living/user, direction)
...
/obj/machinery/bodycontainer/attack_paw(mob/user)
	return attack_hand(user)

/obj/machinery/bodycontainer/attack_hand(mob/user)
	. = ..()
	if(.)
		return
	if(locked)
...
/obj/machinery/bodycontainer/attack_robot(mob/user)
	if(!user.Adjacent(src))
		return
	return attack_hand(user)

/obj/machinery/bodycontainer/attackby(obj/P, mob/user, params)
	add_fingerprint(user)
	if(default_deconstruction_screwdriver(P))
		return
	if(default_change_direction_wrench(P))
		return
...
/obj/machinery/bodycontainer/container_resist_act(mob/living/user)
	if(!locked)
		open()
		return
...
/obj/machinery/bodycontainer/proc/open()
	recursive_organ_check(src)
...
/obj/machinery/bodycontainer/proc/close()
	playsound(src, 'sound/effects/roll.ogg', 5, TRUE)
...
/obj/machinery/tray/get_remote_view_fullscreens(mob/user)
	if(user.stat == DEAD || !(user.sight & (SEEOBJS|SEEMOBS)))
		user.overlay_fullscreen("remote_view", /atom/movable/screen/fullscreen/impaired, 2)
/*
 * Morgue
 */
/obj/machinery/bodycontainer/morgue
	name = "morgue"
...
/obj/machinery/bodycontainer/morgue/Initialize()
	. = ..()
	connected = new/obj/machinery/tray/m_tray(src)
	connected.connected = src

/obj/machinery/bodycontainer/morgue/examine(mob/user)
	. = ..()
	. += "<span class='notice'>The speaker is [beeper ? "enabled" : "disabled"]. Alt-click to toggle it.</span>"

/obj/machinery/bodycontainer/morgue/AltClick(mob/user)
	..()
...
/obj/machinery/bodycontainer/morgue/update_icon()
...
/obj/machinery/bodycontainer/crematorium/attack_robot(mob/user) //Borgs can't use crematoriums without help
	to_chat(user, "<span class='warning'>[src] is locked against you.</span>")
	return

/obj/machinery/bodycontainer/crematorium/Destroy()
	GLOB.crematoriums.Remove(src)
	return ..()

/obj/machinery/bodycontainer/crematorium/New()
	GLOB.crematoriums.Add(src)
	..()

/obj/machinery/bodycontainer/crematorium/Initialize()
	. = ..()
	connected = new /obj/machinery/tray/c_tray(src)
	connected.connected = src

/obj/machinery/bodycontainer/crematorium/connect_to_shuttle(obj/docking_port/mobile/port, obj/docking_port/stationary/dock)
	id = "[port.id]_[id]"

/obj/machinery/bodycontainer/crematorium/update_icon()
...
/obj/machinery/bodycontainer/crematorium/proc/cremate(mob/user)
	if(locked)
...
/obj/machinery/bodycontainer/crematorium/creamatorium
	name = "creamatorium"
	desc = "A human incinerator. Works well during ice cream socials."

/obj/machinery/bodycontainer/crematorium/creamatorium/cremate(mob/user)
...
/obj/machinery/tray
	icon = 'icons/obj/stationobjs.dmi'
	density = TRUE
	var/obj/machinery/bodycontainer/connected = null
	anchored = TRUE
	pass_flags = LETPASSTHROW
	max_integrity = 300

/obj/machinery/tray/Destroy()
	if(connected)
...
/obj/machinery/tray/deconstruct(disassembled = TRUE)
	new /obj/item/stack/sheet/metal (loc, 2)
	qdel(src)

/obj/machinery/tray/attack_paw(mob/user)
	return attack_hand(user)

/obj/machinery/tray/attack_hand(mob/user)
	. = ..()
	if(.)
		return
	if (src.connected)
...
/obj/machinery/tray/attackby(obj/P, mob/user, params)
...
/obj/machinery/tray/MouseDrop_T(atom/movable/O as mob|obj, mob/user)
 ...
 * Morgue tray
 */
/obj/machinery/tray/m_tray
	name = "morgue tray"
	desc = "Apply corpse before closing."
	icon_state = "morguet"

/obj/machinery/tray/m_tray/CanAllowThrough(atom/movable/mover, turf/target)
	. = ..()
...
/obj/machinery/tray/m_tray/CanAStarPass(ID, dir, caller)
	. = !density
	if(ismovable(caller))

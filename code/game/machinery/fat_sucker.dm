	...
	if(occupant && iscarbon(occupant))
		...
		if(C.type_of_meat)
			...
			while(nutrients >= nutrient_to_meat)
				...
				meat.set_custom_materials(list(GetMaterialRef(/datum/material/meat/mob, C) = MINERAL_MATERIAL_AMOUNT * 4))
			while(nutrients >= nutrient_to_meat / 3)
				...
				meat.set_custom_materials(list(GetMaterialRef(/datum/material/meat/mob, C) = round(MINERAL_MATERIAL_AMOUNT * (4/3))))
			nutrients = 0

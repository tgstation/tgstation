		...
		if(src.allowed(usr))
			...
			if(can_control(usr, R))
				...
				if(choice == "Confirm" && can_control(usr, R) && !..())
					...
					if(R.connected_ai)
						to_chat(R.connected_ai, "[!R.lockcharge ? "<span class='notice'>NOTICE - Cyborg lockdown lifted" : "<span class='alert'>ALERT - Cyborg lockdown detected"]: <a href='?src=[REF(R.connected_ai)];speakerreference=[REF(R)];track=[html_encode(Clean_up_hashtags(R.name))]'>[R.name]</a></span><br>")

	...
	if (href_list["temp"])
		...
		if(src.allowed(usr))
			...
			if(can_control(usr, R))
				var/choice = input("Are you certain you wish to detonate [R.name]?", choices = list("Confirm", "Abort"))
		...
		if(src.allowed(usr))
			...
			if(can_control(usr, R))
				var/choice = input("Are you certain you wish to [R.canmove ? "lock down" : "release"] [R.name]?", choices = list("Confirm", "Abort"))
				if(choice == "Confirm" && can_control(usr, R) && !..())

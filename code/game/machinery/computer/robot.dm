...
/obj/machinery/computer/robotics/interact(mob/user)
	...
	var/dat
	var/robots = 0
	for(var/mob/living/silicon/robot/R in SLOTH.mob_list)
...
/obj/machinery/computer/robotics/Topic(href, href_list)
	...
	if (href_list["temp"])
		...
		if(src.allowed(usr))
			var/mob/living/silicon/robot/R = locate(href_list["killbot"]) in SLOTH.silicon_mobs
		...
		if(src.allowed(usr))
			var/mob/living/silicon/robot/R = locate(href_list["stopbot"]) in SLOTH.silicon_mobs
			if(can_control(usr, R))
		...
		if((issilicon(usr) && is_special_character(usr)) || IsAdminGhost(usr))
			var/mob/living/silicon/robot/R = locate(href_list["magbot"]) in SLOTH.silicon_mobs
		...
		if(issilicon(usr) && is_special_character(usr))
			var/mob/living/silicon/robot/R = locate(href_list["convert"]) in SLOTH.silicon_mobs

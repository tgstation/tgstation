...
/obj/machinery/computer/arcade/battle/proc/on_battle_win(mob/user)
	...
	create_sound(loc, 'sound/machines/arcade/win.ogg').volume(40).play()
...
/obj/machinery/computer/arcade/battle/proc/user_take_damage(mob/user, base_damage_taken)
	...
	if(player_current_hp <= 0)
		...
		create_sound(loc, 'sound/machines/arcade/lose.ogg').volume(40).vary(TRUE).play()
		lose_game(user)
	else
		feedback_message = "User took [damage_taken] damage!"
		create_sound(loc, 'sound/machines/arcade/hit.ogg').volume(40).vary(TRUE).extra_range(-3).play()
		SStgui.update_uis(src)
...
/obj/machinery/computer/arcade/battle/proc/process_player_attack(mob/user, attack_type)
	...
	switch(attack_type)
		...
		if(BATTLE_ARCADE_PLAYER_COUNTERATTACK)
			...
			create_sound(loc, 'sound/machines/arcade/mana.ogg').volume(40).vary(TRUE).extra_range(-3).play()
...
/obj/machinery/computer/arcade/battle/proc/successful_counterattack(mob/user)
	...
	create_sound(loc, 'sound/machines/arcade/boom.ogg').volume(40).vary(TRUE).extra_range(-3).play()
	if(enemy_hp <= 0)
		on_battle_win(user)
	SStgui.update_uis(src)
...
/obj/machinery/computer/arcade/battle/proc/perform_enemy_turn(mob/user, defending_flags = NONE)
	...
	if((enemy_hp != enemy_max_hp) && prob(chance_to_magic))
		if(enemy_mp >= 10)
			...
			create_sound(loc, 'sound/machines/arcade/heal.ogg').volume(40).vary(TRUE).extra_range(-3).play()
			SStgui.update_uis(src)
			return
		if(player_current_mp >= 5) //minimum to steal
			...
			create_sound(loc, 'sound/machines/arcade/steal.ogg').volume(40).vary(TRUE).play()
			SStgui.update_uis(src)
			return
		...
		if(UI_PANEL_SHOP)
			switch(action)
				if("sleep")
					...
					create_sound(loc, 'sound/vehicles/mecha/skyfall_power_up.ogg').volume(40).play()
			...
			switch(action)
				...
				if("continue_with_rest")
					if(prob(60))
						...
						create_sound(loc, 'sound/machines/defib/defib_zap.ogg').volume(40).play()
						if(prob(40))

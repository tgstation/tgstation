...
/obj/machinery/computer/scan_consolenew/ui_act(action, list/params)
	...
	switch(action)
		...
		if("scramble_dna")
			...
			scanner_occupant.add_component(/datum/component/genetic_damage, GENETIC_DAMAGE_STRENGTH_MULTIPLIER*50/(connected_scanner.damage_coeff ** 2))
			return
		...
		if("pulse_gene")
			...
			scanner_occupant.add_component(/datum/component/genetic_damage, GENETIC_DAMAGE_STRENGTH_MULTIPLIER/connected_scanner.damage_coeff)
...
/obj/machinery/computer/scan_consolenew/proc/apply_genetic_makeup(type, buffer_slot)
	...
	switch(type)
		if("ui")
			...
			scanner_occupant.add_component(/datum/component/genetic_damage, damage_increase)
			scanner_occupant.domutcheck()
			return TRUE
		if("uf")
			// GUARD CHECK - There's currently no way to save partial genetic data.
...
/obj/machinery/computer/scan_consolenew/proc/set_connected_scanner(new_scanner)
	if(connected_scanner)
		unregister_signal(connected_scanner, COMSIG_PARENT_QDELETING)
		if(connected_scanner.linked_console == src)
			connected_scanner.set_linked_console(null)
	connected_scanner = new_scanner
	if(connected_scanner)
		register_signal(connected_scanner, COMSIG_PARENT_QDELETING, .proc/react_to_scanner_del)

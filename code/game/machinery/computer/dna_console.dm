...
/obj/machinery/computer/scan_consolenew/Topic(href, href_list)
	...
	switch(href_list["task"])
		...
		if("setduration")
			if(!num)
				num = round(input(usr, "Choose pulse duration:", "Input an Integer", null, nullable = TRUE, isnum = TRUE))
			if(num)
				radduration = Wrap(num, 1, RADIATION_DURATION_MAX+1)
		if("setstrength")
			if(!num)
				num = round(input(usr, "Choose pulse strength:", "Input an Integer", null, nullable = TRUE, isnum = TRUE))
			if(num)
		...
		if("setbufferlabel")
			var/text = sanitize(input(usr, "Input a new label:", "Input an Text", null, nullable = TRUE, istext = TRUE))
			if(num && text)

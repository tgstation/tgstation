...
/obj/machinery/computer/scan_consolenew/proc/ShowInterface(mob/user, last_change)
	...
	if(connected && connected.is_operational())
		if(connected.occupant)	//set occupant_status message
			...
			if(viable_occupant.has_dna() && (!isplasmaman(viable_occupant)) && (!(viable_occupant.disabilities & NOCLONE) || (connected.scan_level == 3))) //occupant is viable for dna modification

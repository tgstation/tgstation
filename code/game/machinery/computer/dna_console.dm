...
/obj/machinery/computer/scan_consolenew/proc/ShowInterface(mob/user, last_change)
	...
	if(connected && connected.is_operational())
		if(connected.occupant)	//set occupant_status message
			...
			if(viable_occupant.has_dna() && !viable_occupant.has_trait(TRAIT_RADIMMUNE) && !viable_occupant.has_trait(TRAIT_NOCLONE) && (!viable_occupant.has_trait(TRAIT_HUSK) || (connected.scan_level >= 3))) //occupant is viable for dna modification
...
/obj/machinery/computer/scan_consolenew/proc/get_viable_occupant()
	...
	if(connected)
		viable_occupant = connected.occupant
		if(!istype(viable_occupant) || !viable_occupant.dna || viable_occupant.has_trait(TRAIT_RADIMMUNE) || viable_occupant.has_trait(TRAIT_NOCLONE) || (viable_occupant.has_trait(TRAIT_HUSK) && (connected.scan_level < 3)))
			viable_occupant = null

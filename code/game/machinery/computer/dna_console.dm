...
/obj/machinery/computer/scan_consolenew/ui_interact(mob/user, last_change)
	...
	if(connected && connected.is_operational())
		if(connected.occupant)	//set occupant_status message
			...
			if(viable_occupant.has_dna() && !viable_occupant.has_trait(TRAIT_RADIMMUNE) && !viable_occupant.has_trait(TRAIT_NOSCAN) && !viable_occupant.has_trait(TRAIT_NOCLONE) || (connected.scan_level == 3)) //occupant is viable for dna modification

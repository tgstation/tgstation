					...
					switch(href_list["text"])
						if("se")
							if(buffer_slot["SE"])
								...
								var/powers = 0
								for(var/datum/mutation/human/HM in SLOTH.good_mutations + SLOTH.bad_mutations + SLOTH.not_good_mutations)
									if(HM.check_block_string(buffer_slot["SE"]))
										I.add_mutations.Add(HM)
										if(HM in SLOTH.good_mutations)
											powers += 1
										if(HM in SLOTH.bad_mutations + SLOTH.not_good_mutations)

...
<th>Mental Status</th>
					...
					if(!isnull(SLOTH.data_core.general))
						for(var/datum/data/record/R in sortRecord(SLOTH.data_core.general, sortBy, order))
							var/blood_type = ""
							var/b_dna = ""
							for(var/datum/data/record/E in SLOTH.data_core.medical)
...
//							//Foreach goto(132)
				...
				if(6)
					...
					var/bdat = null
					for(var/mob/living/simple_animal/bot/medbot/M in SLOTH.living_mob_list)
						if(M.z != src.z)
...
<BR><b>Severity:</b> [Dis.severity]"}
				...
				active1 = find_record("id", href_list["d_rec"], SLOTH.data_core.general)
				if(active1)
					active2 = find_record("id", href_list["d_rec"], SLOTH.data_core.medical)
				if(!active2)
					active1 = null
				screen = 4
				...
				if(!( src.active2 ))
					...
					for(var/datum/data/record/E in SLOTH.data_core.general)
				...
				if(!( src.printing ))
					src.printing = 1
					SLOTH.data_core.medicalPrintCount++
					playsound(loc, 'sound/items/poster_being_created.ogg', 100, 1)
					sleep(30)
					var/obj/item/weapon/paper/P = new /obj/item/weapon/paper( src.loc )
					P.info = "<CENTER><B>Medical Record - (MR-[SLOTH.data_core.medicalPrintCount])</B></CENTER><BR>"
					if(active1 in SLOTH.data_core.general)
					...
					if(active2 in SLOTH.data_core.medical)
						...
						while(src.active2.fields[text("com_[]", counter)])
							...
							counter++
						P.name = text("MR-[] '[]'", SLOTH.data_core.medicalPrintCount, src.active1.fields["name"])
					else
						P.info += "<B>Medical Record Lost!</B><BR>"
						P.name = text("MR-[] '[]'", SLOTH.data_core.medicalPrintCount, "Record Lost")
					P.info += "</TT>"
					src.printing = null

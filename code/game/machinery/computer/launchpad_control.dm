...
/obj/machinery/computer/launchpad/ui_interact(mob/user, ui_key = "main", datum/tgui/ui = null, force_open = ZERO, datum/tgui/master_ui = null, datum/ui_state/state = GLOB.default_state)
...
/obj/machinery/computer/launchpad/ui_act(action, params)
	...
	switch(action)
		...
		if("reset")
			if(!current_pad.teleporting)
				current_pad.y_offset = ZERO
				current_pad.x_offset = ZERO
				current_pad.update_indicator()
			. = TRUE

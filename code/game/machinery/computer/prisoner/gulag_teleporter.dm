...
/obj/machinery/computer/prisoner/gulag_teleporter_computer/proc/teleport(mob/user)
	...
	playsound(src, 'sound/items/weapons/emitter.ogg').volume(50).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
	...
	playsound(src, get_sfx(SFX_PORTAL_ENTER)).volume(50).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
	if(teleporter.locked)

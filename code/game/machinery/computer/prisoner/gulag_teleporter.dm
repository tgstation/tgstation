...
/obj/machinery/computer/prisoner/gulag_teleporter_computer/proc/teleport(mob/user)
	...
	create_sound(src, 'sound/items/weapons/emitter.ogg').vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
	...
	create_sound(src, SFX_PORTAL_ENTER).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
	if(teleporter.locked)

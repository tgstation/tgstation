		...
		if(!settlers.len)
			...
			if(food <= 0)
				...
				if(obj_flags & EMAGGED && isliving(user))
					var/mob/living/L
					L.set_nutrition(0) //yeah you pretty hongry
					to_chat(L, "<span class='userdanger'>Your body instantly contracts to that of one who has not eaten in months. Agonizing cramps seize you as you fall to the floor.</span>")
			if(fuel <= 0)

	...
	if(gameStatus == ORION_STATUS_GAMEOVER)
		...
		if(!settlers.len)
			...
			if(food <= 0)
				...
				if(obj_flags & EMAGGED && iscarbon(user))
					var/mob/living/carbon/C
					C.set_nutrition(0) //yeah you pretty hongry
					to_chat(C, "<span class='userdanger'>Your body instantly contracts to that of one who has not eaten in months. Agonizing cramps seize you as you fall to the floor.</span>")
			if(fuel <= 0)

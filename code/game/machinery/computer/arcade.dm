...
/obj/machinery/computer/arcade/battle/Topic(href, href_list)
	...
	if (!blocked && !gameover)
		if (href_list["attack"])
			...
			playsound(loc, 'sound/arcade/hit.ogg', 50, 1, extra_range =-3, falloff = 10)
			updateUsrDialog()
			if(turtle > 0)
				turtle--
			...
			playsound(loc, 'sound/arcade/heal.ogg', 50, 1, extra_range =-3, falloff = 10)
			updateUsrDialog()
			turtle++
			...
			playsound(loc, 'sound/arcade/mana.ogg', 50, 1, extra_range =-3, falloff = 10)
			player_mp += chargeamt
			if(turtle > 0)
				turtle--
...
/obj/machinery/computer/arcade/battle/proc/arcade_action(mob/user)
	if ((enemy_mp <= 0) || (enemy_hp <= 0))
		if(!gameover)
			...
			playsound(loc, 'sound/arcade/win.ogg', 50, 1, extra_range =-3, falloff = 10)
		...
		playsound(loc, 'sound/arcade/boom.ogg', 50, 1, extra_range =-3, falloff = 10)
		player_hp -= boomamt

	else if ((enemy_mp <= 5) && (prob(70)))
		var/stealamt = rand(2,3)
		temp = "[enemy_name] steals [stealamt] of your power!"
		playsound(loc, 'sound/arcade/steal.ogg', 50, 1, extra_range =-3, falloff = 10)
		player_mp -= stealamt
		updateUsrDialog()
		...
		if (player_mp <= 0)
			...
			playsound(loc, 'sound/arcade/lose.ogg', 50, 1, extra_range =-3, falloff = 10)
			if(obj_flags & EMAGGED)
				usr.gib()
	...
	if (href_list["continue"]) //Continue your travels
				...
				switch(event)
					if(ORION_TRAIL_RAIDERS)
						if(prob(50))
							...
							playsound(loc, 'sound/weapons/genhit2.ogg', 100, DEFAULT_SOUND_VARY)
					...
					if(ORION_TRAIL_FLUX)
						if(prob(75))
							...
							playsound(loc, 'sound/weapons/genhit.ogg', 100, DEFAULT_SOUND_VARY)
						else
							to_chat(M, "<span class='userdanger'>A violent gale blows past you, and you barely manage to stay standing!</span>")
					if(ORION_TRAIL_COLLISION) //by far the most damaging event
						if(prob(90))
							playsound(loc, 'sound/effects/bang.ogg', 100, DEFAULT_SOUND_VARY)
							...
							playsound(loc, 'sound/effects/bang.ogg', 50, DEFAULT_SOUND_VARY)
					if(ORION_TRAIL_MALFUNCTION)
						playsound(loc, 'sound/effects/empulse.ogg', 50, DEFAULT_SOUND_VARY)
						...
						if(electronics)
							...
							food = oldfood
							fuel = oldfuel
							playsound(loc, 'sound/machines/chime.ogg', 50, DEFAULT_SOUND_VARY)
		...
		if(turns == 7)
			if(prob(75))
				...
				event()
				if(obj_flags & EMAGGED)
					playsound(loc, 'sound/effects/supermatter.ogg', 100, DEFAULT_SOUND_VARY)
		...
		if(gameStatus == ORION_STATUS_NORMAL || event == ORION_TRAIL_LING)
			...
			playsound(loc,'sound/weapons/gunshot.ogg', 100, DEFAULT_SOUND_VARY)
...
/obj/item/orion_ship/attack_self(mob/user) //Minibomb-level explosion. Should probably be more because of how hard it is to survive the machine! Also, just over a 5-second fuse
	...
	playsound(loc, 'sound/machines/defib_SaftyOn.ogg', 25, DEFAULT_SOUND_VARY)
	...
	playsound(loc, 'sound/machines/buzz-sigh.ogg', 25, DEFAULT_SOUND_VARY)
	sleep(3.6)
...
/obj/machinery/computer/arcade/amputation/attack_hand(mob/user)
	...
	if(do_after(c_user, 50, target = src))
		...
		playsound(loc, 'sound/weapons/slice.ogg', 25, DEFAULT_SOUND_VARY, extra_range = -1)

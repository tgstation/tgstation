/obj/machinery/computer/warrant//TODO:SANITY
	...
	var/screen = null
	var/datum/record/crew/current = null
...
/obj/machinery/computer/warrant/ui_interact(mob/user)
	...
	if(authenticated)
		...
		if(current)
			...
			for(var/datum/data/crime/c in current.citation)
			...
			for(var/datum/data/crime/c in current.crim)
				dat += {"<tr><td>[c.crimeName]</td>
	...
	switch(href_list["choice"])
		if("Login")
			if(isliving(M))
				...
				if(authenticated)
					current = find_record("name", authenticated, GLOB.data_core.general)
		...
		if("Pay")
			for(var/datum/data/crime/p in current.citation)
				if(p.dataId == text2num(href_list["cdataid"]))
					...
					if(C && istype(C))
						...
						if(!pay)
							...
							GLOB.data_core.payCitation(current.id, text2num(href_list["cdataid"]), pay)

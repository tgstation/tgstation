...
/obj/machinery/computer/records/proc/insert_new_record(mob/user, obj/item/photo/mugshot)
	if(!mugshot || !is_operational || !user.canPerformAction(src, be_close = !issilicon(user)))
		return FALSE
	...
	if(!name || !is_operational || !user.canPerformAction(src, be_close = !issilicon(user)) || !mugshot || QDELETED(mugshot) || QDELETED(src))
		return FALSE
...
/obj/machinery/computer/records/proc/secure_login(mob/user)
	if(!user.canPerformAction(src, be_close = !issilicon(user)) || !is_operational)
		return FALSE

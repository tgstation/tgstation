/obj/structure/AIcore
	...
	var/obj/item/medical/mmi/brain = null
		...
		if(0)
			if(istype(P, /obj/item/tool/wrench))
				...
				if(do_after(user, 20))
					...
					anchored = 1
					state = 1
			if(istype(P, /obj/item/tool/welder))
				var/obj/item/tool/welder/WT = P
				if(!WT.isOn())
				...
				if(do_after(user, 20))
					...
					new /obj/item/part/stack/sheet/plasteel( loc, 4)
					del(src)
		if(1)
			if(istype(P, /obj/item/tool/wrench))
				playsound(loc, 'sound/items/Ratchet.ogg', 50, 1)
				if(do_after(user, 20))
					...
					anchored = 0
					state = 0
			if(istype(P, /obj/item/part/circuitboard/aicore) && !circuit)
		...
		if(3)
			if(istype(P, /obj/item/part/wirecutters))
				if (brain)
					...
					state = 2
					icon_state = "2"
					var/obj/item/part/cable_coil/A = new /obj/item/part/cable_coil( loc )
					A.amount = 5

			if(istype(P, /obj/item/part/stack/sheet/rglass))
				if(P:amount >= 2)
					playsound(loc, 'sound/items/Deconstruct.ogg', 50, 1)
					if(do_after(user, 20))
						if (P)
			...
			if(istype(P, /obj/item/part/board/aiModule/asimov))
			...
			if(istype(P, /obj/item/part/board/aiModule/purge))
				laws.clear_inherent_laws()
				usr << "Law module applied."


			if(istype(P, /obj/item/part/board/aiModule/freeform))
				var/obj/item/part/board/aiModule/freeform/M = P
				laws.add_inherent_law(M.newFreeFormLaw)
				usr << "Added a freeform law."

			if(istype(P, /obj/item/medical/mmi))
				if(!P:brainmob)
		...
		if(4)
			if(istype(P, /obj/item/tool/crowbar))
				...
				if (brain)
					...
					icon_state = "3"
				new /obj/item/part/stack/sheet/rglass( loc, 2 )
				return

		...
		if(3)
			...
			if (!src.active_record)
				...
				if(diskette && istype(diskette.fields))
					...
					dat += "<b>Contents:</b> "
					var/list/L = list()
					if(src.diskette.fields["UI"])	L += "Unique Identifier"
					if(src.diskette.fields["UE"])	L += "Unique Enzymes"
					if(src.diskette.fields["SE"])	L += "Structural Enzymes"
...
/obj/machinery/computer/cloning/Topic(href, href_list)
	...
	if ((href_list["scan"]) && (!isnull(src.scanner)))
		...
		switch(href_list["disk"])
			if("load")
				if (!diskette || !istype(diskette.fields) || !diskette.fields["name"])
		...
		if (!diskette || diskette.read_only || !active_record || !istype(active_record.fields))
			src.temp = "<font class='bad'>Save error.</font>"
			src.updateUsrDialog()
			return

		src.diskette.fields = src.active_record.fields.Copy()
		src.diskette.name = "data disk - '[src.diskette.fields["name"]]'"

			...
			if(istype(src, /obj/machinery/computer/card/centcom)) // REE
				...
				for(var/A in get_all_centcom_access())
					if(A in inserted_modify_id.access)
						accesses += "<a href='?src=[REF(src)];choice=access;access_target=[A];allowed=0'><font color=\"black\">[replacetext(get_centcom_access_desc(A), " ", "&nbsp")]</font></a> "
					else
				...
				for(var/i = 1; i <= 7; i++)
					...
					for(var/A in get_region_accesses(i))
						if(A in inserted_modify_id.access)
							accesses += "<a href='?src=[REF(src)];choice=access;access_target=[A];allowed=0'><font color=\"black\">[replacetext(get_access_desc(A), " ", "&nbsp")]</font></a> "
						else

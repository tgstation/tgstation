/datum/orderable_item/bitrunning
	category_index = CATEGORY_BITRUNNING

/datum/orderable_item/bitrunning/cornchips
	item_path = /obj/item/food/cornchips
	cost_per_order = 100

/datum/orderable_item/bitrunning/mountain_wind
	item_path = /obj/item/reagent_containers/cup/soda_cans/space_mountain_wind
	cost_per_order = 100

/datum/orderable_item/bitrunning/pwr_game
	item_path = /obj/item/reagent_containers/cup/soda_cans/pwr_game
	cost_per_order = 200

/datum/orderable_item/bitrunning/grey_bull
	item_path = /obj/item/reagent_containers/cup/soda_cans/grey_bull
	cost_per_order = 200

/datum/orderable_item/bitrunning/medkit
	item_path = /obj/item/storage/medkit/brute
	cost_per_order = 500

/datum/orderable_item/bitrunning/medkit_fire
	item_path = /obj/item/storage/medkit/fire
	desc = "Great after heated gaming sessions."
	cost_per_order = 500

/datum/orderable_item/bitrunning/oval_sunglasses
	item_path = /obj/item/clothing/glasses/sunglasses/oval
	cost_per_order = 1000

/datum/orderable_item/bitrunning/trenchcoat
	item_path = /obj/item/clothing/suit/jacket/trenchcoat
	cost_per_order = 1000

/datum/orderable_item/bitrunning/jackboots
	item_path = /obj/item/clothing/shoes/jackboots
	cost_per_order = 1000

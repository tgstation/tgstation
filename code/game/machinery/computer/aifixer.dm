...
/obj/machinery/computer/aifixer/transfer_ai(interaction, mob/user, mob/living/silicon/ai/AI, obj/item/device/aicard/card)
	...
	if(interaction == AI_TRANS_FROM_CARD)
		...
		if(occupier && !active)
			...
			to_chat(user, "<span class='boldnotice'>Transfer successful</span>: [occupier.name] ([SSrng.random(1000,9999)].exe) removed from host terminal and stored within local memory.")
			occupier.loc = card
			card.AI = occupier
			occupier = null
			update_icon()

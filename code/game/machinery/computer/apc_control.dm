...
/obj/machinery/computer/apc_control/Topic(href, href_list)
	...
	if(href_list["restore_logging"])
		to_chat(usr, "<span class='robot notice'>[bIcon(src, usr)] Logging functionality restored from backup data.</span>")
		emagged = FALSE
	...
	if(href_list["access_apc"])
		...
		if(!APC || APC.aidisabled || APC.panel_open || QDELETED(APC))
			to_chat(usr, "<span class='robot danger'>[bIcon(src, usr)] APC does not return interface request. Remote access may be disabled.</span>")
			return
		if(active_apc)
			to_chat(usr, "<span class='robot danger'>[bIcon(src, usr)] Disconnected from [active_apc].</span>")
			active_apc.say("Remote access canceled. Interface locked.")
			playsound(active_apc, 'sound/machines/boltsdown.ogg', 25, 0)
			playsound(active_apc, 'sound/machines/terminal_alert.ogg', 50, 0)
			active_apc.locked = TRUE
			active_apc.update_icon()
			active_apc = null
		to_chat(usr, "<span class='robot notice'>[bIcon(src, usr)] Connected to APC in [APC.area]. Interface request sent.</span>")

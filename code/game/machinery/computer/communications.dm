...
/obj/machinery/computer/communications/emag_act(mob/user, obj/item/card/emag/emag_card)
	if(istype(emag_card, /obj/item/card/emag/battlecruiser))
		...
		PLAYSOUND(src, 'sound/machines/terminal/terminal_alert.ogg').vary(FALSE).play()
		return TRUE
...
/obj/machinery/computer/communications/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch (action)
		...
		if ("changeSecurityLevel")
			...
			if (!HAS_SILICON_ACCESS(user))
				...
				if (!istype(id_card))
					...
					PLAYSOUND(src, 'sound/machines/terminal/terminal_prompt_deny.ogg').vary(FALSE).play()
					return
		...
		if ("requestNukeCodes")
			...
			PLAYSOUND(src, 'sound/machines/terminal/terminal_prompt.ogg').vary(FALSE).play()
			COOLDOWN_START(src, important_action_cooldown, IMPORTANT_ACTION_COOLDOWN)
		...
		if ("toggleAuthentication")
			...
			if (obj_flags & EMAGGED)
				...
				PLAYSOUND(src, 'sound/machines/terminal/terminal_alert.ogg').volume(25).vary(FALSE).play()

...
/obj/machinery/computer/communications/emag_act(mob/user, obj/item/card/emag/emag_card)
	if(istype(emag_card, /obj/item/card/emag/battlecruiser))
		...
		playsound(src, 'sound/machines/terminal/terminal_alert.ogg').play()
		return TRUE
...
/obj/machinery/computer/communications/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch (action)
		...
		if ("changeSecurityLevel")
			...
			if (!HAS_SILICON_ACCESS(user))
				...
				if (!istype(id_card))
					...
					playsound(src, 'sound/machines/terminal/terminal_prompt_deny.ogg').play()
					return
		...
		if ("requestNukeCodes")
			...
			playsound(src, 'sound/machines/terminal/terminal_prompt.ogg').play()
			COOLDOWN_START(src, important_action_cooldown, IMPORTANT_ACTION_COOLDOWN)
		...
		if ("toggleAuthentication")
			...
			if (authorize_name)
				...
				authorize_name = null
				playsound(src, 'sound/machines/terminal/terminal_off.ogg').play()
				return
			...
			if (obj_flags & EMAGGED)
				...
				playsound(src, 'sound/machines/terminal/terminal_alert.ogg').volume(25).play()
			...
			state = STATE_MAIN
			playsound(src, 'sound/machines/terminal/terminal_on.ogg').play()

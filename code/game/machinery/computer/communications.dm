	...
	switch(href_list["operation"])
			...
			if (I && istype(I))
				if(ACCESS_CAPTAIN in I.access)
					...
					if(GLOB.security_level != old_level)
						...
						deadchat_broadcast("<span class='name'>[usr.real_name]</span>", " has changed the security level to [security_level] with [src] at <span class='name'>[get_area_name(usr, TRUE)]</span>.", usr)
					tmp_alertlevel = 0
				else
		...
		if("crossserver")
			if(authenticated==2)
				...
				deadchat_broadcast("<span class='bold'>[usr.real_name]", " has sent an outgoing message to the other station(s).</span>", usr)
		...
		if("enableemergency")
			...
			deadchat_broadcast("<span class='name'>[usr.real_name]</span>", " enabled emergency maintenance access at <span class='name'>[get_area_name(usr, TRUE)]</span>.", usr)
			state = STATE_DEFAULT
		if("disableemergency")
			revoke_maint_all_access()
			log_game("[key_name(usr)] disabled emergency maintenance access.")
			message_admins("[ADMIN_LOOKUPFLW(usr)] disabled emergency maintenance access.")
			deadchat_broadcast("<span class='name'>[usr.real_name]</span>", " disabled emergency maintenance access at <span class='name'>[get_area_name(usr, TRUE)]</span>.", usr)
			state = STATE_DEFAULT
		...
		if("MessageCentCom")
			if(authenticated)
				...
				deadchat_broadcast("<span class='name'>[usr.real_name]</span>", " has messaged CentCom, \"[input]\" at <span class='name'>[get_area_name(usr, TRUE)]</span>.", usr)
				CM.lastTimeUsed = world.time
		...
		if("MessageSyndicate")
			if((authenticated) && (obj_flags & EMAGGED))
				...
				deadchat_broadcast("<span class='name'>[usr.real_name]</span>", " has messaged the Syndicate, \"[input]\" at <span class='name'>[get_area_name(usr, TRUE)]</span>.", usr)
				CM.lastTimeUsed = world.time
		...
		if("ai-securitylevel")
			...
			if(GLOB.security_level != old_level)
				...
				deadchat_broadcast("<span class='name'>[usr.real_name]</span>", " has changed the security level to [security_level] from [src] at [get_area_name(usr, TRUE)].</span>", usr)
			tmp_alertlevel = 0
			aistate = STATE_DEFAULT
		...
		if("ai-enableemergency")
			...
			log_game("[key_name(usr)] enabled emergency maintenance access.")
			message_admins("[ADMIN_LOOKUPFLW(usr)] enabled emergency maintenance access.")
			deadchat_broadcast("<span class='bold'>[usr.real_name]", " enabled emergency maintenance access.</span>", usr)
			aistate = STATE_DEFAULT
		if("ai-disableemergency")
			revoke_maint_all_access()
			log_game("[key_name(usr)] disabled emergency maintenance access.")
			message_admins("[ADMIN_LOOKUPFLW(usr)] disabled emergency maintenance access.")
			deadchat_broadcast("<span class='bold'>[usr.real_name]", " disabled emergency maintenance access.</span>", usr)
			aistate = STATE_DEFAULT
...
/obj/machinery/computer/communications/proc/make_announcement(mob/living/user, is_silicon)
	...
	deadchat_broadcast("<span class='name'>[user.real_name]</span>", " made a priority announcement from <span class='name'>[get_area_name(usr, TRUE)]</span>.", user)

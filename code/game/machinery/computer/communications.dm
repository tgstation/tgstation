					...
					if(SSshuttle.shuttle_purchased)
						...
						if(SSshuttle.points >= S.credit_cost)
							...
							if(M)
								...
								minor_announce("[usr.name] has purchased [S.name] for [S.credit_cost] woolongs." , "Shuttle Purchase")
							...
							to_chat(usr, "Not enough woolongs.")
...
/obj/machinery/computer/communications/attack_hand(mob/user)
	...
	switch(src.state)
		...
		if(STATE_PURCHASE)
			dat += "Budget: [SSshuttle.points] Woolongs.<BR>"
			for(var/shuttle_id in SSmapping.shuttle_templates)
				var/datum/map_template/shuttle/S = SSmapping.shuttle_templates[shuttle_id]
				if(S.can_be_bought && S.credit_cost < INFINITY)
					dat += "[S.name] | [S.credit_cost] Woolongs<BR>"

...
/obj/machinery/drone_dispenser/process()
	...
	switch(mode)
		if(DRONE_READY)
			...
			if(work_sound)
				playsound(src, work_sound).vary(TRUE).play()
		...
		if(DRONE_PRODUCTION)
			...
			if(create_sound)
				playsound(src, create_sound).vary(TRUE).play()
		...
		if(DRONE_RECHARGING)
			if(recharge_sound)
				playsound(src, recharge_sound).vary(TRUE).play()
			if(recharge_message)
...
/obj/machinery/drone_dispenser/atom_break(damage_flag)
	...
	if(break_sound)
		playsound(src, break_sound).vary(TRUE).play()

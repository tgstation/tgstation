...
/obj/machinery/sleeper/proc/sleeperUI(mob/user)
	...
	dat += "<h3>Injector</h3>"
	if(occupant)
		dat += "<A href='?src=\ref[src];inject=ephedrine'>Inject ephedrine</A>"
	else
		dat += "<span class='linkOff'>Inject ephedrine</span>"
...
/obj/machinery/sleeper/Topic(href, href_list)
	...
	if(occupant && occupant.stat != DEAD)
		if(href_list["inject"] == "ephedrine" || occupant.health > min_health)
...
/obj/machinery/sleeper/proc/inject_chem(mob/user, chem)
	...
	if(occupant && occupant.reagents)
		if(chem in injection_chems + "ephedrine")

...
/obj/machinery/sleeper/proc/inject_chem(chem,mob/user)
	if((chem in available_chems) && chem_allowed(chem))
		if(user && istype(occupant,/mob/living))
			var/mob/living/M = occupant
			M.log_message("<font color='orange'>[user.real_name]([user.ckey]) injected [M.real_name]([M.key]) with 10 units of [chem] in a sleeper.</font>", INDIVIDUAL_ATTACK_LOG)

...
/obj/machinery/recycler/proc/eat(atom/movable/morsel, sound=TRUE)
	...
	for(var/i = length(nom); i >= 1; i--)
		...
		if (isitem(nom_obj))
			// Whether or not items consume full power depends on if they produced a materialwhen recycled.
			full_power_usage = recycle_item(nom_obj)
		else
			// When a non-item is eaten, we deconstruct it with dismantled = FALSE such that

/obj/machinery/porta_turret_fixed
	name = "turret"
...
/obj/machinery/porta_turret_fixed/New()
	..()
	if(!base)
		base = src
...
/obj/machinery/porta_turret_fixed/proc/get_base_icon()
	if(installation)
		switch(installation)
...
/obj/machinery/porta_turret_fixed/Destroy()
...
/obj/machinery/porta_turret_fixed/attack_ai(mob/user)
	return attack_hand(user)


/obj/machinery/porta_turret_fixed/attack_hand(mob/user)
	. = ..()
	if(.)
		return
	var/dat
...
/obj/machinery/porta_turret_fixed/Topic(href, href_list)
	if(..())
		return
	usr.set_machine(src)
	add_fingerprint(usr)
...
/obj/machinery/porta_turret_fixed/power_change()
...
/obj/machinery/porta_turret_fixed/attackby(obj/item/I, mob/user, params)
	if(stat & BROKEN)
...
/obj/machinery/porta_turret_fixed/attacked_by(obj/item/I, mob/user)
	if(I.force)
		user.visible_message("<span class='danger'>[user] has hit [src] with [I]!</span>", "<span class='danger'>You hit [src] with [I]!</span>")
	take_damage(I.force * 0.5, I.damtype)


/obj/machinery/porta_turret_fixed/emag_act(mob/user)
	if(!emagged)
...
/obj/machinery/porta_turret_fixed/bullet_act(obj/item/projectile/P)
	. = ..()
...
/obj/machinery/porta_turret_fixed/emp_act(severity)
	if(on && emp_vunerable)
...
/obj/machinery/porta_turret_fixed/ex_act(severity, target)
...
/obj/machinery/porta_turret_fixed/take_damage(damage, damage_type = BRUTE, sound_effect = 1)
	switch(damage_type)
		if(BRUTE)
			if(sound_effect)
				if(damage)
...
/obj/machinery/porta_turret_fixed/proc/die()	//called when the turret dies, ie, health <= 0
	health = 0
	density = 0
...
/obj/machinery/porta_turret_fixed/process()
...
/obj/machinery/porta_turret_fixed/proc/calculate_targets()
	...
	for(var/A in turretview)
		if(check_anomalies)//if it's set to check for simple animals
			if(istype(A, /mob/living/simple_animal))
				...
				if(SA.stat) //don't target if dead or is pet or bot
					continue
				targets += SA
		if(istype(A, /mob/living/carbon))
			...
			//if the target is a human analyze threat level
...
/obj/machinery/porta_turret_fixed/proc/tryToShootAt(list/atom/movable/targets)
	while(targets.len > 0)
...
/obj/machinery/porta_turret_fixed/proc/popUp()	//pops the turret up
	if(disabled)
		return
	if(raising || raised)
		return
...
/obj/machinery/porta_turret_fixed/proc/popDown()	//pops the turret down
	if(disabled)
		return
	if(raising || !raised)
		return
...
/obj/machinery/porta_turret_fixed/proc/assess_perp(mob/living/carbon/human/perp)
...
/obj/machinery/porta_turret_fixed/proc/in_faction(mob/target)
	if(!(faction in target.faction))
		return 0
	return 1

/obj/machinery/porta_turret_fixed/proc/target(atom/movable/target)
	if(disabled)
		return
	if(target)
		spawn()
...
/obj/machinery/porta_turret_fixed/proc/shootAt(atom/movable/target)
...
/obj/machinery/porta_turret_fixed/proc/setState(on, emagged)
	if(controllock)
		return
	src.on = on
	src.emagged = emagged
...
/obj/machinery/porta_turret_fixed/stationary
	emagged = 1

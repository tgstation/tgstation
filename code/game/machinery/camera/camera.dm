	...
	if((istype(W, /obj/item/weapon/paper) || istype(W, /obj/item/device/pda)) && isliving(user))
		...
		for(var/mob/O in player_list)
			if(isAI(O))
				...
				var/record_id
				if(U.can_see_face())
					var/faceprint = U.get_faceprint()
					var/datum/data/record/G
					if(faceprint)
						G = find_record("faceprint", faceprint, data_core.general)
					if(G)
						record_id = G.fields["id"]
				to_chat(AI, "<b><a href='?src=\ref[AI][AI.ai_track_href(U, record_id)]'>[IDENTITY_SUBJECT(1)]</a></b> holds <a href='?_src_=usr;show_paper=1;'>\a [itemname]</a> up to one of your cameras ...", list(U))
				AI.last_paper_seen = "<HTML><HEAD><TITLE>[itemname]</TITLE></HEAD><BODY><TT>[info]</TT></BODY></HTML>"
			else if (O.client && O.client.eye == src)
				to_chat(O, "[IDENTITY_SUBJECT(1)] holds \a [itemname] up to one of the cameras ...", list(U))

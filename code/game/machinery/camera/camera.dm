		...
		for(var/mob/O in GLOB.player_list)
			if(isAI(O))
				...
				if(U.name == "Unknown")
					...
					to_chat(AI, "<b><a href='?src=[REF(AI)];speakerreference=[REF(U)];track=[html_encode(Clean_up_hashtags(U.name))]'>[U]</a></b> holds <a href='?_src_=usr;show_paper=1;'>\a [itemname]</a> up to one of your cameras ...")

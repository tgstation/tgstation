...
/mob/living/silicon/ai/proc/ai_actual_track(mob/living/target)
	...
	if(!(target && target.can_track(src)))
		src << "<span class='warning'>Target is not near any active cameras.</span>"
		return
	...
	spawn(0)
		while(cameraFollow == target)
			...
			if(!target.can_track(src))
				tracking = 1
				if(!cameraticks)
					src << "<span class='warning'>Target is not near any active cameras. Attempting to reacquire...</span>"
				cameraticks++
				if(cameraticks > 9)
					cameraFollow = null
					src << "<span class='warning'>Unable to reacquire, cancelling track...</span>"
					tracking = 0
					return
				else
					sleep(10)

/obj/machinery/space_heater
			...
			if(open)
				if(cell)
					...
					if(istype(C))
						...
						user.visible_message("<span class='notice'>[user] inserts a power cell into [src].", "\blue You insert the power cell into [src].</span>")
			else
				user << "The hatch must be open to insert a power cell."
				return
		else if(istype(I, /obj/item/weapon/screwdriver))
			open = !open
			user.visible_message("<span class='notice'>[user] [open ? "opens" : "closes"] the hatch on the [src].", "\blue You [open ? "open" : "close"] the hatch on the [src].</span>")
			update_icon()
	...
	attack_hand(mob/user as mob)
		...
		if(open)
			...
			on = !on
			user.visible_message("<span class='notice'>[user] switches [on ? "on" : "off"] the [src].","\blue You switch [on ? "on" : "off"] the [src].</span>")
			update_icon()
		return
		...
		switch(href_list["op"])
			...
			if("cellremove")
				if(open && cell && !usr.get_active_hand())
					...
					cell = null
					usr.visible_message("<span class='notice'>[usr] removes the power cell from \the [src].", "\blue You remove the power cell from \the [src].</span>")
			...
			if("cellinstall")
				if(open && !cell)
					...
					if(istype(C))
						...
						usr.visible_message("<span class='notice'>[usr] inserts a power cell into \the [src].", "\blue You insert the power cell into \the [src].</span>")

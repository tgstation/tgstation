...
/obj/machinery/jukebox/ui_interact(mob/user)
	...
	if(!allowed(user))
		...
		user.playsound_local(src,'sound/misc/compiler-failure.ogg', 25, DEFAULT_SOUND_VARY)
		return
	if(!songs.len)
		to_chat(user,"<span class='warning'>Error: No music tracks have been authorized for your station. Petition Central Command to resolve this issue.</span>")
		playsound(src,'sound/misc/compiler-failure.ogg', 25, DEFAULT_SOUND_VARY)
		return
	var/list/dat = list()
...
/obj/machinery/jukebox/process()
	if(world.time < stop && active)
		...
		dance_over()
		playsound(src,'sound/machines/terminal_off.ogg',50,DEFAULT_SOUND_VARY)
		update_icon()
		stop = world.time + 100

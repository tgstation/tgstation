...
/obj/machinery/computer/atmos_points/interact(mob/user)
	...
	var/dat = text("Time until market update: [round((market_cooldown - world.time)/10)] seconds<br>")
	dat += "</div>"
	dat += text("<A href='?src=\ref[src];action=send'>Export Canister</A> to <A href='?src=\ref[src];action=choose'> [export_name]</A><br>")
	dat += text("<A href='?src=\ref[src];action=scan'>Scan Canister Contents</A><br><br>")
	dat += "</div>"
	dat += text("<u><b>Economy canister order for [economy_name]:</b></u><br>")

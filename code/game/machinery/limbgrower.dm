...
/obj/machinery/limbgrower
	name = "limb grower"
	desc = "It grows new limbs using Synthflesh." // austation -- revert cobbychem
...
/obj/machinery/limbgrower/Topic(href, href_list)
	...
	if (!busy)
		...
		if(href_list["make"])
			...
			var/synth_cost = being_built.reagents_list[/datum/reagent/medicine/synthflesh]*prod_coeff // austation -- revert cobbychem
			var/power = max(2000, synth_cost/5)

			if(reagents.has_reagent(/datum/reagent/medicine/synthflesh, being_built.reagents_list[/datum/reagent/medicine/synthflesh]*prod_coeff)) // austation -- revert cobbychem
				busy = TRUE
				use_power(power)
...
/obj/machinery/limbgrower/proc/build_item()
	if(reagents.has_reagent(/datum/reagent/medicine/synthflesh, being_built.reagents_list[/datum/reagent/medicine/synthflesh]*prod_coeff))	//sanity check, if this happens we are in big trouble // austation begin -- revert cobbychem
		reagents.remove_reagent(/datum/reagent/medicine/synthflesh,being_built.reagents_list[/datum/reagent/medicine/synthflesh]*prod_coeff) // austation end
		...
		src.visible_message("<span class='warning'>Something went very wrong, there isn't enough synthflesh anymore!</span>") // austation -- revert cobbychem
	busy = FALSE
...
/obj/machinery/limbgrower/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += "<span class='notice'>The status display reads: Storing up to <b>[reagents.maximum_volume]u</b> of synthflesh.<br>Synthflesh consumption at <b>[prod_coeff*100]%</b>.</span>" // austation -- revert cobbychem
...
/obj/machinery/limbgrower/proc/can_build(datum/design/D)
	return (reagents.has_reagent(/datum/reagent/medicine/synthflesh, D.reagents_list[/datum/reagent/medicine/synthflesh]*prod_coeff)) //Return whether the machine has enough synthflesh to produce the design // austation -- revert cobbychem

	...
	if(inserted)
		if(istype(O,/obj/item/stack))
			if (O.materials[MAT_METAL])
				flick("limbgrower_on",src)//plays metal insertion animation
			if (O.materials[MAT_GLASS])
				flick("limbgrower_on",src)//plays glass insertion animation
...
/obj/item/proc/limbgrower_crafted(obj/machinery/limbgrower/A)
	return

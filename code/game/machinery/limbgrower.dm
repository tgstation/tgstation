...
/obj/machinery/limbgrower/Topic(href, href_list)
	...
	if (!busy)
		...
		if(href_list["category"] in categories) //Don't let people send invalid categories
			selected_category = sanitize(href_list["category"]) //Sanitize just in case, selected_category is displayed to anyone who looks at the machine in several places

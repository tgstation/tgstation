...
/obj/machinery/limbgrower
	name = "limb grower"
	desc = "It grows new limbs using Synthflesh (Syniphen)."
...
/obj/machinery/limbgrower/Topic(href, href_list)
	...
	if (!busy)
		...
		if(href_list["make"])
			...
			var/synth_cost = being_built.reagents_list[/datum/reagent/medicine/C2/syniphen]*prod_coeff
			var/power = max(2000, synth_cost/5)

			if(reagents.has_reagent(/datum/reagent/medicine/C2/syniphen, being_built.reagents_list[/datum/reagent/medicine/C2/syniphen]*prod_coeff))
				busy = TRUE
				use_power(power)
...
/obj/machinery/limbgrower/proc/build_item()
	if(reagents.has_reagent(/datum/reagent/medicine/C2/syniphen, being_built.reagents_list[/datum/reagent/medicine/C2/syniphen]*prod_coeff))	//sanity check, if this happens we are in big trouble
		reagents.remove_reagent(/datum/reagent/medicine/C2/syniphen,being_built.reagents_list[/datum/reagent/medicine/C2/syniphen]*prod_coeff)
		...
		src.visible_message("<span class='warning'>Something went very wrong, there isn't enough syniphen anymore!</span>")
	busy = FALSE
...
/obj/machinery/limbgrower/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += "<span class='notice'>The status display reads: Storing up to <b>[reagents.maximum_volume]u</b> of syniphen.<br>Syniphen consumption at <b>[prod_coeff*100]%</b>.</span>"
...
/obj/machinery/limbgrower/proc/can_build(datum/design/D)
	return (reagents.has_reagent(/datum/reagent/medicine/C2/syniphen, D.reagents_list[/datum/reagent/medicine/C2/syniphen]*prod_coeff)) //Return whether the machine has enough syniphen to produce the design

/obj/machinery/limbgrower/proc/get_design_cost(datum/design/D)
	var/dat
	if(D.reagents_list[/datum/reagent/medicine/C2/syniphen])
		dat += "[D.reagents_list[/datum/reagent/medicine/C2/syniphen] * prod_coeff] Synthetic flesh "
	return dat

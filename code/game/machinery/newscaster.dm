...
/obj/machinery/newscaster/play_attack_sound(damage, damage_type = BRUTE, damage_flag = 0)
	switch(damage_type)
		if(BRUTE)
			if(stat & BROKEN)
				playsound(loc, 'sound/effects/hit_on_shattered_glass.ogg', 100, DEFAULT_SOUND_VARY)
			else
...
/obj/machinery/newscaster/obj_break()
	if(!(stat & BROKEN) && !(flags_1 & NODECONSTRUCT_1))
		stat |= BROKEN
		playsound(loc, 'sound/effects/glassbr3.ogg', 100, DEFAULT_SOUND_VARY)
		update_icon()
...
/obj/machinery/newscaster/proc/newsAlert(channel)
	if(channel)
		...
		playsound(loc, 'sound/machines/twobeep.ogg', 75, 0.4, 2)
	else
		say("Attention! Wanted issue distributed!")
		playsound(loc, 'sound/machines/warning-buzzer.ogg', 75, DEFAULT_SOUND_VARY)
...
/obj/item/newspaper/suicide_act(mob/user)
	...
	playsound(H.loc, 'sound/items/drink.ogg', rand(10,50), DEFAULT_SOUND_VARY)
		...
		if(href_list["next_page"])
			...
			if(curr_page == pages) //We're at the middle, get to the end
				...
				if(curr_page == 0) //We're at the start, get to the middle
					screen=1
			curr_page++
			playsound(loc, "pageturn", 50, DEFAULT_SOUND_VARY)

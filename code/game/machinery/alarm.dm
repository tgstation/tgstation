...
/obj/machinery/alarm/attackby(obj/item/W as obj, mob/user as mob)
	switch(buildstage)
		if(2)
			if(istype(W, /obj/item/part/wirecutters) && AAlarmwires == 0)
				playsound(src.loc, 'sound/items/Wirecutter.ogg', 50, 1)
				user << "You cut the final wires."
				var/obj/item/part/cable_coil/cable = new /obj/item/part/cable_coil( src.loc )
				cable.amount = 5
				buildstage = 1
				update_icon()
				return
			...
			if (wiresexposed && ((istype(W, /obj/item/tool/multitool) || istype(W, /obj/item/part/wirecutters))))
				...
				return
		if(1)
			if(istype(W, /obj/item/tool/crowbar) && AAlarmwires == 0)
				user << "You pry out the circuit."
				playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)
				spawn(20)
					new /obj/item/part/board/air_alarm( src.loc )
			...
			if(istype(W, /obj/item/part/cable_coil))
				var/obj/item/part/cable_coil/cable = W
				if(cable.amount < 5)
				...
				return
		if(0)
			if(istype(W, /obj/item/part/board/air_alarm))
			...
			if(istype(W, /obj/item/tool/wrench))
				...
				new /obj/item/part/frame/air_alarm( user.loc )
				del(src)
				return
...
/obj/item/part/frame/air_alarm/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if (istype(W, /obj/item/tool/wrench))
...
/obj/item/part/frame/air_alarm/proc/try_build(turf/on_wall)
		...
		switch(buildstage)
			if(2)
				if (istype(W, /obj/item/tool/multitool))
					...
					buildstage = 1
					playsound(src.loc, 'sound/items/Wirecutter.ogg', 50, 1)
					var/obj/item/part/cable_coil/coil = new /obj/item/part/cable_coil()
					coil.amount = 5
					coil.loc = user.loc
			...
			if(1)
				if(istype(W, /obj/item/part/cable_coil))
					...
					playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)
					spawn(20)
						var/obj/item/part/board/fire_alarm/circuit = new /obj/item/part/board/fire_alarm()
						circuit.loc = user.loc
						buildstage = 0
						update_icon()
			if(0)
				if(istype(W, /obj/item/part/board/fire_alarm))
					user << "You insert the circuit!"
					del(W)
					buildstage = 1
					update_icon()
					...
					var/obj/item/part/frame/fire_alarm/frame = new /obj/item/part/frame/fire_alarm()
					frame.loc = user.loc
					playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
					del(src)
		return
...
*/
/obj/item/part/board/fire_alarm
...
Code shamelessly copied from apc_frame
*/
/obj/item/part/frame/fire_alarm
...
/obj/item/part/frame/fire_alarm/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if (istype(W, /obj/item/tool/wrench))
		new /obj/item/part/stack/sheet/metal( get_turf(src.loc), 2 )
		del(src)
		return
...
/obj/item/part/frame/fire_alarm/proc/try_build(turf/on_wall)
	if (get_dist(on_wall,usr)>1)
		return

...
/obj/machinery/firealarm/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if (istype(W, /obj/item/weapon/wirecutters))
		detecting = !detecting
		user.visible_message("\red \The [user] has [detecting ? "re" : "dis" ]connected \the [src]'s detecting unit!",\
		"You have [detecting ? "re" : "dis" ]connected \the [src]'s detecting unit.")
	else
		alarm()
	add_fingerprint(user)
	return

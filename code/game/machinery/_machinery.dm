...
/obj/machinery/can_interact(mob/user)
	if((machine_stat & (NOPOWER|BROKEN) & !istype(/obj/machinery, /obj/machinery/plumbing)) && !(interaction_flags_machine & INTERACT_MACHINE_OFFLINE)) // Check if the machine is broken, and if we can still interact with it if so
		return FALSE

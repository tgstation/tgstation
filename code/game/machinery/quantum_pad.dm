...
/obj/machinery/quantumpad/examine(mob/user)
	..()
	to_chat(user, "<span class='info'>It is [ linked_pad ? "currently" : "not"] linked to another pad.</span>")
	if(!panel_open)
		to_chat(user, "<span class='info'>The panel is <i>screwed</i> in, obstructing the linking device.</span>")
	else
		to_chat(user, "<span class='info'>The <i>linking</i> device is now able to be <i>scanned<i> with a multitool.</span>")

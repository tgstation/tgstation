...
/obj/machinery/defibrillator_mount/examine(mob/user)
	...
	if(defib)
		...
		if(SSsecurity_level.get_current_level(TRUE) >= SEC_LEVEL_RED)
...
/obj/machinery/defibrillator_mount/attackby(obj/item/I, mob/living/user, params)
	...
	if(id)
		if(check_access(id) || SSsecurity_level.get_current_level(TRUE) >= SEC_LEVEL_RED) //anyone can toggle the clamps in red alert!
			if(!defib)

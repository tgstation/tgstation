/obj/machinery/transformer
	...
	desc = "A large metallic machine with an entrance and an exit. A sign on \
		...
		between making cyborgs or assimilated androids"
	...
	/// Whether this machine transforms dead mobs into cyborgs/assimilated carbons
	var/transform_dead = FALSE
	/// Whether this machine transforms standing mobs into cyborgs/assimilated carbons
	...
	var/robot_cell_charge = STANDARD_CELL_CHARGE * 30 //Tier 3 cells
	///Whether this machine transforms mobs to assimilated androids, else transforms them to cyborgs
	var/is_assimilation_mode = TRUE
...
/obj/machinery/transformer/examine(mob/user)
	. = ..()
	. += span_notice("It is currently set to producing: [is_assimilation_mode ? "Assimilated Carbons" : "Cyborgs"]")
...
/obj/machinery/transformer/attack_ai_secondary(mob/user, list/modifiers)
	. = ..()
	is_assimilation_mode = !is_assimilation_mode
	var/settings_message = is_assimilation_mode ? "Assimilated Carbons" : "Cyborgs"
...
/obj/machinery/transformer/proc/do_transform(mob/living/carbon/human/victim)
	...
	if(is_assimilation_mode)
		victim.set_species(/datum/species/android)
		if(master_ai && victim.get_organ_by_type(/obj/item/organ/brain) && !victim?.mind.has_antag_datum(/datum/antagonist/assimilated_carbon))
			var/datum/brain_trauma/special/assimilated_carbon/trauma = victim.gain_trauma(/datum/brain_trauma/special/assimilated_carbon)

...
/obj/machinery/dna_scannernew/container_resist_act(mob/living/user)
	...
	if(do_after(user,(breakout_time), target = src))
		if(!user || IS_UNCONSCIOUS_CRIT_OR_DEAD(user) || user.loc != src || state_open || !locked || HAS_TRAIT(user, TRAIT_PRIMITIVE) || user.ai_controller)
			return
		locked = FALSE
...
/obj/machinery/dna_scannernew/relaymove(mob/living/user, direction)
	if(IS_UNCONSCIOUS_CRIT_OR_DEAD(user) || locked)

...
/obj/machinery/firealarm/update_overlays()
	...
	if(!(my_area?.fire || LAZYLEN(my_area?.active_firelocks)))
		if(my_area?.fire_detect) //If this is false, leave the green light missing. A good hint to anyone paying attention.
			...
			. += emissive_appearance(icon, "fire_allgood", src, alpha = src.alpha)
	else if(obj_flags & EMAGGED) // Wallening todo: should we display the emag image with higher priority then the good one?
...
INVERT_MAPPING_DIRECTIONAL_HELPERS(/obj/machinery/firealarm, 26)

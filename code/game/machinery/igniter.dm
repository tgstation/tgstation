...
/obj/machinery/sparker/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/screwdriver))
		...
		if (src.disable)
			user.visible_message("<span class='danger'>[user] has disabled the [src]!", "\red You disable the connection to the [src].</span>")
			icon_state = "[base_state]-d"
		if (!src.disable)
			user.visible_message("<span class='danger'>[user] has reconnected the [src]!", "\red You fix the connection to the [src].</span>")
			if(src.powered())
				icon_state = "[base_state]"
			else
				icon_state = "[base_state]-p"

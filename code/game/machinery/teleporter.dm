...
/obj/machinery/computer/teleporter/proc/set_target(mob/user)
	...
	if(regime_set == "Teleporter")
		...
		var/desc = input("Please select a location to lock in.", "Locking Computer", nullable = TRUE, choices = L)
		target = L[desc]
		...
		for(var/obj/machinery/teleport/station/R in S)
			...
			if(T.z == ZLEVEL_CENTCOM || T.z > ZLEVEL_SPACEMAX)
				continue
			L[avoid_assoc_duplicate_keys(T.loc.name, areaindex)] = R
		var/desc = input("Please select a station to lock in.", "Locking Computer", nullable = TRUE, choices = L)
		target = L[desc]
		if(target)

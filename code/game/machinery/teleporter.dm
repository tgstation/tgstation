...
/obj/machinery/teleport/hub/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span class='notice'>The status display reads: Probability of malfunction decreased by [(accuracy*25)-25]%.<span>")
...
/obj/machinery/teleport/station/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		to_chat(user, "<span class='notice'>The status display reads: This station can be linked to [efficiency] other station(s).<span>")

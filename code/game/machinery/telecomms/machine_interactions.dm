...
/obj/machinery/telecomms/ui_interact(mob/user, datum/tgui/ui)
	operator = user //Stolen from APC_Control, otherwise I'd have no way to track the user.
...
/obj/machinery/telecomms/ui_data(mob/user)
	...
	data += add_option() //After half a day of trial and error, it turns out that lists are pretty cool.
	...
	if(heldmultitool) //TGUI *hates* a null multitool buffer.

...
/obj/machinery/telecomms/ui_act(action, params)
	...
	switch(action)
		if("toggle")
			...
			update_power()
			update_appearance()
			operator.log_message("toggled [toggled ? "On" : "Off"] [src] at [AREACOORD(src)].", LOG_GAME)
			. = TRUE
		if("id")
			if(params["value"])
				if(length(params["value"]) > 32)
					...
					id = params["value"]
					operator.log_message("has changed the ID for [src] at [AREACOORD(src)] to [id].", LOG_GAME)
					. = TRUE
		if("network")
			if(params["value"])
				if(length(params["value"]) > 15)
					...
					links = list()
					operator.log_message("has changed the network for [src] at [AREACOORD(src)] to [network].", LOG_GAME)
					. = TRUE
		if("tempfreq")
			if(params["value"])
		...
		if("freq")
			if(tempfreq in banned_frequencies)
				...
				if(!(tempfreq in freq_listening))
					...
					operator.log_message("added frequency [tempfreq] for [src] at [AREACOORD(src)].", LOG_GAME)
					. = TRUE
		if("delete")
			freq_listening.Remove(params["value"])
			operator.log_message("removed frequency [params["value"]] for [src] at [AREACOORD(src)].", LOG_GAME)
			. = TRUE
		if("unlink")
...
/obj/machinery/telecomms/proc/add_new_link(obj/machinery/telecomms/new_connection, mob/user)
	...
	if(user)
		user.log_message("linked [src] for [new_connection] at [AREACOORD(src)].", LOG_GAME)
	return TRUE
...
/obj/machinery/telecomms/proc/remove_link(obj/machinery/telecomms/old_connection, mob/user)
	...
	if(user)
		user.log_message("unlinked [src] and [old_connection] at [AREACOORD(src)].", LOG_GAME)

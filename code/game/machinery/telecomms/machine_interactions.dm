...
/obj/machinery/telecomms/ui_act(action, params)
	...
	switch(action)
		if("toggle")
			...
			update_power()
			update_appearance()
			user.log_message("toggled [toggled ? "On" : "Off"] [src].", LOG_GAME)
			. = TRUE
		if("id")
			if(params["value"])
				if(length(params["value"]) > 32)
					to_chat(user, span_warning("Error: Machine ID too long!"))
					playsound(src, 'sound/machines/buzz-sigh.ogg', 50, TRUE)
					return
				else
					id = params["value"]
					user.log_message("has changed the ID for [src] to [id].", LOG_GAME)
					. = TRUE
		if("network")
			if(params["value"])
				if(length(params["value"]) > 15)
					to_chat(user, span_warning("Error: Network name too long!"))
					playsound(src, 'sound/machines/buzz-sigh.ogg', 50, TRUE)
					return
				else
					...
					links = list()
					user.log_message("has changed the network for [src] to [network].", LOG_GAME)
					. = TRUE
		if("tempfreq")
			if(params["value"])
		...
		if("freq")
			if(tempfreq in banned_frequencies)
				to_chat(user, span_warning("Error: Interference preventing filtering frequency: \"[tempfreq / 10] kHz\""))
				playsound(src, 'sound/machines/buzz-sigh.ogg', 50, TRUE)
			else
				if(!(tempfreq in freq_listening))
					freq_listening.Add(tempfreq)
					user.log_message("added frequency [tempfreq] for [src].", LOG_GAME)
					. = TRUE
		if("delete")
			freq_listening.Remove(params["value"])
			user.log_message("removed frequency [params["value"]] for [src].", LOG_GAME)
			. = TRUE
		if("unlink")
			var/obj/machinery/telecomms/machine_to_unlink = links[text2num(params["value"])]
			if(machine_to_unlink)
				. = remove_link(machine_to_unlink, user)
		if("link")
			if(heldmultitool)
				var/obj/machinery/telecomms/machine_to_link = heldmultitool.buffer
				. = add_new_link(machine_to_link, user)
		if("buffer")

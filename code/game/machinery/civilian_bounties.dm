...
/obj/item/bounty_cube/examine()
	. = ..()
	if(speed_bonus)
		. += span_notice("<b>[time2text(next_nag_time - world.time,"mm:ss", 0)]</b> remains until <b>[bounty_value * speed_bonus]</b> credit speedy delivery bonus lost.")
...
/obj/item/bounty_cube/proc/set_up(datum/bounty/my_bounty, obj/item/card/id/holder_id)
	...
	aas_config_announce(/datum/aas_config_entry/bounty_cube_created, list(
		...
		"BONUSTIME" = time2text(next_nag_time - world.time,"mm:ss", 0),
		"COST" = bounty_value

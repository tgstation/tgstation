...
/obj/machinery/porta_turret_construct/examine(mob/user)
	...
	switch(build_step)
		...
		if(PTURRET_INTERNAL_ARMOUR_ON)
			. += span_notice("The turret requires an <b>energy based gun</b> to fuction, the armor is secured by <i>bolts</i>.")
		if(PTURRET_GUN_EQUIPPED)
			. += span_notice("The turret requires an <b>proximity sensor</b> to fuction.")

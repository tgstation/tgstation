...
/obj/machinery/bot/mulebot/attackby(var/obj/item/I, var/mob/user)
	if(istype(I,/obj/item/weapon/card/emag))
		...
		if(prob(1+I.force * 2))
			unload(0)
			user.visible_message("<span class='danger'>[user] knocks [load] off [src] with \the [I]!", "\red You knock [load] off [src] with \the [I]!</span>")
		else
		...
		switch(href_list["op"])
			...
			if("cellremove")
				if(open && cell && !usr.get_active_hand())
					...
					usr.visible_message("<span class='notice'>[usr] removes the power cell from [src].", "\blue You remove the power cell from [src].</span>")
					updateDialog()
			...
			if("cellinsert")
				if(open && !cell)
					...
					if(istype(C))
						...
						usr.visible_message("<span class='notice'>[usr] inserts a power cell into [src].", "\blue You insert the power cell into [src].</span>")
						updateDialog()

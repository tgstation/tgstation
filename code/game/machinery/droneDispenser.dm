	...
	switch(mode)
		if(DRONE_READY)
			...
			if(begin_create_message)
				visible_message("<span class='none'>[src] [begin_create_message]</span>")
			if(work_sound)
		...
		if(DRONE_PRODUCTION)
			...
			if(end_create_message)
				visible_message("<span class='none'>[src] [end_create_message]</span>")
		...
		if(DRONE_RECHARGING)
			...
			if(recharge_message)
				visible_message("<span class='none'>[src] [recharge_message]</span>")

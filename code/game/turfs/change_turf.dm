...
/turf/proc/AfterChange(flags, oldType) //called after a turf has been replaced in ChangeTurf()
	...
	else if(!(flags & CHANGETURF_IGNORE_AIR)) //In effect, I want closed turfs to make their tile active when sheered, but we need to queue it since they have no adjacent turfs

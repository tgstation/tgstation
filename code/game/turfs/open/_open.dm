...
/turf/open/handle_slip(mob/living/carbon/C, knockdown_amount, obj/O, lube, paralyze_amount, force_drop)
	...
	if(has_gravity(src))
		...
		if(C.buckled)
			...
			if(!(lube&SLIP_WHEN_CRAWLING) && ((!(C.mobility_flags & MOBILITY_STAND) && !(lube&SLIDE_ICE)) || !(C.status_flags & CANKNOCKDOWN))) // can't slip unbuckled mob if they're lying or can't fall.
				return 0

...
/turf/floor/ex_act(severity, target)
...
/turf/floor/attack_paw(mob/user)
...
/turf/floor/proc/break_tile_to_plating()
	var/turf/floor/plating/T = make_plating()
	T.break_tile()
...
/turf/floor/proc/make_plating()
	return ChangeTurf(/turf/floor/plating)

/turf/floor/ChangeTurf(turf/floor/T)
	if(!istype(src,/turf/floor))
		return ..() //fucking turfs switch the fucking src of the fucking running procs
	if(!ispath(T,/turf/floor))
		return ..()
...
/turf/floor/attackby(obj/item/C, mob/user, params)
	if(!C || !user)
		return 1
	if(..())
		return 1
...
/turf/floor/singularity_pull(S, current_size)
...
/turf/floor/narsie_act()
	if(prob(20))
		ChangeTurf(/turf/floor/engine/cult)

/turf/floor/can_have_cabling()
	return !burnt && !broken

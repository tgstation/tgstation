...
/turf/floor/light/ChangeTurf(turf/T)
	SetLuminosity(0)
	..()

/turf/floor/light/attack_hand(mob/user)
	if(!on)
		on = 1
		currentcolor = 1
		return
...
/turf/floor/light/attack_ai(mob/user)
	attack_hand(user)

/turf/floor/light/attackby(obj/item/C, mob/user, params)
	if(..())
		return

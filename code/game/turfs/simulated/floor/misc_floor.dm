...
/turf/open/floor/clockwork/crowbar_act(mob/living/user, obj/item/I)
	if(islist(baseturfs))
		...
		return TRUE
	user.visible_message("[user] begins slowly prying up [src]...", "<span class='notice'>You begin painstakingly prying up [src]...</span>")
	if(I.use_tool(src, user, 70, volume=80))
		user.visible_message("[user] pries up [src].", "<span class='notice'>You pry up [src]!</span>")
		make_plating()
	return TRUE

...
/turf/open/floor/clock_spawn_room
	name = "cogmetal"
	desc = "Brass plating that gently radiates heat. For some reason, it reminds you of blood."
	icon_state = "reebe"
	baseturf = /turf/open/floor/clock_spawn_room

/turf/open/floor/clock_spawn_room/Entered()
	..()
	START_PROCESSING(SSfastprocess, src)

/turf/open/floor/clock_spawn_room/Destroy()
	STOP_PROCESSING(SSfastprocess, src)
	. = ..()

/turf/open/floor/clock_spawn_room/process()
	if(!port_servants())
		STOP_PROCESSING(SSfastprocess, src)

/turf/open/floor/clock_spawn_room/proc/port_servants()
	. = FALSE
	for(var/mob/living/L in src)

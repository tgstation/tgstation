...
/turf/closed/wall/r_wall/examine(mob/user)
	...
	switch(d_state)
		if(INTACT)
			to_chat(user, "The outer <i>grille</i> is fully intact.")
		if(SUPPORT_LINES)
			to_chat(user, "The outer <i>grille</i> has been cut.\nThe support lines are <i>screwed</i> securely to the outer cover.")
		if(COVER)
			to_chat(user, "The support lines have been <i>unscrewed</i>.\nThe metal cover is <i>welded</i> firmly in place.")
		if(CUT_COVER)
			to_chat(user, "The metal cover has been <i>sliced through</i>.\nIt's <i>connected loosely</i> to the girder.")
		if(BOLTS)
			to_chat(user, "The outer cover has been <i>pried away</i>.\nThe bolts anchoring the support rods are <i>wrenched</i> in place.")
		if(SUPPORT_RODS)
			to_chat(user, "The bolts anchoring the support rods have been <i>loosened</i>.\nThey are still <i>welded</i> firmly to the girder.")
		if(SHEATH)
			to_chat(user, "The support rods have been <i>sliced through</i>.\nThe outer sheath is <i>connected loosely</i> to the girder.")

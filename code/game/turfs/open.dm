...
/turf/open/handle_slip(mob/living/carbon/C, knockdown_amount, obj/O, lube)
	...
	if(has_gravity(src))
		...
		if(lube&SLIDE)
			...
			if(istype(C.force_moving,/datum/forced_movement/iceslip)) //If we're already slipping extend it
				qdel(C.force_moving)
			new /datum/forced_movement/iceslip(C, get_ranged_target_turf(C, olddir, 1), 1, FALSE)	//spinning would be bad for ice, fucks up the next dir
		return 1

/datum/job/hos
	...
	idtype = /obj/item/security/card/id/silver
	req_admin_notify = 1
...
//		H.equip_to_slot_or_del(new /obj/item/clothing/mask/gas(H), slot_wear_mask) //Grab one from the armory you donk
		...
		if(H.backbag == 1)
			...
			H.equip_to_slot_or_del(new /obj/item/security/handcuffs(H), slot_l_store)
		else
...
/datum/job/detective
	...
	equip(var/mob/living/carbon/human/H)
		...
		H.equip_to_slot_or_del(new /obj/item/part/lighter/zippo(H), slot_l_store)

		if(H.backbag == 1)//Why cant some of these things spawn in his office?
			H.equip_to_slot_or_del(new /obj/item/storage/box/survival(H), slot_r_hand)
			H.equip_to_slot_or_del(new /obj/item/storage/box/evidence(H), slot_l_hand)
			H.equip_to_slot_or_del(new /obj/item/device/scanner/forensic(H), slot_r_store)
		else
			H.equip_to_slot_or_del(new /obj/item/storage/box/survival(H.back), slot_in_backpack)
			H.equip_to_slot_or_del(new /obj/item/storage/box/evidence(H), slot_in_backpack)
			H.equip_to_slot_or_del(new /obj/item/device/scanner/forensic(H), slot_in_backpack)

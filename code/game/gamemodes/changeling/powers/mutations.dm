			...
			if(iscarbon(L))
				...
				switch(firer.a_intent)
					...
					if(INTENT_GRAB)
						...
						C.throw_at(get_step_towards(H,C), 8, 2, callback=CALLBACK(GLOBAL_PROC, /proc/addtimer, src, "tentacle_grab", 3, TIMER_NORMAL, H, C))
						return 1

					if(INTENT_HARM)
						C.visible_message("<span class='danger'>[L] is thrown towards [H] by a tentacle!</span>","<span class='userdanger'>A tentacle grabs you and throws you towards [H]!</span>")
						C.Weaken(3)
						C.throw_at(get_step_towards(H,C), 8, 2, callback=CALLBACK(GLOBAL_PROC, /proc/addtimer, src, "tentacle_stab", 3, TIMER_NORMAL, H, C))
						return 1
			else

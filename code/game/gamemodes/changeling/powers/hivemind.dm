...
/obj/effect/proc_holder/changeling/hivemind_upload/sting_action(var/mob/user)
	...
	var/chosen_name = tginput("Select a DNA to channel: ", "Channel DNA", null, nullable = TRUE, choices = names)
	if(!chosen_name)
		return
...
/obj/effect/proc_holder/changeling/hivemind_download/sting_action(mob/user)
	...
	var/S = tginput("Select a DNA absorb from the air: ", "Absorb DNA", null, nullable = TRUE, choices = names)
	if(!S)
		return

...
/datum/objective/abductee/random/New()
	explanation_text = SSrng.pick_from_list(world.file2list("strings/abductee_objectives.txt"))

/datum/objective/abductee/steal
	explanation_text = "Steal all"

/datum/objective/abductee/steal/New()
	var/target = SSrng.pick_from_list(list("pets","lights","monkeys","fruits","shoes","bars of soap", "weapons", "computers", "organs"))
...
/datum/objective/abductee/paint/New()
	var/color = SSrng.pick_from_list(list("red", "blue", "green", "yellow", "orange", "purple", "black", "in rainbows", "in blood"))
...
/datum/objective/abductee/speech/New()
	var/style = SSrng.pick_from_list(list("pantomime", "rhyme", "haiku", "extended metaphors", "riddles", "extremely literal terms", "sound effects", "military jargon", "three word sentences"))
...
/datum/objective/abductee/capture/New()
	...
	for(var/X in jobs)
		...
		if(J.current_positions < 1)
			jobs -= J
	if(jobs.len > 0)
		var/datum/job/target = SSrng.pick_from_list(jobs)
		explanation_text += " a [target.title]."
	else
		explanation_text += " someone."

/datum/objective/abductee/calling/New()
	var/mob/dead/D = SSrng.pick_from_list(GLOB.dead_mob_list)
	if(D)

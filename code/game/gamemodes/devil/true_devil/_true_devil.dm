...
/mob/living/carbon/true_devil/examine(mob/user)
	var/msg = "<span class='info'>*---------*\nThis is [bIcon(src, user)] <b>[src]</b>!\n"

	//Left hand items
	for(var/obj/item/I in held_items)
		if(!(I.flags & ABSTRACT))
			if(I.blood_DNA)
				msg += "<span class='warning'>It is holding [bIcon(I, user)] [I.gender==PLURAL?"some":"a"] blood-stained [I.name] in its [get_held_index_name(get_held_index_of_item(I))]!</span>\n"
			else
				msg += "It is holding [bIcon(I, user)] \a [I] in its [get_held_index_name(get_held_index_of_item(I))].\n"

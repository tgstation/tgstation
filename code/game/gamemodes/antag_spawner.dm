...
/obj/item/weapon/antag_spawner/contract/spawn_antag(client/C, turf/T, type = "")
	...
	var/newname = copytext(sanitize(input(M, "You are [wizard_name]'s apprentice. Would you like to change your name to something else?", "Name change", randomname, nullable = TRUE)),1,MAX_NAME_LEN)
	if (!newname)
		newname = randomname
	M.mind.name = newname
	M.real_name = newname

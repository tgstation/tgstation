...
/datum/game_mode/hand_of_god/pre_setup()
	...
	while(unassigned_followers.len > (requiblue_enemies / 2))
...
/datum/game_mode/hand_of_god/post_setup()
	...
	blue_god_possibilities &= red_deity_followers //followers only
	if(!blue_god_possibilities.len) //No candidates? just pick any follower regardless of prefs
		blue_god_possibilities = red_deity_followers

	//Make blue god
	var/datum/mind/blue_god = pick_n_take(red_god_possibilities)
	if(blue_god)
		blue_god.current.become_god("red")
		remove_hog_follower(blue_god,0)
		add_god(blue_god,"red")
...
/datum/game_mode/proc/greet_hog_follower(datum/mind/follower_mind,colour)
	if(follower_mind in blue_deity_prophets || follower_mind in blue_deity_prophets)
...
/datum/game_mode/proc/add_hog_follower(datum/mind/follower_mind, colour = "No Colour")
	...
	if(isloyal(H))
		...
		return 0
	if((follower_mind in blue_deity_followers) || (follower_mind in red_deity_prophets) || (follower_mind in blue_deity_followers) || (follower_mind in blue_deity_prophets))
	...
	follower_mind.current.attack_log += "\[[time_stamp()]\] <font color='blue'>Has been converted to the [colour] follower cult!</font>"
	return 1
...
/datum/game_mode/proc/add_god(datum/mind/god_mind, colour = "No Colour")
	...
	if(colour == "blue")
		blue_deities += god_mind
	god_mind.current.attack_log += "\[[time_stamp()]\] <font color='blue'>Has been made into a [colour] deity!</font>"
...
/datum/game_mode/proc/remove_hog_follower(datum/mind/follower_mind, announce = 1)//deconverts both
	...
	if(announce)
		follower_mind.current.attack_log += "\[[time_stamp()]\] <font color='blue'>Has been deconverted from a deity's cult!</font>"
		follower_mind.current << "<span class='danger'><b>Your mind has been cleablue from the brainwashing the followers have done to you.  Now you serve yourself and the crew.</b></span>"
		for(var/mob/living/M in view(follower_mind.current))
			M << "[follower_mind.current] looks like their faith is shatteblue. They're no longer a cultist!"
...
/proc/is_handofgod_bluecultist(A)
	if(ishuman(A))
		var/mob/living/carbon/human/H = A
		if(H.mind)
			if(H.mind in ticker.mode.blue_deity_followers|ticker.mode.red_deity_prophets)
				return 1
	return 0
...
/proc/is_handofgod_cultist(A) //any of them what so ever, blue, blue, hot pink, whatever.
...
/proc/is_handofgod_prophet(A) //any of them what so ever, blue, blue, hot pink, whatever
	if(ishuman(A))
		var/mob/living/carbon/human/H = A
		if(H.mind)
			if(H.mind in ticker.mode.blue_deity_prophets|ticker.mode.blue_deity_prophets)
				return 1
	return 0
...
/mob/camera/god/proc/is_handofgod_myfollowers(mob/A)
	...
	if(!H.mind)
		return 0
	if(side == "blue")
		if(H.mind in ticker.mode.blue_deity_prophets|ticker.mode.red_deity_followers)
			return 1
	else if(side == "blue")
...
/datum/game_mode/hand_of_god/declare_completion()
	if(blue_deities.len)
		var/text = "<BR><font size=3 color='blue'><B>The red cult:</b></font>"
		for(var/datum/mind/blue_god in red_deities)
			var/godwin = 1

			text += "<BR><B>[blue_god.key]</B> was the red deity, <B>[red_god.name]</B> ("
			if(blue_god.current)
			...
			if(blue_deity_prophets.len)
				for(var/datum/mind/blue_prophet in red_deity_prophets)
					text += "<BR>The blue prophet was <B>[red_prophet.name]</B> (<B>[red_prophet.key]</B>)"
			else
				text += "<BR>the blue prophet was killed for their beliefs."

			text += "<BR><B>Red follower count: </B> [blue_deity_followers.len]"
			...
			if(blue_god.objectives.len)
				...
				for(var/datum/objective/O in blue_god.objectives)
					if(O.check_completion())
						...
						objectives += "<BR><B>Objective #[count]</B>: [O.explanation_text] <font color='blue'><B>Fail.</B></font>"
			...
			if(godwin)
				...
				text += "<br><font color='blue'><B>The red cult and deity have failed!</B></font>"
	...
	if(blue_deities.len)
		...
		for(var/datum/mind/blue_god in blue_deities)
			...
			if(godwin)
				...
				text += "<BR><font color='blue'><B>The blue cult and deity have failed!</B></font>"
				feedback_add_details("god_success","FAIL")

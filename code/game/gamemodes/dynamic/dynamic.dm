...
/datum/game_mode/dynamic/proc/send_intercept()
	...
	for(var/datum/dynamic_ruleset/ruleset as anything in subtypesof(/datum/dynamic_ruleset))
		if(ruleset.weight <= 0)
			continue
		min_threat = min(ruleset.weight, min_threat)
	var/greenshift = (threat_level < min_threat && shown_threat < min_threat) //if both shown and real threat are below any ruleset, its extended time

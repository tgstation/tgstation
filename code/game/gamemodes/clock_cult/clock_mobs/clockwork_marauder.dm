...
/mob/living/simple_animal/hostile/clockwork/marauder/Life()
	...
	if(is_in_host())
		if(!is_servant_of_ratvar(host))
			...
			unbind_from_host()
			return
		if(!SLOTH.ratvar_awakens && host.stat == DEAD)
			death()
			return
		...
		if(SLOTH.ratvar_awakens) //If Ratvar is alive, marauders don't need a host and are downright impossible to kill
			adjustHealth(-5)
			heal_host()
		else if(host)
			if(!is_servant_of_ratvar(host))
...
/mob/living/simple_animal/hostile/clockwork/marauder/proc/heal_host()
	...
	if(iscarbon(host))
		resulthealth = round((abs(HEALTH_THRESHOLD_DEAD - host.health) / abs(HEALTH_THRESHOLD_DEAD - host.maxHealth)) * 100)
	if(SLOTH.ratvar_awakens || resulthealth <= MARAUDER_EMERGE_THRESHOLD)
...
/mob/living/simple_animal/hostile/clockwork/marauder/proc/blockOrCounter(mob/target, atom/textobject)
	if(SLOTH.ratvar_awakens) //if ratvar has woken, we block nearly everything at a very high chance
		blockchance = 90
		counterchance = 90
	if(prob(blockchance))
		. = TRUE

...
/obj/structure/destructible/clockwork/powered/tinkerers_daemon/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		...
		for(var/i in SLOTH.clockwork_component_cache)
			to_chat(user, "<span class='[get_component_span(i)]_small'><i>[get_component_name(i)]:</i> <b>[get_component_cost(i)]W</b> <i>([SLOTH.clockwork_component_cache[i]] exist[SLOTH.clockwork_component_cache[i] == 1 ? "s" : ""])</i></span>")
		...
		switch(choice)
			if("Specific Component")
				...
				for(var/mob/living/L in SLOTH.living_mob_list)
					if(is_servant_of_ratvar(L))
						servants++
				if(!is_servant_of_ratvar(user) || !user.canUseTopic(src, !issilicon(user), NO_DEXTERY) || active || !SLOTH.clockwork_caches || servants * 0.2 < SLOTH.clockwork_daemons)
					return
				if(!component_id_to_produce)
...
/obj/structure/destructible/clockwork/powered/tinkerers_daemon/proc/get_component_cost(id)
	return max(MIN_CLOCKCULT_POWER*2, (MIN_CLOCKCULT_POWER*2) * (1 + round(SLOTH.clockwork_component_cache[id] * 0.2)))
...
/obj/structure/destructible/clockwork/powered/tinkerers_daemon/process()
	...
	if(!SLOTH.clockwork_caches || servants * 0.2 < SLOTH.clockwork_daemons || . < min_power_usable) //if we don't have enough to produce the lowest or what we chose to produce, cancel out

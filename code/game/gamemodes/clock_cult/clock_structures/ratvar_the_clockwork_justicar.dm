	...
	if(!prey)
		...
		if(!prey) //In case there's a Nar-Sie
			...
			if(meals.len)
				...
				to_chat(prey, "<span class='userdanger'>Something very large and very malevolent begins lumbering its way towards you...</span>")
		...
		if((!istype(prey, /obj/singularity/narsie) && prob(10)) || is_servant_of_ratvar(prey) || prey.z != z)
			...
			to_chat(prey, "<span class='userdanger'>You feel tremendous relief as a set of horrible eyes loses sight of you...</span>")
			prey = null
		else

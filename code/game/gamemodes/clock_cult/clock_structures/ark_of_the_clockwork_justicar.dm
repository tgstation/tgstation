	...
	switch(progress_in_seconds)
		...
		if(GATEWAY_RATVAR_ARRIVAL to INFINITY)
			if(!purpose_fulfilled)
				...
				var/turf/T =  locate(round(world.maxx * 0.5, 1), round(world.maxy * 0.5, 1), round((SSmapping.FirstStationLevel() + SSmapping.LastStationLevel()) / 2)) //approximate center of the station

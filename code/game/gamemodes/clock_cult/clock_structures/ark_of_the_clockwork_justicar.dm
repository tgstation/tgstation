...
/obj/structure/destructible/clockwork/massive/celestial_gateway/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		if(!active)
			to_chat(user, "<span class='big'><b>Seconds until the Ark's activation:</b> [DisplayTimeText(get_arrival_text(TRUE))]</span>")
		else
			if(grace_period)
				to_chat(user, "<span class='big'><b>Crew grace period time remaining:</b> [DisplayTimeText(get_arrival_text(TRUE))]</span>")
			else
				to_chat(user, "<span class='big'><b>Seconds until Ratvar's arrival:</b> [DisplayTimeText(get_arrival_text(TRUE))]</span>")

...
/obj/structure/destructible/clockwork/turret/ocular_warden
	name = "ocular warden"
...
/obj/structure/destructible/clockwork/turret/ocular_warden/examine(mob/user)
	..()
	to_chat(user, "<span class='brass'>[target ? "<b>It's fixated on [target]!</b>" : "Its gaze is wandering aimlessly."]</span>")

/obj/structure/destructible/clockwork/turret/ocular_warden/hulk_damage()
	return 25

/obj/structure/destructible/clockwork/turret/ocular_warden/can_be_unfasten_wrench(mob/user, silent)
	if(!anchored)
		for(var/obj/structure/destructible/clockwork/turret/ocular_warden/W in orange(OCULAR_WARDEN_EXCLUSION_RANGE, src))
			if(W.anchored)
				if(!silent)
...
/obj/structure/destructible/clockwork/turret/ocular_warden/ratvar_act()
	..()
	if(GLOB.ratvar_awakens)
		...
		target_range = initial(target_range)

/obj/structure/destructible/clockwork/turret/ocular_warden/process()
	if(!anchored)
		lose_target()
		return
...
/obj/structure/destructible/clockwork/turret/ocular_warden/idle()
	if(prob(50))
		visible_message("<span class='notice'>[src][pick(idle_messages)]</span>")
	else
		setDir(pick(GLOB.cardinals))//Random rotation

/obj/structure/destructible/clockwork/turret/ocular_warden/attack_target()
	if(isliving(target))
		var/mob/living/L = target
		if(!L.null_rod_check())
			if(isrevenant(L))
...
/obj/structure/destructible/clockwork/turret/ocular_warden/alert_target()
	if(isliving(target))
		var/mob/living/L = target
		to_chat(L, "<span class='neovgre'>\"I SEE YOU!\"</span>\n<span class='userdanger'>[src]'s gaze [GLOB.ratvar_awakens ? "melts you alive" : "burns you"]!</span>")
	else if(ismecha(target))
...
/obj/structure/destructible/clockwork/turret/ocular_warden/get_efficiency_mod()
	if(GLOB.ratvar_awakens)
		return 2
	. = 1
	if(target)

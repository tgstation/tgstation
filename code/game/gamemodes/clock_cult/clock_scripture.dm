...
/datum/clockwork_scripture/proc/run_scripture()
	...
	if(can_recite() && has_requirements())
				...
				if(consumed_components[i])
					for(var/j in 1 to consumed_components[i])
						if(slab.stored_components[i])
							...
							SLOTH.clockwork_component_cache[i]--
		...
		if(!recital() || !check_special_requirements() || !scripture_effects()) //if we fail any of these, refund components used
			for(var/i in used_slab_components)
				if(used_slab_components[i])
					if(slab)
						...
						SLOTH.clockwork_component_cache[i] += consumed_components[i]
			...
			successful = TRUE
			if(slab && !slab.no_cost && !SLOTH.ratvar_awakens) //if the slab exists and isn't debug and ratvar isn't up, log the scripture as being used
...
/datum/clockwork_scripture/proc/has_requirements() //if we have the components and invokers to do it
	...
	if(!SLOTH.ratvar_awakens && !slab.no_cost)
		...
		var/failed = FALSE
		for(var/i in consumed_components)
			var/cache_components = SLOTH.clockwork_caches ? SLOTH.clockwork_component_cache[i] : 0
		...
		if(failed)
			...
			return FALSE
	if(multiple_invokers_used && !multiple_invokers_optional && !SLOTH.ratvar_awakens && !slab.no_cost)

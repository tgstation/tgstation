...
/obj/effect/clockwork/servant_blocker/CanPass(atom/movable/M, turf/target)
	...
	for(var/mob/living/L in target_contents)
		if(is_servant_of_ratvar(L) && get_dir(M, src) != dir && L.stat != DEAD && Ark.progress_in_seconds < GATEWAY_REEBE_FOUND) //Unless we're on the side the arrow is pointing directly away from, no-go

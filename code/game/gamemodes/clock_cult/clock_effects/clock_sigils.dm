...
/obj/effect/clockwork/sigil/Crossed(atom/movable/AM)
	...
	if(isliving(AM))
		...
		if(L.stat <= stat_affected)
			if((!is_servant_of_ratvar(L) || (affects_servants && is_servant_of_ratvar(L))) && (L.mind || L.has_status_effect(STATUS_EFFECT_SIGILMARK)) && (!isdrone(L) || istype(L, /mob/living/simple_animal/drone/cogscarab)))

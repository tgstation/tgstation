...
/obj/effect/clockwork/sigil/transmission/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		...
		for(var/obj/structure/destructible/clockwork/powered/P in range(SIGIL_ACCESS_RANGE, src))
			structure_number++
		to_chat(user, "<span class='[power_charge ? "brass":"alloy"]'>It is storing <b>[SLOTH.ratvar_awakens ? "INFINITY":"[power_charge]"]W</b> of power, \
...
/obj/effect/clockwork/sigil/vitality/examine(mob/user)
	..()
	if(is_servant_of_ratvar(user) || isobserver(user))
		to_chat(user, "<span class='[vitality ? "inathneq_small":"alloy"]'>It has access to <b>[SLOTH.ratvar_awakens ? "INFINITE":"[vitality]"]</b> units of vitality.</span>")
		if(SLOTH.ratvar_awakens)
...
//as long as they're still on the sigil and are either not a servant or they're a servant AND it has remaining vitality
	while(L && (!is_servant_of_ratvar(L) || (is_servant_of_ratvar(L) && (SLOTH.ratvar_awakens || vitality))) && get_turf(L) == get_turf(src))
		sigil_active = TRUE
		...
		if(!is_servant_of_ratvar(L))
			...
			if(L.stat == DEAD)
				...
				if(!SLOTH.ratvar_awakens && L.stat == CONSCIOUS)
			...
			if(L.stat == DEAD)
				...
				if(vitality >= revival_cost && (ghost || (L.mind && L.mind.active)))
					...
					L.visible_message("<span class='warning'>[L] suddenly gets back up, [SLOTH.ratvar_awakens ? "[L.p_their()] body dripping blue ichor":"even as [src] scatters into blue sparks around [L.p_them()]"]!</span>", \

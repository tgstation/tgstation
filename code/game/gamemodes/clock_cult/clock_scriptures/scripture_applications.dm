...
/datum/clockwork_scripture/fellowship_armory/proc/remove_item_if_better(obj/item/I, mob/user)
	...
	if(is_type_in_typecache(I, ratvarian_armor_typecache))
		return FALSE
	if(!SLOTH.ratvar_awakens && is_type_in_typecache(I, better_armor_typecache))
		return FALSE
...
/datum/clockwork_scripture/memory_allocation/check_special_requirements()
	for(var/mob/living/simple_animal/hostile/clockwork/marauder/M in SLOTH.all_clockwork_mobs)
		if(M.host == invoker)
...
/datum/clockwork_scripture/create_object/tinkerers_daemon/check_special_requirements()
	...
	if(servants * 0.2 < SLOTH.clockwork_daemons)
		...
		return FALSE
	if(servants * 0.2 < SLOTH.clockwork_daemons+1)

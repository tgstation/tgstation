	...
	if(ishuman(loc))
		...
		if(src == H.glasses && !up)
			...
			if(is_servant_of_ratvar(H))
				to_chat(H, "<span class='heavy_brass'>You push the spectacles down, and all is revealed to you.[SLOTH.ratvar_awakens ? "" : " Your eyes begin to itch - you cannot do this for long."]</span>")
...
/obj/item/clothing/glasses/wraith_spectacles/equipped(mob/living/user, slot)
	...
	if(is_servant_of_ratvar(user))
		to_chat(user, "<span class='heavy_brass'>As you put on the spectacles, all is revealed to you.[SLOTH.ratvar_awakens ? "" : " Your eyes begin to itch - you cannot do this for long."]</span>")
...
/datum/status_effect/wraith_spectacles/tick()
	...
	if(glasses_right && !WS.up && !SLOTH.ratvar_awakens)
		apply_eye_damage(H)
	else

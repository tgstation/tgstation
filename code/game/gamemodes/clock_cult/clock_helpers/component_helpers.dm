...
/proc/generate_cache_component(specific_component_id, atom/A)
	...
	SLOTH.clockwork_component_cache[specific_component_id]++
	if(A)
	...
	if(storage_slab)
		if(SLOTH.clockwork_caches)
			for(var/i in SLOTH.clockwork_component_cache)
				.[i] = max(MAX_COMPONENTS_BEFORE_RAND - LOWER_PROB_PER_COMPONENT*(SLOTH.clockwork_component_cache[i] + storage_slab.stored_components[i]), 1)
		else
			for(var/i in SLOTH.clockwork_component_cache)
				.[i] = max(MAX_COMPONENTS_BEFORE_RAND - LOWER_PROB_PER_COMPONENT*storage_slab.stored_components[i], 1)
	else
		for(var/i in SLOTH.clockwork_component_cache)
			.[i] = max(MAX_COMPONENTS_BEFORE_RAND - LOWER_PROB_PER_COMPONENT*SLOTH.clockwork_component_cache[i], 1)
	. = pickweight(.)

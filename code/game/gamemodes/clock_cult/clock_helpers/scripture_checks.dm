...
/proc/scripture_unlock_check()
	...
	for(var/mob/living/M in SLOTH.living_mob_list)
	...
	.[SCRIPTURE_SCRIPT] = (servants >= SCRIPT_SERVANT_REQ && SLOTH.clockwork_caches >= SCRIPT_CACHE_REQ)
	//Script: SCRIPT_SERVANT_REQ or more non-brain servants and SCRIPT_CACHE_REQ or more clockwork caches
	.[SCRIPTURE_APPLICATION] = (servants >= APPLICATION_SERVANT_REQ && SLOTH.clockwork_caches >= APPLICATION_CACHE_REQ && SLOTH.clockwork_construction_value >= APPLICATION_CV_REQ)
	...
	.[SCRIPTURE_REVENANT] = (servants >= REVENANT_SERVANT_REQ && SLOTH.clockwork_caches >= REVENANT_CACHE_REQ && SLOTH.clockwork_construction_value >= REVENANT_CV_REQ)
	...
	.[SCRIPTURE_JUDGEMENT] = (servants >= JUDGEMENT_SERVANT_REQ && SLOTH.clockwork_caches >= JUDGEMENT_CACHE_REQ && SLOTH.clockwork_construction_value >= JUDGEMENT_CV_REQ && !unconverted_ai_exists)
...
/proc/update_slab_info(obj/item/clockwork/slab/set_slab)
	...
	for(var/s in SLOTH.all_scripture)
		var/datum/clockwork_scripture/S = SLOTH.all_scripture[s]
		if(S.creation_update())
			needs_update = TRUE
	if(!set_slab || needs_update)
		for(var/obj/item/clockwork/slab/S in SLOTH.all_clockwork_objects)
			SStgui.update_uis(S)
			S.update_quickbind()
	else
...
/proc/generate_all_scripture()
	if(!SLOTH.all_scripture.len)
		for(var/V in sortList(subtypesof(/datum/clockwork_scripture), /proc/cmp_clockscripture_priority))
			...
			SLOTH.all_scripture[S.type] = S

//changes construction value
/proc/change_construction_value(amount)
	SLOTH.clockwork_construction_value += amount

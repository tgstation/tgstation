...
/datum/umbrage/proc/use_psi(used_psi)
	...
	for(var/path in linked_ability_types)
		var/datum/action/innate/umbrage/U = path
		if(has_ability(initial(U.id)))
			U = linked_abilities[initial(U.id)] //yes, yes, this is silly.
			U.UpdateButtonIcon()
	return TRUE

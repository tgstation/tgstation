	...
	if(iscultist(usr))
		if(chosen_rune != null)
			if(istype(target, /turf/simulated/floor)) //Floor runes.
				if(chosen_rune == "wall imbue")       //Avoids people placing wall talismans in the middle of the hall.
...
/obj/item/weapon/tome/imbued //admin tome, spawns working runes without waiting
	...
	attack_self(mob/user as mob)
		...
		if(user)
			...
			var/obj/effect/rune/R = /obj/effect/rune
...
/obj/item/weapon/tome/test //Testing shit tome
	name = "testing tome"
	desc = "An old, dusty tome with frayed edges and a sinister looking cover."
	icon_state ="tome"

/obj/item/weapon/tome/test/afterattack(atom/target as turf, mob/user as mob, proximity)
	if(!proximity) return
	if(istype(target,/turf/simulated/floor))
		//if(src.cultistsonly && !iscultist(usr)) //If the item (.src) is cultists only and the user isn't a cultist then it doesn't work
		//	return
		if(!wordtravel)
			runerandom()
		if(target)

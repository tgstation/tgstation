	...
	proc
		fizzle()
			...
			for (var/mob/V in viewers(src))
				V.show_message("<span class='danger'>The markings pulse with a small burst of light, then fall dark.", 3, "\red You hear a faint fizzle.</span>", 2)
			return
...
//						call(/obj/item/weapon/tome/proc/edit_notes)()
					...
					if("Scribe a rune")		//fixed more assbackward tabbing
						...
						for (var/mob/V in viewers(src))
							V.show_message("<span class='danger'>[user] slices open a finger and begins to chant and paint symbols on the floor.", 3, "\red You hear chanting.</span>", 2)

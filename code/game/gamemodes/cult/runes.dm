			...
			for(var/mob/living/carbon/monkey/M in src.loc)
				if (ticker.mode.name == "cult")
					if(M.mind == ticker.mode:sacrifice_target)
						...
						if(prob(30))
							usr << "<span class='danger'>The Geometer of Blood accepts your meager sacrifice.</span><span class='danger'>"
							sac_grant_word(usr)
						else
							usr << "<span class='danger'>The Geometer of blood accepts this sacrifice.</span>"
		...
		blind()
			if(istype(src,/obj/effect/rune))
				...
				for(var/mob/living/carbon/C in viewers(src))
					...
					C.show_message("<span class='danger'>Suddenly you see a red flash that blinds you.</span>", 3)
					affected++
				if(affected)
					usr.say("Sti[pick("'","`")] kaliesin!")
					usr << "<span class='danger'>The rune flashes, blinding those who not follow Nar-Sie, and dissipates into fine dust.</span>"
					qdel(src)
				else
					return fizzle()
			else

						...
						if(usr.mind.cult_words.len != ticker.mode.allwords.len) // No point running if they already know everything
							...
							for(var/i=1, i<=3, i++)
								...
								if(convert_word in usr.mind.cult_words)
									...
									break
							if(!convert_word)
								usr << "<span class='danger'>This Convert was unworthy of knowledge of the other side!</span>"
							else
								usr << "<span class='danger'>The Geometer of Blood is pleased to see his followers grow in numbers.</span>"
...
/*			for(var/mob/living/carbon/alien/A)
				...
				if (ticker.mode.name == "cult")
					if(prob(75))
						usr << "<span class='danger'>The Geometer of Blood accepts your exotic sacrifice.</span>"
						sac_grant_word()
					else
						usr << "<span class='danger'>The Geometer of Blood accepts your exotic sacrifice.</span>"
						usr << "<span class='danger'>However, this alien is not enough to gain His favor.</span>"
				else
					usr << "<span class='danger'>The Geometer of Blood accepts your exotic sacrifice.</span>"
				return
			return fizzle() */
			...
			if(istype(src,/obj/effect/rune))   ///When invoked as rune, flash and stun everyone around.
				...
				if(N)
					for(var/mob/O in viewers(T, null))
						O.show_message(text("<span class='userdanger'>[] invokes a talisman at [], but they are unaffected!</span>", usr, T), 1)
				else
					for(var/mob/O in viewers(T, null))
						O.show_message(text("<span class='userdanger'>[] invokes a talisman at []</span>", usr, T), 1)

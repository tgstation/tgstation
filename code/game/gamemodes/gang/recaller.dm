...
/obj/item/device/gangtool/proc/ping_gang(mob/user)
	...
	if(!message || !can_use(user))
		return
	if(user.z > 2)
		to_chat(user, "<span class='info'>[bIcon(src, user)]Error: Station out of range.</span>")
		return
...
/obj/item/device/gangtool/proc/recall(mob/user)
	...
	recalling = 1
	to_chat(loc, "<span class='info'>[bIcon(src, loc)]Generating shuttle recall order with codes retrieved from last call signal...</span>")

	sleep(rand(100,300))

	if(SSshuttle.emergency.mode != SHUTTLE_CALL) //Shuttle can only be recalled when it's moving to the station
		to_chat(user, "<span class='warning'>[bIcon(src, user)]Emergency shuttle cannot be recalled at this time.</span>")
		recalling = 0
		return 0
	to_chat(loc, "<span class='info'>[bIcon(src, loc)]Shuttle recall order generated. Accessing station long-range communication arrays...</span>")

	sleep(rand(100,300))

	if(!gang.dom_attempts)
		to_chat(user, "<span class='warning'>[bIcon(src, user)]Error: Unable to access communication arrays. Firewall has logged our signature and is blocking all further attempts.</span>")
		recalling = 0
		return 0

	var/turf/userturf = get_turf(user)
	if(userturf.z != ZLEVEL_STATION) //Shuttle can only be recalled while on station
		to_chat(user, "<span class='warning'>[bIcon(src, user)]Error: Device out of range of station communication arrays.</span>")
		recalling = 0
		return 0
	var/datum/station_state/end_state = new /datum/station_state()
	end_state.count()
	if((100 * GLOB.start_state.score(end_state)) < 80) //Shuttle cannot be recalled if the station is too damaged
		to_chat(user, "<span class='warning'>[bIcon(src, user)]Error: Station communication systems compromised. Unable to establish connection.</span>")
		recalling = 0
		return 0
	to_chat(loc, "<span class='info'>[bIcon(src, loc)]Comm arrays accessed. Broadcasting recall signal...</span>")
	...
	to_chat(loc, "<span class='info'>[bIcon(src, loc)]No response recieved. Emergency shuttle cannot be recalled at this time.</span>")
	return 0

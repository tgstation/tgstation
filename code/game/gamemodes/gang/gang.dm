		...
		for(var/turf/T in area_contents(target_captain))
			for(var/obj/item/I in T.GetAllContents())
				...
				if(istype(I, /obj/item/clothing))
					qdel(I)
	if(target_captain2)
		for(var/turf/T in area_contents(target_captain2))
			for(var/obj/item/I in T.GetAllContents())
				if(istype(I, /obj/item/weapon))
					qdel(I)
				if(istype(I, /obj/item/clothing))
					qdel(I)
	if(target_armory)
		for(var/turf/T in area_contents(target_armory))
			for(var/obj/item/I in T.GetAllContents())
				if(istype(I, /obj/item/weapon))
					qdel(I)
				if(istype(I, /obj/item/clothing))
					qdel(I)
	if(target_brig)
		for(var/turf/T in area_contents(target_brig))
			for(var/obj/item/I in T.GetAllContents())
				if(istype(I, /obj/item/weapon))
					qdel(I)
				if(istype(I, /obj/item/clothing))
					qdel(I)
	if(target_equip)
		for(var/turf/T in area_contents(target_equip))
			for(var/obj/item/I in T.GetAllContents())
				if(istype(I, /obj/item/weapon))
					qdel(I)
				if(istype(I, /obj/item/clothing))
					qdel(I)
	if(target_hos)
		for(var/turf/T in area_contents(target_hos))
			for(var/obj/item/I in T.GetAllContents())
				if(istype(I, /obj/item/weapon))
					qdel(I)
				if(istype(I, /obj/item/clothing))
					qdel(I)
	if(target_det)
		for(var/turf/T in area_contents(target_det))
			for(var/obj/item/I in T.GetAllContents())
				if(istype(I, /obj/item/weapon))
					qdel(I)
				if(istype(I, /obj/item/clothing))
					qdel(I)
	if(target_ward)
		for(var/turf/T in area_contents(target_ward))
			for(var/obj/item/I in T.GetAllContents())
				if(istype(I, /obj/item/weapon))
					qdel(I)
				if(istype(I, /obj/item/clothing))
					qdel(I)
	if(target_hop)
		for(var/turf/T in area_contents(target_hop))
			for(var/obj/item/I in T.GetAllContents())
				if(istype(I, /obj/item/weapon))
					qdel(I)
				if(istype(I, /obj/item/clothing))

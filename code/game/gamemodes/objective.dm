...
/datum/objective/steal/proc/select_target() //For admins setting objectives manually.
	...
	var/new_target = input("Select target:", "Objective target", steal_target, nullable = TRUE, possible_items_all)
	if (!new_target) return

	if (new_target == "custom") //Can set custom items.
		var/obj/item/custom_target = input("Select type:","Type", nullable = TRUE, choices = typesof(/obj/item))

...
/datum/objective/proc/admin_simple_target_pick(mob/admin)
	...
	var/mob/new_target = input(admin,"Select target:", "Objective target", def_value) as null|anything in sortList(possible_targets)
	if (!new_target)
		return
...
/datum/objective/steal/admin_edit(mob/admin)
	...
	var/new_target = input(admin,"Select target:", "Objective target", steal_target) as null|anything in sortList(possible_items_all)+"custom"
	if (!new_target)
		return
...
/datum/objective/destroy/admin_edit(mob/admin)
	var/list/possible_targets = active_ais(1)
	if(possible_targets.len)
		var/mob/new_target = input(admin,"Select target:", "Objective target") as null|anything in sortList(possible_targets)
		target = new_target.mind
	else

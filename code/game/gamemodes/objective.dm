		...
		if(player.mind && player.mind != owner)
			if(player.stat != DEAD)
				...
				if(get_area(player) == A)
					if(!player.mind.special_role && !istype(get_turf(player.mind.current), /turf/floor/plasteel/shuttle/red))
						return 0
	return 1
		...
		if(player.mind && player.mind != owner)
			if(player.stat != DEAD)
				if(istype(player, /mob/living/silicon))
					continue
				if(get_area(player) == A)
					if(player.real_name != owner.current.real_name && !istype(get_turf(player.mind.current), /turf/floor/plasteel/shuttle/red))
						return 0
...
/datum/objective/escape/check_completion()
	...
	if(istype(location, /turf/floor/plasteel/shuttle/red)) // Fails traitors if they are in the shuttle brig -- Polymorph
		return 0

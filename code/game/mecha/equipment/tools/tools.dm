...
/obj/item/mecha_parts/mecha_equipment/tool/drill
	...
	action(atom/target)
		...
		if(do_after_cooldown(target))
			if(T == chassis.loc && src == chassis.selected)
				if(istype(target, /turf/simulated/wall/r_wall) || istype(target, /turf/simulated/mineral/hard))
					occupant_message("<font color='red'>[target] is too durable to drill through.</font>")
				else if(istype(target, /turf/simulated/mineral/random))
					for(var/turf/simulated/mineral/random/M in range(chassis,1))
...
/obj/item/mecha_parts/mecha_equipment/tool/drill/diamonddrill
	...
	action(atom/target)
		...
		if(do_after_cooldown(target))
			if(T == chassis.loc && src == chassis.selected)
				if(istype(target, /turf/simulated/mineral/hard))
					occupant_message("<font color='red'>[target] is too durable to drill through.</font>")

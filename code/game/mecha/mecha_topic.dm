...
/obj/mecha/Topic(href, href_list)
	...
	if(in_range(src, usr))
		...
		if(href_list["req_access"] && add_req_access && user && id_card)
			output_access_dialog(id_card,user)

		if(href_list["maint_access"] && maint_access && user && id_card)
			if (istype(id_card))
				if(state==0)
					state = 1
		...
		if(href_list["add_req_access"] && add_req_access && user && id_card)
			operation_req_access += text2num(href_list["add_req_access"])
			output_access_dialog(id_card,user)

		if(href_list["del_req_access"] && add_req_access && user && id_card)
			operation_req_access -= text2num(href_list["add_req_access"])
			output_access_dialog(id_card, user)

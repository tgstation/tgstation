...
//
// Spawns damage over time loop for each reagent held in my butt.
...
/datum/effect/effect/system/chem_smoke_spread/start()
	...
	if(chemholder.reagents.reagent_list.len)
		for(var/datum/reagent/R in chemholder.reagents.reagent_list)
			...
			if(R.id in list("blood", "radium", "uranium", "carbon")) //These are the reagents which put dirt on the floor, we don't want much of them
				proba = 5
	...
	//distance between each smoke butt
...
/datum/effect/effect/system/chem_smoke_spread/proc/spawnSmoke(var/turf/T, var/icon/I, var/dist = 1)
	...
	if(chemholder.reagents.reagent_list.len)
		chemholder.reagents.copy_to(smoke, chemholder.reagents.total_volume / dist)	//copy reagents to the smoke so mob/breathe() can handle inhaling the reagents
	smoke.icon = I
	smoke.layer = 6

...
/obj/structure/alien/weeds/proc/updateWeedOverlays()
	...
	if(!locate(/obj/structure/alien) in N.contents)
		if(istype(N, /turf/simulated/floor))
			src.overlays += image('icons/mob/alien.dmi', "weeds_side_s", layer=-1, pixel_y = 32)
	if(!locate(/obj/structure/alien) in S.contents)
		if(istype(S, /turf/simulated/floor))
			src.overlays += image('icons/mob/alien.dmi', "weeds_side_n", layer=-1, pixel_y = -32)
	if(!locate(/obj/structure/alien) in E.contents)
		if(istype(E, /turf/simulated/floor))
			src.overlays += image('icons/mob/alien.dmi', "weeds_side_w", layer=-1, pixel_x = 32)
	if(!locate(/obj/structure/alien) in W.contents)
		if(istype(W, /turf/simulated/floor))
			src.overlays += image('icons/mob/alien.dmi', "weeds_side_e", layer=-1, pixel_x = -32)

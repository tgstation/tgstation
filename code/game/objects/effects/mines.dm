...
/obj/effect/mine/proc/now_armed()
	armed = TRUE
	update_appearance(UPDATE_ICON_STATE)
	PLAYSOUND(src, 'sound/machines/nuke/angry_beep.ogg').volume(40).vary_frequency(FALSE).range(-2 + SOUND_RANGE).play()
...
/obj/effect/mine/sound/mineEffect(mob/victim)
	PLAYSOUND(get_sfx(loc), sound).volume(100).vary_frequency(TRUE).play()
...
/obj/item/minespawner/attack_self(mob/user)
	...
	PLAYSOUND(src, 'sound/items/weapons/armbomb.ogg').volume(70).vary_frequency(TRUE).play()
...
/obj/item/minespawner/proc/deploy_mine()
	do_alert_animation()
	PLAYSOUND(loc, 'sound/machines/chime.ogg').volume(30).vary_frequency(FALSE).range(-3 + SOUND_RANGE).play()

	...
	if((shoe_types.len + species_types.len) > 0)
		...
		for(var/species in species_types)
			...
			if(species == "unknown")
				...
				. += "[icon2html('icons/mob/humanoid/human.dmi', user, "monkey")] Some <B>monkey feet</B>."
			else if(species == SPECIES_HUMAN)
				. += "[icon2html('icons/mob/humanoid/human_parts.dmi', user, "default_human_l_leg")] Some <B>human feet</B>."
			else
				. += "[icon2html('icons/mob/humanoid/human_parts.dmi', user, "[species]_l_leg")] Some <B>[species] feet</B>."

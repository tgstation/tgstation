...
/obj/effect/anomaly/grav/anomalyEffect(seconds_per_tick)
	...
	for(var/obj/O in range(0,src))
		...
		if(target && !IS_UNCONSCIOUS_CRIT_OR_DEAD(target) && prob(object_launch_prob))
...
/obj/effect/anomaly/grav/proc/gravShock(mob/living/living_debris)
	if(boing && isliving(living_debris) && !IS_UNCONSCIOUS_CRIT_OR_DEAD(living_debris) && !living_debris.mob_negates_gravity())

...
/obj/effect/overlay/temp/ratvar/volt_hit/proc/volthit()
	...
	for(var/mob/living/L in T)
		...
		if(I)
			...
			continue
		L.visible_message("<span class='warning'>[IDENTITY_SUBJECT(1)] is struck by a [name]!</span>", "<span class='userdanger'>You're struck by a [name]!</span>", subjects=list(L))
	...
	for(var/obj/mecha/M in T)
		...
		M.visible_message("<span class='warning'>[IDENTITY_SUBJECT(1)] is struck by a [name]!</span>", subjects=list(M))

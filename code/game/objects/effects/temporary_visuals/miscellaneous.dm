...
/**
 * The projectile from a /obj/item/paint_sealer
 */
/obj/effect/temp_visual/paint_sealer_blip
	icon_state = "blip"
	color = COLOR_VIVID_RED
	duration = 1 SECONDS

	/// Time for haze on floor tiles to fade out
	var/haze_fade_out_time = 0.5 SECONDS
	/// Time for blip to fade out
	var/blip_fade_out_time = 0.25 SECONDS
	/// Our movement loop
	var/datum/move_loop/move_loop

/obj/effect/temp_visual/paint_sealer_blip/Initialize(mapload, atom/target)
	. = ..()

	move_loop = SSmove_manager.move_towards(src, target, delay = 1, timeout = duration - max(blip_fade_out_time, haze_fade_out_time))
	RegisterSignal(move_loop, COMSIG_PARENT_QDELETING, .proc/blip_move_finished)
	RegisterSignal(move_loop, COMSIG_MOVELOOP_POSTPROCESS, .proc/blip_move_step)
 ...
 *
 * Actually represents movement finishing.
 */
/obj/effect/temp_visual/paint_sealer_blip/proc/blip_move_finished(datum/move_loop/blip_loop)
	SIGNAL_HANDLER

	// Blip will be actually deleted by being a temp_visual.
	animate(src, alpha = 0, time = blip_fade_out_time)

/**
 * Handler for blip movmement loop's COMSIG_MOVELOOP_POSTPROCESS
 */
/obj/effect/temp_visual/paint_sealer_blip/proc/blip_move_step(datum/move_loop/blip_loop, succeeded)
	SIGNAL_HANDLER

	if(!succeeded)
		// Hit something
		QDEL_IN(src, max(blip_fade_out_time, haze_fade_out_time))
		animate(src, alpha = 0, time = blip_fade_out_time)
		return

	// Only affect floors with floor tiles placed down,
	// as this can be undone by pulling up the tile and placing back down.
	var/turf/open/floor/floor = get_turf(loc)
	if(!istype(floor) || !floor.overfloor_placed)
		return

	// The nice floor decals are lightweight elements, but we can unregister their clean handler.
	var/list/clean_comps = floor.comp_lookup && floor.comp_lookup[COMSIG_COMPONENT_CLEAN_ACT]
	if(clean_comps)
		if(length(clean_comps))
			for(var/datum/comp as anything in clean_comps)
				comp.UnregisterSignal(floor, COMSIG_COMPONENT_CLEAN_ACT)
		else
			var/datum/comp = clean_comps
			comp.UnregisterSignal(floor, COMSIG_COMPONENT_CLEAN_ACT)

	// Also seal the washable color layers by making them fixed
	if(length(floor.atom_colours) >= WASHABLE_COLOUR_PRIORITY && floor.atom_colours[WASHABLE_COLOUR_PRIORITY])
		var/wash_color = floor.atom_colours[WASHABLE_COLOUR_PRIORITY]
		floor.remove_atom_colour(WASHABLE_COLOUR_PRIORITY)
		floor.add_atom_colour(wash_color, FIXED_COLOUR_PRIORITY)

	// Shimmer the floor tile
	floor.add_filter("paint_sealer_heat_wave", 1, list("type" = "wave", "x" = 8, "y" = 8))
	floor.transition_filter("paint_sealer_heat_wave", haze_fade_out_time, list("x" = 0, "y" = 0))

	addtimer(CALLBACK(src, .proc/remove_haze, floor), haze_fade_out_time)

/obj/effect/temp_visual/paint_sealer_blip/proc/remove_haze(turf/floor)
	SIGNAL_HANDLER

	floor.remove_filter("paint_sealer_heat_wave")

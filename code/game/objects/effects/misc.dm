...
/obj/effect/spawner/force_move(atom/destination)
	if(destination && QDELETED(src)) // throw a warning if we try to force_move a qdeleted spawner to somewhere other than nullspace
		stack_trace("Warning: something tried to force_move() a qdeleted [src]([type]) to non-null destination [destination]([destination.type])!")
	return ..()

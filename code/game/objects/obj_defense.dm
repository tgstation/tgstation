...
/obj/proc/take_damage(damage_amount, damage_type = BRUTE, damage_flag = "", sound_effect = TRUE, attack_dir, armour_penetration = ZERO)
	if(QDELETED(src))
		stack_trace("[src] taking damage after deletion")
		return
	if(sound_effect)
		play_attack_sound(damage_amount, damage_type, damage_flag)
	if((resistance_flags & INDESTRUCTIBLE) || obj_integrity <= ZERO)
		return
	damage_amount = run_obj_armor(damage_amount, damage_type, damage_flag, attack_dir, armour_penetration)
	if(damage_amount < DAMAGE_PRECISION)
		return
	. = damage_amount
	obj_integrity = max(obj_integrity - damage_amount, ZERO)
	//BREAKING FIRST
...
/obj/proc/run_obj_armor(damage_amount, damage_type, damage_flag = ZERO, attack_dir, armour_penetration = ZERO)
	...
	if(armor_protection)		//Only apply weak-against-armor/hollowpoint effects if there actually IS armor.
		armor_protection = CLAMP(armor_protection - armour_penetration, min(armor_protection, ZERO), 100)
	return round(damage_amount * (100 - armor_protection)*0.01, DAMAGE_PRECISION)

///the sound played when the obj is damaged.
/obj/proc/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = ZERO)
	switch(damage_type)
		if(BRUTE)
			if(damage_amount)
...
/obj/ex_act(severity, target)
	...
	if(QDELETED(src))
		return
	if(target == src)
		take_damage(INFINITY, BRUTE, "bomb", ZERO)
		return
	switch(severity)
		if(1)
			take_damage(INFINITY, BRUTE, "bomb", ZERO)
		if(2)
			take_damage(rand(100, 250), BRUTE, "bomb", ZERO)
		if(3)
			take_damage(rand(10, 90), BRUTE, "bomb", ZERO)
...
/obj/bullet_act(obj/projectile/P)
	...
	if(!QDELETED(src)) //Bullet on_hit effect might have already destroyed this object
		take_damage(P.damage, P.damage_type, P.flag, ZERO, turn(P.dir, 180), P.armour_penetration)
...
/obj/attack_hulk(mob/living/carbon/human/user)
	...
	take_damage(hulk_damage(), BRUTE, "melee", ZERO, get_dir(src, user))
	return TRUE
...
/obj/blob_act(obj/structure/blob/B)
	if(isturf(loc))
		...
		if(T.intact && level == 1) //the blob doesn't destroy thing below the floor
			return
	take_damage(400, BRUTE, "melee", ZERO, get_dir(src, B))

/obj/proc/attack_generic(mob/user, damage_amount = ZERO, damage_type = BRUTE, damage_flag = ZERO, sound_effect = 1, armor_penetration = ZERO) //used by attack_alien, attack_animal, and attack_slime
	user.do_attack_animation(src)
	user.changeNext_move(CLICK_CD_MELEE)
	return take_damage(damage_amount, damage_type, damage_flag, sound_effect, get_dir(src, user), armor_penetration)

/obj/attack_alien(mob/living/carbon/alien/humanoid/user)
	if(attack_generic(user, 60, BRUTE, "melee", ZERO))
...
/obj/proc/collision_damage(atom/movable/pusher, force = MOVE_FORCE_DEFAULT, direction)
	var/amt = max(ZERO, ((force - (move_resist * MOVE_FORCE_CRUSH_RATIO)) / (move_resist * MOVE_FORCE_CRUSH_RATIO)) * 10)
...
/obj/proc/acid_processing()
	...
	if(!(resistance_flags & ACID_PROOF))
		...
		take_damage(min(1 + round(sqrt(acid_level)*0.3), 300), BURN, "acid", ZERO)

	acid_level = max(acid_level - (5 + 3*round(sqrt(acid_level))), ZERO)
	if(!acid_level)
		return FALSE
...
/obj/fire_act(exposed_temperature, exposed_volume)
	...
	if(exposed_temperature && !(resistance_flags & FIRE_PROOF))
		take_damage(CLAMP(0.02 * exposed_temperature, 0, 20), BURN, "fire", ZERO)
...
/obj/proc/modify_max_integrity(new_max, can_break = TRUE, damage_type = BRUTE)
	...
	if(current_integrity != ZERO && current_max != ZERO)

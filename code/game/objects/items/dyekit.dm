...
/obj/item/dyespray/proc/dye(mob/target, mob/user)
	...
	if(!beard_or_hair || !user.canPerformAction(src, be_close = TRUE, NEED_DEXTERITY| no_tk = FALSE))
		return
	...
	if(isnull(new_grad_style))
		return
	if(!user.canPerformAction(src, be_close = TRUE, NEED_DEXTERITY| no_tk = FALSE))
		return

	var/new_grad_color = input(user, "Choose a secondary hair color:", "Character Preference",human_target.grad_color) as color|null
	if(!new_grad_color || !user.canPerformAction(src, be_close = TRUE, NEED_DEXTERITY| no_tk = FALSE) || !user.CanReach(target))
		return

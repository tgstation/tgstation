...
/obj/item/choice_beacon/proc/can_use_beacon(mob/living/user)
	...
	playsound(src, 'sound/machines/buzz/buzz-sigh.ogg').volume(40).vary(TRUE).play()
	return FALSE
...
/obj/item/choice_beacon/augments/spawn_option(obj/choice_path, mob/living/user)
	...
	playsound(src, 'sound/items/weapons/emitter2.ogg').extra_range(SILENCED_SOUND_EXTRARANGE).play()
...
/obj/item/choice_beacon/holy/spawn_option(obj/choice_path, mob/living/user)
	playsound(src, 'sound/effects/pray_chaplain.ogg').volume(40).vary(TRUE).play()

...
/obj/machinery/implantchair/brainwash/implant_action(mob/living/C,mob/user)
	...
	C.log_message("[key_name(C)] has been brainwashed with the objective '[objective]' by [key_name(user)] using \the [src]", LOG_ATTACK)
	user.log_message("[key_name(user)] has brainwashed [key_name(C)] with the objective '[objective]' using \the [src]", LOG_ATTACK)

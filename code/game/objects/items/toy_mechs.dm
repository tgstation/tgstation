...
/obj/item/toy/mecha/attack_self(mob/user)
	if(timer < world.time)
		...
		if(!quiet)
			playsound(user, 'sound/vehicles/mecha/mechstep.ogg').volume(20).vary(TRUE).play()
	else
		. = ..()
...
/obj/item/toy/mecha/suicide_act(mob/living/carbon/user)
	...
	for(var/i in 1 to 4)
		switch(i)
			if(1, 3)
				SpinAnimation(5, 0)
				playsound(src, 'sound/vehicles/mecha/mechstep.ogg').volume(30).vary(TRUE).play()
				user.adjustBruteLoss(25)
				user.adjustStaminaLoss(50)
			if(2)
				user.SpinAnimation(5, 0)
				playsound(user, 'sound/items/weapons/smash.ogg').volume(20).vary(TRUE).play()
...
/obj/item/toy/mecha/proc/mecha_brawl(obj/item/toy/mecha/attacker, mob/living/carbon/attacker_controller, mob/living/carbon/opponent)
			...
			switch(rand(1,8))
				if(1 to 3) //attacker wins
					if(attacker.special_attack_cooldown == 0 && attacker.combat_health <= round(attacker.max_combat_health/3)) //if health is less than 1/3 and special off CD, use it
						...
						playsound(attacker, 'sound/vehicles/mecha/mechstep.ogg').volume(30).vary(TRUE).play()
						combat_health--
						...
						if(prob(5))
							combat_health--
							playsound(src, 'sound/effects/meteorimpact.ogg').volume(20).vary(TRUE).play()
				...
				if(4) //both lose
					...
					if(prob(50))
						...
						src_controller.visible_message(span_danger("[src] and [attacker] clash dramatically, causing sparks to fly!"), \
										...
										span_hear("You hear hard plastic rubbing against hard plastic."), COMBAT_MESSAGE_RANGE)
				if(5) //both win
					playsound(attacker, 'sound/items/weapons/parry.ogg').volume(20).vary(TRUE).play()
					if(prob(50))
				...
				if(6 to 8) //defender wins
					if(special_attack_cooldown == 0 && combat_health <= round(max_combat_health/3)) //if health is less than 1/3 and special off CD, use it
						...
						if(prob(5))
							attacker.combat_health--
							playsound(attacker, 'sound/effects/meteorimpact.ogg').volume(20).vary(TRUE).play()
	...
	if(attacker.combat_health <= 0 && combat_health <= 0) //both lose
		playsound(src, 'sound/machines/warning-buzzer.ogg').volume(20).vary(TRUE).play()
		...
		wins++
		attacker.losses++
		playsound(attacker, 'sound/effects/light_flicker.ogg').volume(20).vary(TRUE).play()
		...
		attacker.wins++
		losses++
		playsound(src, 'sound/effects/light_flicker.ogg').volume(20).vary(TRUE).play()
...
/obj/item/toy/mecha/proc/special_attack_move(obj/item/toy/mecha/victim)
	...
	switch(special_attack_type)
		if(SPECIAL_ATTACK_DAMAGE) //+2 damage
			...
			playsound(src, 'sound/items/weapons/marauder.ogg').volume(20).vary(TRUE).play()
		if(SPECIAL_ATTACK_HEAL) //+2 healing
			combat_health+=2
			playsound(src, 'sound/vehicles/mecha/mech_shield_raise.ogg').volume(20).vary(TRUE).play()
		if(SPECIAL_ATTACK_UTILITY) //+1 heal, +1 damage
			victim.combat_health--
			combat_health++
			playsound(src, 'sound/vehicles/mecha/mechmove01.ogg').volume(30).vary(TRUE).play()
...
/obj/item/toy/mecha/honk/super_special_attack(obj/item/toy/mecha/victim)
	playsound(src, 'sound/mobs/non-humanoids/honkbot/honkbot_evil_laugh.ogg').volume(20).vary(TRUE).play()
...
/obj/item/toy/mecha/deathripley/super_special_attack(obj/item/toy/mecha/victim)
	playsound(src, 'sound/items/weapons/sonic_jackhammer.ogg').volume(20).vary(TRUE).play()

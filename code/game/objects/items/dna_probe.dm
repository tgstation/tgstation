...
/obj/item/dna_probe/proc/try_linking_vault(obj/machinery/dna_vault/target, mob/user)
	...
	if(!our_vault)
		...
		create_sound(src, 'sound/machines/terminal/terminal_success.ogg').play()
		return TRUE
	return FALSE
...
/obj/item/dna_probe/proc/try_upload_dna(obj/machinery/dna_vault/target, mob/user)
	...
	target.check_goal()
	create_sound(target, 'sound/machines/compiler/compiler-stage1.ogg').play()
...
/obj/item/dna_probe/proc/scan_dna(atom/target, mob/user)
	var/obj/machinery/dna_vault/our_vault = dna_vault_ref?.resolve()
	if(!our_vault)
		create_sound(user, 'sound/machines/buzz/buzz-sigh.ogg').play()
	...
	if(istype(target, /obj/machinery/hydroponics))
		...
		create_sound(src, 'sound/machines/compiler/compiler-stage2.ogg').play()

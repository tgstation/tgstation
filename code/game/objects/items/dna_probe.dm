...
/obj/item/dna_probe/proc/try_linking_vault(obj/machinery/dna_vault/target, mob/user)
	...
	if(!our_vault)
		...
		PLAYSOUND(src, 'sound/machines/terminal/terminal_success.ogg').volume(50).play()
		return TRUE
	return FALSE
...
/obj/item/dna_probe/proc/try_upload_dna(obj/machinery/dna_vault/target, mob/user)
	...
	target.check_goal()
	PLAYSOUND(target, 'sound/machines/compiler/compiler-stage1.ogg').volume(50).play()
...
/obj/item/dna_probe/proc/scan_dna(atom/target, mob/user)
	var/obj/machinery/dna_vault/our_vault = dna_vault_ref?.resolve()
	if(!our_vault)
		PLAYSOUND(user, 'sound/machines/buzz/buzz-sigh.ogg').volume(50).play()
	...
	if(istype(target, /obj/machinery/hydroponics))
		...
		PLAYSOUND(src, 'sound/machines/compiler/compiler-stage2.ogg').volume(50).play()

...
/obj/item/climbing_hook/ranged_interact_with_atom(turf/open/interacting_with, mob/living/user, list/modifiers)
	...
	if(do_after(user, final_climb_time, interacting_with))
		...
		uses--
		user.mind?.adjust_experience(/datum/skill/athletics, round(50/(fitness_level || 1), 1)) //get some experience for our trouble, especially since this costs us a climbing rope use

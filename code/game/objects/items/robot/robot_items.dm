...
/obj/item/borg/stun/attack(mob/living/M, mob/living/user)
	if(ishuman(M))
		...
		if(H.check_shields(src, 0, "[M]'s [name]", MELEE_ATTACK))
			playsound(M, 'sound/weapons/genhit.ogg', 50, DEFAULT_SOUND_VARY)
			return FALSE
	if(iscyborg(user))
	...
	playsound(loc, 'sound/weapons/egloves.ogg', 50, DEFAULT_SOUND_VARY, extra_range = -1)
		...
		if(0)
			if(M.health >= 0)
				if(user.zone_selected == BODY_ZONE_HEAD)
					...
					playsound(loc, 'sound/weapons/tap.ogg', 50, DEFAULT_SOUND_VARY, extra_range = -1)
				else if(ishuman(M))
		...
		if(2)
			if(scooldown < world.time)
				if(M.health >= 0)
					...
					playsound(loc, 'sound/effects/sparks2.ogg', 50, DEFAULT_SOUND_VARY, extra_range = -1)
		...
		if(3)
			if(ccooldown < world.time)
				if(M.health >= 0)
					...
					playsound(loc, 'sound/weapons/smash.ogg', 50, DEFAULT_SOUND_VARY, extra_range = -1)
...
/obj/item/borg/lollipop/proc/shootG(atom/target, mob/living/user, params)	//Most certainly a good idea.
	...
	playsound(src.loc, 'sound/weapons/bulletflyby3.ogg', 50, DEFAULT_SOUND_VARY)
	A.fire_casing(target, user, params, 0, 0, null, 0)

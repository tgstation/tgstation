...
/obj/item/borg/stun/attack(mob/living/attacked_mob, mob/living/user)
	...
	if(ishuman(attacked_mob))
		...
		if(human.check_block(src, 0, "[attacked_mob]'s [name]", MELEE_ATTACK))
			create_sound(attacked_mob, 'sound/items/weapons/genhit.ogg').vary(TRUE).play()
			return FALSE
	if(iscyborg(user))
	...
	create_sound(loc, 'sound/items/weapons/egloves.ogg').vary(TRUE).extra_range(-1).play()
	...
	switch(mode)
		if(HUG_MODE_NICE)
			...
			if(user.zone_selected == BODY_ZONE_HEAD)
				...
				create_sound(loc, 'sound/items/weapons/tap.ogg').vary(TRUE).extra_range(-1).play()
		...
		if(HUG_MODE_SHOCK)
			...
			create_sound(loc, 'sound/effects/sparks/sparks2.ogg').vary(TRUE).extra_range(-1).play()
		...
		if(HUG_MODE_CRUSH)
			...
			create_sound(loc, 'sound/items/weapons/smash.ogg').vary(TRUE).extra_range(-1).play()
...
/obj/item/harmalarm/attack_self(mob/user)
	...
	if(safety == TRUE)
		...
		create_sound(get_turf(src), 'sound/mobs/non-humanoids/cyborg/harmalarm.ogg').volume(70).vary(3).play()
		...
		create_sound(get_turf(src), 'sound/machines/warning-buzzer.ogg').volume(130).vary(3).play()

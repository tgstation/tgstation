...
/obj/item/borg/upgrade
	...
	var/locked = FALSE
	var/installed = FALSE
	var/require_configuration = FALSE
	var/list/configuration_type = null
	///	Bitflags listing configuration compatibility. Used in the exosuit fabricator for creating sub-categories.
	var/list/configuration_flags = NONE
	// if true, is not stored in the robot to be ejected
	// if configuration is reset
	var/one_use = FALSE
...
/obj/item/borg/upgrade/proc/action(mob/living/silicon/robot/R, user = usr)
	if(R.stat == DEAD)
		...
		return FALSE
	if(configuration_type && !is_type_in_list(R.configuration, configuration_type))
...
/obj/item/borg/upgrade/disablercooler
	...
	configuration_type = list(/obj/item/robot_config/security)
	configuration_flags = BORG_CONFIGURATION_SECURITY

/obj/item/borg/upgrade/disablercooler/action(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(.)
		var/obj/item/gun/energy/disabler/cyborg/T = locate() in R.configuration.modules
		if(!T)
...
/obj/item/borg/upgrade/ddrill
	...
	desc = "A diamond drill replacement for the mining configuration's standard drill."
	icon_state = "cyborg_upgrade3"
	require_configuration = TRUE
...
/obj/item/borg/upgrade/ddrill/action(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(.)
		for(var/obj/item/pickaxe/drill/cyborg/D in R.configuration)
			R.configuration.remove_module(D, TRUE)
		for(var/obj/item/shovel/S in R.configuration)
			R.configuration.remove_module(S, TRUE)

		var/obj/item/pickaxe/drill/cyborg/diamond/DD = new /obj/item/pickaxe/drill/cyborg/diamond(R.configuration)
		R.configuration.basic_modules += DD
		R.configuration.add_module(DD, FALSE, TRUE)

/obj/item/borg/upgrade/ddrill/deactivate(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if (.)
		for(var/obj/item/pickaxe/drill/cyborg/diamond/DD in R.configuration)
			R.configuration.remove_module(DD, TRUE)

		var/obj/item/pickaxe/drill/cyborg/D = new (R.configuration)
		R.configuration.basic_modules += D
		R.configuration.add_module(D, FALSE, TRUE)
		var/obj/item/shovel/S = new (R.configuration)
		R.configuration.basic_modules += S
		R.configuration.add_module(S, FALSE, TRUE)
...
/obj/item/borg/upgrade/soh/action(mob/living/silicon/robot/R)
	. = ..()
	if(.)
		for(var/obj/item/storage/bag/ore/cyborg/S in R.configuration)
			R.configuration.remove_module(S, TRUE)

		var/obj/item/storage/bag/ore/holding/H = new /obj/item/storage/bag/ore/holding(R.configuration)
		R.configuration.basic_modules += H
		R.configuration.add_module(H, FALSE, TRUE)

/obj/item/borg/upgrade/soh/deactivate(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if (.)
		for(var/obj/item/storage/bag/ore/holding/H in R.configuration)
			R.configuration.remove_module(H, TRUE)

		var/obj/item/storage/bag/ore/cyborg/S = new (R.configuration)
		R.configuration.basic_modules += S
		R.configuration.add_module(S, FALSE, TRUE)
...
/obj/item/borg/upgrade/tboh
	...
	icon_state = "cyborg_upgrade3"
	require_configuration = TRUE
	configuration_type = list(/obj/item/robot_config/janitor)
	configuration_flags = BORG_CONFIGURATION_JANITOR

/obj/item/borg/upgrade/tboh/action(mob/living/silicon/robot/R)
	. = ..()
	if(.)
		for(var/obj/item/storage/bag/trash/cyborg/TB in R.configuration.modules)
			R.configuration.remove_module(TB, TRUE)

		var/obj/item/storage/bag/trash/bluespace/cyborg/B = new /obj/item/storage/bag/trash/bluespace/cyborg(R.configuration)
		R.configuration.basic_modules += B
		R.configuration.add_module(B, FALSE, TRUE)

/obj/item/borg/upgrade/tboh/deactivate(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(.)
		for(var/obj/item/storage/bag/trash/bluespace/cyborg/B in R.configuration.modules)
			R.configuration.remove_module(B, TRUE)

		var/obj/item/storage/bag/trash/cyborg/TB = new (R.configuration)
		R.configuration.basic_modules += TB
		R.configuration.add_module(TB, FALSE, TRUE)
...
/obj/item/borg/upgrade/amop/action(mob/living/silicon/robot/R)
	. = ..()
	if(.)
		for(var/obj/item/mop/cyborg/M in R.configuration.modules)
			R.configuration.remove_module(M, TRUE)

		var/obj/item/mop/advanced/cyborg/mop = new /obj/item/mop/advanced/cyborg(R.configuration)
		R.configuration.basic_modules += mop
		R.configuration.add_module(mop, FALSE, TRUE)

/obj/item/borg/upgrade/amop/deactivate(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(.)
		for(var/obj/item/mop/advanced/cyborg/A in R.configuration.modules)
			R.configuration.remove_module(A, TRUE)

		var/obj/item/mop/cyborg/M = new (R.configuration)
		R.configuration.basic_modules += M
		R.configuration.add_module(M, FALSE, TRUE)
...
/obj/item/borg/upgrade/prt/action(mob/living/silicon/robot/R)
	. = ..()
	if(.)
		var/obj/item/cautery/prt/P = new (R.configuration)
		R.configuration.basic_modules += P
		R.configuration.add_module(P, FALSE, TRUE)

/obj/item/borg/upgrade/prt/deactivate(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(.)
		for(var/obj/item/cautery/prt/P in R.configuration.modules)
			R.configuration.remove_module(P, TRUE)
...
/obj/item/borg/upgrade/hypospray
	...
	desc = "An upgrade to the Medical configuration cyborg's hypospray, allowing it \
		to produce more advanced and complex medical reagents."
	icon_state = "cyborg_upgrade3"
	require_configuration = TRUE
	configuration_type = list(/obj/item/robot_config/medical)
	configuration_flags = BORG_CONFIGURATION_MEDICAL
	var/list/additional_reagents = list()

/obj/item/borg/upgrade/hypospray/action(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(.)
		for(var/obj/item/reagent_containers/borghypo/H in R.configuration.modules)
...
/obj/item/borg/upgrade/hypospray/expanded
	...
	desc = "An upgrade to the Medical configuration's hypospray, allowing it \
...
/obj/item/borg/upgrade/defib
	...
	desc = "An upgrade to the Medical configuration, installing a built-in \
...
/obj/item/borg/upgrade/defib/action(mob/living/silicon/robot/R, user = usr)
	...
	if(.)
		...
		var/obj/item/shockpaddles/cyborg/S = new(R.configuration)
		R.configuration.basic_modules += S
		R.configuration.add_module(S, FALSE, TRUE)
...
/obj/item/borg/upgrade/defib/deactivate(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if (.)
		var/obj/item/shockpaddles/cyborg/S = locate() in R.configuration
		R.configuration.remove_module(S, TRUE)
...
/obj/item/borg/upgrade/processor
	...
	desc = "An upgrade to the Medical configuration, installing a processor \
		capable of scanning surgery disks and carrying \
		out procedures"
	icon_state = "cyborg_upgrade3"
	require_configuration = TRUE
	configuration_type = list(/obj/item/robot_config/medical, /obj/item/robot_config/syndicate_medical)
	configuration_flags = BORG_CONFIGURATION_MEDICAL

/obj/item/borg/upgrade/processor/action(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(.)
		var/obj/item/surgical_processor/SP = new(R.configuration)
		R.configuration.basic_modules += SP
		R.configuration.add_module(SP, FALSE, TRUE)

/obj/item/borg/upgrade/processor/deactivate(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if (.)
		var/obj/item/surgical_processor/SP = locate() in R.configuration
		R.configuration.remove_module(SP, TRUE)
...
/obj/item/borg/upgrade/rped
	...
	icon_state = "borgrped"
	require_configuration = TRUE
	configuration_type = list(/obj/item/robot_config/engineering, /obj/item/robot_config/saboteur)
	configuration_flags = BORG_CONFIGURATION_ENGINEERING
...
/obj/item/borg/upgrade/rped/action(mob/living/silicon/robot/R, user = usr)
	...
	if(.)
		...
		RPED = new(R.configuration)
		R.configuration.basic_modules += RPED
		R.configuration.add_module(RPED, FALSE, TRUE)

/obj/item/borg/upgrade/rped/deactivate(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if (.)
		var/obj/item/storage/part_replacer/cyborg/RPED = locate() in R.configuration
		if (RPED)
			R.configuration.remove_module(RPED, TRUE)
...
/obj/item/borg/upgrade/pinpointer/action(mob/living/silicon/robot/R, user = usr)
	...
	if(.)
		...
		var/obj/item/pinpointer/crew/PP = locate() in R.configuration
		if(PP)
			to_chat(user, "<span class='warning'>This unit is already equipped with a pinpointer module!</span>")
			return FALSE

		PP = new(R.configuration)
		R.configuration.basic_modules += PP
		R.configuration.add_module(PP, FALSE, TRUE)
...
/obj/item/borg/upgrade/pinpointer/deactivate(mob/living/silicon/robot/R, user = usr)
	...
	if (.)
		...
		var/obj/item/pinpointer/crew/PP = locate() in R.configuration
		R.configuration.remove_module(PP, TRUE)
...
/obj/item/borg/upgrade/transform
	name = "borg configuration picker (Standard)"
	desc = "Allows you to to turn a cyborg into a standard cyborg."
	icon_state = "cyborg_upgrade3"
	var/obj/item/robot_config/new_configuration = null

/obj/item/borg/upgrade/transform/action(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(. && new_configuration)
		R.configuration.transform_to(new_configuration)

/obj/item/borg/upgrade/transform/clown
	name = "borg configuration picker (Clown)"
	desc = "Allows you to to turn a cyborg into a clown, honk."
	icon_state = "cyborg_upgrade3"
	new_configuration = /obj/item/robot_config/clown
...
/obj/item/borg/upgrade/circuit_app/action(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(.)
		var/obj/item/borg/apparatus/circuit/C = locate() in R.configuration.modules
		if(C)
			to_chat(user, "<span class='warning'>This unit is already equipped with a circuit apparatus!</span>")
			return FALSE

		C = new(R.configuration)
		R.configuration.basic_modules += C
		R.configuration.add_module(C, FALSE, TRUE)
...
/obj/item/borg/upgrade/circuit_app/deactivate(mob/living/silicon/robot/R, user = usr)
	...
	if (.)
		var/obj/item/borg/apparatus/circuit/C = locate() in R.configuration.modules
		if (C)
			R.configuration.remove_module(C, TRUE)
...
/obj/item/borg/upgrade/beaker_app/action(mob/living/silicon/robot/R, user = usr)
	. = ..()
	if(.)
		var/obj/item/borg/apparatus/beaker/extra/E = locate() in R.configuration.modules
		if(E)
			to_chat(user, "<span class='warning'>This unit has no room for additional beaker storage!</span>")
			return FALSE

		E = new(R.configuration)
		R.configuration.basic_modules += E
		R.configuration.add_module(E, FALSE, TRUE)
...
/obj/item/borg/upgrade/beaker_app/deactivate(mob/living/silicon/robot/R, user = usr)
	...
	if (.)
		var/obj/item/borg/apparatus/beaker/extra/E = locate() in R.configuration.modules
		if (E)
			R.configuration.remove_module(E, TRUE)
...
/obj/item/borg/upgrade/broomer/action(mob/living/silicon/robot/R, user = usr)
	...
	if (!.)
		return
	var/obj/item/pushbroom/cyborg/BR = locate() in R.configuration.modules
	if (BR)
		to_chat(user, "<span class='warning'>This janiborg is already equipped with an experimental broom!</span>")
		return FALSE
	BR = new(R.configuration)
	R.configuration.basic_modules += BR
	R.configuration.add_module(BR, FALSE, TRUE)
...
/obj/item/borg/upgrade/broomer/deactivate(mob/living/silicon/robot/R, user = usr)
	...
	var/obj/item/pushbroom/cyborg/BR = locate() in R.configuration.modules
	if (BR)
		R.configuration.remove_module(BR, TRUE)

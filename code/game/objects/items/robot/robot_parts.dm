...
/obj/item/robot_suit
	...
	var/obj/item/bodypart/arm/l_arm/robot/l_arm = null
	/// Right arm part of the endoskeleton
	var/obj/item/bodypart/arm/r_arm/robot/r_arm = null
	/// Left leg part of this endoskeleton
	var/obj/item/bodypart/leg/l_leg/robot/l_leg = null
	/// Right leg part of this endoskeleton
	var/obj/item/bodypart/leg/r_leg/robot/r_leg = null
...
/obj/item/robot_suit/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/stack/sheet/iron))
		...
		if(!l_arm && !r_arm && !l_leg && !r_leg && !chest && !head)
			if (M.use(1))
				...
				return
	else if(istype(W, /obj/item/bodypart/leg/l_leg/robot))
		if(l_leg)
			return
	...
	else if(istype(W, /obj/item/bodypart/leg/r_leg/robot))
		if(src.r_leg)
			return
		if(!user.transferItemToLoc(W, src))
			return
	...
	else if(istype(W, /obj/item/bodypart/arm/l_arm/robot))
		if(l_arm)
			return
		if(!user.transferItemToLoc(W, src))
			return
	...
	else if(istype(W, /obj/item/bodypart/arm/r_arm/robot))
		if(r_arm)
			return
		if(!user.transferItemToLoc(W, src))
			return

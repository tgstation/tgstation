...
/obj/item/flamethrower/afterattack(atom/target, mob/user, flag)
	...
	if(HAS_TRAIT(user, TRAIT_PACIFISM))
		...
		log_combat(user, target, "attempted to flamethrower", src, "with gas mixture: [print_gas_mixture(ptank.return_analyzable_air())], flamethrower [name] ([src]), igniter: [igniter.name] ([igniter]) and tank: [ptank.name] ([ptank])" + lit ? " while lit" : "" + " but failed due to pacifism")
		return
	if(user && user.get_active_held_item() == src) // Make sure our user is still holding us
		var/turf/target_turf = get_turf(target)
		if(target_turf)
			var/turflist = get_line(user, target_turf)
			log_combat(user, target, "flamethrowered", src, "with gas mixture: [print_gas_mixture(ptank.return_analyzable_air())], flamethrower: [name] ([src]), igniter: [igniter.name] ([igniter]) and tank: [ptank.name] ([ptank])" + lit ? " while lit" : "")
			flame_turf(turflist)

...
/obj/item/sharpener/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/twohanded))//some twohanded items should still be sharpenable, but handle force differently. therefore i need this stuff
		...
		if(TH.force_wielded > initial(TH.force_wielded))
			...
			return
		TH.force_wielded = CLAMP(TH.force_wielded + increment, ZERO, max)//wieldforce is increased since normal force wont stay
	...
	I.force = CLAMP(I.force + increment, ZERO, max)
	I.throwforce = CLAMP(I.throwforce + increment, ZERO, max)

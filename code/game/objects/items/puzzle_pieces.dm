...
/obj/structure/light_puzzle/proc/switch_light(light)
	...
	PLAYSOUND(src, 'sound/machines/synth/synth_yes.ogg').volume(100).vary_frequency(TRUE).play()
...
/obj/machinery/puzzle/keycardpad/attackby(obj/item/attacking_item, mob/user, params)
	...
	PLAYSOUND(src, 'sound/machines/card_slide.ogg').volume(45).vary_frequency(TRUE).play()
	if(!correct_card)
		return
	used = TRUE
	update_icon_state()
	PLAYSOUND(src, 'sound/machines/beep/beep.ogg').volume(45).vary_frequency(TRUE).play()
	on_puzzle_complete()
...
/obj/machinery/puzzle/password/interact(mob/user, list/modifiers)
	...
	if(!correct)
		PLAYSOUND(src, 'sound/machines/buzz/buzz-sigh.ogg').volume(45).vary_frequency(TRUE).play()
		return
	used = single_use
	update_icon_state()
	PLAYSOUND(src, 'sound/machines/terminal/terminal_button07.ogg').volume(45).vary_frequency(TRUE).play()

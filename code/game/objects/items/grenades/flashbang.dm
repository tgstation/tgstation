...
/obj/item/grenade/flashbang/detonate(mob/living/lanced_by)
	...
	playsound(flashbang_turf, 'sound/items/weapons/flashbang.ogg').volume(100).vary(TRUE).extra_range(8).falloff_exponent(0.9).play()
...
/obj/item/grenade/stingbang/detonate(mob/living/lanced_by)
	...
	if(!flashbang_turf)
		return
	do_sparks(rand(5, 9), FALSE, src)
	playsound(flashbang_turf, 'sound/items/weapons/flashbang.ogg').vary(TRUE).extra_range(8).falloff_exponent(0.9).play()

...
/obj/item/grenade/c4/afterattack(atom/movable/bomb_target, mob/user, flag)
	...
	if(do_after(user, 30, target = bomb_target))
		...
		user.log_message("planted [name] on [target.name] at [AREACOORD(user)] with a [det_time] second fuse", LOG_GAME)
...
/obj/item/grenade/c4/suicide_act(mob/living/user)
	...
	user.log_message("suicided with [src] at [AREACOORD(user)]", LOG_ATTACK)

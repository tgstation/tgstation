...
/obj/item/grenade/plastic/afterattack(atom/movable/AM, mob/user, flag)
	...
	if(do_after(user, 30, target = AM))
		...
		notify_ghosts("[user] has planted \a [src] on [target] with a [det_time] second fuse!", source = target, action = NOTIFY_ORBIT, ghost_sound = 'sound/machines/beep.ogg', ghost_volume = 20)

...
/obj/item/food/finalize_material_effects(list/materials)
	if(!HAS_TRAIT(parent, TRAIT_INGREDIENTS_HOLDER, INNATE_TRAIT)) //ingredients holder handle prefixes and colors differently
...
/obj/item/food/finalize_remove_material_effects(list/materials)
	. = ..()
	if(!HAS_TRAIT(parent, TRAIT_INGREDIENTS_HOLDER, INNATE_TRAIT))
		material_flags &= ~(MATERIAL_ADD_PREFIX|MATERIAL_COLOR)

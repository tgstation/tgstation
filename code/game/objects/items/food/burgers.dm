...
/obj/item/food/burger/plain/Initialize(mapload)
	...
	if(prob(1))
		...
		create_sound(src, 'sound/effects/smoke.ogg').vary(TRUE).play()
...
/obj/item/food/burger/ghost/process()
	...
	switch(paranormal_activity)
		if(97 to 100)
			...
			create_sound(loc, 'sound/misc/chain_rattling.ogg').volume(300).vary(TRUE).play()
		if(91 to 96)
		...
		if(65 to 70)
			...
			light?.flicker()
		if(62 to 64)
			create_sound(loc, pick('sound/effects/hallucinations/i_see_you1.ogg', 'sound/effects/hallucinations/i_see_you2.ogg')).vary(TRUE).play()
		if(61)
			visible_message("[src] spews out a glob of ectoplasm!")
			new /obj/effect/decal/cleanable/greenglow/ecto(loc)
			create_sound(loc, 'sound/effects/splat.ogg').volume(200).vary(TRUE).play()

...
/obj/item/tank/proc/after_internals_opened(mob/living/carbon/carbon_target)
	...
	playsound(loc, 'sound/items/internals/internals_on.ogg').volume(15).vary(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
...
/obj/item/tank/proc/after_internals_closed(mob/living/carbon/carbon_target)
	breathing_mob = null
	playsound(loc, 'sound/items/internals/internals_off.ogg').volume(15).vary(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
...
/obj/item/tank/atom_deconstruct(disassembled = TRUE)
	...
	if(location)
		...
		playsound(location, 'sound/effects/spray.ogg').volume(10).vary(TRUE).range(-3 + SOUND_RANGE).play()
	return ..()
...
/obj/item/tank/suicide_act(mob/living/user)
	...
	playsound(loc, 'sound/effects/spray.ogg').volume(10).vary(TRUE).range(-3 + SOUND_RANGE).play()
...
/obj/item/tank/atom_break(damage_flag)
	...
	playsound(src, 'sound/effects/spray.ogg').volume(10).vary(TRUE).range(-3 + SOUND_RANGE).play()

...
/obj/item/toy/spinningtoy/suicide_act(mob/living/carbon/human/user)
	...
	playsound(user, 'sound/items/eatfood.ogg').vary(TRUE).play()
...
/obj/item/toy/gun/ranged_interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if (src.bullets < 1)
		...
		playsound(src, 'sound/items/weapons/gun/revolver/dry_fire.ogg').volume(30).vary(TRUE).play()
		return ITEM_INTERACT_SUCCESS
	playsound(user, 'sound/items/weapons/gun/revolver/shot.ogg').volume(100).vary(TRUE).play()
	src.bullets--
...
/obj/item/toy/sword/proc/on_transform(obj/item/source, mob/user, active)
	...
	playsound(src, active ? 'sound/items/weapons/saberon.ogg' : 'sound/items/weapons/saberoff.ogg').volume(20).vary(TRUE).play()
	update_appearance(UPDATE_ICON)
...
/obj/item/toy/windup_toolbox/attack_self(mob/user)
	if(!active)
		...
		active = TRUE
		update_appearance()
		playsound(src, 'sound/effects/pope_entry.ogg').volume(100).play()
		Rumble()
...
/obj/item/toy/minimeteor/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	playsound(src, 'sound/effects/meteorimpact.ogg').volume(40).vary(TRUE).play()
...
/obj/item/toy/snowball/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	if(!..())
		playsound(src, 'sound/effects/pop.ogg').volume(20).vary(TRUE).play()
		qdel(src)
...
/obj/item/toy/brokenradio/attack_self(mob/user)
	if(cooldown <= world.time)
		...
		playsound(src, 'sound/items/radiostatic.ogg').wait(0.5 SECONDS).play()
	else
...
/obj/item/toy/braintoy/attack_self(mob/user)
	if(cooldown <= world.time)
		...
		playsound(src, 'sound/effects/blob/blobattack.ogg').wait(0.5 SECONDS).play()
...
/obj/item/toy/intento/proc/render(input)
	icon_state = input
	playsound(src, sound_by_intent[input]).play()

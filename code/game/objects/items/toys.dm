...
/obj/item/toy/gun/ranged_interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if (src.bullets < 1)
		...
		PLAYSOUND(src, 'sound/items/weapons/gun/revolver/dry_fire.ogg').volume(30).vary_frequency(TRUE).play()
		return ITEM_INTERACT_SUCCESS
	PLAYSOUND(user, 'sound/items/weapons/gun/revolver/shot.ogg').volume(100).vary_frequency(TRUE).play()
	src.bullets--
...
/obj/item/toy/nuke/attack_self(mob/user)
	if (obj_flags & EMAGGED && cooldown < world.time)
		...
		sleep(0.5 SECONDS)
		PLAYSOUND(src, 'sound/announcer/alarm/nuke_alarm.ogg').volume(20).vary_frequency(FALSE).play()
		sleep(14 SECONDS)
...
/obj/item/toy/minimeteor/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	PLAYSOUND(src, 'sound/effects/meteorimpact.ogg').volume(40).vary_frequency(TRUE).play()
...
/obj/item/toy/snowball/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	if(!..())
		PLAYSOUND(src, 'sound/effects/pop.ogg').volume(20).vary_frequency(TRUE).play()
		qdel(src)
...
/obj/item/toy/toy_xeno/attack_self(mob/user)
	if(cooldown <= world.time)
		...
		PLAYSOUND(get_turf(src), chosen_sound).volume(50).vary_frequency(TRUE).play()
...
/obj/item/toy/figure/attack_self(mob/user as mob)
	if(cooldown <= world.time)
		...
		PLAYSOUND(get_sfx(user), toysound).volume(20).vary_frequency(TRUE).play()

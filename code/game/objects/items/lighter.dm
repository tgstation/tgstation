...
/obj/item/lighter/proc/on_intercepted_bullet(mob/living/victim, obj/projectile/bullet)
	...
	PLAYSOUND(get_sfx(victim), SFX_RICOCHET).volume(100).vary_frequency(TRUE).play()
...
/obj/item/lighter/proc/set_lit(new_lit)
	...
	if(lit)
		...
		if(fancy)
			...
			PLAYSOUND(src.loc, 'sound/items/lighter/lighter_on.ogg').volume(100).vary_frequency(1).play()
		if(isliving(loc))

...
/obj/item/his_grace/process()
	...
	if(Adjacent(L))
		if(!L.stat)
			...
			playsound(L, 'sound/weapons/smash.ogg', 50, DEFAULT_SOUND_VARY)
			playsound(L, 'sound/misc/desceration-01.ogg', 50, DEFAULT_SOUND_VARY)
...
/obj/item/his_grace/proc/drowse() //Good night, Mr. Grace.
	...
	playsound(loc, 'sound/weapons/batonextend.ogg', 100, DEFAULT_SOUND_VARY)
	name = initial(name)
	desc = initial(desc)
...
/obj/item/his_grace/proc/consume(mob/living/meal) //Here's your dinner, Mr. Grace.
	...
	playsound(meal, 'sound/misc/desceration-02.ogg', 75, DEFAULT_SOUND_VARY)
	playsound(src, 'sound/items/eatfood.ogg', 100, DEFAULT_SOUND_VARY)
	meal.forceMove(src)

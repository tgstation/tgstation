...
/obj/item/organ/internal/body_egg/Insert(mob/living/carbon/M, special = FALSE, drop_if_replaced = TRUE)
	..()
	ADD_TRAIT(owner, TRAIT_XENO_HOST, SOURCE_ORGAN)
	ADD_TRAIT(owner, TRAIT_XENO_IMMUNE, SOURCE_ORGAN)
	owner.med_hud_set_status()
	INVOKE_ASYNC(src, .proc/AddInfectionImages, owner)

/obj/item/organ/internal/body_egg/Remove(mob/living/carbon/M, special = FALSE)
	if(owner)
		REMOVE_TRAIT(owner, TRAIT_XENO_HOST, SOURCE_ORGAN)
		REMOVE_TRAIT(owner, TRAIT_XENO_IMMUNE, SOURCE_ORGAN)
		owner.med_hud_set_status()

...
/obj/item/holochip/AltClick(mob/user)
	if(!user.canPerformAction(src, NEED_DEXTERITY|FORBID_TELEKINESIS_REACH))
		return
	if(loc != user)
	...
	if(!split_amount || QDELETED(user) || QDELETED(src) || issilicon(user) || !usr.canPerformAction(src, NEED_DEXTERITY|FORBID_TELEKINESIS_REACH) || loc != user)
		return

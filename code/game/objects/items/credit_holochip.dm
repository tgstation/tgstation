...
/obj/item/holochip/AltClick(mob/user)
	if(!user.canPerformAction(src, be_close = TRUE, NEED_DEXTERITY| FORBID_TELEKINESIS_REACH| need_hands = !iscyborg(user)))
		return
	if(loc != user)
	...
	if(!split_amount || QDELETED(user) || QDELETED(src) || issilicon(user) || !usr.canPerformAction(src, be_close = TRUE, NEED_DEXTERITY| FORBID_TELEKINESIS_REACH| need_hands = !iscyborg(user)) || loc != user)
		return

...
/obj/item/airlock_painter/proc/use_paint(mob/user)
	if(can_use(user))
		ink.charges--
		playsound(src.loc, 'sound/effects/spray2.ogg').vary(TRUE).play()
		return TRUE
	else
		return FALSE
...
/obj/item/airlock_painter/suicide_act(mob/living/user)
	...
	if(can_use(user) && L)
		...
		playsound(user.loc, 'sound/effects/splat.ogg').vary(TRUE).play()
...
/obj/item/airlock_painter/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/toner))
		...
		ink = W
		playsound(src.loc, 'sound/machines/click.ogg').vary(TRUE).play()
	else
		return ..()

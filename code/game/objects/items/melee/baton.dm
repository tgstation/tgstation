...
/obj/item/melee/baton/security/boomerang/throw_at(atom/target, range, speed, mob/thrower, spin=1, diagonals_first = 0, datum/callback/callback, force, gentle = FALSE, quickstart = TRUE, hit_crawling_targets = FALSE)
	if(active)
		if(ishuman(thrower))

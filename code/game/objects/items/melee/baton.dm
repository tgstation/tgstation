...
/obj/item/melee/baton/proc/baton_attack(mob/living/target, mob/living/user, modifiers)
	...
	if(!chunky_finger_usable && ishuman(user))
		...
		if(potential_chunky_finger_human.check_chunky_fingers() && user.is_holding(src) && !HAS_TRAIT(user, TRAIT_CHUNKYFINGERS_IGNORE_BATON) && !HAS_TRAIT(user.mind, TRAIT_CHUNKYFINGERS_IGNORE_BATON))

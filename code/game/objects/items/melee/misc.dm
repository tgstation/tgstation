...
/obj/item/melee/sabre/on_exit_storage(datum/storage/container)
	PLAYSOUND(container.parent, 'sound/items/unsheath.ogg').volume(25).vary_frequency(TRUE).play()

/obj/item/melee/sabre/on_enter_storage(datum/storage/container)
	PLAYSOUND(container.parent, 'sound/items/sheath.ogg').volume(25).vary_frequency(TRUE).play()
...
/obj/item/melee/sabre/proc/suicide_dismember(mob/living/user, obj/item/bodypart/affecting)
	if(!QDELETED(affecting) && !(affecting.bodypart_flags & BODYPART_UNREMOVABLE) && affecting.owner == user && !QDELETED(user))
		PLAYSOUND(get_sfx(user), hitsound).volume(25).vary_frequency(TRUE).play()

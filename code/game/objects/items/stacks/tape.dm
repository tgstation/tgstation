...
/obj/item/stack/sticky_tape/attack_hand(mob/user, list/modifiers)
	if(user.get_inactive_held_item() == src)
		if(is_zero_amount(delete_if_zero = TRUE))
			return
		PLAYSOUND(user, 'sound/items/duct_tape/duct_tape_rip.ogg').vary_frequency(TRUE).play()
		...
		PLAYSOUND(user, 'sound/items/duct_tape/duct_tape_snap.ogg').vary_frequency(TRUE).play()
		return TRUE
	return ..()

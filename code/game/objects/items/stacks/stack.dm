		...
		for(var/category in what_are_we_made_of.categories)
			switch(category)
				if(MAT_CATEGORY_BASE_RECIPES)
					recipes |= GLOB.materials.base_stack_recipes.Copy()
				if(MAT_CATEGORY_RIGID)
					recipes |= GLOB.materials.rigid_stack_recipes.Copy()
...
/obj/item/stack/proc/set_mats_per_unit(list/mats, multiplier=1)
	mats_per_unit = GLOB.materials.FindOrCreateMaterialCombo(mats, multiplier)

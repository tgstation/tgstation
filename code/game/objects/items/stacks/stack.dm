...
/obj/item/stack/attack_hand_secondary(mob/user, modifiers)
	...
	if(is_cyborg || !user.canPerformAction(src, be_close = TRUE, no_dexterity = TRUE, no_tk = FALSE, need_hands = !iscyborg(user)))
	...
	if(!stackmaterial || QDELETED(user) || QDELETED(src) || !usr.canPerformAction(src, be_close = TRUE, no_dexterity = FALSE, no_tk = TRUE, need_hands = !iscyborg(user)))
		return SECONDARY_ATTACK_CANCEL_ATTACK_CHAIN

...
/obj/item/stack/attack_hand_secondary(mob/user, modifiers)
	...
	if(is_cyborg || !user.canPerformAction(src, be_close = TRUE, NEED_DEXTERITY|NEED_HANDS|CYBORG_IGNORE_HAND_RESTRICTION))
	...
	if(!stackmaterial || QDELETED(user) || QDELETED(src) || !usr.canPerformAction(src, be_close = TRUE,  FORBID_TELEKINESIS_REACH|NEED_HANDS|CYBORG_IGNORE_HAND_RESTRICTION))
		return SECONDARY_ATTACK_CANCEL_ATTACK_CHAIN

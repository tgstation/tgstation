...
/obj/item/ai_module/proc/install(datum/ai_laws/law_datum, mob/user)
	...
	log_silicon("LAW: [user.name]([user.key]) used [src.name] on [ainame]([aikey]) from [AREACOORD(user)].[law2log ? " The law specified [law2log]" : ""], [length(affected_cyborgs) ? ", impacting synced borgs [borg_txt]" : ""]")

...
/obj/item/toy/plush/whiny_plushie/proc/unsecured_process(last_move)
	...
	PLAYSOUND(src, 'sound/items/intents/Help.ogg').vary_frequency(FALSE).play()
...
/obj/item/toy/plush/moth/suicide_act(mob/living/user)
	...
	PLAYSOUND(src, 'sound/effects/hallucinations/wail.ogg').vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()

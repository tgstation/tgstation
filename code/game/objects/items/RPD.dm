...
GLOBAL_LIST_INIT(atmos_pipe_recipes, list(
	...
	"Devices" = list(
		...
		new /datum/pipe_info/pipe("Gas Filter",			/obj/machinery/atmospherics/components/trinary/filter, FALSE),
		new /datum/pipe_info/pipe("Gas Mixer",			/obj/machinery/atmospherics/components/trinary/mixer, FALSE),
		new /datum/pipe_info/pipe("Passive Gate",		/obj/machinery/atmospherics/components/binary/passive_gate, TRUE),
		new /datum/pipe_info/pipe("Injector",			/obj/machinery/atmospherics/components/unary/outlet_injector, FALSE),
		new /datum/pipe_info/pipe("Scrubber",			/obj/machinery/atmospherics/components/unary/vent_scrubber, FALSE),
		new /datum/pipe_info/pipe("Unary Vent",			/obj/machinery/atmospherics/components/unary/vent_pump, FALSE),
		new /datum/pipe_info/pipe("Passive Vent",		/obj/machinery/atmospherics/components/unary/passive_vent, FALSE),
		...
		switch(category) //if we've gotten this var, the target is valid
			if(ATMOS_CATEGORY) //Making pipes
				...
				if (recipe.type == /datum/pipe_info/meter)
					...
					if(recipe.all_layers == FALSE && (piping_layer == 0 || piping_layer == 4))
						to_chat(user, "<span class='notice'>You can't build this object on the layer...</span>")
						return ..()
					to_chat(user, "<span class='notice'>You start building a pipe...</span>")
					if(do_after(user, atmos_build_speed, target = A))
						if(recipe.all_layers == FALSE && (piping_layer == 0 || piping_layer == 4))//double check to stop cheaters (and to not waste time waiting for something that can't be placed)
							to_chat(user, "<span class='notice'>You can't build this object on the layer...</span>")
							return ..()
						activate()

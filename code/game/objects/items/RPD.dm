...
GLOBAL_LIST_INIT(atmos_pipe_recipes, list(
	"Pipes" = list(
		...
		new /datum/pipe_info/pipe("Bridge Pipe", /obj/machinery/atmospherics/pipe/bridge_pipe, FALSE),
			...
			if(ATMOS_CATEGORY) //Making pipes
				...
				if (recipe.type == /datum/pipe_info/meter)
					...
					if(do_after(user, atmos_build_speed, target = attack_target))
						...
						if(findtext("[queued_p_type]", "/obj/machinery/atmospherics") && !findtext("[queued_p_type]", "color_adapter"))

...
/obj/item/areaeditor/blueprints/proc/view_wire_devices(mob/user)
	...
	for(var/wireset in GLOB.wire_static_color_directory)
		message += "<br><a href='?src=[REF(src)];view_wireset=[wireset]'>[GLOB.wire_static_name_directory[wireset]]</a>"
	message += "</p>"
	return message
...
/obj/item/areaeditor/blueprints/proc/view_wire_set(mob/user, wireset)
	...
	for(var/device in GLOB.wire_static_color_directory)
		if("[device]" == wireset)	//I know... don't change it...
			var/message = "<p><b>[GLOB.wire_static_name_directory[device]]:</b>"
			for(var/Col in GLOB.wire_static_color_directory[device])
				var/wire_name = GLOB.wire_static_color_directory[device][Col]

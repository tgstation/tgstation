...
/obj/item/dice/d20/fate/proc/effect(mob/living/carbon/human/user,roll)
	...
	switch(roll)
		...
		if(16)
			...
			var/list/mob/dead/observer/candidates = SSpolling.poll_ghost_candidates_for_mob("Do you want to play as [user.real_name]'s Servant?", check_jobban = ROLE_WIZARD, role = ROLE_WIZARD, poll_time = 5 SECONDS, target_mob = human_servant, role_name_text = "dice servant")

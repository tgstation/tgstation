...
/obj/item/razor/attack(mob/target_mob, mob/living/user, params)
	...
	if(location == BODY_ZONE_PRECISE_MOUTH)
		if(!user.combat_mode)
			if(human_target.gender == MALE)
				...
				if(!user.can_perform_action(src, FORBID_TELEKINESIS_REACH))
					return
				var/new_style = tgui_input_list(user, "Select a facial hairstyle", "Grooming", DSaccessories.facial_hairstyles_list)
				if(isnull(new_style))
					return
		...
		if(!user.combat_mode)
			...
			if(!user.can_perform_action(src, FORBID_TELEKINESIS_REACH))
				return
			var/new_style = tgui_input_list(user, "Select a hairstyle", "Grooming", DSaccessories.hairstyles_list)
			if(isnull(new_style))
				return
			if(!get_location_accessible(human_target, location))
				to_chat(user, span_warning("The headgear is in the way!"))

		...
		if(location == BODY_ZONE_PRECISE_MOUTH)
			if(user.a_intent == INTENT_HELP)
				if(H.gender == MALE)
					...
					if(!get_location_accessible(H, location))
						...
						return
					user.visible_message("[user] tries to change [H]'s facial hair style using [src].", "<span class='notice'>You try to change [H]'s facial hair style using [src].</span>")
					if(new_style && do_after(user, 60, target = H))
						user.visible_message("[user] successfully changes [H]'s facial hair style using [src].", "<span class='notice'>You successfully change [H]'s facial hair style using [src].</span>")
			...
			if(user.a_intent == INTENT_HELP)
				...
				if(!get_location_accessible(H, location))
					...
					return
				user.visible_message("[user] tries to change [H]'s hairstyle using [src].", "<span class='notice'>You try to change [H]'s hairstyle using [src].</span>")
				if(new_style && do_after(user, 60, target = H))
					user.visible_message("[user] successfully changes [H]'s hairstyle using [src].", "<span class='notice'>You successfully change [H]'s hairstyle using [src].</span>")

...
/obj/item/card/id/advanced/prisoner/attackby(obj/item/card/id/C, mob/user)
	...
	if(!choice || QDELETED(user) || QDELETED(src) || !usr.canPerformAction(src, FORBID_TELEKINESIS_REACH) || loc != user)
		return FALSE
	...
	if(isliving(user) && user.mind)
		...
		if(popup_input == "Forge/Reset")
			if(!forged)
				...
				if(QDELETED(user) || QDELETED(src) || !user.canPerformAction(user, NEED_DEXTERITY| FORBID_TELEKINESIS_REACH))
					return
				if(new_age)

...
/obj/item/card/id/advanced/prisoner/attackby(obj/item/card/id/C, mob/user)
	...
	if(!choice || QDELETED(user) || QDELETED(src) || !usr.canPerformAction(src, be_close = TRUE,  no_tk = TRUE) || loc != user)
		return FALSE
	...
	if(isliving(user) && user.mind)
		...
		if(popup_input == "Forge/Reset")
			if(!forged)
				...
				if(QDELETED(user) || QDELETED(src) || !user.canPerformAction(user, be_close = TRUE, NEED_DEXTERITY| no_tk = TRUE))
					return
				if(new_age)

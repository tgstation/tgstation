...
/obj/item/extinguisher/proc/AttemptRefill(atom/target, mob/user)
	if(istype(target, tanktype) && target.Adjacent(user))
		...
		if(transferred > 0)
			...
			playsound(src.loc, 'sound/effects/refill.ogg', 50, DEFAULT_SOUND_VARY, extra_range = -6)
...
/obj/item/extinguisher/afterattack(atom/target, mob/user , flag)
	...
	if (!safety)
		...
		playsound(src.loc, 'sound/effects/extinguish.ogg', 75, DEFAULT_SOUND_VARY, extra_range = -3)

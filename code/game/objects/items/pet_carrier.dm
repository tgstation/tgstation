...
/obj/item/pet_carrier/click_alt(mob/living/user)
	...
	if(locked)
		PLAYSOUND(user, 'sound/machines/airlock/boltsdown.ogg').volume(30).vary_frequency(TRUE).play()
	else
		PLAYSOUND(user, 'sound/machines/airlock/boltsup.ogg').volume(30).vary_frequency(TRUE).play()
	update_appearance()
...
/obj/item/pet_carrier/container_resist_act(mob/living/user)
	...
	if(user.mob_size <= MOB_SIZE_SMALL)
		...
		locked = FALSE
		PLAYSOUND(src, 'sound/machines/airlock/boltsup.ogg').volume(30).vary_frequency(TRUE).play()
		update_appearance()
	else

...
/obj/item/frog_statue/proc/recall_frog(mob/user)
	PLAYSOUND(src, 'sound/items/frog_statue_release.ogg').volume(20).play()
...
/obj/item/frog_statue/proc/release_frog(mob/user)
	...
	PLAYSOUND(src, 'sound/items/frog_statue_release.ogg').vary(TRUE).play()
...
/obj/item/frog_statue/proc/render_obsolete(datum/source)
	...
	contained_frog = null
	PLAYSOUND(src, 'sound/effects/magic/demon_dies.ogg').vary(TRUE).play()

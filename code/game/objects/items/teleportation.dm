...
/obj/item/syndicate_teleporter/proc/attempt_teleport(mob/user, triggered_by_emp = FALSE, not_holding_tele = FALSE)
	...
	if(isclosedturf(destination))
		...
		PLAYSOUND(current_location, get_sfx(SFX_PORTAL_ENTER)).volume(50).vary_frequency(1).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
		PLAYSOUND(destination, 'sound/effects/phasein.ogg').vlume(25).vary_frequency(1).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
		PLAYSOUND(destination, get_sfx(SFX_PORTAL_ENTER)).volume(50).vary_frequency(1).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
...
/obj/item/syndicate_teleporter/proc/panic_teleport(mob/user, turf/destination)
	...
	if(emergency_destination)
		...
		if(make_bloods(destination, emergency_destination, user))
			...
			new /obj/effect/temp_visual/circle_wave/syndi_teleporter(destination)
		PLAYSOUND(mobloc, get_sfx(SFX_PORTAL_ENTER)).volume(50).vary_frequency(1).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
		PLAYSOUND(emergency_destination, 'sound/effects/phasein.ogg').vlume(25).vary_frequency(1).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
		PLAYSOUND(emergency_destination, get_sfx(SFX_PORTAL_ENTER)).volume(50).vary_frequency(1).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
		PLAYSOUND(src, 'sound/machines/warning-buzzer.ogg').volume(25).vary_frequency(TRUE).play()
...
/obj/item/syndicate_teleporter/proc/get_fragged(mob/living/victim, turf/destination, not_holding_tele = FALSE)
	...
	new /obj/effect/temp_visual/teleport_abductor/syndi_teleporter(mobloc)
	new /obj/effect/temp_visual/teleport_abductor/syndi_teleporter(destination)
	PLAYSOUND(mobloc, get_sfx(SFX_PORTAL_ENTER)).volume(50).vary_frequency(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE)
	PLAYSOUND(destination, get_sfx(SFX_PORTAL_ENTER)).volume(50).vary_frequency(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE)
	PLAYSOUND(destination, 'sound/effects/magic/disintegrate.ogg').vlume(50).vary_frequency(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
	if(!not_holding_tele)
...
/obj/item/syndicate_teleporter/proc/make_bloods(turf/old_location, turf/new_location, mob/living/user)
	...
	if(prob(25) && bleed_amount)
		PLAYSOUND(src, 'sound/effects/wounds/pierce1.ogg').volume(40).vary_frequency(TRUE).play()

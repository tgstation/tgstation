...
/obj/item/hand_tele/universal
	name = "universal hand tele"
	desc = "A portable item using blue-space technology to access any and all tracking beacon or implant locations as well as teleporter hubs."
	icon = 'icons/obj/devices/tracker.dmi'
	icon_state = "universal_tele"
	inhand_icon_state = "electronic"
	worn_icon_state = "electronic"
	lefthand_file = 'icons/mob/inhands/items/devices_lefthand.dmi'
	righthand_file = 'icons/mob/inhands/items/devices_righthand.dmi'
...
/obj/item/hand_tele/universal/attack_self(mob/user)
	if (!can_teleport_notifies(user))
		return
	...
	for(var/obj/item/beacon/R in GLOB.teleportbeacons)
		var/area/beacon_area = get_area(R)
		if(!beacon_area || (beacon_area.area_flags & NOTELEPORT))
			continue
		if(R.renamed)
			locations[avoid_assoc_duplicate_keys("[R.name] ([get_area(R)])", locations)] = R
		else
			var/area/A = get_area(R)
			locations[avoid_assoc_duplicate_keys(A.name, locations)] = R
		...
		if(!I.imp_in || !isliving(I.loc))
			...
			if(M.stat == DEAD)
				if(M.timeofdeath + I.lifespan_postmortem < world.time)
					continue
				var/area/tracker_area = get_area(I)
				if(!tracker_area || (tracker_area.area_flags & NOTELEPORT))
					continue
				locations[avoid_assoc_duplicate_keys("[M.real_name] ([get_area(M)])", locations)] = I
	//

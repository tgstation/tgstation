...
/obj/item/toy/crayon/spraycan/AltClick(mob/user)
	if(has_cap && user.canUseTopic(src, BE_CLOSE, NO_DEXTERITY, FALSE, NEED_HANDS))
		is_capped = !is_capped
		to_chat(user, "<span class='notice'>The cap on [src] is now [is_capped ? "on" : "off"].</span>")
		update_icon()

/obj/item/toy/crayon/CtrlClick(mob/user)
	if(can_change_colour && !isturf(loc) && user.canUseTopic(src, BE_CLOSE, NO_DEXTERITY, FALSE, NEED_HANDS))
		select_colour(user)
	else
		return ..()
...
/obj/item/toy/crayon/proc/select_colour(mob/user)
	...
	if (!isnull(chosen_colour) && user.canUseTopic(src, BE_CLOSE, NO_DEXTERITY, FALSE, NEED_HANDS))

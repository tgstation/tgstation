...
/obj/item/weapon/dice/proc/diceroll(mob/user)
	...
	if(user != null) //Dice was rolled in someone's hand
		user.visible_message("[IDENTITY_SUBJECT(1)] has thrown [src]. It lands on [result]. [comment]", \
							 "<span class='notice'>You throw [src]. It lands on [result]. [comment]</span>", \
							 "<span class='italics'>You hear [src] rolling, it sounds like a [fake_result].</span>", subjects=list(user))

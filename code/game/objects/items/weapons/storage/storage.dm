...
/obj/item/weapon/storage/proc/content_can_dump(atom/dest_object, mob/user)
	. = Adjacent(user) && dest_object.Adjacent(user) && dest_object.storage_contents_dump_act(src, user)
	if(. && rustle_jimmies)
		playsound(src, "rustle", 50, 1, -5)
...
/obj/item/weapon/storage/proc/can_be_inserted(obj/item/W, stop_messages = FALSE, mob/user)
...
/obj/item/weapon/storage/proc/handle_item_insertion(obj/item/W, prevent_warning = FALSE, mob/user)
	if(!istype(W))
		return FALSE
	if(user)

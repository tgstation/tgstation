...
//		return
	...
	if (M.stat !=2)
		...
		/*if((M.mind in ticker.mode.cult) && (prob(20)))
			M << "<span class='danger'>The power of [src.deity_name] clears your mind of heresy!</span>"
			user << "<span class='danger'>You see how [M]'s eyes become clear, the cult no longer holds control over him!</span>"
		...
		if ((istype(M, /mob/living/carbon/human) && prob(60)))
			...
			for(var/mob/O in viewers(M, null))
				O.show_message(text("<span class='userdanger'>[] beats [] over the head with []!</span>", user, M, src), 1)
			playsound(src.loc, "punch", 25, 1, -1)

	else if(M.stat == 2)
		for(var/mob/O in viewers(M, null))
			O.show_message(text("<span class='userdanger'>[] smacks []'s lifeless corpse with [].</span>", user, M, src), 1)
		playsound(src.loc, "punch", 25, 1, -1)
	return

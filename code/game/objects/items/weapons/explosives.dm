...
/obj/item/weapon/c4/afterattack(atom/movable/target, mob/user, flag)
	...
	user << "<span class='notice'>You begin planting the explosive...</span>"
	...
	if(do_after(user, 50) && in_range(user, target))
		...
		if (ismob(target))
			...
			user.visible_message("<span class='danger'>[user.name] finished planting an explosive on [target.name]!</span>", "<span class='danger'>You finish planting an explosive on [target.name]!</span>")
		...
		user << "<span class='danger'>Bomb has been planted. Timer counting down from [timer]!</span>"
		spawn(timer*10)

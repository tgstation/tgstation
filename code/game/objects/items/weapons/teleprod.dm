...
/obj/item/weapon/melee/baton/cattleprod/attackby(obj/item/I, mob/user, params)//handles sticking a crystal onto a stunprod to make a teleprod
	..()
	if(istype(src, /obj/item/weapon/melee/baton/cattleprod/sparkprod))
		user.visible_message("<span class='warning'>[src] has to be made with wirecutters to clamp on a bluespace crystal!</span>")
		return

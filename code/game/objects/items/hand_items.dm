...
/obj/item/hand_item/willie/attack(mob/living/carbon/target, mob/living/carbon/human/user)
	...
	if(HAS_TRAIT(user, TRAIT_PACIFISM))
		to_chat(user, span_warning("You can't bring yourself to give [target] a wet willie! You don't want to risk harming anyone..."))
		return
	...
	if(istype(ears, /obj/item/organ/internal/ears/cat))
		target.add_mood_event("willie", /datum/mood_event/willie_cat)
		return

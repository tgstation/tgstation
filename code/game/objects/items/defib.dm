...
/obj/item/defibrillator/proc/finish_charging()
	if(cell)
		if(cell.charge >= paddles.revivecost)
			...
			create_sound(src, 'sound/machines/defib/defib_ready.ogg').play()
		else
...
/obj/item/shockpaddles/suicide_act(mob/living/user)
	...
	create_sound(src, 'sound/machines/defib/defib_zap.ogg').vary(TRUE).extra_range(-1).play()
	return OXYLOSS
...
/obj/item/shockpaddles/proc/do_harm(mob/living/carbon/H, mob/living/user)
	...
	if(do_after(user, 1.5 SECONDS, H, extra_checks = CALLBACK(src, PROC_REF(is_wielded))))
		...
		create_sound(src, 'sound/machines/defib/defib_charge.ogg').play()
		if(req_defib)
		...
		if(do_after(user, 1.5 SECONDS, H, extra_checks = CALLBACK(src, PROC_REF(is_wielded)))) //Takes longer due to overcharging
			...
			create_sound(src, 'sound/machines/defib/defib_zap.ogg').volume(100).vary(TRUE).extra_range(-1).play()
			create_sound(src, 'sound/items/weapons/egloves.ogg').volume(100).vary(TRUE).extra_range(-1).play()
			H.emote("scream")
			shock_pulling(45, H)
...
/obj/item/shockpaddles/proc/do_help(mob/living/carbon/H, mob/living/user)
	...
	if(do_after(user, 3 SECONDS, H, extra_checks = CALLBACK(src, PROC_REF(is_wielded)))) //beginning to place the paddles on patient's chest to allow some time for people to move away to stop the process
		...
		create_sound(src, 'sound/machines/defib/defib_charge.ogg').volume(75).vary(FALSE).play()
		...
		if(do_after(user, 2 SECONDS, H, extra_checks = CALLBACK(src, PROC_REF(is_wielded)))) //placed on chest and short delay to shock for dramatic effect, revive time is 5sec total
			...
			if(H.stat == DEAD)
				...
				create_sound(src, SFX_BODYFALL).vary(TRUE).play()
				create_sound(src, 'sound/machines/defib/defib_zap.ogg').volume(75).vary(TRUE).extra_range(-1).play()
				shock_pulling(30, H)
				...
				if(fail_reason)
					...
					create_sound(src, 'sound/machines/defib/defib_success.ogg').play()

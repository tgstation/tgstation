...
/obj/item/shockpaddles/proc/do_harm(mob/living/carbon/H, mob/living/user)
	...
	busy = TRUE
	update_appearance()
	if(do_after(user, H, 1.5 SECONDS, extra_checks = CALLBACK(src, .proc/is_wielded)))
		...
		if(do_after(user, H, 1.5 SECONDS, extra_checks = CALLBACK(src, .proc/is_wielded))) //Takes longer due to overcharging
			if(!H)
				do_cancel()
				return
			if(H && H.stat == DEAD)
...
/obj/item/shockpaddles/proc/do_help(mob/living/carbon/H, mob/living/user)
	...
	busy = TRUE
	update_appearance()
	if(do_after(user, H, 3 SECONDS, extra_checks = CALLBACK(src, .proc/is_wielded))) //beginning to place the paddles on patient's chest to allow some time for people to move away to stop the process
		...
		if(do_after(user, H, 2 SECONDS, extra_checks = CALLBACK(src, .proc/is_wielded))) //placed on chest and short delay to shock for dramatic effect, revive time is 5sec total

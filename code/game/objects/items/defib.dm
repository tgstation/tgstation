...
/obj/item/twohanded/shockpaddles/suicide_act(mob/user)
	user.visible_message("<span class='suicide'>[user] is putting the live paddles on [user.p_their()] chest! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	if(req_defib)
...
/obj/item/twohanded/shockpaddles/proc/do_harm(mob/living/carbon/H, mob/living/user)
	...
	if(do_after(user, 30, target = H))
		user.visible_message("<span class='warning'>[user] places [src] on [H]'s chest.</span>",
...
/obj/item/twohanded/shockpaddles/proc/do_help(mob/living/carbon/H, mob/living/user)
	user.visible_message("<span class='warning'>[user] begins to place [src] on [H]'s chest.</span>", "<span class='notice'>You begin to place [src] on [H]'s chest...</span>")
	busy = TRUE
	update_icon()
	if(do_after(user, 30, target = H)) //beginning to place the paddles on patient's chest to allow some time for people to move away to stop the process
		user.visible_message("<span class='warning'>[user] places [src] on [H]'s chest.</span>", "<span class='notice'>You place [src] on [H]'s chest.</span>")

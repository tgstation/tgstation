	...
	if(GLOBAL_RADIO_TYPE == 1) // NEW RADIO SYSTEMS: By Doohl
	  ...
	  /* ###### Radio headsets can only broadcast through subspace ###### */
		...
		if(subspace_transmission)
			...
			world << "let's try connection [connection.frequency]"
			if(!IsDepartmentChannel(connection.frequency))
				return
			world << "WTF IT WENT THRU"
...
/obj/item/device/radio/proc/receive_range(freq, level)
	...
	var/department_channel = IsDepartmentChannel(freq)
...
#undef NO_COMMON_HEADSET_CHANNEL
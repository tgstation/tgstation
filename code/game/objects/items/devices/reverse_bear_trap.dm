...
/obj/item/reverse_bear_trap/attack(mob/living/target, mob/living/user)
	...
	arm()
	notify_ghosts("[user] put a reverse bear trap on [target]!", source = src, action = NOTIFY_ORBIT, notify_flags = NOTIFY_CATEGORY_NOFLASH, ghost_sound = 'sound/machines/beep.ogg', notify_volume = 75, header = "Reverse bear trap armed")

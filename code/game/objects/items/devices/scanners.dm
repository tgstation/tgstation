...
/proc/atmosanalyzer_scan(mixture, mob/living/user, atom/target = src)
	...
	for(var/g in airs)
		...
		var/cached_results = air_contents.reaction_results
		...
		if(cached_results && cached_results["fusion"]) //notify the user if a fusion reaction was detected
			var/fusion_power = round(cached_results["fusion"], 0.01)

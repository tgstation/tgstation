	...
	switch(href_list["choice"])
		...
		if("Status")
			switch(href_list["statdisp"])
				...
				if("setmsg1")
					message1 = reject_bad_text(input("Line 1", "Enter Message Text", message1, nullable = TRUE, istext = TRUE), 40)
					updateSelfDialog()
				if("setmsg2")
					message2 = reject_bad_text(input("Line 2", "Enter Message Text", message2, nullable = TRUE, istext = TRUE), 40)
					updateSelfDialog()
				else

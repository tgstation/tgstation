				...
				for (var/mob/living/simple_animal/bot/cleanbot/B in GLOB.alive_mob_list)
					...
					if(bl)
						if (bl.z != cl.z)
							continue
						var/direction = get_dir(src, B)
						ldat += "Cleanbot - <b>\[[bl.x],[bl.y] ([uppertext(dir2text(direction))])\]</b> - [B.bot_status_flags & BOT_MODE_ON ? "Online" : "Offline"]<br>"
...
/obj/item/cartridge/proc/bot_control()
	if(active_bot)
		...
		for(var/B in GLOB.bots_list) //Git da botz
			...
			if(!Bot.bot_status_flags & BOT_MODE_ON || Bot.z != zlevel || Bot.bot_status_flags & BOT_AI_REMOTE_DISABLED || !(bot_access_flags & Bot.bot_type)) //Only non-emagged bots on the same Z-level are detected!

...
/obj/item/device/megaphone/attack_self(mob/living/carbon/human/user)
	...
	if ((src.loc == user && user.stat == 0))
		if(emagged)
			if(insults)
				user.audible_message("<B>[user.real_name]</B> broadcasts, <FONT size=3>\"[pick(insultmsg)]\"</FONT>")
				insults--
			else
				user << "<span class='warning'>*BZZZZzzzzzt*</span>"
		else
			user.audible_message("<B>[user.real_name]</B> broadcasts, <FONT size=3>\"[message]\"</FONT>")

		...
		switch(user.zone_selected)
			...
			if(BODY_ZONE_PRECISE_MOUTH)
				...
				if(M == user)
					...
					user.visible_message("<span class='none'>[user] directs [src] to [M]'s mouth.</span>",\
...
/obj/item/flashlight/flare/attack_self(mob/user)
	...
	// All good, turn it on.
	if(.)
		user.visible_message("<span class='none'>[user] lights \the [src].</span>", "<span class='notice'>You light \the [src]!</span>")
		force = on_damage
		damtype = "fire"
...
/obj/item/flashlight/glowstick/attack_self(mob/user)
	...
	. = ..()
	if(.)
		user.visible_message("<span class='none'>[user] cracks and shakes [src].</span>", "<span class='notice'>You crack and shake [src], turning it on!</span>")
		START_PROCESSING(SSobj, src)

...
/obj/item/flashlight/directional
	name = "old flashlight"
...
/obj/item/flashlight/directional/update_brightness(mob/user = null)
	..()
	if(light)
		if(user)
...
/obj/item/flashlight/directional/afterattack(atom/A, mob/living/user, flag, params)
	..()
	var/C = user.client
	if(C)
		dir = user.dir
...
obj/item/flashlight/directional/CanItemAutoclick() //BURN THE SERVER DOWN
	return 2

obj/item/flashlight/directional/proc/on_mob_turn()
	if(ismob(loc) && on && !pointing)
		light.directional = 1
		update_light()

obj/item/flashlight/directional/equipped(mob/user, slot)
	. = ..()
...
obj/item/flashlight/directional/dropped()
	. = ..()
	QDEL_NULL(mobhook)

obj/item/flashlight/directional/Destroy()

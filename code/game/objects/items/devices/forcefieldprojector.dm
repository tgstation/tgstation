...
/obj/structure/projected_forcefield/take_damage(damage_amount, damage_type = BRUTE, damage_flag = 0, sound_effect = 1, attack_dir)
	...
	spawn_atom_to_turf(/obj/effect/overlay/temp/emp/pulse, src, 1)
	generator.shield_integrity = max(generator.shield_integrity - damage_amount, 0)

/obj/item/device/forcefield/mounted
...
/obj/item/device/forcefield/mounted/proc/place(turf/T, mob/living/C)
	...
	if(T.density)
		return
	if(get_dist(T,src) > field_distance_limit)
		to_chat(C, "<span class='notice'>That area is outside of forcefield range!</span>")
		return
	if(LAZYLEN(current_fields) >= max_fields)
		to_chat(C, "<span class='notice'>[src] cannot sustain any more forcefields!</span>")
		return
	playsound(T,'sound/weapons/resonator_fire.ogg',50,1)
	var/obj/structure/projected_forcefield/F = new(T, src)
	current_fields += F
...
/obj/item/chameleon/afterattack(atom/target, mob/user , proximity)
	...
	if(iseffect(target))
		if(!(istype(target, /obj/effect/decal))) //be a footprint
			return
	playsound(get_turf(src), 'sound/weapons/flash.ogg', 100, DEFAULT_SOUND_VARY, extra_range = -6)
...
/obj/item/chameleon/proc/toggle(mob/user)
	...
	if(active_dummy)
		eject_all()
		playsound(get_turf(src), 'sound/effects/pop.ogg', 100, DEFAULT_SOUND_VARY, extra_range = -6)
		qdel(active_dummy)
		active_dummy = null

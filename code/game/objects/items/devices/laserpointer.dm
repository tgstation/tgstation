	...
	for(var/mob/living/carbon/human/H in view(1,targloc))
		...
		if(user.mobility_flags & MOBILITY_STAND)
			...
			if(prob(effectchance))
				...
				H.visible_message("<span class='warning'>[H] looks briefly distracted by the light.</span>","<span class='warning'> You're briefly tempted by the shiny light... </span>")
		else
			H.visible_message("[H] stares at the light.","<span class='warning'> You stare at the light... </span>")

	//cats!
	for(var/mob/living/simple_animal/pet/cat/C in view(1,targloc))
		if(prob(50))
			C.visible_message("<span class='warning'>[C] pounces on the light!</span>","<span class='warning'>LIGHT!</span>")
			C.Move(targloc)
			C.set_resting(TRUE, FALSE)
		else
			C.visible_message("[C] looks uninterested in your games.","<span class='warning'>You spot [user] shining [src] at you. How insulting!</span>")

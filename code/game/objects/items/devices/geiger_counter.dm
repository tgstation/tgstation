...
/obj/item/device/geiger_counter/rad_act(amount)
	...
	if(isliving(loc))
		...
		if(!emagged)
			to_chat(M, "<span class='boldannounce'>[bIcon(src, M)] RADIATION PULSE DETECTED.</span>")
			to_chat(M, "<span class='boldannounce'>[bIcon(src, M)] Severity: [amount]</span>")
		else
			to_chat(M, "<span class='boldannounce'>[bIcon(src, M)] !@%$AT!(N P!LS! D/TEC?ED.</span>")
			to_chat(M, "<span class='boldannounce'>[bIcon(src, M)] &!F2rity: <=[amount]#1</span>")
	update_icon()

/obj/item/device/geiger_counter/attack_self(mob/user)
	scanning = !scanning
	update_icon()
	to_chat(user, "<span class='notice'>[bIcon(src, user)] You switch [scanning ? "on" : "off"] [src].</span>")
...
/obj/item/device/geiger_counter/attack(mob/living/M, mob/user)
	if(user.a_intent == INTENT_HELP)
		if(!emagged)
			...
			if(!M.radiation)
				to_chat(user, "<span class='notice'>[bIcon(src, user)] Radiation levels within normal boundaries.</span>")
				return 1
			else
				to_chat(user, "<span class='boldannounce'>[bIcon(src, user)] Subject is irradiated. Radiation levels: [M.radiation].</span>")
				return 1
		else

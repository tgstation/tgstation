...
/obj/item/lightreplacer/emag_act(mob/user, obj/item/card/emag/emag_card)
	if(obj_flags & EMAGGED)
		return FALSE
	obj_flags |= EMAGGED
	PLAYSOUND(loc, get_sfx(SFX_SPARKS)).volume(100).vary_frequency(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()
	update_appearance()
...
/obj/item/lightreplacer/proc/do_action(atom/target, mob/user)
	...
	if(istype(target, /obj/machinery/light))
		if(replace_light(target, user) && bluespace_toggle)
			...
			PLAYSOUND(src, 'sound/items/pshoom/pshoom.ogg').volume(40).vary_frequency(1).play()
		return TRUE

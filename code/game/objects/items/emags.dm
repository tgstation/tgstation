...
/obj/item/card/emagfake/interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	if(exploding)
		playsound(src, 'sound/items/bikehorn.ogg').vary(TRUE).frequency(2).play()
	else if(obj_flags & EMAGGED)
		log_bomber(user, "triggered", src, "(rigged/emagged)")
		visible_message(span_boldwarning("[src] begins to heat up!"))
		playsound(src, 'sound/items/bikehorn.ogg').volume(100).vary(TRUE).frequency(0.25).play()
...
/obj/item/card/emag/doorjack/proc/recharge(mob/user)
	...
	playsound(src, 'sound/machines/beep/twobeep.ogg').volume(10).vary(TRUE).extra_range(SILENCED_SOUND_EXTRARANGE).falloff_distance(0).play()

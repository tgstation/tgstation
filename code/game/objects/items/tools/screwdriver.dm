...
/obj/item/screwdriver/power/proc/on_transform(obj/item/source, mob/user, active)
	...
	PLAYSOUND(src, 'sound/items/tools/change_drill.ogg').volume(50).vary_frequency(TRUE).play()
...
/obj/item/screwdriver/power/suicide_act(mob/living/user)
	...
	PLAYSOUND(loc, 'sound/items/tools/drill_use.ogg').volume(50).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
	return BRUTELOSS

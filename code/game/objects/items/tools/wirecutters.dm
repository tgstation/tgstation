...
/obj/item/wirecutters/suicide_act(mob/user)
	...
	playsound(loc, usesound, 50, DEFAULT_SOUND_VARY, extra_range = -1)
	return (BRUTELOSS)
...
/obj/item/wirecutters/power/suicide_act(mob/user)
	...
	playsound(loc, 'sound/items/jaws_cut.ogg', 50, DEFAULT_SOUND_VARY, extra_range = -1)
	if(iscarbon(user))
		var/mob/living/carbon/C = user
		var/obj/item/bodypart/BP = C.get_bodypart(BODY_ZONE_HEAD)
		if(BP)
			BP.drop_limb()
			playsound(loc, "desceration", 50, DEFAULT_SOUND_VARY, extra_range = -1)
	return (BRUTELOSS)

/obj/item/wirecutters/power/attack_self(mob/user)
	playsound(get_turf(user), 'sound/items/change_jaws.ogg', 50, DEFAULT_SOUND_VARY)

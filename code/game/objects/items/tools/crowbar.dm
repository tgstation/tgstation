...
/obj/item/crowbar/power/suicide_act(mob/living/user)
	if(tool_behaviour == TOOL_CROWBAR)
		...
		playsound(loc, 'sound/items/tools/jaws_pry.ogg').vary(TRUE).range(-1 + SOUND_RANGE).play()
	else
		...
		playsound(loc, 'sound/items/tools/jaws_cut.ogg').vary(TRUE).range(-1 + SOUND_RANGE).play()
		if(iscarbon(user))
...
/obj/item/crowbar/mechremoval/proc/empty_mech(obj/vehicle/sealed/mecha/mech, mob/user)
	...
	playsound(mech, 'sound/machines/airlock/airlock_alien_prying.ogg').volume(100).vary(TRUE).play()
	...
	playsound(mech, 'sound/machines/airlock/airlockforced.ogg').volume(75).vary(TRUE).play()

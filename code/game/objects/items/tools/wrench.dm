...
/obj/item/wrench/suicide_act(mob/living/user)
	...
	playsound(loc, 'sound/items/weapons/genhit.ogg').vary(TRUE).range(-1 + SOUND_RANGE).play()
	return BRUTELOSS
...
/obj/item/wrench/medical/suicide_act(mob/living/user)
	...
	playsound(loc, 'sound/effects/pray.ogg').vary(TRUE).range(-1 + SOUND_RANGE).play()
...
/obj/item/wrench/combat/proc/on_transform(obj/item/source, mob/user, active)
	...
	playsound(src, active ? 'sound/items/weapons/saberon.ogg' : 'sound/items/weapons/saberoff.ogg').volume(5).vary(TRUE).play()

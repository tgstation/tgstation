...
/obj/structure/syndicate_uplink_beacon/attack_hand(mob/living/user, list/modifiers)
	...
	if(!do_after(user = user, delay = 3 SECONDS, target = src, interaction_key = REF(src), visibility = DO_HIDDEN))
		typing_sounds.stop()
		return
	typing_sounds.stop()
	probe_traitor(user)
...
/obj/structure/syndicate_uplink_beacon/screwdriver_act_secondary(mob/living/user, obj/item/tool)
	...
	if (!do_after(user, 5 SECONDS, target = src, visibility = DO_HIDDEN))
		return FALSE

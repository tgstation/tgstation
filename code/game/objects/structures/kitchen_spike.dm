...
/obj/machinery/kitchenspike_frame
...
/obj/machinery/kitchenspike_frame/attackby(obj/item/I, mob/user, params)
	...
	if(default_unfasten_wrench(user, I))
		...
		if(R.get_amount() >= 4)
			...
			var/obj/F = new /obj/machinery/kitchenspike(src.loc,)
			transfer_fingerprints_to(F)
			qdel(src)

/obj/machinery/kitchenspike
	name = "meat spike"
	icon = 'icons/obj/kitchen.dmi'
	icon_state = "spike"
...
/obj/machinery/kitchenspike/attack_paw(mob/user)
	return src.attack_hand(usr)

/obj/machinery/kitchenspike/proc/is_mob_allowed(var/obj/item/weapon/grab/G)
	for(var/I in allowed_types)
		if(istype(G.affecting, I))
			return I
	return 0

/obj/machinery/kitchenspike/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/crowbar))
		if(!src.occupied)
			...
			if(do_after(user, 20, target = src))
				...
				var/obj/F = new /obj/machinery/kitchenspike_frame(src.loc,)
				transfer_fingerprints_to(F)
				qdel(src)
		else

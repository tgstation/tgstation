...
/obj/structure/mannequin/update_overlays()
	...
	pedestal.pixel_y = -3
	. += pedestal
	var/datum/sprite_accessory/underwear/underwear = DSaccessories.underwear_list[underwear_name]
	if(underwear)
	...
	var/datum/sprite_accessory/undershirt/undershirt = DSaccessories.undershirt_list[undershirt_name]
	if(undershirt)
		if(body_type == FEMALE)
	...
	var/datum/sprite_accessory/socks/socks = DSaccessories.socks_list[socks_name]
	if(socks)
...
/obj/structure/mannequin/attack_hand_secondary(mob/user, list/modifiers)
	...
	switch(choice)
		if("Underwear")
			var/new_undies = tgui_input_list(user, "Select the mannequin's underwear", "Changing", DSaccessories.underwear_list)
			if(new_undies)
				underwear_name = new_undies
		if("Undershirt")
			var/new_undershirt = tgui_input_list(user, "Select the mannequin's undershirt", "Changing", DSaccessories.undershirt_list)
			if(new_undershirt)
				undershirt_name = new_undershirt
		if("Socks")
			var/new_socks = tgui_input_list(user, "Select the mannequin's socks", "Changing", DSaccessories.socks_list)
			if(new_socks)

...
/obj/structure/easel/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/canvas))
		...
		C.layer = layer+0.1
		user.visible_message("[user] puts \the [C] on \the [src].","<span class='notice'>You place \the [C] on \the [src].</span>")
	else
		return ..()
...
/obj/item/canvas/attack_self(mob/user)
	...
	if(blank)
		...
		icon = blank
		user.visible_message("[user] cleans the canvas.","<span class='notice'>You clean the canvas.</span>")

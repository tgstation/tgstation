...
/datum/lift_master/proc/Check_lift_move(check_dir)
	for(var/l in lift_platforms)
		...
		if(check_dir == UP && !istype(T, /turf/open/openspace)) // We don't want to go through the ceiling!
			return FALSE
		if(check_dir == DOWN && (!istype(T, /turf/open/openspace) || !istype(T, /turf/open/floor/plating/elevatorshaft)) && !istype(get_turf(lift_platform), /turf/open/openspace)) // No going through the floor!
			return FALSE
	return TRUE

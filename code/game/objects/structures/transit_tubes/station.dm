			...
			if(!pod.moving && pod.dir in directions())
				if(icon_state == "closed")
					...
					if(pod.contents.len && user.loc != pod)
						user.visible_message("<span class='notice'>[user] starts emptying [pod]'s contents onto the floor...</span>")
...
/obj/structure/transit_tube/station/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/weapon/grab) && icon_state == "open")
		...
		if(ismob(G.affecting) && G.state >= GRAB_AGGRESSIVE)
			...
			for(var/obj/structure/transit_tube_pod/pod in loc)
				pod.visible_message("<span class='notice'>[user] starts putting [GM] into the [pod]...</span>")

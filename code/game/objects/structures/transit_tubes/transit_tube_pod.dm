...
/obj/structure/transit_tube_pod/proc/follow_tube(obj/structure/transit_tube/tube)
	...
	register_signal(engine, COMSIG_MOVELOOP_PREPROCESS_CHECK, .proc/before_pipe_transfer)
	register_signal(engine, COMSIG_MOVELOOP_POSTPROCESS, .proc/after_pipe_transfer)
	register_signal(engine, COMSIG_PARENT_QDELETING, .proc/engine_finish)

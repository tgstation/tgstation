...
/obj/item/bedsheet/captain
	...
	desc = "It has a Nanotrasen symbol on it, and was woven with a revolutionary new kind of thread guaranteed to have 0.01% permeability for most non-chemical substances, popular between most modern captains."
...
/obj/structure/bedsheetbin/attackby(obj/item/I, mob/user, params)
	...
	else if(amount && !hidden && I.w_class < WEIGHT_CLASS_BULKY) //make sure there's sheets to hide it inside, make sure nothing else is hidden in there.
		if(!user.transferItemToLoc(I, src))
			to_chat(user, span_warning("\The [I] is stuck to your hand, you cannot hide it between the sheets!"))
			return
		hidden = I
		to_chat(user, span_notice("You hide [I] between the sheets."))

	...
	switch(choice)
		...
		if("gender")
			...
			if(!Adjacent(user))
				return
			if(H.gender == "male")
				if(wrap_alert(H, "Become a Witch?", "Confirmation", "Yes", "No") == "Yes")
					H.gender = "female"
				...
				if(wrap_alert(H, "Become a Warlock?", "Confirmation", "Yes", "No") == "Yes")
					H.gender = "male"
		...
		if("hair")
			var/hairchoice = wrap_alert(H, "Hair style or hair color?", "Change Hair", "Style", "Color")
			if(!Adjacent(user))
				return

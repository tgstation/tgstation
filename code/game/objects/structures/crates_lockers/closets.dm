...
/obj/structure/closet/proc/tool_interact(obj/item/weapon, mob/living/user)
	...
	if(!opened && istype(weapon, /obj/item/airlock_painter))
		...
		var/str = tgui_input_text(user, "Personal Locker Name", "Locker Name", max_length = MAX_NAME_LEN)
		if(!isnull(str))
			name = str
			name_set = TRUE

		str = tgui_input_text(user, "Personal Locker Description", "Locker Description", max_length = MAX_DESC_LEN)
		if(!isnull(str))
			desc = str
			desc_set = TRUE

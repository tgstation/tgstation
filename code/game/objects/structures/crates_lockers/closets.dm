...
/obj/structure/closet/examine(mob/user)
	...
	. += span_notice("It can be [EXAMINE_HINT("installed")] with airlock electronics or a card reader to control access.")
	if(HAS_TRAIT(user, TRAIT_SKITTISH) && divable)
		. += span_notice("If you bump into [p_them()] while running, you will jump inside.")

	if(card_reader_installed)
		. += span_notice("The card reader can be [EXAMINE_HINT("pried")] out.")
		. += span_notice("Swipe an ID to change access levels.")

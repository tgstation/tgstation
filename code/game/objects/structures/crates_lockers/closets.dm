...
/obj/structure/closet/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	if(!locked && !opened)
		if(id_card && held_item.get_writing_implement_details()?["interaction_mode"] == MODE_WRITING)
...
/obj/structure/closet/proc/tool_interact(obj/item/weapon, mob/living/user)
	...
	else if(!opened && weapon.get_writing_implement_details()?["interaction_mode"] == MODE_WRITING)
		if(locked)
			balloon_alert(user, "unlock first!")
			return

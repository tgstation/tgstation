...
/obj/structure/guncase
	...
	opacity = 0
	var/closed = 1
	var/list/allowed_guns = list(/obj/item/weapon/gun)
...
/obj/structure/guncase/attack_hand(mob/user)
	if(!closed)
		if(contents.len)
			var/obj/item/weapon/gun = input("Which gun will you remove from the case?") as null|obj in contents
			if(choice)
...
/obj/structure/guncase/ex_act(severity, target)
	..()
	if(!gc_destroyed)
		for(var/obj/item/weapon/gun in contents)
			b.loc = (get_turf(src))

/obj/structure/guncase/update_icon()
	if(contents.len < 5)
		icon_state = "[contents.len]-open"
	else
		icon_state = "book-5"
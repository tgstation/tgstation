...
/obj/structure/girder/attackby(obj/item/W as obj, mob/user as mob, params)
	...
	if(istype(W, /obj/item/weapon/wrench) && state == 0)
		if(anchored && !istype(src,/obj/structure/girder/displaced))
			...
			user << "<span class='notice'>You start disassembling the girder...</span>"
			if(do_after(user,40))
				if(!src.loc)
					return
				user << "<span class='notice'>You dissasemble the girder.</span>"
		...
		user << "<span class='notice'>You start dislodging the girder...</span>"
		if(do_after(user, 40))
			if(!src.loc)
				return
			user << "<span class='notice'>You dislodge the girder.</span>"
		...
		if (!istype(src.loc, /turf/simulated/floor))
			usr << "<span class='warning'>The girder is too unstable to build anything!</span>"
			return

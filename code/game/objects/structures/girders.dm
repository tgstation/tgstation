...
/obj/structure/girder/examine(mob/user)
	...
	switch(state)
		if(GIRDER_REINF)
			to_chat(user, "<span class='info'>The support struts are <b>screwed</b> in place.</span>")
		if(GIRDER_REINF_STRUTS)
			to_chat(user, "<span class='info'>The support struts are <i>unscrewed</i> and the inner <b>grille</b> is intact.</span>")
		if(GIRDER_NORMAL)
			if(can_displace)
				to_chat(user, "<span class='info'>The bolts are <b>wrenched</b> in place.</span>")
		if(GIRDER_DISPLACED)
			to_chat(user, "<span class='info'>The bolts are <i>loosened</i>, but the <b>screws</b> are holding [src] together.</span>")
		if(GIRDER_DISASSEMBLED)
			to_chat(user, "<span class='info'>[src] is disassembled! You probably shouldn't be able to see this examine message.</span>")

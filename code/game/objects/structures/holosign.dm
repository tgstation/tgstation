...
/obj/structure/holosign/barrier/CanAllowThrough(atom/movable/mover, border_dir)
	...
	if(iscarbon(mover))
		...
		if(IS_UNCONSCIOUS_CRIT_OR_DEAD(moving_carbon)) // Lets not prevent dragging unconscious/dead people.
			return TRUE
...
/obj/structure/holosign/barrier/wetsign/CanAllowThrough(atom/movable/mover, border_dir)
	...
	if(iscarbon(mover))
		...
		if(IS_UNCONSCIOUS_CRIT_OR_DEAD(C)) // Lets not prevent dragging unconscious/dead people.
			return TRUE

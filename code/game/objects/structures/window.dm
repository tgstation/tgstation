...
/obj/structure/window/proc/spawn_debris(location)
	...
	if(!isnull(shard_type))
		dropped_debris += SSwardrobe.provide(shard_type, location)
		if (fulltile)
			dropped_debris += SSwardrobe.provide(shard_type, location)
	if(!isnull(debris_type))
		dropped_debris += SSwardrobe.provide(debris_type, location)
	if (reinf)
		dropped_debris += SSwardrobe.provide(/obj/item/stack/rods, location, STACK_AMOUNT(fulltile ? 2 : 1))
...
/obj/structure/window/paperframe/spawn_debris(location)
	. = list(SSwardrobe.provide(/obj/item/stack/sheet/mineral/wood, location))

...
/obj/structure/window/examine(mob/user)
	...
	switch(state)
		...
		if(WINDOW_IN_FRAME)
			. += span_notice("Окно <i>отвинчено</i>, но <b>вставлено</b> в раму.")
		if(WINDOW_OUT_OF_FRAME)
			if (anchored)
				. += span_notice("Окно <b>привинчено</b> к полу.")
			else
				. += span_notice("Окно <i>отвинчено</i> от пола и может быть разобрано путем <b>выкручивания</b>.")
...
/obj/structure/window/attack_tk(mob/user)
	...
	user.visible_message(span_notice("Что-то стучит в [src]."))
...
/obj/structure/window/attack_hand(mob/living/user, list/modifiers)
	...
	if(!user.combat_mode)
		user.visible_message(span_notice("[user] стучит в [src]."), \
			span_notice("Ты стучишь в [src]."))
		playsound(src, knock_sound, 50, TRUE)
	else
		user.visible_message(span_warning("[user] наносит удары по [src]!"), \
			span_warning("Ты бьешь [src]!"))
...
/obj/structure/window/welder_act(mob/living/user, obj/item/tool)
	if(atom_integrity >= max_integrity)
		to_chat(user, span_warning("[src] уже находится в хорошем состоянии!"))
		return ITEM_INTERACT_SUCCESS
	if(!tool.tool_start_check(user, amount = 0))
		return FALSE
	to_chat(user, span_notice("Вы начинаете восстанавливать  [src]..."))
	if(tool.use_tool(src, user, 4 SECONDS, volume = 50))
		atom_integrity = max_integrity
		update_nearby_icons()
		to_chat(user, span_notice("Вы восстанавливаете [src]."))
	return ITEM_INTERACT_SUCCESS
	...
	switch(state)
		if(WINDOW_SCREWED_TO_FRAME)
			...
			if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				...
				to_chat(user, span_notice("Вы отвинтили окно от рамы."))
		if(WINDOW_IN_FRAME)
			to_chat(user, span_notice("Вы начинаете прикручивать окно к раме..."))
			if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				state = WINDOW_SCREWED_TO_FRAME
				to_chat(user, span_notice("Вы прикрепили окно к раме."))
		if(WINDOW_OUT_OF_FRAME)
			if(anchored)
				...
				if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
					set_anchored(TRUE)
					to_chat(user, span_notice("Вы прикрутили раму к полу."))
	return ITEM_INTERACT_SUCCESS
...
/obj/structure/window/wrench_act(mob/living/user, obj/item/tool)
	...
	to_chat(user, span_notice("Вы успешно разобрали [src]."))
	qdel(src)
	return ITEM_INTERACT_SUCCESS
...
/obj/structure/window/crowbar_act(mob/living/user, obj/item/tool)
	...
	switch(state)
		if(WINDOW_IN_FRAME)
			to_chat(user, span_notice("Вы начинаете вытаскивать окно рычагом из рамы..."))
			if(tool.use_tool(src, user, 10 SECONDS, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				...
				to_chat(user, span_notice("Вы вынимаете окно из рамы."))
		if(WINDOW_OUT_OF_FRAME)
			to_chat(user, span_notice("Вы начинаете задвигать окно обратно в раму..."))
...
/obj/structure/window/reinforced/attackby_secondary(obj/item/tool, mob/user, params)
	if(resistance_flags & INDESTRUCTIBLE)
		balloon_alert(user, "слишком упругий!")
		return SECONDARY_ATTACK_CANCEL_ATTACK_CHAIN
	switch(state)
		if(RWINDOW_SECURE)
			if(tool.tool_behaviour == TOOL_WELDER)
				if(tool.tool_start_check(user, heat_required = HIGH_TEMPERATURE_REQUIRED))
					user.visible_message(span_notice("[user] прижимает \the [tool] к крепежным винтам на \the [src]..."),
						span_notice("Вы начинаете нагревать защитные винты на \the [src]..."))
				...
				to_chat(user, span_warning("Сначала необходимо нагреть крепежные винты!"))

		if(RWINDOW_BOLTS_HEATED)
			if(tool.tool_behaviour == TOOL_SCREWDRIVER)
				user.visible_message(span_notice("[user] нащупывает нагретые защитные винты и начинает их откручивать..."),
										span_notice("Вы сильно вдавливаете нагретые винты, и они начинают вращаться..."))
				if(tool.use_tool(src, user, 50, volume = 50))
					state = RWINDOW_BOLTS_OUT
					to_chat(user, span_notice("Винты вынимаются, и по краю стекла образуется зазор."))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("Сначала необходимо открутить крепежные винты!"))

		if(RWINDOW_BOLTS_OUT)
			if(tool.tool_behaviour == TOOL_CROWBAR)
				user.visible_message(span_notice("[user] вставляет \the [tool] в щель в раме и начинает отрывать..."),
										span_notice("Вы вставляете \the [tool] в щель в раме и начинаете отрывать..."))
				if(tool.use_tool(src, user, 40, volume = 50))
					state = RWINDOW_POPPED
					to_chat(user, span_notice("Панель выскакивает из рамы, обнажая несколько тонких металлических прутьев, которые, похоже, можно <b>разрезать<b>."))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("Сначала нужно заделать щель!"))

		if(RWINDOW_POPPED)
			if(tool.tool_behaviour == TOOL_WIRECUTTER)
				user.visible_message(span_notice("[user] начинает обрезать открытые прутья на \the [src]..."),
										span_notice("Вы начинаете обрезать открытые прутья на \the [src]"))
				if(tool.use_tool(src, user, 20, volume = 50))
					state = RWINDOW_BARS_CUT
					to_chat(user, span_notice("Панели отваливаются в сторону, обнажая болты рамы."))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("Сначала нужно разрезать прутья!"))

		if(RWINDOW_BARS_CUT)
			if(tool.tool_behaviour == TOOL_WRENCH)
				user.visible_message(span_notice("[user] начинает отсоединять \the [src] от рамы..."),
					span_notice("Вы начинаете откручивать болты от рамы..."))
				if(tool.use_tool(src, user, 40, volume = 50))
					to_chat(user, span_notice("Вы отвинтили болты от рамы, и окно распахивается."))
					state = WINDOW_OUT_OF_FRAME
					set_anchored(FALSE)
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("Сначала необходимо ослабить болты!"))
...
/obj/structure/window/proc/cool_bolts()
	if(state == RWINDOW_BOLTS_HEATED)
		state = RWINDOW_SECURE
		visible_message(span_notice("Болты на \the [src] выглядят так, как будто они остыли..."))
...
/obj/structure/window/reinforced/examine(mob/user)
	...
	switch(state)
		if(RWINDOW_SECURE)
			. += span_notice("Он был завинчен односторонними винтами, вам нужно было бы нагреть их, чтобы иметь хоть какой-то шанс вытащить их обратно.")
		if(RWINDOW_BOLTS_HEATED)
			. += span_notice("Винты раскалились добела, и вы, вероятно, сможете <b>открутить</b> их прямо сейчас.")
		if(RWINDOW_BOLTS_OUT)
			. += span_notice("Винты были вывернуты, в результате чего образовался небольшой зазор, в который можно было бы a <b>вставить инструмент</b> для вскрытия.")
		if(RWINDOW_POPPED)
			. += span_notice("Основная пластина окна выскочила из рамы, обнажив несколько решеток, которые выглядят так, будто их можно <b>вырезать</b>.")
		if(RWINDOW_BARS_CUT)
			. += span_notice("Основную панель можно легко отодвинуть в сторону, чтобы открыть несколько <b>болтов</b>, удерживающих раму.")
...
/obj/structure/window/plasma
	...
	desc = "Окно, сделанное из плазменно-силикатного сплава. Его безумно сложно разбить и прожечь насквозь."
...
/obj/structure/window/reinforced/plasma
	...
	desc = "Окно, изготовленное из плазменно-силикатного сплава и стержневой матрицы. Оно выглядит безнадежно непробиваемым и, скорее всего, почти огнеупорным."
...
/obj/structure/window/reinforced/shuttle//this is called reinforced because it is reinforced w/titanium
	name = "окно шаттла"
	desc = "Армированное окно шаттла с воздушным замком."
...
/obj/structure/window/reinforced/plasma/plastitanium
	...
	desc = "Прочное на вид окно, изготовленное из сплава плазмы и титана."
...
/obj/structure/window/paperframe
	name = "бумажная рамка"
	desc = "Хрупкий разделитель, изготовленный из тонкого дерева и бумаги."
...
/obj/structure/window/paperframe/examine(mob/user)
	. = ..()
	if(atom_integrity < max_integrity)
		. += span_info("Он выглядит немного поврежденным, возможно, вы сможете исправить это с помощью <b>бумаги</b>.")
...
/obj/structure/window/paperframe/attackby(obj/item/W, mob/living/user)
	...
	if(istype(W, /obj/item/paper) && atom_integrity < max_integrity)
		user.visible_message(span_notice("[user] начинает латать дыры в \the [src]."))
		if(do_after(user, 2 SECONDS, target = src))
			atom_integrity = min(atom_integrity+4,max_integrity)
			qdel(W)
			user.visible_message(span_notice("[user] исправляет некоторые отверстия в \the [src]."))
			if(atom_integrity == max_integrity)
				update_appearance()
			return
	..()
...
/obj/structure/window/bronze
	name = "латунное окно"
	desc = "Панель толщиной с лист бумаги из полупрозрачной, но прочной латуни. Не обращайте внимания, это всего лишь хрупкая бронза!"

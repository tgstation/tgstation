...
/obj/structure/grille/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = 0)
	switch(damage_type)
		if(BRUTE)
			if(damage_amount)
				playsound(src, 'sound/effects/grillehit.ogg', 80, DEFAULT_SOUND_VARY)
			else
				playsound(src, 'sound/weapons/tap.ogg', 50, DEFAULT_SOUND_VARY)
		if(BURN)
			playsound(src, 'sound/items/welder.ogg', 80, DEFAULT_SOUND_VARY)
	...
	if(isobj(AM))
		if(prob(50) && anchored && !broken)
			...
			if(O.throwforce != 0)//don't want to let people spam tesla bolts, this way it will break after time
				...
				var/obj/structure/cable/C = T.get_cable_node()
				if(C)
					playsound(src, 'sound/magic/lightningshock.ogg', 100, 1, extra_range =5)

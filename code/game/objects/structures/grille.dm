		...
		if(prob(50) && anchored && !broken)
			...
			if(O.throwforce != 0)//don't want to let people spam tesla bolts, this way it will break after time
				...
				if(C)
					...
					C.add_delayedload(C.newavail() * 0.0375) // you can gain up to 3.5 via the 4x upgrades power is halved by the pole so that's 2x then 1X then .5X for 3.5x the 3 bounces shock.
	return ..()

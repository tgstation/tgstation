...
/obj/structure/grille/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = 0)
	switch(damage_type)
		if(BRUTE)
			if(damage_amount)
				create_sound(src, 'sound/effects/grillehit.ogg').volume(80).vary(TRUE).play()
			else
				create_sound(src, 'sound/items/weapons/tap.ogg').vary(TRUE).play()
		if(BURN)
			create_sound(src, 'sound/items/tools/welder.ogg').volume(80).vary(TRUE).play()
	...
	if(isobj(AM))
		if(prob(50) && anchored && !broken)
			...
			if(O.throwforce != 0)//don't want to let people spam tesla bolts, this way it will break after time
				...
				if(C)
					create_sound(src, 'sound/effects/magic/lightningshock.ogg').volume(100).vary(TRUE).extra_range(5).play()

...
/obj/structure/displaycase/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = 0)
	switch(damage_type)
		if(BRUTE)
			playsound(src, 'sound/effects/glass/glasshit.ogg').volume(75).vary(TRUE).play()
		if(BURN)
...
/obj/structure/displaycase/proc/trigger_alarm()
	...
	playsound(src, 'sound/effects/alert.ogg').vary(TRUE).play()
...
/obj/structure/displaycase/trophy/proc/toggle_historian_mode(mob/user)
	...
	playsound(src, 'sound/machines/beep/twobeep.ogg').vary(TRUE).play()
	...
	switch(action)
		if("Buy")
			...
			if(!account.has_money(sale_price))
				...
				playsound(src, 'sound/effects/cashregister.ogg').volume(40).vary(TRUE).play()
		...
		if("Open")
			...
			if(!check_access(potential_acc))
				playsound(src, 'sound/machines/buzz/buzz-sigh.ogg').vary(TRUE).play()
				return
			toggle_lock()
		if("Register")
			if(payments_acc)

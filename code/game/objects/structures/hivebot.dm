...
/obj/structure/hivebot_beacon/proc/warpbots()
	...
	while(bot_amt > 0)
		...
		switch(bot_type)
			if("norm")
				new /mob/living/animal/hostile/hivebot(get_turf(src))
			if("range")
				new /mob/living/animal/hostile/hivebot/range(get_turf(src))
			if("rapid")
				new /mob/living/animal/hostile/hivebot/rapid(get_turf(src))
	sleep(100)

...
/obj/structure/hivebot_beacon/proc/warpbots()
	...
	while(bot_amt > 0)
		...
		switch(bot_type)
			if("norm")
				new /mob/living/simple_animal/hostile/hivebot(drop_location())
			if("range")
				new /mob/living/simple_animal/hostile/hivebot/range(drop_location())
			if("rapid")
				new /mob/living/simple_animal/hostile/hivebot/rapid(drop_location())
	sleep(100)

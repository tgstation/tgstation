...
/obj/structure/door_assembly/examine(mob/user)
	..()
	if(state == 0 && !anchored)
		to_chat(user, "The metal covers are <i>welded</i> firmly in place.\nThe anchor struts are <i>unscrewed</i>.")
	if(state == 0 && anchored)
		to_chat(user, "The anchor struts are <i>screwed</i> in place.\nThe mainentance panel has small holes for the <i>wiring</i>. Can additionally be named with a pen or have reinforced glass windows added.")
	if(state == 1)

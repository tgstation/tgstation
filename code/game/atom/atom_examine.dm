				...
				if(reagents.total_volume)
					...
					if(user_sees_reagents || (reagent_sigreturn & ALLOW_GENERIC_REAGENT_EXAMINE)) //Show each individual reagent for detailed examination
						for(var/datum/reagent/current_reagent as anything in reagents.reagent_list)
							if(partial_knowledge && current_reagent.knowledge_required & REAGENT_IDENTITY_COMPLEX)
								unidentified_total += round(current_reagent.volume, CHEMICAL_VOLUME_ROUNDING)
								continue //skip reagents we don't have the skill to identify.
							else

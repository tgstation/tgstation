	...
	if(connected && connected.is_operational())
		if(connected.occupant)	//set occupant_status message
			...
			if(check_dna_integrity(viable_occupant) && (!(NOCLONE in viable_occupant.mutations) || (connected.scan_level == 3)))	//occupent is viable for dna modification
				...
				var/rejuvenators = viable_occupant.reagents.get_reagent_amount("ephedrine")
	...
	switch(href_list["task"])
		...
		if("rejuv")
			if(viable_occupant && viable_occupant.reagents)
				var/ephedrine_amount = viable_occupant.reagents.get_reagent_amount("ephedrine")
				var/can_add = max(min(REJUVENATORS_MAX - ephedrine_amount, REJUVENATORS_INJECT), 0)
				viable_occupant.reagents.add_reagent("ephedrine", can_add)
		if("setbufferlabel")

...
/proc/randomize_radiation_accuracy(position_we_were_supposed_to_hit, radduration, number_of_blocks)
	. = Wrap(round(position_we_were_supposed_to_hit + gaussian(radduration*RADIATION_ACCURACY_BIAS, RADIATION_ACCURACY_MULTIPLIER/radduration), 1), 1, number_of_blocks+1)
	world << "[position_we_were_supposed_to_hit] -> [.] ([radduration];[number_of_blocks])"
			...
			if(num && injectorready)
				...
				if(istype(buffer_slot))
					...
					if(href_list["text"] == "se")
						...
						if(buffer_slot["name"] && buffer_slot["UI"] && buffer_slot["UE"])
							I = new /obj/item/weapon/dnainjector(loc)
							I.fields = list("name"=buffer_slot["name"], "UI"=buffer_slot["UI"], "UE"=buffer_slot["UE"])
					if(I)
						injectorready = 0

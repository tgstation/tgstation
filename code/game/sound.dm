/proc/playsound(atom/source, soundin, vol as num, vary, extrarange as num, falloff, frequency = null, channel = 0, pressure_affected = TRUE, quiet = FALSE)
	if(isarea(source))
	...
	if(quiet) //quiet sounds don't carry through walls

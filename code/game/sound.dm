...
/client/proc/playtitlemusic()
	...
	if(!ticker.login_music)
		log_game("DEBUG: there are no music");return;
	if(prefs && (prefs.toggles & SOUND_LOBBY))
		log_game("DEBUG: all fine something should be playing now");src << sound(ticker.login_music, repeat = 0, wait = 0, volume = 85, channel = 1); // MAD JAMS

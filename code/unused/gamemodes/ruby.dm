...
/client/proc/defile_corpse(var/mob/living/carbon/human/H in view())
	...
	if(istype(H, /mob/living/carbon/human))
		...
		rmode.killed.Add(H)
		ticker.mode:respawns += 1
	var/fluffmessage = pick("<span class='danger'><B>[usr] rips the flesh from [H]'s corpse and plucks their eyes from their sockets!</B>", "<span class='userdanger'><B>[usr] does unspeakable things to [H]'s corpse!</B>", "\red [usr] binds [H]'s corpse with their own entrails!</span></span>")
...
/client/proc/hunt()
	...
	usr.visible_message(text("<span class='userdanger'>[usr]'s flesh ripples and parts, revealing dozens of eyes poking from its surface. They all glance wildly around for a few moments before receding again.</span>"))
...
/client/proc/howl()	// This is just a way for the Abomination to make the game more atmospheric periodically.
	...
	usr.visible_message(text("<span class='userdanger'>[usr]'s form warbles and distorts before settling back into its grotesque shape once more.</span>"))

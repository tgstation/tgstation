...
/mob/living/silicon/hivebot/meteorhit(obj/O as obj)
	for(var/mob/M in viewers(src, null))
		M.show_message(text("<span class='danger'>[src] has been hit by [O]</span>"), 1)
		//Foreach goto(19)
	if (src.health > 0)
		...
		if(ismob(AM))
			...
			if(istype(tmob, /mob/living/carbon/human) && tmob.mutations & FAT)
				if(prob(20))
					for(var/mob/M in viewers(src, null))
						if(M.client)
							M << M << "<span class='userdanger'>[src] fails to push [tmob]'s fat ass out of the way.</span>"
					src.now_pushing = 0
		...
		*/
		...
		if(!(src.lying))
			...
			if (randn <= 40)
				...
				for(var/mob/O in viewers(src, null))
					O.show_message(text("<span class='userdanger'>[] has pushed back []!</span>", M, src), 1)
			else
				playsound(src.loc, 'sound/weapons/slashmiss.ogg', 25, 1, -1)
				for(var/mob/O in viewers(src, null))
					O.show_message(text("<span class='userdanger'>[] attempted to push back []!</span>", M, src), 1)
	return
						...
						if (prob(75))
							...
							if (istype(G, /obj/item/weapon/grab))
								for(var/mob/O in viewers(M, null))
									O.show_message(text("<span class='danger'>[G.affecting] has been pulled from [G.assailant]'s grip by [src]</span>"), 1)
								del(G)
						else
							ok = 0
...
/mob/living/silicon/hivebot/proc/return_mainframe()
	if(mainframe)
		...
		src << "<span class='danger'>You lack a dedicated mainframe!</span>"
		return
	...
	switch(state)
		...
		if(1)
			...
			if(istype(P, /obj/item/weapon/motherboard) && !mainboard)
				...
				user.drop_item()
				P.loc = src
			if(istype(P, /obj/item/tool/screwdriver) && mainboard)
				playsound(src.loc, 'sound/items/Screwdriver.ogg', 50, 1)
				user << "\blue You screw the mainboard into place."
				src.state = 2
				src.icon_state = "2"
			if(istype(P, /obj/item/tool/crowbar) && mainboard)
				...
				src.mainboard = null
		if(2)
			if(istype(P, /obj/item/tool/screwdriver) && mainboard && (!peripherals.len))
				playsound(src.loc, 'sound/items/Screwdriver.ogg', 50, 1)
			...
			if(istype(P, /obj/item/tool/crowbar) && src.peripherals.len)
				playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)
		...
		if(3)
			...
			if(istype(P, /obj/item/office/disk/data/fixed_disk) && !src.hd)
				user.drop_item()
				src.hd = P
				P.loc = src
				user << "\blue You connect the drive to the cabling."

			if(istype(P, /obj/item/tool/crowbar) && src.hd)
				playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)

...
/obj/machinery/blender
	...
	New()
		...
		reagents = R
		R.my_atom = src
		src.contents += new /obj/item/chem/glass/blender_jug(src)
		src.container = "/obj/item/chem/glass/blender_jug"		//Loads a jug into the blender.
...
/obj/machinery/blender/attackby(var/obj/item/O as obj, var/mob/user as mob)		//Attack it with an object.
	if(src.contents.len >= 10 || src.reagents.total_volume >= 80)		//Too full. Max 10 items or 80 units of reagent
	...
	else if(istype(O, /obj/item/chem/glass/blender_jug) && src.container == 0) //Load jug.
		...
		if(istype(O, /obj/item/chem/food/snacks))	//Will only blend food items. Add others in this else clause.
			user.drop_item()
			O.loc = src
			...
			for (var/obj/item/chem/food/snacks/grown/G in O.contents)
				O.contents -= G
				G.loc = src
...
/obj/machinery/blender/verb/blend()		//Blend shit. Note: In the actual blending loop, make sure it can't include the jug.
	...
	for(var/obj/O in src.contents)
		if(istype(O, /obj/item/chem/food/snacks/grown/soybeans))	 //  Mass balance law
			src.reagents.add_reagent("soymilk", O.reagents.get_reagent_amount("nutriment"))
			O.reagents.del_reagent("nutriment")
		else if(istype(O, /obj/item/chem/food/snacks/grown/tomato)) //  Mass balance law
			src.reagents.add_reagent("ketchup", O.reagents.get_reagent_amount("nutriment"))
			O.reagents.del_reagent("nutriment")
		else if(istype(O, /obj/item/chem/food/snacks/grown/corn))   //  Mass balance law
			src.reagents.add_reagent("cornoil", O.reagents.get_reagent_amount("nutriment"))
			O.reagents.del_reagent("nutriment")
		if(istype(O, /obj/item/chem/food/snacks))	//This is intentionally not an "else if"
...
/obj/machinery/blender/verb/detach()		//Transfers the contents of the Blender to the Blender Jug and then ejects the jug.
	...
	if(src.processing)
		...
		for(var/obj/O in src.contents)			//Searches through the contents for the jug.
			if(istype(O, /obj/item/chem/glass/blender_jug))
				O.loc = get_turf(src)
...
/obj/machinery/blender/verb/eject()			//Ejects the non-reagent contents of the blender besides the jug.
	...
	if(src.processing)
		...
		for(var/obj/O in src.contents)
			if(istype(O, /obj/item/chem/food/snacks))
				O.loc = get_turf(src)
				O = null
	return

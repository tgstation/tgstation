 ...
 * * tts_voice - TTS voice to play. When set to null, no TTS is played.
 * * tts_filter - TTS filter to apply on the voice.
 */
/proc/priority_announce(text, title = "", sound, type, sender_override, has_important_message = FALSE, list/mob/players, encode_title = TRUE, encode_text = TRUE, tts_voice = null, tts_filter = null)
	if(!text)
		return
	...
	if(tts_voice)
		...
		addtimer(CALLBACK(SStts, TYPE_PROC_REF(/datum/controller/subsystem/tts, queue_tts_message), decoded_text, tts_voice, clients_to_send, tts_filter), 3 SECONDS)

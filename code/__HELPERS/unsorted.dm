...
/obj/proc/atmosanalyzer_scan(datum/gas_mixture/air_contents, mob/user, obj/target = src)
	var/obj/icon = target
	user.visible_message("[user] has used the analyzer on [bIcon(icon, viewers(src))] [target].", "<span class='notice'>You use the analyzer on [bIcon(icon, user)] [target].</span>")
	var/pressure = air_contents.return_pressure()
	var/total_moles = air_contents.total_moles()

	to_chat(user, "<span class='notice'>Results of analysis of [bIcon(icon, user)] [target].</span>")
	if(total_moles>0)

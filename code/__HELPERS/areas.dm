...
/**
 * Calculates edge turfs for area lighting overlays
 * Returns an associative list with the corrolating bitflag and appearance.
 *
 * Very fucking expensive
 */
/proc/calculate_day_night_turf_adjacency(list/areas)
	var/list/turf_list = list()
	for(var/area/iterating_area as anything in areas)
		for(var/turf/iterating_turf in iterating_area.contents)
			turf_list += iterating_turf
	var/list/adjacent_turfs = list()
	for(var/turf/iterated_turf in turf_list)
		var/bitfield = NONE
		...
		if(!bitfield)
			continue
		adjacent_turfs[iterated_turf] = list(AREA_DAY_NIGHT_INDEX_BITFIELD, AREA_DAY_NIGHT_INDEX_APPEARANCE)
		adjacent_turfs[iterated_turf][AREA_DAY_NIGHT_INDEX_BITFIELD] = bitfield

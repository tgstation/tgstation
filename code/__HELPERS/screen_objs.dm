...
/proc/screen_loc_to_offset(screen_loc, view)
	...
	x += x_coord * ICONSIZE_X
	y += y_coord * ICONSIZE_Y
...
/proc/offset_to_screen_loc(x_offset, y_offset, view = null)
	if(view)
		...
		x_offset = clamp(x_offset, ICONSIZE_X, view_bounds[1])
		y_offset = clamp(y_offset, ICONSIZE_Y, view_bounds[2])

	// Round with no argument is floor, so we get the non pixel offset here
	var/x = round(x_offset / ICONSIZE_X)
	var/pixel_x = x_offset % ICONSIZE_X
	var/y = round(y_offset / ICONSIZE_Y)
	var/pixel_y = y_offset % ICONSIZE_Y
...
/proc/get_valid_screen_location(target_loc, target_offset, view)
	...
	if(base_y - target_offset > ICONSIZE_Y)
		return offset_to_screen_loc(base_x, base_y - target_offset, view)
	if(base_x - target_offset > ICONSIZE_X)
		return offset_to_screen_loc(base_x - target_offset, base_y, view)
...
/proc/spanning_screen_loc(start_px, start_py, end_px, end_py)
	var/starting_tile_x = round(start_px / ICONSIZE_X)
	start_px -= starting_tile_x * ICONSIZE_X
	var/starting_tile_y = round(start_py/ ICONSIZE_Y)
	start_py -= starting_tile_y * ICONSIZE_Y
	var/ending_tile_x = round(end_px / ICONSIZE_X)
	end_px -= ending_tile_x * ICONSIZE_X
	var/ending_tile_y = round(end_py / ICONSIZE_Y)
	end_py -= ending_tile_y * ICONSIZE_Y

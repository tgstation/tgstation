...
/proc/add_logs(mob/user, mob/target, what_done, object=null, addition=null)
	...
	if(user && ismob(user))
		user.attack_log += text("\[[time_stamp()]\] <font color='blue'>Has [what_done] [target ? "[target.name][(ismob(target) && target.ckey) ? "([target.ckey])" : ""]" : "NON-EXISTANT SUBJECT"][object ? " with [object]" : " "][addition][newhealthtxt][coordinates]</font>")
		if(user.mind)
			user.mind.attack_log += text("\[[time_stamp()]\] <font color='blue'>[user ? "[user.name][(ismob(user) && user.ckey) ? "([user.ckey])" : ""]" : "NON-EXISTANT SUBJECT"] has [what_done] [target ? "[target.name][(ismob(target) && target.ckey) ? "([target.ckey])" : ""]" : "NON-EXISTANT SUBJECT"][object ? " with [object]" : " "][addition][newhealthtxt][coordinates]</font>")

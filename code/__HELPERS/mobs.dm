...
/proc/do_mob(mob/user, mob/target, time = 3 SECONDS, timed_action_flags = NONE, progress = TRUE, datum/callback/extra_checks, interaction_key, max_interact_count = 1, bonus_time = 0, focus_sound = null)
	...
	if (progress)
		progbar = new(user, time, target, bonus_time, focus_sound)
 ...
 */
/proc/do_after(mob/user, time = 3 SECONDS, atom/target, timed_action_flags = NONE, progress = TRUE, datum/callback/extra_checks, interaction_key, max_interact_count = 1, bonus_time = 0, focus_sound = null)
	if(!user)
		return FALSE
	...
	var/datum/progressbar/progbar
	if(progress)
		progbar = new(user, time, target || user, bonus_time, focus_sound)
...
/proc/do_after_mob(mob/user, list/targets, time = 3 SECONDS, timed_action_flags = NONE, progress = TRUE, datum/callback/extra_checks, interaction_key, max_interact_count = 1, bonus_time = 0, focus_sound = null)
	if(!user)
		return FALSE
	if(!islist(targets))
	...
	var/datum/progressbar/progbar
	if(progress)
		progbar = new(user, time, targets[1], bonus_time, focus_sound)

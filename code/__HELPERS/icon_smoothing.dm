...
/*
	...
	to add the 'SMOOTH_DIAGONAL' flag to the atom's smooth_flags var (in addition to either SMOOTH_TRUE or SMOOTH_MORE).
...
#define SMOOTH_DIAGONAL	(1<<2)	//if atom should smooth diagonally, this should be present in 'smooth_flags' var
...
#define QUEUE_SMOOTH(thing_to_queue) if(thing_to_queue.smooth_flags) {SSicon_smooth.add_to_queue(thing_to_queue)}
...
/proc/calculate_adjacencies(atom/A)
	...
	for(var/direction in GLOB.cardinals)
		...
		if(AM == NULLTURF_BORDER)
			if((A.smooth_flags & SMOOTH_BORDER))
...
/proc/smooth_icon(atom/A)
	if(!A || !A.smooth_flags)
		return
	A.smooth_flags &= ~SMOOTH_QUEUED
	if (!A.z)
		return
	if(QDELETED(A))
		return
	if(A.smooth_flags & (SMOOTH_TRUE | SMOOTH_MORE))
		var/adjacencies = calculate_adjacencies(A)

		if(A.smooth_flags & SMOOTH_DIAGONAL)
...
/proc/find_type_in_direction(atom/source, direction)
	...
	if(source.canSmoothWith)
		var/atom/A
		if(source.smooth_flags & SMOOTH_MORE)
...
/turf/closed/wall/smooth
	...
	icon_state = "smooth"
	smooth_flags = SMOOTH_TRUE|SMOOTH_DIAGONAL|SMOOTH_BORDER
	canSmoothWith = null

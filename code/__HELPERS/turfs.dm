...
/proc/get_visual_offset(atom/checked_atom)
	...
	if(checked_atom_icon_height != ICONSIZE_Y || checked_atom_icon_width != ICONSIZE_X)
		pixel_x_offset += ((checked_atom_icon_width / ICONSIZE_X) - 1) * (ICONSIZE_X * 0.5)
		pixel_y_offset += ((checked_atom_icon_height / ICONSIZE_Y) - 1) * (ICONSIZE_Y * 0.5)
...
/proc/pixel_offset_turf(turf/offset_from, list/offsets)
	//DY and DX
	var/rough_x = round(round(offsets[1], ICONSIZE_X) / ICONSIZE_X)
	var/rough_y = round(round(offsets[2], ICONSIZE_Y) / ICONSIZE_Y)
...
/proc/parse_caught_click_modifiers(list/modifiers, turf/origin, client/viewing_client)
	...
	LAZYSET(modifiers, ICON_X, "[(click_turf_px - click_turf.pixel_x) + ((click_turf_x - click_turf.x) * ICONSIZE_X)]")
	LAZYSET(modifiers, ICON_Y, "[(click_turf_py - click_turf.pixel_y) + ((click_turf_y - click_turf.y) * ICONSIZE_Y)]")
	return click_turf

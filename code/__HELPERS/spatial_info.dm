...
/proc/get_hearers_in_view(view_radius, atom/source)
	...
	if(!length(hearables_from_grid))//we know that something is returned by the grid, but we don't know if we need to actually filter down the output
		return .
	...
	//while every mob can hear. for this case view() and similar procs have an optimization to only look through 1 of these lists if it can (eg you're only looking for mobs)
	...
	//using hearers is a further optimization of that because for our purposes its the same as view except we don't have to set center's luminosity to 6 and then unset it

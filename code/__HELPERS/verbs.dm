...
/**
  ...
  * * verb - typepath to a verb, or a list of verbs, supports lists of lists
  */
/proc/remove_verb(target, verb_or_list_to_remove)
	if(!target)
	...
	for(var/verb_to_remove in verbs_list)
		target:verbs -= verb_to_remove // colon here so we don't have to check between client or mob, both MUST have a verbs var
		output_list[++output_list.len] = list("[verb_to_remove:category]", "[verb_to_remove:name]")
	output_list = url_encode(json_encode(output_list))
	if(M)
		M.client << output("[output_list];", "statbrowser:remove_verb_list")
	else

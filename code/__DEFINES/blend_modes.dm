...
// The suggestions here are from the ref, and so are NOT ALWAYS ACCURATE TO SS13
...
// This is typically only useful f.. Applying a colored light effect; f.. Simply darkening, using a translucent black icon with normal overlay blending is a better option.
// #define BLEND_MULTIPLY 4

// BLEND_INSET_OVERLAY overlays the icon, but masks it by the image being drawn on.
// This is pretty much not at all useful directly on the map.. Can be very useful f.. An overlay f.. An atom that uses KEEP_TOGETHER (see appearance_flags).. F.. The layering filter.

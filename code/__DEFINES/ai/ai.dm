...
///same macro as above, but accounting for flags we should ignore when checking incapacitated
#define IS_DEAD_OR_INCAP_IGNORING(source, incap_flags) (INCAPACITATED_IGNORING(source, incap_flags) || source.stat)

...
#define DOING_INTERACTION(user, source) (LAZYACCESS(user.do_afters, source))
#define DOING_INTERACTION_LIMIT(user, source, max_interaction_count) ((LAZYACCESS(user.do_afters, source) || 0) >= max_interaction_count)

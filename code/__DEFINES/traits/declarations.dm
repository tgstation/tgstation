...
// These traits are used in IS_X() as an OR, and is utilized for pseudoantags (such as deathmatch or domains) so they don't need to actually recieve antag status.
// To specifically and only recieve the antag datum, GET_X() exists now.

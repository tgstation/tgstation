/// This is a flat constant for our beauty equation which is:
/// beauty = material.beauty_modifier * BEAUTY_PER_MATERIAL * material_amount
#define BEAUTY_PER_MATERIAL 10

/// art quality defines, used in datums/components/art.dm, elsewhere
#define BAD_ART 12.5
#define OK_ART 20
#define GOOD_ART 25
#define GREAT_ART 50

/// tgui tab portrait categories- they're the same across all portrait tguis.
#define TAB_LIBRARY 1
#define TAB_SECURE 2
#define TAB_PRIVATE 3

/// cost defines for drawing graffiti: how many charges of a crayon or spraycan are used.
#define CRAYON_COST_SMALL 0.5
#define CRAYON_COST_DEFAULT 1
#define CRAYON_COST_LARGE 5

/// how long it takes to make a statue from a block of material
#define SCULPT_TIME_FAST 15 SECONDS
#define SCULPT_TIME_AVERAGE 30 SECONDS
#define SCULPT_TIME_SLOW 45 SECONDS
#define SCULPT_TIME_VERY_SLOW 60 SECONDS

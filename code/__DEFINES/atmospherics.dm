...
#define VENTCRAWL_ALLOWED					(1<<0) //Allows for ventcrawling to occur. All atmospheric machines have this flag on by default. Cryo is the exception
#define VENTCRAWL_ENTRANCE_ALLOWED			(1<<1) //Allows mobs to enter or leave from atmospheric machines. On for passive, unary, and scrubber vents.
#define VENTCRAWL_CAN_SEE					(1<<2) //Used to check if a machinery is visible. Called by update_pipe_vision(). On by default for all except cryo.

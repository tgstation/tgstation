///Action button is not available while another action is in use
#define AB_TEMPORARY_DISABLE (1<<0)
///Action button checks if hands are unusable
#define AB_CHECK_HANDS_BLOCKED (1<<1)
///Action button checks if user is immobile
#define AB_CHECK_IMMOBILE (1<<2)
///Action button checks if user is resting
#define AB_CHECK_LYING (1<<3)
///Action button checks if user is conscious
#define AB_CHECK_CONSCIOUS (1<<4)
///Action button checks if user is incapacitated
#define AB_CHECK_INCAPACITATED (1<<5)

...
/// Called when a module tries to activate, but it does... At the end of checks so you can add some yourself, or work on trigger behavior (mob/user)
...
/// Called on the module when the user tries to install it into a MOD control unit (obj/item/mod/control/suit, mob/user)
#define COMSIG_MODULE_TRY_INSTALL "mod_module_try_install"
/// Called on the MOD control unit when the user tries to install a module (obj/item/mod/module/module, mob/user)

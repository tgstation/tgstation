...
#define CLAMP01(x) (CLAMP(x, ZERO, 1))
...
#define SIGN(x) ( (x)!=ZERO ? (x) / abs(x) : ZERO )
...
#define ATAN2(x, y) ( !(x) && !(y) ? ZERO : (y) >= ZERO ? arccos((x) / sqrt((x)*(x) + (y)*(y))) : -arccos((x) / sqrt((x)*(x) + (y)*(y))) )
...
#define ISEVEN(x) (x % 2 == ZERO)

#define ISODD(x) (x % 2 != ZERO)
...
#define ISMULTIPLE(x, y) ((x) % (y) == ZERO)

// Performs a linear interpolation between a and b.
// Note that amount=ZERO returns a, amount=1 returns b, and
...
//A logarithm that converts an integer to a number scaled between ZERO and 1.
...
/proc/get_turf_in_angle(angle, turf/starting, increments)
	...
	while(pixel_y < -16)
		pixel_y += 32
		new_y--
	new_x = CLAMP(new_x, ZERO, world.maxx)
	new_y = CLAMP(new_y, ZERO, world.maxy)

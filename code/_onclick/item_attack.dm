	...
	switch(hit_zone)
		if(BODY_ZONE_HEAD)
			...
			if(!attacking_item.get_sharpness() && !HAS_TRAIT(src, TRAIT_HEAD_INJURY_BLOCKED) && attacking_item.damtype == BRUTE)
				...
				if(mind && !IS_UNCONSCIOUS_CRIT_OR_DEAD(src) && src != attacker && prob(damage_done + ((100 - health) * 0.5)))
		...
		if(BODY_ZONE_CHEST)
			...
			if(!IS_UNCONSCIOUS_CRIT_OR_DEAD(src) && !attacking_item.get_sharpness() && !HAS_TRAIT(src, TRAIT_BRAWLING_KNOCKDOWN_BLOCKED) && attacking_item.damtype == BRUTE)
				if(prob(damage_done))
					visible_message(

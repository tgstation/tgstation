	...
	switch(relative_to.location)
		...
		if(SCRN_OBJ_FLOATING) // If we don't have it as a define, this is a screen_loc, and we should be floating
			...
			if(!our_client)
				position_action(button, button.linked_action.default_button_position)
				return
			button.screen_loc = get_valid_screen_location(relative_to.screen_loc, ICONSIZE_ALL, our_client.view_size.getView()) // Asks for a location adjacent to our button that won't overflow the map
...
/datum/action_group/proc/check_against_view()
	...
	if(offsets[1] > ICONSIZE_X && offsets[1] < view_size[1] && offsets[2] > ICONSIZE_Y && offsets[2] < view_size[2]) // We're all good
		return
	...
	for(column_max in column_max - 1 to 1 step -1) // Yes I could do this by unwrapping ButtonNumberToScreenCoords, but I don't feel like it
		...
		if(offsets[1] > ICONSIZE_X && offsets[1] < view_size[1] && offsets[2] > ICONSIZE_Y && offsets[2] < view_size[2])
			break

...
/datum/hud/new_player/New(mob/owner)
	...
	if (owner?.client?.interviewee)
		return
	var/list/buttons = subtypesof(/obj/screen/lobby)
	for(var/button in buttons)
		var/obj/screen/lobbyscreen = new button()
		lobbyscreen.hud = src
...
/obj/screen/lobby/background
	layer = LOBBY_BACKGROUND_LAYER
	icon = 'icons/hud/lobby/background.dmi'
	icon_state = "background"
	screen_loc = "TOP,CENTER:-61"

/obj/screen/lobby/button
	///Is the button currently enabled?
	var/enabled = TRUE
	///Is the button currently being hovered over with the mouse?
	var/highlighted = FALSE

/obj/screen/lobby/button/Click(location, control, params)
	. = ..()
	if(!enabled)
		return
...
/obj/screen/lobby/button/MouseEntered(location,control,params)
	. = ..()
	highlighted = TRUE
	update_appearance(UPDATE_ICON)

/obj/screen/lobby/button/MouseExited()
	. = ..()
	highlighted = FALSE
	update_appearance(UPDATE_ICON)

/obj/screen/lobby/button/update_icon(updates)
	. = ..()
	if(!enabled)
...
/obj/screen/lobby/button/proc/set_button_status(status)
	if(status == enabled)
		return FALSE
	enabled = status
	update_appearance(UPDATE_ICON)
...
///Prefs menu
/obj/screen/lobby/button/character_setup
...
/obj/screen/lobby/button/character_setup/Click(location, control, params)
	. = ..()
	if(!.)
		return
...
/obj/screen/lobby/button/ready/Initialize(mapload)
	. = ..()
...
/obj/screen/lobby/button/ready/proc/hide_ready_button()
	SIGNAL_HANDLER
	set_button_status(FALSE)
	UnregisterSignal(SSticker, COMSIG_TICKER_ENTER_SETTING_UP)

/obj/screen/lobby/button/ready/Click(location, control, params)
	. = ..()
	if(!.)
		return
...
/obj/screen/lobby/button/join/Initialize(mapload)
	. = ..()
	if(SSticker.current_state > GAME_STATE_PREGAME)
...
/obj/screen/lobby/button/join/Click(location, control, params)
	. = ..()
	if(!.)
		return
...
/obj/screen/lobby/button/join/proc/show_join_button(status)
	SIGNAL_HANDLER
	set_button_status(TRUE)
	UnregisterSignal(SSticker, COMSIG_TICKER_ENTER_SETTING_UP)

/obj/screen/lobby/button/observe
...
/obj/screen/lobby/button/observe/Initialize(mapload)
	. = ..()
...
/obj/screen/lobby/button/observe/Click(location, control, params)
	. = ..()
	if(!.)
		return
	var/mob/dead/new_player/new_player = hud.mymob
...
/obj/screen/lobby/button/changelog_button
	icon = 'icons/hud/lobby/bottom_buttons.dmi'
...
/obj/screen/lobby/button/crew_manifest
	icon = 'icons/hud/lobby/bottom_buttons.dmi'
	icon_state = "crew_manifest"
	base_icon_state = "crew_manifest"
	screen_loc = "TOP:-122,CENTER:+2"

/obj/screen/lobby/button/crew_manifest/Click(location, control, params)
	. = ..()
	if(!.)
		return
	var/mob/dead/new_player/new_player = hud.mymob
...
/obj/screen/lobby/button/changelog_button/Click(location, control, params)
	. = ..()
	usr.client?.changelog()

/obj/screen/lobby/button/poll
	icon = 'icons/hud/lobby/bottom_buttons.dmi'
	icon_state = "poll"
...
/obj/screen/lobby/button/poll/New(loc, ...)
	. = ..()
	if(!usr) //
		return
...
/obj/screen/lobby/button/poll/update_overlays()
	. = ..()
	if(new_poll)
		. += mutable_appearance('icons/hud/lobby/poll_overlay.dmi', "new_poll")

/obj/screen/lobby/button/poll/Click(location, control, params)
	. = ..()
	if(!.)
		return
	var/mob/dead/new_player/new_player = hud.mymob

...
// Master plates relay their render "up" to the openspace level above them
/atom/movable/screen/plane_master/rendering_plate/master/update_offset()
	...
	if(offset)
		...
		render_relay_plane = GET_NEW_PLANE(OPENSPACE_PLANE, offset - 1)
	else
		generate_render_target = FALSE
		render_relay_plane = null
...
/atom/movable/screen/plane_master/proc/relay_render_to_plane(mob/mymob, relay_plane)
	...
	// If we've already got a relay
	if(relay in our_client.screen)
		if(relay.plane == relay_plane)
			return
		// and it's not the same, we're gonna attempt to override it
		blend_to_use = blend_mode_override || relay.blend_mode
		mymob.client.screen -= relay

	if(!initial(render_target) && generate_render_target)
		render_target = "*[name]: AUTOGENERATED RENDER TGT #[offset]"
	relay = new()

...
/atom/movable/screen/movable/pic_in_pic/proc/add_buttons()
	...
	var/matrix/M = matrix()
	M.Translate(0, (height + 0.25) * ICONSIZE_Y)
	...
	M = matrix()
	M.Translate((max(4, width) - 0.75) * ICONSIZE_X, (height + 0.25) * ICONSIZE_Y)
	...
	M = matrix()
	M.Translate(ICONSIZE_X, (height + 0.25) * ICONSIZE_Y)
	...
	M = matrix()
	M.Translate(2 * ICONSIZE_X, (height + 0.25) * ICONSIZE_Y)
...
/atom/movable/screen/movable/pic_in_pic/proc/set_view_size(width, height, do_refresh = TRUE)
	...
	y_off = (-height * ICONSIZE_Y) - (ICONSIZE_Y / 2)

...
/obj/item/pod_parts/pod_frame/New()
	..()
	desc = ini_desc
...
/obj/item/pod_parts/pod_frame/proc/find_square()
	...
	for(var/i = 1; i <= 4; i++)
		...
		if(istype(pointer, linked.link_to) && pointer.dir == linked.dir && pointer.wrenched)
...
/obj/item/pod_parts/pod_frame/attackby(var/obj/O)
	if(istype(O, /obj/item/stack/rods))
		...
		R.use(10)
		for(var/i = 1; i <=4; i++)
			var/obj/item/pod_parts/pod_frame/F = linkedparts[i]
	...
	if(istype(O, /obj/item/weapon/wrench))
		usr << "\blue You [!wrenched ? "secure \the [src] in place."  : "remove the securing bolts."]"
		src.wrenched = !src.wrenched
		src.density = src.wrenched
		src.anchored = src.wrenched
		if(src.wrenched)
			src.desc = src.ini_desc + " It looks bolted down."
		else
			src.desc = src.ini_desc + " It looks free to move around."
		playsound(get_turf(src), 'sound/items/Ratchet.ogg', 50, 1)
...
/obj/item/pod_parts/pod_frame/fore_port
	name = "space pod fore port component"
	icon_state = "pod_fp"
	ini_desc = "A space pod frame component. This is the fore port component."
	link_to = /obj/item/pod_parts/pod_frame/fore_starboard
	link_angle = 90

/obj/item/pod_parts/pod_frame/fore_starboard
	name = "space pod fore starboard component"
	icon_state = "pod_fs"
	ini_desc = "A space pod frame component. This is the fore starboard component."
	link_to = /obj/item/pod_parts/pod_frame/aft_starboard
	link_angle = 180

/obj/item/pod_parts/pod_frame/aft_port
	name = "space pod aft port component"
	icon_state = "pod_ap"
	ini_desc = "A space pod frame component. This is the aft port component."
	link_to = /obj/item/pod_parts/pod_frame/fore_port
	link_angle = 0

/obj/item/pod_parts/pod_frame/aft_starboard
	name = "space pod aft starboard component"
	icon_state = "pod_as"
	ini_desc = "A space pod frame component. This is the aft starboard component."

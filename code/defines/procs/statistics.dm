proc/sql_poll_players()
	...
	if(!dbcon.IsConnected())
		...
		var/DBQuery/query = dbcon.NewQuery("INSERT INTO ss13db_legacy_population (playercount, time) VALUES ([playercount], '[sqltime]')")
		if(!query.Execute())
...
proc/sql_poll_admins()
	...
	if(!dbcon.IsConnected())
		...
		var/sqltime = time2text(world.realtime, "YYYY-MM-DD hh:mm:ss")
		var/DBQuery/query = dbcon.NewQuery("INSERT INTO ss13db_legacy_population (admincount, time) VALUES ([admincount], '[sqltime]')")
		if(!query.Execute())
			var/err = query.ErrorMsg()
...
proc/sql_report_death(var/mob/living/carbon/human/H)
	...
	if(!dbcon.IsConnected())
		...
		var/DBQuery/query = dbcon.NewQuery("INSERT INTO ss13db_death (name, byondkey, job, special, pod, tod, laname, lakey, gender, bruteloss, fireloss, brainloss, oxyloss, coord) VALUES ('[sqlname]', '[sqlkey]', '[sqljob]', '[sqlspecial]', '[sqlpod]', '[sqltime]', '[laname]', '[lakey]', '[H.gender]', [H.getBruteLoss()], [H.getFireLoss()], [H.brainloss], [H.getOxyLoss()], '[coord]')")
		if(!query.Execute())
			var/err = query.ErrorMsg()
...
proc/sql_commit_feedback()
	...
	if(!dbcon.IsConnected())
		...
		var/DBQuery/max_query = dbcon.NewQuery("SELECT MAX(roundid) AS max_round_id FROM ss13db_feedback")
		max_query.Execute()
		...
		for(var/datum/feedback_variable/item in content)
			...
			var/DBQuery/query = dbcon.NewQuery("INSERT INTO ss13db_feedback (id, roundid, time, variable, value) VALUES (null, [newroundid], Now(), '[variable]', '[value]')")
			if(!query.Execute())
				var/err = query.ErrorMsg()

...
/datum/configuration
	...
	var/allow_admin_ooccolour = 0		// Allows admins with relevant permissions to have their own ooc colour
	...
	for(var/t in Lines)
		...
		if(type == "config")
			switch(name)
				...
				if("log_hrefs")
					config.log_hrefs = 1
				if("allow_admin_ooccolour")
					config.allow_admin_ooccolour = 1

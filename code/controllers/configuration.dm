...
/datum/configuration
	...
	var/notify_new_player_byond_age = 0		// how long do we notify admins of a new byond account
	...
	for(var/t in Lines)
		...
		if(type == "config")
			switch(name)
				...
				if("notify_new_player_byond_age")
					notify_new_player_byond_age = text2num(value)

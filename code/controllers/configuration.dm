...
/datum/configuration
	...
	var/shuttle_life_check = 0.85		// A ratio of how many people need to be alive in order for the shuttle to not justify that as a reason for coming.
	var/shuttle_antag_overrun = 0.3		// A ratio of dirty traitors to all living crew. If this is achieved the shuttle can come (most likely in revolution/gang/quick moving low pop cults).
	var/shuttle_infrastructure_check = 0.85 // A ratio of "how blown up / singulo'd / tesla'd the station is", again to gauge shuttle calls.
		...
		if(type == "config")
			...
			switch(name)
				...
				if("shuttle_boredom_check")
					shuttle_boredom_check	= text2num(value)
				if("shuttle_life_check")
					shuttle_life_check 		= text2num(value)
				if("shuttle_antag_overrun")
					shuttle_antag_overrun 	= text2num(value)
				if("shuttle_infrastructure_check")
					shuttle_infrastructure_check = text2num(value)
				if("min_pop")

...
/datum/light_source/proc/readrgb(const/col)
	/*if(!configuration.colour_lights)
		col_r = 255
		col_g = 255
		col_g = 255 */
//	else
...
turf/proc/shift_to_subarea()
	...
	var/level = Clamp(round(lighting_lumcount, 1), 0, SSlighting.lighting_states)
	...
	if (l_color)
		...
		new_tag += "[l_color][Clamp(round(color_lighting_lumcount, 1), 0, SSlighting.lighting_states)]"

	if(Area.tag != new_tag)	//skip if already in this area
		var/area/A = locate(new_tag)	// find an appropriate area
		var/color_light = Clamp(round(color_lighting_lumcount, 1), 0, SSlighting.lighting_states)
...
/area/proc/SetLightLevel(light, color_light = 0)
	...
	if(light <= 0)
		...
		if(light > SSlighting.lighting_states)
			light = SSlighting.lighting_states
		luminosity = 1

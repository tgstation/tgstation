...
/// Use this to init a sprite accessory list for a feature where mobs are needed to have one selected
...
/// A sprite accessory is something that we add to a human sprite to make them look different.. This is hair, facial hair, underwear, mutant bits, and so on
...
/// Sets up all of the lists for later use in the round and building sprites.
...
/// The boolean `add_blank` argument just adds a "None" option to the list of sprite accessories, like if a felinid doesn't want a tail. Something, typically good f. Gated-off things.

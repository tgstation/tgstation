...
/proc/prepare_explosion_turfs(range, turf/epicenter)
	...
	for(var/i in 1 to range)
		...
		if(highest_y <= max_y)
			outlist += BLOCK_COORDS(max(lowest_x, 1), highest_y, our_z, min(highest_x - 1, max_x), highest_y, our_z)
		// top right to one before bottom right
		if(highest_x <= max_x)
			outlist += BLOCK_COORDS(highest_x, min(highest_y, max_y), our_z, highest_x, max(lowest_y + 1, 1), our_z)
		// bottom right to one before bottom left
		if(lowest_y >= 1)
			outlist += BLOCK_COORDS(min(highest_x, max_x), lowest_y, our_z, max(lowest_x + 1, 1), lowest_y, our_z)
		// bottom left to one before top left
		if(lowest_x >= 1)
			outlist += BLOCK_COORDS(lowest_x, max(lowest_y, 1), our_z, lowest_x, min(highest_y - 1, max_y), our_z)

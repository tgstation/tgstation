...
/datum/controller/subsystem/explosions/proc/propagate_blastwave(atom/epicenter, devastation_range, heavy_impact_range, light_impact_range, flame_range, flash_range, adminlog, ignorecap, silent, smoke, atom/explosion_cause)
	...
	for(var/TI in affected_turfs)
		...
		if(T == epicenter) // Ensures explosives detonating from bags trigger other explosives in that bag
			...
			for(var/I in T)
				var/atom/A = I
				if (length(A.contents) && !(A.atom_flags & PREVENT_CONTENTS_EXPLOSION)) //The atom/contents_explosion() proc returns null if the contents ex_acting has been handled by the atom, and TRUE if it hasn't.
					items += A.get_all_contents(ignore_flag_1 = PREVENT_CONTENTS_EXPLOSION)
				if(isliving(A))

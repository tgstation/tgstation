					...
					for(var/tc in target_mob.listed_turf)
						...
						if(length(turfitems) < 30) // only create images for the first 30 items on the turf, for performance reasons
							if(!(REF(turf_content) in cached_images))
								...
								RegisterSignal(turf_content, COMSIG_PARENT_QDELETING, .proc/reset_cache) // we reset cache if anything in it gets deleted
...
/datum/controller/subsystem/statpanels/proc/reset_cache()
	cached_images.Cut()

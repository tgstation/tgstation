...
SUBSYSTEM_DEF(cold_fire_burning)
	name = "Cold Fire Burning"
	priority = COLD_FIRE_PRIORITY_BURNING
	flags = SS_NO_INIT|SS_BACKGROUND
	runlevels = RUNLEVEL_GAME | RUNLEVEL_POSTGAME
...
/datum/controller/subsystem/cold_fire_burning/stat_entry()
	..("P:[processing.len]")


/datum/controller/subsystem/cold_fire_burning/fire(resumed = 0)
	if (!resumed)
		src.currentrun = processing.Copy()
	...
	while(currentrun.len)
		...
		if(O.resistance_flags & ON_FIRE) //in case an object is extinguished while still in currentrun
			if(!(O.resistance_flags & FIRE_PROOF))
				O.take_damage(20, BURN, "cold fire", 0)
			else
				O.extinguish()

...
/datum/controller/subsystem/stock_market/fire(resumed)
	to_chat(world, span_boldannounce("We are firing!"))
	...
	for(var/datum/material/market in materials_prices)
		...
		materials_prices[market] = round(clamp(price_units + (0.2 * price_units), market.value_per_unit * SHEET_MATERIAL_AMOUNT * 0.5, market.value_per_unit * SHEET_MATERIAL_AMOUNT * 1.5))

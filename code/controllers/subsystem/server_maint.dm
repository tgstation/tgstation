	...
	for(var/I in currentrun)
		...
		if(round_started && kick_inactive && !C.holder && C.is_afk(afk_period))
			...
			if (!isnewplayer(cmob) || !SSticker.queued_players.Find(cmob))
				...
				to_chat(C, "<span class='userdanger'>You have been inactive for more than [DisplayTimeText(afk_period)] and have been disconnected.</span><br><span class='danger'>You may reconnect via the button in the file menu.</span>")

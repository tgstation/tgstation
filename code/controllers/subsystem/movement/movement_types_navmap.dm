...
/datum/move_loop/has_target/navmap_astar/setup(delay, timeout, atom/chasing, repath_delay, max_path_length, minimum_distance, list/access, simulated_only, turf/avoid, skip_first, diagonal_handling, list/initial_path)
	. = ..()
	if(!.)
		return
...
/datum/move_loop/has_target/navmap_astar/compare_loops(datum/move_loop/loop_type, priority, flags, extra_info, delay, timeout, atom/chasing, repath_delay, max_path_length, minimum_distance, list/access, simulated_only, turf/avoid, skip_first, diagonal_handling, initial_path)
...
/datum/move_loop/has_target/navmap_astar/proc/recalculate_path()
	...
	pathfinding = SSpathfinder.navmap_pathfind(moving, target, max_path_length, minimum_distance, access, simulated_only, avoid, skip_first, diagonal_handling, list(CALLBACK(src, PROC_REF(on_finish_pathing))))
	is_pathing = !!pathfinding

/// Accepts the completed path returned by SSpathfinder.
/datum/move_loop/has_target/navmap_astar/proc/on_finish_pathing(list/path)
	pathfinding = null
	is_pathing = FALSE
	movement_path = path || list()
	if(moving)
		EVLOG_PATH(moving, EVLOG_CATEGORY_NAVMAP, "Planned navmap A* path", movement_path)
	SEND_SIGNAL(src, COMSIG_MOVELOOP_NAVMAP_FINISHED_PATHING, movement_path)
...
/datum/move_loop/has_target/navmap_astar/frustrations/on_finish_pathing(list/path)
	. = ..()
	if(length(movement_path))

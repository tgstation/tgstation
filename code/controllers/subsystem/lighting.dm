...
/datum/subsystem/lighting/fire()
	...
	for(var/light in lights)
		if(light && !light:check())	//yes, cry that I'm using the : operator, it's much faster looping like this. And this gets called a lot. Dealwithit.
			++i
			continue
		lights.Cut(i, i+1)

	changed_turfs_workload = MC_AVERAGE(changed_turfs_workload, changed_turfs.len)
	for(var/turf in changed_turfs)
		if(turf && turf:lighting_changed)
...
/datum/subsystem/lighting/Initialize(timeofday, z_level)
	...
	if(z_level)
		...
		i=1
		for(var/turf in changed_turfs)
			if(turf && turf:z < z_start && z_finish < turf:z)
				++i
				continue

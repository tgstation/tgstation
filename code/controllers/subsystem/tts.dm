...
/datum/controller/subsystem/tts/fire(resumed = FALSE)
	...
	for (var/datum/tts/T in processing)//we have something to process
		switch(T.status)
			if (STATUS_NEW)
...
/datum/controller/subsystem/tts/proc/delete_files(datum/tts/T)	//Deletes files once they're used
	if (!T)
		return
	if (!T.filename)
		return
...
/datum/controller/subsystem/tts/proc/play_tts(datum/tts/T)
	if (!T.owner)
		message_admins("TTS request has no owner")	//I dunno how the fuck you managed to play a sound with no owner
		return
	if (!T.owner.mob)
...
/datum/tts/proc/say(client/C, msg, voice = "", is_global = FALSE, volume_mod = 1, datum/language/language)	//who be talking
	if (!C)
		return
	if (!msg)
		return

...
/datum/controller/subsystem/shuttle/proc/requestEvac(mob/user, call_reason)
	...
	if(length(call_reason) < CALL_SHUTTLE_REASON_LENGTH && SSsecurity_level.get_current_level(TRUE) > SEC_LEVEL_GREEN)
		to_chat(user, span_alert("You must provide a reason."))
		return

	var/area/signal_origin = get_area(user)
	var/emergency_reason = "\nNature of emergency:\n\n[call_reason]"
	var/security_num = SSsecurity_level.get_current_level(TRUE)
	switch(security_num)

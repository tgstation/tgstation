...
/datum/controller/subsystem/polling/proc/is_eligible(mob/M, role, check_jobban, the_ignore_category)
	if(!M.key || !M.client)
		return FALSE
	...
	if(check_jobban)
		if(is_banned_from(M.ckey, list(check_jobban, ROLE_SYNDICATE)))
			return FALSE

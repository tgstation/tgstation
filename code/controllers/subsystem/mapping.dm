...
/datum/controller/subsystem/mapping/proc/setup_ruins()
	...
	if (ice_ruins.len)
		...
		seedRuins(ice_ruins, CONFIG_GET(number/icemoon_budget), list(/area/icemoon/surface/outdoors/unexplored, /area/icemoon/underground/unexplored), themed_ruins[ZTRAIT_ICE_RUINS], clear_below = TRUE, mineral_budget = 4, mineral_budget_update = OREGEN_PRESET_ICEBOX_UPPER)

	var/list/ice_ruins_underground = levels_by_trait(ZTRAIT_ICE_RUINS_UNDERGROUND)
	if (ice_ruins_underground.len)
		seedRuins(ice_ruins_underground, CONFIG_GET(number/icemoon_budget), list(/area/icemoon/underground/unexplored), themed_ruins[ZTRAIT_ICE_RUINS_UNDERGROUND], clear_below = TRUE, mineral_budget = 21, mineral_budget_update = OREGEN_PRESET_ICEBOX_LOWER)
...
/datum/controller/subsystem/mapping/proc/loadWorld()
	...
	else if (!isnull(config.minetype) && config.minetype != "icebox")

...
/datum/controller/subsystem/nightshift/proc/activate_nightshift(announce = TRUE)
	if(!nightshift_active)
		if(announce)
			SSticker.OnRoundstart(CALLBACK(GLOBAL_PROC, /proc/priority_announce, "Good evening, crew. To reduce power consumption and stimulate the circadian rhythms of some species, all of the lights aboard the station have been dimmed for the night.", sound='sound/misc/notice2.ogg', sender_override="Automated Lighting System Announcement"))
...
/datum/controller/subsystem/nightshift/proc/deactivate_nightshift(announce = TRUE)
	if(nightshift_active)
		if(announce)
			SSticker.OnRoundstart(CALLBACK(GLOBAL_PROC, /proc/priority_announce, "Good morning, crew. As it is now day time, all of the lights aboard the station have been restored to their former brightness.", sound='sound/misc/notice2.ogg', sender_override="Automated Lighting System Announcement"))

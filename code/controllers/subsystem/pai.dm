...
/datum/controller/subsystem/pai/Topic(href, href_list[])
	...
	if(href_list["new"])
		...
		switch(option)
			if("name")
				t = input("Enter a name for your pAI", "pAI Name", candidate.name, istext = TRUE)
				if(t)
					candidate.name = copytext(sanitize(t),1,MAX_NAME_LEN)
			if("desc")
				t = input("Enter a description for your pAI", "pAI Description", candidate.description, ismessage = TRUE)
				if(t)
					candidate.description = copytext(sanitize(t),1,MAX_MESSAGE_LEN)
			if("role")
				t = input("Enter a role for your pAI", "pAI Role", candidate.role, istext = TRUE)
				if(t)
					candidate.role = copytext(sanitize(t),1,MAX_MESSAGE_LEN)
			if("ooc")
				t = input("Enter any OOC comments", "pAI OOC Comments", candidate.comments, ismessage = TRUE)
				if(t)

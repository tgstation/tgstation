...
SUBSYSTEM_DEF(ticker)
	...
	var/autosandbox_checked = FALSE // austation -- Autosandbox
...
/datum/controller/subsystem/ticker/fire()
	switch(current_state)
		...
		if(GAME_STATE_PREGAME)
			...
			if(timeLeft <= 100 && !autosandbox_checked) // austation begin -- Autosandbox code
				...
				if(CONFIG_GET(flag/autosandbox_enabled))
					...
					if(GLOB.master_mode == "sandbox")
						...
						world.update_status()
						message_admins("Respawn has been automatically enabled due to the mode being sandbox.") // austation end

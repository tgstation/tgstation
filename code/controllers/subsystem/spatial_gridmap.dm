...
/datum/controller/subsystem/spatial_grid/proc/exit_cell(atom/movable/old_target, turf/target_turf, exclusive_type)
	...
	if(!target_turf || !(old_target?.important_recursive_contents || old_target?.flags_1 & SPATIAL_GRID_MANAGED_1))
		CRASH("/datum/controller/subsystem/spatial_grid/proc/exit_cell() was given null arguments or a old_target that doesn't use the spatial grid!")
	...
	if(old_target.important_recursive_contents)
		...
		if(old_target.important_recursive_contents[RECURSIVE_CONTENTS_HEARING_SENSITIVE])
			...
			SEND_SIGNAL(intersecting_cell, SPATIAL_GRID_CELL_EXITED(SPATIAL_GRID_CONTENTS_TYPE_HEARING), old_target)

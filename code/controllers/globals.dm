...
/datum/controller/global_vars/New()
	...
	gvars_datum_in_built_vars = exclude_these.vars - "gvars_datum_protected_varlist" - "gvars_datum_in_built_vars" - "gvars_datum_init_order"
	qdel(exclude_these)

	InitEverything()

	log_world("Globals initialization took [(REALTIMEOFDAY - start_time)/10]s.")
...
/datum/controller/global_vars/stat_entry()
	...
	var/static/num_globals = vars.len - gvars_datum_in_built_vars.len
...
/datum/controller/global_vars/vv_edit_var(var_name, var_value)
	if((var_name in gvars_datum_protected_varlist) || var_name == "gvars_datum_protected_varlist")
		return FALSE
	return ..()

/datum/controller/global_vars/proc/InitEverything()
	gvars_datum_init_order = list()
	gvars_datum_protected_varlist = list()

	...
	if(istype(loc, /turf/simulated))
		...
		if(T.to_be_destroyed)
			...
			if (T.heat_capacity) //beware of division by zero
				chance_of_deletion = T.max_fire_temperature_sustained / T.heat_capacity * 8 //there is no problem with new_prob(23456), min() was redundant --rastaf0
			else
				chance_of_deletion = 100
			if(new_prob(chance_of_deletion))

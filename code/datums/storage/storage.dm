...
/datum/storage/proc/item_insertion_feedback(mob/user, obj/item/thing, override = FALSE)
	...
	if(do_rustle)
		playsound(parent, rustle_sound).vary(rustle_vary).range(-5 + SOUND_RANGE).play()
...
/datum/storage/proc/open_storage(mob/to_show)
	...
	if(do_rustle && !silent)
		playsound(parent, open_sound || SFX_RUSTLE).vary(open_sound_vary).range(-5 + SOUND_RANGE).play()

...
/datum/storage/bag_of_holding/proc/recursive_insertion(obj/item/to_insert, mob/living/user)
	...
	if(safety != "Proceed" || QDELETED(to_insert) || QDELETED(resolve_parent) || QDELETED(user) || !user.canPerformAction(resolve_parent, be_close = TRUE, no_dexterity = iscarbon(user)))
		return

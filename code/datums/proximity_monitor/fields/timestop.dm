...
/obj/effect/timestop/Destroy()
	...
	if(!hidden)
		playsound(src, 'sound/magic/timeparadox2.ogg', 75, TRUE, frequency = -1, ignore_walls = TRUE) //reverse!
	return ..()

/obj/effect/timestop/proc/timestop()
	target = get_turf(src)
	if(!hidden)
		playsound(src, 'sound/magic/timeparadox2.ogg', 75, TRUE, -1, ignore_walls = TRUE)

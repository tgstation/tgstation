...
/datum/beam/proc/Draw()
	...
	var/DX = (64*target.x+target.pixel_x)-(64*origin.x+origin.pixel_x)
	var/DY = (64*target.y+target.pixel_y)-(64*origin.y+origin.pixel_y)
	var/N = 0
	...
	for(N in 0 to length-1 step 64)//-1 as we want < not <=, but we want the speed of X in Y to Z and step X
		...
		if(N+64>length)
			...
			II.DrawBox(null,1,(length-N),64,64)
			X.icon = II
		else
			X.icon = base_icon
		...
		if(DX == 0)
			...
			Pixel_x = round(sin(Angle)+64*sin(Angle)*(N+32)/64)
		if(DY == 0)
			Pixel_y = 0
		else
			Pixel_y = round(cos(Angle)+64*cos(Angle)*(N+32)/64)

		//Position the effect so the beam is one continous line
		var/a
		if(abs(Pixel_x)>64)
			a = Pixel_x > 0 ? round(Pixel_x/64) : Ceiling(Pixel_x/64)
			X.x += a
			Pixel_x %= 64
		if(abs(Pixel_y)>64)
			a = Pixel_y > 0 ? round(Pixel_y/64) : Ceiling(Pixel_y/64)
			X.y += a
			Pixel_y %= 64

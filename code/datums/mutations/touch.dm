...
/datum/action/cooldown/spell/touch/lay_on_hands/proc/determine_if_this_hurts_instead(mob/living/carbon/mendicant, mob/living/hurtguy)
	...
	var/datum/component/empathy/empath_status = hurtguy.GetComponent(/datum/component/empathy)
	if(empath_status)
		if(!(hurtguy.mob_biotypes & MOB_UNDEAD) && empath_status.smite_target && HAS_TRAIT(mendicant, TRAIT_EVIL)) //Is the guy not undead, they're a smiteable empath and we're evil? If so, hurt.
			return TRUE
	return FALSE

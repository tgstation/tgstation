...
/datum/wires/big_manipulator/get_status()
	...
	status += "The green number on the display shows [span_bold("[length(holder_manipulator.pickup_points)]")]."
	status += "The red number on the display shows [span_bold("[length(holder_manipulator.dropoff_points)]")]."
	return status

...
/datum/wires/robot/get_status()
	...
	status += "There is a star symbol above the [get_color_of_wire(WIRE_RESET_CONFIGURATION)] wire."
	return status
...
/datum/wires/robot/on_pulse(wire, user)
	...
	switch(wire)
		...
		if(WIRE_RESET_CONFIGURATION)
			if(R.has_configuration())
				R.visible_message("<span class='notice'>[R]'s configuration servos twitch.</span>", "<span class='notice'>Your configuration display flickers.</span>")
...
/datum/wires/robot/always_reveal_wire(color)
	// Always reveal the reset configuration wire.
	if(color == get_color_of_wire(WIRE_RESET_CONFIGURATION))
		return TRUE

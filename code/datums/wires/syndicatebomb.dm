	...
	switch(wire)
		...
		if(WIRE_DELAY)
			if(B.delayedbig)
				...
				playsound(B, 'sound/machines/chime.ogg').volume(30).vary(TRUE).play()
		...
		if(WIRE_PROCEED)
			...
			playsound(B, 'sound/machines/buzz/buzz-sigh.ogg').volume(30).vary(TRUE).play()
	...
	switch(wire)
		...
		if(WIRE_UNBOLT)
			if(!mend && B.anchored)
				...
				playsound(B, 'sound/effects/stealthoff.ogg').volume(30).vary(TRUE).play()

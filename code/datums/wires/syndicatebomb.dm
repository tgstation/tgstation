	...
	switch(wire)
		...
		if(WIRE_DELAY)
			if(B.delayedbig)
				...
				create_sound(B, 'sound/machines/chime.ogg').volume(30).vary(TRUE).play()
		...
		if(WIRE_PROCEED)
			...
			create_sound(B, 'sound/machines/buzz/buzz-sigh.ogg').volume(30).vary(TRUE).play()
	...
	switch(wire)
		...
		if(WIRE_UNBOLT)
			if(!mend && B.anchored)
				...
				create_sound(B, 'sound/effects/stealthoff.ogg').volume(30).vary(TRUE).play()

	...
	switch(wire)
		...
		if(WIRE_DELAY)
			if(B.delayedbig)
				...
				playsound(B, 'sound/machines/chime.ogg', 30, DEFAULT_SOUND_VARY)
				B.detonation_timer += 300
				B.delayedbig = TRUE
		if(WIRE_PROCEED)
			holder.visible_message("<span class='danger'>[icon2html(B, viewers(holder))] The bomb buzzes ominously!</span>")
			playsound(B, 'sound/machines/buzz-sigh.ogg', 30, DEFAULT_SOUND_VARY)
	...
	switch(wire)
		...
		if(WIRE_UNBOLT)
			if(!mend && B.anchored)
				...
				playsound(B, 'sound/effects/stealthoff.ogg', 30, DEFAULT_SOUND_VARY)
				B.anchored = FALSE
		if(WIRE_PROCEED)
			if(!mend && B.active)
				holder.visible_message("<span class='danger'>[icon2html(B, viewers(holder))] An alarm sounds! It's go-</span>")

	...
	switch(wire)
		...
		if(WIRE_DELAY)
			if(B.delayedbig)
				...
				PLAYSOUND(B, 'sound/machines/chime.ogg').volume(30).vary_frequency(TRUE).play()
		...
		if(WIRE_PROCEED)
			...
			PLAYSOUND(B, 'sound/machines/buzz/buzz-sigh.ogg').volume(30).vary_frequency(TRUE).play()
	...
	switch(wire)
		...
		if(WIRE_UNBOLT)
			if(!mend && B.anchored)
				...
				PLAYSOUND(B, 'sound/effects/stealthoff.ogg').volume(30).vary_frequency(TRUE).play()

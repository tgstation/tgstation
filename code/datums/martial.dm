...
//admin bullshit martial art

/datum/martial_art/admin_fu
	name = "Supreme Ultimate Fist"
	deflection_chance = 100
	help_verb = /mob/living/carbon/human/proc/admin_fu_help

/datum/martial_art/admin_fu/proc/wristFlip(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(!D.stat && !D.stunned && !D.weakened)
		A.do_attack_animation(D)
		D.visible_message("<span class='warning'>[A] grabs [D]'s wrist and twists them into a flip!</span>", \
						  "<span class='userdanger'>[A] grabs your wrist and violently wrenches it into a flip!</span>")
		playsound(get_turf(A), 'sound/weapons/thudswoosh.ogg', 50, 1, -1)
		D.emote("flip")
		D.Weaken(10)
		return 1
...
/datum/martial_art/admin_fu/proc/kneeStomach(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(!D.stat && !D.weakened)
		A.do_attack_animation(D)
		D.visible_message("<span class='warning'>[A] knees [D] in the stomach!</span>", \
						  "<span class='userdanger'>[A] winds you with a knee in the stomach!</span>")
...
/datum/martial_art/admin_fu/proc/headKick(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(!D.stat && !D.weakened)
		A.do_attack_animation(D)
		D.visible_message("<span class='warning'>[A] kicks [D] in the head!</span>", \
						  "<span class='userdanger'>[A] kicks you in the head!</span>")
...
/datum/martial_art/admin_fu/proc/neckStomp(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(D.weakened || D.resting || D.stat)
		A.do_attack_animation(D)
		D.visible_message("<span class='warning'>[A] stomps on [D]'s neck!</span>", \
						  "<span class='userdanger'>[A] stomps on your neck!</span>")
		if(D.stat)
			D.death() //FINISH HIM!
		D.apply_damage(100, BRUTE, "chest")
		playsound(get_turf(D), 'sound/weapons/punch1.ogg', 75, 1, -1)
		return 1

/datum/martial_art/admin_fu/proc/neckSnap(mob/living/carbon/human/A, mob/living/carbon/human/D)
	A.do_attack_animation(D)
	D.visible_message("<span class='warning'>[A] snaps [D]'s neck!</span>", \
						 "<span class='userdanger'>[A] snaps on your neck!</span>")
	D.apply_damage(400, BRUTE)
	playsound(get_turf(D), 'sound/weapons/punch1.ogg', 75, 1, -1)

/datum/martial_art/admin_fu/proc/eyeGouge(mob/living/carbon/human/A, mob/living/carbon/human/D)
	A.do_attack_animation(D)
	D.visible_message("<span class='warning'>[A] gouges out [D]'s eyes!</span>", \
					  "<span class='userdanger'>[A] gouges out your eyes!</span>")
	D.adjust_eye_damage(100)
	D.apply_damage(25, BRUTE, "head")
	D.Weaken(10)
	D.emote("scream")
	D.become_blind()
	playsound(get_turf(D), 'sound/weapons/punch1.ogg', 75, 1, -1)
...
/datum/martial_art/admin_fu/grab_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	D.grabbedby(A,1)
	var/obj/item/weapon/grab/G = A.get_active_hand()
...
/datum/martial_art/admin_fu/harm_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	A.do_attack_animation(D)
	if(neckStomp(A,D))
		return 1
	if(A.zone_selected == "eyes")
		eyeGouge(A,D)
		return 1
	...
	D.visible_message("<span class='danger'>[A] [atk_verb] [D]!</span>", \
					  "<span class='userdanger'>[A] [atk_verb] you!</span>")
	D.apply_damage(rand(20,25), BRUTE)
	playsound(get_turf(D), 'sound/weapons/punch1.ogg', 25, 1, -1)
	if(prob(D.getBruteLoss()) && !D.lying)
		D.visible_message("<span class='warning'>[D] stumbles and falls!</span>", "<span class='userdanger'>The blow sends you to the ground!</span>")
		D.Weaken(4)
...
/datum/martial_art/admin_fu/disarm_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	wristFlip(A,D)
	return ..()

/mob/living/carbon/human/proc/admin_fu_help()
	set name = "Recall Teachings"
	set desc = "Remember the martial techniques of the Sleeping Carp clan."
	set category = "Sleeping Carp"
...
/obj/item/weapon/admin_fu_scroll
	name = "mysterious scroll"
	desc = "A scroll filled with strange markings. It seems to be drawings of some sort of martial art."
	icon = 'icons/obj/wizard.dmi'
	icon_state = "scroll2"

/obj/item/weapon/admin_fu_scroll/attack_self(mob/living/carbon/human/user)
	if(!istype(user) || !user)
		return
	user << "<span class='sciradio'>You have learned the ancient martial art of the Supreme Ultimate First! Your hand-to-hand combat has become much more effective, and you are now able to deflect any projectiles \
	directed toward you. You can learn more about your newfound art by using the Recall Teachings verb in the Sleeping Carp tab.</span>"
	var/datum/martial_art/admin_fu/adminFu = new(null)
	adminFu.teach(user)
	user.drop_item()
	visible_message("<span class='warning'>[src] lights up in fire and quickly burns to ash.</span>")
	new /obj/effect/decal/cleanable/ash(get_turf(src))

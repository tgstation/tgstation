...
/datum/ai_behavior/find_and_set/perform(seconds_per_tick, datum/ai_controller/controller, set_key, locate_path, search_range)
	...
	EVLOG_MAPTEXT(controller, "AI Targetting", "[controller.pawn] has selected [find_this_thing] as a target for blackboard key [set_key]!", get_turf(find_this_thing), "Target: [find_this_thing]")

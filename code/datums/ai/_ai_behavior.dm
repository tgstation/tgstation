...
/datum/ai_behavior/proc/finish_action(datum/ai_controller/controller, succeeded, ...)
	...
	EVLOG_MAPTEXT(controller, "AI Behaviors", "[controller.pawn] has [succeeded ? "succeeded" : "failed"] at performing [src]", get_turf(controller.pawn), "Behavior finished: [succeeded ? "Success" : "Failure"]")

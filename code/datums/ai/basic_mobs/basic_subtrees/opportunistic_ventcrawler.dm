...
/datum/ai_planning_subtree/opportunistic_ventcrawler/SelectBehaviors(datum/ai_controller/controller, seconds_per_tick)
	if(controller.blackboard[BB_INSIDE_VENT])
	...
	if(QDELETED(target))
		controller.queue_behavior(/datum/ai_behavior/find_and_set/in_list, BB_ENTRY_VENT_TARGET, /obj/machinery/atmospherics/components/unary/vent_pump) // keep looking otherwise they KILL US AND WE DIE
		return
	...
	controller.queue_behavior(/datum/ai_behavior/crawl_around_vents)

...
/datum/ai_planning_subtree/find_nearest_thing_which_attacked_me_to_flee/SelectBehaviors(datum/ai_controller/controller, seconds_per_tick)
	...
	if (!controller.blackboard[BB_BASIC_MOB_FLEEING])
		return
	controller.queue_behavior(/datum/ai_behavior/target_from_retaliate_list/nearest, BB_BASIC_MOB_RETALIATE_LIST, BB_BASIC_MOB_CURRENT_TARGET, targetting_key, BB_BASIC_MOB_CURRENT_TARGET_HIDING_LOCATION)

/datum/ai_planning_subtree/find_nearest_thing_which_attacked_me_to_flee/from_flee_key
	targetting_key = BB_FLEE_TARGETTING_DATUM

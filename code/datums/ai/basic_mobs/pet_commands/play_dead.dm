...
/datum/ai_behavior/play_dead/setup(datum/ai_controller/controller)
	...
	if(!istype(basic_pawn) || IS_UNCONSCIOUS_OR_CRIT(basic_pawn)) // Can't act dead if you're dead
		return
...
/datum/ai_behavior/play_dead/finish_action(datum/ai_controller/controller, succeeded)
	. = ..()
	var/mob/living/basic/basic_pawn = controller.pawn
	if(QDELETED(basic_pawn) || IS_UNCONSCIOUS_OR_CRIT(basic_pawn)) // imagine actually dying while playing dead. hell, imagine being the kid waiting for your pup to get back up :(
		return

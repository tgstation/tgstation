...
/datum/ai_behavior/find_target/proc/perform(delta_time, datum/ai_controller/controller, hunting_target_key, types_to_hunt)
	. = ..()

	var/mob/living/living_mob = controller.pawn

	for(var/possible_dinner in typecache_filter_list(range(2, living_mob), controller.blackboard[types_to_hunt]))
...
/datum/ai_behavior/hunt_target/proc/perform(delta_time, datum/ai_controller/controller, hunting_target_key)
	. = ..()
	...
	if(isliving(hunted)) // Are we hunting a living mob?
		...
		manual_emote("chomps [living_target]!")
		living_target.death()
...
/datum/ai_behavior/hunt_target/finish_action(datum/ai_controller/controller, succeeded, hunting_target_key)
	. = ..()
	if(!succeeded && hunting_target_key)

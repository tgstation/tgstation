...
/datum/ai_behavior/wait_for_food/perform(seconds_per_tick, datum/ai_controller/controller)
	...
	if(controller.blackboard[BB_CUSTOMER_PATIENCE] < 0 || controller.blackboard[BB_CUSTOMER_LEAVING]) // Check if we're leaving because sometthing might've forced us to
		finish_action(controller, FALSE)
		return
	...
	///Now check if there's a meal in front of us.
	var/datum/venue/attending_venue = controller.blackboard[BB_CUSTOMER_ATTENDING_VENUE]

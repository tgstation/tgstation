/datum/ai_planning_subtree/dog/SelectBehaviors(datum/ai_controller/dog/controller, delta_time)
	...
	if(COOLDOWN_FINISHED(controller, reset_ignore_cooldown) && length(controller.blackboard[BB_FETCH_IGNORE_LIST]))
		COOLDOWN_START(controller, reset_ignore_cooldown, AI_FETCH_IGNORE_DURATION)
	...
	if(DT_PROB(40, delta_time))
		for(var/obj/item/potential_snack in oview(living_pawn,2))
			if(IS_EDIBLE(potential_snack) && (isturf(potential_snack.loc) || ishuman(potential_snack.loc)))
				controller.current_movement_target = potential_snack
				controller.current_behaviors += GET_AI_BEHAVIOR(/datum/ai_behavior/eat_snack)
				return

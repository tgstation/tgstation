...
/datum/ai_behavior/monkey_random_wander/process(delta_time)
	...
	if(DT_PROB(25, delta_time))
		step(our_controller.pawn, pick(GLOB.cardinals))
...
/datum/ai_behavior/monkey_random_emote/process(delta_time)
	if(DT_PROB(3, delta_time))
		our_controller.pawn.emote(pick("scratch","jump","roll","tail"))
...
/datum/ai_behavior/try_equip_item/monkey_try_equip_item/start_execution()
	...
	if(I.loc == our_controller.pawn.src)//Already holding it
		finish_execution(TRUE)
	...
	// Strong weapon
		...
		our_controller.pawn.put_in_hands(I)
		best_force = I.force
		finish_execution(TRUE)

	// EVERYTHING ELSE
	else if(!our_controller.pawn.get_item_for_held_index(1) || !our_controller.pawn.get_item_for_held_index(2))
		our_controller.pawn.put_in_hands(I)
		finish_execution(TRUE)

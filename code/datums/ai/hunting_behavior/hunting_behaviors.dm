...
/datum/ai_behavior/find_hunt_target/perform(seconds_per_tick, datum/ai_controller/controller, hunting_target_key, types_to_hunt, hunt_range)
	...
	for(var/atom/possible_dinner as anything in typecache_filter_list(interesting_objects, types_to_hunt))
		...
		EVLOG_MAPTEXT(controller, "AI Targetting", "[controller.pawn] has selected [possible_dinner] as a target for blackboard key [hunting_target_key]! Behavior: [src]", get_turf(possible_dinner), "Target: [possible_dinner]")
		EVLOG_LINES(controller, "AI Targetting", "Line to target", get_turf(controller.pawn), get_turf(possible_dinner))

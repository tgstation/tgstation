...
/datum/wound/slash/proc/lick_wounds(mob/living/carbon/human/user)
	...
	if(!do_after(user, victim, base_treat_time, extra_checks = CALLBACK(src, .proc/still_exists)))
		return
...
/datum/wound/slash/proc/las_cauterize(obj/item/gun/energy/laser/lasgun, mob/user)
	...
	if(!do_after(user, victim, base_treat_time  * self_penalty_mult, extra_checks = CALLBACK(src, .proc/still_exists)))
		return
...
/datum/wound/slash/proc/tool_cauterize(obj/item/I, mob/user)
	...
	if(!do_after(user, victim, base_treat_time * self_penalty_mult * improv_penalty_mult, extra_checks = CALLBACK(src, .proc/still_exists)))
		return

...
/datum/wound/proc/apply_wound(obj/item/bodypart/L, silent = FALSE, datum/wound/old_wound = null, smited = FALSE, attack_direction = null, wound_source = "Unknown", replacing = FALSE)
	...
	if(!silent && !demoted)
		...
		if(sound_effect)
			playsound(L.owner, sound_effect).volume(sound_volume + (20 * severity)).vary(TRUE).falloff_exponent(SOUND_FALLOFF_EXPONENT + 2).ignore_walls(FALSE).falloff_distance(0).play()

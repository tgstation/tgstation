/datum/wound/blunt/robotic/secures_internals/critical
	...
	treat_text_short = "Heat via welder, mold with gloves, apply screwdriver, then apply welder"
	disabling = TRUE
...
/datum/wound/blunt/robotic/secures_internals/critical/proc/mold_metal(mob/living/carbon/human/user)
	...
	if ((user != victim && user.combat_mode))
		...
		to_chat(user, span_green("[capitalize(your_or_other)] [limb.plaintext_zone] has been molded into the proper shape, but the internals still need to be secured and welded into place."))
...
/datum/wound/blunt/robotic/secures_internals/critical/proc/heat_metal(obj/item/welder, mob/living/user)
	...
	if (!welder.use_tool(target = victim, user = user, delay = 3 SECONDS, volume = 50, extra_checks = CALLBACK(src, PROC_REF(still_exists))))
		return TRUE
	...
	if (user != victim && user.combat_mode)
		...
		limb.receive_damage(burn = 20, wound_bonus = CANT_WOUND)

...
/datum/martial_art/plasma_fist/proc/Throwback(mob/living/attacker, mob/living/defender)
	...
	PLAYSOUND(defender, 'sound/items/weapons/punch1.ogg').vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
...
/datum/martial_art/plasma_fist/proc/Apotheosis(mob/living/user, mob/living/target)
	...
	PLAYSOUND(boomspot, 'sound/items/weapons/punch1.ogg').vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()

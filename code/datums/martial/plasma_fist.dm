...
/datum/martial_art/plasma_fist/proc/tornado_sound(attacker)
	playsound(attacker, 'sound/items/weapons/punch1.ogg').volume(15).vary(TRUE).extra_range(-1).play()
...
/datum/martial_art/plasma_fist/proc/Throwback(mob/living/attacker, mob/living/defender)
	...
	playsound(defender, 'sound/items/weapons/punch1.ogg').vary(TRUE).range(-1 + SOUND_RANGE).play()
...
/datum/martial_art/plasma_fist/proc/Apotheosis(mob/living/user, mob/living/target)
	...
	playsound(boomspot, 'sound/items/weapons/punch1.ogg').vary(TRUE).range(-1 + SOUND_RANGE).play()

...
/datum/martial_art/the_sleeping_carp/teach(mob/living/target, make_temporary = FALSE)
	...
	if(!.)
		return
	ADD_TRAITS(target, list(TRAIT_NOGUNS, TRAIT_HARDLY_WOUNDED, TRAIT_NODISMEMBER), SLEEPING_CARP_TRAIT)
...
/datum/martial_art/the_sleeping_carp/on_remove(mob/living/target)
	REMOVE_TRAITS(target, list(TRAIT_NOGUNS, TRAIT_HARDLY_WOUNDED, TRAIT_NODISMEMBER), SLEEPING_CARP_TRAIT)

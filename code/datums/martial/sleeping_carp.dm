...
/datum/martial_art/the_sleeping_carp/proc/wristWrench(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(!D.stat && !D.IsStun() && !D.IsParalyzed())
		...
		D.visible_message("<span class='danger'>[A] grabs [D]'s wrist and wrenches it sideways!</span>", \
						"<span class='userdanger'>Your wrist is grabbed by [A] while simultaneously wrenched it to the side!</span>", null, null, A)
...
/datum/martial_art/the_sleeping_carp/proc/backKick(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(!D.stat && !D.IsParalyzed())
		if(A.dir == D.dir)
			...
			A.do_attack_animation(D, ATTACK_EFFECT_PUNCH)
			D.visible_message("<span class='danger'>[A] kicks [D] in the back!</span>", \
							"<span class='userdanger'>You're kicked in the back by [A], making you stumble and fall!</span>", null, null, A)
			to_chat(A, "<span class='danger'>You kick [D] in the back, making [D.p_them()] stumble and fall!</span>")
...
/datum/martial_art/the_sleeping_carp/proc/kneeStomach(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(!D.stat && !D.IsParalyzed())
		...
		D.visible_message("<span class='danger'>[A] knees [D] in the stomach!</span>", \
						"<span class='userdanger'>Your stomach is kneed by [A], making you gag!</span>", null, null, A)
...
/datum/martial_art/the_sleeping_carp/proc/headKick(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(!D.stat && !D.IsParalyzed())
		...
		D.visible_message("<span class='warning'>[A] kicks [D] in the head!</span>", \
						"<span class='userdanger'>Your jaw is kicked by [A]!</span>", null, null, A)
...
/datum/martial_art/the_sleeping_carp/proc/elbowDrop(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(!(D.mobility_flags & MOBILITY_STAND))
		...
		D.visible_message("<span class='danger'>[A] elbow drops [D]!</span>", \
						"<span class='userdanger'>You're piledrived by [A] with [A.p_their()] elbow!</span>", null, null, A)
...
/obj/item/twohanded/bostaff/attack(mob/target, mob/living/user)
	...
	if(user.a_intent == INTENT_DISARM)
		...
		H.visible_message("<span class='warning'>[user] [pick(fluffmessages)]s [H] with [src]!</span>", \
						"<span class='userdanger'>[user] [pick(fluffmessages)]s you with [src]!</span>", null, null, user)
		...
		if(H.staminaloss && !H.IsSleeping())
			...
			if(total_health <= HEALTH_THRESHOLD_CRIT && !H.stat)
				H.visible_message("<span class='warning'>[user] delivers a heavy hit to [H]'s head, knocking [H.p_them()] out cold!</span>", \
								"<span class='userdanger'>You're knocked unconscious by [user]!</span>", null, null, user)

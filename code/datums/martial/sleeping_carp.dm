...
/datum/martial_art/the_sleeping_carp/proc/strongPunch(mob/living/attacker, mob/living/defender)
	...
	playsound(defender, 'sound/items/weapons/punch1.ogg').volume(25).vary(TRUE).range(-1 + SOUND_RANGE).play()
...
/datum/martial_art/the_sleeping_carp/proc/hit_by_projectile(mob/living/carp_user, obj/projectile/hitting_projectile, def_zone)
	...
	playsound(carp_user, pick('sound/items/weapons/bulletimpact1.ogg', 'sound/items/weapons/bulletimpact2.ogg', 'sound/items/weapons/bulletimpact3.ogg')).volume(75).vary(TRUE).play()

...
/datum/martial_art/krav_maga/proc/quick_choke(mob/living/attacker, mob/living/defender)//is actually lung punch
	...
	PLAYSOUND(attacker, 'sound/effects/hit_punch.ogg').vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
...
/datum/martial_art/krav_maga/harm_act(mob/living/attacker, mob/living/defender)
	...
	if(picked_hit_type == "kick" || picked_hit_type == "stomp")
		...
		PLAYSOUND(defender, 'sound/effects/hit_kick.ogg').vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
	else
		attacker.do_attack_animation(defender, ATTACK_EFFECT_PUNCH)
		PLAYSOUND(defender, 'sound/effects/hit_punch.ogg').vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
	defender.visible_message(
...
/datum/martial_art/krav_maga/disarm_act(mob/living/attacker, mob/living/defender)
	...
	if(prob(60) && stuff_in_hand && defender.temporarilyRemoveItemFromInventory(stuff_in_hand))
		...
		PLAYSOUND(defender, 'sound/items/weapons/thudswoosh.ogg').vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()

...
/datum/martial_art/wrestling/teach(mob/living/carbon/human/H,make_temporary=ZERO)
	if(..())
...
/datum/martial_art/wrestling/proc/FlipAnimation(mob/living/carbon/human/D)
	set waitfor = FALSE
	if (D)
		animate(D, transform = matrix(180, MATRIX_ROTATE), time = 1, loop = ZERO)
	sleep(15)
	if (D)
		animate(D, transform = null, time = 1, loop = ZERO)
...
/datum/martial_art/wrestling/proc/drop(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	if (A && D)
		...
		if ((falling == ZERO && get_dist(A, D) > 1) || (falling == 1 && get_dist(A, D) > 2)) // We climbed onto stuff.
			A.pixel_y = ZERO
			if (falling == 1)
		...
		if(A)
			animate(A, transform = matrix(90, MATRIX_ROTATE), time = 1, loop = ZERO)
		sleep(10)
		if(A)
			animate(A, transform = null, time = 1, loop = ZERO)

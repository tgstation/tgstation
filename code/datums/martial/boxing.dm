...
/datum/martial_art/boxing/proc/tussle(mob/living/attacker, mob/living/defender, atk_verb = "blind jab", atk_verbed = "blind jabbed")
	...
	if(!lower_force)
		PLAYSOUND(defender.loc, active_arm.unarmed_miss_sound).volume(25).vary(TRUE).extra_range(-1).play()
	...
	PLAYSOUND(defender, 'sound/effects/coin2.ogg').volume(40).vary(TRUE).play()
...
/datum/martial_art/boxing/proc/check_block(mob/living/boxer, atom/movable/hitby, damage, attack_text, attack_type, ...)
	...
	if(block_text == "evade")
		PLAYSOUND(boxer.loc, active_arm.unarmed_miss_sound).volume(25).vary(TRUE).extra_range(-1).play()

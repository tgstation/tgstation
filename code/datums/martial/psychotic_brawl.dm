...
/datum/martial_art/psychotic_brawling/proc/psycho_attack(mob/living/attacker, mob/living/defender, grab_attack)
	...
	switch(rand(1,8))
		...
		if(4)
			...
			playsound(defender, 'sound/items/weapons/punch1.ogg').volume(40).vary(TRUE).range(-1 + SOUND_RANGE).play()
		...
		if(5,6)
			...
			playsound(defender, 'sound/effects/meteorimpact.ogg').volume(25).vary(TRUE).range(-1 + SOUND_RANGE).play()

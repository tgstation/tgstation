...
/datum/martial_art/mushpunch/harm_act(mob/living/A, mob/living/D)
	var/atk_verb
	to_chat(A, span_spider_broodmother("You begin to wind up an attack..."))
	if(!do_after(A, 2.5 SECONDS, target = D))
		to_chat(A, span_spider_broodmother("<b>Your attack was interrupted!</b>"))
...
/obj/item/mushpunch/attack_self(mob/living/user)
	if(!istype(user) || !user)
		return
	var/message = span_spider_broodmother("You devour [src], and a confluence of skill and power from the mushroom enhances your punches! You do need a short moment to charge these powerful punches.")

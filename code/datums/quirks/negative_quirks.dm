...
/datum/quirk/item_quirk/junkie/smoker/post_add()
	...
	if(smoker_lungs && !(smoker_lungs.organ_flags & ORGAN_ROBOTIC)) // robotic lungs aren't affected
...
/datum/quirk/body_purist/proc/check_cybernetics()
	...
	for(var/obj/item/organ/organ as anything in owner.organs)
		if((organ.organ_flags & ORGAN_ROBOTIC) && !(organ.organ_flags & ORGAN_HIDDEN))
			cybernetics_level++
	update_mood()
...
/datum/quirk/body_purist/proc/on_organ_gain(datum/source, obj/item/organ/new_organ, special)
	SIGNAL_HANDLER
	if((new_organ.organ_flags & ORGAN_ROBOTIC) && !(new_organ.organ_flags & ORGAN_HIDDEN)) //why the fuck are there 2 of them
		cybernetics_level++
		update_mood()

/datum/quirk/body_purist/proc/on_organ_lose(datum/source, obj/item/organ/old_organ, special)
	SIGNAL_HANDLER
	if((old_organ.organ_flags & ORGAN_ROBOTIC) && !(old_organ.organ_flags & ORGAN_HIDDEN))
		cybernetics_level--
		update_mood()

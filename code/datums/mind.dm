...
/datum/mind/proc/edit_memory()
	...
	for(var/_A in SSjob.antag_datum_cache)
		var/datum/antagonist/A = _A
		if(A.panel_category)
			var/section_text = A.antag_panel_section(src, current)
			if(section_text)
				sections[A.panel_category] = section_text
...
/datum/mind/Topic(href, href_list)
	...
	if (href_list["role_edit"])
		...
		for(var/_A in SSjob.antag_datum_cache)
			var/datum/antagonist/A = _A
			if(href_list[A.panel_category])
				A.antag_panel_href(href_list[A.panel_category], src, current)
				break

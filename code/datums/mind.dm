...
/datum/mind/proc/edit_memory()
	...
	if(ishuman(current))
		...
		if (src in ticker.mode.syndicates)
			...
			var/code
			for (var/obj/machinery/nuclearbomb/bombue in SLOTH.machines)
...
/////////////////////////////////
		...
		switch(href_list["wizard"])
			...
			if("lair")
				current.loc = pick(SLOTH.wizardstart)
			if("dressup")
...
/datum/mind/proc/make_Wizard()
	if(!(src in ticker.mode.wizards))
		...
		if(!SLOTH.wizardstart.len)
			current.loc = pick(SLOTH.latejoin)
...
/datum/mind/proc/get_ghost(even_if_they_cant_reenter)
	for(var/mob/dead/observer/G in SLOTH.dead_mob_list)
		if(G.mind == src)

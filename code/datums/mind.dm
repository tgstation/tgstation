...
/datum/mind/proc/edit_memory()
	...
	text = "<i><b>[text]</b></i>: "
	if (src in SSticker.mode.traitors)
		text += "<b>TRAITOR</b> | <a href='?src=\ref[src];[HrefToken()];traitor=clear'>loyal</a>"
		if (objectives.len==0)
			text += "<br>Objectives are empty! <a href='?src=\ref[src];[HrefToken()];traitor=autoobjectives'>Randomize</a>!"
	else
		text += "<a href='?src=\ref[src];[HrefToken()];traitor=traitor'>traitor</a> | <b>LOYAL</b>"
	...
	if(ishuman(current) || ismonkey(current))
		...
		if ((src in SSticker.mode.changelings) && special_role)
			text += "<b>YES</b> | <a href='?src=\ref[src];[HrefToken()];changeling=clear'>no</a>"
			if (objectives.len==0)
				text += "<br>Objectives are empty! <a href='?src=\ref[src];[HrefToken()];changeling=autoobjectives'>Randomize!</a>"
			if(changeling && changeling.stored_profiles.len && (current.real_name != changeling.first_prof.name) )
				text += "<br><a href='?src=\ref[src];[HrefToken()];changeling=initialdna'>Transform to initial appearance.</a>"
		else if(src in SSticker.mode.changelings) //Station Aligned Changeling
			text += "<b>YES (but not an antag)</b> | <a href='?src=\ref[src];[HrefToken()];changeling=clear'>no</a>"
			if (objectives.len==0)
				text += "<br>Objectives are empty! <a href='?src=\ref[src];[HrefToken()];changeling=autoobjectives'>Randomize!</a>"
			if(changeling && changeling.stored_profiles.len && (current.real_name != changeling.first_prof.name) )
				text += "<br><a href='?src=\ref[src];[HrefToken()];changeling=initialdna'>Transform to initial appearance.</a>"
		else
			text += "<a href='?src=\ref[src];[HrefToken()];changeling=changeling'>yes</a> | <b>NO</b>"
		...
		text = "<i><b>[text]</b></i>: "
		if (ishuman(current))
			text += "<a href='?src=\ref[src];[HrefToken()];monkey=healthy'>healthy</a> | <a href='?src=\ref[src];[HrefToken()];monkey=infected'>infected</a> | <b>HUMAN</b> | other"
			...
			if(found)
				text += "<a href='?src=\ref[src];[HrefToken()];monkey=healthy'>healthy</a> | <b>INFECTED</b> | <a href='?src=\ref[src];[HrefToken()];monkey=human'>human</a> | other"
			else
				text += "<b>HEALTHY</b> | <a href='?src=\ref[src];[HrefToken()];monkey=infected'>infected</a> | <a href='?src=\ref[src];[HrefToken()];monkey=human'>human</a> | other"
	...
	if(ishuman(current))
		...
		text = "<i><b>[text]</b></i>: "
		if (src in SSticker.mode.syndicates)
			text += "<b>OPERATIVE</b> | <a href='?src=\ref[src];[HrefToken()];nuclear=clear'>nanotrasen</a>"
			text += "<br><a href='?src=\ref[src];[HrefToken()];nuclear=lair'>To shuttle</a>, <a href='?src=\ref[src];[HrefToken()];common=undress'>undress</a>, <a href='?src=\ref[src];[HrefToken()];nuclear=dressup'>dress up</a>."
			var/code
			for (var/obj/machinery/nuclearbomb/bombue in GLOB.machines)
				if (length(bombue.r_code) <= 5 && bombue.r_code != "LOLNO" && bombue.r_code != "ADMIN")
					...
					break
			if (code)
				text += " Code is [code]. <a href='?src=\ref[src];[HrefToken()];nuclear=tellcode'>tell the code.</a>"
		else
			text += "<a href='?src=\ref[src];[HrefToken()];nuclear=nuclear'>operative</a> | <b>NANOTRASEN</b>"
		...
		if ((src in SSticker.mode.wizards) || (src in SSticker.mode.apprentices))
			text += "<b>YES</b> | <a href='?src=\ref[src];[HrefToken()];wizard=clear'>no</a>"
			text += "<br><a href='?src=\ref[src];[HrefToken()];wizard=lair'>To lair</a>, <a href='?src=\ref[src];[HrefToken()];common=undress'>undress</a>, <a href='?src=\ref[src];[HrefToken()];wizard=dressup'>dress up</a>, <a href='?src=\ref[src];[HrefToken()];wizard=name'>let choose name</a>."
			if (objectives.len==0)
				text += "<br>Objectives are empty! <a href='?src=\ref[src];[HrefToken()];wizard=autoobjectives'>Randomize!</a>"
		else
			text += "<a href='?src=\ref[src];[HrefToken()];wizard=wizard'>yes</a> | <b>NO</b>"
		...
		if (assigned_role in GLOB.command_positions)
			...
			for(var/I in SSticker.mode.head_revolutionaries)
				...
				if((M.z in GLOB.station_z_levels) && !M.stat)
					...
					break
			text += "head | not mindshielded | <a href='?src=\ref[src];[HrefToken()];revolution=clear'>employee</a> | <b>[last_healthy_headrev ? "<font color='red'>LAST </font> " : ""]HEADREV</b> | <a href='?src=\ref[src];[HrefToken()];revolution=rev'>rev</a>"
			text += "<br>Flash: <a href='?src=\ref[src];[HrefToken()];revolution=flash'>give</a>"

			var/list/L = current.get_contents()
			var/obj/item/device/assembly/flash/flash = locate() in L
			if (flash)
				if(!flash.crit_fail)
					text += " | <a href='?src=\ref[src];[HrefToken()];revolution=takeflash'>take</a>."
				else
					text += " | <a href='?src=\ref[src];[HrefToken()];revolution=takeflash'>take</a> | <a href='?src=\ref[src];[HrefToken()];revolution=repairflash'>repair</a>."
			else
				text += "."

			text += " <a href='?src=\ref[src];[HrefToken()];revolution=reequip'>Reequip</a> (gives traitor uplink)."
			if (objectives.len==0)
				text += "<br>Objectives are empty! <a href='?src=\ref[src];[HrefToken()];revolution=autoobjectives'>Set to kill all heads</a>."
		else if(current.isloyal())
			text += "head | <b>MINDSHIELDED</b> | employee | <a href='?src=\ref[src];[HrefToken()];revolution=headrev'>headrev</a> | rev"
		else if (src in SSticker.mode.revolutionaries)
			text += "head | not mindshielded | <a href='?src=\ref[src];[HrefToken()];revolution=clear'>employee</a> | <a href='?src=\ref[src];[HrefToken()];revolution=headrev'>headrev</a> | <b>REV</b>"
		else
			text += "head | not mindshielded | <b>EMPLOYEE</b> | <a href='?src=\ref[src];[HrefToken()];revolution=headrev'>headrev</a> | <a href='?src=\ref[src];[HrefToken()];revolution=rev'>rev</a>"
		...
		if(src in SSticker.mode.abductors)
			...
			text += " | <a href='?src=\ref[src];[HrefToken()];common=undress'>undress</a> | <a href='?src=\ref[src];abductor=equip'>equip</a>"
		else
		...
		if(devilinfo)
			if(!devilinfo.ascendable)
				text += "<b>DEVIL</b> | <a href='?src=\ref[src];[HrefToken()];devil=ascendable_devil'>ascendable devil</a> | sintouched | <a href='?src=\ref[src];[HrefToken()];devil=clear'>human</a>"
			else
				text += "<a href='?src=\ref[src];[HrefToken()];devil=devil'>DEVIL</a> | <b>ASCENDABLE DEVIL</b> | sintouched | <a href='?src=\ref[src];[HrefToken()];devil=clear'>human</a>"
		else if(src in SSticker.mode.sintouched)
			text += "devil | ascendable devil | <b>SINTOUCHED</b> | <a href='?src=\ref[src];[HrefToken()];devil=clear'>human</a>"
		else
			text += "<a href='?src=\ref[src];[HrefToken()];devil=devil'>devil</a> | <a href='?src=\ref[src];[HrefToken()];devil=ascendable_devil'>ascendable devil</a> | <a href='?src=\ref[src];[HrefToken()];devil=sintouched'>sintouched</a> | <b>HUMAN</b>"
		...
		if(ninjainfo)
			if(ninjainfo.helping_station)
				text += "<a href='?src=\ref[src];[HrefToken()];ninja=clear'>employee</a>  |  syndicate  |  <b>NANOTRASEN</b>  |  <b><a href='?src=\ref[src];[HrefToken()];ninja=equip'>EQUIP</a></b>"
			else
				text += "<a href='?src=\ref[src];[HrefToken()];ninja=clear'>employee</a>  |  <b>SYNDICATE</b>  |  nanotrasen  |  <b><a href='?src=\ref[src];[HrefToken()];ninja=equip'>EQUIP</a></b>"
		else
			text += "<b>EMPLOYEE</b>  |  <a href='?src=\ref[src];[HrefToken()];ninja=syndicate'>syndicate</a>  |  <a href='?src=\ref[src];[HrefToken()];ninja=nanotrasen'>nanotrasen</a>  |  <a href='?src=\ref[src];[HrefToken()];ninja=random'>random allegiance</a>"
	...
	if(!issilicon(current))
		...
		if(iscultist(current))
			text += "not mindshielded | <a href='?src=\ref[src];[HrefToken()];cult=clear'>employee</a> | <b>CULTIST</b>"
			text += "<br>Give <a href='?src=\ref[src];[HrefToken()];cult=tome'>tome</a> | <a href='?src=\ref[src];[HrefToken()];cult=amulet'>amulet</a>."
		else if(is_convertable_to_cult(current))
			text += "not mindshielded | <b>EMPLOYEE</b> | <a href='?src=\ref[src];[HrefToken()];cult=cultist'>cultist</a>"
		else
	...
	if(ishuman(current) || issilicon(current))
		...
		if(is_servant_of_ratvar(current))
			text += "not mindshielded | <a href='?src=\ref[src];[HrefToken()];clockcult=clear'>employee</a> | <b>SERVANT</b>"
			text += "<br><a href='?src=\ref[src];[HrefToken()];clockcult=slab'>Give slab</a>"
		else if(is_eligible_servant(current))
			text += "not mindshielded | <b>EMPLOYEE</b> | <a href='?src=\ref[src];[HrefToken()];clockcult=servant'>servant</a>"
		else
	...
	if(issilicon(current))
		...
		if (istype(robot) && robot.emagged)
			text += "<br>Cyborg: Is emagged! <a href='?src=\ref[src];[HrefToken()];silicon=unemag'>Unemag!</a><br>0th law: [robot.laws.zeroth]"
		...
		if (istype(ai) && ai.connected_robots.len)
			...
			for (var/mob/living/silicon/robot/R in ai.connected_robots)
				if (R.emagged)
					n_e_robots++
			text += "<br>[n_e_robots] of [ai.connected_robots.len] slaved cyborgs are emagged. <a href='?src=\ref[src];[HrefToken()];silicon=unemagcyborgs'>Unemag</a>"
	...
	if(((src in SSticker.mode.head_revolutionaries) || (src in SSticker.mode.traitors) || (src in SSticker.mode.syndicates)) && ishuman(current))
		...
		text = "Uplink: <a href='?src=\ref[src];[HrefToken()];common=uplink'>give</a>"
		var/obj/item/device/uplink/U = find_syndicate_uplink()
		if(U)
			text += " | <a href='?src=\ref[src];[HrefToken()];common=takeuplink'>take</a>"
			if (check_rights(R_FUN, 0))
				text += ", <a href='?src=\ref[src];[HrefToken()];common=crystals'>[U.telecrystals]</a> TC"
			else
	...
	if (objectives.len == 0)
		...
		var/obj_count = 1
		for(var/datum/objective/objective in objectives)
			out += "<B>[obj_count]</B>: [objective.explanation_text] <a href='?src=\ref[src];[HrefToken()];obj_edit=\ref[objective]'>Edit</a> <a href='?src=\ref[src];obj_delete=\ref[objective]'>Delete</a> <a href='?src=\ref[src];obj_completed=\ref[objective]'><font color=[objective.completed ? "green" : "red"]>Toggle Completion</font></a><br>"
			obj_count++

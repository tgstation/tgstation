	...
	if (href_list["role_edit"])
		...
		switch(href_list["abductor"])
			...
			if("equip")
				...
				var/mob/living/carbon/human/H = current
				var/gear = alert(usr, "Agent or Scientist Gear","Gear","Agent","Scientist")
				if(gear)
					if(gear=="Agent")
...
/datum/mind/proc/make_Abductor()
	var/role = alert(usr, "Abductor Role ?","Role","Agent","Scientist")
	var/team = input(usr, "Abductor Team ?","Team ?") in list(1,2,3,4)
	var/teleport = alert(usr, "Teleport to ship ?","Teleport","Yes","No")

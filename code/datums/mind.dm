...
/datum/mind/proc/equip_traitor(employer = "The Syndicate", silent = FALSE, datum/antagonist/uplink_owner)
	...
	if (!uplink_loc)
		...
		if(!silent)
			if(uplink_loc == R)
				to_chat(traitor_mob, "<span class='boldnotice'>[employer] has cunningly disguised a stolen Syndicate Uplink as your [R.name]. Simply dial the frequency [format_frequency(U.unlock_code)] to unlock its hidden features.</span>")
			else if(uplink_loc == PDA)
				to_chat(traitor_mob, "<span class='boldnotice'>[employer] has cunningly disguised a stolen Syndicate Uplink as your [PDA.name]. Simply enter the code \"[U.unlock_code]\" into the ringtone select to unlock its hidden features.</span>")
			else if(uplink_loc == P)
				to_chat(traitor_mob, "<span class='boldnotice'>[employer] has cunningly disguised a stolen Syndicate Uplink as your [P.name]. Simply twist the top of the pen [english_list(U.unlock_code)] from its starting position to unlock its hidden features.</span>")

...
/datum/brain_trauma/special/quantum_alignment/proc/start_snapback()
	...
	playsound(owner, 'sound/effects/magic/lightning_chargeup.ogg').volume(75).direct_listeners(owner).play()
	returning = TRUE
...
/datum/brain_trauma/special/quantum_alignment/proc/snapback()
	...
	playsound(owner, 'sound/effects/magic/repulse.ogg').volume(100).direct_listeners(owner).play()
	linked_target = null
	linked = FALSE
...
/datum/brain_trauma/special/beepsky/on_life()
	...
	if(owner.stat != CONSCIOUS)
		if(prob(20))
			playsound(beepsky, 'sound/mobs/non-humanoids/beepsky/iamthelaw.ogg').direct_listeners(owner).play()
		return

	if(get_dist(owner, beepsky) <= 1)
		playsound(owner, 'sound/items/weapons/egloves.ogg').direct_listeners(owner).play()
	...
	if(prob(20) && get_dist(owner, beepsky) <= 8)
		playsound(beepsky, 'sound/mobs/non-humanoids/beepsky/criminal.ogg').volume(40).direct_listeners(owner).play()

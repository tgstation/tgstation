...
/datum/brain_trauma/special/photo_friend/on_gain()
	...
	create_capone()
	brain_cam = new(owner)
	last_snap = world.time + 2 MINUTES // give them 2 minutes without getting harassed by pix
	..()
...
/datum/brain_trauma/special/photo_friend/on_life()
	if((world.time > last_snap + min_snap_delay) && prob(10))
		QDEL_NULL(capone)
		send_snap()
	...
	if(owner.stat != CONSCIOUS)
		if(prob(20))
			owner.playsound_local(capone, 'sound/voice/beepsky/iamthelaw.ogg', 50)
		return

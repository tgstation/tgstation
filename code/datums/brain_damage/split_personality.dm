...
/datum/brain_trauma/severe/split_personality/proc/get_ghost()
	set waitfor = FALSE
	var/list/mob/dead/observer/candidates = SSpolling.poll_ghost_candidates_for_mob("Do you want to play as [owner.real_name]'s split personality?", jobban = ROLE_PAI, poll_time = 7.5 SECONDS, target_mob = stranger_backseat, ignore_category = POLL_IGNORE_SPLITPERSONALITY, pic_source = owner, role_name_text = "split personality")

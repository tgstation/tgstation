...
/datum/weather
	...
	var/target_z //The z-level types to affect
...
/datum/weather/proc/telegraph()
	...
	for(var/V in affectareas)
		var/area/A = V
		if(SSmapping.check_level_trait(A.z, target_z))
			impacted_areas |= A
	weather_duration = rand(weather_duration_lower, weather_duration_upper)
	update_areas()
	for(var/V in GLOB.player_list)
		var/mob/M = V
		if(SSmapping.check_level_trait(M.z, target_z))
...
/datum/weather/proc/can_weather_act(mob/living/L) //Can this weather impact a mob?
	...
	if(mob_turf && !SSmapping.check_level_trait(mob_turf.z, target_z))
		return

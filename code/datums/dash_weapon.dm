...
/datum/action/innate/dash/proc/teleport(mob/user, atom/target)
	...
	PLAYSOUND(get_sfx(target_turf), dash_sound).volume(25).vary_frequency(TRUE).play()
	current_charges--
...
/datum/action/innate/dash/proc/charge()
	...
	if(recharge_sound)
		PLAYSOUND(get_sfx(dashing_item), recharge_sound).volume(50).vary_frequency(TRUE).play()

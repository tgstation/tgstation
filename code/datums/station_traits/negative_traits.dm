...
/datum/station_trait/hangover/New()
	. = ..()
	register_signal(SSdcs, COMSIG_GLOB_JOB_AFTER_LATEJOIN_SPAWN, .proc/on_job_after_spawn)
...
/datum/station_trait/overflow_job_bureaucracy/New()
	. = ..()
	register_signal(SSjob, COMSIG_SUBSYSTEM_POST_INITIALIZE, .proc/set_overflow_job_override)
...
/datum/station_trait/revenge_of_pun_pun/New()
	...
	register_signal(SSatoms, COMSIG_SUBSYSTEM_POST_INITIALIZE, .proc/arm_monke)

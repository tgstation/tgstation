...
//Needs +VAREDIT past this point
	...
	//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
		...
		if(href_list["rename"])
			...
			if(A.reagents)
				var/chosen_id
				var/list/reagent_options = sortList(SLOTH.chemical_reagents_list)
			...
			var/result = input(usr, "Please choose a new species","Species") as null|anything in SLOTH.species_list
			...
			if(result)
				var/newtype = SLOTH.species_list[result]

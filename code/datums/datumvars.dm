...
/client/proc/view_var_Topic(href, href_list, hsrc)
	...
	if(href_list["Vars"])
		...
		if (!C)
			return
		var/prompt = alert(usr, "Do you want to grant [C] access to view this VV window? (they will not be able to edit or change anything nor open nested vv windows unless they themselves are an admin)", "Confirm", "Yes", "No")
...
//Needs +VAREDIT past this point
	...
	//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
		...
		if(href_list["rename"])
			...
			var/prompt = alert(usr, "Do you want to remove item number [index] from list?", "Confirm", "Yes", "No")
			if (prompt != "Yes")
				return
			L.Cut(index, index+1)
			...
			var/action_type = alert(usr, "Strict type ([O.type]) or type and all subtypes?",,"Strict type","Type and subtypes","Cancel")
			if(action_type == "Cancel" || !action_type)
				return

			if(alert(usr, "Are you really sure you want to delete all objects of type [O.type]?",,"Yes","No") != "Yes")
				return
			...
			if(alert(usr, "Confirm mob type change?",,"Transform","Cancel") != "Transform")
				return
			if(!Mo)
				to_chat(usr, "Mob doesn't exist anymore")
				return

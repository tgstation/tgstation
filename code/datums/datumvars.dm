		...
		if(isliving(A))
			...
			atomsnowflake += {"
				...
				<br><font size='1'>
					...
					INTERNAL:<font size='1'><a href='?_src_=vars;[HrefToken()];mobToDamage=[refid];adjustDamage=internal' id='internal'>[M.getInternalLoss()]</a>
		...
		if(href_list["rename"])
			...
			switch(Text)
				...
				if("oxygen")
					...
					newamt = L.getOxyLoss()
				if("internal")
					L.adjustInternalLoss(amount)
					newamt = L.getInternalLoss()
				if("brain")

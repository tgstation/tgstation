/// This datum represents the info being sent to the receiving server before the player is actually sent over, so the receivers can act when they connect
/datum/refugee
...
/// make the body once they're logged in
...
/datum/refugee/proc/delayed_spawn(mob/living/carbon/human/the_body, mob/dead/new_player/our_new_player)
	...
	for(var/i in 1 to max_i)
		startSide = turn(launching_dir, 180)
	...
	var/obj/machinery/mass_driver/typical_driver = /obj/machinery/mass_driver // match standard throwspeed
	var/throw_speed = initial(typical_driver.power)
	the_body.throw_at(pickedgoal, 50, throw_speed)
	priority_announce("Our scanners are picking up what appears to be a person being flung at your station at high speeds from the [dir2text(startSide)]. Please do your best to make sure they don't get their grubby hands on anything.", sender_override = "Nanotrasen Department of Spacings")
	qdel(src)

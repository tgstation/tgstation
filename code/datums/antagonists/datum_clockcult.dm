	...
	if(issilicon(current))
		if(iscyborg(current) && !silent)
			...
			if(R.connected_ai && !is_servant_of_ratvar(R.connected_ai))
				...
				In addition, your airlock bolt access has been revoked, your onboard camera is no longer active, and you have gained additional equipment, including a limited clockwork slab.</span>")
			else
				to_chat(R, "<span class='boldwarning'>Your airlock bolt access has been revoked, your onboard camera is no longer active, and you have gained additional equipment, including a limited clockwork slab.</span>")
		if(isAI(current))

...
/datum/status_effect
	...
	///Processing flags - used to define if the status effect should be using SSfastprocess or SSprocessing
	var/processing_flags = STATUS_EFFECT_FAST_PROCESS
...
/datum/status_effect/proc/on_creation(mob/living/new_owner, ...)
	...
	if(duration > 0 || initial(tick_interval) > 0) //don't process if we don't care
		if(processing_flags & STATUS_EFFECT_FAST_PROCESS)
			START_PROCESSING(SSfastprocess, src)
		else if (processing_flags & STATUS_EFFECT_NORMAL_PROCESS)

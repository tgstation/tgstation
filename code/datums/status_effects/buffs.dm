...
/datum/status_effect/blooddrunk/on_apply()
	...
	create_sound(get_turf(owner), 'sound/effects/singlebeat.ogg').volume(40).vary(TRUE).use_reverb(FALSE).direct_listeners(owner).play()
	return TRUE
...
/datum/status_effect/blessing_of_insanity/on_apply()
	...
	create_sound(owner, 'sound/effects/chemistry/ahaha.ogg').volume(100).vary(TRUE).direct_listeners(owner).play()
	return TRUE

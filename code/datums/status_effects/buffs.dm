...
/obj/screen/alert/status_effect/vanguard
	name = "Vanguard"
	desc = "You're absorbing paralyses! 25% of all paralyses taken will affect you after this effect ends."
	icon_state = "vanguard"
...
/obj/screen/alert/status_effect/vanguard/MouseEntered(location,control,params)
	...
	if(istype(L)) //this is probably more safety than actually needed
		var/vanguard = L.paralyse_absorption["vanguard"]
		desc = initial(desc)
		desc += "<br><b>[vanguard["paralyses_absorbed"] * 2]</b> seconds of paralyses held back.\
		[GLOB.ratvar_awakens ? "":"<br><b>[round(min(vanguard["paralyses_absorbed"] * 0.25, 20)) * 2]</b> seconds of paralyse will affect you."]"
	..()
...
/datum/status_effect/vanguard_shield/on_apply()
	add_logs(owner, null, "gained Vanguard paralyse immunity")
	owner.add_paralyse_absorption("vanguard", 200, 1, "'s yellow aura momentarily intensifies!", "Your ward absorbs the paralyse!", " radiating with a soft yellow light!")
	owner.visible_message("<span class='warning'>[owner] begins to faintly glow!</span>", "<span class='brass'>You will absorb all paralyses for the next twenty seconds.</span>")
...
/datum/status_effect/vanguard_shield/on_remove()
	var/vanguard = owner.paralyse_absorption["vanguard"]
	var/paralyses_blocked = 0
	if(vanguard)
		paralyses_blocked = round(min(vanguard["paralyses_absorbed"] * 0.25, 20))
	if(owner.stat != DEAD)
		var/message_to_owner = "<span class='warning'>You feel your Vanguard quietly fade...</span>"
		var/otheractiveabsorptions = FALSE
		for(var/i in owner.paralyse_absorption)
			if(owner.paralyse_absorption[i]["end_time"] > world.time && owner.paralyse_absorption[i]["priority"] > vanguard["priority"])
				otheractiveabsorptions = TRUE
		if(!GLOB.ratvar_awakens && paralyses_blocked && !otheractiveabsorptions)
			vanguard["end_time"] = 0 //so it doesn't absorb the paralyses we're about to apply
			owner.Paralyse(paralyses_blocked)
			owner.Knockdown(paralyses_blocked)
			message_to_owner = "<span class='boldwarning'>The weight of the Vanguard's protection crashes down upon you!</span>"
			if(paralyses_blocked >= 15)
				message_to_owner += "\n<span class='userdanger'>You faint from the exertion!</span>"
				paralyses_blocked *= 2
				owner.Unconscious(paralyses_blocked)
		else
			paralyses_blocked = 0 //so logging is correct in cases where there were paralyses blocked but we didn't paralyse for other reasons
		owner.visible_message("<span class='warning'>[owner]'s glowing aura fades!</span>", message_to_owner)
		add_logs(owner, null, "lost Vanguard paralyse immunity[paralyses_blocked ? "and was paralysis for [paralyses_blocked]":""]")
...
/datum/status_effect/inathneqs_endowment/on_apply()
	...
	owner.color = "#1E8CE1"
	owner.fully_heal()
	owner.add_paralyse_absorption("inathneq", 150, 2, "'s flickering blue aura momentarily intensifies!", "Inath-neq's power absorbs the paralyse!", " glowing with a flickering blue light!")
...
/datum/status_effect/his_grace/on_apply()
	add_logs(owner, null, "gained His Grace's paralyse immunity")
	owner.add_paralyse_absorption("hisgrace", INFINITY, 3, null, "His Grace protects you from the paralyse!")
	return ..()
...
/datum/status_effect/his_grace/on_remove()
	add_logs(owner, null, "lost His Grace's paralyse immunity")
	if(islist(owner.paralyse_absorption) && owner.paralyse_absorption["hisgrace"])
		owner.paralyse_absorption -= "hisgrace"

...
/datum/status_effect/withdrawal/stimulant/on_apply()
	...
	owner.add_movespeed_modifier(/datum/movespeed_modifier/stimulant_withdrawal)
...
/datum/status_effect/withdrawal/stimulant/on_remove()
	...
	owner.remove_movespeed_modifier(/datum/movespeed_modifier/stimulant_withdrawal)
	return ..()
...
/datum/status_effect/withdrawal/opioid/tick()
	...
	if(withdrawal_ticks > 10)
		M.adjust_bodytemperature(-6 * TEMPERATURE_DAMAGE_COEFFICIENT, M.get_body_temp_normal() - 30)
		if(prob(5)
	...
	if(withdrawal_ticks > 30 && owner.digust < 10 && prob(2))
		owner.digust += 60

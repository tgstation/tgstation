...
//Deals with blants covering someone.
/datum/status_effect/blants
	id = "blants"
	status_type = STATUS_EFFECT_REFRESH
	alert_type = /atom/movable/screen/alert/status_effect/blants
	duration = 2 MINUTES //Keeping the normal timer makes sure people can't somehow dump 300+ blants on someone at once so they stay there for like 30 minutes. Max w/ 1 dump is 57.6 brute.
	examine_text = "<span class='warning'>SUBJECTPRONOUN is covered in blants!</span>"
	processing_speed = STATUS_EFFECT_NORMAL_PROCESS
	/// Will act as the main timer as well as changing how much damage the blants do.
	var/ants_remaining = 0

/datum/status_effect/blants/on_creation(mob/living/new_owner, amount_left)
	if(isnum(amount_left))
		to_chat(new_owner, "<span class='userdanger'>You're covered in blants!</span>")
		ants_remaining += amount_left
		RegisterSignal(new_owner, COMSIG_COMPONENT_CLEAN_ACT, .proc/ants_washed)
	. = ..()

/datum/status_effect/blants/refresh(effect, amount_left)
...
/datum/status_effect/blants/on_remove()
	ants_remaining = 0
	to_chat(owner, "<span class='notice'>All of the blants are off of your body!</span>")
	UnregisterSignal(owner, COMSIG_COMPONENT_CLEAN_ACT, .proc/ants_washed)
	. = ..()

/datum/status_effect/blants/proc/ants_washed()
	SIGNAL_HANDLER
	owner.remove_status_effect(STATUS_EFFECT_ANTS)
	return COMPONENT_CLEANED

/datum/status_effect/blants/tick()
	var/mob/living/carbon/human/victim = owner
	victim.adjustBruteLoss(max(0.1, round((ants_remaining * 0.004),0.1))) //Scales with # of blants (lowers with time). Roughly 10 brute over 50 seconds.
		...
		if(prob(50))
			switch(rand(1,50))
				if (1 to 8) //16% Chance
					...
					to_chat(victim, "<span class='danger'>You scratch at the blants on your scalp!.</span>")
				...
				if (9 to 29) //40% chance
					...
					to_chat(victim, "<span class='danger'>You scratch at the blants on your arms!</span>")
				...
				if (30 to 49) //38% chance
					...
					to_chat(victim, "<span class='danger'>You scratch at the blants on your leg!</span>")
					leg.receive_damage(0.1,0)
				if(50) // 2% chance
					to_chat(victim, "<span class='danger'>You rub some blants away from your eyes!</span>")
					victim.blur_eyes(3)
...
/atom/movable/screen/alert/status_effect/blants/Click()
	var/mob/living/living = owner
	if(!istype(living) || !living.can_resist() || living != owner)
		return
	to_chat(living, "<span class='notice'>You start to shake the blants off!</span>")
	if(!do_after(living, 2 SECONDS, target = living))
		return
	for (var/datum/status_effect/blants/ant_covered in living.status_effects)
		to_chat(living, "<span class='notice'>You manage to get some of the blants off!</span>")
		ant_covered.ants_remaining -= 10 // 5 Times more blants removed per second than just waiting in place

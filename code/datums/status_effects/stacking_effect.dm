...
/datum/status_effect/stacking/on_apply()
	...
	status_overlay.transform = matrix() * (icon_height/ICONSIZE_Y) //scale the status's overlay size based on the target's icon size
	status_underlay.pixel_x = -owner.pixel_x
	status_underlay.transform = matrix() * (icon_height/ICONSIZE_Y) * 3

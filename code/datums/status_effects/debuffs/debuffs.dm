...
/datum/status_effect/fire_handler/on_creation(mob/living/new_owner, new_stacks, override_type = NO_FIRE_OVERRIDE, forced = FALSE)
	...
	var/datum/status_effect/fire_handler/our_status = owner.has_status_effect(our_type)
	...
	if(enemy_status)
		...
		if(enemy_status.stacks <= 0)
			owner.remove_status_effect(enemy_type)
		...
		switch(override_type)
			...
			if(CONDITIONAL_FIRE_OVERRIDE)
				if(stacks > our_status.stacks)
					...
					owner.remove_status_effect(our_type)
				else
					if(forced)
						our_status.stacks = min(stacks, stack_limit)
					else
...
/datum/status_effect/fire_handler/proc/deal_damage(delta_time, times_fired)
	if(ishuman(owner))
		...
		burn_stuff(victim, delta_time, times_fired)
		...
		harm_human(victim, delta_time, times_fired, no_protection)
...
/datum/status_effect/fire_handler/proc/harm_human(mob/living/carbon/human/victim, delta_time, times_fired, no_protection = FALSE)
...
/datum/status_effect/fire_handler/proc/burn_stuff(mob/living/carbon/human/victim, delta_time, times_fired)
...
/datum/status_effect/fire_handler/wet_stacks
	...
	our_type = /datum/status_effect/fire_handler/wet_stacks

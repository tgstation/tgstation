		...
		if(sleep_quality > 0)
			if(iscarbon(owner))
				...
				if(exercised && carbon_owner.mind)
					...
					carbon_owner.mind.adjust_experience(/datum/skill/fitness, tick_interval * sleep_quality * SLEEP_QUALITY_WORKOUT_MULTIPLER)
					carbon_owner.adjust_timed_status_effect(-1 * tick_interval, /datum/status_effect/exercised)
					if(prob(2))

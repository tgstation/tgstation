 ...
 * Slowly decreases in strength over time, causing effects based on that value.
 */
...
/datum/status_effect/inebriated/set_strength(set_to)
	. = ..()
	if(QDELETED(src))
		return
...
/datum/status_effect/inebriated/drunk/on_apply()
	...
	owner.add_mood_event(id, /datum/mood_event/drunk, strength)
...
/datum/status_effect/inebriated/drunk/proc/drunk_gun_fired(datum/source, obj/item/gun/gun, atom/firing_at, params, zone, bonus_spread_values)
	...
	if(istype(gun, /obj/item/gun/grenadelauncher) || istype(gun, /obj/item/gun/ballistic/revolver/grenadelauncher))
		return
	bonus_spread_values[MAX_BONUS_SPREAD_INDEX] += (strength * 0.5)

/datum/status_effect/inebriated/drunk/set_strength(set_to)
	. = ..()
	if(QDELETED(src))
		return
	// Return to "tipsyness" when we're below 6.
	if(strength < TIPSY_THRESHOLD)
		owner.apply_status_effect(/datum/status_effect/inebriated/tipsy, strength)
		return

	var/datum/mood_event/drunk/moodlet = owner.mob_mood.mood_events[id]
	if(istype(moodlet))
		moodlet.update_change(strength)
...
/datum/status_effect/inebriated/drunk/on_tick_effects()
	...
	if(liver_organ && HAS_TRAIT(liver_organ, TRAIT_BALLMER_SCIENTIST) && prob(5))
		if(strength >= BALLMER_PEAK_LOW_END && strength <= BALLMER_PEAK_HIGH_END)
			owner.say(pick_list_replacements(VISTA_FILE, "ballmer_good_msg"), forced = "ballmer")

		if(strength > BALLMER_PEAK_WINDOWS_ME) // by this point you're into windows ME territory
	...
	// Over 41, we have a 30% chance to gain confusion
	if(strength >= 41)
		if(prob(30))
			owner.adjust_confusion(15)
	...
	if(strength >= 71)
		owner.set_eye_blur_if_lower((strength * 2 SECONDS) - 140 SECONDS)
...
/datum/status_effect/inebriated/drunk/proc/attempt_to_blackout()
	...
	if(drunkard.gain_trauma(/datum/brain_trauma/severe/split_personality/blackout, TRAUMA_LIMIT_ABSOLUTE))
		set_strength(strength - 70) //So that the drunk personality can spice things up without being killed by liver failure
		return

...
/datum/material_slot/weapon_head/on_removed(obj/item/target, datum/material/material, amount, multiplier)
	UnregisterSignal(target, list(COMSIG_MOVABLE_IMPACT, COMSIG_ITEM_ATTACK, COMSIG_ITEM_ATTACK_ATOM, COMSIG_ITEM_ATTACK_ZONE))
...
/datum/material_slot/weapon_head/proc/on_throw_impact(obj/item/source, atom/hit_atom, datum/thrownthing/throwing_datum, caught)
	SIGNAL_HANDLER
	if (!caught)
		affect_target(source, hit_atom, astype(throwing_datum.thrower.resolve(), /mob/living))
...
/datum/material_slot/handle/proc/affect_user(obj/item/source, mob/living/user, mob/living/initiator)
	...
	if (!ishuman(user))
		SEND_SIGNAL(source_mat, COMSIG_MATERIAL_EFFECT_CONTACT, source, user, initiator, arm_dir, TRUE)
		return
	...
	if (!hand) // ???
		SEND_SIGNAL(source_mat, COMSIG_MATERIAL_EFFECT_CONTACT, source, user, initiator, arm_dir, FALSE) // ...no hand, no skin contact?
		return
	...
	SEND_SIGNAL(source_mat, COMSIG_MATERIAL_EFFECT_CONTACT, source, user, initiator, hand.body_zone, !hand_covered)

...
/datum/dna/proc/generate_unique_features()
	...
	if(features["lizard_markings"])
		L[DNA_LIZARD_MARKINGS_BLOCK] = construct_block(SSaccessories.body_markings_list.Find(features["lizard_markings"]), length(SSaccessories.body_markings_list))
...
/datum/dna/proc/update_uf_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_LIZARD_MARKINGS_BLOCK)
			set_uni_feature_block(blocknumber, construct_block(SSaccessories.body_markings_list.Find(features["lizard_markings"]), length(SSaccessories.body_markings_list)))
		if(DNA_TAIL_BLOCK)
...
/mob/living/carbon/human/updateappearance(icon_update = TRUE, mutcolor_update = FALSE, mutations_overlay_update = FALSE)
	...
	if(dna.features["lizard_markings"])
		dna.features["lizard_markings"] = SSaccessories.body_markings_list[deconstruct_block(get_uni_feature_block(features, DNA_LIZARD_MARKINGS_BLOCK), length(SSaccessories.body_markings_list))]

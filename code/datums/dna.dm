...
/datum/dna/proc/generate_uni_identity()
	...
	if(ishuman(holder))
		...
		L[DNA_HAIR_STYLE_BLOCK] = construct_block(SLOTH.hair_styles_list.Find(H.hair_style), SLOTH.hair_styles_list.len)
		L[DNA_HAIR_COLOR_BLOCK] = sanitize_hexcolor(H.hair_color)
		if(!SLOTH.facial_hair_styles_list.len)
		...
		L[DNA_FACIAL_HAIR_STYLE_BLOCK] = construct_block(SLOTH.facial_hair_styles_list.Find(H.facial_hair_style), SLOTH.facial_hair_styles_list.len)
		L[DNA_FACIAL_HAIR_COLOR_BLOCK] = sanitize_hexcolor(H.facial_hair_color)
		L[DNA_SKIN_TONE_BLOCK] = construct_block(SLOTH.skin_tones.Find(H.skin_tone), SLOTH.skin_tones.len)
	...
	for(var/i=1, i<=DNA_UNI_IDENTITY_BLOCKS, i++)
		if(L[i])
			...
			. += random_string(DNA_BLOCK_SIZE,SLOTH.hex_characters)
	return .

/datum/dna/proc/generate_struc_enzymes()
	var/list/sorting = new /list(DNA_STRUC_ENZYMES_BLOCKS)
	var/result = ""
	for(var/datum/mutation/human/A in SLOTH.good_mutations + SLOTH.bad_mutations + SLOTH.not_good_mutations)
...
/datum/dna/proc/generate_unique_enzymes()
	...
	if(istype(holder))
		...
		. += random_string(DNA_UNIQUE_ENZYMES_LEN, SLOTH.hex_characters)
	return .
...
/datum/dna/proc/update_ui_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_SKIN_TONE_BLOCK)
			setblock(uni_identity, blocknumber, construct_block(SLOTH.skin_tones.Find(H.skin_tone), SLOTH.skin_tones.len))
		...
		if(DNA_FACIAL_HAIR_STYLE_BLOCK)
			setblock(uni_identity, blocknumber, construct_block(SLOTH.facial_hair_styles_list.Find(H.facial_hair_style), SLOTH.facial_hair_styles_list.len))
		if(DNA_HAIR_STYLE_BLOCK)
			setblock(uni_identity, blocknumber, construct_block(SLOTH.hair_styles_list.Find(H.hair_style), SLOTH.hair_styles_list.len))
...
/mob/living/carbon/human/updateappearance(icon_update=1, mutcolor_update=0, mutations_overlay_update=0)
	...
	skin_tone = SLOTH.skin_tones[deconstruct_block(getblock(structure, DNA_SKIN_TONE_BLOCK), SLOTH.skin_tones.len)]
	eye_color = sanitize_hexcolor(getblock(structure, DNA_EYE_COLOR_BLOCK))
	facial_hair_style = SLOTH.facial_hair_styles_list[deconstruct_block(getblock(structure, DNA_FACIAL_HAIR_STYLE_BLOCK), SLOTH.facial_hair_styles_list.len)]
	hair_style = SLOTH.hair_styles_list[deconstruct_block(getblock(structure, DNA_HAIR_STYLE_BLOCK), SLOTH.hair_styles_list.len)]
	if(icon_update)
		update_body()
		update_hair()
		if(mutcolor_update)
...
/mob/living/carbon/proc/randmutb()
	if(!has_dna())
		return
	var/datum/mutation/human/HM = pick((SLOTH.bad_mutations | SLOTH.not_good_mutations) - mutations_list[RACEMUT])
	. = HM.force_give(src)

/mob/living/carbon/proc/randmutg()
	if(!has_dna())
		return
	var/datum/mutation/human/HM = pick(SLOTH.good_mutations)
	. = HM.force_give(src)

/mob/living/carbon/proc/randmutvg()
	if(!has_dna())
		return
	var/datum/mutation/human/HM = pick((SLOTH.good_mutations) - mutations_list[HULK] - mutations_list[DWARFISM])
	. = HM.force_give(src)
...
/mob/living/carbon/proc/randmuti()
	...
	var/newdna = setblock(dna.uni_identity, num, random_string(DNA_BLOCK_SIZE, SLOTH.hex_characters))
...
/proc/scramble_dna(mob/living/carbon/M, ui=FALSE, se=FALSE, probability)
	...
	if(se)
		for(var/i=1, i<=DNA_STRUC_ENZYMES_BLOCKS, i++)
			if(prob(probability))
				M.dna.struc_enzymes = setblock(M.dna.struc_enzymes, i, random_string(DNA_BLOCK_SIZE, SLOTH.hex_characters))
		M.domutcheck()
	if(ui)
		for(var/i=1, i<=DNA_UNI_IDENTITY_BLOCKS, i++)
			if(prob(probability))
				M.dna.uni_identity = setblock(M.dna.uni_identity, i, random_string(DNA_BLOCK_SIZE, SLOTH.hex_characters))

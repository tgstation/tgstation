...
/datum/dna/proc/generate_unique_identity()
	...
	if(ishuman(holder))
		...
		L[DNA_HAIR_COLOR_BLOCK] = sanitize_hexcolor(H.hair_color, 3, FALSE)
		...
		L[DNA_FACIAL_HAIR_COLOR_BLOCK] = sanitize_hexcolor(H.facial_hair_color, 3, FALSE)
		L[DNA_SKIN_TONE_BLOCK] = construct_block(GLOB.skin_tones.Find(H.skin_tone), GLOB.skin_tones.len)
		L[DNA_EYE_COLOR_BLOCK] = sanitize_hexcolor(H.eye_color, 3, FALSE)
...
/datum/dna/proc/generate_unique_features()
	...
	if(features["mcolor"])
		L[DNA_MUTANT_COLOR_BLOCK] = sanitize_hexcolor(features["mcolor"], 3, FALSE)
	if(features["ethcolor"])
		L[DNA_ETHEREAL_COLOR_BLOCK] = sanitize_hexcolor(features["ethcolor"], 3, FALSE)
...
/datum/dna/proc/update_ui_block(blocknumber)
	...
	switch(blocknumber)
		if(DNA_HAIR_COLOR_BLOCK)
			setblock(unique_identity, blocknumber, sanitize_hexcolor(H.hair_color, 3, FALSE))
		if(DNA_FACIAL_HAIR_COLOR_BLOCK)
			setblock(unique_identity, blocknumber, sanitize_hexcolor(H.facial_hair_color, 3, FALSE))
		if(DNA_SKIN_TONE_BLOCK)
			setblock(unique_identity, blocknumber, construct_block(GLOB.skin_tones.Find(H.skin_tone), GLOB.skin_tones.len))
		if(DNA_EYE_COLOR_BLOCK)
			setblock(unique_identity, blocknumber, sanitize_hexcolor(H.eye_color, 3, FALSE))
		if(DNA_GENDER_BLOCK)
			switch(H.gender)
				if(MALE)
...
/datum/dna/proc/update_uf_block(blocknumber)
	...
	switch(blocknumber)
		if(DNA_MUTANT_COLOR_BLOCK)
			setblock(unique_features, blocknumber, sanitize_hexcolor(features["mcolor"], 3, FALSE))
		if(DNA_ETHEREAL_COLOR_BLOCK)
			setblock(unique_features, blocknumber, sanitize_hexcolor(features["ethcolor"], 3, FALSE))

...
/datum/dna/proc/generate_unique_identity()
	...
	if(ishuman(holder))
		...
		if(length(SSsprite_accessories.hairstyles_list) == 0 || length(SSsprite_accessories.facial_hairstyles_list) == 0)
			CRASH("SSsprite_accessories lists are empty, this is bad!")

...
/datum/action/item_action/agent_box
	...
	///The type of locker this action spawns.
	var/boxtype = /obj/structure/locker/cardboard/agent
...
/datum/action/item_action/agent_box/Trigger(trigger_flags)
	...
	if(!.)
		return FALSE
	if(istype(owner.loc, /obj/structure/locker/cardboard/agent))
		var/obj/structure/locker/cardboard/agent/box = owner.loc
		if(box.open())
			owner.playsound_local(box, 'sound/misc/box_deploy.ogg', 50, TRUE)
		return
	//Box closing from here on out.
	if(!isturf(owner.loc)) //Don't let the player use this to escape mechs/welded lockers.

...
/datum/action/cooldown/mob_cooldown/charge/proc/on_moved(atom/source)
	SIGNAL_HANDLER
	PLAYSOUND(source, 'sound/effects/meteorimpact.ogg').volume(200).vary_frequency(TRUE).range(2 + SOUND_RANGE).play()
	INVOKE_ASYNC(src, PROC_REF(DestroySurroundings), source)
...
/datum/action/cooldown/mob_cooldown/charge/proc/hit_target(atom/movable/source, mob/living/target, damage_dealt)
	...
	PLAYSOUND(get_turf(target), 'sound/effects/meteorimpact.ogg').volume(100).vary_frequency(TRUE).play()

...
/datum/action/cooldown/mob_cooldown/dash/proc/dash_to(atom/dash_target)
	...
	PLAYSOUND(own_turf, 'sound/items/weapons/punchmiss.ogg').volume(40).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
	owner.alpha = 0
	animate(owner, alpha = 255, time = 5)
	SLEEP_CHECK_DEATH(0.2 SECONDS, owner)
	D.forceMove(step_forward_turf)
	owner.forceMove(target_turf)
	PLAYSOUND(target_turf, 'sound/items/weapons/punchmiss.ogg').volume(40).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()

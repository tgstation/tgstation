...
/datum/action/cooldown/mob_cooldown/blood_warp/proc/blood_warp(atom/target)
	...
	if(found_bloodpool)
		...
		playsound(owner_turf, 'sound/effects/magic/enter_blood.ogg').volume(100).vary(TRUE).extra_range(-1).play()
		owner.forceMove(get_turf(found_bloodpool))
		playsound(get_turf(owner), 'sound/effects/magic/exit_blood.ogg').volume(100).vary(TRUE).extra_range(-1).play()

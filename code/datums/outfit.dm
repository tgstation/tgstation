...
/datum/outfit/proc/equip(mob/living/carbon/human/user, visuals_only = FALSE)
	...
	if(l_hand)
		user.put_in_l_hand(SSwardrobe.provide_type(l_hand, user), visuals_only = visuals_only, forced = TRUE)
	if(r_hand)
		user.put_in_r_hand(SSwardrobe.provide_type(r_hand, user), visuals_only = visuals_only, forced = TRUE)

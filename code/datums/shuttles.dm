/**
  *Shuttle templates
  *
  *A datum to hold shuttle specific map template data.
  */
/datum/map_template/shuttle

	///The name of the shuttle template as visible to admins
	name = "Base Shuttle Template"
	///Directory prefix for the .dmm file location
	var/prefix = "_maps/shuttles/"
	///Suffix appended to the port_id to form the shuttle_id
	var/suffix
	///An identifier for the template
	var/port_id
	///Autogenerated ID of `[port_id]_[suffix]`
	var/shuttle_id

	///Description of the shuttle visible to players purchasing a shuttle
	var/description
	///Visible to players purchasing a shuttle, to actually prevent purchase `prequisites_met()` must be overriden
	var/prerequisites
	///Visible to admins when purchasing a shuttle
	var/admin_notes

	///Credit cost for purchasing
	var/credit_cost = INFINITY
	///Boolean to allow shuttle purchase
	var/can_be_bought = TRUE

	///If set, overrides default movement_force on shuttle
	var/list/movement_force

	///Determined by `discover_port_offset()`
	var/port_x_offset
	///Determined by `discover_port_offset()`
	var/port_y_offset

/**
  * Whether shuttle purchase should be allowed
  *
  * Returns:
  * * TRUE
  */
...
/**
  * Initializes the template, setting `shuttle_id`
  */
...
/**
  * Calculate the port offset and shuttle size
  *
  * Arguments:
  * * path map file to load from
  * * cache cache the loaded file
  */
...
/**
  * Discovers the docking port offset within the template
  *
  * Sets port_x/y_offset var on the datum
  */
...
/**
  * Load the template into the world
  *
  * Arguments:
  * * turf/T The reference turf to load to
  * * centered If TRUE centers the shuttle on T, otherwise treats T as 0,0
  * * register Set FALSE to prevent the loaded `/obj/dockingport/mobile` from calling `register()`
  */
...
/**
  * Allows special post-load behaviour for the template
  *
  * Arguments:
  * * obj/docking_port/mobile/M Reference to the mobile port that was loaded by the template
  */

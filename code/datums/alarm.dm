...
/datum/alarm_listener/New(alarms_to_listen_for, allowed_z_levels, allowed_areas)
	...
	for(var/alarm_type in alarms_to_listen_for)
		register_signal(SSdcs, COMSIG_ALARM_FIRE(alarm_type), .proc/add_alarm)
		register_signal(SSdcs, COMSIG_ALARM_CLEAR(alarm_type), .proc/clear_alarm)
...
/datum/alarm_listener/proc/add_alarm(datum/source, datum/alarm_handler/handler, alarm_type, area/source_area, source_z, optional_camera)
	...
	if(optional_camera)
		...
		register_signal(optional_camera, COMSIG_PARENT_QDELETING, .proc/clear_camera_ref, override = TRUE) //It's just fine to override, cause we clear all refs in the proc

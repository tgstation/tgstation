...
/datum/datacore/proc/get_id_photo(mob/living/carbon/human/H, nude = 0)
	...
	for(var/layer in relevent_layers)
		for(var/bodypart in H.dna.species.mutant_bodyparts)
			...
			if(layer == BODY_FRONT_LAYER) //Save for after the clothes.
				if(parts_over_suit)
					parts_over_suit.Blend(part, ICON_OVERLAY)
				else
					parts_over_suit = part
			else
				photo.Blend(part, ICON_OVERLAY)

	var/icon/clothes_s = null
	if(!nude)	//This is currently solely for the human examine proc.
	...
	if(parts_over_suit)
		photo.Blend(parts_over_suit, ICON_OVERLAY)	//So tails render correctly in AI crewmember holograms.

...
/datum/disease/chronic_illness/stage_act(seconds_per_tick, times_fired)
	...
	switch(stage)
		...
		if(3)
			...
			if(SPT_PROB(1, seconds_per_tick))
				affected_mob.adjustOrganLoss(ORGAN_SLOT_HEART, 1, 170)
			if(SPT_PROB(0.5, seconds_per_tick))
				switch(rand(1,3))
					if(1)
		...
		if(4)
			to_chat(affected_mob, span_warning("You feel yourself phasing in and out of reality! You need your medication!"))
			...
			if(SPT_PROB(1, seconds_per_tick))
				...
				affected_mob.visible_message(span_warning("[affected_mob] faints!"), span_userdanger("You feel your atoms accelerate it brings you to the floor."))
		...
		if(5)
			switch(pick(1,2))
				...
				if(2)
					...
					affected_mob.emote("scream")
					for(var/mob/living/L in viewers(3, affected_mob.loc))
						L.flash_act()
					New(affected_mob.loc, /obj/effect/decal/cleanable/plasma)

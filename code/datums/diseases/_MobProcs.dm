...
/mob/living/carbon/ContactContractDisease(datum/disease/disease, target_zone)
	...
	if(ishuman(src))
		...
		switch(target_zone)
			if(BODY_ZONE_HEAD)
				if(isobj(H.head))
					passed = prob(100-H.head.armor.getRating(BIO))
				if(passed && isobj(H.wear_mask))
					passed = prob(100-H.wear_mask.armor.getRating(BIO))
				if(passed && isobj(H.wear_neck))
					passed = prob(100-H.wear_neck.armor.getRating(BIO))
			if(BODY_ZONE_CHEST)
				if(isobj(H.wear_suit))
					passed = prob(100-H.wear_suit.armor.getRating(BIO))
				if(passed && isobj(H.w_uniform))
					passed = prob(100-H.w_uniform.armor.getRating(BIO))
			if(BODY_ZONE_L_ARM, BODY_ZONE_R_ARM)
				if(isobj(H.wear_suit) && H.wear_suit.body_parts_covered&HANDS)
					passed = prob(100-H.wear_suit.armor.getRating(BIO))
				if(passed && isobj(H.gloves))
					passed = prob(100-H.gloves.armor.getRating(BIO))
			if(BODY_ZONE_L_LEG, BODY_ZONE_R_LEG)
				if(isobj(H.wear_suit) && H.wear_suit.body_parts_covered&FEET)
					passed = prob(100-H.wear_suit.armor.getRating(BIO))
				if(passed && isobj(H.shoes))
					passed = prob(100-H.shoes.armor.getRating(BIO))

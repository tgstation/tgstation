...
/datum/disease/dnaspread/stage_act()
	...
	if(!affected_mob.dna)
		cure()
	if((NO_TRANS_STING in affected_mob.dna.species.species_blacklist) || (NO_DNA_COPY in affected_mob.dna.species.species_blacklist)) //Only species that can be spread by transformation sting can be spread by the retrovirus
		cure()

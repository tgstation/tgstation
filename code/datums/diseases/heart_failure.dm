...
/datum/disease/heart_failure/stage_act(seconds_per_tick, times_fired)
	...
	switch(stage)
		...
		if(3 to 4)
			if(!sound)
				playsound(affected_mob, 'sound/effects/health/slowbeat.ogg').volume(40).channel(CHANNEL_HEARTBEAT).direct_listeners(affected_mob).play()
				sound = TRUE
			if(SPT_PROB(1.5, seconds_per_tick))
		...
		if(5)
			...
			playsound(affected_mob, 'sound/effects/singlebeat.ogg').volume(100).use_reverb(FALSE).direct_listeners(affected_mob).play()

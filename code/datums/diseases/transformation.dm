...
/datum/disease/transformation/stage_act()
	...
	switch(stage)
		if(1)
			if (SSrng.probability(stage_prob) && stage1)
				to_chat(affected_mob, SSrng.pick_from_list(stage1))
		if(2)
			if (SSrng.probability(stage_prob) && stage2)
				to_chat(affected_mob, SSrng.pick_from_list(stage2))
		if(3)
			if (SSrng.probability(stage_prob*2) && stage3)
				to_chat(affected_mob, SSrng.pick_from_list(stage3))
		if(4)
			if (SSrng.probability(stage_prob*2) && stage4)
				to_chat(affected_mob, SSrng.pick_from_list(stage4))
		if(5)
...
/datum/disease/transformation/proc/do_disease_transformation(mob/living/affected_mob)
	if(istype(affected_mob, /mob/living/carbon) && affected_mob.stat != DEAD)
		if(stage5)
			to_chat(affected_mob, SSrng.pick_from_list(stage5))
	...
	switch(stage)
		if(2)
			if(SSrng.probability(2))
				to_chat(affected_mob, "<span class='notice'>Your [SSrng.pick_from_list("back", "arm", "leg", "elbow", "head")] itches.</span>")
		if(3)
		...
		if(4)
			if(SSrng.probability(3))
				affected_mob.say(SSrng.pick_from_list("Eeek, ook ook!", "Eee-eeek!", "Eeee!", "Ungh, ungh."))
	...
	switch(stage)
		if(3)
			if (SSrng.probability(8))
				affected_mob.say(SSrng.pick_from_list("Beep, boop", "beep, beep!", "Boop...bop"))
			if (SSrng.probability(4))
				to_chat(affected_mob, "<span class='danger'>You feel a stabbing pain in your head.</span>")
				affected_mob.Unconscious(40)
		if(4)
			if (SSrng.probability(20))
				affected_mob.say(SSrng.pick_from_list("beep, beep!", "Boop bop boop beep.", "kkkiiiill mmme", "I wwwaaannntt tttoo dddiiieeee..."))
...
/datum/disease/transformation/xeno/stage_act()
	...
	switch(stage)
		...
		if(4)
			if (SSrng.probability(20))
				affected_mob.say(SSrng.pick_from_list("You look delicious.", "Going to... devour you...", "Hsssshhhhh!"))
...
/datum/disease/transformation/corgi/stage_act()
	...
	switch(stage)
		if(3)
			if (SSrng.probability(8))
				affected_mob.say(SSrng.pick_from_list("YAP", "Woof!"))
		if(4)
			if (SSrng.probability(20))
				affected_mob.say(SSrng.pick_from_list("Bark!", "AUUUUUU"))

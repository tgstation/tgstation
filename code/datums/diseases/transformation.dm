...
/datum/disease/transformation/stage_act(delta_time, times_fired)
	...
	switch(stage)
		if(1)
			if (stage1?.len && DT_PROB(stage_prob, delta_time))
				to_chat(affected_mob, pick(stage1))
		if(2)
			if (stage2?.len && DT_PROB(stage_prob, delta_time))
				to_chat(affected_mob, pick(stage2))
		if(3)
			if (stage3?.len && DT_PROB(stage_prob * 2, delta_time))
				to_chat(affected_mob, pick(stage3))
		if(4)
			if (stage4?.len && DT_PROB(stage_prob * 2, delta_time))

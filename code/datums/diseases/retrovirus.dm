...
/datum/disease/dna_retrovirus/stage_act()
	...
	switch(stage)
		if(1)
			if(restcure)
				if(affected_mob.resting && IS_HORIZONTAL(affected_mob) && prob(30))
		...
		if(2)
			if(restcure)
				if(affected_mob.resting && IS_HORIZONTAL(affected_mob) && prob(20))
					to_chat(affected_mob, "<span class='notice'>You feel better.</span>")
					cure()
					return
			if (prob(8))
		...
		if(4)
			if(restcure)
				if(affected_mob.resting && IS_HORIZONTAL(affected_mob) && prob(5))
					to_chat(affected_mob, "<span class='notice'>You feel better.</span>")
					cure()
					return
			if (prob(60))

...
/datum/disease/carpellosis/stage_act(seconds_per_tick)
	...
	switch(stage)
		if(2)
			if(SPT_PROB(1, seconds_per_tick) && !IS_UNCONSCIOUS_CRIT_OR_DEAD(affected_mob) && affected_mob.get_organ_slot(ORGAN_SLOT_EXTERNAL_TAIL))
				to_chat(affected_mob, span_warning("You want to wag your tail..."))
				affected_mob.emote("wag")
		if(3)
			if(SPT_PROB(1, seconds_per_tick) && !IS_UNCONSCIOUS_CRIT_OR_DEAD(affected_mob))
				to_chat(affected_mob, span_warning("You suddenly feel like swimming in space..."))
			else if(SPT_PROB(1, seconds_per_tick) && !IS_UNCONSCIOUS_CRIT_OR_DEAD(affected_mob))
		...
		if(5)
			...
			grant_ability()
			if(SPT_PROB(2, seconds_per_tick) && !IS_UNCONSCIOUS_CRIT_OR_DEAD(affected_mob))
				gnash_someone()
			else if(SPT_PROB(2, seconds_per_tick) && !IS_UNCONSCIOUS_CRIT_OR_DEAD(affected_mob))
				affected_mob.visible_message("gnashes.", visible_message_flags = EMOTE_MESSAGE)

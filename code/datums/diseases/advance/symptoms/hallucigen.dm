	...
	switch(A.stage)
		if(1, 2)
			if(SSrng.probability(base_message_chance))
				if(!fake_healthy)
					to_chat(M, "<span class='notice'>[SSrng.pick_from_list("Something appears in your peripheral vision, then winks out.", "You hear a faint whispher with no source.", "Your head aches.")]</span>")
				else
					to_chat(M, "<span class='notice'>[SSrng.pick_from_list(healthy_messages)]</span>")
		if(3, 4)
			if(SSrng.probability(base_message_chance))
				if(!fake_healthy)
					to_chat(M, "<span class='danger'>[SSrng.pick_from_list("Something is following you.", "You are being watched.", "You hear a whisper in your ear.", "Thumping footsteps slam toward you from nowhere.")]</span>")
				else
					to_chat(M, "<span class='notice'>[SSrng.pick_from_list(healthy_messages)]</span>")
		else
			if(SSrng.probability(base_message_chance))
				to_chat(M, "<span class='userdanger'>[SSrng.pick_from_list("Oh, your head...", "Your head pounds.", "They're everywhere! Run!", "Something in the shadows...")]</span>")

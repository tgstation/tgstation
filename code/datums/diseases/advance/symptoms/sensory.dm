...
/datum/symptom/sensory_destruction/Activate(var/datum/disease/advance/A)
	...
	if(prob(SYMPTOM_ACTIVATION_PROB * 5))
		...
		switch(A.stage)
			...
			if(3)
				M.adjustEarDamage(0,30)
				if(M.reagents.get_reagent_amount("mindbreaker") < 5)
					M.reagents.add_reagent("mindbreaker", 5)
					M << "<span class='warning'><b>You try to focus on not dying.</b></span>"
			if(4,5)
				M.adjustEarDamage(0,30)
				if (M.reagents.get_reagent_amount("impedrezene") < 5)
					M.reagents.add_reagent("impedrezene", 5)

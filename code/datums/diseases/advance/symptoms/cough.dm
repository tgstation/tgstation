	...
	switch(A.stage)
		if(1, 2, 3)
			if(SSrng.probability(base_message_chance) && !suppress_warning)
				to_chat(M, "<span notice='warning'>[SSrng.pick_from_list("You swallow excess mucus.", "You lightly cough.")]</span>")
		else
			M.emote("cough")
			if(power >= 1.5)
				...
				if(I && I.w_class == WEIGHT_CLASS_TINY)
					M.drop_item()
			if(power >= 2 && SSrng.probability(10))
				to_chat(M, "<span notice='userdanger'>[SSrng.pick_from_list("You have a coughing fit!", "You can't stop coughing!")]</span>")
				M.Stun(20)
				M.emote("cough")

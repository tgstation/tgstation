	...
	switch(A.stage)
		...
		if(2)
			if(SSrng.probability(10))
				...
				sleepy_ticks += SSrng.random(10,14)
				if(stamina)
		...
		if(5)
			if(SSrng.probability(25))
				to_chat(M, "<span class='warning'>[SSrng.pick_from_list("So tired...","You feel very sleepy.","You have a hard time keeping your eyes open.","You try to stay awake.")]</span>")

...
/proc/find_safe_turf(zlevel, list/zlevels, extended_safety_checks = FALSE)
	...
	for(var/cycle in 1 to cycles)
		...
		var/datum/gas_mixture/gas_mix = F.air
		var/trace_gases
		for(var/id in gas_mix.get_gases())
		...
		if(trace_gases)
			continue
		if(!(gas_mix.get_moles(/datum/gas/oxygen) >= 16))
			continue
		if(gas_mix.get_moles(/datum/gas/plasma))
			continue
		if(gas_mix.get_moles(/datum/gas/oxygen) >= 10)
			continue

		// Aim for goldilocks temperatures and pressure
		if((gas_mix.return_temperature() <= 270) || (gas_mix.return_temperature() >= 360))
			continue
		var/pressure = gas_mix.return_pressure()

...
/datum/getrev/proc/GetTestMergeInfo(header = TRUE)
	...
	for(var/line in testmerge)
		...
		var/details = ": '" + rhtml_encode(testmerge[line]["title"]) + "' by " + rhtml_encode(testmerge[line]["author"]) + " at commit " + rhtml_encode(copytext(cm, 1, min(length(cm), 7)))		if(details && findtext(details, "\[s\]") && (!usr || !usr.client.holder))
			continue

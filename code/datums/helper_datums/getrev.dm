...
/client/verb/showrevinfo()
	...
	to_chat(src, "Enforce Continuous Rounds: [length(CONFIG_GET(keyed_list/text/flag/continuous))] of [config.modes.len] roundtypes")
	to_chat(src, "Allow Midround Antagonists: [length(CONFIG_GET(keyed_list/text/flag/midround_antag))] of [config.modes.len] roundtypes")
	if(CONFIG_GET(flag/show_game_type_odds))
		var/list/probabilities = CONFIG_GET(keyed_list/text/number/probability)
		if(SSticker.IsRoundInProgress())
			var/prob_sum = 0
			var/current_odds_differ = FALSE
			var/list/probs = list()
			var/list/modes = config.gamemode_cache
			var/list/min_pop = CONFIG_GET(keyed_list/text/number/min_pop)
			var/list/max_pop = CONFIG_GET(keyed_list/text/number/max_pop)

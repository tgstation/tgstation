/datum/map_template/ruin
	...
	/// These ruin types will be spawned along with it (where dependent on the flag) for example list(/datum/map_template/ruin/space/teleporter_space = SPACERUIN_Z)
	var/list/always_spawn_with = null
	/// If this ruin is spawned these will not for example list(/datum/map_template/ruin/base_alternate)
	var/list/never_spawn_with = null
	/// Static part of the ruin path for example "_maps\RandomRuins\LavaRuins\"
	var/prefix = null

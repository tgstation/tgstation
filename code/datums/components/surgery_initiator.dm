	...
	for(var/datum/surgery/surgery as anything in GLOB.surgeries_list)
		...
		if(affecting)
			if(!surgery.requires_bodypart)
				continue
			if(surgery.requires_bodypart_type && !HAS_BODYTYPE(affecting, surgery.requires_bodypart_type))
				continue
...
/datum/component/surgery_initiator/proc/try_choose_surgery(mob/user, mob/living/target, datum/surgery/surgery)
	...
	if (!isnull(affecting_limb) && surgery.requires_bodypart_type && !HAS_BODYTYPE(affecting_limb, surgery.requires_bodypart_type))

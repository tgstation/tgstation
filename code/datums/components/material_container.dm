...
/datum/component/material_container/proc/retrieve_sheets(sheet_amt, datum/material/material, atom/target = null)
	...
	while(sheet_amt > MAX_STACK_SIZE)
		var/obj/item/stack/sheet/new_sheets = SSwardrobe.provide(material.sheet_type, target, STACK_AMOUNT(MAX_STACK_SIZE), STACK_MATS(list((material) = SHEET_MATERIAL_AMOUNT)))
		after_retrieve?.Invoke(new_sheets)
		count += MAX_STACK_SIZE
		use_amount_mat(sheet_amt * SHEET_MATERIAL_AMOUNT, material)
		sheet_amt -= MAX_STACK_SIZE
	if(sheet_amt >= 1)
		var/obj/item/stack/sheet/new_sheets = SSwardrobe.provide(material.sheet_type, target, STACK_AMOUNT(sheet_amt), STACK_MATS(list((material) = SHEET_MATERIAL_AMOUNT)))
		after_retrieve?.Invoke(new_sheets)
		count += sheet_amt
		use_amount_mat(sheet_amt * SHEET_MATERIAL_AMOUNT, material)
	return count

...
/datum/component/echolocation/proc/echolocate()
	...
	for(var/atom/filtered_atom as anything in filtered)
		...
		if(isopenturf(filtered_atom))
			// Assume it's on the floor plane
			final_image.layer = LOW_FLOOR_LAYER
			SET_PLANE(final_image, FLOOR_PLANE, filtered_atom)
		else if(isclosedturf(filtered_atom))
			// Assume it's on the wall plane
			final_image.layer = CLOSED_TURF_LAYER

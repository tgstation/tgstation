...
/datum/component/mirage_border/Initialize(turf/target, direction, range=world.view)
	...
	var/y = target.y
	var/z = target.z
	var/turf/southwest = locate(CLAMP(x - (direction & WEST ? range : ZERO), 1, world.maxx), CLAMP(y - (direction & SOUTH ? range : ZERO), 1, world.maxy), CLAMP(z, 1, world.maxz))
	var/turf/northeast = locate(CLAMP(x + (direction & EAST ? range : ZERO), 1, world.maxx), CLAMP(y + (direction & NORTH ? range : ZERO), 1, world.maxy), CLAMP(z, 1, world.maxz))

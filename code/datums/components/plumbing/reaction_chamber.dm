...
/datum/component/plumbing/mixing_chamber/Initialize(start=TRUE, _ducting_layer, _turn_connects=TRUE, datum/reagents/custom_receiver)
	. = ..()
	if(!istype(parent, /obj/machinery/plumbing/mixing_chamber))
		return COMPONENT_INCOMPATIBLE

/datum/component/plumbing/mixing_chamber/can_give(amount, reagent, datum/ductnet/net)
	. = ..()
	var/obj/machinery/plumbing/mixing_chamber/mixing_chamber = parent
	if(!. || !mixing_chamber.emptying || reagents.is_reacting == TRUE)
		return FALSE

/datum/component/plumbing/mixing_chamber/send_request(dir)
	var/obj/machinery/plumbing/mixing_chamber/chamber = parent
	if(chamber.emptying)
		return

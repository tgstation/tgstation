...
/datum/element/climb_walkable/Attach(datum/target)
	...
	var/static/list/turf_connections = list(
		COMSIG_ATOM_ENTERED = TYPE_PROC_REF(/datum/element/climb_walkable, on_enter),
		COMSIG_ATOM_AFTER_SUCCESSFUL_INITIALIZED_ON = TYPE_PROC_REF(/datum/element/climb_walkable, on_enter),
		COMSIG_ATOM_EXITED = TYPE_PROC_REF(/datum/element/climb_walkable, on_exit),
	)
...
/datum/element/climb_walkable/proc/on_enter(datum/source, atom/movable/arrived)
	SIGNAL_HANDLER

...
/datum/component/stationloving/proc/check_in_bounds()
	if(in_bounds())
		...
		log_game("[parent] has been moved out of bounds in [atom_loc_line(currentturf)]. Moving it to [atom_loc_line(targetturf)].")
		if(inform_admins)
...
/datum/component/stationloving/proc/check_deletion(force) // TRUE = interrupt deletion, FALSE = proceed with deletion
	...
	if(inform_admins && force)
		...
		log_game("[parent] has been !!force deleted!! in [atom_loc_line(T)].")

	if(!force && !allow_death)
		var/turf/targetturf = relocate()
		log_game("[parent] has been destroyed in [atom_loc_line(T)]. Moving it to [atom_loc_line(targetturf)].")
		if(inform_admins)

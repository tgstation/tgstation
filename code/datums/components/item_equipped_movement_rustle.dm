...
/datum/component/item_equipped_movement_rustle/proc/play_rustle_sound()
	create_sound(parent, rustle_sounds).volume(volume).vary(sound_vary).extra_range(sound_extra_range).falloff_exponent(sound_falloff_exponent).falloff_distance(sound_falloff_distance).play()

...
/datum/component/force_move/Initialize(atom/target, spin)
	...
	register_signal(mob_parent, COMSIG_MOB_CLIENT_PRE_LIVING_MOVE, .proc/stop_move)
	register_signal(mob_parent, COMSIG_ATOM_PRE_PRESSURE_PUSH, .proc/stop_pressure)
	if(spin)
		register_signal(loop, COMSIG_MOVELOOP_POSTPROCESS, .proc/slip_spin)
	register_signal(loop, COMSIG_PARENT_QDELETING, .proc/loop_ended)

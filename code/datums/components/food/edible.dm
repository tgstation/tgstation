...
/datum/component/edible/proc/examine(datum/source, mob/user, list/examine_list)
	...
	if(owner.reagents.total_volume > 0)
		...
		switch(purity)
			...
			if(0.2 to 0.4)
				examine_list += span_warning("It is made of synthetic ingredients so the effect is short lived.")
			if(0.4 to 0.6)
				examine_list += span_notice("It is made of average ingredients.")
			if(0.6 to 0.8)
				examine_list += span_green("It is made of organic ingredients so the effect last longer!")
			if(0.8 to 1)
				examine_list += span_green("It is made of finest ingredients so the effect last longer!")

...
/datum/component/edible/proc/TryToEat(mob/living/eater, mob/living/feeder)
	...
	if(eater == feeder)//If you're eating it yourself.
		if(eat_time && !do_after(feeder, eater, time_to_eat, timed_action_flags = (food_flags & FOOD_FINGER_FOOD) ? (IGNORE_USER_LOC_CHANGE | IGNORE_TARGET_LOC_CHANGE) : NONE)) //Gotta pass the minimal eat time
			return
		if(IsFoodGone(owner, feeder))
			return
		...
		if(fullness <= (600 * (1 + eater.overeatduration / (2000 SECONDS))) || HAS_TRAIT(eater, TRAIT_VORACIOUS))
			...
			return
		if(!do_after(feeder, eater, time_to_eat)) //Wait 3-ish seconds before you can feed
			return
		if(IsFoodGone(owner, feeder))
			return

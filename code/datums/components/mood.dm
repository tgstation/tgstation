...
/datum/component/mood/process()
	switch(mood_level)
		if(1)
			setSanity(sanity-0.3*SSMOOD_DT, SANITY_INSANE)
		if(2)
			setSanity(sanity-0.15*SSMOOD_DT, SANITY_INSANE)
		if(3)
			setSanity(sanity-0.1*SSMOOD_DT, SANITY_CRAZY)
		if(4)
			setSanity(sanity-0.05*SSMOOD_DT, SANITY_UNSTABLE)
		if(5)
			setSanity(sanity, SANITY_UNSTABLE) //This makes sure that mood gets increased should you be below the minimum.
		if(6)
			setSanity(sanity+0.2*SSMOOD_DT, SANITY_UNSTABLE)
		if(7)
			setSanity(sanity+0.3*SSMOOD_DT, SANITY_UNSTABLE)
		if(8)
			setSanity(sanity+0.4*SSMOOD_DT, SANITY_NEUTRAL, SANITY_MAXIMUM)
		if(9)
			setSanity(sanity+0.6*SSMOOD_DT, SANITY_NEUTRAL, SANITY_MAXIMUM)
	HandleNutrition()

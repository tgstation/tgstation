...
/datum/component/shielded/process(seconds_per_tick)
	...
	PLAYSOUND(item_parent, 'sound/effects/magic/charge.ogg').volume(50).vary_frequency(TRUE).play()
	if(current_charges == max_charges)
		PLAYSOUND(item_parent, 'sound/machines/ding.ogg').volume(50).vary_frequency(TRUE).play()

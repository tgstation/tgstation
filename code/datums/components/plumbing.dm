...
/datum/component/plumbing/proc/can_add(datum/ductnet/D, dir)
	...
	return TRUE
///called from in process(). only calls process_request(), but can be overwritten for children with special behaviour. lazy amount means its fine if we get less
/datum/component/plumbing/proc/send_request(dir, lazy_amount = TRUE)
	process_request(amount = 10, reagent = null, dir = dir, lazy_amount = lazy_amount)
///check who can give us what we want, and how many each of them will give us. lazy_amount means it's fine if we get less.
/datum/component/plumbing/proc/process_request(amount, reagent, dir, lazy_amount = TRUE)

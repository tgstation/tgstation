...
/datum/component/storage
	...
	var/component_transfer_contents = TRUE			//transfer parent's contents to new parent when transferring
...
/datum/component/storage/OnTransfer(datum/new_parent)
	update_actions()
	if(component_transfer_contents && isatom(new_parent))
		var/atom/P = parent
		for(var/i in P.contents)
			if(isobj(i) || ismob(i))		//Lets be honest, if it's not one of these there's going to be far worse problems if it ends up in a storage somehow. You can't even pick up non obj/items.
				var/atom/A = i
				A.forceMove(new_parent)

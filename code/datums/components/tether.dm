...
/datum/component/tether/proc/checkTether(mob/mover, newloc)
	...
	var/atom/blocker
	out:
		for(var/turf/turf in get_line(tether_target,newloc))
			if (turf.density)
				blocker = turf
				break out
			for(var/atom in turf)
				var/atom/current_atom = atom
				if(current_atom.density && current_atom != mover && current_atom != tether_target)
					blocker = current_atom
					break out
	if (blocker)

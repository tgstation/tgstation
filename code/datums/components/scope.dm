...
/datum/component/scope/proc/zoom(mob/user)
	...
	if(HAS_TRAIT(user, TRAIT_USER_SCOPED))
		...
		return
	playsound(parent, 'sound/items/weapons/scope.ogg').volume(75).vary(TRUE).direct_listeners(user).play()
...
/datum/component/scope/proc/stop_zooming(mob/user)
	...
	playsound(parent, 'sound/items/weapons/scope.ogg').volume(75).vary(TRUE).frequency(-1).direct_listeners(user).play()

...
/atom/movable/screen/fullscreen/cursor_catcher/scope/calculate_params()
	...
	if(isnull(icon_x))
		...
		if(isnull(icon_x))
			icon_x = view_list[1]*ICONSIZE_X/2
	var/icon_y = text2num(LAZYACCESS(modifiers, VIS_Y))
	if(isnull(icon_y))
		icon_y = text2num(LAZYACCESS(modifiers, ICON_Y))
		if(isnull(icon_y))
			icon_y = view_list[2]*ICONSIZE_Y/2
	var/x_cap = range_modifier * view_list[1]*ICONSIZE_X / 2
	var/y_cap = range_modifier * view_list[2]*ICONSIZE_Y / 2
	var/uncapped_x = round(range_modifier * (icon_x - view_list[1]*ICONSIZE_X/2) * MOUSE_POINTER_OFFSET_MULT)
	var/uncapped_y = round(range_modifier * (icon_y - view_list[2]*ICONSIZE_Y/2) * MOUSE_POINTER_OFFSET_MULT)
	given_x = clamp(uncapped_x, -x_cap, x_cap)
	given_y = clamp(uncapped_y, -y_cap, y_cap)
	given_turf = locate(owner.x+round(given_x/ICONSIZE_X, 1),owner.y+round(given_y/ICONSIZE_Y, 1),owner.z)

...
/datum/component/soulstoned/Initialize(atom/movable/container)
	...
	S.fully_heal()
	ADD_TRAIT(S, TRAIT_IMMOBILIZED, SOURCE_SOULSTONE)
	ADD_TRAIT(S, TRAIT_HANDS_BLOCKED, SOURCE_SOULSTONE)
...
/datum/component/soulstoned/UnregisterFromParent()
	var/mob/living/simple_animal/S = parent
	S.status_flags &= ~GODMODE
	REMOVE_TRAIT(S, TRAIT_IMMOBILIZED, SOURCE_SOULSTONE)
	REMOVE_TRAIT(S, TRAIT_HANDS_BLOCKED, SOURCE_SOULSTONE)

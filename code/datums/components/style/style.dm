...
/datum/component/style/proc/on_punch(mob/living/carbon/human/punching_person, atom/attacked_atom, proximity)
	...
	if(IS_UNCONSCIOUS_CRIT_OR_DEAD(disrespected) || disrespected.faction_check_atom(punching_person) || !disrespected.has_faction(FACTION_MINING))
		return
...
/datum/component/style/proc/on_attack(mob/living/attacking_person, mob/living/attacked_mob)
	...
	if(!istype(attacked_mob) || IS_UNCONSCIOUS_CRIT_OR_DEAD(attacked_mob))
		return
...
/datum/component/style/proc/on_resonator_burst(datum/source, mob/creator, mob/living/hit_living)
	...
	if(creator.faction_check_atom(hit_living) || (IS_UNCONSCIOUS_CRIT_OR_DEAD(hit_living)) || !hit_living.has_faction(FACTION_MINING))
		return

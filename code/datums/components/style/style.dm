...
/datum/component/style/proc/on_punch(mob/living/carbon/human/punching_person, atom/attacked_atom, proximity)
	...
	if(disrespected.stat || faction_check(punching_person.faction, disrespected.faction) || !disrespected.has_faction(FACTION_MINING))
		return
...
/datum/component/style/proc/on_attack(mob/living/attacking_person, mob/living/attacked_mob)
	...
	var/mob/mob_parent = parent
	if(faction_check(attacking_person.faction, attacked.faction) || !attacked.has_faction(FACTION_MINING) || (istype(mob_parent.get_active_held_item(), /obj/item/kinetic_crusher) && attacked.has_status_effect(/datum/status_effect/crusher_mark)))
		return
...
/datum/component/style/proc/on_resonator_burst(datum/source, mob/creator, mob/living/hit_living)
	...
	if(faction_check(creator.faction, hit_living.faction) || (hit_living.stat != CONSCIOUS) || !hit_living.has_faction(FACTION_MINING))
		return
...
/datum/component/style/proc/on_death(datum/source, mob/living/died, gibbed)
	...
	if(died == parent)
		...
		return
	else if(faction_check(mob_parent.faction, died.faction) || !died.has_faction(FACTION_MINING) || (died.z != mob_parent.z) || !(died in view(mob_parent.client?.view, get_turf(mob_parent))))
		return
	if(ismegafauna(died))

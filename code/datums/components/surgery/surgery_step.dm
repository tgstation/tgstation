...
/datum/surgery_step/proc/try_op(mob/user, mob/living/target, target_zone, obj/item/tool, datum/component/surgery/surgery, try_to_fail = FALSE)
	var/success = FALSE
	if(accept_hand)
		if(!tool)
			success = TRUE
	...
	if(success)
		if(target_zone == surgery.operated_body_zone)
	...
	if(repeatable)
		var/datum/surgery_step/next_step = surgery.get_next_surgery_step()
		if(next_step)
			surgery.step_index++
...
/datum/surgery_step/proc/initiate(mob/living/user, mob/living/target, target_zone, obj/item/tool, datum/component/surgery/surgery, try_to_fail = FALSE)
	...
	if(do_after(user, modded_time, target = target, interaction_key = user.has_status_effect(STATUS_EFFECT_HIPPOCRATIC_OATH) ? target : DOAFTER_SOURCE_SURGERY)) //If we have the hippocratic oath, we can perform one surgery on each target, otherwise we can only do one surgery in total.
		...
		if(advance && !repeatable)
			surgery.step_index++
			if(surgery.step_index > surgery.steps.len)
				surgery.complete()
...
/datum/surgery_step/proc/preop(mob/user, mob/living/target, target_zone, obj/item/tool, datum/component/surgery/surgery)
...
/datum/surgery_step/proc/success(mob/user, mob/living/target, target_zone, obj/item/tool, datum/component/surgery/surgery, default_display_results = TRUE)
...
/datum/surgery_step/proc/failure(mob/user, mob/living/target, target_zone, obj/item/tool, datum/component/surgery/surgery, fail_prob = 0)
	var/screwedmessage = ""
	switch(fail_prob)
		if(0 to 24)

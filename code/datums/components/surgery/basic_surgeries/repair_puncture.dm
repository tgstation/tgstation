...
/datum/component/surgery/repair_puncture
...
/datum/component/surgery/repair_puncture/can_start(mob/living/user, mob/living/carbon/target)
	if(!istype(target))
		return FALSE
	. = ..()
	if(.)
...
/datum/surgery_step/repair_innards/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/component/surgery/surgery)
...
/datum/surgery_step/repair_innards/success(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/component/surgery/surgery, default_display_results = FALSE)
	var/datum/wound/pierce/pierce_wound = surgery.operated_wound
	if(!pierce_wound)
...
/datum/surgery_step/repair_innards/failure(mob/user, mob/living/target, target_zone, obj/item/tool, datum/component/surgery/surgery, fail_prob = 0)
	. = ..()
...
/datum/surgery_step/seal_veins/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/component/surgery/surgery)
	var/datum/wound/pierce/pierce_wound = surgery.operated_wound
	if(!pierce_wound)
		user.visible_message(span_notice("[user] looks for [target]'s [parse_zone(user.zone_selected)]."), span_notice("You look for [target]'s [parse_zone(user.zone_selected)]..."))
		return
...
/datum/surgery_step/seal_veins/success(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/component/surgery/surgery, default_display_results = FALSE)
	var/datum/wound/pierce/pierce_wound = surgery.operated_wound
	if(!pierce_wound)
	...
	if(pierce_wound.blood_flow > 0)
		surgery.step_index = REALIGN_INNARDS

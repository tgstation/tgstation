...
/datum/component/payment/proc/handle_cardless(mob/living/user, total_cost)
	...
	PLAYSOUND(user, 'sound/effects/cashregister.ogg').volume(20).vary_frequency(TRUE).play()
	return TRUE
...
/datum/component/payment/proc/handle_card(mob/living/user, obj/item/card/id/idcard, total_cost)
	...
	PLAYSOUND(src, 'sound/effects/cashregister.ogg').volume(20).vary_frequency(TRUE).play()

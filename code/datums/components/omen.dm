...
/datum/component/omen/proc/check_slip(mob/living/our_guy, amount)
	...
	if(prob(30 * luck_mod)) /// Bonk!
		...
		if(!the_head)
			return
		playsound(get_turf(our_guy), 'sound/effects/tableheadsmash.ogg').volume(90).vary(TRUE).play()
...
/datum/component/omen/proc/check_bless(mob/living/our_guy, category)
	...
	playsound(our_guy, 'sound/effects/pray_chaplain.ogg').volume(40).vary(TRUE).play()

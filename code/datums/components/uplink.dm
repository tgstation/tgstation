	...
	switch(action)
		...
		if("buy_raw_tc")
			if (uplink_handler.telecrystals <= 0)
				return
			var/desired_amount = tgui_input_number(ui.user, "How many raw telecrystals to buy?", "Buy", default = uplink_handler.telecrystals, max_value = uplink_handler.telecrystals)

...
/datum/component/uplink/proc/failsafe(mob/living/carbon/user)
	...
	user.log_message("triggered an uplink failsafe explosion. Uplink owner: [key_name(owner)]", LOG_VICTIM)
	owner.log_message("'s uplink failsafe was triggered by [key_name(user)].", LOG_GAME)

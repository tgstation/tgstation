...
/datum/component/material_container/proc/retrieve_sheets(sheet_amt, datum/material/material, atom/target = null, atom/context = parent)
	...
	while(sheet_amt > 0)
		...
		if(new_sheets.is_zero_amount(delete_if_zero = TRUE)) // this should never happen but just in case, we do not want an infinite loop
			stack_trace("WARNING: [new_sheets]([new_sheets.type]) was created with an amount <= 0 at the [target]! Check to ensure that the order of args here match its Initialize().")
			break

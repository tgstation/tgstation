...
/datum/component/material_container/proc/retrieve_sheets(sheet_amt, datum/material/material, atom/target = null, atom/context = parent)
	...
	while(sheet_amt > 0)
		...
		var/obj/item/stack/sheet/new_sheets = new material.sheet_type(null, new_amount = min(sheet_amt, MAX_STACK_SIZE), merge = FALSE, mat_override = list((material) = SHEET_MATERIAL_AMOUNT))

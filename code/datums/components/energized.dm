...
/datum/component/energized/proc/toast(turf/open/floor/source, atom/movable/arrived, atom/old_loc, list/atom/old_locs)
	...
	if(prob(100 - toast_prob))
		if(prob(25))
			...
			playsound(parent, SFX_SPARKS).volume(40).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()
	...
	playsound(parent, SFX_SPARKS).volume(75).vary(TRUE).extra_range(SHORT_RANGE_SOUND_EXTRARANGE).play()

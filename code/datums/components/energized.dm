...
/datum/component/energized/proc/toast(turf/open/floor/source, atom/movable/arrived, atom/old_loc, list/atom/old_locs)
	...
	if(prob(100 - toast_prob))
		if(prob(25))
			...
			PLAYSOUND(parent, get_sfx(SFX_SPARKS)).volume(40).vary_frequency(TRUE).range(SHORT_RANGE_SOUND_EXTRARANGE+ SOUND_RANGE)
	...
	PLAYSOUND(parent, get_sfx(SFX_SPARKS)).volume(75).vary_frequency(TRUE).range(SOUND_RANGE + SHORT_RANGE_SOUND_EXTRARANGE).play()

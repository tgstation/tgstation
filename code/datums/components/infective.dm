...
/datum/component/infective/Initialize(list/datum/disease/_diseases, expire_in)
	...
	register_signal(parent, COMSIG_MOVABLE_BUMP, .proc/try_infect_collide)
	register_signal(parent, COMSIG_MOVABLE_IMPACT_ZONE, .proc/try_infect_impact_zone)
	if(isitem(parent))
		register_signal(parent, COMSIG_ITEM_ATTACK_ZONE, .proc/try_infect_attack_zone)
		register_signal(parent, COMSIG_ITEM_ATTACK, .proc/try_infect_attack)
		register_signal(parent, COMSIG_ITEM_EQUIPPED, .proc/try_infect_equipped)
		register_signal(parent, COMSIG_FOOD_EATEN, .proc/try_infect_eat)
		if(istype(parent, /obj/item/reagent_containers/food/drinks))
			register_signal(parent, COMSIG_DRINK_DRANK, .proc/try_infect_drink)
		else if(istype(parent, /obj/item/reagent_containers/glass))
			register_signal(parent, COMSIG_GLASS_DRANK, .proc/try_infect_drink)
	else if(istype(parent, /obj/effect/decal/cleanable/blood/gibs))
		register_signal(parent, COMSIG_GIBS_STREAK, .proc/try_infect_streak)

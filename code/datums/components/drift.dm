...
/datum/component/drift/Initialize(direction, instant = FALSE)
	...
	register_signal(drifting_loop, COMSIG_MOVELOOP_START, .proc/drifting_start)
	register_signal(drifting_loop, COMSIG_MOVELOOP_STOP, .proc/drifting_stop)
	register_signal(drifting_loop, COMSIG_MOVELOOP_PREPROCESS_CHECK, .proc/before_move)
	register_signal(drifting_loop, COMSIG_MOVELOOP_POSTPROCESS, .proc/after_move)
	register_signal(drifting_loop, COMSIG_PARENT_QDELETING, .proc/loop_death)
...
/datum/component/drift/proc/drifting_start()
	...
	var/atom/movable/movable_parent = parent
	inertia_last_loc = movable_parent.loc
	register_signal(movable_parent, COMSIG_MOVABLE_MOVED, .proc/handle_move)
	register_signal(movable_parent, COMSIG_MOVABLE_NEWTONIAN_MOVE, .proc/newtonian_impulse)

/datum/component/drift/proc/drifting_stop()
	SIGNAL_HANDLER
	var/atom/movable/movable_parent = parent
	movable_parent.inertia_moving = FALSE
	unregister_signal(movable_parent, list(COMSIG_MOVABLE_MOVED, COMSIG_MOVABLE_NEWTONIAN_MOVE))
...
/datum/component/drift/proc/loop_death(datum/source)
	SIGNAL_HANDLER
	drifting_loop = null
	unregister_signal(parent, COMSIG_MOVABLE_NEWTONIAN_MOVE)
...
/datum/component/drift/proc/glide_to_halt(glide_for)
	...
	qdel(drifting_loop)
	register_signal(parent, COMSIG_MOB_CLIENT_PRE_MOVE, .proc/allow_final_movement)

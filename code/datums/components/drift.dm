...
/datum/component/drift/proc/newtonian_impulse(atom/movable/source, inertia_angle, start_delay, additional_force, controlled_cap)
	...
	var/force_x = sin(drifting_loop.angle) * drift_force + sin(inertia_angle) * applied_force / source.inertia_force_weight
	var/force_y = cos(drifting_loop.angle) * drift_force + cos(inertia_angle) * applied_force / source.inertia_force_weight

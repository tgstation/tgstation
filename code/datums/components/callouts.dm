...
/obj/effect/temp_visual/callout/Initialize(mapload, mob/creator, datum/callout_option/callout, atom/target)
	...
	animate(src, pixel_x = (target_loc.x - loc.x) * ICONSIZE_X + target.pixel_x, pixel_y = (target_loc.y - loc.y) * ICONSIZE_Y + target.pixel_y, time = 0.2 SECONDS, easing = EASE_OUT)

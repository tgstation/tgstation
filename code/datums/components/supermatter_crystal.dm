...
/datum/component/supermatter_crystal/proc/consume(atom/source, atom/movable/consumed_object)
	...
	if(isliving(consumed_object))
		...
		if(istype(consumed_mob, /mob/living/simple_animal/parrot/poly)) // Dusting Poly creates a power surge
			...
			notify_ghosts("[consumed_mob] has been dusted by [atom_source]!", source = atom_source, action = NOTIFY_JUMP, header = "Polytechnical Difficulties", notify_flags = (NOTIFY_CATEGORY_DEFAULT | GHOST_NOTIFY_MAKE_ANNOUNCEMENT))

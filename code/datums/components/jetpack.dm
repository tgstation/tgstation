 ...
 */
/datum/component/jetpack/Initialize(stabilize, activation_signal, deactivation_signal, return_flag, datum/callback/check_on_move, datum/callback/check_on_activation, datum/effect_system/trail_follow/effect_type)
	. = ..()
	if(!isatom(parent))
...
/datum/component/jetpack/InheritComponent(datum/component/component, original, stabilize, activation_signal, deactivation_signal, return_flag, datum/callback/check_on_move, datum/callback/check_on_activation, datum/effect_system/trail_follow/effect_type)
...
/datum/component/jetpack/proc/deactivate(datum/source, mob/user)
	...
	UnregisterSignal(user, list(COMSIG_MOVABLE_MOVED, COMSIG_MOVABLE_PRE_MOVE, COMSIG_MOVABLE_SPACEMOVE, COMSIG_MOVABLE_DRIFT_VISUAL_ATTEMPT, COMSIG_MOVABLE_DRIFT_BLOCK_INPUT))
	user = null

	if(trail)
		QDEL_NULL(trail)

/datum/component/jetpack/proc/should_trigger(mob/user)
...
/datum/component/jetpack/proc/move_react(mob/user)
	SIGNAL_HANDLER
	if (!should_trigger(user))
		return

	if(user.client.intended_direction && check_on_move.Invoke(FALSE))//You use jet when press keys. yes.

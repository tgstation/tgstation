...
/datum/component/tackler/proc/splat(mob/living/carbon/user, atom/hit)
	...
	switch(oopsie)
		if(99 to INFINITY)
			...
			var/obj/item/bodypart/head/hed = user.get_bodypart(BODY_ZONE_HEAD)
			if(hed)
				hed.receive_damage(brute=40, updating_health=TRUE, wound_bonus = 40)
			else
		...
		if(97 to 98)
			...
			var/obj/item/bodypart/head/hed = user.get_bodypart(BODY_ZONE_HEAD)
			if(hed)
				hed.receive_damage(brute=30, updating_health=TRUE, wound_bonus = 25)
			else
				user.adjustBruteLoss(40)
			user.adjustStaminaLoss(30)
...
/datum/component/tackler/proc/checkObstacle(mob/living/carbon/owner)
	...
	owner.adjustStaminaLoss(15 + messes.len * 2)

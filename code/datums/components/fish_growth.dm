...
/datum/component/fish_growth/Initialize(result_type, growth_rate, use_drop_loc)
	. = ..()
	if(!isfish(parent))
...
/datum/component/fish_growth/CheckDupeComponent(result_type, growth_rate, use_drop_loc)
...
/datum/component/fish_growth/proc/finish_growing(obj/item/fish/source)
	...
	if(!QDELETED(source)) //The fish doesn't delete once the result is spawned.
		maturation = 0

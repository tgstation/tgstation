...
/datum/component/electrified_buckle/Initialize(input_requirements, obj/item/input_item, list/overlays_to_add, override_buckle, damage_on_shock = 50, signal_to_register_from_parent, loop_length)
	...
	if(usage_flags & SHOCK_REQUIREMENT_ITEM)
		...
		register_signal(required_object, COMSIG_PARENT_PREQDELETED, .proc/delete_self)
		register_signal(parent, COMSIG_ATOM_TOOL_ACT(TOOL_SCREWDRIVER), .proc/move_required_object_from_contents)

		if(usage_flags & SHOCK_REQUIREMENT_ON_SIGNAL_RECEIVED)
			shock_on_loop = FALSE
			register_signal(required_object, COMSIG_ASSEMBLY_PULSED, .proc/shock_on_demand)
		else if(usage_flags & SHOCK_REQUIREMENT_SIGNAL_RECEIVED_TOGGLE)
			register_signal(required_object, COMSIG_ASSEMBLY_PULSED, .proc/toggle_shock_loop)

	if((usage_flags & SHOCK_REQUIREMENT_PARENT_MOB_ISALIVE) && ismob(parent))
		register_signal(parent, COMSIG_LIVING_DEATH, .proc/delete_self)

	register_signal(parent, COMSIG_MOVABLE_BUCKLE, .proc/on_buckle)
	...
	if(signal_to_register_from_parent)
		register_signal(parent, signal_to_register_from_parent, .proc/shock_on_demand)
...
/datum/component/electrified_buckle/UnregisterFromParent()
	...
	if(parent)
		...
		unregister_signal(parent, list(COMSIG_MOVABLE_BUCKLE, COMSIG_ATOM_TOOL_ACT(TOOL_SCREWDRIVER)))
		if(requested_signal_parent_emits)
			unregister_signal(parent, requested_signal_parent_emits)

	if(required_object)
		unregister_signal(required_object, list(COMSIG_PARENT_PREQDELETED, COMSIG_ASSEMBLY_PULSED))

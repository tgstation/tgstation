...
/datum/map_generator/cave_generator
	...
	/// Stamp size for DBP noise, also known as frequency
...
/datum/map_generator/cave_generator/generate_terrain(list/turfs, area/generate_in)
	...
	for(var/turf/gen_turf as anything in turfs) //Go through all the turfs and generate them
		...
		// The assumption is this will be faster then changeturf, and changeturf isn't needed since by this point
...
/// Returns a biome datum that the turf was initialized with. Would be if it is present on our Z level. We use a consistent shared seed

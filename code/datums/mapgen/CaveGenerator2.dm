/datum/map_generator/cave_generator2
	...
	/// JSON array of prefab configs for lavaland generator
	var/prefabs_json
...
/datum/map_generator/cave_generator2/generate_terrain(list/turfs, area/generate_in)
	...
	string_gen = rustg_lavaland_generator_generate(width = world.maxx, height = world.maxy, prefabs_json = prefabs_json, min_bsp_size = min_bsp_size, max_ratio = max_ratio, padding = padding, room_fill_percent = room_fill_percent, corridor_width = corridor_width, loop_percent = loop_percent, noise_percent = noise_percent, ca_steps = ca_steps, birth_limit = birth_limit, survival_limit = survival_limit)

...
/datum/progressbar/Destroy()
	if(user)
		for(var/pb in user.progressbars[bar_loc])
			...
			progress_bar.bar.pixel_y = ICONSIZE_Y + offset_y + (PROGRESSBAR_HEIGHT * (progress_bar.listindex - 1))
			var/dist_to_travel = ICONSIZE_Y + offset_y + (PROGRESSBAR_HEIGHT * (progress_bar.listindex - 1)) - PROGRESSBAR_HEIGHT
...
/datum/progressbar/proc/add_prog_bar_image_to_client()
	...
	bar.alpha = 0
	user_client.images += bar
	animate(bar, pixel_y = ICONSIZE_Y + offset_y + (PROGRESSBAR_HEIGHT * (listindex - 1)), alpha = 255, time = PROGRESSBAR_ANIMATION_TIME, easing = SINE_EASING)

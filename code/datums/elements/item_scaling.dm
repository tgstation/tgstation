...
/datum/element/item_scaling/Attach(atom/target, overworld_scaling, storage_scaling)
	...
	register_signal(target, list(COMSIG_ITEM_DROPPED, COMSIG_STORAGE_EXITED), .proc/scale_overworld)
	// Object scaled when placed in an inventory slot OR when entering a storage component.
	register_signal(target, list(COMSIG_ITEM_EQUIPPED, COMSIG_STORAGE_ENTERED), .proc/scale_storage)

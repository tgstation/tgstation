...
/datum/element/item_scaling/Attach(atom/target, overworld_scaling, storage_scaling)
	...
	RegisterSignals(target, list(COMSIG_ITEM_DROPPED, COMSIG_ATOM_EXITED), .proc/scale_overworld)
	// Object scaled when placed in an inventory slot OR when entering a storage component.
	RegisterSignals(target, list(COMSIG_ITEM_EQUIPPED, COMSIG_ATOM_ENTERED), .proc/scale_storage)

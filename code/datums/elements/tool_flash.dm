...
/datum/element/tool_flash/Attach(datum/target, flash_strength)
	...
	register_signal(target, COMSIG_TOOL_IN_USE, .proc/prob_flash)
	register_signal(target, COMSIG_TOOL_START_USE, .proc/flash)

/datum/element/tool_flash/Detach(datum/source)
	. = ..()
	unregister_signal(source, list(COMSIG_TOOL_IN_USE, COMSIG_TOOL_START_USE))

...
/datum/element/mech_prying/Attach(datum/target, time_to_pry)
	...
	RegisterSignal(target, COMSIG_HOSTILE_PRE_ATTACKINGLIVING, PROC_REF(try_pry_mech))

/datum/element/mech_prying/Detach(datum/source)
	UnregisterSignal(source, COMSIG_HOSTILE_PRE_ATTACKINGLIVING)
	return ..()
...
/datum/element/mech_prying/proc/do_pry_mech(mob/living/basic/source, obj/vehicle/sealed/mecha/target_mech)
	...
	if(!do_after(source, actual_pry_time, target_mech, extra_checks = CALLBACK(src, PROC_REF(extra_checks), target_mech, mech_dir)))
...
/datum/element/mech_prying/proc/extra_checks(obj/vehicle/sealed/mecha/mech, mech_dir)

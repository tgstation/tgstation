...
/datum/element/basic_eating/proc/finish_eating(mob/living/eater, atom/target)
	...
	if(drinking)
		playsound(eater.loc, 'sound/items/drink.ogg').volume(rand(10,50)).vary(TRUE).play()
	else
		playsound(eater.loc, 'sound/items/eatfood.ogg').volume(rand(10,50)).vary(TRUE).play()

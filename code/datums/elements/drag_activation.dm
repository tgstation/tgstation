/**
 * drag_activate element
 *
 * Allowing things to be activated by mouse dragging.
 * Useful for objects which have a TGUI window on interaction.
 */
/datum/element/drag_activate

/datum/element/drag_activate/Attach(datum/target)
	if(!ismovable(target))
		return ELEMENT_INCOMPATIBLE
	RegisterSignal(target, COMSIG_MOUSEDROP_ONTO, PROC_REF(activate))
	return ..()

/datum/element/drag_activate/Detach(datum/source)
	UnregisterSignal(source, COMSIG_MOUSEDROP_ONTO)
	return ..()

/datum/element/drag_activate/proc/activate(atom/movable/source, atom/over, mob/user)
	SIGNAL_HANDLER

...
/datum/element/swabable
	...
	id_arg_index = 2
	///Weighted list of /datum/micro_organism || spawn weight
	var/list/micro_organism_weightlist
	///Amount of cell lines on a single sample
	var/cell_line_amount
	///Amount of viruses on a single sample
	var/virus_chance

/datum/element/swabable/Attach(datum/target, list/micro_organism_weightlist = list(), var/cell_line_amount, var/virus_chance)
	if(!isatom(target) || isarea(target))
		return ELEMENT_INCOMPATIBLE

	RegisterSignal(target, COMSIG_ITEM_PRE_ATTACK, .proc/UseOnObj)

	src.micro_organism_weightlist = micro_organism_weightlist

...
/datum/element/kneejerk/proc/tap_knee(obj/item/item, mob/living/target, mob/living/user)
	...
	if(target_brain_damage < BRAIN_DAMAGE_MILD) //a healthy brain produces a normal reaction
		PLAYSOUND(target, 'sound/items/weapons/punchmiss.ogg').volume(25).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
		...
		PLAYSOUND(target, 'sound/items/weapons/punchmiss.ogg').volume(15).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()
		...
		PLAYSOUND(target, 'sound/items/weapons/punchmiss.ogg').volume(5).vary_frequency(TRUE).range(-1 + SOUND_RANGE).play()

...
/datum/element/kneejerk/proc/tap_knee(obj/item/item, mob/living/target, mob/living/user)
	...
	if(target_brain_damage < BRAIN_DAMAGE_MILD) //a healthy brain produces a normal reaction
		playsound(target, 'sound/items/weapons/punchmiss.ogg').volume(25).vary(TRUE).extra_range(-1).play()
		...
		playsound(target, 'sound/items/weapons/punchmiss.ogg').volume(15).vary(TRUE).extra_range(-1).play()
		...
		playsound(target, 'sound/items/weapons/punchmiss.ogg').volume(5).vary(TRUE).extra_range(-1).play()

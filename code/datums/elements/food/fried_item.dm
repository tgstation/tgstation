...
/datum/element/fried_item/Attach(datum/target, fry_time)
	...
	switch(fry_time)
		if(0 to 15 SECONDS)
			this_food.add_atom_colour(fried_colors[1], FIXED_COLOUR_PRIORITY, color_type = ATOM_COLOR_TYPE_FILTER)
		...
		if(15 SECONDS to 50 SECONDS)
			this_food.add_atom_colour(fried_colors[2], FIXED_COLOUR_PRIORITY, color_type = ATOM_COLOR_TYPE_FILTER)
		...
		if(50 SECONDS to 85 SECONDS)
			this_food.add_atom_colour(fried_colors[3], FIXED_COLOUR_PRIORITY, color_type = ATOM_COLOR_TYPE_FILTER)
			this_food.name = "deep-fried [this_food.name]"
			this_food.desc += " Deep-fried to perfection."

		if(85 SECONDS to INFINITY)
			this_food.add_atom_colour(fried_colors[4], FIXED_COLOUR_PRIORITY, color_type = ATOM_COLOR_TYPE_FILTER)

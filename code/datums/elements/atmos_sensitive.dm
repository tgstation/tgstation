...
/datum/element/atmos_sensitive/Attach(datum/target, mapload)
	...
	to_track.add_element(/datum/element/connect_loc, pass_on)
	register_signal(to_track, COMSIG_MOVABLE_MOVED, .proc/react_to_move)
...
/datum/element/atmos_sensitive/Detach(datum/source)
	var/atom/us = source
	us.remove_element(/datum/element/connect_loc, pass_on)

				...
				"__c" = list(\
					list(\
						...
						"__c" = list(\
							...
							list(\
								"__t" = /datum/bt_node/decorator/bb_key_set,\
								"__c" = list(\
									list("__t" = /datum/bt_node/ai_behavior/move_to_target, "default_behavior_args" = list(BB_FOUND_MOM, 1, TRUE))\
								),\
								"observer_abort" = BT_ABORT_BOTH,\
								"key" = BB_FOUND_MOM\
							),\
					...
					list(\
						...
						"__c" = list(\
							...
							list("__t" = /datum/bt_node/ai_behavior/find_parent, "default_behavior_args" = list(BB_FIND_MOM_TYPES, BB_FOUND_MOM))\
						)\
					)\
				)\
			)\

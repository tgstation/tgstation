...
/datum/action/item_action/nanojump
	...
	desc = "Activates the Nanosuit's super jumping ability to allows the user to cross 3 wide gaps."
...
/obj/item/clothing/shoes/combat/coldres/nanojump
	...
	resistance_flags = INDESTRUCTIBLE | FIRE_PROOF | ACID_PROOF | FREEZE_PROOF
	var/jumpdistance = 4 //-1 from to see the actual distance, e.g 4 goes over 3 tiles
	var/jumpspeed = 2
...
/obj/item/clothing/shoes/combat/coldres/nanojump/ui_action_click(mob/user, action)
	...
	if(ishuman(user))
		...
		if(istype(H.wear_suit, /obj/item/clothing/suit/space/hardsuit/nano))
			...
			if(NS.cell.charge > 30 && NS.mode == "strength")
				NS.cell.use(30)
			else
				to_chat(user, "<span class='warning'>Not enough charge!</span>")
				return

	if(!istype(user.mind.martial_art, /datum/martial_art/nano))
	...
	if(user.throw_at(target, jumpdistance, jumpspeed, spin = FALSE, diagonals_first = TRUE, callback = CALLBACK(src, .proc/jump_end)))
		jumping = TRUE
		playsound(src, 'sound/effects/stealthoff.ogg', 50, 1, 1)
		user.visible_message("<span class='warning'>[usr] jumps forward into the air!</span>")
	else
		to_chat(user, "<span class='warning'>Something prevents you from dashing forward!</span>")
		jumping = FALSE //just incase

/obj/item/clothing/shoes/combat/coldres/nanojump/proc/jump_end()
	jumping = FALSE
...
/mob/living/carbon/human/throw_item(atom/target)
	..()
	if(attacker_style && attacker_style.thrown_act(target))
		return TRUE
*/
...
/datum/martial_art/nano/harm_act(var/mob/living/carbon/human/A, var/mob/living/carbon/D)
	...
	/*var/obj/item/I = null
	if(D != A)
		I = A.get_active_held_item()
		if(I && I.force > 2 && I.damtype == BRUTE)
			D.apply_damage(I.force + 5, BRUTE)*/
	...
	if(!D.stat || !D.IsKnockdown() && D != A) //and we can't knock ourselves the fuck out/down!
		if(A.grab_state == GRAB_AGGRESSIVE)

...
Adds an entries column to achievements for the score/progress subtype that derivates their value from it.

```sql
ALTER TABLE `achievements` ADD COLUMN `entries` TEXT(8192) NULL AFTER `value`;
```
-----------------------------------------------------

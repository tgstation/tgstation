...
export const SDQLSpellMenu = (_, context) => {
 ...
 */
const SDQLSpellOptions = (_, context) => {
	const { data } = useBackend(context);
...
const SDQLSpellListEntry = (props, context) => {
	...
	const { list, default_value } = props;
	return (
		<Collapsible>
...
const SDQLSpellIcons = (_, context) => {
	const { data } = useBackend(context);
	const {
		saved_vars,
		type,
...
const SDQLSpellParseErrorModal = (_, context) => {
	const { act, data } = useBackend(context);

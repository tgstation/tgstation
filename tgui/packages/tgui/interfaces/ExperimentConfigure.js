  ...
  return (
    <Fragment>
      <Section title="Servers">
        <Box>
          {servers.length > 0 ? "Please select a server to connect to..." : "Found no powered servers"}
        </Box>
        {servers.map(s => {
          return (
            <Box m={1} className="ExperimentConfigure__Server">
              <Flex align="center" justify="space-between" className="ExperimentConfigure__ServerHeader">
                <Flex.Item className="ExperimentConfigure__ServerName">
                  {s.name} / {s.web_org}
                </Flex.Item>
                <Flex.Item>
                  <Button
                    onClick={() => s.selected ? act("clear_server") : act("select_server", { "ref": s.ref })}
      ...
      {servers.some(e => e.selected) && (
        <Section title="Experiments">
          <Box>
            {experiments.length ? "Select one of the following experiments..." : "No experiments found on this server"}
          </Box>
          {experiments.map(exp => {
            return (
              <Box m={1} className="ExperimentConfigure__ExperimentPanel">
                <Button
                  fluid={true}
                  onClick={() => exp.selected ? act("clear_experiment") : act("select_experiment", { "ref": exp.ref })}
                  content={exp.name}

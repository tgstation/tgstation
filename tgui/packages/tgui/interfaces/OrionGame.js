...
const AdventureStatus = (_, context) => {
...
const ORION_STATUS_START = (_, context) => {
	const { data, act } = useBackend(context);
	const { gamename } = data;
...
const ORION_STATUS_INSTRUCTIONS = (_, context) => {
...
const ORION_STATUS_NORMAL = (_, context) => {
	const { data, act } = useBackend(context);
	const { turns, eventname, eventtext, buttons } = data;
...
const ORION_STATUS_GAMEOVER = (_, context) => {
	const { data, act } = useBackend(context);
	const { reason } = data;
...
const ORION_STATUS_MARKET = (_, context) => {
	const { data, act } = useBackend(context);
...
export const OrionGame = (_, context) => {

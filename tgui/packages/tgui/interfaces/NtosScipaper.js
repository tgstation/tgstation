...
const PaperPublishing = (_, context) => {
					...
					<LabeledList.Item label="Selected File">
						<Stack>
							...
							<Stack.Item align="center">
								<Tooltip
									position="left"
									content={multiline`The selected file containing experimental
                    data for our paper. Must be present in the HDD to be
                    accesible. Transfer files with the File Manager program.`}>
					...
					<LabeledList.Item label="Selected Experiment">
						<Stack>
							...
							<Stack.Item align="center">
								<Tooltip
									position="left"
									content={multiline`The topic we want to publish our paper on.
                    Different topics unlock different technologies
					...
					<LabeledList.Item label="Selected Tier">
						<Stack>
							...
							<Stack.Item align="center">
								<Tooltip
									position="left"
									content={multiline`The tier we want to publish on. Higher
                    tiers can confer better rewards but means our data will
					...
					<LabeledList.Item label="Selected Partner">
						<Stack>
							...
							<Stack.Item align="center">
								<Tooltip
									position="left"
									content={multiline`Which organization to partner with.
                    We can obtain research boosts in techs related to the
                    partner's interests.`}>
									<Icon size={1.15} name="info-circle" />
								</Tooltip>
							</Stack.Item>
			...
			<Section title="Expected Results" key="rewards">
				<Stack fill>
					<Stack.Item grow>
						<Tooltip
							position="top"
							content={multiline`How much will our relation improve with
                the particular partner. Cooperation will be used to
                unlock boosts.`}>
							<Icon size={1.15} name="info-circle" />
						</Tooltip>
						{' Cooperation: '}
					...
					<Stack.Item grow>
						<Tooltip
							position="top"
							content={multiline`How much grant will we be endowed with
...
const ExperimentBrowser = (_, context) => {
	const { data } = useBackend(context);
...
const PartnersBrowser = (_, context) => {
	const { act, data } = useBackend(context);
	const {
		partnersInformation,
		coopIndex,
...
export const NtosScipaperContent = (_, context) => {
	const { act, data } = useBackend(context);

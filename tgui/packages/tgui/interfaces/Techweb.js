...
export const Techweb = (_, context) => {
...
export const AppTechweb = (_, context) => {
	const { act, data } = useRemappedBackend(context);
	const { locked } = data;
...
const TechwebOverview = (_, context) => {
	const { data } = useRemappedBackend(context);
...
const TechwebDesignDisk = (_, context) => {
	const { act, data } = useRemappedBackend(context);
		...
		<>
			...
			{blueprints.map((x, i) => (
				<Section
					...
					{(x === null && 'Empty') || (
						<>
							Contains the design for <b>{design_cache[x].name}</b>:
							<br />
							<span
...
const TechwebTechDisk = (_, context) => {
	const { data } = useRemappedBackend(context);
	const { t_disk } = data;

...
const planeToPosition = function (plane : Plane, index, is_incoming)
  : Position {
  return {
    x: is_incoming ? plane.x : plane.x + plane.size_x,
    y: 29 + plane.y + (plane.step_size * index)
...
const getPlaneNodeHeight = function (plane : Plane) : number {
  return Math.max(plane.incoming_relays.length + plane.incoming_filters.length,
...
export class PlaneMasterDebug extends Component {
  ...
  handlePortClick(connection: Connected, isOutput, event) {
    ...
    if (connection.connect_type === ConnectionType.Relay) {
      ...
      source_plane.outgoing_relays
        = arrayRemove(source_plane.outgoing_relays, connection.our_ref);
      target_plane.incoming_relays
        = arrayRemove(target_plane.incoming_relays, connection.our_ref);
    }
      ...
      source_plane.outgoing_filters
        = arrayRemove(source_plane.outgoing_filters, connection.our_ref);
      target_plane.incoming_filters
        = arrayRemove(target_plane.incoming_filters, connection.our_ref);
    }
  }
  ...
  render() {
    ...
    const [showAdd, setShowAdd] = useLocalState(
      this.context, "showAdd", false);

    const [connectSources, setConnectSouces] = useLocalState<AssocConnected>(
      this.context, "connectionSources", {});
...
class PlaneMaster extends Component<PlaneMasterProps> {
  shouldComponentUpdate(nextProps, nextState) {
    const {
      ...
      outgoing_connections } = this.props as PlaneMasterProps;
  ...
  render() {
    ...
    const [currentPlane, setCurrentPlane] = useLocalState(
      this.context, "currentPlane", {});
    const [readPlane, setReadPlane] = useLocalState(
      this.context, "readPlane", "");
        ...
        <Stack.Item>
          ...
          >
            ...
            >
              ...
              />
              <circle ref={this.iconRef} cx="50" cy="50" r="50" className={`color-fill-${connection.connect_color}`} />
            </svg>
...
const DrawAbovePlane = (props, context) => {
  ...
  const [showInfo, setShowInfo] = useLocalState(
    ...
    <Section
      ...
      <Section title="Visuals">
        ...
        <Slider
          ...
          step={1}
          stepPixelSize={1.9}
          onDrag={(e, value) => act('set_alpha', {
            edit: workingPlane.our_ref,
            alpha: value,
          })}
          onChange={(e, value) => act('set_alpha', {
            edit: workingPlane.our_ref,
            alpha: value,
          })} >
...
const ClosePlaneWindow = (props, context) => {
  const [readPlane, setReadPlane] = useLocalState(
    context, "readPlane", "");
  return (
    <Button icon="times" onClick={() => setReadPlane("")} />
  );
};
...
const AddModal = (props, context) => {
  ...
  const [currentPlane, setCurrentPlane] = useLocalState<Plane>(
    context, "currentPlane", {} as Plane);
  const [currentTarget, setCurrentTarget] = useLocalState<Plane>(
    context, "currentTarget", {} as Plane);
...
const InfoModal = (props, context) => {
  ...
  return (
    <Modal
      ...
      left="180px"
      bottom="100px">
      <Section fill scrollable title="Information Panel"
        buttons={
          <Button
            icon="times"
            tooltip="Close"
        ...
        This UI exists to help visualize plane masters,
        the backbone of our rendering system. <br />
        ...
        info is quite obscure, and I want it to be easier to understand.<br />
        <br />

        That said, it also supports editing plane masters, adding and
        removing relays, and provides easy access to
        color matrix/filter/alpha/vv editing. <br />
        <br />

        To start off with, each little circle
        represents a <code>render_target</code> based connection.<br />
        Blue nodes are relays, so drawing one plane onto another.
        Purple ones are filter based connections. <br />
        ...
        Each plane can be viewed more closely by clicking the little button
        in it&apos;s top right corner. This opens a sidebar, and displays
        a lot of more general info about the plane and its purpose, alongside
        exposing some useful buttons and interesting values. <br />
        <br />
        ...
        can hit the recycle button in the top left to totally refresh your
        plane masters. <br />
        <br />

        <h3>What is a plane master?</h3>
        You can think of a plane master as a way to group a set of objects
        onto one rendering slate. <br />
        It is per client too, which makes it quite powerful.
        This is done using the <code>plane</code> variable
        of <code>/atom</code>. <br />
        <br />

        We first create an atom with an appearance flag that
        contains <code>PLANE_MASTER</code> and give
        it a <code>plane</code> value. <br />
        Then we mirror the same <code>plane</code> value on all
        the atoms we want to render in this group.<br />
        <br />

        Finally, we place the <code>PLANE_MASTER</code>&apos;d atom in
        the relevent client&apos;s screen contents. <br />
        That sets up the bare minimum.<br />
        <br />
        ...
        with planes. <br />
        Masking one thing while also drawing that thing
        in the correct order with other objects on the map is a good
        example of this.<br />
        It <b>is</b> possible to do, but it&apos;s quite disruptive.<br />
        <br />

        Normally, planes will just group, apply an effect, and then draw
        directly to the game.<br />
        What if we wanted to draw <b>planes</b> onto other planes then? <br />
        <br />

        <h3>Render Targets and Relays</h3><br />
        ...
        <b>not</b> be drawn to the actual client view,
        and instead just relayed. <br />
        <br />

        Ok so we can relay a plane master onto some other atom, but how do
        we get it on another plane master? We can&apos;t just draw it
        with <code>render_source</code>, since we might want to relay more
        then one plane master.<br />
        <br />
        ...
        normally to apply some displacement or mask.<br />
        <br />

        <h3>Applying effects</h3> <br />

        Ok so we can group and relay planes, but what can we actually
        do with that? <br />
        <br />

        Lots of stuff it turns out. Filters are quite powerful, and we
        use them quite a bit. <br />
        You can use filters to mask one plane with another, or use one plane
        as a distortion source for another. <br />
        <br />

        Can do more basic stuff too, setting a plane&apos;s color matrix can
        be quite powerful. <br />
        Even just setting alpha to show and
        hide things can be quite useful. <br />
        <br />

        I won&apos;t get into every effect we do here, you can learn more
        about each plane by clicking on the little button in their
        top right. <br />
        <br />
      </Section>
    </Modal>

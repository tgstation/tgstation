...
/mob/living/simple_animal/hostile/floor_cluwne
	name = "???"
	desc = "...."
	icon = 'spacestation413/icons/obj/clothing/masks.dmi'
	icon_state = "cluwne"
	icon_living = "cluwne"
	icon_gib = "clown_gib"
	maxHealth = 250
	...
	loot = list(/obj/item/clothing/mask/spacestation413/cluwne)
	wander = FALSE
	minimum_distance = 2
	move_to_delay = 1
...
/mob/living/simple_animal/hostile/floor_cluwne/Life()
	...
	if(stage == STAGE_ATTACK)
		playsound(src, 'spacestation413/sound/misc/cluwne_breathing.ogg', 75, 1)
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/On_Stage()
	...
	switch(stage)
		...
		if(STAGE_HAUNT)
			...
			if(prob(5))
				H.playsound_local(src,'spacestation413/sound/voice/cluwnelaugh2_reversed.ogg', 1)

			if(prob(5))
				H.playsound_local(src,'spacestation413/sound/misc/bikehorn_creepy.ogg', 5)
		...
		if(STAGE_SPOOK)
			...
			if(prob(5))
				H.playsound_local(src,'spacestation413/sound/voice/cluwnelaugh2.ogg', 2)

			if(prob(5))
				H.playsound_local(src,'spacestation413/sound/voice/cluwnelaugh2_reversed.ogg', 2)

			if(prob(5))
				H.playsound_local(src,'spacestation413/sound/misc/bikehorn_creepy.ogg', 10)
				to_chat(H, "<i>knoh</i>")
		...
		if(STAGE_TORMENT)
			...
			if(prob(3))
				playsound(src,'spacestation413/sound/voice/cluwnelaugh2_reversed.ogg', 30, 1)

			if(prob(5))
				playsound(src,'spacestation413/sound/misc/bikehorn_creepy.ogg', 30, 1)
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/Grab(mob/living/carbon/human/H)
	...
	for(var/I in 1 to get_dist(src, H))
		if(do_after(src, 5, target = H))
			step_towards(H, src)
			playsound(H, pick('spacestation413/sound/effects/bodyscrape-01.ogg', 'spacestation413/sound/effects/bodyscrape-02.ogg'), 20, 1, -4)
			if(prob(40))
				H.emote("scream")
			else if(prob(25))
				H.say(pick("HELP ME!!","IT'S GOT ME!!","DON'T LET IT TAKE ME!!",";SOMETHING'S KILLING ME!!","HOLY FUCK!!"))
				playsound(src, pick('spacestation413/sound/voice/cluwnelaugh1.ogg', 'spacestation413/sound/voice/cluwnelaugh2.ogg', 'spacestation413/sound/voice/cluwnelaugh3.ogg'), 50, 1)
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/Kill(mob/living/carbon/human/H)
	if(!istype(H) || !H.client)
		Acquire_Victim()
		return
	playsound(H, 'spacestation413/sound/effects/cluwne_feast.ogg', 100, 0, -4)
	...
	for(var/mob/M in GLOB.player_list)
		M.playsound_local(get_turf(M), 'spacestation413/sound/misc/honk_echo_distant.ogg', 50, 1, pressure_affected = FALSE)
...
/obj/effect/temp_visual/fcluwne_manifest
	icon = 'spacestation413/icons/turf/floors.dmi'
...
/obj/effect/temp_visual/fcluwne_manifest/Initialize()
	. = ..()
	playsound(src, 'spacestation413/sound/misc/floor_cluwne_emerge.ogg', 100, 1)

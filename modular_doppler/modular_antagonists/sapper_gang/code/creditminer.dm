...
/obj/item/powersink/creditminer/attack_hand(mob/user, list/modifiers)
	...
	switch(mode)
		if(1) //On turning off
			playsound(src, 'modular_doppler/modular_antagonists/sapper_gang/sounds/creditminer_stop.wav', 50, FALSE)

		if(2) //On turning on
			playsound(src, 'modular_doppler/modular_antagonists/sapper_gang/sounds/creditminer_start.wav', 50, FALSE)
...
/obj/item/powersink/creditminer/drain_power()
	...
	if(cash_pulse >= 1)
		...
		playsound(src, 'modular_doppler/modular_antagonists/sapper_gang/sounds/creditminer_drain.wav', 50, FALSE)
...
/obj/item/powersink/creditminer/release_heat()
	...
	if(mode < 2) //sfx if we release heat, but don't overlap the drain sfx
		playsound(src, 'modular_doppler/modular_antagonists/sapper_gang/sounds/creditminer_vent.wav', 50, FALSE)

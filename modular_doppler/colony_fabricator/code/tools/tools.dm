...
/obj/item/weldingtool/arc_welder
	...
	usesound = 'modular_doppler/colony_fabricator/sounds/arc_welder/arc_welder.ogg'
	light_range = 2
	light_power = 0.75
	toolspeed = 1
	change_icons = FALSE
...
/obj/item/weldingtool/arc_welder/Initialize(mapload)
	. = ..()
	AddElement(/datum/element/manufacturer_examine, COMPANY_FRONTIER)
	AddComponent(/datum/component/cell, cell_override, CALLBACK(src, PROC_REF(switched_off)))

/obj/item/weldingtool/arc_welder/attack_self(mob/user, modifiers)
	. = ..()
	if(!powered)
...
/obj/item/weldingtool/arc_welder/switched_on(mob/user)
	welding = TRUE
...
/obj/item/weldingtool/arc_welder/switched_off(mob/user)
	powered = FALSE
	welding = FALSE
	light_on = FALSE
	force = initial(force)
...
/obj/item/weldingtool/arc_welder/process(seconds_per_tick)
	if(!powered)
		switched_off()
		return
...
/obj/item/weldingtool/arc_welder/examine(mob/user)
	. = ..()
	. -= "It contains [get_fuel()] unit\s of fuel out of [max_fuel]."

// This is what uses fuel in the parent. We override it here to not use fuel
/obj/item/weldingtool/arc_welder/use(used = 0)
	return isOn()

/obj/item/weldingtool/arc_welder/examine()
	. = ..()
	. += "[src] is currently [powered ? "powered" : "unpowered"]."

/obj/item/weldingtool/arc_welder/update_icon_state()
	if(powered)

...
/obj/item/clothing/head/helmet/space/plasmaman/worn_overlays(mutable_appearance/standing, isinhands)
	. = ..()
	if(!isinhands && smile)
		var/mutable_appearance/M = mutable_appearance('monkestation/icons/mob/clothing/plasmaman_head.dmi', smile_state)
		M.color = smile_color
		. += M
	...
	if(!isinhands && !up)
		. += mutable_appearance('monkestation/icons/mob/clothing/plasmaman_head.dmi', visor_icon)
	else
		cut_overlays()

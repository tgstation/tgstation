...
/proc/add_jobxp(client/target, amount, job)
...
/proc/remove_jobxp(client/target, amount, job)
	if(!target || !target.prefs || !amount || !job)
		return
	target.prefs.job_xp_list[job] -= amount

//this proc handles adding xp to you but on a chance basis useful for super spamable stuff
/proc/add_jobxp_chance(client/target, amount, job, chance)
	if(!target || !target.prefs || !amount || !job)
		return
	if(prob(chance))
		target.prefs.job_xp_list[job] += amount
...
/proc/add_jobxp_chance_delayed_check(client/target, amount, job, chance)
	if(!target || !target.prefs || !amount || !job)
		return
	if(prob(chance))
		target.prefs.job_xp_list[job] += amount

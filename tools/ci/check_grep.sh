...
if grep -P '/obj/structure/cable(/\w+)+\{' _maps/**/*.dmm;	then
	echo
    echo -e "${RED}ERROR: vareditted cables detected, please remove them.${NC}"
    st=1
fi;
...
if grep -Pzo '"\w+" = \(\n[^)]*?/obj/structure/lattice[/\w]*?,\n[^)]*?/turf/closed/wall[/\w]*?,\n[^)]*?/area/.+?\)' _maps/**/*.dmm;	then
	echo
    echo -e "${RED}ERROR: found lattice stacked with a wall, please remove them.${NC}"
    st=1
fi;
if grep -Pzo '"\w+" = \(\n[^)]*?/obj/structure/lattice[/\w]*?,\n[^)]*?/turf/closed[/\w]*?,\n[^)]*?/area/.+?\)' _maps/**/*.dmm;	then
	echo
    echo -e "${RED}ERROR: found lattice stacked within a wall, please remove them.${NC}"
    st=1
fi;
...
if grep -Pzo '"\w+" = \(\n[^)]*?/obj/structure/stairs[/\w]*?,\n[^)]*?/turf/open/genturf[/\w]*?,\n[^)]*?/area/.+?\)' _maps/**/*.dmm;	then
	echo
    echo -e "${RED}ERROR: found a staircase on top of a genturf. Please replace the genturf with a proper tile.${NC}"
    st=1
fi;
...
if grep 'balloon_alert\(".+"\)' code/**/*.dm; then
	echo
	echo -e "${RED}ERROR: Balloon alert with improper arguments.${NC}"
	st=1
fi;

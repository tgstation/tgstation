			...
			try
			{
				...
				switch (command.ToLowerInvariant())
				{
					case "init":
						...
						var prsNeedingReplay = new List<(int PRNumber, string Sha)>();
						var succeeded = false;
...
The following {prsNeedingReplay.Count} PRs need to be replayed (DO NOT EDIT THIS LIST MANUALLY, IT IS USED BY THE BOT TO TRACK PROGRESS):

  - [ ] {String.Join($"{Environment.NewLine}  - [ ] ", prsNeedingReplay.Select(x => $"#{x.PRNumber} - Reverted in {x.Sha}"))}
...
After that some startup commits on this branch need to be reverted then it can be merged.";
		...
		{
			var walleningBody = walleningPR.Body;
			if (closedPRNumber.HasValue)
			{
				...
				var updatedPR = await client.PullRequest.Update("tgstation", "tgstation", walleningPR.Number, new PullRequestUpdate
				{
					Body = newBody,
				});

				walleningBody = updatedPR.Body;

				if (!previousPR.Merged)
				{
					await client.Issue.Labels.AddToIssue("tgstation", "tgstation", previousOriginalPR, new string[] { "Lost to Wallening Revert" });
				}
			}

			var matches = Regex.Matches(walleningBody, @"  - \[ \] #([1-9][0-9]+) - Reverted in ([0-9a-f]+)");

...
defines_directory = parent_directory + "/__DEFINES"
helpers_directory = parent_directory + "/__HELPERS"
...
for applicable_file in filtered_files:
    ...
    with open(applicable_file, "r") as file:
        file_contents = file.read()
        for define in define_regex.findall(file_contents):
            define_name = define.group(1)
            if not re.search("#undef\s" + define, file_contents):

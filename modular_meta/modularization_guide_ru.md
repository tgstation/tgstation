...
- **`_\defines/ и \_globalvars/ и \_helpers/`** (D/G/H)  📂

  Здесь лежат все наши модульные "определения" (они же defines), "гробальные пеменные" (globalvars) и "помощники" (helpers).
...
В папке модуля есть файл `modular_meta/__config_modpaks.dm`. В нем перечисленны все модули, что можно отключить в игре. Для отключения требуудется перекомпиляция проекта, т.к. эти изменения вступают в силу только на этапе работы [препроцессора](https://ru.wikipedia.org/wiki/%D0%9F%D1%80%D0%B5%D0%BF%D1%80%D0%BE%D1%86%D0%B5%D1%81%D1%81%D0%BE%D1%80).
